...
!
!   Integer variables
!   nstation:       number of stations being processed
!                   always at least one station (primary site) or can
!                   be two stations (secondary station)
...
!   updates1:        2x2x3 array of start month, day, year and end month,
!                   day and year.  1st dimension is primary station (1) or secondary station (2)
!                   2nd dim is the start (1) or end (2)
!                   3rd dim is year (1), month (2), and day (3)
!                   
...
!   upstarts:       start date by station (1=primary, 2=secondary)
!   upends:         end date by station (1=primary, 2=secondary)
!
!   Real variables
...
!  10:              denotes that DATA2 keyword (ikey=42) found for upper air data.
!  11:              denotes NO_MISSING2 keyword found (ikey=43)
!  12:              denotes EXTRACT2 (ikey=44) keyword found. 
!  13:              denotes that QAOUT2 (ikey=45) keyword found.
!  14:              denotes LOCATION2 keyword found (ikey=46)
!  15:              denotes MODIFY2 keyword found (ikey=47)
!  16:              denotes RANGE2 keyword found (ikey=48)
!  17:              denotes AUDIT2 keyword found (ikey=49)
...
!   lsecondary:     denotes a secondary site is being processed
!
!   Character variables
...
!   station_type:       station type (PRIMARY or SECONDARY)
!
...
!   JAT 1/28/2025 A058 (A058_SOUND); new variable nstation, number of stations being processed; minimum value is 1 and always
!   at least 1 station
    integer(kind=4) :: nstation=1
! JAT   4/25/23 A040_GMT_INIT; initialize upgmt2lst to -99
!    integer(kind=4) :: upgmt2lst=-9
!   JAT 1/28/2025 A058 (A058_SOUND); make upgmt2lst,iupform, nupdays,nsound,nuplevels, nsnd, nup_audit_vars
!   nup_audit_vars, writelevs, wind_stats, tempstat,nsnd_5km,no_sound_days, n_am_days, 2-D arrays (1=primary station, 2=secondary station)
!   make updates a 3-D array
!   also comment out nup_calc_audit as it is only used locally in up_proc
    !integer(kind=4) :: upgmt2lst=-99
    !integer(kind=4) :: iupform=1
    integer(kind=4) :: updates(2,3)=0
!   JAT 6/12/2025 A058 (A058_SOUND); add updates1 for primary and secondary stations. same as updates except for 2 stations
    integer(kind=4) :: updates1(2,2,3)=0
    ...
    integer(kind=4) :: nup_audit_vars(2)=0
    ...
    integer(kind=4) :: upgmt2lst(2)=-99
    integer(kind=4) :: iupform(2)=1
    integer(kind=4) :: nsound(2)=0
    integer(kind=4) :: nuplevels(2)=0
    integer(kind=4) :: nsnd(6,2)=0
    integer(kind=4) :: wind_stats(2,2)=0
    integer(kind=4) :: tempstat(2)=0
    integer(kind=4) :: nsnd_5k(2)=0
    integer(kind=4) :: no_sound_days(2)=0
    integer(kind=4) :: no_am_days(2)=0
!   JAT 6/12/2025 A058 (A058_SOUND); add upstarts and upends for start/end dates by station
    integer(kind=8) :: upstarts(2)=0
    integer(kind=8) :: upends(2)=0
    integer(kind=4), parameter :: nwarnlim=100
   
    logical :: ltop=.false.
    logical :: lskip=.false.
!   JAT 1/28/2025 A058 (A058_SOUND); make have_soundings 2-D array (1=primary station, 2=secondary station)
!    logical :: have_soundings=.false.
    logical :: have_soundings(2)=.false.
...
!   JAT 1/28/2025 A058 (A058_SOUND); add to nupkeys to add DATA2, XDATES2, LOCATION2, QAOUT2, EXTRACT2, RANGE2, AUDIT2, MODIFY2, NO_MISSING2
    !integer(kind=4), parameter :: nupkeys=9
    integer(kind=4), parameter :: nupkeys=17
    integer(kind=4), parameter :: nmods=3
    integer(kind=4), parameter :: up_inc=500
     
!   JAT 1/28/2025 A058 (A058_SOUND); add another dimension to arrays to account for primary and secondary stations 
...
!   JAT 1/28/2025 A058 (A058_SOUND); make these variables 2-D arrays (1=primary station, 2=secondary station) 
    !real(kind=r8) :: uplat=0.0_r8
    !real(kind=r8) :: uplon=0.0_r8
    !real(kind=r8) :: upelev=0.0_r8
    real(kind=r8) :: uplat(2)=0.0_r8
    real(kind=r8) :: uplon(2)=0.0_r8
    real(kind=r8) :: upelev(2)=0.0_r8
...
!   JAT 1/28/2025 A058 (A058_SOUND); add another dimension to arrays to account for primary and secondary stations 
    ...
    logical :: writeext(2)=.false.
    logical :: lmodify(nmods,2)=.false.
    logical :: lgmt2lst(2)=.false.
    logical :: lelev(2)=.false.
    logical :: upstage1(2)=.false.
!   JAT 1/28/2025 A058 (A058_SOUND); new variable, lsecondary, denoting a secondary site is being included
    logical :: lsecondary=.false.
  
!   JAT 1/28/2025 A058 (A058_SOUND); add another dimension to arrays to account for primary and secondary stations to upid, iupid, qaout_vars
    !character(len=8) :: upid=''
    character(len=8) :: iupid='0'
    character(len=8) :: upid(2)=''
    character(len=60) :: ext_form(14)
    character(len=55) :: ext_errstr(5),ext_endstr(4)
    character(len=4), allocatable, dimension(:) :: qaout_vars
!   JAT 1/30/2025 A058 (A058_SOUND); add variable station_type secondary
    character(len=9) :: station_type(2)
    
!   JAT 1/28/2025 A058 (A058_SOUND); make variables 2-D array to account for primary and secondary stations
 !   type uavars
 ...
 !       real(kind=r8) :: conv
    ...
    type uavars
        character(len=4) varname
        logical laudit(2),lmod(2),lnomiss(2),lincbound(2)
        integer(kind=4) :: missval(2),lowbound(2),upbound(2)
        real(kind=r8) :: conv(2)
    end type uavars
      
    type(uavars) :: upvars(nupvars)
      
!   JAT 1/28/2025 A058 (A058_SOUND); make each variable a 2-element array to account for primary and secondary sites
    type uformats
    ...
    !type uformats
 !       character(len=10) :: uformat(2)
 !       logical lupform(2)
 !       integer(kind=4) :: maxblock(2),rdsz(2),rdpos(2),offset(2)
 !   end type uformats
      
    type(uformats) :: upformats(nupformats)
    
!   JAT 1/28/2025 A058 (A058_SOUND); add another dimension to sound_info to account for primary and secondary stations
    type sdinfo
...
!   JAT 1/30/2025 A058 (A058_SOUND); define station_type as primary (1) and secondary (2)
    data station_type /'PRIMARY','SECONDARY'/
    contains 
!*********************************************************************************************************
...
!   istation:       station counter (1=primary upper air station; 2= secondary upper air station)
!
!   Logical variables
...
!   JAT 1/29/2025 A058 (A058_SOUND); new variable istation, denoting if primary (1) or secondary station(2) 
!    integer(kind=4):: i,i1,j,jj  
    integer(kind=4):: i,i1,j,jj,istation
    logical :: lvalid,lgood
    character(len=6) :: form1
    character(len=10) :: fileform
!JAT 10/16/2024 AO68_SOUND; increase formstr array from 4 to 5 for warning message regarding missing station elevation
!    character(len=50) :: formstr(4)
    character(len=50) :: formstr(5)
...
!   5. missing station elevation; JAT 10/16/2024 A068_SOUND
...
!   JAT 1/29/2025 A058 (A058_SOUND);  add logic for new keywords (XDATES2, AUDIT2, etc.) associated with
!   secondary site
!   add logic below for those keywords as well
!   also set nstation=2 and lsecondary to true, if false and one of the relavant keywords found
!   also set value for istation based on ikey (ikey >= 42) is secondary
        !if ((ikey == 4 .or. ikey== 5) .or. (ikey >= 7 .and. ikey <=13)) then
        if ((ikey == 4 .or. ikey== 5) .or. (ikey >= 7 .and. ikey <=13) .or. ikey >= 42) then
!   JAT 1/29/2025 A058 (A058_SOUND); initial value of istation
            istation=1
            if (ikey <=5) then
                jj=ikey-3
            elseif (ikey >= 42) then !   JAT 1/29/2025 A058 (A058_SOUND);
                jj=ikey-32
!               reset istation to 2 for secondary site
                istation=2
...
!   JAT 1/29/2025 A058 (A058_SOUND); add DATA2 keyword to logic (ikey=42)
        !if (ikey == 4) then !data file
        if (ikey == 4 .or. ikey == 42) then !data file
            !upstage1=.true.
            upstage1(istation)=.true.
...
!               JAT 1/29/2025 A058 (A058_SOUND); get appropriate file for primary or secondary site
                !read(inpline(fileind(1):fileind(2)),'(a)')up_inpfile
                read(inpline(fileind(1):fileind(2)),'(a)')up_inpfile(istation)
!               get the DATA filename
                !call checkfile(up_data_unit,up_inpfile,1,lgood)  
                call checkfile(up_data_unit(istation),up_inpfile(istation),1,lgood)
!               read the uppercase version of the line to get the file format
!               JAT 8/10/22 A013_ASOS_FLAG; read as free format
!                read(inpline1(formind(1):formind(2)),'(a)')fileform
                read(inpline1(formind(1):formind(2)),*)fileform
                    
!               check the format
!               JAT 1/28/2025 A058 (A058_SOUND); check for primary or secondary station
                j=1
                lvalid=.false.
...
!               JAT 1/28/2025 A058 (A058_SOUND); primary or secondary message
                if (.not. lvalid) then
!                   invalid file format
                    ...
                    !    'FOR UPPER AIR FILE',trim(adjustl(up_inpfile))
                        'FOR UPPER AIR FILE',trim(adjustl(up_inpfile(istation)))
                    lbad=.true.
                endif
            endif

!   JAT 1/29/2025 A058 (A058_SOUND); add NO_MISSING2 keyword to logic (ikey=50)
        !else if (ikey == 5) then !no_missing
        else if (ikey == 5 .or. ikey == 43) then !no_missing or no_missing2
!           this keyword is repeatable so duplicate entries allowed
!           JAT 1/29/2025 A058 (A058_SOUND); add istation as argument to upper_list
            !call upper_list(i+i1)
            call upper_list(i+i1,istation)
              
!       JAT 1/29/2025 A058 (A058_SOUND); add EXTRACT2
        !else if (ikey == 7) then !extract
        else if (ikey == 7 .or. ikey == 44) then !extract
            !upstage1=.true.
            upstage1(istation)=.true.
!           if number of fields is not 2, then line is bad
            if (nfield /= 2) then
                ...
                read(inpline(i+i1+1:ilen),form1)up_extfile(istation)
!               get the EXTRACT filename
                !call checkfile(up_extract_unit,up_extfile,4,lgood) 
                call checkfile(up_extract_unit(istation),up_extfile(istation),4,lgood) 
            endif

!       JAT 1/29/2025 A058 (A058_SOUND); add QAOUT2
        !else if (ikey == 8) then !qaout
        else if (ikey == 8 .or. ikey == 45) then !qaout
!           if number of fields is not 2, then line is bad
            if (nfield /= 2) then
                write(writeunit,formstr(2))adjustl(pathid(ipath)),'E03',modnam,nfield,'IS AN INVALID NUMBER OF FIELDS FOR KEYWORD',&
                    trim(adjustl(keywrd(ikey)))
                ...
                read(inpline(i+i1+1:ilen),form1)up_qafile(istation)
!               get the QAOUT filename
                !call checkfile(up_qaout_unit,up_qafile,4,lgood)
                call checkfile(up_qaout_unit(istation),up_qafile(istation),4,lgood)
            endif
...
!               JAT 6/12/2025 A058 (A058_SOUND); get primary station's dates
                !call getdates(i+i1,upstart,upend,updates)
                call getdates(i+i1,upstarts(1),upends(1),updates)
                updates1(1,:,:)=updates
                updates=0
            endif

!       JAT 1/29/2025 A058 (A058_SOUND); add LOCATION2
!       else if (ikey == 10) then !location
        else if (ikey == 10 .or. ikey == 46) then !location
!           incorrect number of fields                  
            if (nfield < 4 .or. nfield > 6) then
                ...
                call getloc(i+i1,upid(istation),uplat(istation),uplon(istation),upgmt2lst(istation),upelev(istation),&
                    lgmt2lst(istation),lelev(istation))
!JAT 10/16/2024; A068_SOUND; issue error that station elevation not entered, may be needed later
!               if (.not. lelev)write(writeunit,formstr(5))adjustl(pathid(ipath)),'W36',modnam,&
                !if (.not. lelev) then
                if (.not. lelev(istation)) then
                    write(writeunit,formstr(5))adjustl(pathid(ipath)),'E05',modnam,&
                    'STATION ELEVATION NOT ENTERED; MAY BE NEEDED FOR UPPERAIR PROCESSING'
                    lbad=.true.
                endif
                
            endif
     
!       JAT 1/30/2025 A058 (A058_SOUND); add RANGE2
!       else if (ikey == 12) then !range
        else if (ikey == 12 .or. ikey == 48) then !range
            if (nfield /= 6) then
                write(writeunit,formstr(2))adjustl(pathid(ipath)),'E03',modnam,nfield,'IS AN INVALID NUMBER OF FIELDS FOR KEYWORD',&
                    trim(adjustl(keywrd(ikey)))
                lbad=.true.
            else
!       JAT 1/30/2025 A058 (A058_SOUND); add istation as input argument
!                call up_range(i+i1)
                call up_range(i+i1,istation)
            endif
            
!       JAT 1/30/2025 A058 (A058_SOUND); add AUDIT2
        !else if (ikey == 13) then !audit
        else if (ikey == 13 .or. ikey == 49) then !audit
!           JAT 1/30/2025 A058 (A058_SOUND); add istation as input argument
            !call upper_list(i+i1)
            call upper_list(i+i1,istation)

!       JAT 1/30/2025 A058 (A058_SOUND); add MODIFY2
        !else if (ikey == 11) then !modify keyword
        else if (ikey == 11 .or. ikey == 48) then !modify keyword
...
!   JAT 1/29/2025 A058 (A058_SOUND); add istation as input argument to upper_list to denote
!   primary or secondary station
    !subroutine upper_list(i1)
    subroutine upper_list(i1,istation)
!=========================================================================================================
...
!   ISTATION:       DENOTES PRIMARY (1) OR SECONDARY STATION (2)
!
!   Variable definitions
!      
!   Integer variables
...
!   JAT 2/5/2025 A058 (A058_SOUND); add keyword to main1 use list
    !use main1, only : var_list,nfield,writeunit,ikey,inpline1,msg_form
    use main1, only : var_list,nfield,writeunit,ikey,inpline1,msg_form,keywrd
    implicit none
!   JAT 1/29/2025 A058 (A058_SOUND); add istation as input variable
    !integer(kind=4), intent(in) :: i1
    integer(kind=4), intent(in) :: i1,istation
...
!   JAT 1/29/2025 A058 (A058_SOUND); reset formstr to use actual keywords
...
!   1/29/2025 A058 (A058_SOUND); add appropriate keywords and update arrays as appropriate
!   to differeniate between primary and secondary site
    v1: do i=1,nupvars
        uvar(i)=upvars(i)%varname
!       set uplogical to appropriate values based on keyword
        !if (ikey == 5) then
        !    uplogical(i)=upvars(i)%lnomiss
        if (ikey == 5 .or. ikey == 50) then
            uplogical(i)=upvars(i)%lnomiss(istation)
        else
            !uplogical(i)=upvars(i)%laudit
            uplogical(i)=upvars(i)%laudit(istation)
        endif
    enddo v1
...
!   JAT 1/29/2025 A058 (AO58_SOUND); add code to differentiate between NO_MISSING and NO_MISSING2 and
!   AUDIT and AUDIT2
    v2: do i=1,nupvars
        !if (ikey == 5) then
        if (ikey == 5 .or. ikey == 43) then
            ...
            if (upvars(i)%lnomiss(istation))write(writeunit,formstr)adjustl(pathid(ipath)), 'W02',modnam,&
                'DUPLICATING LISTING OF VARIABLE FOR',trim(adjustl(upvars(i)%varname)),'KEYWORD',trim(adjustl(keywrd(ikey)))
            upvars(i)%lnomiss(istation)=uplogical(i)
        else
            ...
            if (upvars(i)%laudit(istation) .and. index(inpline1,upvars(i)%varname) > 0) then
                write(writeunit,formstr)adjustl(pathid(ipath)), 'W02',modnam,'DUPLICATING LISTING OF VARIABLE FOR',&
                    trim(adjustl(upvars(i)%varname)),'KEYWORD',trim(adjustl(keywrd(ikey)))
            else
                !upvars(i)%laudit=uplogical(i)
                upvars(i)%laudit(istation)=uplogical(i)
            endif
        endif
    enddo v2
...
!   JAT 1/30/2025 A058 (A058_SOUND); add istation as input argument to up_range to denote
!   primary or secondary station
!    subroutine up_range(i1)
    subroutine up_range(i1,istation)
!=========================================================================================================
!   SUBROUTINE UP_RANGE
!   THIS SUBROUTINE PROCESSES LINES FROM THE INPUT RUNSTREAM FILE ASSOCIATED WITH
...
!   modnam:         Subroutine name
!=========================================================================================================   
!   JAT 1/30/2025 A058 (A058_SOUND); include ikey and keywrd from main1      
!    use main1, only : range_mod,nfield,getfields,writeunit,msg_form
    use main1, only : range_mod,nfield,getfields,writeunit,msg_form,ikey,keywrd
    implicit none
!   JAT 1/30/2025 A058 (A058_SOUND); add istation
    !integer(kind=4), intent(in) :: i1
    integer(kind=4), intent(in) :: i1,istation
...
!   JAT 1/30/2025 A058 (A058_SOUND); modify formstr to allow for specific keyword RANGE or RANGE2
...
!   then get the new ranges and/or missing data value.
    if (lfound) then
        ...
        if (lgood) then
!           JAT 1/30/2025 A058 (A058_SOUND); modify message to include specific keyword RANGE or RANGE2 and
!           istation index
!           if variable has already been modified then warn user but still reset values
            ...
            if (upvars(i)%lmod(istation)) write(writeunit,formstr)adjustl(pathid(ipath)), 'W02',modnam,&
                'DUPLICATING LISTING OF VARIABLE FOR',trim(adjustl(keywrd(ikey))),'KEYWORD:',trim(adjustl(upvars(i)%varname))
!           even if variable modified previously, use the latest values
!           JAT 1/30/2025 A058 (A058_SOUND); modify specific value based on istation
            ...
            upvars(i)%lincbound(istation)=linc
            upvars(i)%lowbound(istation)=lowval
            upvars(i)%upbound(istation)=hival
            upvars(i)%missval(istation)=missval
            if (.not. upvars(i)%lmod(istation)) upvars(i)%lmod(istation)=.true.
        endif
    else
!       JAT 1/30/2025 A058 (A058_SOUND); modify message to include specific keyword RANGE or RANGE2
        !write(writeunit,formstr)adjustl(pathid(ipath)),'E05',modnam,'INVALID VARIABLE NAME FOR RANGE KEYWORD:',&
        !    trim(adjustl(varnam1(1)))
        write(writeunit,formstr)adjustl(pathid(ipath)),'E05',modnam,'INVALID VARIABLE NAME FOR',trim(adjustl(keywrd(ikey))),&
            'KEYWORD:',trim(adjustl(varnam1(1)))
        lbad=.true.
    endif
...
!   THIS SUBROUTINE ALSO PERFORMS CHECKS ON SECONDARY SITE KEYWORDS
!
!   MODIFIED DECEMBER 3, 2021
!     
!   JAMES THURMAN
...
!   JAT 1/30/2025 A058 (A058_SOUND); add istation as local variable to check primary (1) or secondary (2) station
!   ikey1, ikey2, and ikey3 to denote which keywords to look for
!    integer(kind=4) :: iflag
    integer(kind=4) :: iflag,istation,ikey1,ikey2,ikey3,ikey4
    logical :: no_extract
...
!   JAT 7/7/2025 A058 (A058_SOUND); message regarding primary and secondary station timezones
...
!   JAT 6/16/2025 A058 (A058_SOUND); initially set upstart, upend, and updates to primary station's values
!   may get reset in pbl_test; also set the secondary station's dates if stage 1 for secondary station
    upstart=upstarts(1)
    upend=upends(1)
    updates=updates1(1,:,:)
    if (lsecondary .and. upstage1(2)) then
      updates1(2,:,:)=updates
      upstarts(2)=upstart
      upends(2)=upend
    endif
...
!   JAT 6/16/2025 A058 (A058_SOUND); if only running stage 1 and secondary station data
!    is not being read in using DATA2 keyword, reset lsecondary to false and nstation to 1
    if (lsecondary .and. .not. lstage(2) .and. .not. lupkeywords(10)) then
        lsecondary=.false.
        nstation=1
        write(msg_unit,formstr(2))adjustl(pathid(ipath)),'W37',modnam,&
            'SECONDARY STATION WILL NOT BE READ; STAGE 2 PROCESSING NOT REQUESTED'
    endif
    
!   JAT 1/30/2025 A058 (A058_SOUND); check EXTRACT for primary and/or secondary station  
    l1: do istation=1,nstation
        if (istation == 1) then 
            ikey1=1 !DATA position in lupkeywords
            ikey2=4 !DATA position in keywords
            ikey3=ikey2+3 !EXTRACT position in keywords
            ikey4=ikey2+4 !QAOUT position in keywords
        else
            ikey1=10 !DATA2 position in lupkeywords
            ikey2=42 !DATA2 position in keywords
            ikey3=ikey2+2  !EXTRACT2 position in keywords
            ikey4=ikey2+3 !QAOUT2 position in keywords
        endif
        if (upstage1(istation) .and. .not. lupkeywords(ikey1)) then
            no_extract=.false.
!           if EXTRACT or EXTRACT2 keyword has been found, check to see if
!           the file is empty. 1st line should have word LOCATION
            if (lupkeywords(ikey1+2)) then
                read(up_extract_unit(istation),*,iostat=iflag)junk
                if ((iflag == 0 .and. index(junk,'LOCATION') == 0) .or. iflag /=0) no_extract=.true. !file is empty or bad
                rewind(up_extract_unit(istation))
            else
                no_extract=.true.
            endif
...
!           running stage 1, then error
            if ((no_extract .or. (.not. no_extract .and. .not. lupkeywords(ikey1+3))) .and. .not. lstage(2)) then
                    write(msg_unit,formstr(1))adjustl(pathid(ipath)),'E08',modnam,'MISSING KEYWORD',trim(adjustl(keywrd(ikey2)))
                lbad=.true.
            endif
        elseif (lupkeywords(ikey1)) then
            writeext=.true.
        endif
        
    !   check EXTRACT if processing stage 1 only
!       if stage 1 and 2, then it is optional
        if (upstage1(istation) .and. .not. lstage(2) .and. .not. lupkeywords(ikey1+2)) then
            write(msg_unit,formstr(1))adjustl(pathid(ipath)),'E08',modnam,'MISSING KEYWORD',trim(adjustl(keywrd(ikey3)))
            lbad=.true.
        endif
        
!       check QAOUT if stage 2 only
!       if stage 1 or stage 1 and 2, then it's optional
        if (.not. upstage1(istation) .and. lstage(2) .and. .not. lupkeywords(ikey1+3)) then
            write(msg_unit,formstr(1))adjustl(pathid(ipath)),'E08',modnam,'MISSING KEYWORD',trim(adjustl(keywrd(ikey4)))
            lbad=.true.
        endif
    enddo l1
!   JAT 1/30/2025 A058 (A058_SOUND); old code, comment out
...
!   JAT 1/30/2025 A058 (A058_SOUND); change upstage1 to upstage(1)
!    if (upstage1) then
    if (upstage1(1) .or. upstage1(2)) then
...
!   JAT 1/30/2025 A058 (A058_SOUND); check LOCATION and LOCATION2
!   JAT 1/16/2025 A058 (A058_SOUND);
!   check LOCATION if stage 1 regardless of stage 2 status
    l2: do istation=1,nstation
        if (istation == 1) then
            ikey1=6
            ikey2=10
        else
        ...
        !if (upstage1(istation) .and. .not. lstage(2) .and. .not. lupkeywords(ikey1)) then
        if (upstage1(istation) .and. .not. lupkeywords(ikey1)) then
            write(msg_unit,formstr(1))adjustl(pathid(ipath)),'E08',modnam,'MISSING KEYWORD',trim(adjustl(keywrd(ikey2)))
            lbad=.true.
        endif
...
!   JAT 1/30/2025 A058 (A058_SOUND); check for AUDIT2 keyword.  If not present and
!   AUDIT is present, audit the same variables in the secondary site that are listed for the primary site
    
    if (upstage1(2) .and. lsecondary .and. .not. lupkeywords(17) .and. lupkeywords(9)) upvars%laudit(2)=upvars%laudit(1)
...
!   JAT 7/7/2025 A058 (A058_SOUND); issue warning if timezone difference between primary and secondary station is more than 1 hour
    !if (lsecondary .and. abs(upgmt2lst(2)-upgmt2lst(1)) > 1)   write(msg_unit,formstr(2))adjustl(pathid(ipath)),'W36',modnam,&
...
!   istation:       denotes primary (1) or secondary station (2)
!   nupdays_1:      number of sounding days for primary station 
!   itype:          file type EXTRACT or QAOUT for header subroutine
!   iform:          upper air station format identifier
!   ikey1:          keyword index based on station type (primary or secondary)
!   ikey2:          keyword index based on station type (primary or secondary)
!
!   Logical variables
!   leap:           variable denoting if year is a leap year
!   lqaout:         QAOUT or QAOUT2 file requested
!   laudit:         AUDIT or AUDIT2 requested 
!
!   Character variables
...
!   modnam:         Subroutine name
!========================================================================================================= 
!   JAT 6/11/2025 A058 (A058_SOUND); add use_second and lstage from main1
    use main1, only: days,leapyr,numdays,istage,msg_form,use_second,lstage
    use file_units, only: msg_unit,up_qaout_unit,up_extract_unit
    implicit none
!   JAT 1/30/2025 A058 (A058_SOUND); add variable istation (1=primary, 2=secondary), ikey1, ikey2, iform, and itype
!    integer(kind=4) :: d,h,l,idattim(8),y,m,imon(2),idy(2),ivar1,jj,icount,iday1,ivar
    integer(kind=4) :: d,h,l,idattim(8),y,m,imon(2),idy(2),ivar1,jj,icount,iday1,ivar,istation,ikey1,ikey2,itype,iform
    integer(kind=4) :: nup_calc_audit=0
!   JAT 1/30/2025 A058 (A058_SOUND); add logical variables to replace some references to lupkeywords
!   because of adding secondary site
    !logical :: leap
    logical :: leap,lqaout,laudit
...
!   JAT 2/5/2025 A058 (A058_SOUND); update nsnd to 2-D array references
!    initialize
    d=0
    h=0
    l=0
...
!   JAT 1/30/2025 A058 (A058_SOUND); modify formstr(1), formstr(3) and formstr(4) to allow for primary or secondary site
!   write formats
!   1.  format for extraction start/stop
    !write(formstr(1),'(2(a))')trim(adjustl(msg_form)),'a,1x,a8,1x,i2.2,2(a,i2.2))'
    write(formstr(1),'(2(a))')trim(adjustl(msg_form)),'2(a,1x),a8,1x,i2.2,2(a,i2.2))'
...
!   3.  no data extracted
   ! write(formstr(3),'(2(a))')trim(adjustl(msg_form)),'a,1x,i8,1x,a,1x,i8,a)'
    write(formstr(3),'(2(a))')trim(adjustl(msg_form)),'a,1x,i8,1x,a,1x,i8,2(1x,a),a)'
!   4. no sounding for the day or no AM sounding
    !write(formstr(4),'(2(a))')trim(adjustl(msg_form)),'i8.8,1x,a)'
    write(formstr(4),'(2(a))')trim(adjustl(msg_form)),'i8.8,3(1x,a,))'
    
!   JAT 1/30/2025 A058 (A058_SOUND); put all code below into a loop to loop through the primary and secondary station
!   (if applicable); move certain operations outside the loop. they will be commented out below within the loop
    
!   JAT 6/11/2025 A058 (A058_SOUND); reset nstation and lsecondary if running both stage 1 and 2 and secondary 
!   substitution is not used
    if (lsecondary .and. lstage(2) .and. .not. use_second) then
        lsecondary=.false.
        write(msg_unit,formstr(2))adjustl(pathid(ipath)),'W37',modnam,&
            'SECONDARY STATION WILL NOT BE PROCESSED; NO SECONDARY SUBSTITUTION REQUESTED'
        close(up_qaout_unit(2))
        close(up_extract_unit(2))
    endif
    
    nupdays=numdays(upstart,upend)
    allocate(sound_info(nupdays,nstation))
    
!   JAT 2/3/2025 A058 (A058_SOUND); move the following allocations to here and comment out in the loop below
!   move assignment to snd_date to here as well
...
!               JAT 2/3/2025 A058 (A058_SOUND); move assignment of sound_info%snddate to here 
                sound_info(iday1,:)%snddate=snd_date(iday1)
            enddo d1
        enddo m1        
    enddo y1
    
!   JAT 5/2/2025 A058 (A058_SOUND); move allocation of updata to outside station loop and allocate
!   based on maximum number of levels allowed and 24 hours in a day
    allocate(updata(nupvars_keep,maxlevels,24,nupdays,nstation))
    
    !s1a: do istation=1,nstation
    !   nup_calc_audit=0
    !   vv: do ivar=1,nupvars
 !       !if (upvars(ivar)%laudit)then
    !   !if (istation > 1) then
    !   !   if (upvars(ivar)%laudit(istation) .and. upvars(ivar)%laudit(1)) cycle vv
    !   !endif
    !   if (upvars(ivar)%laudit(istation))then
 !           nup_audit_vars(istation)=nup_audit_vars(istation)+1
...
!       JAT 2/4/2025 A058 (A058_SOUND); calculate variables to audit and how many for each station
        nup_calc_audit=0
        vv: do ivar=1,nupvars
        !if (upvars(ivar)%laudit)then
        !if (istation > 1) then
...
!       initialize key variables
        if (istation == 1) then
            lqaout=lupkeywords(4)
            laudit=lupkeywords(9)
        else
            lqaout=lupkeywords(13)
            laudit=lupkeywords(17)
        endif
...
!       allocate qaoutvars if auditing requested or QAOUT file requsted
        ...
        if ((lqaout .or. laudit .and. istage == 1 .and. upstage1(istation))) allocate(qaout_vars(nupvars1))
        
!       JAT 2/3/2025 A058 (A058_SOUND); these are moved to outside the loop
        !allocate(snd_date(nupdays))
        !allocate(snd_hrs(nupdays,24))
        !allocate(s_levels(nupdays,24))
        !allocate(nsnding1(nupdays))
...
!       on the missing values from upvar
    ...
    v1: do ivar=1,nupvars !nupvars
            if (ivar <= nupvars_keep) then !one of core six variables
                ...
                updata1(ivar,:,:,:)=real(upvars(ivar)%missval(istation),r8)*upvars(ivar)%conv(istation)
                ...
                if (upvars(ivar)%laudit(istation)) then !if audited then part of updata1
                    ivar1=ivar1+1
                    !updata1(ivar1+nupvars_keep,:,:,:)=real(upvars(ivar)%missval,r8)*upvars(ivar)%conv
                    updata1(ivar1+nupvars_keep,:,:,:)=real(upvars(ivar)%missval(istation),r8)*upvars(ivar)%conv(istation)
                    if(allocated(qaout_vars)) qaout_vars(ivar1+nupvars_keep)=upvars(ivar)%varname
                endif
            endif
        enddo v1
    
!       JAT 2/3/2025 A058 (A058_SOUND); move this code to outside the loop above     
!       initialize snd_date to have all dates within the data period
!       even if a day may not have a sounding
!        iday1=0
!    y1: do y=updates(1,1),updates(2,1)
...
!       JAT 1/30/2025 A058; notify user which station is being processed
        !write(msg_unit,formstr(1))adjustl(pathid(ipath)),'I20',modnam,'UPPER AIR EXTRACTION BEGIN',cdate,idattim(5),':',idattim(6),':',&
        !    idattim(7)
        write(msg_unit,formstr(1))adjustl(pathid(ipath)),'I20',modnam,trim(adjustl(station_type(istation))),&
            'UPPER AIR EXTRACTION BEGIN',cdate,idattim(5),':',idattim(6),':',idattim(7)   
 ...
 !       call read_ext !read data for stage 2
    !endif
!       JAT 2/5/2025 A058 (A058_SOUND); add istation as an argument to read_6201, read_fsl, read_igra, and read_ext
!       JAT 6/11/2025 A058 (A058_SOUND); only read the secondary station if stage 2 also being and the second station
!       will be used for substitution
        if (upstage1(istation)) then
            iform=iupform(istation)
            if (iform == 1 .or. iform == 2) then
                call read_6201(istation)
            elseif (iform == 3) then
                call read_fsl(istation)
            elseif (iform == 4) then
                call read_igra(istation)
            else
                call read_ext(istation)
            endif
        else
            call read_ext(istation) !read data for stage 2
        endif
...
!   if stage 2, unique valid soundings is same as extracted because data has already been extracted in stage 1
    ex1:do jj=23,26
            ...
            if (jj==23) then
                !icount=nsnd(2)
                icount=nsnd(2,istation)
            elseif (jj==24) then
                !if (istage == 1 .and. upstage1) then
                if (istage == 1 .and. upstage1(istation)) then
                    !icount=nsnd(4)-nsnd(5)
                    icount=nsnd(4,istation)-nsnd(5,istation)
                else
                    !icount=nsnd(2)
                    icount=nsnd(2,istation)
                endif
...
!       get time time after extraction
    ...
    !    idattim(7)
        write(msg_unit,formstr(1))adjustl(pathid(ipath)),'I20',modnam,trim(adjustl(station_type(istation))),&    
            'UPPER AIR EXTRACTION END',cdate,idattim(5),':',idattim(6),':',idattim(7)
...
!           alert user if no valid obs pulled, may be outside of XDATES
        ...
        !    have_soundings=.false.
            !if ((istage == 1 .and. upstage1(istation) .and. (nsnd(4)-nsnd(5)) == 0) .or. (istage==2 .and. nsnd(2) == 0)) then
            if ((istage == 1 .and. upstage1(istation) .and. (nsnd(4,istation)-nsnd(5,istation)) == 0) .or. &
                (istage==2 .and. nsnd(2,istation) == 0)) then
                write(msg_unit,formstr(3))adjustl(pathid(ipath)),'W34',modnam,'NO EXTRACTED DATA BETWEEN',&
                    upstart,'-',upend,'FOR',trim(adjustl(station_type(istation))),'; CHECK XDATES'
                have_soundings(istation)=.false.
            else
!               write data to the sound_info data type
!               JAT 1/30/2025 A058 (A058_SOUND); move sound_info allocation to top of loop and allocate when istation=1 (primary)
            !allocate(sound_info(nupdays))
            !have_soundings=.true.
            
                have_soundings(istation)=.true.

    d2:         do d=1,nupdays
!               JAT 2/3/2025 A058 (A058_SOUND); this assignment is outside loop; make various updates based on primary or secondary station
                    ...
                    if (nsnding1(d) > 0) then
                        ...
                        sound_info(d,istation)%nsnding=nsnding1(d) !number of soundings for the day
                    else
                        !sound_info(d)%nsnding=1 !at least one sounding for array processing purposes
                        sound_info(d,istation)%nsnding=1 !at least one sounding for array processing purposes
!                       post 21DRF, increment counter for days with no soundings
                        !no_sound_days=no_sound_days+1
                        no_sound_days(istation)=no_sound_days(istation)+1
                        !write(msg_unit,formstr(4))adjustl(pathid(ipath)),'I28',modnam,snd_date(d),&
                        !    'NO SOUNDINGS FOR THE DAY'
!                       JAT 6/12/2025 A058 (A058_SOUND); only write message about no soundings if the date
!                       is in the date range for the station
                        if (sound_info(d,istation)%snddate >= upstarts(istation) .and. &
                            sound_info(d,istation)%snddate <= upends(istation))write(msg_unit,formstr(4))adjustl(pathid(ipath)),&
                            'I28',modnam,snd_date(d),'NO SOUNDINGS FOR THE DAY FOR',trim(adjustl(station_type(istation))),&
                                'STATION'
                    endif
                    !sound_info(d)%sounding=lsound(d) !if the day has any soundings
                    sound_info(d,istation)%sounding=lsound(d) !if the day has any soundings
...
!                   allocate levels array and assign values
                    ...
                    allocate(sound_info(d,istation)%snd_hr(sound_info(d,istation)%nsnding))
                    allocate(sound_info(d,istation)%nlevels(sound_info(d,istation)%nsnding))
                    if (nsnding1(d) > 0) then
                        do h=1,nsnding1(d)
...
!                           increment no_am_days
                            if (minval(snd_hrs(d,:)) > 11) then
                                ...
                                no_am_days(istation)=no_am_days(istation)+1
                                !write(msg_unit,formstr(4))adjustl(pathid(ipath)),'I29',modnam,snd_date(d),&
                                !    'NO AM SOUNDINGS FOR THE DAY'
                                write(msg_unit,formstr(4))adjustl(pathid(ipath)),'I29',modnam,snd_date(d),&
                                    'NO AM SOUNDINGS FOR THE DAY FOR',trim(adjustl(station_type(istation))),'STATION'
                            endif
                            !sound_info(d)%snd_hr(h)=snd_hrs(d,h)
                            !sound_info(d)%nlevels(h)=s_levels(d,h)
                            sound_info(d,istation)%snd_hr(h)=snd_hrs(d,h)
                            sound_info(d,istation)%nlevels(h)=s_levels(d,h)
                        enddo
                    else
                        !sound_info(d)%snd_hr(1)=snd_hrs(d,1)
                        !sound_info(d)%nlevels(1)=s_levels(d,1)
                        sound_info(d,istation)%snd_hr(1)=snd_hrs(d,1)
                        sound_info(d,istation)%nlevels(1)=s_levels(d,1)
                    endif
                enddo d2
...
!                   write non-QA'd data to the EXTRACT file if requested
                    ...
                    if ((lupkeywords(3) .and. writeext(istation))) then
!                       write header
                        !call header(up_extract_unit)
                        itype=1
                        call header(up_extract_unit(istation),istation,itype)
...
!                       write core data to file                   
    d3:                 do d=1,nupdays
                            ...
                            if (.not. sound_info(d,istation)%sounding) cycle d3
    !h1:                     do h=1,sound_info(d)%nsnding
    h1:                     do h=1,sound_info(d,istation)%nsnding
    !l1:                         do l=1,sound_info(d)%nlevels(h)
    l1:                         do l=1,sound_info(d,istation)%nlevels(h)
                                    !write(up_extract_unit,data_form)sound_info(d)%snddate,h,sound_info(d)%snd_hr(h),l,&
                                    write(up_extract_unit(istation),data_form)sound_info(d,istation)%snddate,h,&
                                        sound_info(d,istation)%snd_hr(h),l,(updata1(ivar,l,h,d),ivar=1,nupvars_keep) 
                                enddo l1
                            enddo h1
                        enddo d3
                    endif
...
!                   of input data type
!                   JAT 2/4/2025 A058 (A058_SOUND); modify for primary and/or secondary station
                    !if (lupkeywords(7))call up_modify    
                    if ((lupkeywords(7) .and. istation == 1) .or. (lupkeywords(15) .and. istation == 2))call up_modify(istation)
...
!                   JAT 2/4/2025 A058 (A058_SOUND); specify station
                    ...
                    if (lupkeywords(ikey1) .or. (lupkeywords(ikey2) .and. upstage1(istation))) call up_audit(istation)
...
!               JAT 5/2/2025 A058 (A058_SOUND); move updata allocation to top of subroutine
!                allocate(updata(nupvars_keep,nuplevels,nsound,nupdays))
!               allocate(updata(nupvars_keep,maxval(nuplevels),maxval(nsound),nupdays,nstation))
  
!               JAT 2/5/2025 A058 (A058_SOUND); account for primary and secondary stations
!               fill in updata array
    d4:         do d=1,nupdays
    h2:             do h=1,nsound(istation)
    l2:                 do l=1,nuplevels(istation)
    v2:                     do ivar=1,nupvars_keep !nupvars
                                updata(ivar,l,h,d,istation)=updata1(ivar,l,h,d)
                            enddo v2
                        enddo l2
                    enddo h2
                enddo d4
...
!               JAT 2/5/2025 A058 (A058_SOUND); account for primary or secondary station
!               write QA'd output to the QAOUT files if requested, only stage 1
                if (istage == 1 .and. upstage1(istation)) then
                    ...
                    if (lupkeywords(ikey1) .and. upstage1(istation)) then 
!                       write header
                        itype=2
                        call header(up_qaout_unit(istation),istation,itype)
!                       write format for data format
...
!                       write core variables and any calculated variables
    d5:                 do d=1,nupdays
                            if (.not. sound_info(d,istation)%sounding) cycle d5
    h3:                     do h=1,sound_info(d,istation)%nsnding
    l3:                         do l=1,sound_info(d,istation)%nlevels(h) 
                                    !write(up_qaout_unit,data_form)sound_info(d,istation)%snddate,h,sound_info(d,istation)%snd_hr(h),l,&
                                    write(up_qaout_unit(istation),data_form)sound_info(d,istation)%snddate,h,&
                                        sound_info(d,istation)%snd_hr(h),l,(updata1(ivar,l,h,d),ivar=1,nupvars1)
                                enddo l3
                            enddo h3
                        enddo d5
                    endif
...
!   deallocate temporary arrays, regardless of stage
    ...
    enddo s1
!   JAT 1/30/2025 A058 (A058_SOUND); end loop
!   JAT 4/23/2025 A058 (A058_SOUND); move snd_date deallocation outside station loop
    if (allocated(snd_date)) deallocate(snd_date)
    if (allocated(snd_hrs)) deallocate(snd_hrs)
    if (allocated(s_levels)) deallocate(s_levels)
    if (allocated(nsnding1)) deallocate(nsnding1)
...
!   JAT 2/6/2025 A058 (A058_SOUND); add istation and itype as input arguments
    !subroutine header(iunit)
    subroutine header(iunit,istation,itype)
!=========================================================================================================
!   SUBROUTINE HEADER
...
!   ISTATION:       DENOTES PRIMARY (1) OR SECONDARY STATION (2)
!   ITYPE:          OUTPUT UNIT TYPE, EXTRACT OR QAOUT
!
!   Variable definitions
!      
!   Integer variables
...
!   JAT 2/6/2025 A058 (A058_SOUND); no longer use up_extract_unit
    !use file_units, only: up_extract_unit
    implicit none
!   JAT 2/6/2025 A058 (A058_SOUND); add istation and itype as input arguments
    !integer(kind=4), intent(in) :: iunit
    integer(kind=4), intent(in) :: iunit,istation,itype
    integer(kind=4) :: ivar
    character(len=2) :: bndstr
    character(len=7) :: outtype(2)
!   JAT 12/19/2024 MULTI_ISSUE (A073); change length of celev to 9 to accomodate
!   elevations >= 1000 m
    !character(len=7) :: celev=''
    character(len=9) :: celev=''
...
!   and GMT to LST time factor.  If not included on LOCATION keyword, they
!   are left blank
!   JAT 2/6/2025 A058 (A058_SOUND); set lgmt2lst based on istation
    !if (lgmt2lst) write(cgmt2lst,'(i3)')upgmt2lst
    if (lgmt2lst(istation)) write(cgmt2lst,'(i3)')upgmt2lst(istation)
    
!   JAT 12/19/2024 MULTI_ISSUE (A073); change format for elevation to f9.3 to accomodate
!   elevations >= 1000 m
    !if (lelev) write(celev,'(f7.3)')upelev
    
!   JAT 2/6/2025 A058 (A058_SOUND); set variables below based on istation
    !if (lelev) write(celev,'(f9.3)')upelev
    if (lelev(istation)) write(celev,'(f9.3)')upelev(istation)
!   set values for alat/alon
    if (uplat(istation) < 0.0_r8) then
        write(alat,'(f7.3,a1)')dabs(uplat(istation)),'S'
    else
        write(alat,'(f7.3,a1)')uplat(istation),'N'
    endif
    if (uplon(istation) < 0.0_r8) then
        write(alon,'(f8.3,a1)')dabs(uplon(istation)),'W'
    else
        write(alon,'(f8.3,a1)')uplon(istation),'E'
    endif
...
!   JAT 2/6/2025 A058 (A058_SOUND); add istation below
    write(iunit,header_form)'AERMET ',versn,'LOCATION ',upid(istation),alat,alon,trim(adjustl(cgmt2lst)),&
    trim(adjustl(celev)),'FILE TYPE: ',trim(adjustl(outtype(itype))),'DATES ',updates(1,1),updates(1,2),&
...
!   JAT 2/6/2025 A058 (A058_SOUND); add istation below 
    v1: do ivar=1,nupvars
        if (upvars(ivar)%lmod(istation)) then
             if (upvars(ivar)%lincbound(istation)) then
                bndstr='<='
            else
                bndstr='<'
            endif
            write(iunit,'(a5,1x,a4,i6,1x,a2,1x,2(1x,i6))')'RANGE',upvars(ivar)%varname,upvars(ivar)%lowbound(istation),bndstr,&
                upvars(ivar)%upbound(istation),upvars(ivar)%missval(istation)
        endif
...
!   JAT 2/6/2025 A058 (A058_SOUND); change to itype
...
!   ikeepbase:      surface or lowest level height retained from most recent sounding with non-missing height
...
!=========================================================================================================  
    ...
    implicit none
!   JAT 2/5/2025 A058 (A058_SOUND); add istation as input variable
    integer(kind=4), intent(in) :: istation
! JAT 10/16/2024 A068_SOUND; add variable keepbase which is latest value or observed surface level height
    integer(kind=4) :: eof,iflag1,v,iday1,ivar,nlevs,iwarn,linetype,linetype1,iline,iflag,iyear,imonth,iday,ihr,ibase,missval(2),&
        ikeepbase
! JAT 11/4/2025 A058 (A058_SOUND); initialize readlevs, iupvars, and sdate later
...
!   initialize  
!   JAT 11/4/2025 A058 (A058_SOUND); initialize sdate, readlevs, and iupvars
    sdate=0
    iupvars=0
    readlevs=0
    lmissvals=.false.
...
!   JAT 2/6/2025 A058 (A058_SOUND); initialize writelevs
    writelevs=0
    
!   JAT 2/5/2025 A058 (A058_SOUND); update nsnd to be 2-D array
      
!   JAT 3/14/23 A019_UPPER; initialize rbase to 0
    rbase=0.0_r8
    
!   JAT 10/16/2024 A068_SOUND; initialize ikeepbase to 999999
    ikeepbase=999999
    
!   JAT 07/13/2022 A005_SAVEHR_UNINIT
...
!   JAT 2/5/2025 A058 (A058_SOUND); denote which station's up_data_unit is being read
!   read first line to make sure possible FSL format
    !read(up_data_unit,'(a60)',iostat=eof)datline
    
    read(up_data_unit(istation),'(a60)',iostat=eof)datline
...
!               issue error if not betwen 4 and 9
                if (iline > 1 .and. linetype1 < 4 .or. linetype1 > 9) then
                    ...
                    if (eof ==0 .and. .not. lbad) nsnd(1,istation)=nsnd(1,istation)+1  !add to total soundings being read
...
!                       otherwise issue error that month couldn't be found
                        if (lmonth) then  !calculate date and see if inside the data window
                            ...
                            call data_dates(ihr,iday,imonth,iyear,upgmt2lst(istation),upstarts(istation),upends(istation),32,&
                                sdate,lgo)
                            write(date_hr_str,'(a5,1x,i8,1x,a2,1x,i2.2)')'DATE:',sdate,'HR',ihr
!                           JAT 2/6/2025 A058 (A058_SOUND); add istation to arguments
                            !if (lgo) call new_sound(sdate,ihr,iday1) !increment/initialize key variables for new sounding'
                            if (lgo) call new_sound(sdate,ihr,istation,iday1) !increment/initialize key variables for new sounding
                            ...
                            write(msg_unit,ext_form(3))adjustl(pathid(ipath)),'E31',modnam,trim(adjustl(ext_errstr(2))),&
                                ...
                                nsnd(1,istation),'; LINE',trim(adjustl(datline))
                            lbad=.true.
                        endif
...
!                       the station check was done outside of the date check
                        ...
                        if (trim(adjustl(iupid)) /= trim(adjustl(upid(istation))) .and. lgo) then
                            iwarn=iwarn+1
!                           write warning message if still below the limit of message writes
                            if (iwarn <= nwarnlim) write(msg_unit,ext_form(4))adjustl(pathid(ipath)),'W30',modnam,&
                                ...
                                trim(adjustl(upid(istation))),'FOR SOUNDING NUMBER',nsnd(1,istation),date_hr_str
...
!                           JAT 2/5/2025 A058 (A058_SOUND)
                            !call fsl_versn(datline,nsnd(1),linetype,upvars(1)%missval,v,set_conv,lgo1)
                            !call fsl_versn(datline,nsnd(1),linetype,upvars(1)%missval(istation),v,set_conv,lgo1)
                            call fsl_versn(datline,nsnd(1,istation),linetype,upvars(1)%missval(istation),v,set_conv,lgo1)
...
!                           if so, issue error
                            if (lversion(1) .and. lversion(2)) then
                                write(msg_unit,ext_form(8))adjustl(pathid(ipath)),'E35',modnam,&
                                    ...
                                    'FSL VERSION 1 AND 2 BOTH DETECTED FOR SOUNDING #',nsnd(1,istation),date_hr_str
                                lbad=.true.
                            endif
                        endif
                    else
...
!                       if so, issue error
                        ...
                        elseif (trim(adjustl(wind_units)) == 'ms') then
                            ...
                            write(msg_unit,ext_form(6))adjustl(pathid(ipath)),'E33',modnam,'INVALID WIND SPEED UNITS:',&
                                ...
                                trim(adjustl(wind_units)),'FOR SOUNDING #',nsnd(1,istation)
                            lbad=.true.
                        endif
                    endif
                endif
                ...
                if (linetype == 9) then
                    if (lsurface) then
                        write(msg_unit,ext_form(12))adjustl(pathid(ipath)),'E34',modnam,&
                            ...
                            'MULTIPLE SURFACE DATA LINES FOR SOUNDING #',nsnd(1,istation),date_hr_str
                        lbad=.true.
                    else
                        lsurface=.true.
                    endif
...
!                   process if within date window
                    ...
                    if (lgo) then
                        if (iflag1 == 0 .and. readlevs(1) <= maxlevels.and. .not. ltop) then
                            ...
                            call fsl_versn(datline,nsnd(1,istation),linetype,iupvars(1),v,set_conv,lgo1)
                            if (v > 0) lversion(v)=.true.
!                           check to see if both lversion(1) and lversion(2) are true
!                           if so, issue error
                            if (lversion(1) .and. lversion(2)) then
...
!                               if first level not the surface, skip processing
                                if (readlevs(1) == 1 .and. linetype/= 9) then
                                    ...
                                    write(msg_unit,ext_form(9))adjustl(pathid(ipath)),'W31',modnam,'SKIP SOUNDING; 1ST LEVEL TYPE',&
                                        ...
                                        linetype,'NOT TYPE 9, FOR SOUNDING #',nsnd(1,istation),date_hr_str
                                    !nsnd(6)=nsnd(6)+1
                                    nsnd(6,istation)=nsnd(6,istation)+1
...
!                                   also take 1 away from nsnding1 and nsnd(3) for the day
                                    ...
                                    snd_hrs(iday1,nsnd(3,istation))=-9
                                    s_levels(iday1,nsnd(3,istation))=0
                                    nsnding1(iday1)=nsnding1(iday1)-1
!                                   JAT A019_UPPER; only subtract from nsnd(3) when it is 2 or greater
                                    !nsnd(3)=nsnd(3)-1
                                    !if (nsnd(3) > 1) nsnd(3)=nsnd(3)-1
                                    if (nsnd(3,istation) > 1) nsnd(3,istation)=nsnd(3,istation)-1
                                elseif (readlevs(1) == 1) then
! JAT 10/16/2024 A068_SOUND; calculate surface height if necessary 
                                    if (iupvars(2) == missval(v)) then
                                        !call calc_base(nsnd(1),date_hr_str,ikeepbase,iupvars(1),unit_conv(1),missval(1),&
                                        call calc_base(istation,nsnd(1,istation),date_hr_str,ikeepbase,iupvars(1),&
                                            unit_conv(1),missval(1),iupvars(2),unit_conv(2))
                                    else
                                        ikeepbase=iupvars(2) !reset to latest value
                                    endif
!                                   if the surface level set ibase based on height
                                    if (iupvars(2) /= missval(v)) then
                                        ...
                                        write(msg_unit,ext_form(10))adjustl(pathid(ipath)),'W32',modnam,&
                                            ...
                                            'SKIP SOUNDING; SURFACE LEVEL HEIGHT MISSING FOR SOUNDING #',nsnd(1,istation),&
                                            date_hr_str
!                                       reset snd_hrs to -9 and s_levels to 0 so that AERMET will know 
!                                       not to process this sounding later in stage 2
...
!                                       JAT 2/5/2025 A058 (A058_SOUND)
                                        ...
                                        rupvars(ivar,1)=real(upvars(ivar)%missval(istation),r8)!*upvars(ivar)%conv
                                    else
...
!                               adding to make sure. linetype 6 is wind data only
                                ...
                                if (.not. lmissvals(3) .and. .not. lmissvals(1) .and. iupvars(1) /= savepress .and. .not. &
                                    ...
                                    s_levels(iday1,nsnd(3,istation))=writelevs !increment s_levels  
!                                   check writelevs against nuplevels only if within data window

!                                   JAT 2/6/2025 A058 (A058_SOUND); add istation to nuplevels
!                                   reset nuplevels to writelevs if writelevs larger than nuplevels
                                    !if (writelevs > nuplevels) nuplevels=writelevs
                                    if (writelevs > nuplevels(istation)) nuplevels(istation)=writelevs
...
!                                   write data to temporary array
    v2:                             do ivar=1,6
                                        ...
                                        updata1(ivar,writelevs,nsnd(3,istation),iday1)=rupvars(ivar,1)
                                    enddo v2
                                endif                     
                            else !lgo1 bad
                                lbad=.true.
                        ...
                        elseif (iflag1 /=0) then !error reading data
                            write(msg_unit,ext_form(11))adjustl(pathid(ipath)),'E31',modnam,&
                                ...
                                'ERROR READING UPPER AIR DATA SOUNDING #',nsnd(1,istation),'; LINE',trim(adjustl(datline)),&
                                date_hr_str
                            lbad=.true.
                        endif
                    endif !end lgo if
...
!**************  linetype = 4-9 SOUNDING data data line****************************************
            ...
            if (eof == 0) then
                ...
                write(msg_unit,ext_form(3))adjustl(pathid(ipath)),'E31',modnam,trim(adjustl(ext_errstr(4))),nsnd(1,istation),&
...
!       increment number of valid soundings
        ...
        if (.not. lskip .and. linetype==9 .and. lgo) nsnd(4,istation)=nsnd(4,istation)+1
    enddo
...
!       reached data that exceeded xdates,issue message
    ...
    elseif (nsnd(2,istation) == 0 .and. .not. lbadup) then
!       no soundings extracted but no errors, issue error
    ...
    elseif (lbadup .and. nsnd(1,istation) > 0) then
!       soundings extracted but error
        !write(msg_unit,ext_form(14))adjustl(pathid(ipath)),'E38',modnam,trim(adjustl(ext_endstr(4))),nsnd(1)
        write(msg_unit,ext_form(14))adjustl(pathid(ipath)),'E38',modnam,trim(adjustl(ext_endstr(4))),nsnd(1,istation)
    !elseif (lbadup .and. nsnd(1) == 0) then
    elseif (lbadup .and. nsnd(1,istation) == 0) then
...
!   JAT 07/28/22 A002_COARE; make set_conv in/out variable
    !logical, intent(out) :: set_conv,lgood
    logical, intent(out) :: lgood
    logical, intent(inout) :: set_conv
...
! JAT 10/16/2024 A068_SOUND; add variable keepbase which is latest value or observed surface level height
    integer(kind=4) :: eof,k,k1,rtime,irh,itime,iflag1,iflag2,iday1,ivar,nlevs,iwarn,linetype,linetype1,iline,iflag,iyear,imonth,&
        iday,ihr,ibase,missval(2),ikeepbase
    integer(kind=4) :: readlevs(2) !=0
    integer(kind=4) :: iupvars(6) !=0
    integer(kind=8) :: sdate !=0
    real(kind=r8), allocatable, dimension(:,:) :: rupvars
...
!   JAT 2/5/2025 A058 (A058_SOUND); change all references to nsnd to be 2-D array
...
!   read first line to make sure possible IGRA format
    ...
    read(up_data_unit(istation),'(a75)',iostat=eof)datline
    if (datline(1:1) /= '#') then
...
!                           JAT 2/6/2025 A058 (A058_SOUND); add istation information
                            !call data_dates(ihr,iday,imonth,iyear,upgmt2lst,upstart,upend,32,sdate,lgo)
                            call data_dates(ihr,iday,imonth,iyear,upgmt2lst(istation),upstarts(istation),upends(istation),32,&
                                sdate,lgo)
                            write(date_hr_str,'(a5,1x,i8,1x,a2,1x,i2.2)')'DATE:',sdate,'HR',ihr
...
!                           JAT 2/6/2025 A058 (A058_SOUND); add istation to arguments
                            ...
                            call new_sound(sdate,ihr,istation,iday1) !increment/initialize key variables for new sounding
...
!                               the station check was done outside of the date check    
                                ...
                                if (trim(adjustl(iupid)) /= trim(adjustl(upid(istation)))) then
                                    iwarn=iwarn+1
!                                   write warning message if still below the limit of message writes
                                    if (iwarn <= nwarnlim) write(msg_unit,ext_form(4))adjustl(pathid(ipath)),'W30',modnam,&
                                        'STATION ID FROM FILE',trim(adjustl(iupid)),'DOES NOT MATCH USER ENTERED STATION ID',&
...
!                           if first sounding not the surface, skip processing
                            if (readlevs(1) == 1 .and. (linetype /= 21 .and. linetype /= 11)) then
                                ...
                                write(msg_unit,ext_form(9))adjustl(pathid(ipath)),'W31',modnam,'SKIP SOUNDING; 1 LEVEL TYPE',&
                                        linetype,'NOT TYPE 11 or 21, FOR SOUNDING #',nsnd(1,istation),date_hr_str
...
!                               also take 1 away from nsnding1 and nsnd(3) for the day
                                ...
                                nsnding1(iday1)=nsnding1(iday1)-1
                                !nsnd(3)=nsnd(3)-1
                                nsnd(3,istation)=nsnd(3,istation)-1
                            elseif (readlevs(1) == 1) then
! JAT 10/16/2024 A068_SOUND; calculate surface height if necessary
...
!                               check writelevs against nuplevels only if within data window
!                               reset nuplevels to writelevs if writelevs larger than nuplevels
!                               JAT 2/6/2025 A058 (A058_SOUND); add istation
                                !if (writelevs > nuplevels)nuplevels=writelevs
                                if (writelevs > nuplevels(istation))nuplevels(istation)=writelevs 
                                savepress=iupvars(1) !set savepress to pressure                   
                                if (iupvars(2) > uptop+ibase) ltop=.true.!exceed top
...
!       increment number of valid soundings
        ...
        if (.not. lskip .and. (linetype==21 .or. linetype ==11) .and. lgo)nsnd(4,istation)=nsnd(4,istation)+1
    enddo
...
!   keepbase:       most recent read in surface height
...
!   modnam:         Subroutine name
!=========================================================================================================  
    use main1, only: data_dates,msg_form
    use file_units, only: msg_unit,up_data_unit
    use, intrinsic :: iso_fortran_env,only : output_unit
    implicit none
!   JAT 2/5/2025 A058 (A058_SOUND); add istation as input variable
    ...
    integer(kind=8) :: sdate !=0
    real(kind=r8) :: unit_conv(6)
! JAT 10/16/2024 A068_SOUND; add keepbase as variable
    real(kind=r8) :: rbase,a,b,es,e,teten,rh,realht,ruptop,rmissval(6),keepbase
    real(kind=r8), allocatable, dimension(:,:) :: rupvars
    logical :: lgo,lmissvals(6)
!   JAT 2/5/2025 A058 (A058_SOUND); use 1st maxblock
    !character(len=upformats(iupform)%maxblock) :: datline
    character(len=upformats(iupform(istation))%maxblock) :: datline
    character(len=20) :: date_hr_str
    character(len=12) :: j1
    character(len=10) :: form1
    character(len=50) :: form2
! JAT 10/15/2024 A06_SOUND; add formstr for messages related to resetting surface level
...
!   JAT 10/16/2024 A068_SOUND; format for informational message
!   1. use station elevation
    write(formstr(1),'(2(a))')trim(adjustl(msg_form)),'a,1x,f8.2,1x,a,1x,i5,1x,a20)'
    
!   2. use station pressure and standard atmosphere
    write(formstr(2),'(2(a))')trim(adjustl(msg_form)),'a,f8.2,1x,a,1x,f7.2,1x,a,1x,i5,1x,a20)'
    
!   JAT 2/5/2025 A058 (A058_SOUND); change all references to nsnd to be 2-D array
!   JAT 2/5/2025 A058 (A058_SOUND); initialize sdate to 0
!   initialize
    sdate=0
    lmissvals=.false.
    nlevs=0
...
!   read first line to make sure possible 6201 format
    ...
    if (.not. lbad) then
!   JAT 2/5/2025 A058 (A058_SOUND); denote which station's up_data_unit is being used
        !rewind(up_data_unit)
        rewind(up_data_unit(istation))
    endif
...
!               JAT 2/6/2025 A058 (A058_SOUND); add istation information
                !call data_dates(ihr,iday,imonth,iyear,upgmt2lst,upstart,upend,32,sdate,lgo)
                call data_dates(ihr,iday,imonth,iyear,upgmt2lst(istation),upstarts(istation),upends(istation),32,sdate,lgo)
                write(date_hr_str,'(a5,1x,i8,1x,a2,1x,i2.2)')'DATE:',sdate,'HR',ihr
                if (lgo) then
!                   JAT 2/6/2025 A058 (A058_SOUND); add istation to arguments
                    !call new_sound(sdate,ihr,iday1) !increment/initialize key variables for new sounding
...
!                           only this message once
                        ...
                        write(msg_unit,ext_form(3))adjustl(pathid(ipath)),'E31',modnam,trim(adjustl(ext_errstr(3))),&
                                nsnd(1,istation),'; LINE',trim(adjustl(j1))
...
!                       process the levels
!                       JAT 5/31/2023 A041; correct gfortran warning for real to integer
                        !if (.not. lbad) lastht=rupvars(1,2) !set last height
                        if (.not. lbad) lastht=int(rupvars(1,2)) !set last height
...
!                           note rupvars(4,ilev) (dewpoint) will be recalculated below
    ...
    v1:                     do ivar=1,6
                                if (rupvars(ilev,ivar) == rmissval(ivar)) then
                                    lmissvals(ivar)=.true.
! JAT 2/5/2025 A058 (A058_SOUND); denote station
                                    !rupvars(ilev,ivar)=real(upvars(ivar)%missval,r8)!*upvars(ivar)%conv
                                    rupvars(ilev,ivar)=real(upvars(ivar)%missval(istation),r8)!*upvars(ivar)%conv
! JAT 10/16/2024 AO68_SOUND, if height is missing, recalculate based on elevation or pressure of level
                                    if (ilev == 1 .and. ivar == 2) then
                                        if (keepbase /= rmissval(2)) then
                                            rupvars(ilev,ivar)=keepbase
                                            lmissvals(ivar)=.false.
                                            write(msg_unit,formstr(1))adjustl(pathid(ipath)),'I30',modnam,&
                                                'SURFACE LEVEL HEIGHT BASED ON MOST RECENT SURFACE LEVEL',&
                                                keepbase,'M FOR SOUNDING #',nsnd(1,istation),date_hr_str
                                                !keepbase,'M FOR SOUNDING #',nsnd(1),date_hr_str
                                        !elseif (lelev) then 
                                        elseif (lelev(istation)) then !JAT 2/6/2025 A058
                                            !rupvars(ilev,ivar)=upelev
                                            rupvars(ilev,ivar)=upelev(istation) !JAT 2/6/2025 A058
                                            lmissvals(ivar)=.false.
                                            write(msg_unit,formstr(1))adjustl(pathid(ipath)),'I31',modnam,&
                                                'SURFACE LEVEL HEIGHT BASED ON UPPER AIR STATION ELEVATION',&
                                                upelev(istation),'M FOR SOUNDING #',nsnd(1,istation),date_hr_str
                                                !upelev,'M FOR SOUNDING #',nsnd(1),date_hr_str
                                        elseif (.not. lmissvals(1)) then
                                            rupvars(ilev,ivar)=(288.15_r8/0.0065_r8)*&
                                                (1.0_r8-(rupvars(ilev,1)/1013.25_r8)**(1.0_r8/5.255_r8))
                                            lmissvals(ivar)=.false.
                                            write(msg_unit,formstr(2))adjustl(pathid(ipath)),'I32',modnam,&
                                                'SURFACE LEVEL HEIGHT',rupvars(ilev,ivar),'BASED ON SURFACE LEVEL PRESSURE',&
                                                rupvars(ilev,1),'& STD. ATMOSPHERE FOR SOUNDING #',nsnd(1,istation),date_hr_str
                                                !rupvars(ilev,1),'& STD. ATMOSPHERE FOR SOUNDING #',nsnd(1),date_hr_str
                                        endif
                                    endif
                                else
...
! JAT 10/16/2024 A068_SOUND; reset keepbase
                                        !if (ilev==1)rbase=rupvars(ilev,ivar)
                                        if (ilev==1)then
                                            rbase=rupvars(ilev,ivar)
                                            keepbase=rbase
                                        endif
...
!                           check surface height and if missing, skip sounding
                            if (ilev == 1 .and. lmissvals(2)) then 
                                ...
                                nsnd(6,istation)=nsnd(6,istation)+1
                                write(msg_unit,ext_form(10))adjustl(pathid(ipath)),'W32',modnam,&
                                    'SKIP SOUNDING; SURFACE LEVEL HEIGHT MISSING FOR SOUNDING #',nsnd(1,istation),date_hr_str
                                    !'SKIP SOUNDING; SURFACE LEVEL HEIGHT MISSING FOR SOUNDING #',nsnd(1),date_hr_str
!                               reset snd_hrs to -9 and s_levels to 0 so that AERMET will know 
!                               not to process this sounding later in stage 2
!                               also take 1 away from nsnding1 for the day
...
!                               if (nint(realht) /= lastht .or. ilev == 1)  writelevs=writelevs+1  !increment if not a duplicate height
                                ...
                                s_levels(iday1,nsnd(3,istation))=writelevs
!                               JAT 2/6/2025 A058 (A058_SOUND); add istation
                                !if (writelevs > nuplevels) nuplevels=writelevs
                                if (writelevs > nuplevels(istation)) nuplevels(istation)=writelevs
    ...
    v2:                         do ivar=1,6
                                    ...
                                    updata1(ivar,writelevs,nsnd(3,istation),iday1)=rupvars(ilev,ivar)
                                enddo v2
                            endif
...
!       increment number of valid soundings
        ...
        if (.not. lskip .and. lgo .and. eof==0) nsnd(4,istation)=nsnd(4,istation)+1
    enddo
...
!   JAT 2/5/2025 A058 (A058_SOUND); change all references to nsnd to be 2-D array and set sdate to 0
...
!   JAT 2/5/2025 A058 (A058_SOUND); denote which station's up_data_unit is being read   
    ...
    if (upstage1(istation)) then
        if (lupkeywords(1)) then
            !ifile=up_data_unit
            ifile=up_data_unit(istation)
        else
            !ifile=up_extract_unit
            ifile=up_extract_unit(istation)
        endif
    else
        !ifile=up_qaout_unit
        ifile=up_qaout_unit(istation)
    endif
...
!                           JAT 2/6/2025 A058 (A058_SOUND); add istation below
                            ...
                            !    trim(adjustl(upid))
                            if (trim(adjustl(iupid)) /= trim(adjustl(upid(istation)))) &
                                write(msg_unit,formstr(2))adjustl(pathid(ipath)),'W30',modnam,'STATION ID FROM FILE',&
                                trim(adjustl(iupid)),'DOES NOT MATCH USER ENTERED STATION ID',trim(adjustl(upid(istation)))
                            if (dabs(rlat-uplat(istation)) > eps .or. dabs(rlon-uplon(istation)) > eps) write(msg_unit,formstr(3))&
       ...
       !                         'STATION COORDINATES DO NOT MATCH INPUT COORDINATES IN RUNSTREAM FILE'
                            ...
                            if (ifield >=4 .and. gmt2lst /= upgmt2lst(istation) .and. istage == 1)write(msg_unit,formstr(4))&
                            adjustl(pathid(ipath)),'W30',modnam,'GMT TO LST CONVERSION',gmt2lst,&
                            'DOES NOT MATCH INPUT GMT TO LST CONVERSION',upgmt2lst(istation)
                        else
                            write(msg_unit,formstr(5))adjustl(pathid(ipath)),'E31',modnam,&
...
!               actual data accounting that previous lines could be RANGE keywords or data header
                ...
                if (iflag == 0) then
                    ...
                    if (ilev==1) then
                        ...
                        nsnd(1,istation)=nsnd(1,istation)+1 !total number of soundings
                    endif
...
!                           reset some variables if no sounding
                            if (ihr == -9) then
                                ...
                                !nsnd(4)=nsnd(4)+1
                                nsnd(4,istation)=nsnd(4,istation)+1
                            endif
                        endif
                        writelevs=writelevs+1
...
!   JAT 1/30/2025 A058 (A058_SOUND); add istation as input argument
    ...
    subroutine new_sound(sdate,ihr,istation,iday1)
!=========================================================================================================
...
!   istation:       denotes primary (1) or secondary station (2)
!   savestation:    saved station index
!
!   Logical variables
...
!========================================================================================================= 
    use, intrinsic :: iso_fortran_env,only : output_unit
    use main1, only : numdays,noprint,istage,msg_form
    use file_units, only: msg_unit
    implicit none
!   JAT 2/5/2025 A058 (A058_SOUND); add istation as input variable
    !integer(kind=4), intent(in) :: ihr
    integer(kind=4), intent(in) :: ihr,istation
    integer(kind=8), intent(in) :: sdate
    integer(kind=4) :: i
    integer(kind=4), intent(out) :: iday1
!   JAT 2/5/2025 A058 (A058_SOUND); add savestation; if istation and savestation are not equal, reset savedate to 0
!   to avoid warning message when reading two stations in same AERMET run
    integer(kind=4), save :: savestation=0
...
!   JAT 6/30/22 A001_SOUND_ERROR
!   add msg_form to format string and a space
...
!   JAT 5/2/2025 A058 (A058_SOUND); if savestation not equal to istation
!   set savestation = istation and reset savedate=0
!   this is to avoid warning message W35 below when reading the secondary station
!   after the primary station in the same AERMET run
    if (savestation /= istation) then
        savestation=istation
        savedate=0
    endif
...
!   JAT 2/5/2025 A058 (A058_SOUND); change all references to nsnd to be 2-D array
    ...
    nsnd(2,istation)=nsnd(2,istation)+1 !number of soundings being processed
...
!           duplicate hour for the day
            ...
            !nsnd(5)=nsnd(5)+1
            nsnd(5,istation)=nsnd(5,istation)+1
        else
            i=i+1
        endif
    enddo
...
!   if not a duplicate process
    if (.not. lfound) then
        ...
        nsnd(3,istation)=nsnding1(iday1)
        !if(nupdays == 1)nsound=nsnd(3) !only 1 day of soundings
!       JAT 2/6/2025 A058 (A058_SOUND); add istation
        if(nupdays == 1)nsound(istation)=nsnd(3,istation) !only 1 day of soundings
        !if (nsound < nsnding1(iday1)) nsound=nsnding1(iday1)
        if (nsound(istation) < nsnding1(iday1)) nsound(istation)=nsnding1(iday1)
...
!*********************************************************************************************************
!   JAT 1/30/2025 A058 (A058_SOUND); add istation as input argument to up_modkey to denote
!   primary or secondary station      
    !subroutine up_modkey(i1)
    subroutine up_modkey(i1,istation)
!=========================================================================================================
    ...
    l1: do i=1,nfield1
            ...
            do while (j .le. nmods .and. .not. lfound)
                if (trim(adjustl(mods1(i))) == trim(adjustl(u_mods(j)))) then
                    lfound=.true.
!           JAT 1/30/2025 A058 (A058_SOUND); add istation to arrays
                    ...
                    !lmodify(j)=.true.
                    if (lmodify(j,istation))write(writeunit,formstr)adjustl(pathid(ipath)),'W02',modnam,&
                        'DUPLICATING LISTING OF MODIFICATION',trim(adjustl(mods1(i))),'FOR KEYWORD:',trim(adjustl(keywrd(ikey)))
                    lmodify(j,istation)=.true.
                else
                    j=j+1
                endif
            enddo
...
!*********************************************************************************************************
!   JAT 2/4/2025 A058 (A058_SOUND); add istation as input argument to up_modify to denote
!   primary or secondary station          
    !subroutine up_modify
    subroutine up_modify(istation)
!=========================================================================================================
!   SUBROUTINE UP_MODIFY
...
!=========================================================================================================  
    ...
    use main1, only: msg_form
    implicit none
!   JAT 2/4/2025 A058 (A058_SOUND); add istation as input argument
    integer(kind=4), intent(in) :: istation
...
!   JAT 2/6/2025 A058 (A058_SOUND); add istation information
 ...
 !       misstemp(ivar-2)=real(upvars(ivar)%missval,r8)*0.1_r8
    !enddo v1
    misspress=real(upvars(1)%missval(istation),r8) !*0.1_r8
    v1: do ivar=3,4
        misstemp(ivar-2)=real(upvars(ivar)%missval(istation),r8)*0.1_r8
    enddo v1
!   delete mandatory levels before resetting wind speeds and temperature/dewpoints since
!   data levels will change.
!   JAT 2/4/2025 A058 (A058_SOUND); specify station
    !if (lmodify(1)) then
    if (lmodify(1,istation)) then
    d1: do d=1,nupdays
...
!                                   JAT 2/4/2025 A058 (A058_SOUND); specify station
                                    write(msg_unit,formstr(1))adjustl(pathid(ipath)),'I23',modnam,snd_date(d),hrstr,&
                                        trim(adjustl(station_type(istation))),'STATION MANDATORY LEVEL: DELETED',mand_levs(l1),'MB'
                    ...
                    enddo levs_loop
                !enddo h1
!               JAT 3/16/23; A037_UP_MOD; put the if/endif statement below inside the h1 loop
...
!                       JAT 2/4/2025 A058 (A058_SOUND); specify station
                        write(msg_unit,formstr(2))adjustl(pathid(ipath)),'I23',modnam,snd_date(d),hrstr,&
                            trim(adjustl(station_type(istation))),'STATION SOUNDING HAS NO LEVELS AFTER MANDATORY LEVEL DELETIONS'
...
!           JAT A054 A054_UP_MOD 3/11/2024; only set lsound to false for the particular day, not the entire array
!            if (lsound(d) .and. nsnding1(d) == 0) lsound=.false.
            if (lsound(d) .and. nsnding1(d) == 0) lsound(d)=.false.
        enddo d1
    endif
...
!   JAT 2/4/2025 A058 (A058_SOUND); specify station
    ...
    if (lmodify(2,istation) .or. lmodify(3,istation)) then
    d2: do d=1,nupdays
            if (lsound(d)) then
...
!                       JAT 2/4/2025 A058 (A058_SOUND); specify station
                        ...
                        if (lmodify(2,istation) .and. nint(updata1(5,l,h,d)) /= 0 .and. nint(updata1(6,l,h,d)) == 0) then
                            ...
                            write(msg_unit,formstr(3))adjustl(pathid(ipath)),'I24',modnam,snd_date(d),hrstr,&
                                trim(adjustl(station_type(istation))),'STATION LEVEL',l,'WIND DIRECTION SET TO 0 - CALM WIND'
                        endif
!                       temperature and dewpoint, start with level 2 and do not process upper limit
!                       JAT 2/4/2025 A058 (A058_SOUND); specify station
                        !if (lmodify(3) .and. l >=2 .and. l < s_levels(d,h)) then
                        if (lmodify(3,istation) .and. l >=2 .and. l < s_levels(d,h)) then
    v3:                     do ivar=3,4
!                               JAT 2/6/2025 A058 (A058_SOUND); add station information
                                if (updata1(ivar,l,h,d) == misstemp(ivar-2)) then
                                    updata1(ivar,l,h,d)=newtemp(istation,l,h,d,ivar)
...
!   JAT 2/6/2025 A058 (A058_SOUND); add istation as input argument to denote
!   primary or secondary station
    !real(kind=r8) function newtemp(l,h,d,v)
    real(kind=r8) function newtemp(istation,l,h,d,v)
!=========================================================================================================
!   FUNCTION NEWTEMP
...
!    integer(kind=4), intent(in):: l,h,d,v
    integer(kind=4), intent(in):: l,h,d,v,istation
    integer(kind=4) :: l1
...
!   JAT 2/6/2025 A058 (A058_SOUND); add istation information
    ...
    misspress=real(upvars(1)%missval(istation),r8)
    misstemp=real(upvars(v)%missval(istation),r8)*0.1_r8
...
!*********************************************************************************************************
!   JAT 2/4/2025 A058 (A058_SOUND); add istation as input argument to up_audit
    !subroutine up_audit
    subroutine up_audit(istation)
!=========================================================================================================
!   SUBROUTINE UP_AUDIT
...
!   lowbound:       lower bound for station istation
!   upbound:        upper bound for station istation
!
!   Logical variables
...
!   JAT 2/4/2025 A058 (A058_SOUND); add variables lowbound, upbound to be local values of
!   lower/upper boundaries
    real(kind=r8) :: lowbound(nupvars),upbound(nupvars)
    logical lcalc,lowviolate,upviolate,debug1 
!   JAT 2/4/2025 A058 (A058_SOUND); increase formstr from 9 to 10 to have message about which station is being QA'd
    !character(len=60) :: formstr(9)
    character(len=60) :: formstr(10)
!   JAT 1/12/2026 A099 (A058_SOUND); increase debug_form length to 90
    !character(len=75) :: debug_form(2)
    character(len=90) :: debug_form(2)
...
!   JAT 1/12/2026 A099 (A05_SOUND); increase height format to f7.1 from f6.1
    !write(debug_form(2),'(a)')'(/t15,a4,t26,a,t42,a4,t52,a3,t61,a4,t70,a3/t10,g13.6,3x,i3,5(3x,f6.1))'
    write(debug_form(2),'(a)')'(/t15,a4,t26,a,t42,a4,t52,a3,t61,a4,t70,a3/t10,g13.6,3x,i3,2x,f7.1,2(3x,f6.1,2x,f7.1))'
...
!   JAT 2/4/2025 A058 (A058_SOUND); add message about which station is being QA'd
    write(formstr(10),'(2(a))')trim(adjustl(msg_form)),'a,2(1x,a))'
    
    ht=0.0_r8
    
!   allocate the audit variable array
!   JAT 2/4/2025 A058 (A058_SOUND); allocate arrays if necessary
    if (.not. allocated(up_audit_index)) then
        allocate(up_audit_index(maxval(nup_audit_vars),nstation))
        up_audit_index=0
    endif
    if (.not. allocated(up_audit_counts)) then
        allocate(up_audit_counts(maxval(nup_audit_vars),10,4,nstation))
        up_audit_counts=0
    endif
...
!   set missing values as real
    v1: do ivar=1,nupvars
        ...
        if (upvars(ivar)%laudit(istation)) then
            ...
            up_audit_index(i,istation)=ivar
            
        endif
        !missvals(ivar)=real(upvars(ivar)%missval,r8) !*upvars(ivar)%conv
        missvals(ivar)=real(upvars(ivar)%missval(istation),r8) !*upvars(ivar)%conv
        lowbound(ivar)=real(upvars(ivar)%lowbound(istation),r8)*upvars(ivar)%conv(istation)
        upbound(ivar)=real(upvars(ivar)%upbound(istation),r8)*upvars(ivar)%conv(istation)
    enddo v1
...
!   JAT 2/4/2025 A058 (A058_SOUND); write message saying which station is being QA'd
    
    write(msg_unit,formstr(10))adjustl(pathid(ipath)),'Q19',modnam,'PERFORMING QA FOR',trim(adjustl(station_type(istation))),&
        'UPPER AIR STATION'
...
!       JAT 2/4/2025 A058 (A058_SOUND); write message saying which station is being debugged
        !write(debug_unit,'(a)')'STAGE 1 UPPERAIR DEBUG PROCESSING START'
        write(debug_unit,'(a,2(1x,a))')'STAGE 1 UPPERAIR DEBUG PROCESSING START FOR',trim(adjustl(station_type(istation))),&
            'STATION'
        debug1=.true.
    else
        debug1=.false.
...
!   JAT 2/4/2025 A058 (A058_SOUND); update certain variables below to account for station type
    d1: do d=1,nupdays
        if (lsound(d)) then
...
!           write to screen the date and hour being processed if NOPRINT not requested
    ...
    l1:     do h=1,nsnding1(d)
                ...
                if ((real(uptop,r8)-updata1(2,s_levels(d,h),h,d)) > eps) then
                    ...
                    !nsnd_5k=nsnd_5k+1
                    nsnd_5k(istation)=nsnd_5k(istation)+1
                endif
!               write debug hour
...
!                       JAT 5/31/2023 A041; correct gfortran warning of using real for integer value
                        !ilayer = ((updata1(2,ilev,h,d)-updata1(2,1,h,d))/up_inc) + 2
                        ilayer = int((updata1(2,ilev,h,d)-updata1(2,1,h,d))/up_inc) + 2
                    else
...
!                               wind speed = 0 for non-zero wind direction 
                                ...
                                wind_stats(2,istation)=wind_stats(2,istation)+1
                            endif
                        else
                            if (dabs(updata1(6,ilev,h,d)-0.0_r8) < eps) then
!                               calm wind
                                ...
                                wind_stats(1,istation)=wind_stats(1,istation)+1
                            endif
                        endif
                    endif
!                   temperature and dewpoint check      
                    if (dabs(updata1(3,ilev,h,d)-missvals(3)) > eps .and. dabs(updata1(4,ilev,h,d)-missvals(4)) > eps .and.&
                        ...
                        !tempstat=tempstat+1
                        tempstat(istation)=tempstat(istation)+1
                    ...
                    if ((lupkeywords(9) .and. istation == 1) .or. (lupkeywords(17) .and. istation == 2)) then
                        i1=0  !variable counter for calculated variables
!                       JAT 2/6/2025 A058 (A058_SOUND); add station information
   ! v2:                 do ivar=1,nup_audit_vars
    v2:                 do ivar=1,nup_audit_vars(istation)
                            !iv=up_audit_index(ivar)
                            iv=up_audit_index(ivar,istation)
...
!                           levels not missing
                            ...
                            if (iv > 6 .and. ilev > 1) then
                                ...
                                if (dabs(updata1(2,ilev,h,d)-missvals(2)) < eps .and. &
                                    ...
                                    up_audit_counts(ivar,ilayer,1,istation)=up_audit_counts(ivar,ilayer,1,istation)+1
                                    cycle l2
                                endif
...
!                           JAT 2/6/2025 A058 (A058_SOUND); add station information
                            if (dabs(updata1(i2,ilev,h,d)-missvals(iv)) < eps .and. (iv <= 6 .or. (iv >= 7 .and. iv <=9 .and. &
                                ...
                                up_audit_counts(ivar,ilayer,2,istation)=up_audit_counts(ivar,ilayer,2,istation)+1
                                !if (.not. upvars(iv)%lnomiss .and. iv <=6) write(msg_unit,formstr(6))adjustl(pathid(ipath)),'Q23',&
                                if (.not. upvars(iv)%lnomiss(istation) .and. iv <=6) write(msg_unit,formstr(6))&
                                    adjustl(pathid(ipath)),'Q23',modnam,date_hr_str,trim(adjustl(upvars(iv)%varname)),&
                                    'MISSING FOR LEVEL',ilev
                            endif
...
!                           check bounds for observed and calculated variables
                            if (dabs(updata1(i2,ilev,h,d)-missvals(iv)) > eps) then
                                ...
                                if (upvars(iv)%lincbound(istation)) then  !the bounds are acceptable, included in range
                                    ...
                                    if (updata1(i2,ilev,h,d) < lowbound(iv) .and. &
                                        dabs(updata1(i2,ilev,h,d)-lowbound(iv)) > eps) then
                                        lowviolate=.true.
                                        lstr(2)='<'
                                    endif
                                    ...
                                    if (updata1(i2,ilev,h,d) > upbound(iv) .and. &
                                        dabs(updata1(i2,ilev,h,d)-upbound(iv)) > eps) then
                                        upviolate=.true.
                                        ustr(2)='>'
                                    endif
                                    ...
                                    if (updata1(i2,ilev,h,d) <= lowbound(iv)) then
                                        lowviolate=.true.
                                        lstr(2)='<='
                                    endif
                                
                                    !if (updata1(i2,ilev,h,d) >= real(upvars(iv)%upbound,r8)*upvars(iv)%conv) then
                                    if (updata1(i2,ilev,h,d) >= upbound(iv)) then
                                        upviolate=.true.
                                        ustr(2)='>='
                                    endif
                                endif
...
!                           JAT 2/6/2025 A058 (A058_SOUND); add station information
                            if (lowviolate) then
                                ...
                                up_audit_counts(ivar,ilayer,3,istation)=up_audit_counts(ivar,ilayer,3,istation)+1
...
!   JAT 4/21/2025 A058 (A058_SOUND); add istation as input argument
!    subroutine up_stage2
    subroutine up_stage2(istation)
!=========================================================================================================
!   SUBROUTINE UP_STAGE2
...
!========================================================================================================= 
    ...
    implicit none
!   JAT 2/6/2025 A058 (A058_SOUND); add istation as input argument
    integer(kind=4), intent(in) :: istation
...
!   JAT 4/21/2025 A058 (A058_SOUND); modify code below to add istation
...
!   read in the header from the QAOUT file to get station information and dates
    ...
    do while (eof == 0 .and. .not. lbad .and. iline <=3)
        ...
        read(up_qaout_unit(istation),form1,iostat=eof)inpline1 !A058
        iline=iline+1
...
!               get the number of fields because it can vary depending on if GMT to LST or elevation included
    l2:         do i=1,len_trim(inpline1)
                ...
                call getloc(i+8,upid(istation),uplat(istation),uplon(istation),upgmt2lst(istation),upelev(istation),& !A058
                    lgmt2lst(istation),lelev(istation)) !A058
            ...
            if (index(inpline1,'DATES') > 0) then
                ...
                nfield=7
!               JAT 6/12/2025 A058 (A058_SOUND); get station specific dates
                !call getdates(i+5,upstart,upend,updates)
                call getdates(i+5,upstarts(istation),upends(istation),updates)
                updates1(istation,:,:)=updates
                updates=0
            endif
            if (index(inpline1,'RANGE') > 0) then
                ...
                nfield=6
                !call up_range(i+5)
                call up_range(i+5,istation) !A058
            endif
        endif
    enddo
    !rewind(up_qaout_unit)
    rewind(up_qaout_unit(istation)) !A058

    return
    end subroutine up_stage2
!*********************************************************************************************************
    
! JAT 10/16/2024 A068_SOUND; subroutine to calculate the surface level of sounding if missing
! JAT 2/6/2025 A058 (A058_SOUND); add istation as input argument 
!    subroutine calc_base(sndno,datestr,ikeepbase,pres,pres_conv,missval,ibase,ht_conv)
    subroutine calc_base(istation,sndno,datestr,ikeepbase,pres,pres_conv,missval,ibase,ht_conv)
!=========================================================================================================
!   SUBROUTINE CALC_BASE
!   THIS SUBROUTINE CALCULATES SURFACE LEVEL HEIGHT BASED ON STATION ELEVATION OR SURFACE LEVEL PRESSURE
...
!   US EPA/AQMG
!
!   CALLED BY:  MODULE UPPERAIR (READ_FSL, READ_IGRA)
!
!   INPUT ARGUMENTS
!
!   ISTATION:   DENOTES PRIMARY (1) OR SECONDARY STATION (2)
!   SNDNO:      SOUNDING NUMBER
!   DATESTR:    DATE AND HOUR STRING
!   IKEEPBASE:  LATEST OBSERVED VALUE FOR SURFACE LEVEL 
!   PRES:       INPUT INTEGER SURFACE LEVEL PRESSURE
!   PRES_CONV:  PRESSURE CONVERSION FACTOR
!   MISSVAL:    MISSING VALUE INDICATOR
!   HT_CONV:    HEIGHT CONVERSION FACTOR
!   ELEV:       STATION ELEVATION
!
!   OUTPUT ARGUMENTS:
!   IBASE:      INTEGER BASE HEIGHT
!
!   Variable definitions
!      
!   Integer variables
...
!   sndno:          sounding number
!   ikeepbase:      latest observed value for surface level
!   pres:           input surface level pressure
!   missval:        input missing value indicator
!   ibase:          integer base height
!    
!   Real variables
!   pres_conv:      pressure conversion factor
!   ht_conv:        height conversion factor
!   rbase:          real base height
!   rpres:          real pressure with conversion factor applied
!   expon:          1.0/5.255
...
!=========================================================================================================
    ...
    use main1, only: msg_form
    implicit none
!   JAT 2/6/2026 A058 (A058_SOUND); add istation as input variable
    !integer(kind=4), intent(in) :: pres,missval,sndno,ikeepbase
    integer(kind=4), intent(in) :: pres,missval,sndno,ikeepbase,istation
    integer(kind=4), intent(inout) :: ibase
    real(kind=r8),intent(in) :: pres_conv,ht_conv
    real(kind=r8) :: rbase,rpres,expon
    character(len=20) :: datestr
    character(len=80) :: formstr(2)
    character(len=10) :: modnam='UP_STAGE2'
    
!   JAT 10/16/2024 A068_SOUND; format for informational message
!   1. use station elevation or ikeepbase
    write(formstr(1),'(2(a))')trim(adjustl(msg_form)),'a,1x,i5,1x,a,1x,i5,1x,a20)'
    
!   2. use station pressure and standard atmosphere
    write(formstr(2),'(2(a))')trim(adjustl(msg_form)),'a,1x,i5,1x,a,1x,f7.2,1x,a,1x,i5,1x,a20)'
    
!   hiearchy
!   1. use most recent value of surface level
!   2. use elevation
!   3.  use standard atmosphere (least consistent)
    
!   if have upper air station elevation use that, if not then use pressure to calculate height
    if (ikeepbase /= 999999) then
        ibase=ikeepbase
        write(msg_unit,formstr(1))adjustl(pathid(ipath)),'I30',modnam,&
            'SURFACE LEVEL HEIGHT BASED ON MOST RECENT SURFACE LEVEL',ibase,'M FOR SOUNDING #',sndno,datestr
    !elseif (lelev) then
    elseif (lelev(istation)) then
!       JAT 2/6/2025 A058 (A058_SOUND); use input variable elev instead of upelev because there can now be more
!       than one station
        !ibase=nint(upelev)
        ibase=nint(upelev(istation))
        write(msg_unit,formstr(1))adjustl(pathid(ipath)),'I31',modnam,&
            'SURFACE LEVEL HEIGHT BASED ON UPPER AIR STATION ELEVATION',ibase,'M FOR SOUNDING #',sndno,datestr
    elseif (pres /= missval) then
        rpres=real(pres,r8)*pres_conv
        expon=1.0_r8/5.255_r8
        rbase=(288.15_r8/0.0065_r8)*(1.0_r8-(rpres/1013.25_r8)**expon)
        ibase=nint(rbase/ht_conv)
        write(msg_unit,formstr(2))adjustl(pathid(ipath)),'I32',modnam,&
            'SURFACE LEVEL HEIGHT',ibase,'BASED ON SURFACE LEVEL PRESSURE',rpres,'& STD. ATMOSPHERE FOR SOUNDING #',&
            sndno,datestr
    endif
    return
    end subroutine calc_base
!*********************************************************************************************************      
    end module upperair
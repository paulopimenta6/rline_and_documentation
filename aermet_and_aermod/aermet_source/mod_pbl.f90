...
!   bad_sfc_vars:   number of bad records in OUTPUT (surface file) for each surface variable
!   bad_pfl_vars:   number of bad records in PROFILE for each profile variable
!   mixopt:         mixing height option for COARE
!   day_window:     number of days to look before and after a sounding date to check for substitution if sounding missing
!                   default value is plus/minus 1 day
!   temp_upgmt      upper air station GMT-LST for primary and secondary station based on user GMT-LST or longitude
!
!   Real variables
...
!                   will be an error.
!   19:             denotes SSTDEPTH keyword found (ikey=37)
!   20:             denotes ZI_GUST keyword found (ikey=38)
!   21:             denotes ZI_MIN keyword found (ikey=39)
!   22:             denotes MIN_MOL keyword found (ikey=40)
!   23:             denotes DEF_VPTG keyword found (ikey=41)
!   24:             denotes UADAYWIN keyword found (ikey=50)
!
...
!   6:              denotes CCVR item found
!   7:              denotes TEMP item found
!   8:              denotes COARE item found
!   9:              denotes UPP_SUB item found
!
...
!   13:             denotes NO_COARE action found
!   14:             denotes RUN_COARE action found
!   15:             denotes WARM action found
!   16:             denotes NO_WARM action found
!   17:             denotes COOL action found
!   18:             denotes NO_COOL action found
!   19:             denotes ZO_U* action found
!   20:             denotes ZO_U*_PER action found
!   21:             denotes ZO_PER_HT action found
!   22:             denotes MIX_DFAULT action found
!   23:             denotes MIX_OBS action found
!   24:             denotes ZIC_OBS_NS action found
!   25:             denotes CALC_ZI_NS action found
!   26:             denotes ZIC_OBS_SM action found
!   27:             denotes CALC_ZI_SM action found
!   28:             denotes SUBUA action found
!   29:             denotes NOSUB_UA action found
!   30:             denotes NO_SEC_UA action found
!
...
!   lbadout:        bad surface or profile file (overflow of record)
!   luse_sst_dep    use value specified by SSTDEPTH instead of hourly ZDEP
!   valid_depth:    have valid SSTDEPTH
!   run_coare:      run COARE algorithms
!   run_coare_orig: original value of run_coare before any resets
!   warm_layer:     use warm layer COARE options
!   cool_skin:      use cool skin COARE options
!   coare_zo:       denotes which COARE zo option to use
!   coare_mix:      denotes which COARE mixing height option to use
!   sub_upper:      denotes to substitute upper air data from primary and secondary (if selected)
!   check_sec_first:denotes for a missing sounding day for the primary station, check the same day for the
!                   secondary station before going back/forward in time at the primary station
!     
!   Character variables
...
!   sfcvar:         surface variable names
!   pflvar:         profile variable names   
!   
!
...
!   JAT 2/11/2025 A058 (A058_SOUND); make my_sounding and target_hr_gmt a 2-D array to account for two upper air stations
    !integer(kind=4) :: my_sounding=0
    !integer(kind=4) :: target_hr_gmt=0
    integer(kind=4) :: my_sounding(2)=0
    integer(kind=4) :: target_hr_gmt(2)=0
!   JAT 2/11/2025 A058 (A058_SOUND); make snd_hr_gmt 2x2 array to account for two upper air stations
    !integer(kind=4) :: snd_hr_gmt(2)=0
    integer(kind=4) :: snd_hr_gmt(2,2)=0
...
!   JAT 11/29/22 A026_OVERFLOW; add counts of bad surface and profile variables (overflow formats)
    integer(kind=4) :: bad_sfc_vars(25)=0
    integer(kind=4) :: bad_pfl_vars(11)=0
!   JAT 2/10/2025 A058 (A058_SOUND); add day_window for upper air sounding substitution
    integer(kind=4) :: day_window(2)
!   JAT 2/10/2025 A058 (A058_SOUND); bring temp_upgmt from pbl_proc to main part of module for use
!   with upp_dates
    integer(kind=4) :: temp_upgmt(2)=0
...
!   JAT 7/18/22 A002_COARE; add to npblkeys for COARE keywords
!   JAT 2/10/2025 A058 (A058_SOUND); increase nbplkeys to 24 to account for UADAYWIN
    integer(kind=4), parameter :: npblkeys=24
    !integer(kind=4), parameter :: npblkeys=23
    !integer(kind=4), parameter :: npblkeys=18 !17
    integer(kind=4), parameter :: minsec=1
    integer(kind=4), parameter :: maxsec=12
    integer(kind=4) :: mixopt=0
!   JAT 11/21/22 A026_OVERFLOW; add variables nsfc_over and npfl_over to count 
!   number of records with data overflow of formats; if either of these is 
!   non-zero, AERMET will consider it an un-sucessful run
    integer(kind=4) :: nsfc_over=0
    integer(kind=4) :: npfl_over=0
    ...
    real(kind=r8) :: inso
    real(kind=r8) :: nrad
!   JAT 7/18/22 A002_COARE; move cp, pi, and vonk to main module as they wil be used for
!   COARE algorithms as well.
    !real(kind=r8),parameter :: cp=1004.0_r8
    !real(kind=r8), parameter :: pi=3.14159_r8
    !real(kind=r8), parameter :: vonk=0.4_r8
    
!   JAT 7/18/22 A002_COARE; add default sst_depth of 0.5 m
!   this number is overwritten by value specified by SSTDEPTH
!   and will over-ride value of SSTD if that is input
!   (similar to OSHEIGHTS and HTTNN);
!   also add max value for depth
!   note AERMET will allow for negative depth and just use absolute value
!   JAT 12/29/22 A002_COARE; add zi_gust for COARE gust calculations and min_l for COARE
!   calculations and zi_min for minimum zi for COARE
!   these variables are not used for non-COARE processing
    real(kind=r8) :: sst_depth=0.5_r8
    real(kind=r8) :: min_sst_depth=0.0_r8
    real(kind=r8) :: max_sst_depth=10.0_r8
    real(kind=r8) :: zi_gust=600.0_r8  !default value from COARE used for Richardson number calculations
    real(kind=r8) :: zi_min=25.0_r8 !default for COARE
    real(kind=r8) :: min_l=5.0_r8 !default for COARE
    real(kind=r8) :: def_vptg=0.01 ! for COARE
    real(kind=r8) :: fw=0.5_r8
    real(kind=r8) :: dt_pwp=19.0_r8 !thickness of warm layer (initialized to 19)
    real(kind=r8),parameter :: rho_h2o=1022.0_r8 !density of water
    real(kind=r8) :: alpha_h2o=0.0_r8 !water thermal expansion coefficient
    real(kind=r8),parameter  :: beta_salt=0.026_r8 !salinity expansion coefficient
    real(kind=r8), parameter :: cp_h2o=4000._r8 ! specific heat for water J/kg/K
...
!   JAT 2/12/2025 A058 (A058_SOUND); make up_sunrise and up_sunset 2-D arrays
!   to account for primary and possible secondary station
    ...
    !logical :: wave_period !have wave period
    !logical :: wave_ht !have wave height
    !logical :: have_sst_depth !have hourly sst depth
...
!   JAT 11/21/22 A026_OVERFLOW; add lbadout variable to indicate data overflow in surface or profile file
    logical :: lbadout=.false.    
!   JAT 7/18/22 A002_COARE; add variable luse_sst_dep to denote to use SSTDEPTH instead of ZDEP values if specified
!   read_sst_depth indicates if depth read from ONSITE/PROG data (used for reporting in mod_reports, input_summ)
!   valid_depth indicates that value associated with SSTDEPTH is valid
!   7/19/22 add variables warm_layer, cool_skin, and coare_zo
    logical :: luse_sst_dep=.false.
    logical :: valid_depth=.false.
    logical :: run_coare=.false.
    logical :: warm_layer=.false.
    logical :: cool_skin=.false.
    logical :: coare_zo(3)=.false. 
    logical :: coare_mix(6)=.false. 
!   JAT 3/12/24 A043_COARE; add run_coare_orig; do not initialize (initialize in pbl_test) issue A055
    logical :: run_coare_orig
    
!   JAT 5/5/2025 A058 (A058_SOUND); add sub_upper logical variable; default action is false (no substitution)
!   user has to request substitution using UPP_SUB item and SUB_UA action
!   JAT 2/12/2026 A058 (A058_SOUND); logical variable check_sec_first denotes that on a day with a missing
!   sounding for primary station, check the secondary site for that day before any other checks; this is
!   set to true if SUBUASEC1 is selected and secondary data has been supplied; otherwise it is false
    logical :: sub_upper=.false.
    logical :: check_sec_first=.false.
    
    character(len=9) :: sites(2)
    character(len=8) :: pblid='-9'
    character(len=20) :: mmif_versn='UNKNOWN'
!   JAT 11/22/22 A026_OVERFLOW; add surface and profile file variables for checking overflows
    character(len=6) :: sfcvar(25)
    character(len=6) :: pflvar(11)
...
!   JAT 11/29/22 A026_OVERFLOW; surface and profile variable names
!   JAT 6/1/23 A041 fix truncated line
    data sfcvar /'YEAR','MONTH','DAY','JDAY','HOUR','HFLUX','U*','W*','VPTG','ZIC','ZIM','L','ZO','BO','ALBEDO','WSPD','WDIR',&
        'ZREF','TEMP','ZTEMP','IPCODE','PRECIP','RH','PRES','CCVR'/
    data pflvar /'YEAR','MONTH','DAY','HOUR','HT','ITOP','WDIR','WSDP','TEMP','SA','SW'/
!   initialize snding_win, may be reset later in up_window
    data snding_win /-1,1/
    
!   JAT 2/10/2025 A058 (A058_SOUND); initialize day_window, may be reset later
    data day_window /-1,1/
...
!   JAT 7/19/22 A002_COARE; add coare reference
!   use coare, only : sstdepth
    implicit none
...
!   JAT 12/5/2022 A014_METPREP_LOC; pblid already initialized in main part of module
!   this correct issue with pblid later in pbl_test and mod_reports for PBL ID.
...
!       JAT 7/18/22 A002_COARE; add SSTDEPTH to keyword list
!       JAT 12/29/22 A002_COARE; add ZI_GUST, ZI_MIN, MIN_MOL
        if ((ikey == 4 .or. ikey == 9 .or. ikey == 10) .or. (ikey >=18 .and. ikey <=24) .or. (ikey >= 28 .and. ikey <= 31) .or. &
        !(ikey >=33 .and. ikey <=35) .or. ikey == 26) then
        !(ikey >=33 .and. ikey <=35) .or. ikey == 26 .or. ikey == 37) then
        (ikey >=33 .and. ikey <=35) .or. ikey == 26 .or. ikey >= 37) then
            if (ikey==4) then
                jj=18
            elseif (ikey <=10) then
            ...
            elseif (ikey >=33 .and. ikey <=35) then
                jj=ikey-18
!           JAT AOO2_COARE; add SSTDEPTH
!           JAT 12/29/22 A002_COARE; add ZI_GUST, ZI_MIN, MIN_MOL
            !elseif (ikey == 37) then
!           JAT 2/10/2025 A058 (A058_SOUND); modify ikey >=37 to ikey >=37 and ikey < 50 for new 
!           upper air substitution keyword UADAYWIN
            !elseif (ikey >= 37) then
            elseif (ikey >= 37 .and. ikey < 50) then
                jj=ikey-18
!           JAT 2/10/2025 A058 (A058_SOUND); add ikey info for UADAYWIN
            elseif (ikey == 50) then
                jj=ikey-26
            else
                jj=ikey-16
...
!               JAT A002_COARE; add SSTDEPTH
!               JAT 12/29/22 A002_COARE; add ZI_GUST, ZI_MIN, MIN_MOL
                !if ((ikey /= 4 .and. ikey <=10) .or. ikey == 21 .or. ikey == 22 .or. ikey == 24 .or. ikey == 26 .or. ikey == 37) then
!               JAT 6/1/23 A041 correct line truncation
                if ((ikey /= 4 .and. ikey <=10) .or. ikey == 21 .or. ikey == 22 .or. ikey == 24 .or. ikey == 26 .or. &
                    ikey >= 37) then
!                    ikey == 34) then
...
!               use the subroutine DATALINE to get the AERSURFACE filename
                call dataline(i,i1)
!               JAT A006_LINUX; remove comma before tempfil
...
!       JAT A002_COARE; add call for sstdepth
        elseif (ikey==37) then !SST DEPTH
            if (nfield /=2) then
                write(writeunit,formstr(2))adjustl(pathid(ipath)),'E03',modnam,nfield,&
                    'IS AN INVALID NUMBER OF FIELDS FOR KEYWORD',trim(adjustl(keywrd(ikey)))
                lbad=.true.
...
!       JAT 12/29/22 A002_COARE; add call for zi_gust height
        elseif (ikey==38) then !ZI GUST HEIGHT FOR COARE
            if (nfield /=2) then
                write(writeunit,formstr(2))adjustl(pathid(ipath)),'E03',modnam,nfield,&
                    'IS AN INVALID NUMBER OF FIELDS FOR KEYWORD',trim(adjustl(keywrd(ikey)))
                lbad=.true.
...
!       JAT 1/3/23 A002_COARE; add call for zi_min height
        elseif (ikey==39) then !MINIMUM ZI FOR COARE
            if (nfield /=2) then
                write(writeunit,formstr(2))adjustl(pathid(ipath)),'E03',modnam,nfield,&
                    'IS AN INVALID NUMBER OF FIELDS FOR KEYWORD',trim(adjustl(keywrd(ikey)))
                lbad=.true.
...
!       JAT 1/3/23 A002_COARE; add call for minimum L (min_mol)
        elseif (ikey==40) then !MINIMUM L FOR COARE
            if (nfield /=2) then
                write(writeunit,formstr(2))adjustl(pathid(ipath)),'E03',modnam,nfield,&
                    'IS AN INVALID NUMBER OF FIELDS FOR KEYWORD',trim(adjustl(keywrd(ikey)))
                lbad=.true.
...
!       JAT 1/3/23 A002_COARE; add call for default theta lapse rate (def_vptg)
!       JAT A082 (A082_VPTG) 1/10/2025; reset ikey to 41 as this is the correct reference for DEF_VPTG
!       in the keyword array
        !elseif (ikey==40) then !DEFAULT VPTG
        elseif (ikey==41) then !DEFAULT VPTG
            if (nfield /=2) then
                write(writeunit,formstr(2))adjustl(pathid(ipath)),'E03',modnam,nfield,&
                    'IS AN INVALID NUMBER OF FIELDS FOR KEYWORD',trim(adjustl(keywrd(ikey)))
                lbad=.true.
...
!       JAT 2/10/2025 A058 (A058_SOUND); process UADAYWIN keyword
        elseif (ikey == 50) then !UADAYWIN
            if (nfield /=3) then
                write(writeunit,formstr(2))adjustl(pathid(ipath)),'E03',modnam,nfield,&
                    'IS AN INVALID NUMBER OF FIELDS FOR KEYWORD',trim(adjustl(keywrd(ikey)))
                lbad=.true.
...
!           also ignore the DATA keyword
            if (ikey /= 4 .and. ikey /=22 .and. ikey /= 25) then
                write(writeunit,formstr(1))adjustl(pathid(ipath)),'E01',modnam,&
                'INVALID KEYWORD:',trim(adjustl(keywrd(ikey)))
!       JAT A082 (A082_VPTG) 1/10/2025; set lbad to true if an invalid keyword so AERMET stops
                lbad=.true.
            endif
        endif
    endif
...
!*********************************************************************************************************
! JAT A002_COARE; add new subroutine to process the SST depth
    subroutine sstdepth(i1)
!=========================================================================================================
!   SUBROUTINE SSTDEPTH
!   THIS SUBROUTINE PROCESSES LINES FROM THE INPUT RUNSTREAM FILE ASSOCIATED WITH
!   THE SSTDEPTH KEYWORD FOR ONSITE OR PROG DATA TO SPECIFY A DEFAULT SEA SURFACE TEMPERATURE
!   MEASUREMENT DEPTH.
!
...
!   US EPA/AQMG
!
!   CALLED BY:      MODULE READINP (PBL_PATH)
!     
!   INPUT ARGUMENTS 
!
!   I1:             STARTING INDEX TO READ FROM INPUT LINE
...
!
!   Character variables
!   datafield:      data for SSTDEPTH keyword read from input line
!   formstr:        formats for messages
!   modnam:         Subroutine name
!========================================================================================================= 
    use main1, only: ikey,keywrd,getfields,nfield,msg_form,writeunit
    implicit none
    integer(kind=4), intent(in):: i1
    integer(kind=4) :: nfield1,iflag
    real(kind=r8) :: depth
    real(kind=r8), parameter :: missval=9999._r8
    character(len=60) :: formstr(2)
    character(len=100),allocatable, dimension(:) :: datafield
    character(len=10) :: modnam='SSTDEPTH'
...
!   2.  depth exceeds missing value
    write(formstr(2),'(2(a))')trim(adjustl(msg_form)),'2(a,1x,f12.5,1x),a)'
...
!   read input line to get threshold
    ...
    read(datafield(1),*,iostat=iflag)depth


    if (iflag == 0) then
!       take absolute value in case user specifies a negative value
        if (depth < 0.0_r8) then
            write(writeunit,formstr(2))adjustl(pathid(ipath)),'I68',modnam,trim(adjustl(keywrd(ikey))),depth,'M < 0 RESET TO',&
                abs(depth),'M'
            depth=abs(depth)
        endif
        if (depth > max_sst_depth) then 
            if (depth >=  missval) then
!               equal to or exceeds missing value; do not reset sst_depth
                write(writeunit,formstr(2))adjustl(pathid(ipath)),'W65',modnam,&
                    trim(adjustl(keywrd(ikey))),depth,' M EXCEEDS MISSING INDICATOR',missval,'M; DO NOT USE'
            else
                write(writeunit,formstr(2))adjustl(pathid(ipath)),'W65',modnam,&
                    trim(adjustl(keywrd(ikey))),depth,' M EXCEEDS',max_sst_depth,'M'
                    sst_depth=depth
                    valid_depth=.true.
            endif
        else
    ...
    return
    end subroutine sstdepth
!*********************************************************************************************************
! JAT A002_COARE; add new subroutine to process the mixing height for COARE gust calculations
    subroutine zigust(i1)
!=========================================================================================================
!   SUBROUTINE ZIGUST
!   THIS SUBROUTINE PROCESSES LINES FROM THE INPUT RUNSTREAM FILE ASSOCIATED WITH
!   THE ZI_GUST KEYWORD FOR ONSITE OR PROG DATA TO SPECIFY MIXING HEIGHT FOR COARE
!   GUST CALCULATIONS.
!
...
!
!   Character variables
!   datafield:      data for ZI_GUST keyword read from input line
!   formstr:        formats for messages
!   modnam:         Subroutine name
!========================================================================================================= 
    use main1, only: ikey,keywrd,getfields,nfield,msg_form,writeunit
    ...
    character(len=100),allocatable, dimension(:) :: datafield
    character(len=10) :: modnam='ZIGUST'
...
!   read input line to get zi for gust calculations
    call getfields(i1,nfield1,datafield)  

    read(datafield(1),*,iostat=iflag)zi


    if (iflag == 0) then
        if (zi < 0.0_r8) then 
            write(writeunit,formstr)adjustl(pathid(ipath)),'E05',modnam,'INVALID VALUE FOR KEYWORD',trim(adjustl(keywrd(ikey))),&
            ':',trim(adjustl(datafield(1)))
            lbad=.true.
        else
            zi_gust=zi
    ...
    return
    end subroutine zigust
!*********************************************************************************************************
! JAT A002_COARE; add new subroutine to process the minimum mixing height for COARE gust calculations
    subroutine zimin(i1)
!=========================================================================================================
!   SUBROUTINE ZIMIN
!   THIS SUBROUTINE PROCESSES LINES FROM THE INPUT RUNSTREAM FILE ASSOCIATED WITH
!   THE ZI_MIN KEYWORD FOR ONSITE OR PROG DATA TO SPECIFY the MINIUMUM MIXING HEIGHT FOR COARE
!   CALCULATIONS.
!
!   MODIFIED JANUARY 9, 2023
!
...
!   read input line to get minimum zi
    call getfields(i1,nfield1,datafield)  
    ...
    return
    end subroutine zimin
!*********************************************************************************************************
! JAT A002_COARE; add new subroutine to process the default theta lapse rate for COARE calculations
    subroutine vptg_def(i1)
!=========================================================================================================
!   SUBROUTINE VPTG_DEF
!   THIS SUBROUTINE PROCESSES LINES FROM THE INPUT RUNSTREAM FILE ASSOCIATED WITH
!   THE ZI_MIN KEYWORD FOR ONSITE OR PROG DATA TO SPECIFY the MINIUMUM MIXING HEIGHT FOR COARE
...
!   
!   Real variables
!   vptg1:          theta lapse rate (C/m)
!
!   Character variables
!   datafield:      data for DEF_VPTG keyword read from input line
!   formstr:        formats for messages
!   modnam:         Subroutine name
!========================================================================================================= 
    use main1, only: ikey,keywrd,getfields,nfield,msg_form,writeunit
    ...
    integer(kind=4) :: nfield1,iflag
    real(kind=r8) :: vptg1
    real(kind=r8), parameter :: minval=0.0_r8, maxval=0.1
    character(len=60) :: formstr(2)
    character(len=100),allocatable, dimension(:) :: datafield
    character(len=10) :: modnam='DEF_VPTG'
...
!   formats for messages
!   1. invalid value for keyword, exceeds range
    write(formstr(1),'(2(a))')trim(adjustl(msg_form)),'3(a,1x),a,2(1x,a,1x,f3.1))'
...
!   read input line to get minimum zi
    ...
    read(datafield(1),*,iostat=iflag)vptg1


    if (iflag == 0) then
        if (def_vptg < minval .or. def_vptg > maxval ) then 
            write(writeunit,formstr(1))adjustl(pathid(ipath)),'E05',modnam,'INVALID VALUE FOR KEYWORD',trim(adjustl(keywrd(ikey))),&
            ':',trim(adjustl(datafield(1))),'MUST BE BETWEEN',minval,'AND',maxval
            lbad=.true.
        else
            def_vptg=vptg1
        endif
    ...
    return
    end subroutine vptg_def
!*********************************************************************************************************
! JAT A002_COARE; add new subroutine to process the minimum L for COARE
    subroutine mol_min(i1)
!=========================================================================================================
!   SUBROUTINE MOL_MIN
!   THIS SUBROUTINE PROCESSES LINES FROM THE INPUT RUNSTREAM FILE ASSOCIATED WITH
!   THE MIN_MOL KEYWORD FOR ONSITE OR PROG DATA TO SPECIFY A MINIMUM MONIN-OBUKHOV LENGTH FOR COARE
!   CALCULATIONS.
!
!   MODIFIED MARCH 24, 2023
!
...
!   
!   Real variables
!   mol:            input mininimum Monin-Obukhov length
!
!   Character variables
!   datafield:      data for minimum Monin-Obukhov keyword read from input line
!   formstr:        formats for messages
!   modnam:         Subroutine name
!========================================================================================================= 
    use main1, only: ikey,keywrd,getfields,nfield,msg_form,writeunit
    ...
    character(len=60) :: formstr(2)
    character(len=100),allocatable, dimension(:) :: datafield
    character(len=10) :: modnam='MOL_MIN'
...
!   2.  invalid value
    write(formstr(2),'(2(a))')trim(adjustl(msg_form)),'2(a,1x,f9.5,1x),a)'

    allocate(datafield(nfield1))
      
    datafield='0000000000000000000000000'
    
!   read input line to get minimum L
    call getfields(i1,nfield1,datafield)  

    read(datafield(1),*,iostat=iflag)mol


    if (iflag == 0) then
!       take absolute value in case user specifies a negative value
        if (mol < 0.0_r8) then
            write(writeunit,formstr(2))adjustl(pathid(ipath)),'I69',modnam,trim(adjustl(keywrd(ikey))),mol,'M < 0 RESET TO',&
                abs(mol),'M'
            min_l=abs(mol)
        elseif (mol == 0.0_r8) then
!           can't be zero
            write(writeunit,formstr(1))adjustl(pathid(ipath)),'E05',modnam,'INVALID VALUE FOR KEYWORD',trim(adjustl(keywrd(ikey))),&
            ':',trim(adjustl(datafield(1)))
        lbad=.true.
...
! JAT A002_COARE; add new subroutine to process COARE algorithms
    subroutine coare1(d,h,rh_ht)
!=========================================================================================================
!   SUBROUTINE COARE1
!   THIS SUBROUTINE IS MAIN SUBROUTINE FOR COARE CALCULATIONS.
!
...
!   D:      DAY COUNTER
!   H:      HOUR COUNTER
!   RH_HT:  RELATIVE HUMIDITY MEASUREMENT HEIGHT
!
!   Variable definitions
!      
!   Integer variables
...
!   d:                  day counter
!   h:                  hour counter
!   lst2gmt:            conversion from LST to GMT
!   ndays:              number of days since coare1 called for warm layer calculations
!   nits:               number of iterations
!   dummy_date:         temporary date
!   idate:              date
!   nhrs:               number of hours since coare1 called for warm layer calculations
!   nread:              number of times coare1 called
!   last_hr:            hour of previous call of coare1
!   last_day:           day of previous call of coare1
!   jj:                 loop counter
!   izopt:              zo option index
!   lastdate:           previous date for last subroutine call
!   nhrs:               number of hours since last subroutine call
!   nread:              number of times subroutine called
!
!   Real variables
!   rh_ht:              height for relative humidity observation
!   g:                  value for gravity; calculated for first time coare1 called and
!                       saved
!   zi:                 default mixing height of 600 m used in calculations
!   solar_time:         solar time based on the GMT time and longitude
!   sst_prev:           previous hour's sea surface temperature
!   e_air:              saturation vapor pressure for air (variable ea in AERCOARE)
!   e_sea:              saturation vapor pressure for sea (variable es in AERCOARE)
!   q_air:              specific humidity for air (variable q in AERCOARE)
!   q_sea:              specific humidity for sea (variable qs in AERCOARE)
!   rho_air:            density of air (variable rhoa in AERCOARE)
!   numer_cool:         numerator of Eq. 14 in Fairall et al. cool skin/warm layer paper
!                       Fairall et al. (1996) "Cool-skinand warm-layer effects on sea surface
!                       temperature" Journal of Geophysical Research vol. 101, no. c1 pgs 1295-1308
!                       January 15, 1996 (variable bigc in AERCOARE)
!   gust_factor:        gustiness factor (variable Wg in AERCOARE)
!   sst:                sea surface temperature in Celcius (variable ts in AERCOARE)
!   airtempc:           air temperature in Celcius (based on sfcdata%airtemp - 273.15)
!   airtempk:           air temperature in Kelvin (equal to sfcdata%airtemp)
!   sst_diff:           SST - delta_t_cool (equal to ts - dter in AERCAORE)
!   ustar:              surface friction velocity (variable usr in AERCOARE)
!   latent_heat_vap:    latent heat of vaporization
!   claisius:           Clausius-Claperyon (variable wetc in AERCOARE)
!   cc:                 k*ct/cd
!   viscosity_air:      viscosity of air (variable visa in AERCOARE)
!   delta_t_cool:       cool skin temperature difference (variable dter in AERCOARE)
!   delta_q_cool:       cool skin specific humidity difference (variable dqer in AERCOARE)
!   zo:                 surface roughness (variable zo in AERCOARE)
!   cool_thick:         cool layer thickness (variable tkt in AERCOARE)
!   delta_u:            wind speed including gust factor (variable du in AERCOARE)
!   delta_t:            temperature difference (variable dt in AERCOARE)
!   wind_10:            wind speed at 10 m (variable u10 in AERCAORE) 
!   zo_10:              surface roughness based on u* based on 10 m wind (variable zo10 in AERCOARE)
!   cd10:               stress coefficient based on 10 m wind height and zo_10 (variable cd10 in AERCOARE)
!   ch10:               sensible heat coefficient (varaible ch10 in AERCOARE)
!   ct10:               temperature transfer coeffiicent based on ch10 and cd10 (variable ct10 in AERCOARE)
!   zot10:              zo based on ct10 (variable zot10 in AERCOARE)
!   cd:                 stress coefficient based on actual wind height and zo (variable cd in AERCOARE)
!   ct:                 temperature transfer coeffiicent based on temperature height and zot10 (variable ct in AERCOARE)
!   ribcu:              saturation bulk Richardson number (variable ribcu in AERCOARE)
!   ribu:               bulk Richardson number (variable ribu in AERCOARE)
!   wind_speed:         wind speed (equal to sfc_data%wind_speed)
!   wind_ht:            anenomemter height (equal to sfc_data%wind_ht)
!   temp_ht:            temperature measurement height (equal to sfc_data%temp_ht)
!   rh:                 relative humidity (equal to sfc_data%rh)
!   press:              station pressure (equal to sfc_data%press)
!   zetu:               Monin-Obukhov similiarity (MOS) stability parameter (variable zetu in AERCOARE)
!   l10:                Monin-Obukhov length based on 10 m (variable l10 in AERCOARE)
!   tstar:              t* (variable tsr in AERCOARE)
!   qstar:              q* (variable qsr in AERCOARE)
!   charn:              Charnock parameter (variable charn in AERCOARE)
!   twave:              wave period (onsite variable TWAV; variable twave in AERCOARE)
!   hwave:              wave height (onsite variable HWAV; variable hwave in AERCOARE)
!   cwave:              pahse speed of wave period (variable cwave in AERCOARE)
!   lwave:              wavelenght associated with peak of wave frequency-size spectrum (wave period) (variable lwave in AERCOARE)
!   um:                 maximum of 10 m wind speed or 0.5 m/s (variable um in AERCOARE)
!   rough_reynolds:     roughness Reynolds number (variable rr in AERCOARE)
!   zoq:                roughness length for humidity
!   zot:                roughness length for temperature
!   z_over_l:           wind height divided by L
!   mol:                Monin-Obukhov length (variable L in AERCOARE)
!   psu:                psiu function
!   pst:                psit function
!   bf:                 -g/airtempk*ustar*(tstar+0.61_r8*airtempk*qstar)
!   tau_prev:           previous hour's tau (variable tau_old in AERCOARE)
!   hflux_sensible:     sensible heat flux for cool skin or warm layer calculations (variable hsb in AERCOARE)
!   hflux_latent:       latent heat flux for cool skin or warm layer calculations (varaible hlb in AERCOARE)
!   qout:               total cooling at air/sea interface (variable qout in AERCOARE)
!   dels:               mean value of solar flux absored within cool-skin sublayer (variable dels in AERCOARE)
!   qcol:               total cooling at air/sea interface - mean value of solar flux absored within cool-skin sublayer
!                       (variable qcol in AERCOARE)
!   q_cool:             virtual cooling of cool skin (variable alq in AERCOARE)
!   lambda:             term calculated by Equation 14 Fairall et al, 1996 Cool skin and warm layer (variable xlamx in AERCOARE)
!   delta_t_warm:       Price-Weller-Pinkel warming (variable dt_wrm in AERCOARE)
!   fw:                 fraction of net solar radiation incident at surface (variable fxp in AERCOARE)
!   depth_pwp:          depth of changes for PWP calculations (variable tk_pwp in AERCOARE)
!   sst_warm:           SST temp for warm layer calculations (variable tsw in AERCOARE)
!   qcol_accum:         accumulated qcol
!   net_rad_long:       net long wave radiation (variable rnl in AERCOARE)
!   net_rad_short:      net short wave radiation (variable rns in AERCOARE)
!   long_down:          long wave radiation
!   prev_solar_time:            solar time for previous hour
!   tau_accum:          accumulated tau
!   beta:               constant
!   twopi:              pi divided by 2.0
!   viscosity_h2o:      viscosity of water (variable visw in AERCOARE)
!   rho_h2o:            density of water (varaible rhow in AERCOARE)
!   tc_h2o:             thermal conductivity of water
!   rich:               critical Richardson number (variable rich in AERCOARE)
!   depth_const:        portion of thickness calculation made up of constants (cdt1 in AERCOARE with correction)
!   temp_const:         portion of temperature calculation (variable ctd2 in AERCOARE)
!   delta_time:         difference between current solar time and previous hour's time (variable dtime in AERCOARE)
!   qr_out:             cooling at surface (variable qr_out in AERCOARE)
!   f:                  3-element variable used to calculate fw 
!   gamma1:             3-element variable used to calculate fw 
!   jj:                 loop counter for calculating fw
!   sum_f:              sum in equation 26 of Fairall et al, 1996 Cool skin and warm layer
!   sea_warm:           dsea in aercoare
!   diffuse_h2o:        water vapor diffusivity (dwat in AERCOARE)
!   diffuse_heat:       heat diffusivity (dtmp in AERCOARE)
!   wet_bulb_fact:      wet bulb factor (alfac in AERCOARE)
!   hflux_rain:         heat flux due to rain (rf in AERCOARE)
!   rain:               rainfall rate (rain aercoare)
!   tau_rain:           momentum flux due to rainfal (taur in aercoare)
!   wbar:               vertical velocity with Webb correction (wbar in AERCOARE)
!   hflux_sensible_prev:previous time step sensible heat flux (hf_old)
!   hflux_latent_prev:  previous time step latent heat flux (ef_old)
!   hflux_rain_prev:    previous time step rain heat flux (rf_old)
!   sst_previ:          previous time step SST
!   depth_pwp:          warm layer thickness
!   coare_albedo:       albedo
!   qjoule:             heat
!   sstdepth:           SST measurement depth
!   tau:                momentum
!   bowen:              Bowen ratio
!   mol1:               Monin-Obukhov length
!   mixht:              mixing height
!   sst_warm:           SST for warm layer calculations
!   qr_out:             cooling at air/water surface
!   q_pwp:              total heat absorption in warm layer
!   delta_q:            q_sea-q_air
!   psiu1:              psiu for anemometer height/L@ 10 m
!   psit1:               psit for temperature height/L@ 10 m    
!   psit2:              psit for RH height/L @10 m
!   psq:                psit for RH height/L
!
!   Logical variables
!   calc_grav:          denotes that gravity has been calculated
!   lfirst:             first day of calculations
!   lintegrate:         perform integration
!   have_sst_depth:     have hourly SST depth
!   ljunk:              dummy logical variable
!   wave_period:        have hourly wave period
!   wave_ht:            have hourly wave height
!
!   Character variables
!   adate:              character string of date in LST (YYYYMMDDHH)
!   adate_gmt:          character string of date in GMT (YYYYMMDDHH)
!   formstr:            formats for messages
!   date_hr_str:        date and hour text string
!   zo_opt:             zo option text strings
!   modnam:             Subroutine name
!========================================================================================================= 
    use main1, only: r,cp,vonk,debug,data_dates,gravity,numdays,msg_form
    use file_units, only: debug_unit,msg_unit
    use onsite, only: tsea_var,twav_var,hwav_var,sstd_var,mix_var,wstar_var,vptg_var
    implicit none
    integer(kind=4), intent(in) :: d,h
!   JAT 2/12/24 A053_COARE_ZO; drop lstgmt (not needed); use lst2gmt instead
!    integer(kind=4) :: iyear,imonth,iday,ihr,lst2gmt,ndays,nits,jj,lstgmt,iter
    integer(kind=4) :: iyear,imonth,iday,ihr,lst2gmt,ndays,nits,jj,iter
    integer(kind=4), save :: izopt=0
    integer(kind=8) :: dummy_date,idate !idategmt
    integer(kind=8), save :: lastdate=0
    integer(kind=4) :: nhrs=0
    integer(kind=4), save :: nread=0
    integer(kind=4), save :: last_hr=0
    integer(kind=4), save :: last_day=0 !index for data
    real(kind=r8),intent(in) :: rh_ht
    real(kind=r8),save :: g=0.0_r8
    real(kind=r8),save :: sst_prev=0.0_r8
    !real(kind=r8), intent(in) :: lat,lon
    real(kind=r8) :: solar_time,e_air,e_sea,q_air,q_sea,rho_air,numer_cool,gust_factor,delta_t_warm,sst,&
        airtempc,airtempk,sst_diff,ustar,latent_heat_vap,cc,viscosity_air,delta_t_cool,delta_q_cool,zo,&
        cool_thick,delta_u,delta_t,wind_10,zo_10,cd10,ch10,ct10,zot10,cd,ct,ribcu,ribu,wind_speed,&
        wind_ht,temp_ht,rh,press,zetu,l10,tstar,qstar,charn,twave,cwave,lwave,hwave,um,zoq,zot,z_over_l,mol,psu,pst,&
        bf,hflux_sensible,qout,dels,qcol,q_cool,net_rad_long,net_rad_short,long_down,lambda,qcol_accum,fw,&
        depth_pwp,rich,depth_const,delta_time,f(3),gamma1(3),sum_f,qjoule,temp_const,sstdepth,sea_warm,tau,diffuse_h2o,&
        diffuse_heat,hflux_rain,rain,tau_rain,wbar,sst_warm,qr_out,q_pwp,delta_q,rough_reynolds,hflux_latent,&
        wet_bulb_fact,bowen,mol1,psiu1,psit1,psit2,psq,clausius,mixht
    real(kind=r8),save :: prev_solar_time=0
    real(kind=r8), save :: tau_accum=0.0_r8
    real(kind=r8), save :: hflux_sensible_prev=0.0_r8
    real(kind=r8), save :: hflux_latent_prev=0.0_r8
    real(kind=r8), save :: hflux_rain_prev=0.0_r8
    real(kind=r8), save :: tau_prev=0.06_r8
    real(kind=r8), parameter :: beta=1.2_r8
    real(kind=r8), parameter :: coare_albedo=0.055_r8
    real(kind=r8), parameter :: rho_h2o=1022._r8 !density of water kg/m^3
    real(kind=r8), parameter :: twopi=3.14159_r8*2.0_r8
    real(kind=r8), parameter :: viscosity_h2o=1.0e-6_r8 !kinematic viscosity of water
    real(kind=r8), parameter :: tc_h2o=0.6_r8  !thermal conductivity of water (1/K)
    !real(kind=r8) :: net_rad_long,net_rad_short !long and short wave net radiation
    !real(kind=r8) :: long_down !downward long wave radiation
    character(len=10) :: modnam='COARE1'
    character(len=8) :: adate
    character(len=80) :: formstr(2)
    !character(len=100) :: debugform(13)
    character(len=20) :: date_hr_str
    character(len=26) :: zo_opt(3)
    !character(len=8) :: adate_gmt
    logical, save :: calc_grav=.true.
    logical, save :: lfirst=.true. !analogous to jday1=1 in aercoare.f; first day of processing
    logical, save :: lintegrate=.false. !analagous to jamset=1 when true
    logical :: ljunk,wave_period,wave_ht,have_sst_depth
    
    data f /0.45_r8, 0.27_r8, 0.28_r8/
    data gamma1 /12.8_r8,0.357_r8,0.014_r8/
    
    data zo_opt /'U*','U* & WAVE PERIOD','WAVE PERIOD & WAVE HEIGHT'/
    write(date_hr_str,'(a5,1x,i8,1x,a2,1x,i2.2)')'DATE:',sfc_data(d)%sfcdate,'HR',h
    

!   1.  low zo
    write(formstr(1),'(2(a))')trim(adjustl(msg_form)),'a,1x,f8.6,1x,a)'
!   10. convective mixing height set to conv_max    
    write(formstr(2),'(2(a))')trim(adjustl(msg_form)),'a,1x,f6.1,1x,a,1x,a20)'
    
!   if debug, write beginning COARE processing
    if (debug) write(debug_unit,'(/t5,a)')'BEGIN COARE PROCESSING'
    nread=nread+1
    dummy_date=0
    !xtime=0.0
    fw=0.5_r8
...
!   variables needed for warm layer calculations
!   wave period
    call have_data(d,h,twav_var,wave_period)
!   wave height
    call have_data(d,h,hwav_var,wave_ht)
!   sst depth
    call have_data(d,h,sstd_var,have_sst_depth)

!   variables from sfc_data.  with exception of temperature, values do not change, just easier to have shorter variable names
    airtempc=sfc_data(d)%airtemp(h)-273.15_r8
    airtempk=sfc_data(d)%airtemp(h)
    wind_speed=sfc_data(d)%wspd(h)
    temp_ht=sfc_data(d)%temp_ht(h)
    wind_ht=sfc_data(d)%wind_ht(h)
    rh=sfc_data(d)%rh(h)
    press=sfc_data(d)%pres(h)
    
    if (sfc_data(d)%pamt(h) < 0.0_r8) then
        rain=0.0_r8
    else
        rain=sfc_data(d)%pamt(h)
    endif
    
    sst=osdata1(osvars(tsea_var)%readvar,1,h,ios(d)) 
    
!   SST depth; use onsite value if SSTDEPTH not specified on control file
    if (have_sst_depth .and. .not. luse_sst_dep) then
        sstdepth=dabs(osdata1(osvars(sstd_var)%readvar,1,h,ios(d)))
    else
        sstdepth=sst_depth
    endif
...
!   write initial parameters
    if (debug) then
!       air temperature
        write(debug_unit,'(/t10,a,/t10,f8.3,t25,f8.3,t40,f7.2)')'AIR TEMP (C)   AIR TEMP(K)   TEMP HT (M)',airtempc,airtempk,&
            temp_ht
!       wind speed
        write(debug_unit,'(/t10,a,/t10,f8.3,t29,f7.2)')'WIND SPEED (M/S)  WIND HT (M)',wind_speed,wind_ht
!       RH and pressure
        write(debug_unit,'(/t10,a,/t10,f5.1,t19,f7.2,t31,f7.2)')'RH (%)   RH HT (M)  PRESSURE (MB)',rh,rh_ht,press
!       rainfall
        write(debug_unit,'(/t10,a,1x,f8.3)')'RAINFALL (MM/HR/):',rain
!       SST and SST depth
        write(debug_unit,'(/t10,a,/t10,f8.3,t21,f8.3)')'SST (C)    SST DEPTH',sst,sstdepth
    endif
    
    
!   initialize local sea surface temperature SST and sst_diff (sea surface temperature with cool skin differences)
    if (nread == 1) then
        sst_warm=osdata1(osvars(tsea_var)%readvar,1,h,ios(d))
        if (cool_skin) then
            delta_t_cool=0.3_r8
        else
            delta_t_cool=0.0_r8
        endif
        sst_diff=sst-delta_t_cool !set SST with cool skin difference (may be 0)
        sst_prev=sst_diff
    endif
    
    net_rad_long=0.0_r8
    net_rad_short=0.0_r8
    long_down=0.0_r8
!   convert the local time to GMT; use data_dates and don't worry about the start/end dates since we
!   just want the time.  Lgo (output from data_dates) is more relevant for pbl_proc where it's been used.
!   get month, day, and year of date
    write(adate,'(i8.8)')sfc_data(d)%sfcdate
    read(adate,'(i4,2(i2.2))')iyear,imonth,iday 
    
!   going back to GMT so negative of what is in AERMET control file
!   JAT 2/12/24 A053_COARE_ZO; lstgmt should be lst2gmt
!   lstgmt=-pblgmt2lst
    lst2gmt=-pblgmt2lst
    
    ihr=h
    call data_dates(ihr,iday,imonth,iyear,lst2gmt,dummy_date,dummy_date,42,idate,ljunk)
    
!   create COARE time in GMT (needed for solar time)
    !write(adate_gmt,'(i8.8,i2.2)')idate,ihr
    !read(adate_gmt,'(i10.0)')idategmt
    
!   solar time (hours) is based on the GMT time and longitude
    !solar_time=mod(lon/15.0_r8+real(ihr,r8)+24.0_r8,24.0_r8)
    solar_time=mod(pbllon/15.0_r8+real(ihr,r8)+24.0_r8,24.0_r8)
    
!   calculate gravity for first time subroutine is called. also initialize some variables
    if (calc_grav) then 
        !g=gravity(lat)
        g=gravity(pbllat)
        calc_grav=.false.

        !if (debug)write(debug_unit,'(/t10,a,1x,f7.3,1x,a,1x,f8.3)')'VALUE OF GRAVITY',g,'M/S^2 AT LATITUDE',pbllat
!       since gravity is calculated only for first time subroutine is called, get zo option
        if (debug) then
            if (coare_zo(1))izopt=1
            if (coare_zo(2))izopt=2
            if (coare_zo(3))izopt=3
        endif
    endif
    
    if (debug)write(debug_unit,'(/t10,a,1x,f7.3,1x,a,1x,f8.3/)')'VALUE OF GRAVITY',g,'M/S^2 AT LATITUDE',pbllat
    
!   JAT 9/6/22 A002_COARE ; calculate sat vapor pressure and specific humidity for the air
    if (debug) write(debug_unit,'(/t10,a)')'VAPOR PRESSURE AND SPECIFIC HUMIDITY CALCULATIONS FOR AIR TEMPERATURE'
    e_air=vapor_pres(airtempc,press)
    e_air=e_air*rh/100.0_r8  !convert from RH using vapor pressure
    if (debug) write(debug_unit,'(/t10,a,1x,g13.6)')'VAPOR PRESSURE AFTER RH scaling (E x RH/100):',e_air
    q_air=0.62197_r8*(e_air/(press-0.378_r8*e_air)) !specific humidity for air
    
    if (debug) then
        write(debug_unit,'(/t10,a)')'SPECIFIC HUMIDITY CALCULATION'
        write(debug_unit,'(t18,a/t10,g13.6,t28,f7.2,t38,g13.6)')'Q         PRESSURE        E',q_air,press,e_air
    endif
    
!   calculate air density (not the same as rho calculated in pbl_proc)
    rho_air=press*100.0_r8/(r*(airtempk)*(1.0_r8+0.61_r8*q_air)) !kg/m3  Moist air density now q in kg/kg
    if (debug) then
        write(debug_unit,'(2(/t10,a))')'AIR DENSITY CALCULATION','DENSITY   PRESSURE   TEMP (K)        Q'
        write(debug_unit,'(t10,f7.3,t20,f7.2,t32,f6.2,t41,g13.6)')rho_air,press,airtempk,q_air
    endif
    
!   calculate water thermal expansion coefficient
    alpha_h2o=2.1e-5_r8*(sst+3.2_r8)**0.79_r8
    if (debug) write(debug_unit,'(/t10,a,1x,g13.6)')'WATER THERMAL EXPANSION COEFFICIENT:',alpha_h2o
!   calculate sat vapor pressure (mb) for sea surface temp
    if (debug) write(debug_unit,'(/t10,a)')'VAPOR PRESSURE AND SPECIFIC HUMIDITY CALCULATIONS FOR SEA SURFACE TEMPERATURE'
    e_sea=vapor_pres(sst,press)
    e_sea=e_sea*0.98_r8 ! reduced for salinity Kraus 1972, pg. 46
    if (debug) write(debug_unit,'(/t10,a,1x,g13.6)')'VAPOR PRESSURE AFTER REDUCING FOR SALINITY:',e_sea
    q_sea=0.62197_r8*(e_sea/(press-0.378_r8*e_sea)) !specific humidity for sea
    
    if (debug) then
        write(debug_unit,'(/t10,a)')'SPECIFIC HUMIDITY CALCULATION'
        write(debug_unit,'(t18,a/t10,g13.6,t28,f7.2,t38,g13.6)')'Q         PRESSURE        E',q_sea,press,e_sea
    endif
    
!   calculate net radiation
    if (warm_layer .or. cool_skin) then
!       set value for long_down, either calculated or from observed data
        long_down=lw_down(d,h)
!       calculate net longwave radiation
        net_rad_long=0.97_r8*(5.67e-8_r8*(sst_prev+273.15_r8)**4-long_down) !Net longwave (up = +). Typically 3C warming=15W/m2
        net_rad_short=0.945_r8*inso
    endif
    
    if (warm_layer) then
!       get number of hours since last hour processed
!       numdays will give the number of days including the first day
!       so if lastdate and sfc_data(d)%sfcdate are 1 day apart, ndays will be 2
        ndays=numdays(lastdate,sfc_data(d)%sfcdate)
    
!       subtract 1 to get # of days since last date
        ndays=ndays-1 !number of days since the last
    
        if (h >= last_hr) then
            nhrs=h-last_hr+(24*(ndays-1))
        else
            nhrs=(h+24)-last_hr+(24*(ndays-1))
        endif
!       only do the checks below if not the first occurrence
        if (nread > 1) then
            if (solar_time < prev_solar_time .or. nhrs > 4) then
!               reset at local midnight or missing more than 4 hours
                tau_accum=0.0_r8
                qcol_accum=0.0_r8
                delta_t_warm=0.0_r8
!               from aercoare.f subroutine bulk_flux:
!               initial guess for warm layer parameters expected in early morning
!               fw=0.5 implies a shallow heating layer to start integration;
!               depth_pwp=19.0 implies warm layer thickness is a maximum from the day before
!               anot not meant to match current hour's fw.
!               fw term in Fairall et al., Cool skin and warm layer 1996 equation 26
                fw=0.5_r8
!               depth_pwp is Dt term in Fairall et al., Cool skin and warm layer 1996, Section 3.3 pg. 1300
                depth_pwp=19.0_r8
                sst_warm=sst
                
                lfirst=.false. !analogous to setting jday1=0 in aercoare.f (subroutine bulk_flux)
                lintegrate=.false. !analagous to setting jamset=0 in aercoare.f (subroutine bulk_flux)
!           fill in once understand what variables are
            elseif (solar_time > 6.0_r8 .and. lfirst) then
!               6:00 AM too late to start on first day
                delta_t_warm=0.0_r8
                sst_warm=sst
            else
!               computer warm layer
                ...
                rich=0.65_r8
!               calculate depth_pwp based on equation 21 of Fairall et al., 
!               calculate part of Ri and denominator
                depth_const=dsqrt((2._r8*rich*cp_h2o)/(tc_h2o*g*rho_h2o))
!               temp constant
                temp_const=dsqrt(2._r8*tc_h2o*g/(rich*rho_h2o))/(cp_h2o**1.5_r8)
                
!               get amount of time between current time and previous time in seconds 
                delta_time=(solar_time-prev_solar_time)*3600._r8
!               get cooling at surface
                qr_out=net_rad_long+hflux_sensible_prev+hflux_latent_prev+hflux_rain_prev
!               total heat absorption in warm layer
                q_pwp=fw*net_rad_short-qr_out
                if (q_pwp < 50._r8 .and. .not. lintegrate) then
                    sst_warm=sst
                else
                    lintegrate=.true.
!                   calculate momemtum integral
                    tau_accum=tau_accum+dmax1(0.002_r8,tau_prev)*delta_time
                    if (qcol_accum+q_pwp*delta_time > 0.0_r8) then
!                       iterate warm layer thickness
    l1:                 do iter=1,5
                            sum_f=0.0_r8
    f1:                     do jj=1,3
                                sum_f=sum_f+f(jj)*gamma1(jj)*(1-dexp(-depth_pwp/gamma1(jj)))
                            enddo f1
                            fw=1-sum_f/depth_pwp
                            qjoule=(fw*net_rad_short-qr_out)*delta_time
                            if (qcol_accum+qjoule > 0.0) depth_pwp=dmin1(19.0_r8,depth_const*tau_accum/dsqrt(qcol_accum+qjoule))
                        enddo l1
                    else
                        fw=0.75_r8
                        depth_pwp=19.0_r8
                    ...
                    qcol_accum=qcol_accum+qjoule
                    if (qcol_accum > 0.0_r8) then
                        delta_t_warm=temp_const*(qcol_accum)**1.5_r8/tau_accum
                    else
                        delta_t_warm=0.0_r8
                    endif
                    if (depth_pwp < sstdepth) then
                        sea_warm=delta_t_warm
                    else
                        sea_warm=delta_t_warm*sstdepth/depth_pwp
                    endif
                    sst_warm=sst+sea_warm
                endif
            endif
        endif
!       reset lastdate, prev_solar_time,last_hr, and last_day
        lastdate=sfc_data(d)%sfcdate
        prev_solar_time=solar_time
        last_hr=h
        last_day=d
    else
        sst_warm=sst
    endif !end warm layer 
    prev_solar_time=solar_time
    lastdate=sfc_data(d)%sfcdate
    last_hr=h
        
!   what is in ASL subroutine in aercoare.f
    !calculate u* and L (do these first before filling in code above)
    latent_heat_vap=(2.501_r8-0.00237_r8*sst)*1.0e+6_r8 !J/kg latent heat of vaporization at ts (3C warming=0.3%)
    if (debug) write(debug_unit,'(/t10,a,1x,g13.6)')'LATENT HEAT OF VAPORIZATION:',latent_heat_vap
!   clausius is wetc from aercoare
    clausius=0.622_r8*latent_heat_vap*q_sea/(r*(sst+273.16_r8)**2) !Clausius-Clapeyron
    if (debug) then
        write(debug_unit,'(/t10,a/t13,a)')'CLAUSIUS CALCULATION','CLAUSIUS    LATENT HEAT       QSEA       SST'
        write(debug_unit,'(t10,g13.6,2(1x,g13.6),1x,f7.2)')clausius,latent_heat_vap,q_sea,sst
    endif
    
!   kinematic viscosity of dry air
    viscosity_air=1.326e-5_r8*(1._r8+6.542e-3_r8*airtempc+8.301e-6_r8*airtempc*airtempc-4.84e-9*airtempc*airtempc*airtempc)
    if (debug) write(debug_unit,'(/t10,a,1x,g13.6)')'KINEMATIC VISCOSITY OF DRY AIR:',viscosity_air
    
    if (cool_skin) then
!   set cool skin temperature difference
        delta_t_cool=0.3_r8
        delta_q_cool=cc*delta_t_cool
        cool_thick=0.001_r8
!       this term is from equation 14 of the Fairall et al. paper on cool skin and warm layer
!       formerly bigc
        numer_cool=16._r8*g*cp_h2o*(rho_h2o*viscosity_h2o)**3/(tc_h2o*tc_h2o*rho_air*rho_air)
    else
        delta_t_cool=0.0_r8
        delta_q_cool=0.0_r8
        cool_thick=0.0_r8
...
!   initial gustiness factor
    gust_factor=0.5
    
    if (debug) write(debug_unit,'(/t10,a,1x,f6.4/t10,a,1x,f5.1)')'INITIAL ZO:',zo,'INITIAL GUSTINESS FACTOR:',gust_factor
    
!   air-sea differences
!   wind speed
    delta_u=(wind_speed**2.0_r8+gust_factor**2.0_r8)**0.5_r8
    
    if (debug) write(debug_unit,'(/t10,a,1x,f7.3)')'ADJUSTED WIND SPEED WITH GUST FACTOR:',delta_u
!   temperature difference (C)
    delta_t=sst-airtempc-0.0098_r8*temp_ht
    if (debug) then
        write(debug_unit,'(/t10,a)')'SST - AIR DIFFERENCE BASED ON TEMPERATURE HEIGHT'
        write(debug_unit,'(/t12,a)')'SST (C)   AIR TEMP (C)   TEMP HT (M)  LAPSE RATE  DELTA T'
        write(debug_unit,'(t11,f8.3,t23,f8.3,t39,f8.3,t52,a6,t61,f8.3)')sst,airtempc,temp_ht,'0.0098',delta_t
    endif
    
!   mixing ratio differences
    delta_q=q_sea-q_air
    if (debug) then
        write(debug_unit,'(/t10,a,1x,g13.6)')'Q SEA - Q AIR:',delta_q
    endif
        
!   neutral coefficients
    wind_10=delta_u*dlog(10.0_r8/zo)/dlog(wind_ht/zo) !wind speed at 10 m (u10 in AERCOARE)
    ustar=0.035_r8*wind_10 !u*
    zo_10=0.011_r8*ustar*ustar/g+0.11*viscosity_air/ustar
    cd10=(vonk/dlog(10._r8/zo_10))**2 !transfer coefficent for stress @ 10 m
    ch10=0.00115_r8 !transfer coefficient for sensible heat @ 10 m
    ct10=ch10/sqrt(cd10) !temperature transfer coefficient at 10 m
    zot10=10.0_r8/dexp(vonk/ct10)
    cd=(vonk/dlog(wind_ht/zo_10))**2

!   from Grachev and Fairall (JAM, 1997)
    ct=vonk/(dlog(temp_ht/zot10)) !temperature transfer coefficient
    cc=vonk*ct/cd !z/l vs. Rib linear coefficient
    ribcu=-wind_ht/(zi_gust*0.004_r8*beta**3) !saturation or plateau bulk Richardson number
    ribu=-g*wind_ht*((delta_t-delta_t_cool)+0.61_r8*airtempk*delta_q)/(airtempk*delta_u**2)
    
    if (debug) then
!       10 m wind speed
        write(debug_unit,'(/t10,a)')'CALCULATED 10 M WIND'
        write(debug_unit,'(/t12,a)')'ADJUSTED WIND  WIND HT     ZO     10 M WIND'
        write(debug_unit,'(t14,f6.2,t27,f7.2,t36,f7.5,t47,f6.2)')delta_u,wind_ht,zo,wind_10
        
!       u*
        write(debug_unit,'(/t10,a,1x,f6.4)')'U* (10 m wind x 0.035):',ustar

!       zo based on 10 m wind and u*
        write(debug_unit,'(/t14,a)')'ZO (10 m)     U*        G       KINEMATIC VISCOSITY OF DRY AIR'
        write(debug_unit,'(t10,g13.6,t26,f6.4,t37,f7.3,t54,g13.6)')zo_10,ustar,g,viscosity_air
        
!       transfer coefficient for stress @ 10 m
        write(debug_unit,'(/t13,a)')'CD (10 M)     K        ZO (10 M)'
        write(debug_unit,'(t10,g13.6,t26,f3.1,t34,g13.6)')cd10,vonk,zo_10
        
!       transfer coefficient for sensible heat @ 10 m
        write(debug_unit,'(/t10,a,1x,f7.5)')'CH (10 M):',ch10
        
!       temperature transfer coefficient @ 10m
        write(debug_unit,'(/t12,a)')'CT (10 M)     CH (10 M)     CD (10 M)'
        write(debug_unit,'(t10,g13.6,t26,f7.5,t38,g13.6)')ct10,ch10,cd10
        
!       zot10
        write(debug_unit,'(/t11,a)')'ZOT10     K      CT (10 M)'
        write(debug_unit,'(t10,f7.5,t20,f3.1,t26,g13.6)')zot10,vonk,ct10
        
!       drag coefficient
        write(debug_unit,'(/t15,a)')'CD          K    WIND HT   ZOT10'
        write(debug_unit,'(t10,g13.6,t26,f3.1,t32,f6.2,t41,f7.5)')cd,vonk,wind_ht,zot10
        
!       temperature transfer coefficient
        write(debug_unit,'(/t15,a)')'CT          K    TEMP HT   ZOT10'
        write(debug_unit,'(t10,g13.6,t26,f3.1,t32,f6.2,t41,f7.5)')ct,vonk,temp_ht,zot10     
        
!       cc
        write(debug_unit,'(/t15,a)')'CC          K          CT          CD'
        write(debug_unit,'(t10,g13.6,t26,f3.1,t32,g13.6,1x,g13.6)')cc,vonk,ct,cd
        
!       plateau Bulk Richardson number
        write(debug_unit,'(/t13,a)')'RiBcu    WIND HT   GUST ZI   BETA'
        write(debug_unit,'(t10,g13.6,t25,f6.2,t35,f7.2,t46,f3.1)')ribcu,wind_ht,zi_gust,beta
        
!       Bulk Richardson number
        write(debug_unit,'(/t45,a)')'SST-AIR        COOL SKIN                   SEA - AIR'
        write(debug_unit,'(t13,a)')&
            'RiBu         G      WIND HT     DELTA T         DELTA T   AIR TEMP (K)      DELTA Q       ADJUSTED WIND SPEED'
        write(debug_unit,'(t10,g13.6,t25,f5.2,t34,f7.3,t45,f8.3,t61,f8.3,t74,f6.2,t88,g13.6,t111,f6.2)')ribu,g,wind_ht,delta_t,&
            delta_t_cool,airtempk,delta_q,delta_u
    endif
    
    if (ribu < 0.0_r8) then
        zetu=cc*ribu/(1.0+ribu/ribcu) !unstable G and F
        if (debug) then
            write(debug_unit,'(/t15,a)')'ZETU           CC           RIBu        RIBcu'
            write(debug_unit,'(t10,g13.6,3(1x,g13.6))')zetu,cc,ribu,ribcu
        endif
    else
        zetu=cc*ribu*(1.0+27.0/9.0*ribu/cc) !stable
        if (debug) then
            write(debug_unit,'(/t15,a)')'ZETU           CC           RIBu'
            write(debug_unit,'(t10,g13.6,3(1x,g13.6))')zetu,cc,ribu,ribcu
        endif
    endif
    l10=wind_ht/zetu
    if (debug) write(debug_unit,'(/t16,a/t10,g13.6,t27,f6.2,t35,g13.6)')'L10        WIND_HT     ZETU',l10,wind_ht,zetu

!   set number of iterations
    if (zetu > 50.0_r8) then
        nits=1
    else
        nits=3
    endif
    
!   first guess Monin-Obukhov stability dependent scaling parameters
!   psiu is a function
    if (debug) write(debug_unit,'(/t10,a)')'CALCULATING PSIU1 = PSIU; INPUT Z_OVER_L = WIND HT/L10'
    psiu1=psiu(wind_ht/l10)
    if (debug) write(debug_unit,'(/t10,a,1x,g13.6)')'PSIU1 =',psiu1
    if (debug) write(debug_unit,'(/t10,a)')'CALCULATING PSIT1 = PSIT; INPUT Z_OVER_L = TEMP HT/L10'
    psit1=psit(temp_ht/l10)
    if (debug) write(debug_unit,'(/t10,a,1x,g13.6)')'PSIT1 =',psit1
    if (debug) write(debug_unit,'(/t10,a)')'CALCULATING PSIT2 = PSIT; INPUT Z_OVER_L = RH HT/L10'
    psit2=psit(rh_ht/l10)
    if (debug) write(debug_unit,'(/t10,a,1x,g13.6)')'PSIT2 =',psit2
    
    ustar=delta_u*vonk/(dlog(wind_ht/zo_10)-psiu1)
    tstar=-(delta_t-delta_t_cool)*vonk/(dlog(temp_ht/zot10)-psit1)
    qstar=-(delta_q-delta_q_cool)*vonk/(dlog(rh_ht/zot10)-psit2)
    
    charn=0.011_r8
    
!   write initial guesses
    if (debug) then
        write(debug_unit,'(/t10,a)')'INITIAL GUESSES'
        
!       u*
        write(debug_unit,'(/t27,a/t16,a)')'ADJUSTED','U*        WIND SPEED   K     WIND HT       ZO @ 10          PSIU1'
        write(debug_unit,'(t10,g13.6,t28,f6.2,t38,f3.1,t45,f6.2,t56,g13.6,t72,g13.6)')ustar,delta_u,vonk,wind_ht,zo_10,psiu1
        
!       t*
        write(debug_unit,'(/t37,a/t16,a)')'COOL SKIN','T*        DELTA_T     DELTA_T     K     TEMP HT       ZOT10           PSIT1'
        write(debug_unit,'(t10,g13.6,t25,f8.3,t37,f8.3,t49,f3.1,t56,f6.2,t67,g13.6,t83,g13.6)')tstar,delta_t,delta_t_cool,vonk,&
            temp_ht,zot10,psit1
        
!       qstar
        write(debug_unit,'(/t37,a/t16,a)')'COOL SKIN','Q*        DELTA_T     DELTA_T     K     TEMP HT       ZOT10           PSIT2'
        write(debug_unit,'(t10,g13.6,t25,f8.3,t37,f8.3,t49,f3.1,t56,f6.2,t67,g13.6,t83,g13.6)')qstar,delta_t,delta_t_cool,vonk,&
            rh_ht,zot10,psit2
        
!       charn
        write(debug_unit,'(/t10,a,1x,f5.3)')'INITIAL CHARNOCK CONSTANT:',charn
    endif
    
    if (delta_u > 10.0_r8) then
        charn=0.011_r8+(0.018_r8-0.011_r8)*(delta_u-10.0_r8)/(18.0_r8-10.0_r8)
        if (debug) write(debug_unit,'(/t10,a/t20,f8.6,t44,f6.2)')'ADJUSTED CHARNOCK CONSTANT   ADJUSTED WIND SPEED',charn,delta_u
    elseif (delta_u > 18.0_r8) then
        charn=0.018_r8
        if (debug) write(debug_unit,'(/t10,a,1x,f5.3)')'ADJUSTED CHARNOCK CONSTANT (ADJUSTED WIND SPEED > 18):',charn
    endif
    
!   start iterations
    if (debug)write(debug_unit,'(/t10,a,1x,i1/t10,a,1x,a)')'NUMBER OF ITERATIONS:',nits,'SURFACE ROUGHNESS OPTION:',&
        trim(adjustl(zo_opt(izopt)))
    l2: do iter=1,nits
        if (debug) write(debug_unit,'(/t10,a,1x,i1)')'START ITERATION:',iter
!       surface roughness options
        if (coare_zo(1)) then
!           zo function of u*
!           zo based on equation 25 of Fairall et al., 1986
            zo=charn*ustar*ustar/g+0.11_r8*viscosity_air/ustar
            if (debug) then
                write(debug_unit,'(/t26,a/t18,a)')'CHARNOCK                 KINEMATIC VISCOSITY OF',&
                    'ZO     CONSTANT   USTAR        G        DRY AIR'
                write(debug_unit,'(t15,f7.5,t26,f5.3,t36,f6.2,t46,f7.3,t56,g13.6)')zo,charn,ustar,g,viscosity_air
            endif
        elseif (coare_zo(2) .or. coare_zo(3)) then
            um=dmax1(wind_10,0.5_r8)
            if (debug)write(debug_unit,'(/t15,a,1x,f6.2,1x,a)')'UM = MAX OF ',wind_10,'AND 0.5 M/S'
            
            if ((coare_zo(2) .and. wave_period) .or. (coare_zo(3) .and. wave_period .and. wave_ht)) then
!               using u* and significant wave period (coare_zo(2)) and have measured data 
!               or
!               using u* and significant wave period and height (coare_zo(3)) and have measured data for both 
!               period and height
!   JAT A053_COARE_ZO 2/12/24; correct reference for twave
!                twave=osdata1(twav_var,1,h,ios(d)) ! use measured data
                twave=osdata1(osvars(twav_var)%readvar,1,h,ios(d)) ! use measured data
                if (debug) write(debug_unit,'(/t15,a,1x,f8.2)')'WAVE PERIOD (SECONDS) SET TO MEASURED VALUE:',twave
!               if using u* and significant wave period and height (coare_zo(3)) using measured wave height
                if (coare_zo(3)) then
!   JAT A053_COARE_ZO 2/12/24; correct reference for hwave
!                    hwave=osdata1(hwav_var,1,h,ios(d)) ! use measured data
                    hwave=osdata1(osvars(hwav_var)%readvar,1,h,ios(d)) ! use measured data
                    if (debug) write(debug_unit,'(/t15,a,1x,f7.2)')'WAVE HEIGHT (M) SET TO MEASURED VALUE:',hwave
                endif
            else
!               if coare_zo(2) and don't have measured wave period 
!               or
!               if coare_zo(3) and either wave period or wave height is missing then calculate twave (even if have measured data)
                twave=0.729_r8*um
                if (debug) write(debug_unit,'(/t15,a,1x,f8.2,1x,a,1x,f6.2)')'WAVE PERIOD (SECONDS)',twave,'SET TO 0.729 x',um
!               if coare_zo(3) and don't have wave height or period (even if have wave height), calculate wave height
                if (coare_zo(3)) then
                    hwave=0.018_r8*um*um*(1.0_r8+0.015_r8*um)  
                    if (debug) write(debug_unit,'(/t13,a/t15,f7.2,t27,f6.2)')'WAVE HEIGHT WIND SPEED',hwave,um
                endif
            endif
            cwave=g*twave/twopi
            lwave=cwave*twave
            if (debug) then
                write(debug_unit,'(/t13,a,/t15,f7.2,t25,f7.3,t36,f8.2,t47,f6.4)')'PHASE SPEED    G     WAVE PERIOD    2 PI',&
                    cwave,g,twave,twopi
                write(debug_unit,'(/t14,a,/t15,f9.3,t30,f7.2,t42,f8.2)')'WAVELENGTH PHASE SPEED WAVE PERIOD',lwave,cwave,twave
            endif
            
!           if coare_zo(2), zo based on u* and significant wave period use equation 25b from Fairall et al. 2003
            if (coare_zo(2)) then
                zo=(50._r8/twopi)*lwave*(ustar/cwave)**4.5_r8+0.11_r8*viscosity_air/ustar
                if (debug) write(debug_unit,'(/t59,a/t17,a/t15,f7.5,t24,f9.3,t38,f7.2,t49,f6.2,t58,g13.6,t74,f6.4)')&
                    'VISCOSITY OF','Z0     WAVELENGTH  PHASE SPEED     U*       DRY AIR       2 PI',&
                    zo,lwave,cwave,ustar,viscosity_air,twopi
            endif
!           if coare_zo(3), zo based on significant wave period and height use quation 25a from Fairall et al. 2003
            if (coare_zo(3)) then
                zo=1200._r8*hwave*(hwave/lwave)**4.5_r8+0.11_r8*viscosity_air/ustar !Taylor and Yelland
                if (debug) write(debug_unit,'(/t59,a/t17,a/t15,f7.5,t24,f9.3,t38,f7.2,t49,f6.2,t58,g13.6,t76,f7.2)')&
                    'VISCOSITY OF','Z0     WAVELENGTH  PHASE SPEED     U*       DRY AIR      WAVE HEIGHT',&
                    zo,lwave,cwave,ustar,viscosity_air,hwave
            endif
        endif
        
!       reset zo if small
        
        !if (zo < 0.0001_r8) then
        !    write(msg_unit,formstr(1))adjustl(pathid(ipath)),'W75',modnam,'Z0 FOR < 0.0001:',&
  !                  zo,'RESET TO 0.0001'
        !   zo=0.0001_r8
        !endif
!       calculate roughness Reynolds number equation 10 from Fairall et al., 1996
        rough_reynolds=zo*ustar/viscosity_air
!       zoq (surface roughness for humidity) and zot (surface roughness for temperature) fitted to results from several ETL cruises
!       
        zoq=dmin1(1.15e-04_r8,5.5e-05_r8/rough_reynolds**0.6_r8)
        zot=zoq
        z_over_l=vonk*g*wind_ht*(tstar*(1.0+0.61_r8*q_air)+0.61_r8*airtempk*qstar)/(airtempk*ustar*ustar*(1._r8+0.61_r8*q_air))
        mol=wind_ht/z_over_l !calculate L
        
!       large batch of debug statements
        if (debug) then
!           Reynolds number
            write(debug_unit,'(/t16,a,t45,a/t16,a/t15,f8.2,t26,f7.5,t36,f6.2,t45,g13.6)')'ROUGH','VISCOSITY OF',&
                'REYNOLDS    Z0     U*       DRY AIR',rough_reynolds,zo,ustar,viscosity_air
            
!           zoq and zot
            write(debug_unit,'(/t15,a,1x,f7.5)')'ZOQ ZOT SET TO',zoq

!           z_over_L
            write(debug_unit,'(/t69,a/t18,a)')'AIR VAPOR      AIR',&
                'Z_OVER_L     K       G     WIND HT         T*       PRESSURE    TEMP (K)          Q*             U*'
            write(debug_unit,'(t15,g13.6,t30,f3.1,t36,f7.3,t45,f6.2,t54,g13.6,t68,g13.6,t83,f6.2,t96,g13.6,t112,f8.4)')&
                z_over_l,vonk,g,wind_ht,tstar,q_air,airtempk,qstar,ustar
            
!           Monin-Obukhov length
            write(debug_unit,'(/t15,a,1x,g13.6)')'MONIN-OBUKHOV LENGTH (WIND_HT/Z_OVER_L):',mol
        endif
        
        if (debug) write(debug_unit,'(/t15,a)')'CALCULATING PSU = PSIU; INPUT Z_OVER_L = WIND HT/MOL'
        psu=psiu(wind_ht/mol)
        if (debug) write(debug_unit,'(/t15,a,1x,g13.6)')'PSU =',psu
        
        if (debug) write(debug_unit,'(/t15,a)')'CALCULATING PST = PSIT; INPUT Z_OVER_L = TEMP HT/MOL'
        pst=psit(temp_ht/mol)
        if (debug) write(debug_unit,'(/t15,a,1x,g13.6)')'PST =',pst
        
        if (debug) write(debug_unit,'(/t15,a)')'CALCULATING PSQ = PSIT; INPUT Z_OVER_L = RH HT/MOL'
        psq=psit(rh_ht/mol)
        if (debug) write(debug_unit,'(/t15,a,1x,g13.6)')'PSQ =',psq
        
        delta_q_cool=clausius*delta_t_cool
        ustar=delta_u*vonk/(dlog(wind_ht/zo)-psu)
        tstar=-(delta_t-delta_t_cool)*vonk/(dlog(temp_ht/zot)-pst)
        qstar=-(delta_q-delta_q_cool)*vonk/(dlog(rh_ht/zoq)-psq)
        bf=-g/airtempk*ustar*(tstar+0.61_r8*airtempk*qstar)
        
        if (debug) then
!               delta_q_cool
                write(debug_unit,'(/t17,a,t46,a/t18,a)')'COOL SKIN','COOL SKIN','DELTA Q        CLAUSIUS      DELTA T'
                write(debug_unit,'(t15,g13.6,t29,g13.6,t46,f8.3)')delta_q_cool,clausius,delta_t_cool

!               u*
                write(debug_unit,'(/t30,a/t21,a)')'ADJUSTED','U*      WIND SPEED    K    WIND HT     ZO         PSU'
                write(debug_unit,'(t15,g13.6,t29,f6.2,t42,f3.1,t48,f6.2,t57,f8.6,t66,g13.6)')ustar,delta_u,vonk,wind_ht,zo,psu
    
!               t*
!               JAT 6/1/23 A041 correct truncated lines
                write(debug_unit,'(/t42,a/t21,a)')&
                    'COOL SKIN','T*        DELTA_T     DELTA_T     K     TEMP HT       ZOT             PST'
                write(debug_unit,'(t15,g13.6,t30,f8.3,t42,f8.3,t54,f3.1,t61,f6.2,t72,g13.6,t88,g13.6)')tstar,delta_t,delta_t_cool,&
                    vonk,temp_ht,zot,pst
                
!               qstar
!               JAT 6/1/23 A041 correct truncated line
                write(debug_unit,'(/t42,a/t21,a)')&
                    'COOL SKIN','Q*        DELTA_T     DELTA_T     K     TEMP HT       ZOT           PSQ'
                write(debug_unit,'(t15,g13.6,t30,f8.3,t42,f8.3,t54,f3.1,t61,f6.2,t72,g13.6,t88,g13.6)')qstar,delta_t,delta_t_cool,&
                    vonk,rh_ht,zot10,psq

!               bf
                write(debug_unit,'(/t40,a/t21,a)')'AIR','BF         G     TEMP (K)        U*            T*           Q*'
                write(debug_unit,'(t15,g13.6,t29,f7.3,t38,f6.2,t48,g13.6,2(1x,g13.6))')bf,g,airtempk,ustar,tstar,qstar
                
        endif
        
        
!       include gust component; use a ballpark zi
        if (bf > 0) then
            gust_factor=beta*(bf*zi_gust)**0.333_r8
            if (debug) then
                write(debug_unit,'(/t16,a)')'GUST FACTOR   BETA           BF      ZI GUST'
                write(debug_unit,'(t15,g13.6,t31,f3.1,t39,g13.6,t53,f7.2)')gust_factor,beta,bf,zi_gust
            endif
        else
            gust_factor=0.2_r8
            if (debug) write(debug_unit,'(/t15,a,1x,f3.1)')'GUST_FACTOR=',gust_factor
        endif
!       include gust factor in wind speed
        delta_u=dsqrt(wind_speed*wind_speed+gust_factor*gust_factor) 
        
        if (debug) write(debug_unit,'(/t15,a,1x,f7.3)')'ADJUSTED WIND SPEED WITH GUST FACTOR:',delta_u
        
!       if warm layer or cool skin chosen, recompute net longwave radiation; 
        if (cool_skin .or. warm_layer) net_rad_long=0.97_r8*(5.67e-8_r8*(sst+273.16_r8-delta_t_cool)**4-long_down)
        
        
!       cool skin
        if (cool_skin) then
!           calculate sensible and latent heat fluxes
            hflux_sensible=-rho_air*cp*ustar*tstar
            hflux_latent=-rho_air*latent_heat_vap*ustar*qstar
!           calculate Q (equation 2, Fairall et al, 1996 Cool skin and warm layer)
            qout=net_rad_long+hflux_sensible+hflux_latent
!           mean value of solar flux absored within cool-skin sublayer (Fairall et al, 1996 Cool skin and warm layer, pg. 1298)
            dels=net_rad_short*(0.065_r8+11.0_r8*cool_thick-6.6e-5_r8/cool_thick*(1.0_r8-dexp(-cool_thick/8.0e-4_r8))) ! eq 16. Ohlmann
            qcol=qout-dels !cool skin
            q_cool=alpha_h2o*qcol+beta_salt*hflux_latent*cp_h2o/latent_heat_vap
            if (q_cool > 0.0_r8) then
!               calculate lambda (equation 14, Fairall et al, 1996 Cool skin and warm layer)
                lambda=6._r8/(1._r8+(numer_cool*q_cool/ustar**4)**.75_r8)**.333_r8
                cool_thick=lambda*viscosity_h2o/(dsqrt(rho_air/rho_h2o)*ustar)
            else
                lambda=6.0_r8
                cool_thick=dmin1(0.01_r8,lambda*viscosity_h2o/(dsqrt(rho_air/rho_h2o)*ustar))
            endif
            delta_t_cool=qcol*cool_thick/tc_h2o                                  ! Eq.12 Cool skin
            delta_q_cool=clausius*delta_t_cool
        endif
        if (debug) write(debug_unit,'(/t10,a,1x,i1)')'END ITERATION:',iter
    enddo l2
    
    if (debug)write(debug_unit,'(t10,a)')'END ITERATIONS'
    
    !this is end of ASL subroutine in AERCOARE; code below will be in bulk_flux after ASL called.
!   final SST for this hour
    sst=sst_warm-delta_t_cool
    tau=rho_air*ustar*ustar*wind_speed/delta_u
    hflux_sensible=-rho_air*cp*ustar*tstar
    hflux_latent=-rho_air*latent_heat_vap*ustar*qstar
    
    if (debug) then
!       sensible heat flux
        write(debug_unit,'(/t12,a/t11,a)')'SENSIBLE      AIR','HEAT FLUX    DENSITY    Cp          U*             T*'
        write(debug_unit,'(t10,f10.3,t24,f7.3,t33,f6.1,t40,g13.6,1x,g13.6)')hflux_sensible,rho_air,cp,ustar,tstar
        
!       latent heat flux
        write(debug_unit,'(/t12,a/t11,a)')'LATENT       AIR        LATENT HEAT',&
            'HEAT FLUX    DENSITY   OF VAPORIZATION          U*           Q*'
        write(debug_unit,'(t10,f10.3,t24,f7.3,t35,g13.6,t53,g13.6,1x,g13.6)')hflux_latent,rho_air,latent_heat_vap,ustar,qstar
    endif
    
    
!   heat flux due to rainfall
!   check to see if these variables needed
    diffuse_h2o=2.11e-5_r8*(airtempk/273.15_r8)**1.94_r8 !water vapor diffusivity
    diffuse_heat=(1._r8+3.309e-3_r8*airtempc-1.44e-6_r8*airtempc*airtempc)*0.02411/(rho_air*cp) !heat diffusivity
    !dqs_dt=q_air*latent_heat_vap/(r*(airtempk)**2)
    wet_bulb_fact=1._r8/(1._r8+(clausius*latent_heat_vap*diffuse_h2o)/(cp*diffuse_heat)) !wet bub factor
    hflux_rain=rain*wet_bulb_fact*cp_h2o*((sst-airtempc)+(q_sea-q_air-delta_q_cool)*latent_heat_vap/cp)/3600._r8
    

!   momentum flux due to rainfall
    tau_rain=0.85_r8*rain/3600._r8*wind_speed
    
!   Webb correction
    wbar=-1.61_r8*ustar*qstar/(1._r8+1.61_r8*q_air)-ustar*tstar/(airtempk)
!!!!!!
!   previous hour's variables for next time step warm layer calculations
!   maybe use save statement instead of variables?
    sst_prev=sst !osdata1(osvars(tsea_var)%readvar,1,last_hr,ios(last_day))
    tau_prev=tau
    hflux_latent_prev=hflux_latent
    hflux_sensible_prev=hflux_sensible
    hflux_rain_prev=hflux_rain
    
!   start with transfer coefficients after old assigments
    !cd=(ustar/delta_u)**2
    !ch=ustar*tstar/(delta_u*(airtempc-sst+0.0098_r8*temp_ht))
    !ce=ustar*qstar/(delta_u*(q_air-q_sea+delta_t_cool))
    
!   assign certain variables to sfc_data (similiar to process_met in AERCOARE)
    
!   sensible heat flux
    sfc_data(d)%hflux(h)=hflux_sensible
    
!   calculate Bowen ratio as ratio of sensible/latent heat flux (not used by AERMOD)
    if ((hflux_sensible > 0.0_r8 .and. hflux_latent < 0.0_r8) .or. (hflux_sensible < 0.0_r8 .and. hflux_latent > 0.0_r8)) then
        bowen=-1.0_r8
    elseif (hflux_latent == 0.0_r8) then
        if (hflux_sensible > 0.0_r8) then
            bowen=10.0_r8
        elseif (hflux_sensible < 0.0_r8) then
            bowen=-1.0_r8
        else
            bowen=0.0_r8
        endif
    else
        bowen=hflux_sensible/hflux_latent
    endif
    bowen=dmin1(bowen,10.0_r8)
    !if (bowen >= 0.0_r8) bowen=dmax1(0.01_r8,bowen)
!   set Bowen ratio to a minimum of 0.01
    bowen=dmax1(0.01_r8,bowen)
    sfc_data(d)%bowen(h)=bowen
    if (debug)write(debug_unit,'(t10,a,1x,f6.2)')'BOWEN RATIO SET TO:',sfc_data(d)%bowen(h)
!   albedo
    sfc_data(d)%albedo(h)=coare_albedo
    if (debug)write(debug_unit,'(t10,a,1x,f6.2)')'ALBEDO SET TO:',sfc_data(d)%albedo(h)
!   zo
    sfc_data(d)%zo(h)=zo
    if (debug)write(debug_unit,'(t10,a,1x,f6.2)')'ZO SET TO:',sfc_data(d)%zo(h)
    
!   Monin-Obukhov length (use last iterated value)
    if (dabs(mol) < min_l) then
        mol1=dsign(1.0_r8,mol)*min_l
        if (debug) write(debug_unit,'(/t10,a,1x,g13.6,1x,a1,1x,g13.6)')'MONIN-OBUKHOV LENGTH',mol,'<',min_l
    else
        mol1=mol
    endif
    if (debug) write(debug_unit,'(t10,a,1x,g13.6)')'SET MONIN-OBUKHOV LENGTH TO:',mol1
    
!   reset u*
    sfc_data(d)%ustar(h)=ustar*(mol1/mol)**(1.0_r8/3.0_r8)
    if (debug) write(debug_unit,'(/t10,a,1x,g13.6)')'RESET U* BASED RATIO OF NEW L/OLD L:',sfc_data(d)%ustar(h)
        
    
!   limit max L to absolute value of 8888
    if (dabs(mol1) > 8888.0_r8) then
        sfc_data(d)%mol(h)=dsign(1.0_r8,mol1)*8888.0_r8
        if (debug) write(debug_unit,'(a)')'ABSOLUTE VALUE OF L > 8888, RESET'
    else
        sfc_data(d)%mol(h)=mol1
    endif
    if (debug) write(debug_unit,'(t10,a,1x,g13.6)')'SET FINAL MONIN-OBUKHOV LENGTH TO:',sfc_data(d)%mol(h)
    
!   JAT A047_MIX_DFAULT 3/26/24, set value for cbl array needed for mechanical mixing height subroutine
!   cbl(h,d) has not been set because stability subroutine not called for COARE processing so set it here
    if (sfc_data(d)%mol(h) < 0.0_r8) then
        cbl(h,d)=.true.
    else
        cbl(h,d)=.false.
    endif
...
!   only assign convective height when L < 0 (consistent with rest of AERMET)
    !if ((coare_mix(2) .or. coare_mix(3) .or. coare_mix(5)) .and. onmix) then
    !if ((coare_mix(1) .and. sfc_data(d)%mol(h) < 0.0_r8) .or. (coare_mix(2) .or. coare_mix(3) .or. coare_mix(5)) .and. onmix) then
    if (sfc_data(d)%mol(h) < 0.0_r8 .and. onmix .and. (coare_mix(1) .or. coare_mix(2) .or. coare_mix(3) .or. coare_mix(5))) then
!       convective mixing height set to input mixing height 
        mixht=dmax1(osdata1(osvars(mix_var)%readvar,1,h,ios(d)),zi_min)
        if (osdata1(osvars(mix_var)%readvar,1,h,ios(d)) < zi_min) write(msg_unit,formstr(2))adjustl(pathid(ipath)),&
            'I92',modnam,'CBL HT SET TO',zi_min,'FOR',date_hr_str
            
        if (debug) write(debug_unit,'(t10,2(a),f10.4,a,f6.1)')'CONVECTIVE MIXING HEIGHT SET TO MAXIMUM OF ',&
                trim(adjustl(pathid(os_ipath))),osdata1(osvars(mix_var)%readvar,1,h,ios(d)),&
                ' AND MINIMUM COARE MIXING HEIGHT ',zi_min
    
        sfc_data(d)%zic(h)=dmin1(mixht,conv_max)
        if (mixht > conv_max) write(msg_unit,formstr(2))adjustl(pathid(ipath)),&
            'I79',modnam,'CBL HT SET TO',conv_max,'FOR',date_hr_str
        
        if (debug) write(debug_unit,'(t10,2(a),f10.4,a,f6.1)')'CONVECTIVE MIXING HEIGHT SET TO MINIMUM OF ',&
            trim(adjustl(pathid(os_ipath))),mixht,' AND MAX CONVECTIVE HEIGHT ',conv_max
    endif
    
!   call mech_ht to set the mechanical mixing height
    !if (.not. coare_mix(1)) then
    call mech_ht(d,h)
    if (coare_mix(4) .or. coare_mix(6) .and. sfc_data(d)%mol(h) < 0.0_r8 ) sfc_data(d)%zic(h)=sfc_data(d)%zim(h)
    !endif
...
!       set only if L < 0
        if (sfc_data(d)%mol(h) < 0.0_r8) then
            ...
            if (debug) write(debug_unit,'(/t10,a,1x,f7.3)')'W* SET TO OBSERVED VALUE:',sfc_data(d)%wstar(h)
        endif
    !elseif (.not. coare_mix(1) .and. sfc_data(d)%zic(h) > 0.0_r8 .and. sfc_data(d)%mol(h) < 0.0_r8) then
    elseif (sfc_data(d)%zic(h) > 0.0_r8 .and. sfc_data(d)%mol(h) < 0.0_r8) then
        sfc_data(d)%wstar(h)=wstar(d,h)
    endif
...
!   theta lapse rate
    if (onvptg) then
        !       set only if L < 0
        if (sfc_data(d)%mol(h) < 0.0_r8)then
            sfc_data(d)%vptg(h)=osdata1(osvars(vptg_var)%readvar,1,h,ios(d))
            if (debug) write(debug_unit,'(/t10,a,1x,f7.3)')'THETA LAPSE RATE SET TO OBSERVED VALUE:',sfc_data(d)%vptg(h)
        endif
        !if (sfc_data(d)%vptg(h) < real(osvars(vptg_var)%missval,r8)*osvars(vptg_var)%conv) then
        !    if (debug) write(debug_unit,'(/t10,a,1x,f7.3,1x,a,1x,f7.3)')'THETA LAPSE RATE:',sfc_data(d)%vptg(h),&
        !       'LESS THAN DEFAULT MIN VALUE',real(osvars(vptg_var)%missval,r8)*osvars(vptg_var)%conv
        !    sfc_data(d)%vptg(h)=def_vptg
        !   if (debug) write(debug_unit,'(/t10,a,1x,f7.3)')'THETA LAPSE RATE SET TO DEFAULT VALUE:',sfc_data(d)%vptg(h)
        !endif
    else
        if (sfc_data(d)%mol(h) < 0) sfc_data(d)%vptg(h)=def_vptg        
        if (debug) write(debug_unit,'(/t10,a,1x,f7.3)')'THETA LAPSE RATE SET TO DEFAULT VALUE:',sfc_data(d)%vptg(h)
    endif
    
    if (debug) write(debug_unit,'(/t5,a)')'END COARE PROCESSING'
    return
    end subroutine coare1
!*********************************************************************************************************
!   JAT 9/6/22 A002_COARE; PSIU function
    real(kind=r8) function psiu(z_over_l)
!=========================================================================================================
!   FUNCTION PSIU
...
!   OUTPUT
!     
!   PSIU:       STABILITY FUNCTION
!
!   Variable definitions
!      
!   Real variables
!   z_over_l:       z overl L
!   x:              stability variable
!   y:              stability variable
!   psik:           stability variable
!   psic:           stability variable
!   f:              stability variable
!   c:              stability variable
!
!   Character variables
!   modnam:         Subroutine name
!=========================================================================================================
    !use onsite, only: rdow_var
    use file_units, only: debug_unit
    use main1, only: debug
    implicit none
    real(kind=r8), intent(in) :: z_over_L
    real(kind=r8) :: x,y,psik,psic,f,c
    character(len=10) :: modnam='PSIU' 
    
    if (debug) write(debug_unit,'(/t10,a,1x,a)')trim(adjustl(modnam)),'START CALCULATIONS'
...
!   stable form from Beljaars and Holtslag (1991)
    ...
    if (z_over_l < 0.0) then
        x=(1.0_r8 -15._r8*z_over_l)**0.25_r8 !Kansas unstable
        psik=2.0_r8*dlog((1.0_r8+x)/2.0_r8)+dlog((1.0_r8+x*x)/2.0_r8)-2.0_r8*datan(x)+2.0_r8*datan(1.0_r8)
        y=(1.0_r8-10.15_r8*z_over_l)**0.3333_r8  !Convective
        psic=1.5_r8*dlog((1.0_r8+y+y*y)/3.0_r8)-dsqrt(3.0_r8)*datan((1.0_r8+2.0_r8*y)/dsqrt(3.0_r8))+4.*&
           datan(1.0_r8)/dsqrt(3.0_r8)
        f=z_over_l*z_over_l/(1.+z_over_l*z_over_l)     
        psiu=(1.0_r8-f)*psik+f*psic
       
        if (debug) then
            write(debug_unit,'(/t10,a)')'CONVECTIVE CONDITIONS'
            write(debug_unit,'(/t10,a,1x,g13.6)')'X=',x
            write(debug_unit,'(t10,a,1x,g13.6)')'PSIK=',psik
            write(debug_unit,'(t10,a,1x,g13.6)')'Y=',y
            write(debug_unit,'(t10,a,1x,g13.6)')'PSIC=',psic
            write(debug_unit,'(t10,a,1x,g13.6)')'F=',f
        endif
    else
        c=dmin1(50.0_r8,0.35_r8*z_over_l)                       !Stable
        psiu=-((1.0_r8+1.0_r8*z_over_l)**1.0_r8+0.6667_r8*(z_over_l-14.28_r8)/dexp(c)+8.525_r8)
        if (debug) write(debug_unit,'(/t10,a)')'STABLE CONDITIONS'
    endif
    
    if (debug) then
        write(debug_unit,'(t10,a,1x,g13.6)')'PSIU=',psiu
        write(debug_unit,'(/t10,a,1x,a)')trim(adjustl(modnam)),'END CALCULATIONS'
    endif
    return
    end function psiu
!*********************************************************************************************************
!   JAT 11/3/22 A002_COARE; PSIT function
    real(kind=r8) function psit(z_over_l)
!=========================================================================================================
!   FUNCTION PSIU
!   CALCULATE STABILITY FUNCTION
!     
...
!   stable form from Beljaars and Holtslag (1991)
    ...
    if (z_over_l < 0.0) then
        x=(1.0_r8 -15._r8*z_over_l)**0.5_r8 !Kansas unstable
        psik=2.0_r8*dlog((1.0_r8+x)/2.0_r8)
        
        y=(1.0_r8-34.15_r8*z_over_l)**0.3333_r8  !Convective
        psic=1.5_r8*dlog((1.0_r8+y+y*y)/3.0_r8)-dsqrt(3.0_r8)*datan((1.0_r8+2.0_r8*y)/dsqrt(3.0_r8))+4.*datan(1.0_r8)/dsqrt(3.0_r8)
        f=z_over_l*z_over_l/(1.+z_over_l*z_over_l)
        psit=(1.0_r8-f)*psik+f*psic
        if (debug) then
            write(debug_unit,'(/t10,a)')'CONVECTIVE CONDITIONS'
            write(debug_unit,'(/t10,a,1x,g13.6)')'X=',x
            write(debug_unit,'(t10,a,1x,g13.6)')'PSIK=',psik
        ...
        c=dmin1(50.0_r8,0.35_r8*z_over_l)                       !Stable
        if (debug) write(debug_unit,'(/t10,a)')'STABLE CONDITIONS'
        psit=-((1.0_r8+2.0_r8*z_over_l/3.0_r8)**1.5_r8+0.6667_r8*(z_over_l-14.28_r8)/dexp(c)+8.525_r8)
    endif
    
    if (debug) then
        write(debug_unit,'(t10,a,1x,g13.6)')'PSIT=',psit
        write(debug_unit,'(/t10,a,1x,a)')trim(adjustl(modnam)),'END CALCULATIONS'
    endif
    return
    end function psit
...
!   istation:       upper air station index
!   station_use:    upper air station index of station to use for sounding for the day
!
!   Real variables
...
!   rh_ht:          RH measurement height used for COARE; added 11/3/22 AOO2_COARE
!
!   Logical variables
...
!   havesst:        have SST data for the hour to execute COARE processing
!   have_rh:        have RH available for hour to execute COARE processing
!   lsub1:          denotes upper air data substituted from primary upper air site substitution requested
!   lsub2:          denotes upper air data substituted from secondary upper air site substitution requested
!
!   Character variables
!   formstr:        format for messages
...
!=========================================================================================================
    use, intrinsic :: iso_fortran_env,only : output_unit 
!   JAT 7/18/22 A002_COARE; use cp and vonk from main1
    !use main1, only: lpath,numdays,leapyr,lkey,days,r,g,msg_form
    use main1, only: lpath,numdays,leapyr,lkey,days,r,g,msg_form,vonk,cp
    use file_units, only: msg_unit,sfc_charfile1,sfc_charfile2
!   JAT 2/11/2025 A058 (A058_SOUND); add nstation, station_type, and lsecondary to upperair use list
    !use upperair, only: upgmt2lst,uplat,uplon,updata,sound_info,nsnd,upstage1
    use upperair, only: upgmt2lst,uplat,uplon,updata,sound_info,nsnd,upstage1,lsecondary,nstation,station_type
    use surface, only: nws_hgt,sfvars,sfc_init,nsfc
!   JAT 7/5/22 A002_COARE; add COARE variables
!   JAT 3/10/23 A029_SUBHR_SUM; add samt_var
...
!        nos,osstage1,cloud_var
        nos,osstage1,cloud_var,tsea_var,sstd_var,hwav_var,twav_var,rdow_var
!   JAT 8/4/22 A002_COARE; add variables from COARE module
!   use coare, only:  run_coare,coare1
    
    implicit none
    integer(kind=4) :: nsite=1 !at least 1 site
!   JAT 2/11/2025 A058 (A058_SOUND); make my_zone a 2-D array to account for 2 stations
    !integer(kind=4) :: my_zone=0
    integer(kind=4) :: my_zone(2)=0
!   JAT 1/23/23 A023_UPPER_LST; add temp_upgmt and temp_pblgmt
    !integer(kind=4) ::d,h,idattim(8),y,m,imon(2),idy(2),iday1,iw1,iw2,iw3,iswitch,cblhr1,v,ilev,isite,!,iup,ios,isf,isf1,isnd,isite
!   JAT 2/11/2025 A058 (A058_SOUND); add istation for multiple upper air station processing and make temp_upgmt a 2-element array 
!   to accomodate multiple stations; add variable station_use for upper air data
!   JAT 1/21/2026 A058 (A058_SOUND); move temp_upgmt to main part of module for use in upp_dates
    !integer(kind=4) ::d,h,idattim(8),y,m,imon(2),idy(2),iday1,iw1,iw2,iw3,iswitch,cblhr1,v,ilev,isite,temp_upgmt,temp_pblgmt
    !integer(kind=4) ::d,h,idattim(8),y,m,imon(2),idy(2),iday1,iw1,iw2,iw3,iswitch,cblhr1,v,ilev,isite,temp_upgmt(2),temp_pblgmt,&
    integer(kind=4) ::d,h,idattim(8),y,m,imon(2),idy(2),iday1,iw1,iw2,iw3,iswitch,cblhr1,v,ilev,isite,temp_pblgmt,&
        istation,station_use
    integer(kind=4) :: sdg2use(-12:12)
!   JAT 11/3/22 A002_COARE; add rh_ht for COARE calculations
    real(kind=r8) :: rhomin,skyfract,ustar_ratio,temp_ustar(2),temp_l,temp_thetastar,rh_ht
...
!   JAT 7/5/22 A002_COARE; add variable ol_ow_msg for overland site with COARE overwater variables
    !logical :: leap,calcflux,stable_rate,bulkri,reset,calc_conv,firstcbl,posflux,stablecalcs,lcycle,ostemp,lbadsc
!   JAT 2/12/24 AO53_COARE_ZO; add variable havesst to check to see if have sst data for the hour
!    logical :: leap,calcflux,stable_rate,bulkri,reset,calc_conv,firstcbl,posflux,stablecalcs,lcycle,ostemp,lbadsc,ol_ow_msg
!   JAT 2/24/24 AO53_COARE_ZO; add have_rh to check to see if valid RH available for hour
!   JAT 2/19/2025 A058 (A058_SOUND); add lsub1 and lsub2 for upper air processing

    logical :: leap,calcflux,stable_rate,bulkri,reset,calc_conv,firstcbl,posflux,stablecalcs,lcycle,ostemp,lbadsc,ol_ow_msg,&
        havesst,have_rh,lsub1,lsub2
        !havesst,have_rh
    logical, allocatable, dimension(:,:) :: wind_reset
    character(len=10) :: modnam='PBL_PROC'
    character(len=20) :: date_hr_str
!   JAT 1/24/23 A023_UPPER_LST; increase formstr from 12 to 13 to accomodate new message regarding GMT to LST for upper
!   air and pbl location
    !character(len=80) :: formstr(12)
    character(len=80) :: formstr(13)
    character(len=100) :: debugform(13)
...
!      JAT 7/5/22 A002_COARE; data is overland but COARE overwater variables present
...
!   JAT 1/23/23 A023_UPPER_LST; GMT TO LST conversion based on longitude
    write(formstr(13),'(2(a))')trim(adjustl(msg_form)),'a,1x,i3,1x,a)'
...
!   JAT 7/27/23 A045_GFORTRAN; remove comma
...
!   JAT 7/5/22 A002_COARE; initialize ol_ow_msg
    ol_ow_msg=.false.
...
!   JAT 3/13/2025 A058 (A058_SOUND); modify logic to use primary station
    
    if (((upstage1(1) .and. nsnd(4,1)-nsnd(5,1) == 0) .or. (.not. upstage1(1) .and. lstage(2) .and. nsnd(2,1) == 0)) &
        .and. nsfc(4) == 0.and. nos(2) == 0) then
...
!   JAT A002_COARE; alert user if data is overland but COARE overwater variables read in
!   only issue once, as done os_test
!   also add to list of variable that will not be used, I80 message
    if (overland .and. .not. osstage1) then
        
    v1: do v=5,nosvars
            if ((v == tsea_var .or. v == sstd_var .or. v == hwav_var .or. v == twav_var .or. v == rdow_var) .and. &
                osvars(v)%lread .and. .not. ol_ow_msg) then
                ol_ow_msg=.true.
                write(msg_unit,formstr(9))adjustl(pathid(ipath)),'W60',modnam,&
                    'LOCATION IS OVERLAND BUT DATA FLAG INDICATES OVERWATER'
            endif 
            ...
            if (osvars(v)%lread .and. (v == l_var .or. v == hflux_var .or. v == wstar_var .or. v == vptg_var .or. v == zo_var &
            .or. v == bo_var .or. v == alb_var .or. v == lflux_var .or. v == tsea_var .or. v == sstd_var .or. v == hwav_var &
                .or. v == twav_var .or. v == rdow_var)) then
!   JAT A043_COARE 3/11/2024; as part of A043_COARE, take out prognostic text string as overwater is now applicable to onsite and prognostic
                ...
                write(msg_unit,formstr(7))adjustl(pathid(ipath)),'I80',modnam,'LOCATION IS OVERLAND; VARIABLE',&
                    trim(adjustl(osvars(v)%varname)),'WILL NOT BE USED FROM STAGE 1 BUT CALCULATED IN STAGE 2'
            endif
        enddo v1
    endif
    
!   JAT A043_COARE 3/11/2024; issue similar message as above for COARE and certain variables read in and location is overwater
    if (run_coare) then
        v2: do v=5,nosvars
            !if (osvars(v)%lread .and. (v == l_var .or. v == hflux_var .or. v == wstar_var .or. v == vptg_var .or. v == zo_var &
            !.or. v == bo_var .or. v == alb_var .or. v == lflux_var)) then
            if (osvars(v)%lread .and. (v == l_var .or. v == hflux_var .or. v == wstar_var .or. v == vptg_var .or. v == zo_var &
            .or. v == bo_var .or. v == alb_var .or. v == ustar_var .or. &
                (v == mix_var .and. (coare_mix(4) .or. coare_mix(6))))) then
                write(msg_unit,formstr(7))adjustl(pathid(ipath)),'I80',modnam,'COARE PROCESSING REQUESTED; VARIABLE',&
                    trim(adjustl(osvars(v)%varname)),'WILL NOT BE USED FROM STAGE 1 BUT CALCULATED IN STAGE 2 COARE'
            endif
        enddo v2    
    endif
...
!   JAT 1/10/23 A002_COARE; do not issue a message if running coare
...
!   JAT 4/9/2025 A058 (A058_SOUND); allocate pbl_obs based on if secondary upper air station being used
    if (lsecondary) then
        allocate(pbl_obs(5,npbldays))
    else
...
!   JAT 2/12/2025 A058 (A058_SOUND); allocate up_sunrise and up_sunset based on 
!   number of days and now number of upper air stations
    !allocate(up_sunrise(npbldays))
    !allocate(up_sunset(npbldays))
    allocate(up_sunrise(npbldays,nstation))
    allocate(up_sunset(npbldays,nstation))
    ...
    if (lpath(2)) then
!       JAT 6/9/2025 A058 (A058_SOUND); allocate iup and isnd to npbldays with plus/minus day_window if substituting. if not substituting, just npbldays
        allocate(iup(npbldays))
        allocate(isnd(npbldays))
        !if (sub_upper) then
        !   allocate(iup(day_window(1):npbldays+day_window(2)))
        !   allocate(isnd(day_window(1):npbldays+day_window(2)))
        !else
        !   allocate(iup(npbldays))
  !          allocate(isnd(npbldays))
        !endif
...
!   JAT 2/11/2025 A058 (A058_SOUND); account for possibility of two stations, loop through code below for nstation
...
!   JAT 8/25/22 A016_GMT2LST; only do the sounding times if reading upper air data
!    if (.not. lactions(6)) then
    s3: do istation=1,nstation
        if (lpath(2) .and. .not. lactions(6)) then
...
!       because longitudes in old AERMET are opposite of real world.
            ...
            if (uplon(istation) < 0.0_r8) then  !western hemisphere
                my_zone(istation)=-int(-uplon(istation)/15._r8+0.5_r8)
            elseif (uplon(istation) > 0.0_r8) then !eastern hemisphere
                my_zone(istation)=-int(-uplon(istation)/15._r8-0.5_r8)
            else
                my_zone(istation)=0 !prime meridian
            endif
            my_sounding(istation)=sdg2use(my_zone(istation))
!           JAT A015_SOUND 8/15/22; simply set target_hr_gmt to absolute value of my_sounding and use absolute value of my_sounding for snd_hr_gmt
            !target_hr_gmt=abs(my_sounding)
            target_hr_gmt(istation)=abs(my_sounding(istation))
...
!       sounding windows
        ...
        !snd_hr_gmt(2)=my_sounding+snding_win(2)
            !snd_hr_gmt(1)=abs(my_sounding)+snding_win(1)
            !snd_hr_gmt(2)=abs(my_sounding)+snding_win(2)
            snd_hr_gmt(1,istation)=abs(my_sounding(istation))+snding_win(1) !A058
            snd_hr_gmt(2,istation)=abs(my_sounding(istation))+snding_win(2) !A058
!           JAT A015_SOUND 8/15/22; if snd_hr_gmt(1) is < 0 then reset to 25-snd_hr_gmt(1) to make a later hour for the day before
!           likewise if snd_hr_gmt(2) is > 24 then reset to 26-snd_hr_gmt(2) to make it early hour for next day
            !if (snd_hr_gmt(1) < 0) snd_hr_gmt(1)=25+snd_hr_gmt(1)
            !if (snd_hr_gmt(2) > 24) snd_hr_gmt(2)=26-snd_hr_gmt(2)
            if (snd_hr_gmt(1,istation) < 0) snd_hr_gmt(1,istation)=25+snd_hr_gmt(1,istation) !A058
            if (snd_hr_gmt(2,istation) > 24) snd_hr_gmt(2,istation)=26-snd_hr_gmt(2,istation) !A058
!           debug information 
!           JAT 2/11/2025 A058 (A058_SOUND); specify if primary or secondary station if secondary station specified
            ...
            !    '(',snding_win(2),')'
            if (debug) then
                if (lsecondary) write(debug_unit,'(/t10,a,1x,a)')trim(adjustl(station_type(istation))),'UPPER AIR STATION'
                write(debug_unit,debugform(11))'UPPER AIR SOUNDING WINDOW INFORMATION',&
                'UPPER LON    MY_ZONE  MY_SOUNDING    WINDOW BEGIN      TARGET HOUR         WINDOW END',&
                uplon(istation),my_zone(istation),my_sounding(istation),snd_hr_gmt(1,istation),'(',snding_win(1),')',&
                target_hr_gmt(istation),snd_hr_gmt(2,istation),'(',snding_win(2),')'
            endif
        endif
    
!       JAT 1/23/23 A023_UPPER_LST; If upper air data is in local time, sun angle calculations will be incorrect
!       because sundat subroutine assumes the upper air data was in GMT originally in stage 1
!       if upgmt2lst is 0 and the longitude doesn't agree, use a temporary variable for sundat.
!       when comparing the temporary conversion and upgmt2lst, allow for a 1-hour difference
!       use temp_upgmt in sundat
!       use negative of longitude since western (eastern) hemisphere GMT to LST in AERMET is positive (negative)
        
!       JAT 2/11/2025 A058 (A058_SOUND); modify code below for istation
        temp_upgmt(istation)=int(-uplon(istation)/15.0_r8)
                
        if (upgmt2lst(istation) == 0) then
            if (abs(temp_upgmt(istation)) <= 1) then
!               set temp_upgmt to 0
                temp_upgmt(istation)=0
            else
                if (lsecondary) then
                    if (istation == 1) then
                        write(msg_unit,formstr(13))adjustl(pathid(ipath)),'W88',modnam,'PRIMARY UPPER AIR GMT TO LST',&
                            upgmt2lst(istation),'DOES NOT ALIGN WITH UPPER AIR LONGITUDE'
                        write(msg_unit,formstr(13))adjustl(pathid(ipath)),'I91',modnam,&
                            'PRIMARY UPPER AIR SUNRISE/SUNSET GMT TO LST',temp_upgmt(istation),'BASED ON UPPER AIR LONGITUDE'
                    else
                        write(msg_unit,formstr(13))adjustl(pathid(ipath)),'W88',modnam,'SECONDARY UPPER AIR GMT TO LST',&
                            upgmt2lst(istation),'DOES NOT ALIGN WITH UPPER AIR LONGITUDE'
                        write(msg_unit,formstr(13))adjustl(pathid(ipath)),'I91',modnam,&
                            'SECONDARY UPPER AIR SUNRISE/SUNSET GMT TO LST',temp_upgmt(istation),'BASED ON UPPER AIR LONGITUDE'
                    endif
                    
                else
                    write(msg_unit,formstr(13))adjustl(pathid(ipath)),'W88',modnam,'UPPER AIR GMT TO LST',upgmt2lst(istation),&
                        'DOES NOT ALIGN WITH UPPER AIR LONGITUDE'
                    write(msg_unit,formstr(13))adjustl(pathid(ipath)),'I91',modnam,&
                        'UPPER AIR SUNRISE/SUNSET GMT TO LST',temp_upgmt(istation),'BASED ON UPPER AIR LONGITUDE'
                endif
            endif
        elseif (upgmt2lst(istation) /= 0) then
!           set temp_upgmt to upgmt2lst
            temp_upgmt(istation)=upgmt2lst(istation)
        endif
!       temp_upgmt=int(-uplon/15.0_r8)
!                
!        if (upgmt2lst == 0) then
!            if (abs(temp_upgmt) <= 1) then
!!               set temp_upgmt to 0
!                temp_upgmt=0
!            else
!                write(msg_unit,formstr(13))adjustl(pathid(ipath)),'W88',modnam,'UPPER AIR GMT TO LST',upgmt2lst,&
!                    'DOES NOT ALIGN WITH UPPER AIR LONGITUDE'
!                write(msg_unit,formstr(13))adjustl(pathid(ipath)),'I91',modnam,&
!                    'UPPER AIR SUNRISE/SUNSET GMT TO LST',temp_upgmt,'BASED ON UPPER AIR LONGITUDE'
!            endif
!        elseif (upgmt2lst /= 0) then
!!           set temp_upgmt to upgmt2lst
!            temp_upgmt=upgmt2lst
!        endif
    enddo s3
    
!   do the same for pblgmt2lst
    temp_pblgmt=int(-pbllon/15.0_r8)
                
    if (pblgmt2lst == 0) then
        if (abs(temp_pblgmt) <= 1) then
!           set temp_upgmt to 0
            temp_pblgmt=0
        else
            write(msg_unit,formstr(13))adjustl(pathid(ipath)),'W88',modnam,'LOCAL GMT TO LST',pblgmt2lst,&
                'DOES NOT ALIGN WITH LOCAL LONGITUDE'
            write(msg_unit,formstr(13))adjustl(pathid(ipath)),'I91',modnam,&
                'LOCAL AIR SUNRISE/SUNSET GMT TO LST',temp_pblgmt,'BASED ON LOCAL LONGITUDE'
        endif
    elseif (pblgmt2lst /= 0) then
!       set temp_upgmt to upgmt2lst
        temp_pblgmt=pblgmt2lst
    endif   
...
!               JAT 1/24/23 A023_UPPER_LST; use temp_pblgmt
                !call sundat(pbllat,pbllon,pblgmt2lst,sfc_data(iday1)%jday,local_sunrise(iday1),local_sunset(iday1),&
                !    sun_ang(:,iday1))

                call sundat(pbllat,pbllon,temp_pblgmt,sfc_data(iday1)%jday,local_sunrise(iday1),local_sunset(iday1),&
                    sun_ang(:,iday1))
!               upper air sunrise/sunset
!               JAT 8/25/22 A016_GMT2LST; only call sundat for upper air data if upper air data read in
...
!               when comparing the temporary conversion and upgmt2lst, allow for a 1-hour difference
!               use temp_upgmt in sundat
                !if (lpath(2)) call sundat(uplat,uplon,upgmt2lst,sfc_data(iday1)%jday,up_sunrise(iday1),up_sunset(iday1))
!               JAT 2/12/2025 A058 (A058_SOUND); loop sundat through number of stations
                !if (lpath(2)) call sundat(uplat,uplon,temp_upgmt,sfc_data(iday1)%jday,up_sunrise(iday1),up_sunset(iday1))
                if (lpath(2)) then
    s2:             do istation=1,nstation
                        call sundat(uplat(istation),uplon(istation),temp_upgmt(istation),sfc_data(iday1)%jday,&
                            up_sunrise(iday1,istation),up_sunset(iday1,istation))
                    enddo s2
                endif
...
!   JAT 1/9/23 A002_COARE; only get surface characteristics when not running COARE
    if (.not. run_coare) then
        if (sec_sc)nsite=2
    s1:     do isite=1,nsite
...
!           JAT 2/19/2025 A058 (A058_SOUND); add index to array of which station to use
            iswitch=2
            !call check_obs(iswitch,d)
            call check_obs(iswitch,d,station_use,lsub1,lsub2)
            if (up_obs(d)) then
...
!                        call read_sound(d)
!               endif
                if (sound_info(iup(d),station_use)%nlevels(isnd(d)) == 1 .and. &
                    updata(2,1,isnd(d),iup(d),station_use) == 0.0_r8) then
!                   reset to having no sounding for the day
                    up_obs(d)=.false.
!                   if only 1 level in the sounding and it is the surface, issue message and skip
!                   post 21DRF, only issue the message if mixing heights not read
                    ...
                    call read_sound(d,station_use,lsub1)
                endif
            else
!               write to message file, no sounding
!               post 21DRF, only write message if no mixing heights
!               JAT 2/26/24 AO53_COARE_ZO; only write missing sounding message if not running COARE
!                if (.not. osvars(mix_var)%lread)write(msg_unit,formstr(6))adjustl(pathid(ipath)),'I78',modnam,&
                if (.not. osvars(mix_var)%lread .and. .not. run_coare)write(msg_unit,formstr(6))adjustl(pathid(ipath)),'I78',&
                    modnam,'NO SOUNDING AVAILABLE FOR DATE',sfc_data(d)%sfcdate,'NO CONVECTIVE PARAMETERS WILL BE CALCULATED'
            endif   
!           JAT 2/26/24 AO53_COARE_ZO; only increment n_conv_miss if not running COARE
            !if (.not. up_obs(d) .and. .not. osvars(mix_var)%lread) n_conv_miss=n_conv_miss+1
            if (.not. up_obs(d) .and. .not. osvars(mix_var)%lread .and. .not. run_coare) n_conv_miss=n_conv_miss+1
...
!               JAT 1/11/23 A025_NO_OS_TEMP_PFL; add ostemp as argument to profile
!                call profile(d,h)
                call profile(d,h,ostemp)
                
!               assign/calculate pressure   
                call press(d,h,ostemp)    
!               assign/calculate dewpoint and RH
!   JAT 2/26/2024; AO53_COARE_ZO; new output variable have_rh from subroutine rh
                if (nws_obs(h,d) .or. (os_obs(h,d) .and. (osvars(rh_var)%lread .or. (osvars(temp_var)%lread .and. &
                     osvars(dewpt_var)%lread)))) call rh(d,h,rh_ht,have_rh) !call rh(d,h,rh_ht)
...
!               JAT 1/11/23 A002_COARE; Don't call stability subroutine if running COARE
                !call stability(d,h)
                if (.not. run_coare) call stability(d,h)
...
!               JAT 8/4/22 A002_COARE; initial start point for COARE calculations, before stability?
                if (run_coare) then
!               JAT 2/12/2024 A053_COARE_ZO; only call coare1 when SST is not missing
!                    call coare1(d,h,rh_ht)
                    call have_data(d,h,tsea_var,havesst)
!               JAT 2/26/2204 A053_COARE_Z0; only call coare1 when SST is not missing and winds are not missing or calm and RH not missing
                    if (windsrc(h,d) == 0 .or. (sfc_data(d)%wspd(h) < eps .and. sfc_data(d)%wdir(h) < eps) .or. &
                        (sfc_data(d)%wspd(h) > 900._r8 .and. sfc_data(d)%wdir(h) > 900.0_r8) .or. .not. have_rh .or. &
                        .not. havesst) then
                        write(msg_unit,formstr(5))adjustl(pathid(ipath)),'I72',modnam,date_hr_str,&
                        'KEY VARIABLES MISSING, SKIP COARE CALCULATIONS'
                    else
                        call coare1(d,h,rh_ht)
                    endif
                    !if (.not. coare_mix(1)) cycle h3
                    cycle h3
                endif
...
!                   apply check for convective mixing height to compare against conv_max
                    if (onmix) then
                        ...
                        sfc_data(d)%zic(h)=dmin1(osdata1(osvars(mix_var)%readvar,1,h,ios(d)),conv_max)
                        
                        !if (sfc_data(d)%zic(h) > conv_max) write(msg_unit,formstr(10))adjustl(pathid(ipath)),'I79',modnam,&
!                       JAT 6/1/23 A041 correct line truncation
                        if (osdata1(osvars(mix_var)%readvar,1,h,ios(d)) > conv_max) write(msg_unit,formstr(10))&
                            adjustl(pathid(ipath)),'I79',modnam,'CBL HT SET TO',conv_max,'FOR',date_hr_str
...
!                   calculate Monin-Obukhov length and heat flux if necessary
                    ...
                    if (.not. onmol) then
                        ...
                        !           use L=1100*ustar*ustar and normal L, take max
                                    sfc_data(d)%mol(h)=dmax1((1100._r8*sfc_data(d)%ustar(h)*sfc_data(d)%ustar(h)),&
...
!           flux for the day, don't calculate mixing heights
            ...
            if (calc_conv .and. (.not. posflux .or. dabs(sfc_data(d)%hflux(cblhr1)-(-999.0_r8)) < eps .or. &
                ...
                calc_conv=.false. 
                write(msg_unit,formstr(6))adjustl(pathid(ipath)),'I78',modnam,'HEAT FLUX CONDITIONS NOT MET FOR DATE:',&
...
!           if there are onsite mixing heights, convective heights are handled in the h3 loop
            ...
            if (calc_conv) then
!               JAT 4/21/2025 A058 (A058_SOUND); add station_use as input argument
                !if (up_obs(d)) call conv_ht(d,cblhr1) !then
                if (up_obs(d)) call conv_ht(d,station_use,cblhr1)
            endif
...
!   JAT A044 7/26/23; assign variable dir as real instead of integer
!    integer(kind=4) :: i,ilen1,istart,nfield1,isec,iflag,dir(2),iyr,iyr1
    integer(kind=4) :: i,ilen1,istart,nfield1,isec,iflag,iyr,iyr1
    real(kind=r8) :: dir(2)
    logical :: goodsec=.false.
...
!    write(formstr(4),'(2(a))')trim(adjustl(msg_form)),'5(a,1x),i4)'   
    write(formstr(4),'(2(a))')trim(adjustl(msg_form)),'5(a,1x),f7.2)'   
...
!       get directions
 ...
 !               if (dir(i) < 0 .or. dir(i) > 360) then
                if (dir(i) < 0.0_r8 .or. dir(i) > 360.0_r8) then
...
!                               JAT A044 7/26/23; dir is now real instead of integer
                                !tsectors(i,isec,iyr1)=real(dir(i),r8)
                                tsectors(i,isec,iyr1)=dir(i)
                            endif
                        enddo l3
                    endif
                endif
...
!   JAT 7/18/22 A002_COARE; COARE can also be allowed more than once 
    if (lfound) then
!       JAT 7/18/22 A002_COARE; modify to allow for COARE (item=8) 
        !if (litems(item) .and. item /=3) then
        if (litems(item) .and. item /=3 .and. item /=8) then
            iword1=1
            iword2=1
...
!   ignore duplicate error if action is NOPERS (not used)
    if (lfound) then
!       JAT 7/18/22 A002_COARE; change nactions below to 12 for NOPERS
...
!   valid pairings
!   JAT 7/18/22 A002_COARE add COARE combinations
!   JAT 5/5/2025 A058 (A058_SOUND) add items for upper air substitution
!   JAT 5/5/2025 A058 (A058_SOUND) modify item 8 logic below to account for new actions after 
!   COARE actions
!   item        action
!   1           1,2
!   2           3
...
!   6           8,9,12
!   7           10,11,12
!   8           13,14,15,16,17,18,19,21,22,23,24,25,26,27
!   9           28,29,30,31
...
!       JAT 7/18/22 A002_COARE add COARE combinations
!       JAT 5/5/2025 A058 (A058_SOUND) add UA_SUB combinations
        if ((item==1 .and. iaction > 2) .or. ((item == 2 .or. item == 4  .or. item == 5) .and.  iaction /= item +1).or. &
            (item == 3 .and. iaction /=4 .and. iaction /= 7) .or. (item == 6 .and. iaction /=8 .and. iaction /= 9 .and. &
            iaction /=12) .or. (item == 7 .and. iaction /=10 .and. iaction /=11 .and. iaction /=12) .or. &
            (item == 8 .and. (iaction < 13 .or. iaction > 27)) .or. (item == 9 .and. iaction < 28)) then
            !(item == 8 .and. iaction < 13)) then
...
!   JAT 2/10/2025 A058 (A058_SOUND); modify subroutine to be able to read UAWINDOW or UADAYWIN
    subroutine up_window(i1)
!=========================================================================================================
!   SUBROUTINE UP_WINDOW
!   THIS SUBROUTINE PROCESSES THE LINE UAWINDOW OR UADAYWIN FROM THE RUNSTREAM INPUT FILE
!
...
!   nfield1:        number fields on line minus 1 to account for keyword
!   iflag:          IO flag
!   times:          2-element array of hours or day before and after to check for sounding
!   threshtimes:    2-element array or hours or days not to exceed
!   j:              index value of UAWINDOW
!
!   Character variables
!   datafield:      data for DELTA_TEMP keyword read from input line
!   formstr:        formats for messages
!   keywrd1:        2-element array of keywords UAWINDOW or UADAYWIN
!   str1:            2-element array of text string (1=HOUR, 2=DAY)
!   str2:            2-element array of text string (1=HRS, 2=DAYS)
!   modnam:         Subroutine name
!=========================================================================================================
!   JAT 2/10/2025 A058 (A058_SOUND); add inpline1 as a variable to use from main1
    !use main1, only: nfield,getfields,writeunit,msg_form
    use main1, only: nfield,getfields,writeunit,msg_form,inpline1
    implicit none
    integer(kind=4), intent(in):: i1
!   JAT 2/10/2025 A058 (A058_SOUND); add threshtime as variable for UAWINDOW (12) and UADAYWIN (2)
    integer(kind=4) :: i,nfield1,iflag,times(2),threshtimes(2),j
    character(len=10) :: modnam='UP_WINDOW'
!   JAT 2/10/2025 A058 (A058_S0UND); increase size of formstr from 3 to 4
    !character(len=60) :: formstr(3)
    character(len=60) :: formstr(4)
    character(len=100), allocatable, dimension(:) :: datafield
!   JAT 2/10/2025 A058 (A058_S0UND); add keywrd1 and str so subroutine knows which keyword is being 
!   processed
    character(len=12) :: keywrd1(2)
    character(len=4):: str1(2),str2(2)
      
    data keywrd1 /'UAWINDOW','UADAYWIN'/
    data str1 /'HOUR','DAY'/
    data str2 /'HRS','DAYS'/
    data threshtimes /12,3/
    
!   JAT 2/10/2025 A058 (A058_S0UND); modify formats to do hours or days, UAWINDOW or UADAYWIN
!   1.  window exeeds 12 hours or beginning hour positive or ending hour is negative (window exceeds 2 days or beginning 
!       day is positive or ending day is negative)
    !write(formstr(1),'(2(a))')trim(adjustl(msg_form)),'a,1x,i3,1x,a)'
    write(formstr(1),'(2(a))')trim(adjustl(msg_form)),'2(a,1x),i3,1x,a,1x,i3,1x,a)'
    
!   JAT 2/10/2025 A058 (A058_S0UND); 
!   2. beginning hour or day is positive
    write(formstr(2),'(2(a))')trim(adjustl(msg_form)),'3(a,1x),i3,1x,a)'
!   JAT 2/10/2025 A058 (A058_S0UND)
!   3.  invalid value
    write(formstr(3),'(2(a))')trim(adjustl(msg_form)),'a,4(1x,a))'
!   2.  invalid value
!    write(formstr(2),'(2(a))')trim(adjustl(msg_form)),'a,1x,a)'

!   JAT 2/10/2025 A058 (A058_SOUND); modify for hour or day
!   4.  beginning hour > ending hour
    write(formstr(4),'(2(a))')trim(adjustl(msg_form)),'2(a,1x),i3,2(i1x,a),1x,i3)'
...
! JAT 2/10/2025 A058 (A058_SOUND); check inpline1 for UAWINDOW or UADAYWIN
    if (index(inpline1,'UAWINDOW') >0) then
        j=1
    else
        j=2
    endif

!   JAT 2/10/2025 A058 (A058_SOUND); modify code below to handle hours or days
!   get hours or day
!   beginning hour or day
    read(datafield(1),*,iostat=iflag)times(1)
    if (iflag == 0) then
        !if (times(1) < -12) then
        if (abs(times(1)) > threshtimes(j)) then
            !write(writeunit,formstr(1))adjustl(pathid(ipath)),'E05',modnam,'UAWINDOW HOUR',times(1),'EXCEEDS 12 HRS'
            write(writeunit,formstr(1))adjustl(pathid(ipath)),'E05',modnam,trim(adjustl(keywrd1(j))),trim(adjustl(str1(j))),&
                times(1),'EXCEEDS',threshtimes(j),trim(adjustl(str2(j)))
            lbad=.true.
        elseif (times(1) > 0) then
            ...
            write(writeunit,formstr(2))adjustl(pathid(ipath)),'E05',modnam,trim(adjustl(keywrd1(j))),'BEGINNING',&
                trim(adjustl(str1(j))),times(1),'IS POSITIVE'
            lbad=.true.
        endif
    else
        !write(writeunit,formstr(2))adjustl(pathid(ipath)),'E05',modnam,'INVALID VALUE FOR UAWINDOW HOUR:',&
        !   trim(adjustl(datafield(1)))
        write(writeunit,formstr(3))adjustl(pathid(ipath)),'E05',modnam,'INVALID VALUE FOR',trim(adjustl(keywrd1(j))),&
            trim(adjustl(str1(j))),':',trim(adjustl(datafield(1)))
        lbad=.true.
    endif
...
!   ending hour
    ...
    if (iflag == 0) then
        ...
        elseif (times(2) < 0) then
            ...
            write(writeunit,formstr(2))adjustl(pathid(ipath)),'E05',modnam,trim(adjustl(keywrd1(j))),'ENDING',&
                trim(adjustl(str1(j))),times(1),'IS NEGATIVE'
            lbad=.true.
        elseif (times(2) < times(1)) then
            ...
            write(writeunit,formstr(4))adjustl(pathid(ipath)),'E78',modnam,trim(adjustl(keywrd1(j))),'BEGINNING',&
                trim(adjustl(str1(j))),times(1),'IS GREATER THAN ENDING',trim(adjustl(str1(j))),times(2)
            lbad=.true.
        endif
    else
        !write(writeunit,formstr(2))adjustl(pathid(ipath)),'E05',modnam,'INVALID VALUE FOR UAWINDOW HOUR:',&
...
!   JAT 2/10/2025 A058 (A058_SOUND); set snding_win or day_window
    if (.not. lbad) then
    l1:    do i=1,2
            if (j==1) then
                snding_win(i)=times(i)
            else
                day_window(i)=times(i)
            endif
        enddo l1
    endif
...
!                       an error is issued to allow the user to check for syntax in input file
            ...
            enddo y1
            if (lbadsc)lbad=.true.
!           JAT 9/7/22 A011_AERSURF2; if AERSURF or AERSURF2 then check for correct keywords.  AERSURF2 file can
!           have the primary or secondary keywords (with a mix of say FREQ_SECT and SECTORS2).bring code up from below in subroutine
!           to do check earlier for iswitch=1
            if (lpblkeywords(isite+14)) then
...
!                               if isite = 1 then must be FREQ_SECT, SECTORS, SITE_CHAR
!                               if isite = 2 then can be FREQ_SECT, FREC_SECT2, SECTORS, SECTORS2, AND SITE_CHAR, SITE_CHAR2
                                
                                if ((trim(adjustl(akey)) == trim(adjustl(sc_keys(isite,ikey1))))) then
                                    lfound=.true.
                                    lfound2(isite,ikey1)=.true.
                                else if (isite == 2) then
                                    if (trim(adjustl(akey)) == trim(adjustl(sc_keys(isite,ikey1))) .or. & !site is secondary site so can be either set of keywords
                                        trim(adjustl(akey)) == trim(adjustl(sc_keys(isite-1,ikey1)))) then
                                        lfound=.true.
                                        lfound2(isite,ikey1)=.true.
                                    else
                                        ikey1=ikey1+1       
...
!                                   JAT 6/1/23 A041 correct truncated line
                                    write(writeunit,formstr(2))adjustl(pathid(ipath)),'E75',modnam,&
                                        'FREQ_SECT OR FREQ_SECT2 NOT FOUND'
                                    lbad=.true.
                                    eof=1
                                endif
                            endif
...
!               check to see if keywords for site found
    ...
    l3:             do k=1,3
                        lbadsc=.true.
!                                   JAT 6/1/23 A041 correct truncated line
                        if (.not. lfound2(isite,k)) write(msg_unit,formstr(6)) adjustl(pathid(ipath)),'E73',modnam,&
                                'MISSING KEYWORD ',trim(adjustl(sc_keys(isite,k))),'FOR',trim(adjustl(sites(isite))),&
...
!                   but hasn't been done for the AERSURF files.
                    if (index(inpline1,'FREQ_SECT') > 0 .or. index(inpline1,'SITE_CHAR') > 0 .or. &
                        ...
                        enddo
!                       JAT 9/7/22 A011_AERSURF2; if the secondary site AERSURF2 file, and lfound is false, check against primary site keywords
!                       this won't apply to secondary characteristics entered via the control file because that is caught in pbl_test
                        if (isite == 2 .and. iunit == sfc_char_unit(isite) .and. .not. lfound) then 
                            ikey1=1
                            lfound=.false.
                            do while (ikey1 <=3 .and. .not. lfound)
                                if (trim(adjustl(akey)) == trim(adjustl(sc_keys(1,ikey1)))) then
                                    lfound=.true.
                                    lfound2(isite,ikey1)=.true.
                                else
                                    ikey1=ikey1+1
                        ...
                        if (lfound) then
!                           JAT 9/7/22 A011_AERSURF2; error messages handled above for iswitch 1, comment out
                            lfound1(ikey1)=.true.
                            !if (ikey1 >= 2 .and. .not. lfound1(1)) then !error FREQ_SECT should be first
            ...
            enddo
!           check to see if keywords for site found
!           JAT 9/7/22 A011_AERSURF2; no longer needed here, done above for iswitch 1
    !        if (.not. lfound2(isite,1) .or. .not. lfound2(isite,2) .or. .not. lfound2(isite,3)) then
    !l3:         do k=1,3
    !                lbadsc=.true.
...
!   JAT 1/9/23 A002_COARE; only flag as bad if not running COARE
    if (iswitch == 1) then
...
!   bad_coare:      denotes bad COARE processing
!   Character variables
!   formstr:        formats for messages
...
!   modnam:         Subroutine name
!=========================================================================================================
!   JAT A002_COARE add actions to main1 list
    !use main1, only: keywrd,writeunit,lpath,lkey,msg_form
!   JAT 6/11/2025 A058 (A058_SOUND); add use_second from main1
    use main1, only: keywrd,writeunit,lpath,lkey,msg_form,actions,use_second
    use file_units, only: msg_unit
!   JAT 5/5/2025 A058 (A058_SOUND); add lsecondary variable (secondary station read in)
    !use upperair, only: upstart,upend,updates,upgmt2lst,lupkeywords,upstage1
    use upperair, only: upstart,upend,updates,upgmt2lst,lupkeywords,upstage1,lsecondary,nstation,upstarts,upends,updates1
!   JAT 2/12/24 A053_COARE_ZO; add sfgmt2lst
!    use surface, only: sfstart,sfend,sfdates,lasos,iasos,asoscommdates,sfid,nws_hgt,sflat,sflon,lsfkeywords,sfstage1
    use surface, only: sfstart,sfend,sfdates,lasos,iasos,asoscommdates,sfid,nws_hgt,sflat,sflon,lsfkeywords,sfstage1,sfgmt2lst
!   JAT 7/19/22 A002_COARE; add sstd_var and tsea_var, wind_vars, dewpt_var, clht_var, rdow_var and RH var
!   JAT 12/5/2022 A014_METPREP_LOC; add osid to list of variables to use
!   JAT 2/12/24 A053_COARE_ZO; add osgmt2lst
    use onsite, only: osstart,osend,osdates,oslat,oslon,temp_var,cloud_var,inso_var,nrad_var,ustar_var,loskeywords,osstage1,&
        ...
        !mix_var,overland,onsite_init,sstd_var,tsea_var,wind_vars,rh_var,dattype,osid,dewpt_var,clht_var,rdow_var
        mix_var,overland,onsite_init,sstd_var,tsea_var,wind_vars,rh_var,dattype,osid,dewpt_var,clht_var,rdow_var,osgmt2lst
    implicit none
!   JAT 7/26/22 A002_COARE; add variable ipath1
    integer(kind=4):: i,j,ikey1,isurf,isite,ipath1,c
    !integer(kind=8), allocatable, dimension(:) :: start1
    !integer(kind=8), allocatable, dimension(:) :: end1
    
!   JAT 7/26/22 A002_COARE; add variable bad_coare to be used for COARE processing
    !logical lbaddate(3),lbadsc,usercloud,usertemp,use_os_ustar,use_os_cloud
    logical lbaddate(3),lbadsc,usercloud,usertemp,use_os_ustar,use_os_cloud,bad_coare,lfound
    !character(len=flength) :: fname
    character(len=6) :: str
...
!   JAT 7/19/22 A002_COARE; action chosen with no_coare action for COARE
    write(formstr(2),'(2(a))')trim(adjustl(msg_form)),'a,2(1x,a))'
...
!       no data overlap  
!       JAT 7/19/22 A002_COARE; COARE requested 
!       with overland data
!       JAT 7/26/22 A002_COARE; SSTDEPTH and TSEA both requested
!       JAT 3/5/2024 A053_COARE_ZO; no ZO option chosen
!       JAT 5/5/2025 A058 (A058_SOUND); secondary site not used for substitution
    write(formstr(5),'(2(a))')trim(adjustl(msg_form)),'a)'

!   6. keyword listed with site keywords
!       JAT 7/26/22 A002_COARE; conflicting options for COARE or variable not read in
!       JAT 1/3/23 A002_COARE; no input mixing heights for COARE mixing height option
...
!   JAT AO43_COARE 3/12/24; initialize run_coare_orig to value of run_coare
    run_coare_orig=run_coare
...
!   JAT A002_COARE; COARE options
!   check COARE options first as it will impact some checks later
    bad_coare=.false.
    if (litems(8)) then
        if (lpath(4)) then
            ipath1=4
...
!       if overland, ignore COARE option
        if (overland) then
!           JAT A043_COARE 3/12/24; reword message to say COARE detected, not requested because COARE NO_COARE can be selected
!            write(msg_unit,formstr(5))adjustl(pathid(ipath)),'W82',modnam,'COARE REQUESTED FOR OVERLAND DATA; COARE IGNORED'
            write(msg_unit,formstr(5))adjustl(pathid(ipath)),'W82',modnam,'COARE ITEM DETECTED FOR OVERLAND DATA; COARE IGNORED'
!           JAT A043_COARE 3/12/24, set run_coare_orig to true if COARE processing requested
            if (lactions(14)) run_coare_orig=.true.
        else
!           if NO_COARE selected with RUN_COARE, issue error
!           however, if NO_COARE selected with one of the other COARE options, issue warning that other option will be ignored
!           if NO_COARE not selected but one of the others are, then that will invoke COARE
!           this means can invoke COARE without RUN_COARE. can use COARE WARM, COARE NO_WARM, etc.
!           if RUN_COARE selected and the others are not, notify user that COARE run with defaults
            if (lactions(13)) then
    c1:         do i=14,27 !21
                    if (lactions(i)) then
!                       if RUN_COARE, issue error, otherwise issue warning
                        if (trim(adjustl(actions(i))) == 'RUN_COARE') then
                            write(msg_unit,formstr(2))adjustl(pathid(ipath)),'E83',modnam,'COARE ACTION',&
                            trim(adjustl(actions(i))),'SELECTED WITH NO_COARE ACTION'
                            lbad=.true.
                            bad_coare=.true.
                        else
                            write(msg_unit,formstr(2))adjustl(pathid(ipath)),'W83',modnam,'COARE ACTION',&
                            trim(adjustl(actions(i))),'SELECTED WITH NO_COARE ACTION; COARE NOT INVOKED'
                        endif
                    endif
                enddo c1
            endif
!           check to see if conflicting entries for the warm layer, cool skin and zo options
!           e.g., can't have warm layer and no warm layer action
            
!           warm layer actions
            if (lactions(15) .and. lactions(16)) then
                write(msg_unit,formstr(6))adjustl(pathid(ipath)),'E84',modnam,'COARE ACTION',&
                    trim(adjustl(actions(15))),'SELECTED WITH COARE ACTION',trim(adjustl(actions(16)))
                lbad=.true.
                bad_coare=.true.
            endif
            
!           cool skin option
            if (lactions(17) .and. lactions(18)) then
                write(msg_unit,formstr(6))adjustl(pathid(ipath)),'E84',modnam,'COARE ACTION',&
                    trim(adjustl(actions(17))),'SELECTED WITH COARE ACTION',trim(adjustl(actions(18)))
                lbad=.true.
                bad_coare=.true.
            endif
            
!           zo u* and zo u* with significant wave period
            if (lactions(19) .and. lactions(20)) then
                write(msg_unit,formstr(6))adjustl(pathid(ipath)),'E84',modnam,'COARE ACTION',&
                    trim(adjustl(actions(19))),'SELECTED WITH COARE ACTION',trim(adjustl(actions(20)))
                lbad=.true.
                bad_coare=.true.
            endif
            
!           zo u* and zo using sigificant wave period and significant wave height
            if (lactions(19) .and. lactions(21)) then
                write(msg_unit,formstr(6))adjustl(pathid(ipath)),'E84',modnam,'COARE ACTION',&
                    trim(adjustl(actions(19))),'SELECTED WITH COARE ACTION',trim(adjustl(actions(21)))
                lbad=.true.
                bad_coare=.true.
            endif
    
!           zo u* with significant wave period and zo using significant wave period and significant wave height
            if (lactions(20) .and. lactions(21)) then
                write(msg_unit,formstr(6))adjustl(pathid(ipath)),'E84',modnam,'COARE ACTION',&
                    trim(adjustl(actions(20))),'SELECTED WITH COARE ACTION',trim(adjustl(actions(21)))
                lbad=.true.
                bad_coare=.true.
            endif
            
!           JAT A053_COARE_Z0; 3/5/2024; issue error if no zo option chosen; issue A051
            if (.not. lactions(19) .and. .not. lactions(20) .and. .not. lactions(21)) then
                write(msg_unit,formstr(5))adjustl(pathid(ipath)),'E84',modnam,'NO COARE ZO OPTION SELECTED'
                lbad=.true.
                bad_coare=.true.
            endif
!           check the mixing height options
!           JAT 4/17/2024 A047 24XXX_dev_branch; correct last increment for loop
            lfound=.false.
!    c2:     do i=22,26
    c2:     do i=22,27
                if (lactions(i)) lfound=.true.
!               JAT 4/17/2024 A047 24XXX_dev_branch; only do check below if i < 27
                if (i < 27 .and. lactions(i)) then
    c3:             do c=i+1,27
!                        if (lactions(i) .and. lactions(c)) then
                        if (lactions(c)) then
                            write(msg_unit,formstr(6))adjustl(pathid(ipath)),'E84',modnam,'COARE ACTION',&
                                trim(adjustl(actions(i))),'SELECTED WITH COARE ACTION',trim(adjustl(actions(c)))
                            lbad=.true.
                            bad_coare=.true.
                        endif
                    enddo c3
            ...
            enddo c2
!           JAT 3/5/2024; A047_MIX_DFAULT; issue error if no mixing height option selected
            if (.not. lfound) then
                write(msg_unit,formstr(5))adjustl(pathid(ipath)),'E84',modnam,'NO MIXING HEIGHT OPTION SELECTED'
                    lbad=.true.
                    bad_coare=.true.
            endif
!           if no issues, set appropriate logical variables
            if (.not. bad_coare) then   
!               set run_coare to true if RUN_COARE or one of the warm layer, cool skin, or zo options
                if (lactions(14) .or. lactions(15) .or. lactions(16) .or. lactions(17) .or. lactions(18) .or. lactions(19) .or. &
                    lactions(20) .or. lactions(21)) run_coare=.true.
!               JAT A043_COARE 3/12/24; set run_coare_orig to value of run_coare
                run_coare_orig=run_coare
                if (run_coare) then
!                   check to see if minimal variables available, if not issue warning but continue
!                   JAT 6/1/23 A041 correct line truncation
                    if (.not. osvars(wind_vars(1))%lread .or. .not. osvars(wind_vars(2))%lread .or. .not. osvars(temp_var)%lread &
                        .or. .not. osvars(tsea_var)%lread) then
                        run_coare=.false.
                        if (.not. osvars(wind_vars(1))%lread)write(msg_unit,formstr(6))adjustl(pathid(ipath)),'W85',modnam,&
                            trim(adjustl(dattype(ipath1-3))),'VARIABLE',trim(adjustl(osvars(wind_vars(1))%varname)),&
                            'NOT READ, COARE WILL NOT BE INVOKED'
                        if (.not. osvars(wind_vars(2))%lread)write(msg_unit,formstr(6))adjustl(pathid(ipath)),'W85',modnam,&
                            trim(adjustl(dattype(ipath1-3))),'VARIABLE',trim(adjustl(osvars(wind_vars(2))%varname)),&
                            'NOT READ, COARE WILL NOT BE INVOKED'
                        if (.not. osvars(temp_var)%lread)write(msg_unit,formstr(6))adjustl(pathid(ipath)),'W85',modnam,&
                            trim(adjustl(dattype(ipath1-3))),'VARIABLE',trim(adjustl(osvars(temp_var)%varname)),&
                            'NOT READ, COARE WILL NOT BE INVOKED'
                        
                        if (.not. osvars(tsea_var)%lread)write(msg_unit,formstr(6))adjustl(pathid(ipath)),'W85',modnam,&
                            trim(adjustl(dattype(ipath1-3))),'VARIABLE',trim(adjustl(osvars(tsea_var)%varname)),&
                            'NOT READ, COARE WILL NOT BE INVOKED'
                    endif
!                   if RH not read check to see if dewpoint read in as it can be used to calculate RH
                    if (.not. osvars(rh_var)%lread .and. .not. osvars(dewpt_var)%lread .and. .not. osvars(temp_var)%lread) then
                        run_coare=.false.
                        if (.not. osvars(rh_var)%lread)write(msg_unit,formstr(6))adjustl(pathid(ipath)),'W85',modnam,&
                            trim(adjustl(dattype(ipath1-3))),'VARIABLE',trim(adjustl(osvars(rh_var)%varname)),&
                            'NOT READ, COARE WILL NOT BE INVOKED'
                        if (.not. osvars(dewpt_var)%lread)write(msg_unit,formstr(6))adjustl(pathid(ipath)),'W85',modnam,&
                            trim(adjustl(dattype(ipath1-3))),'VARIABLE',trim(adjustl(osvars(dewpt_var)%varname)),&
                            'NOT READ, COARE WILL NOT BE INVOKED'
                    endif
                endif
                if (run_coare) then
!                   JAT 3/19/24 A043_COARE; reset use_os_ustar to false in case adjust u* requested later
                    if (osvars(ustar_var)%lread) use_os_ustar=.false.
!                   warm layer
                    if (lactions(15)) warm_layer=.true.
                
!                   cool skin
                    if (lactions(17)) cool_skin=.true.
                
!                   if cool skin or warm layer option invoked then need insolation and downward longwave radiation


                    if (.not. osvars(inso_var)%lread .and. (warm_layer .or. cool_skin)) then
                        warm_layer=.false.
                        cool_skin=.false.
                        if (.not. osvars(inso_var)%lread)write(msg_unit,formstr(6))adjustl(pathid(ipath)),'W85',modnam,&
                            trim(adjustl(dattype(ipath1-3))),'VARIABLE',trim(adjustl(osvars(inso_var)%varname)),&
                            'NOT READ, COARE COOL SKIN OR WARM LAYER WILL NOT BE INVOKED'
                    endif
!                   if downward longwave radiation not availabe will need cloud cover and ceiling height
                    if (.not. osvars(rdow_var)%lread .and. .not. osvars(clht_var)%lread .and. .not. osvars(cloud_var)%lread .and. &
                        (warm_layer .or. cool_skin)) then
                        warm_layer=.false.
                        cool_skin=.false.
                        if (.not. osvars(clht_var)%lread)write(msg_unit,formstr(6))adjustl(pathid(ipath)),'W85',modnam,&
                            trim(adjustl(dattype(ipath1-3))),'VARIABLE',trim(adjustl(osvars(clht_var)%varname)),&
                            'NOT READ, COARE COOL SKIN OR WARM LAYER WILL NOT BE INVOKED'
                        if (.not. osvars(cloud_var)%lread)write(msg_unit,formstr(6))adjustl(pathid(ipath)),'W85',modnam,&
                            trim(adjustl(dattype(ipath1-3))),'VARIABLE',trim(adjustl(osvars(cloud_var)%varname)),&
                            'NOT READ, COARE COOL SKIN OR WARM LAYER WILL NOT BE INVOKED'
                    endif
!                   zo options
                    if (lactions(19)) coare_zo(1)=.true. !zo based on u*
                    if (lactions(20)) coare_zo(2)=.true. !zo based on u* and significant wave period
                    if (lactions(21))  coare_zo(3)=.true. !zo based on significant wave period and height
                    
!                   get the mixing height option chosen
                    c=22
                    lfound=.false.
                    do while (c <= 27 .and. .not. lfound)
                        if (lactions(c)) then
                            mixopt=c-21
                            coare_mix(mixopt)=.true.
                            lfound=.true.
                        else
                            c=c+1
                        endif
    ...
    !   JAT 1/3/23 A002_COARE; certain mixing height options require input mixing heights
    if ((coare_mix(1) .or. coare_mix(2) .or. coare_mix(3) .or. coare_mix(5)) .and. ((lpath(4) .or. lpath(5)) .and. .not. &
        osvars(mix_var)%lread)) then
        if (lpath(4)) then
            write(msg_unit,formstr(6))adjustl(pathid(ipath)),'E82',modnam,&
                'NO INPUT',trim(adjustl(pathid(4))),'MIXING HEIGHTS FOR COARE MIXING HEIGHT OPTION:',&
                trim(adjustl(actions(mixopt+21)))
        else
            write(msg_unit,formstr(6))adjustl(pathid(ipath)),'E82',modnam,&
                'NO INPUT',trim(adjustl(pathid(5))),'MIXING HEIGHTS FOR COARE MIXING HEIGHT OPTION:',&
                trim(adjustl(actions(mixopt+21)))
        endif
        lbad=.true.
        endif
    
!   JAT 5/5/2025 A058 (A058_SOUND); check upper air substitution; do nothing is NOSUB_UA selected
    
    if (litems(9) .and. (lactions(28) .or. lactions(30) .or. lactions(31))) then
!       JAT 5/5/2025 A058 (A058_SOUND); full substitution or substitution without secondary station has been requested, set 
!       sub_upper to true
        sub_upper=.true.
        if (lsecondary) then
            if (lactions(30)) then
                
!               JAT 5/5/2025 A058 (A058_SOUND); warn user that secondary station will not be used
                write(msg_unit,formstr(5))adjustl(pathid(ipath)),'W89',modnam,&
                    'SECONDARY STATION WILL NOT BE USED FOR UPPER AIR SUBSTITUTION'
!               JAT 6/11/2025 A058 (A058_SOUND); reset nstation to 1; warning message will be issued in up_proc
                nstation=1
                lsecondary=.false.
            else
                use_second=.true.
                if (lactions(31)) check_sec_first=.true. !secondary sounding will be checked first on a missing day before going back/forward in time
            endif
        else
!           JAT 2/12/2025 A058 (A058_SOUND); if user selected UASUBSEC1 and no secondary station, issue warning
            if (lactions(31)) then
                write(msg_unit,formstr(5))adjustl(pathid(ipath)),'W90',modnam,&
                'NO SECONDARY STATION AVAILABLE FOR UPPER AIR SUBSTITUTION' 
                check_sec_first=.false.
            endif
            
        endif
    elseif (.not. litems(9) .or. (litems(9) .and. lactions(29))) then
        nstation=1
    endif
!   JAT 7/7/2025 A058 (A058_SOUND); issue warning if timezone difference between primary and secondary station is more than 1 hour
    if (lsecondary .and. abs(upgmt2lst(2)-upgmt2lst(1)) > 1)    write(msg_unit,formstr(5))adjustl(pathid(ipath)),'W36',modnam,&
                    'PRIMARY AND SECONDARY STATIONS MORE THAN 1 TIMEZONE APART'
!   JAT 6/12/2025 A058 (A058_SOUND); set updates
!   use min value for upstart and max value for upend
    if (nstation == 2 .and. sub_upper) then
        if (upstarts(1) <= upstarts(2)) then
            upstart=upstarts(1)
            updates(1,:)=updates1(1,1,:)
        else
            upstart=upstarts(2)
            updates(1,:)=updates1(2,1,:)
        endif
        
        if (upends(1) >= upends(2)) then
            upend=upends(1)
            updates(2,:)=updates1(1,2,:)
        else
            upend=upends(2)
            updates(2,:)=updates1(2,2,:)
        endif
    else
        upstart=upstarts(1)
        upend=upends(1)
...
!               JAT 8/25/22 A016_GMT2LST set to mix_var instead of 5
!                if (osvars(5)%lread) lbaddate(1)=.false.
                if (osvars(mix_var)%lread) lbaddate(1)=.false.
...
!               JAT 1/12/2026 A058 (A058_SOUND); only reset dates if not substituting upper air
                if (.not. sub_upper) then
                    upstart=pblstart
                    upend=pblend
                    updates=pbldates
!                   JAT 1/12/2026 A058 (A058_SOUND); reset upstarts and upends
                    upstarts(1)=upstart
                    upstarts(2)=upstart
                    upends(1)=upend
                    upends(2)=upend
...
!                   JAT 3/13/2025 A058 (A058_SOUND); use upstage1(1)
                    !if (.not. upstage1 .or. (.not. lupkeywords(3) .and. .not. lupkeywords(4))) then
!                   JAT 6/10/2025 A058 (A058_SOUND); add condition about no upper air substitution
                    !if (.not. upstage1(1) .or. (.not. lupkeywords(3) .and. .not. lupkeywords(4))) then
                    if (.not. sub_upper .and. (.not. upstage1(1) .or. (.not. lupkeywords(3) .and. .not. lupkeywords(4)))) then
                        upstart=pblstart
                        updates(1,:)=pbldates(1,:)
!                       JAT 1/12/2026 A058 (A058_SOUND); reset upstarts
                        upstarts(1)=upstart
                        upstarts(2)=upstart
                    endif
                endif
...
!       JAT 1/9/23 A002_COARE; update lbaddates for upper air and surface if running COARE and those
!       data not included
        if (run_coare .and. .not. lpath(2) .and. (.not. coare_mix(1) .or. (coare_mix(1) .and. osvars(mix_var)%lread))) &
            lbaddate(1)=.false.
        if (run_coare .and. .not. lpath(3))lbaddate(2)=.false.
        if (lbaddate(1) .and. lbaddate(2) .and. lbaddate(3)) then
            lbad=.true.
...
!       JAT 8/25/22 A016_GMT2LST; correct reference to osvars = 'MGHT' not u*
        ...
        if (.not. ((lpath(4) .or. lpath(5)) .and. osvars(mix_var)%lread .and. .not. lpath(2))) then !onsite data with mixing height and without upper air data
...
!           reset coordinates to surface or onsite (PROG)
            if (lpath(4) .or. lpath(5)) then
                pbllat=oslat
                pbllon=oslon
!               JAT 12/5/2022 A014_METPREP_LOC; set pblid to ONSITE ID
                pblid=osid
!               JAT 2/2/24 A053_COARE_ZO; set pblgmtlst to onsite value
                pblgmt2lst=osgmt2lst
            else
                pbllat=sflat
                pbllon=sflon
!               JAT 12/5/2022 A014_METPREP_LOC; set pblid to surface ID
                pblid=sfid
!               JAT 2/2/24 A053_COARE_ZO; set pblgmtlst to onsite value
                pblgmt2lst=sfgmt2lst
            endif
!           JAT 2/12/24 A053_COARE_ZO; set pblgmt2lst to upgmt2lst only if upper air data read in
!            pblgmt2lst=upgmt2lst !set pblgmt2lst to the upper air stations GMT-LST 
!           JAT 3/13/2025 A058 (A058_SOUND); set pblgmt2lst to primary upper air station
            !if (lpath(2)) pblgmt2lst=upgmt2lst !set pblgmt2lst to the upper air stations GMT-LST 
            if (lpath(2)) pblgmt2lst=upgmt2lst(1) !set pblgmt2lst to the upper air stations GMT-LST
        endif
    else !keyword not present
 !       JAT 8/25/22 A016_GMT2LST; correct reference to osvars = 'MGHT' not u*
        !if ((lpath(4) .or. lpath(5)) .and. osvars(8)%lread .and. .not. lpath(2)) then !onsite data without upper air data
        if ((lpath(4) .or. lpath(5)) .and. osvars(mix_var)%lread .and. .not. lpath(2)) then !onsite data without upper air data
            if (lpath(4)) then
                str='ONSITE'
            else
                str='PROG'
...
!   JAT 3/16/2023 A097 (A097_SUBNWS) issue error if SUBNWS not requested when only using NWS data for
!   surface data. this makes the AERMET code match the user's guide in that an error is suppposed to
!   be issue
    if (.not. lpath(4) .and. .not. lpath(5) .and. lpath(3) .and. .not. lactions(3)) then
        lbad=.true.
        write(writeunit,formstr(5))adjustl(pathid(ipath)),'E87',modnam,'NWS DATA ONLY AND SUBNWS ACTION NOT INVOKED' 
    endif
...
!   examples:  AERSURF2 allowed with FREQ_SECT, SITE_CHAR, and SECTOR
    ...
    s1: do isurf=15,16
        ...
        if (lpblkeywords(isurf) .and. (lpblkeywords(j) .or. lpblkeywords(j+1) .or. lpblkeywords(j+2))) then
            lbadsc=.true.
!           JAT 9/8/22 A011_AERSURF2; correct scstr reference to be isurf-14
            ...
            write(writeunit,formstr(6))adjustl(pathid(ipath)),'E73',modnam,trim(adjustl(keywrd(ikey1))),'KEYWORD LISTED WITH',&
                trim(adjustl(scstr(isurf-14))),'SITE SC KEYWORDS'
        endif
    enddo s1
...
!               JAT 9/6/222 A011_AERSURF; make sure keywords present
!               if ((lpblkeywords(j) .neqv. lpblkeywords(j+1)) .or. (lpblkeywords(j+1) .neqv. lpblkeywords(j+2))) then
!               JAT 6/1/23 A041 corrected truncated lines
                if (.not. lpblkeywords(isurf) .and. (.not. lpblkeywords(j) .or. .not. lpblkeywords(j+1) .or. .not. &
                    lpblkeywords(j+2))) then
                    lbadsc=.true.
                    write(writeunit,formstr(2))adjustl(pathid(ipath)),'E73',modnam,'ONE OR MORE',trim(adjustl(scstr(i))),&
                        'SITE SC KEYWORDS MISSING'
                endif
            enddo i2
!       JAT 9/6/222 A011_AERSURF2; if surface characteristics specified by the input file, check to make sure necessary keywords present.
        elseif (.not. lpblkeywords(15) .and. .not. lpblkeywords(16)) then
    i3:     do i=1,2
                if (i == 1) then
                    j=3
                else
                ...
                if (.not. lpblkeywords(j) .or. .not. lpblkeywords(j+1) .or. .not. lpblkeywords(j+2)) then
                        lbadsc=.true.
                    write(writeunit,formstr(2))adjustl(pathid(ipath)),'E73',modnam,'ONE OR MORE',trim(adjustl(scstr(i))),&
                        'SITE SC KEYWORDS MISSING'
                endif
...
!       JAT 6/23/2025 A046 (A046_A091_A092_A093); only reset cloud cover substitution when not running COARE
!        if (lactions(4) .or. osvars(inso_var)%lread) then
        if ((lactions(4) .and. .not. run_coare) .or. osvars(inso_var)%lread) then
            subcloud=.false. 
            if (litems(6) .and. lactions(8)) write(msg_unit,formstr(5))adjustl(pathid(ipath)),'W73',modnam,&
...
!   JAT 6/23/2025 A095 (A046_A091_A093) correct logic to be lpath(4) or lpath(5)
    !elseif (lpath(4) .and. lpath(5)) then
    elseif (lpath(4) .or. lpath(5)) then
...
!   JAT 6/23/2025 A046 (A046_A091_A092_A093); alert user Bulk-Richardson not used with COARE
    if (lactions(4) .and. run_coare) write(msg_unit,formstr(5))adjustl(pathid(ipath)),'I94',modnam,&
            'BULKRN REQUESTED WITH COARE; BULKRN NOT INVOKED'    
!   set adjust u* flag; alert user that valid onsite u* will not be adjusted
    if (litems(3) .and. lactions(7)) then
        adjustar=.true.
        if (lpath(4) .or. lpath(5)) then
!           JAT 3/19/24 A043_COARE; if adjust u* requested and running COARE, reset adjustar to false and
!           issue message because adjust u* not used with COARE at this time
            if (run_coare) then
                adjustar=.false.
                write(msg_unit,formstr(5))adjustl(pathid(ipath)),'I93',modnam,&
                'ADJUST U* REQUESTED WITH COARE; ADJUST U* NOT INVOKED'
...
!   JAT 1/3/23 A002_COARE; only do this check if not running COARE or running COARE with the default mixing height option
    !if (.not. lpath(2) .and. ((lpath(4) .or. lpath(5)) .and. .not. osvars(mix_var)%lread)) then
    if (run_coare .and. coare_mix(1) .and. .not. lpath(2) .and. ((lpath(4) .or. lpath(5)) .and. .not. osvars(mix_var)%lread)) then
        if (lpath(4)) then
...
!   JAT 7/26/22 A002_COARE; check to see if a default sea surface temperature depth is to be used
    if (run_coare) then
        if (lpblkeywords(19)) then
            luse_sst_dep=.true.
            if (osvars(sstd_var)%lread) write(msg_unit,formstr(5))adjustl(pathid(ipath)),'W84',modnam,&  
                'SSTDEPTH and ZDEP BOTH SPECIFIED. SSTDEPTH KEYWORD WILL BE USED AND ZDEP VARIABLES IGNORED'
        else
            if (.not. osvars(sstd_var)%lread .and. osvars(tsea_var)%lread) then
                luse_sst_dep=.false.
                write(msg_unit,formstr(5))adjustl(pathid(ipath)),'I90',modnam,&  
                'TSEA SPECIFIED WITHOUT ZDEP or SSTDEPTH . USE DEFAULT SST DEPTH'
            endif
        endif
    endif
...
!   JAT 2/12/2025 A058 (A058_SOUND); add output argument, station_use to determine which upper air 
!   station to use; not needed for NWS or onsite/PROG data
!   JAT 2/19/2025 A058 (A058_SOUND); add lsub1 and lsub2 as output arguments for upper air; not needed for NWS
!   or onsite/PROG data
    
    !subroutine check_obs(iswitch,iday1)
    subroutine check_obs(iswitch,iday1,station_use,lsub1,lsub2)
!=========================================================================================================
...
!   OUTPUT ARGUMENTS
!   
!   STATION_USE:    INDEX OF UPPER AIR STATIONS TO USE (1=PRIMARY, 2=SECONDARY)
!   LSUB1:      DENOTES UPPER AIR DATA SUBSTITUTED FROM PRIMARY UPPER AIR SITE SUBSTITUTION REQUESTED
!   LSUB2:      DENOTES UPPER AIR DATA SUBSTITUTED FROM SECONDARY UPPER AIR SITE SUBSTITUTION REQUESTED
!
!   Variable definitions
...
!   target_date:first sounding date to check
...
!   istation:   primary (1) or secondary (2) upper air station
!   station_use:    index of upper air stations to use (1=primary, 2=secondary)
!   offset:     offset of day to use for data_dates input when searching for soundings
!               using UADAYWIN
!   iday2:      iday1+day_offset
!
!   Logical variables
...
!   lsub1:      denotes upper air data substituted from primary upper air site substitution requested
!   lsub2:      denotes upper air data substituted from secondary upper air site substitution requested
!   lstop:      denotes when to stop looking for soundings
!
!   Character variables
...
!========================================================================================================= 
!   JAT 6/11/2025 A058 A058_SOUND add use_second from main1
    use main1, only : data_dates,lpath,lkey,use_second
    use upperair, only: upstart,upend,upgmt2lst,nupdays,sound_info,have_soundings,nstation,lsecondary
...
!   JAT 2/12/2025 A058 (A058_SOUND); add station_use as output variable
    integer(kind=4),intent(out),optional :: station_use

    integer(kind=4) :: snd_hr(2),target_hr,itargetday,stophr,iupday,h,nstep,nstation1,istep,day_offset,iday2
    integer(kind=8) :: snd_window(2),target_date
    
!   JAT 2/13/2025 A058 (A058_SOUND); add lsub1 and lsub2 for upper air substitution
!   lsub1 is data is substituted from the primary station, lsub2 is from secondary station
!   also add lstop
!   remove lfound1
    !logical lfound,lfound1,lupper(3),ljunk,lgo(3)
    logical lfound,lupper(3),ljunk,lgo(3),lstop
    logical, intent(out),optional :: lsub1,lsub2
...
!   JAT 7/13/22 A005_SAVEHR_UNINIT
!   initialize these variables
!   JAT 4/16/2025 A058 (A058_SOUND); comment out variables not being used anymore
    !iyr=0
    !imon=0
    !idy=0
    !ihr=0
    ...
    !lfound1=.false.
    ljunk=.false.
!   JAT 4/16/2025 A058 (A058_SOUND); initialize lstop
    lstop=.false.
    iday2=1
    
!   JAT 2/12/2025 A058 (A058_SOUND); initialize station_use to 1; maybe reset later for upper air data
!   not really needed for NWS or ONSITE/PROG data
!   also initialize lsub1 and lsub2 to false
    if (present(station_use)) station_use=1
    if (present(lsub1)) lsub1=.false.
    if (present(lsub2)) lsub2=.false.
...
!   JAT 2/12/2025 A058 (A058_SOUND), check for both stations if needed
    !if (lpath(2) .and. iswitch == 1 .and. have_soundings) then
    if (lpath(2) .and. iswitch == 1 .and. (have_soundings(1) .or. have_soundings(2))) then
!       JAT 4/9/2025 A058 (A058_SOUND); loop through upper air stations
        iupday=1
        do while (iupday <= nupdays .and. .not. lfound)
            if (sfc_data(iday1)%sfcdate == sound_info(iupday,1)%snddate) then
                lfound=.true.
                if (sound_info(iupday,1)%sounding)pbl_obs(1,iday1)=sound_info(iupday,1)%nsnding
                if (lsecondary) then
                    if (sound_info(iupday,2)%sounding)pbl_obs(5,iday1)=sound_info(iupday,2)%nsnding
                endif
            else
                iupday=iupday+1
            endif
...
!   JAT 2/12/2025 modify code below to add istation information; will add !A058 as a comment on changed lines
!   check upper air for target sounding if iswitch is not 1.
    !if (lpath(2) .and. iswitch /= 1 .and. have_soundings) then
    if (lpath(2) .and. iswitch /= 1 .and. (have_soundings(1) .or. (have_soundings(2) .and. use_second))) then

!       JAT 4/14/2025 A058 (A058_SOUND); methodology
!       A.  assign values to lgo
!           lgo(1):  target date and hour (can be based on sunrise time or default 00Z or 12Z sounding) is in upper air data
!           lgo(2):  pre-target sounding window date (UAWINDOW) is in upper air data
!           lgo(3):  post-target date sounding window date (UAWINDOW) is in upper air data
!           lgo does not mean there is a valid sounding, just that the dates fall into the window of upper air data
!
!       B. if any value of lgo is true, then find a sounding to use based on the following hiearchy
...
!           if one of the three above conditions are met, then set up_obs to true for the day and alert user
!           if not and no substitution is requested, then no sounding for the day. 

!       Steps A and B are equivalent to methodology in AERMET pre 25xxx.
!   
!       New with 25xxx,
!       C.  if the conditions of Step A and/or Step B are not met, and there is data substitution requested (UA_SUB),
!           repeat steps A and B for the station using the date windows specified by UADAYWIN or default day window using same target hour
!           if one of the three conditions are met, then set up_obs to true for the day, using the selected sounding (and alert user)
!           if the first step and the SUBUASEC1 is chosen check the secondary station before proceeding to the other steps
!       D.  if still no sounding after step C, and there is secondary station, repeat steps A and B for the secondary station using
!           same criteria laid out with step C.
        
!       JAT 4/16/2025 A058 (A058_SOUND); set number of steps and number of stations to use
        if (sub_upper) then
!           JAT data substitution, steps A, B, and C
!           may also repeat if secondary station
            nstep=abs(day_window(1))+abs(day_window(2))+1
            !nstep=3
            nstation1=nstation
        else
!           JAT no data substitution so will only do steps A and B above (one step)
            nstep=1
            nstation1=1
        endif
        istep=1
        ...
        do while (.not. lstop .and. .not. up_obs(iday1))
            lgo=.false.
            lupper=.false.
!           get which day to search for.  If step 1, current day, otherwise, use day_window to offset the day
            if (istep == 1) then
                day_offset=0
            else
                if (day_offset == day_window(1)) then
                    day_offset=1 !reached the lower window, now start looking post surface date
                elseif (day_offset <= 0) then
                    day_offset=day_offset-1 !look back
                else
                    day_offset=day_offset+1 !look forward
                endif
            endif
!           get values for lgo
            iday2=iday1+day_offset
!           subtract 1 to account for 0 if iday1 is 1
            !if (iday2 < 0) iday2=iday2-1

            
!           do not let iday2 be <= 0 or greater than npbldays
            !if (iday2 <=0) then
            !   iday2=iday1
            !elseif (iday2 > npbldays) then
            !   iday2=npbldays
            !endif
            
            
            call upp_dates(iday2,station_use,target_date,target_hr,snd_hr,snd_window,lgo) 
            if (.not. lgo(1) .and. .not. lgo(2) .and. .not. lgo(3)) then
                if (istep == nstep) then
                    if (station_use == nstation1) then
                        lstop=.true.
                    else
                        station_use=station_use+1
                        istep=1
                    endif
                else
                    istep=istep+1
                ...
                call get_sound(iday1,station_use,target_date,target_hr,snd_hr,snd_window,lgo,lupper)
!               if the first step (current date) for primary station and check_sec_first is true check the secondary station
                if (istep == 1 .and. station_use == 1 .and. check_sec_first .and. .not. lupper(1) .and. .not. lupper(2) .and. &
                    .not. lupper(3)) then
                    station_use=2
                    call get_sound(iday1,station_use,target_date,target_hr,snd_hr,snd_window,lgo,lupper)
                    if (.not. lupper(1) .and. .not. lupper(2) .and. .not. lupper(3)) station_use=1 !reset
                endif
                if (lupper(1) .or. lupper(2) .or. lupper(3)) then
                    up_obs(iday1)=.true.
                else
                    if (istep == nstep) then
                        if (station_use == nstation1) then
...
!       JAT 6/6/2025 A058 (A058_SOUND); set lsub1 and lsub2
        if (up_obs(iday1)) then
            if (station_use == 2) then
                lsub2=.true.
            else
                if (istep /= 1) lsub1=.true.
            endif
        endif
        ...
        if (debug)then
            if (lsub1) then
                if (use_second) then
                    write(debug_unit,'(/t5,a)')'PRIMARY SOUNDING STATION USED WITH SUBSTITUTION'
                else
                    write(debug_unit,'(/t5,a)')'SUBSTITUTION USED'
                endif
            endif
            if (lsub2) write(debug_unit,'(/t5,a)')'SECONDARY SOUNDING STATION USED FOR SUBSTITUTION'
            write(debug_unit,debugform)'SOUNDING WINDOW FOR',sfc_data(iday1)%sfcdate,&
...
!*********************************************************************************************************
!   JAT 4/16/2025 A058 (A058_SOUND); add subroutine to check sounding dates
    subroutine upp_dates(iday1,istation,target_date,target_hr,snd_hr,snd_window,lgo)
!=========================================================================================================
!   SUBROUTINE upp_dates
!   THIS SUBROUTINE DETERMINES IF SOUNDINGS DATES ARE WITHIN THE DATA PERIOD
!
...
!   US EPA/AQMG
!
!   CALLED BY MODULE (PBL) (CHECK_OBS)
!
!   INPUT ARGUMENTS 
!
!   IDAY1:      DAY COUNTER FOR SFC_DATA
!   ISTATION:   STATION INDICATOR (1=PRIMARY, 2=SECONDARY (IF APPLICABLE)
!
!   OUTPUT ARGUMENTS
!   TARGET_DATE:    FIRST SOUNDING DATE TO CHECK
!   TARGET_HR:  TARGET HOUR OF DAY FOR A SOUNDING (00Z OR 12Z GMT)
!   SND_HR:     2 ELEMENT ARRAY OF THE HOUR OF DAY FOR BEGINNING AND ENDING
!               WINDOW FOR UPPER AIR CHECK (UAWINDOW)
!   SND_WINDOW: BOUNDS OF DATES AROUND THE TARGET DATE
!   LGO:        3-ELEMENT ARRAY USED IN SUBROUTINE DATA_DATES TO 
!               DETERMINE IF TARGET DATE (INDEX 1), AND WINDOW SOUNDING DATES (INDICES 2 AND 3)
!               FOUND IN UPPER AIR DATA.
!   
...
!   jday:       Julian day
!   snd_window: bounds of dates around the target date
!   idate:      date in YYYYMMDD
!
!   Real variables:
!   rsunrise:   sunrise hour of target_date
!   rsunset:    sunset hour of target_date
!
!   Logical variables
!   lfound:     logical variable denoting current day found in various data
!               (upperair, surface, onsite/PROG, AERMINUTE output)
...
!   modnam:     Subroutine name
!=========================================================================================================  
    use main1, only : data_dates,leapyr
    use upperair, only: upstart,upend,upgmt2lst,uplat,uplon
    implicit none
    integer(kind=4), intent(in) :: iday1,istation
    integer(kind=4), intent(out) :: snd_hr(2),target_hr
    integer(kind=8), intent(out) :: snd_window(2),target_date
    integer(kind=4) :: i,iyr,imon,idy,ihr,iw1,iw2,iw3,jday
    integer(kind=8) :: idate
    real(kind=r8) :: rsunrise,rsunset
    logical, intent(out) :: lgo(3)
    logical :: ljunk,leap
    character(len=8) :: adate
    character(len=10) :: modnam='UPP_DATES'
    
!   if iday1 < 0, then go back the number of days
    if (iday1 <= 0) then
        !ihr=1
        ihr=0
        write(adate,'(i8)')sfc_data(1)%sfcdate
        read(adate,'(i4,3(i2))')iyr,imon,idy

        call data_dates(ihr,idy,imon,iyr,24*abs(iday1-1),upstart,upend,42,idate,ljunk)
    elseif (iday1 > npbldays) then
        ihr=1
        write(adate,'(i8)')sfc_data(npbldays)%sfcdate
        read(adate,'(i4,3(i2))')iyr,imon,idy
        call data_dates(ihr,idy,imon,iyr,-24*(iday1-npbldays),upstart,upend,42,idate,ljunk)
    else
        idate=sfc_data(iday1)%sfcdate
    endif
    write(adate,'(i8)')idate
    read(adate,'(i4,3(i2))')iyr,imon,idy
...
!       if iday1 is within npbldays use the sunrise time of the day from up_sunrise, otherwise recalculate the sunrise time for the new day
        if (iday1 > 0 .and. iday1 <= npbldays) then
            target_hr=int(up_sunrise(iday1,istation))
        else
            call leapyr(iyr,leap)
            iw1=mod((imon+9),12)
            iw2=(iw1*153+2)/5+idy+58
            if (leap) then
                iw3=366
                iw2=iw2+1
            else
            ...
            jday=1+mod(iw2,iw3)
            call sundat(uplat(istation),uplon(istation),temp_upgmt(istation),jday,rsunrise,rsunset)
            target_hr=int(rsunrise)
        endif
...
!       initially, the day will be the day based on GMT
        ...
        if (my_sounding(istation) < 0) then                
...
!           set target_hr to target hour in GMT
            target_hr=target_hr_gmt(istation)
            call data_dates(target_hr,idy,imon,iyr,upgmt2lst(istation),upstart,upend,42,target_date,lgo(1))
        else
...
!           GMT of hour at beginning (i=1) or end (in=2) of window
 !          JAT A015_SOUND 8/15/22;    use absolute value of my_sounding for snd_hr
            !snd_hr(i)=my_sounding+snding_win(i)
            snd_hr(i)=abs(my_sounding(istation)+snding_win(i))
!           JAT A015_SOUND 8/15/22; if snd_hr(1) is < 0 then reset to 25-snd_hr(1) to make a later hour for the day before
!           likewise if snd_hr(2) is > 24 then reset to 26-snd_hr(2) to make it early hour for next day
            if (snd_hr(1) < 0) snd_hr(1)=25+snd_hr(1)
            if (snd_hr(2) > 24) snd_hr(2)=26-snd_hr(2)  
...
!               snd_hr will be reset from GMT to LST
                ...
                read(adate,'(i4,3(i2))')iyr,imon,idy
                call data_dates(snd_hr(i),idy,imon,iyr,upgmt2lst(istation),upstart,upend,42,snd_window(i),lgo(2))
...
!               set snd_hr to snd_hr_gmt and then it will be reset to LST
                snd_hr(i)=snd_hr_gmt(i,istation)
                call data_dates(snd_hr(i),idy,imon,iyr,upgmt2lst(istation),upstart,upend,42,snd_window(i),lgo(3))
    ...
    return
    end subroutine upp_dates
!*********************************************************************************************************  
!   JAT 4/16/2025 A058 (A058_SOUND); add subroutine to see if sounding exists for target day
    subroutine get_sound(iday1,istation,target_date,target_hr,snd_hr,snd_window,lgo,lupper)
!=========================================================================================================
!   SUBROUTINE GET_SOUND
!   THIS SUBROUTINE DETERMINES IF THERE IS A SOUNDING FOR THE DAY, EITHER FOR THE TARGET DAY OR
!   ANOTHER DAY AND/OR OTHER STATION
!
!   MODIFIED APRIL 16, 2025
!   
!   JAMES THURMAN
...
!   modnam:     Subroutine name
!=========================================================================================================  
    use upperair, only: nupdays,sound_info
    implicit none
    integer(kind=4), intent(in) :: iday1,istation,target_hr,snd_hr(2)
    integer(kind=8), intent(in) :: target_date,snd_window(2)
    integer(kind=4) :: itargetday,stophr
    logical, intent(in) :: lgo(3)
    logical, intent(out) :: lupper(3)
    logical :: lfound1,lfound
    character(len=10) :: modnam='GET_SOUND'

    lupper=.false.
!   go back a few days
    if (iup(iday1) > abs(day_window(1))+1) iup(iday1)=iup(iday1)+day_window(1)-1
...
!       data even if there are no soundings
        ...
        do while (iup(iday1) <= nupdays .and. .not. lfound) 
            if (target_date == sound_info(iup(iday1),istation)%snddate) then
                lfound=.true.
            else
        ...
        if (lfound) then
            if (sound_info(iup(iday1),istation)%sounding) then
!               only proceed if this day has a sounding
!               find sounding that matches target hour and has data
                lfound1=.false.
                do while (isnd(iday1) <= sound_info(iup(iday1),istation)%nsnding .and. .not. lfound1)
                    if (target_hr == sound_info(iup(iday1),istation)%snd_hr(isnd(iday1)) .and. &
                        sound_info(iup(iday1),istation)%nlevels(isnd(iday1)) > 0 ) then
                        lfound1=.true.
                    else
...
!                   find sounding before target hour that is closest but after the stop hour defined above
                    isnd(iday1) = sound_info(iup(iday1),istation)%nsnding
                    
                    do while (isnd(iday1) >= 1 .and. .not. lfound1)
                        if (sound_info(iup(iday1),istation)%snd_hr(isnd(iday1)) <= target_hr .and. &
                            sound_info(iup(iday1),istation)%snd_hr(isnd(iday1)) >= stophr .and. &
                            sound_info(iup(iday1),istation)%nlevels(isnd(iday1)) > 0) then
...
!       both false, reset to iup(iday1) to 1
        ...
        do while (iup(iday1) <= nupdays .and. .not. lfound) 
            if (snd_window(1) == sound_info(iup(iday1),istation)%snddate) then
                lfound=.true.
            else
                iup(iday1)=iup(iday1)+1
            endif     
...
!               beginning window sounding is on a different day so the hour will be >= than target_hour
                if (sound_info(iup(iday1),istation)%snd_hr(isnd(iday1)) >= target_hr .and. &
                    sound_info(iup(iday1),istation)%snd_hr(isnd(iday1)) >= snd_hr(1) .and. &
                    sound_info(iup(iday1),istation)%nlevels(isnd(iday1)) > 0) then
!                   sounding is between the window and target hour
                    lfound1=.true.
                else
            ...
            if (isnd(iday1) <= 0) isnd(iday1)=sound_info(iup(iday1),istation)%nsnding !reset
        endif
    endif
...
!       both false, reset to iup(iday1) to 1
        ...
        do while (iup(iday1) <= nupdays .and. .not. lfound) 
            if (snd_window(2) == sound_info(iup(iday1),istation)%snddate) then
                lfound=.true.
            else
                iup(iday1)=iup(iday1)+1
            endif     
...
!               beginning window sounding is on a different day so the hour will be <= than target_hour
                if (sound_info(iup(iday1),istation)%snd_hr(isnd(iday1)) <= target_hr .and. &
                    sound_info(iup(iday1),istation)%snd_hr(isnd(iday1)) <= snd_hr(2) .and. &
                    sound_info(iup(iday1),istation)%nlevels(isnd(iday1)) > 0) then
!                   sounding is between the window and target hour
                    lfound1=.true.
                else
...
!               JAT 1/10/23 A002_23; only call sfc_chars when not running COARE
                !call sfc_chars(isite,d,osdata1(osvars(wind_vars(1))%readvar,ilev,h,ios(d)),albedo,bowen,zo)
!               JAT A084 (A084_A085) 1/10/2025; add missing wind direction indicator in input arguments
                !if (.not. run_coare) call sfc_chars(isite,d,osdata1(osvars(wind_vars(1))%readvar,ilev,h,ios(d)),albedo,bowen,zo)
                if (.not. run_coare) &
                    call sfc_chars(isite,d,osdata1(osvars(wind_vars(1))%readvar,ilev,h,ios(d)),osvars(wind_vars(1))%missval,&
                    albedo,bowen,zo)
...
!               JAT 1/23/23 A022_WIND; only set calmhr to true if not missing winds
                if (dabs(nws_speed-real(sfvars(11)%missval,r8)) > eps .and. dabs(nws_dir-sfvars(10)%missval) > eps ) calmhr=.false. !calmhr may have been set to true for 1-minute data
...
!           a variable wind
!           JAT A084 (A084_A085) 1/10/2025; check to see if nws_speed is also not missing in case user uses a negative
!           number for the missing wind speed indicator
            !if (nws_speed <= 3.1_r8 .and. .not. calmhr .and. dabs(nws_dir-sfvars(10)%missval) <= eps .and. sfc_data(d)%sfcdate >= &
            !19960701) varwind=.true.
            if (nws_speed <= 3.1_r8 .and. dabs(nws_speed-real(sfvars(11)%missval,r8)) > eps .and. .not. calmhr .and. &
            dabs(nws_dir-sfvars(10)%missval) <= eps .and. sfc_data(d)%sfcdate >=  19960701) varwind=.true.
...
!           JAT A084 (A084_A085) 1/10/2025; check to see if nws_dir is also not missing in case user uses a negative
!           number for the missing wind direction indicator
            !if (.not. lactions(2) .and. nws_dir < sfvars(10)%missval .and. .not. calmhr) then
            if (.not. lactions(2) .and. dabs(nws_dir-sfvars(10)%missval) > eps .and. .not. calmhr) then
...
!           JAT A084 (A084_A085) 1/10/2025; add missing wind direction indicator in input arguments
            !if (.not. run_coare) call sfc_chars(isite,d,nws_dir,albedo,bowen,zo)
            if (.not. run_coare) call sfc_chars(isite,d,nws_dir,sfvars(10)%missval,albedo,bowen,zo)
            sfc_data(d)%bowen(h)=bowen
            sfc_data(d)%albedo(h)=albedo
!           now get zo
            isite=2
...
!       number for the missing wind speed indicator
        ...
        if (nws_speed < dsqrt(2.d0)*minsigv .and. dabs(nws_speed-real(sfvars(11)%missval,r8)) > eps .and. .not. calmhr) then
!           JAT 8/8/22 A010_WINDS; correct message for NWS winds < 0.28
            !write(msg_unit,formstr(1))adjustl(pathid(ipath)),'I74',modnam,sfc_data(d)%sfcdate,h
            write(msg_unit,formstr(1))adjustl(pathid(ipath)),'I74',modnam,'RESET NWS WIND TO 0.282 M/S FOR',date_hr_str
            nws_speed = dsqrt(2.d0)*minsigv
        endif
!       JAT A084 (A084_A085) 1/10/2025; check to see if nws_speed is not equal to the missing code in case user uses a negative
!       number for the missing wind speed indicator
        !if (nws_speed < 900._r8) then
!       JAT A081 (A084_A085) 1/10/2025; if wind speed is not missing but wind direction is missing, then don't 
!       set wdir to nws_dir and set lgotwind to false (if not a variable wind)
        if (dabs(nws_speed-real(sfvars(11)%missval,r8)) > eps) then
            sfc_data(d)%wspd(h)=nws_speed
            !sfc_data(d)%wdir(h)=nws_dir
!           JAT A081 (A084_A085) 1/10/2025; initially set lgotwind to true and then reset if missing wind direction and
!           not a variable wind
            lgotwind=.true.
            if (dabs(nws_dir-real(sfvars(11)%missval,r8)) > eps) then
                sfc_data(d)%wdir(h)=nws_dir
            elseif (.not. varwind) then
                lgotwind=.false.
            endif
            
        else
            lgotwind=.false.
!           JAT 1/23/23 A022_WIND; do not set calmhr to false if NWS speed is missing because the onsite data may be calm and will be used
!           not setting to true here will eliminate the reset of a calm obs to 0.28 below
!            calmhr=.false.
            nws_dir=999._r8
        endif
...
!       JAT A084 (A084_A085) 1/10/2025; add missing wind direction indicator in input arguments
!       since using 999 as the wind direction, just use 999 as the indicator
        !if (.not. run_coare) call sfc_chars(isite,d,999.0_r8,albedo,bowen,zo)
        if (.not. run_coare) call sfc_chars(isite,d,999.0_r8,999,albedo,bowen,zo)
        sfc_data(d)%bowen(h)=bowen
        sfc_data(d)%albedo(h)=albedo
...
!       JAT 11/2/22 A024_PRES, move this logic to outside parent logic, i.e. do regardless if hour is NWS hour
        if (lsfelev) then
            nwselev=sfelev
...
!       nwselev=-99999._r8
!    endif
!   JAT 11/2/22 A024_PRES; add code from above in case surface station elevation needed even if not an NWS obs    
    if (lsfelev) then
        nwselev=sfelev
    elseif (user_elev) then
        nwselev=sf_user_elev
...
!*********************************************************************************************************
!   JAT 11/3/22 A002_COARE; add rh_ht as output variable
!   JAT 2/26/2024 A053_COARE_ZO; add logical variable that RH available for hour for COARE
!   subroutine rh(d,h)
    subroutine rh(d,h,rh_ht,have_rh)
!=========================================================================================================
!   SUBROUTINE RH
...
!   OUTPUT ARGUMENTS
!
!   RH_HT:      RELATIVE HUMIDITY HEIGHT
!   HAVE_RH:    LOGICAL FLAG TO DENOTE IF RH AVAILABLE OR CALCULATED
!   Variable definitions
!      
!   Integer variables
!   d:          day counter
...
!   have_rh:    logical variable denoting if RH is available or calculated
!
!   Character variables
!   modnam:     Subroutine name
!========================================================================================================= 
    ...
    use surface, only: sfdata,sfvars
!   JAT 12/29/2022 A002_COARE; add height_var
    !use onsite, only: nlevel,rh_var,dewpt_var
    use onsite, only: nlevel,rh_var,dewpt_var!,height_var
...
!   JAT 11/3/22 AOO2_COARE; add rh_ht
    real(kind=r8), intent(out) :: rh_ht
    logical lfound,lfound1
!   JAT 2/26/2024 AO53_COARE_ZO: add have_rh
    logical,intent(out) :: have_rh
...
!   JAT 11/3/22 A002_COARE; initialize RH_HT to missing (same as temperature height)
    rh_ht=999.0_r8
    
    lfound=.false.
    lfound1=.false.

!   JAT 2/26/2024 AO53_COARE_ZO; initialize have_rh to false
    have_rh=.false.
...
!                   valid RH, assign to rh variable
                    ...
                    rh_ht=osdata1(1,ilev,h,ios(d))
                endif
                ilev=ilev+1
            enddo
!   JAT 2/26/2024 AO53_COARE_ZO; set have_rh to true if lfound is true
            if (lfound) have_rh=.true.
        endif
...
!   JAT A087 (A087_RH) 1/22/2025; change osvars(height_var)readvar to 1 as height is always the first
!   variable in osdata1
                        !rh_ht=osdata1(osvars(height_var)%readvar,ilev,h,ios(d))
                        rh_ht=osdata1(1,ilev,h,ios(d))
                        lfound1=.true.
                    endif
                    ilev=ilev+1
...
!                   JAT 6/1/2023 A041; correct gfortran warning of using real for integer value
                    !irh=humidity(sfc_data(d)%airtemp(h)-273.15_r8,dewpt)
                    irh=int(humidity(sfc_data(d)%airtemp(h)-273.15_r8,dewpt))
                    sfc_data(d)%rh(h)=real(irh,r8)
!   JAT AO53_COARE_ZO; set have_rh to true 
                    have_rh=.true.
                endif
            endif
        endif
...
!   JAT AO53_COARE_ZO; set have_rh to true (not needed for COARE but set for consistency sake)
            have_rh=.true.
        else
...
!               JAT 6/1/2023 A041; correct gfortran warning of using real for integer value
                ...
                irh=int(humidity(sfdata(7,h,isf(d)),sfdata(8,h,isf(d))))
                sfc_data(d)%rh(h)=real(irh,r8)
                rh_ht=2.0_r8
!   JAT AO53_COARE_ZO; set have_rh to true (not needed for COARE but set for consistency sake)
                have_rh=.true.
...
!       JAT 6/1/2023 A041; correct gfortran warning of using real for integer value
        ...
        sfc_data(d)%ipcode(h)=int(sfdata(5,h,isf(d)))
...
!   modnam:     Subroutine name
!========================================================================================================= 
!   JAT A002_COARE; include vonk, cp, and pi from main1
    !use main1, only : g
    use main1, only : g,vonk,cp,pi
...
!*********************************************************************************************************
!   JAT 9/6/22 A002_COARE; VAPOR PRESSURE FUNCTION
    real(kind=r8) function vapor_pres(temp1,pres)
!=========================================================================================================
!   FUNCTION VAPOR_PRES
!   CALCULATE VAPOR PRESSURE FROM TEMPERATURE AND PRESSURE USING TETEN'S FORMULA
!   ADAPTED FROM AERCOARE
!     
...
!
!   INPUT ARGUMENTS
!   TEMP1:          INPUT TEMPERATURE (C)
!   PRES:           PRESSURE (MB)
!
!   OUTPUT
!     
!   VAPOR_PRES:     VAPOR PRESSURE
!
!   Variable definitions
!
!   Real variables
...
!=========================================================================================================
    implicit none
    real(kind=r8), intent(in) :: temp1,pres
    
    character(len=10) :: modnam='VAPOR_PRES'
    
!   need to convert temp to C from K
    vapor_pres = (1.0007_r8+3.46e-6_r8*pres)*6.1121_r8*dexp(17.502_r8*temp1/(240.97+temp1)) !mb
    
    if (debug) write(debug_unit,'(/t18,a/t10,g13.6,t28,f7.2,t38,f8.3)')'E         PRESSURE    TEMP (C)',vapor_pres,pres,temp1
    return
    end function vapor_pres
!*********************************************************************************************************
!   JAT 9/6/22 A002_COARE; LONGWAVE DOWNWARD RADIATION FUNCTION
    real(kind=r8) function lw_down(d,h)
!=========================================================================================================
!   FUNCTION LW_DOWN
!   CALCULATE LONGWAVE DOWNWARD RADIATION OR ASSIGN FROM PROG/ONSITE DATA
!     
...
!   OUTPUT
!     
!   LW_DOWN:       DOWNWARD LONGWAVE RADIATION
!
!   Variable definitions
!      
!   Integer variables
...
!   modnam:         Subroutine name
!=========================================================================================================
    use onsite, only: rdow_var,clht_var
    implicit none
    integer(kind=4), intent(in) :: d,h
    integer(kind=4) :: ceil,i
    real(kind=r8), parameter :: sb=5.67e-8_r8 !stefan boltzman constnat
    real(kind=r8) :: pw  !precipitable water
    real(kind=r8) :: rn1,rn2,rn3,rh_frac,cloud_factor
    logical :: lcalc,have_lw,have_ceil
    character(len=10) :: modnam='LW_DOWN' 
    
!   initialize
    lw_down=osvars(rdow_var)%missval
    
    if (debug) write(debug_unit,'(/t10,a//)')'START DOWNWARD LONG WAVE RADIATION CALCULATIONS'
    
    lcalc=.false.
!   check to see if longwave radiation read in
    call have_data(d,h,rdow_var,have_lw)
!   check to see if ceiling height read in if needed
    call have_data(d,h,clht_var,have_ceil)
    
    if (have_lw) then
        lw_down=osdata1(osvars(rdow_var)%readvar,1,h,ios(d))
        if (debug) write(debug_unit,'(/t15,a,1x,g13.6)')'Downward long wave radiation set to:',lw_down
    else
!       calculate if RH, cloud cover, and ceiling height assigned for the hour 
!       (temperature is valid because coare is called after temperature/pressure check in pbl_proc)
        if (sfc_data(d)%rh(h) > 0.0_r8 .and. sfc_data(d)%rh(h) <  999.0_r8 .and. &
            sfc_data(d)%ccvr(h) > 0.0 .and. sfc_data(d)%ccvr(h) < 99 .and. &
            have_ceil) lcalc=.true.
        if (debug .and. .not. lcalc) then
            write(debug_unit,'(/t15,a)')'One or more of the following variables is missing or zero; no calculations:'
            if (sfc_data(d)%rh(h) <= 0.0_r8 .or. sfc_data(d)%rh(h) >= 999.0_r8) write(debug_unit,'(t15,a)')'RH is missing or 0%'
            if (sfc_data(d)%ccvr(h) < 0 .or. sfc_data(d)%ccvr(h) >= 99) write(debug_unit,'(t15,a)')&
                'Cloud cover is missing or 0 tenths'
            if (.not. have_ceil) write(debug_unit,'(t15,a)')'Ceiling height is missing'
        endif
    !else
    !   lcalc=.true.
    endif
...
!   calculate if needed
    if (lcalc) then
        ...
        if (debug) then
    i1:     do i=1,3
                write(debug_unit,'(t15,a,i1,a)')'Initial value rn',i,': 0.0'
            enddo i1
        endif
        
    
!       convert RH from percent to fraction
        rh_frac=sfc_data(d)%rh(h)*0.01_r8
!       bound precipitable water between 0.1 and 5.0 cm (per original AERCOARE code)
        pw=0.439_r8*rh_frac*dexp(26.23_r8-5416._r8/sfc_data(d)%airtemp(h))/sfc_data(d)%airtemp(h)
        
        if (debug) then
            write(debug_unit,'(/t15,a)')'PRECIPITABLE WATER CALCULATION'
            write(debug_unit,'(/t15,a)')'PRECIP. WATER  RH FRACTION     TEMP'
            write(debug_unit,'(t15,g13.6,t30,f5.2,t45,f6.2)')pw,rh_frac,sfc_data(d)%airtemp(h)
        endif
        pw=dmax1(pw,0.1_r8)
        pw=dmin1(pw,5.0_r8)
        if (debug)write(debug_unit,'(t15,a,1x,f6.4)')'FINAL PW (0.1 - 5.0 BOUNDS):',pw
        
!       calculate radiation
        lw_down =(0.725_r8+0.17_r8*dlog10(pw))*sb*sfc_data(d)%airtemp(h)**4
        
        if (debug)then
            write(debug_unit,'(/t16,a/t17,a)')'DOWNWARD LW      STEFAN','RADIATION      BOLTZMANN     PW     TEMP'
            write(debug_unit,'(t15,g13.6,t33,e8.2,t45,f5.2,t52,f6.2)')lw_down,sb,pw,sfc_data(d)%airtemp(h)
        endif
!       cloud enhancement
!       convert ceiling to m instead of 10's of km
        ceil=int(osdata1(osvars(clht_var)%readvar,1,h,ios(d))*100._r8)

        if (debug) then
            write(debug_unit,'(/t15,a)')'Cloud enhancement'
            write(debug_unit,'(t15,a,1x,i6/)')'Ceiling height in m:',ceil
        endif
        if (ceil < 1950) then
            rn1=real(sfc_data(d)%ccvr(h),r8)
            if (debug)write(debug_unit,'(t15,a,1x,f5.2)')'Ceiling < 1,950 m; reset rn1:',rn1
        elseif (ceil <= 6400) then
            rn2=real(sfc_data(d)%ccvr(h),r8)
            if (debug)write(debug_unit,'(t15,a,1x,f5.2)')'Ceiling <= 6,400 m; reset rn2:',rn2
        elseif (ceil > 6400) then
            rn3=real(sfc_data(d)%ccvr(h),r8)
            if (debug)write(debug_unit,'(t15,a,1x,f5.2)')'Ceiling > 6,400 m; reset rn3:',rn3
        endif
        cloud_factor=0.26_r8*rn1+0.22_r8*rn2 + 0.006_r8*rn3
        lw_down=lw_down*(1.0_r8+cloud_factor)
        if (debug) write(debug_unit,'(/t15,a,1x,f5.2/t15,a,1x,g13.6)')'Cloud factor:',cloud_factor,&
            'Recalculated downward LW radiation:',lw_down
    endif
    if (debug) write(debug_unit,'(/t10,a//)')'END DOWNWARD LONG WAVE RADIATION CALCULATIONS'
    return
    end function lw_down
!*********************************************************************************************************
...
!   JAT A084 (A085_A085) 1/10/2025; add missing wind direction indicator as input argument in case
!   user uses negative value for missing wind direction instead of 999 (default)
!    subroutine sfc_chars(isite,d,wd,albedo,bowen,zo)
    subroutine sfc_chars(isite,d,wd,missdir,albedo,bowen,zo)
!=========================================================================================================
...
!   D:      DAY COUNTER
!   WD:     WIND DIRECTION
!   JAT A084 (A085_A085) 1/10/2025; add missing wind direction indicator as input argument
!   MISSDIR: MISSING WIND DIRECTION INDICATOR
!
!
!   OUTPUT ARGUMENTS
!
...
!   JAT A084 (A085_A085) 1/10/2025; add missing wind direction indicator 
!   missdir: missing wind direction indicator
!
!   Real variables
!   wd:         wind direction
...
!    integer(kind=4),intent(in) :: d,isite
    integer(kind=4),intent(in) :: d,isite,missdir
...
!       JAT A084 (A085_A085) 1/10/2025; check against missing value indicator
        !if (wd > 900._r8) then
        if (dabs(wd-real(missdir,r8)) <= eps) then
...
!   nn:     number of times to execute the loop (2 if upper air data read in, otherwise 1)
!
!   Real variables
...
!   modnam:     Subroutine name
!========================================================================================================= 
!   JAT A016_GMT2LST; add main1 lpath
!   JAT 3/13/2025 A058 (A058_SOUND); add lsecondary from upper air
    use main1, only: lpath
    use upperair, only: lsecondary
    implicit none
    integer(kind=4),intent(in) :: d
    integer(kind=4) :: i,j,hms(3,2),nn
    ...
    times=0
    hms=0
!   JAT 8/25/22 A016_GMT2LST; don't write sunrise/sunset for upper air data if not read in
    if (lpath(2)) then
        nn=2
    else
        nn=1
...
!               JAT 3/13/2025 A058 (A058_SOUND); use primary upper air station
                !times(1)=up_sunrise(d)
                !times(2)=up_sunset(d)
                times(1)=up_sunrise(d,1)
                times(2)=up_sunset(d,1)
            endif
    l2:     do j=1,2
...
!   JAT 3/13/2025 A058 (A058_SOUND); write secondary upper air sunrise/sunset
    if (lsecondary) then
        times(1)=up_sunrise(d,2)
        times(2)=up_sunset(d,2)
    l3: do j=1,2
            hms(1,j)=int(times(j)) !hour
            hms(2,j)=int((times(j)-real(hms(1,j),r8))*60.0_r8) !minute
            hms(3,j)=int((((times(j)-real(hms(1,j),r8))*60.0_r8)-real(hms(2,j),r8))*60.0_r8) !second
        enddo l3
        write(debug_unit,'(t5,2(a),2(1x,i2.2,2(a1,i2.2)))')trim(adjustl(locstr(2))),' SUNRISE/SUNSET: ',hms(1,1),':',&
            hms(2,1),':',hms(3,1),hms(1,2),':',hms(2,2),':',hms(3,2)
    endif
    return
...
!   modnam:     Subroutine name
!========================================================================================================= 
!   JAT A002_COARE; include vonk from main1
    !use main1, only : g
    use main1, only : g,vonk
...
!               apply upper limit on thetastar1 based on eq. 23 of Luhar and Raynor (2009), page 451
                thetastar1 = dmin1(thetastar1,0.1_r8*(1.0_r8-(dmin1((2._r8*ustar1)/ustarmax,1.0_r8)-1.0_r8)**2))
...
!   modnam:     Subroutine name
!=========================================================================================================
!   JAT A002_COARE; include vonk and cp from main1
    !use main1, only : g
    use main1, only : g,vonk,cp
    implicit none
...
!   US EPA/AQMG
!
!   CALLED BY:      MODULE PBL (PBL_PROC, COARE1)
!
!   INPUT ARGUMENTS
!   D:              DAY COUNTER
!   H:              HOUR COUNTER
...
!   
!   Logical variables
!   lsmooth:        smooth the mechanical mixing height
!
!   Character variables
!   formstr:        formats for messages
!   date_hr_str:    date and hour text string
...
!   JAT 1/3/23 A002_COARE; add variable lsmooth to determine if to smooth for the hour
!   logical :: lsmooth
    character(len=80) :: formstr(2)
    character(len=20) :: date_hr_str
...
!   JAT 1/3/23 A002_COARE; set mixing height to observed value if not running COARE and have onsite mixing
!   heights or running COARE

    !if (onmix .and. .not. cbl(h,d)) then
!    if (onmix .and. ((.not. cbl(h,d) .and. .not. run_coare) .or. (run_coare .and. coare_mix(2)))) then  
!   JAT 2/6/24 A047_MIX_DFAULT; add coare_mix(1) to allow onsite mixing height to be used for MIXHT_DFAULT
!   JAT 3/26/24 A047_MIX_DFAULT; update so that onsite mixing height is used when hour is not convective and not running COARE or
!   hour is not convective and running coare mixing height option 1 (AERMET_DFAULT) or coare mixing height option 2 (use onsite for mechanical and convective heights)
    if (onmix .and. ((.not. cbl(h,d) .and. .not. run_coare) .or. (run_coare .and. ((coare_mix(1) .and. .not. cbl(h,d)) .or. &
        coare_mix(2))))) then
...
!       post 21DRF, if mixing heights are read in but missing for hour, alert user
    ...
    !   JAT 2/9/23 A002_COARE; if running coare apply minimum mixing height
    if (run_coare .and. mixht < zi_min) then
        mixht=zi_min
        write(msg_unit,formstr(1))adjustl(pathid(ipath)),'I92',modnam,'SBL HT SET TO',zi_min,'FOR',date_hr_str
        if (debug)write(debug_unit,'(/t10,a,f6.1)')'RESET MIXING HEIGHT TO ZI_MIN',mixht
    endif
!   reset if > mech_max
...
!   JAT 1/10/23 A002_COARE; do not smooth mixing height if coare_mix(3) or coare_mix(4)
    elseif (run_coare .and. (coare_mix(2) .or. coare_mix(3) .or. coare_mix(4))) then
        sfc_data(d)%zim(h)=mixht
        if (debug) write(debug_unit,'(t10,a,f11.4)')'COARE MECHANICAL MIXING HEIGHT SET TO UNSMOOTHED HEIGHT',sfc_data(d)%zim(h)
    else
...
!       JAT 7/7/22 A003_MECH_HT_MISS_USTAR; used unsmoothed height if ustar is missing
!       ustar most likely to be missing in this instance when u* is set to missing
!       in bulk_rich and convergence not reached
!        if (sfc_data(prev_day)%zim(prev_hr) > 0.0_r8) then
        if (sfc_data(prev_day)%zim(prev_hr) > 0.0_r8 .and. dabs(sfc_data(d)%ustar(h)-(-9.0_r8)) > eps) then 
...
!       JAT 7/7/22 A003_MECH_HT_MISS_USTAR; add logic for ustar missing and previous hour's height not missing; use
!       unsmoothed height
        elseif (sfc_data(prev_day)%zim(prev_hr) > 0.0_r8 .and. dabs(sfc_data(d)%ustar(h)-(-9.0_r8)) <= eps) then
            sfc_data(d)%zim(h)=mixht
            if (debug) write(debug_unit,'(t10,a,f11.4)')'U* MISSING; SET TO UNSMOOTHED HEIGHT: ',&
                sfc_data(d)%zim(h)
        else
...
!*********************************************************************************************************
!   JAT 2/19/2025 A058 (A058_SOUND); add input argument, station_use to determine which upper air 
!   station to use; add lsub1 as input arguments
    !subroutine read_sound(d)
    subroutine read_sound(d,station_use,lsub1)
!=========================================================================================================
...
!=========================================================================================================
    ...
    implicit none
!   JAT 2/19/2025 A058 (A058_SOUND); add station_use as input argument
!    integer(kind=4), intent(in) :: d
    integer(kind=4), intent(in) :: d,station_use
    integer(kind=4) :: ilev
...
!   JAT 2/19/2025 A058 (A058_SOUND); add lsub1
    logical, intent(in) :: lsub1
...
!   JAT 2/19/2025 A058 (A058_SOUND); add station_use as index to arrays when necessary
!   write message to message file indicating the sounding
    ...
    if (station_use == 1) then
        if (lsub1) then
            write(msg_unit,formstr(1))adjustl(pathid(ipath)),'I77',modnam,'SOUNDING FOR DATE:',sfc_data(d)%sfcdate,&
                'IS SUBSTITUTED WITH DATE/HR',&
                sound_info(iup(d),station_use)%snddate,'HR',sound_info(iup(d),station_use)%snd_hr(isnd(d)),'SOUNDING #',isnd(d)
        else
            write(msg_unit,formstr(1))adjustl(pathid(ipath)),'I77',modnam,'SOUNDING FOR DATE:',sfc_data(d)%sfcdate,'IS',&
                sound_info(iup(d),station_use)%snddate,'HR',sound_info(iup(d),station_use)%snd_hr(isnd(d)),'SOUNDING #',isnd(d)
        endif
        ...
        write(msg_unit,formstr(1))adjustl(pathid(ipath)),'I77',modnam,'SOUNDING FOR DATE:',sfc_data(d)%sfcdate,&
                'IS SECONDARY SUBSTITUTION WITH DATE/HR',&
                sound_info(iup(d),station_use)%snddate,'HR',sound_info(iup(d),station_use)%snd_hr(isnd(d)),'SOUNDING #',isnd(d)
    endif
    
!   number of levels will depend on the sounding height
!   if below 5 km, then add 1 level
!   JAT 2/19/2025 A058 (A058_SOUND); add station_use as index of array
    
    !baseht=updata(2,1,isnd(d),iup(d))
    baseht=updata(2,1,isnd(d),iup(d),station_use) !A058
...
!   JAT 2/19/2025 A058 (A058_SOUND); add station_use as index of array
 ...
 !       nlevs=sound_info(iup(d))%nlevels(isnd(d))
    ...
    if (updata(2,sound_info(iup(d),station_use)%nlevels(isnd(d)),isnd(d),iup(d),station_use) < real(uptop,r8)) then
        nlevs=sound_info(iup(d),station_use)%nlevels(isnd(d))+1
        lextend=.true.
    else
        nlevs=sound_info(iup(d),station_use)%nlevels(isnd(d))
    endif
...
!   JAT 2/19/2025 A058 (A058_SOUND); use appropriate station for missing values
    ...
    pres=real(upvars(1)%missval(station_use),r8)
    ht=real(upvars(2)%missval(station_use),r8)
    tempk=real(upvars(3)%missval(station_use),r8)
    theta=real(upvars(3)%missval(station_use),r8)
...
!   JAT 2/19/2025 A058 (A058_SOUND); add station_use information
!    l1: do ilev=1,nlevs
...
!    enddo l1
    l1: do ilev=1,nlevs
!       account for fact that nlevs maybe more than one level above original sounding
        if (ilev <= sound_info(iup(d),station_use)%nlevels(isnd(d))) then
            pres(ilev)=updata(1,ilev,isnd(d),iup(d),station_use)
            ht(ilev)=updata(2,ilev,isnd(d),iup(d),station_use)
            tempk(ilev)=updata(3,ilev,isnd(d),iup(d),station_use) !temperature in C for now
        endif
!       skip if height, temperature, or pressure is missing
        if (dabs(pres(ilev)-real(upvars(1)%missval(station_use),r8)) < eps .or. &
            dabs(ht(ilev)-real(upvars(2)%missval(station_use),r8)) < eps .or. &
            dabs(tempk(ilev)-real(upvars(3)%missval(station_use),r8)) < eps) cycle l1
            tempk(ilev)=tempk(ilev)+273.15_r8
            theta(ilev)=tempk(ilev)*(1000._r8/pres(ilev))**0.2857_r8
            !ht(ilev)=ht(ilev)-baseht
    enddo l1
...
!   JAT 2/19/2025 A058 (A058_SOUND); add station_use
    if (debug) then
        ...
        write(debug_unit,debugform(1))'SOUNDING DATE:',sound_info(iup(d),station_use)%snddate,'HR',&
            sound_info(iup(d),station_use)%snd_hr(isnd(d)),'SOUNDING #',sound_info(iup(d),station_use)%nsnding
        if (lextended) then
...
!   JAT 4/21/2025 A058 (A058_SOUND), add station use as an input argument to conv_ht
!    subroutine conv_ht(d,cblhr1)
    subroutine conv_ht(d,station_use,cblhr1)
!=========================================================================================================
!   SUBROUTINE CONV_HT
...
!   INPUT ARGUMENTS
!   D:              DAY COUNTER
!   STATION_USE:    INDEX OF UPPER AIR STATION (1=PRIMARY, 2=SECONDARY)
...
!=========================================================================================================
    !use main1, only: eps    
    use file_units, only: msg_unit
!   JAT A002_COARE; include cp from main1
    !use main1, only: msg_form
    use main1, only: msg_form,cp
    use upperair, only: sound_info
    implicit none
!   JAT 4/21/2025 A058 (A058_SOUND), add station use as an input argument to conv_ht
    !integer(kind=4), intent(in) :: d,cblhr1
    integer(kind=4), intent(in) :: d,station_use,cblhr1
...
!   JAT 4/21/2025 A058 (A058_SOUND); add station_use as array index
!    hhmm=sound_info(iup(d))%snd_hr(isnd(d))*100
    hhmm=sound_info(iup(d),station_use)%snd_hr(isnd(d))*100
...
!   JAT A002_COARE; include cp from main1
    ...
    use main1, only: g,msg_form,cp,vonk
    use file_units, only: msg_unit
    implicit none
    integer(kind=4), intent(in) :: d,h   
    character(len=60) :: formstr
...
!   JAT 1/3/23 A002_COARE; have different w* calculate for COARE
    if (run_coare) then
        wstar=max(0.0_r8,-sfc_data(d)%ustar(h)**3.0_r8*sfc_data(d)%zic(h)/(vonk*sfc_data(d)%mol(h)))
        wstar=wstar**(1.0_r8/3.0_r8)
    else
...
!   JAT 1/11/23 A025_NO_OS_TEMP_PFL; add ostemp as input argument
    !subroutine profile(d,h)
    subroutine profile(d,h,ostemp)
!=========================================================================================================
!   SUBROUTINE PROFILE
...
!       JAT 1/11/23 A025_NO_OS_TEMP_PFL; add condition for tempsrc(h,d)=3 must also be onsite temperature
        !if ((tempsrc(h,d) == 1 .or. tempsrc(h,d) == 3) .or. (tempsrc(h,d) == 0 .and. osvars(temp_var)%lread)) then
        if ((tempsrc(h,d) == 1 .or. (tempsrc(h,d) == 3 .and. ostemp)) .or. (tempsrc(h,d) == 0 .and. osvars(temp_var)%lread)) then
...
!   modnam:         Subroutine name
!=========================================================================================================
!   A002_COARE 7/25/23; add items for COARE in header
    use main1, only: versn,lkey,actions,lpath,items
...
!   JAT 3/13/23 A032_A033_A034;  bug fix A032, change header format to allow for E or W to print
!   for longitude
    !character(len=50) :: hdrform='(2(2x,a8),8x,3(a,a8),t85,a,1x,a5)'
    character(len=50) :: hdrform='(2x,a9,a9,8x,3(a,a8),t85,a,1x,a5)'
...
!   A002_COARE 7/25/23; add string for COARE in header
    character(len=5) :: coarestr
...
!   JAT 5/2/2025 A058 (A058_SOUND); add istation index to upid; use primary station
    !write(hdrstr,hdrform)alat,alon,'  UA_ID: ',upid,'  SF_ID: ',sfid,'  OS_ID: ',osid,'VERSION:',versn
    write(hdrstr,hdrform)alat,alon,'  UA_ID: ',upid(1),'  SF_ID: ',sfid,'  OS_ID: ',osid,'VERSION:',versn
...
!   Bulk-Richardson
    bulkrnstr=''
!   JAT 6/20/2025 A046 (A046_A091_A092_A093); only output BULKRN when not running COARE
!    if (lactions(4)) then
    if (lactions(4) .and. .not. run_coare) then
        if (lpath(5)) then
...
!   A002_COARE 7/25/23; add string for COARE in header
    coarestr=''
    if (run_coare) coarestr=trim(adjustl(items(8)))
...
!        trim(adjustl(substr)),trim(adjustl(progstr))
    write(sfc_out_unit,'(a98,6(1x,a))')hdrstr,trim(adjustl(threshstr)),trim(adjustl(adjustarstr)),trim(adjustl(bulkrnstr)),&
        trim(adjustl(substr)),trim(adjustl(progstr)),trim(adjustl(coarestr))
    return
...
!   nasterisks1:    index of * in the surface data line
!   nasterisks2:    index of * in the profile data line 
!   n_nan1:         index of NaN in the surface data line
!   n_nan2:         index of NaN in the profile data line
!
!   Character variables
!   sfcform:        format for surface file data
!   pflform:        format for profile file data
!   sfcline:        character string of line of data in surface file
!   pflline:        character string of line of data in profile file
!   modnam:         Subroutine name
!=========================================================================================================
...
!    use main1, only: eps
    implicit none
    integer(kind=4), intent(in) :: d
!   JAT 11/22/22 A026_OVERFLOW; add variable nasterisks1 and nasterisks2 to check for asterisks in overflow of data
!   JAT 3/20/23 A026_OVERFLOW; add variables n_nan1 and n_nan2 to check for NaN in data
!    integer(kind=4) :: h,ilev,iyear,imonth,iday,itop
    integer(kind=4) :: h,ilev,iyear,imonth,iday,itop,nasterisks1,nasterisks2,n_nan1,n_nan2
    character(len=8) :: adate
    character(len=180) :: sfcform
    character(len=80) pflform
!   JAT 11/22/22 A026_OVERFLOW; add variables sfcline and pflline to aid in checking for format overflows
    character(len=300) :: sfcline,pflline 
    character(len=10) :: modnam='WRITEFILES' 
    
!   JAT 6/20/2025 A093 (A046_A091_A092_A093); change sfcform and pflform to write out 4-digit years
    !write(sfcform,'(2(a))')'(3(i2,1x),i3,1x,i2,1x,f6.1,1x,f6.3,1x,f6.3,1x,f6.3,1x,2(f5.0,1x),f8.1,1x,f7.4,1x,f6.2,1x,f6.2,1x,',&
    write(sfcform,'(2(a))')'(i4,1x,2(i2,1x),i3,1x,i2,1x,f6.1,1x,f6.3,1x,f6.3,1x,f6.3,1x,2(f5.0,1x),f8.1,1x,f7.4,1x,f6.2,1x,',&
        'f6.2,1x,f7.2,1x,f6.1,3(1x,f6.1),1x,i5,1x,f6.2,2(1x,f6.0),1x,i5,1x,a7,1x,a9)'

    !write(pflform,'(a)')'(4(i2,1x),f7.1,1x,i1,1x,f7.1,1x,f8.2,1x,f8.2,1x,f8.2,1x,f8.2)'
    write(pflform,'(a)')'(i4,1x,3(i2,1x),f7.1,1x,i1,1x,f7.1,1x,f8.2,1x,f8.2,1x,f8.2,1x,f8.2)'

    write(adate,'(i8)')sfc_data(d)%sfcdate
!   get year, month, day; note that year is 2 digit year
!   JAT 6/20/2025 A093 (A046_A091_A092_A093); now get 4-digit year instead of 2-digit year
    !read(adate,'(2x,3(i2.2))')iyear,imonth,iday
    read(adate,'(i4,2(i2.2))')iyear,imonth,iday
    
!   write surface and profile data
    h1: do h=1,24
!       JAT 11/22/22 A026_OVERFLOW; write data to a character string and check for asterisks, which indicate data overflow 
!       JAT 3/10/23 A026_OVERFLOW; also check for NaN
        write(sfcline,sfcform)iyear,imonth,iday,sfc_data(d)%jday,h,sfc_data(d)%hflux(h),sfc_data(d)%ustar(h),&
...
!       check for asterisks and if greater than 0 issue error but still write data to file so user can see it.
        nasterisks1=index(sfcline,'*')
        n_nan1=index(sfcline,'NaN')
        if (nasterisks1 > 0 .or. n_nan1 > 0) then
            nsfc_over=nsfc_over+1
            call check_surf(d,h,sfcline)
            lbadout=.true.
        endif
!       write surface file
...
!           JAT 11/22/22 A026_OVERFLOW; write data to a character string and check for asterisks, which indicate data overflow 
!           JAT 3/10/23 A026_OVERFLOW; also check for NaN
            write(pflline,pflform)iyear,imonth,iday,h,pfl_data(h,d)%ht(ilev),itop,pfl_data(h,d)%dir(ilev),&
                pfl_data(h,d)%speed(ilev),pfl_data(h,d)%airtemp(ilev),pfl_data(h,d)%sigma_a(ilev),pfl_data(h,d)%sigma_w(ilev)
            nasterisks2=index(pflline,'*')
            n_nan2=index(pflline,'NaN')
            if (nasterisks2 > 0 .or. n_nan2 > 0) then
                npfl_over=npfl_over+1
                call check_prof(d,h,ilev,pflline)
                lbadout=.true.
            endif
...
!*********************************************************************************************************
!   JAT 11/22/22 A026_OVERFLOW; subroutine to determine which variables overflow surface file format    
!   JAT 3/10/23 A026_OVERFLOW; add a check for NaN as well.     
    subroutine check_surf(d,h,sfcline)
!=========================================================================================================
!   SUBROUTINE PROFILE
!   CHECK SURFACE DATA FOR VARIABLE OVERFLOWS
!     
!   MODIFIED MARCH 10, 2023
!     
!   JAMES THURMAN
!   US EPA/AQMG
!
!   CALLED BY:      MODULE PBL (WRITEFILES)
!
!   INPUT ARGUMENTS
!   D:              DAY COUNTER
!   H:              HOUR COUNTER
!   SFCLINE:        CHARACTER STRING OF SURFACE DATA
!
!   Variable definitions
!      
!   Integer variables
!   d:              day counter
!   h:              hour counter
!   istops:         array of last character in surface line for each variable
!   istart:         start position to read variable in surface line
!   istop:          stop position to read variable in surface line
!   warnlim:        maximum number of times to write warning message
!   iwarn:          number of times to write warning message about overflow
!
!   Logical variables
!   
!   Character variables
...
!=========================================================================================================
    ...
    implicit none
    integer(kind=4), intent(in) :: d,h
    integer(kind=4) :: istops(25),ivar,istart,istop,warnlim
    integer(kind=4), save :: iwarn=0
    character(len=300), intent(in) :: sfcline 
    character(len=80) :: formstr(2)
    character(len=20) :: date_hr_str
    character(len=10) :: modnam='CHECK_SURF'

    data istops /2,5,8,12,15,22,29,36,43,49,55,64,72,79,86,94,101,108,115,122,128,135,142,149,155/
    
    warnlim=100
    iwarn=iwarn+1
!   format for warning message
    write(formstr(1),'(2(a))')trim(adjustl(msg_form)),'a,1x,i3,1x,a3,1x,a)'
    
!   format for writing message
    write(formstr(2),'(2(a))')trim(adjustl(msg_form)),'3(a,1x),a20)'
    
    if (iwarn==warnlim)write(msg_unit,formstr(1))adjustl(pathid(ipath)),'W87',modnam,'REACH LIMIT OF',&
        warnlim,'E85','ERROR MESSAGES'

    
    write(date_hr_str,'(a5,1x,i8,1x,a2,1x,i2.2)')'DATE:',sfc_data(d)%sfcdate,'HR',h
    
!   loop through variables and see which ones have overflow
    l1: do ivar=1,25
        if (ivar == 1) then
            istart=1
        else
            istart=istops(ivar-1)+1
        endif
        istop=istops(ivar)
        if (index(sfcline(istart:istop),'*') > 0) then
!           JAT 6/1/23 A041 correct line truncation
            if (iwarn <= warnlim) write(msg_unit,formstr(2))adjustl(pathid(ipath)),'E85',modnam,'SURFACE VARIABLE',&
                trim(adjustl(sfcvar(ivar))),'EXCEEDS FORMAT FOR',date_hr_str
            bad_sfc_vars(ivar)=bad_sfc_vars(ivar)+1
        endif
        if (index(sfcline(istart:istop),'NaN') > 0) then
!           JAT 6/1/23 A041 correct line truncation
!           JAT 6/202/2025 A091 (A046_A091_A092_A093); change error code from E84 to E85, consistent with UG and error above
            !if (iwarn <= warnlim) write(msg_unit,formstr(2))adjustl(pathid(ipath)),'E84',modnam,'SURFACE VARIABLE',&
            if (iwarn <= warnlim) write(msg_unit,formstr(2))adjustl(pathid(ipath)),'E85',modnam,'SURFACE VARIABLE',&
                trim(adjustl(sfcvar(ivar))),'HAS BAD VALUE FOR',date_hr_str
            bad_sfc_vars(ivar)=bad_sfc_vars(ivar)+1
        endif
    enddo l1
    return
    end subroutine check_surf
    
!*********************************************************************************************************
!   JAT 11/22/22 A026_OVERFLOW; subroutine to determine which variables overflow profile file format    
!   JAT 3/10/23 A026_OVERFLOW; add a check for NaN as well.     
    subroutine check_prof(d,h,ilev,pflline)
!=========================================================================================================
!   SUBROUTINE PROFILE
!   CHECK PROFILE DATA FOR VARIABLE OVERFLOWS
!     
!   MODIFIED MARCH 10, 2023
!     
!   JAMES THURMAN
...
!   H:              HOUR COUNTER
!   ILEV:           LEVEL COUNTER
!   PROFLINE:       CHARACTER STRING OF PROFILE DATA
!
!   Variable definitions
!      
!   Integer variables
...
!=========================================================================================================
    ...
    use main1, only: msg_form
    implicit none
    integer(kind=4), intent(in) :: d,h,ilev
    integer(kind=4) :: istops(11),ivar,istart,istop,warnlim
    integer(kind=4), save :: iwarn=0
    character(len=300), intent(in) :: pflline 
    character(len=80) :: formstr(2)
    character(len=20) :: date_hr_str
    character(len=10) :: modnam='CHECK_PROF'
    
    data istops /2,5,8,11,19,21,29,38,47,56,65/
...
!   format for writing message
    write(formstr(2),'(2(a))')trim(adjustl(msg_form)),'3(a,1x),i4,1x,a,1x,a20)'
    
    write(date_hr_str,'(a5,1x,i8,1x,a2,1x,i2.2)')'DATE:',pfl_data(h,d)%pfldate,'HR',h
    
    if (iwarn==warnlim)write(msg_unit,formstr(1))adjustl(pathid(ipath)),'W87',modnam,'REACH LIMIT OF',&
        warnlim,'E86','ERROR MESSAGES'
!   loop through variables and see which ones have overflow
    l1: do ivar=1,11
        if (ivar == 1) then
            istart=1
        ...
        istop=istops(ivar)
        if (index(pflline(istart:istop),'*') > 0) then
            if (iwarn <= warnlim) write(msg_unit,formstr(2))adjustl(pathid(ipath)),'E86',modnam,'PROFILE VARIABLE',&
                trim(adjustl(pflvar(ivar))),'EXCEEDS FORMAT FOR LEVEL',ilev,'FOR',date_hr_str
            bad_pfl_vars(ivar)=bad_pfl_vars(ivar)+1
        endif
        if (index(pflline(istart:istop),'NaN') > 0) then
!           JAT 6/202/2025 A091 (A046_A091_A092_A093); change error code from E85 to E86, consistent with UG and error above
            !if (iwarn <= warnlim) write(msg_unit,formstr(2))adjustl(pathid(ipath)),'E85',modnam,'PROFILE VARIABLE',&
            if (iwarn <= warnlim) write(msg_unit,formstr(2))adjustl(pathid(ipath)),'E86',modnam,'PROFILE VARIABLE',&
                trim(adjustl(pflvar(ivar))),'HAS BAD VALUE FOR LEVEL',ilev,'FOR',date_hr_str
            bad_pfl_vars(ivar)=bad_pfl_vars(ivar)+1
        endif
    enddo l1
    return

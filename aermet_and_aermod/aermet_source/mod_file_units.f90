...
!   up_data_unit:       upper air DATA file unit (14, 29)
!   up_extract_unit:    upper air EXTRACT file unit (15, 30)
!   up_qaout_unit:      upper air QAOUT file unit (16, 31)
...
!   up_inpfile:         primary upper air DATA filename
!   up_qafile:          primary upper air QAOUT filename
!   up_extfile:         primary upper air EXTRACT filename
!   up_inpfile:         secondary upper air DATA filename
!   up_qafile:          secondary upper air QAOUT filename
!   up_extfile:         secondary upper air EXTRACT filename
...
!   JAT 1/27/2025 A058 (A058_SOUND); make upper file units 2-element arrays for primary site (1) and 
!   secondary site (2) for substitution in stage 2
!    integer (kind=4) :: up_data_unit    = 14  
!    integer (kind=4) :: up_extract_unit = 15  
!    integer (kind=4) :: up_qaout_unit   = 16  
    integer (kind=4) :: up_data_unit(2)  
    integer (kind=4) :: up_extract_unit(2)  
    integer (kind=4) :: up_qaout_unit(2)  
    data up_data_unit /14,29/
    data up_extract_unit /15,30/
    data up_qaout_unit /16,31/
    
!   JAT 1/27/2025 A058 (A058_SOUND); add upper file names 2-element arrays for primary site (1) and 
!   secondary site (2) for substitution in stage 2
!   initialize the filenames to blanks
    ...
    character(len=flength) :: up_inpfile(2)= ''
    character(len=flength) :: up_qafile(2) = ''
    character(len=flength) :: up_extfile(2) = ''
...
!   JAT 12/15/2025 A067_GHCN add ghcn_units and initialize (similar to ishd_units);
!   use same units as ishd_units since user cannot use both in the same run
    integer(kind=4) :: ghcn_units(2)
    
    data ishd_units /21,22/
    data ghcn_units /21,22/
...
!   JAT 12/15/2025 A067_GHCN add ghcn_files and initialize (similar to ishd_files)
    character(len=flength) :: ghcn_files(2)
    
    data ishd_files /'ISHD_discard.txt','ISHD_replace.txt'/
    data ghcn_files /'GHCN_discard.txt','GHCN_replace.txt'/

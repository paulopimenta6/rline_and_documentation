...
!   use_second:         denotes to use secondary upper air station for substitution if available or requested
!
!   Character variables
...
!   JAT 12/29/22 A002_COARE; adjust nkeys to account for COARE keywords
    !integer(kind=4), parameter :: nkeys = 36
!   JAT 2/10/2025 A058 (A058_SOUND); increase nkeys to 51 to acccount for new UPPERAIR keywords, UADAYWIN for a day window to search for substituting soundings
!    integer(kind=4), parameter :: nkeys = 41
    integer(kind=4), parameter :: nkeys = 50
!   JAT 7/6/22 A002_COARE; adjust nitems to account for COARE
    !integer(kind=4), parameter :: nitems=7
!   JAT 5/2/2025 A058 (A058_SOUND); increase nitems to 9 for UPP_SUB
    !integer(kind=4), parameter :: nitems=8
    integer(kind=4), parameter :: nitems=9
    
!   JAT 7/6/22 A002_COARE; adjust nactions to account for COARE
!    integer(kind=4), parameter :: nactions=12
!    integer(kind=4), parameter :: nactions=20
!   JAT 1/3/23 A002+COARE; adjust nactions to account for COARE
    !integer(kind=4), parameter :: nactions=21
!   JAT 5/5/2025 A058 (A058_SOUND); adjust nactions to 30 to add new upper air substitution actions
!    integer(kind=4), parameter :: nactions=27
    integer(kind=4), parameter :: nactions=31
...
!   JAT 7/18/22 A003_COARE; move the following constants from the pbl module to the
!   main module as they will be used by COARE algorithms
...
!   JAT 5/5/2025 A058 (A058_SOUND); use_second logical variable to denote to use secondary station
!   for upper air substitution; if a secondary station is read in and upper air substitution requested
!   use_second will be set to true; user will have to request NO_SEC_UA with UPP_SUB to not use secondary station
    logical :: use_second=.false.
    
    character(len=10) :: pathid(npath)
    character(len=12) :: keywrd(nkeys)
    character(len=8) :: items(nitems)
!   JAT 7/18/22 A002_COARE; adjust length of actions  from 7 to 10 to account for COARE actions
    !character(len=7) :: actions(nactions)
    character(len=10) :: actions(nactions)
    character(len=8) :: instr(ninstr)
      
    character(len=linelen) :: inpline
    character(len=linelen) :: inpline1
!   JAT 5/13/2024; update version to 24142
!   JAT 5/21/2026 update version to 26135
    !character(len=5) :: versn='24142'
    character(len=5) :: versn='26135'
...
!   keywords
!   JAT 7/6/22 A002_COARE; add keyword SSTDEPTH
!   JAT 12/29/22 A002_COARE; add keywords ZI_GUST, ZI_MIN, and MIN_MOL
!   JAT 1/27/2025 A058 (A058_SOUND); add DATA2, EXTRACT2, QAOUT2, XDATES2, LOCATION2, AUDIT2, RANGE2, MODIFY2, NO_MISSING2 for secondary 
!   upper air station
!   JAT 1/27/2025 A058 (A058_SOUND); add UADAYWIN for upper air substitution day window
    data keywrd /'REPORT','MESSAGES','CHK_SYNTAX','DATA','NO_MISSING','FORMAT','EXTRACT','QAOUT','XDATES','LOCATION','MODIFY',&
        ...
        'THRESH_1MIN','NOPRINT','SSTDEPTH','ZI_GUST','ZI_MIN','MIN_MOL','DEF_VPTG','DATA2','NO_MISSING2','EXTRACT2','QAOUT2',&
        'LOCATION2','MODIFY2','RANGE2','AUDIT2','UADAYWIN'/
!        'THRESH_1MIN','NOPRINT','SSTDEPTH','ZI_GUST','ZI_MIN','MIN_MOL','DEF_VPTG'/  
        !'THRESH_1MIN','NOPRINT'/            

!   items
    !data items /'WIND_DIR','REFLEVEL','STABLEBL','ASOS_ADJ','UASELECT','CCVR','TEMP'/
!   JAT A002_COARE; add COARE to items
!   JAT 5/2/2025 A058 (A058_SOUND); add UPP_SUB as item
    data items /'WIND_DIR','REFLEVEL','STABLEBL','ASOS_ADJ','UASELECT','CCVR','TEMP','COARE','UPP_SUB'/

!   actions
    !data actions /'RANDOM','NORAND','SUBNWS','BULKRN','NO_ADJ','SUNRISE','ADJ_U*','SUB_CC','NO_SUB','SUB_TT','NOTSUB','NOPERS'/
!   JAT A002_COARE; add COARE options to actions
!   JAT A058 (A058_SOUND); add SUBUA, NOSUB_UA, NO_SEC_UA actions for upper air substitution
!   SUB_UA = full substitution from primary and/or secondary station
!   NO_SEC_UA = do not use secondary station for substitution, just primary
!   NO_SUB_UA = do not substitute any data
!   note default action if UPP_SUB is not listed is no substitution
    data actions /'RANDOM','NORAND','SUBNWS','BULKRN','NO_ADJ','SUNRISE','ADJ_U*','SUB_CC','NO_SUB','SUB_TT','NOTSUB','NOPERS',&
        'NO_COARE','RUN_COARE','WARM','NO_WARM','COOL','NO_COOL','ZO_U*','ZO_U*_PER','ZO_PER_HT','MIX_DFAULT','MIX_OBS',&
        'ZIC_OBS_NS','CALC_ZI_NS','ZIC_OBS_SM','CALC_ZI_SM','SUBUA','NOSUB_UA','NO_SEC_UA','SUBUA_SEC1'/
!   instruments
    data instr /'WIND'/
...
!               MODULE UPPERAIR (UP_PROC), MODULE ONSITE (OS_PROC), MODULE PBL (PBL_PROC,UPP_DATES)
!
!   INPUT ARGUMENT(S)
!   IYEAR:          4-DIGIT YEAR
!
...
!           JAT 7/19/22 A006_LINUX; correct if statement to not compare a logical variable to an integer
            !if (qindx(1) == 0) then
            if (.not. qindx(1)) then
                qindx(1)=.true.
            else
...
!           JAT 3/10/23; A027_GMT_LOCATION; change message from error (E05), to warning (W04)
!           and also issue error if more than 3 hours
            if ((gmt2lst > 0 .and. lon > 0.0_r8) .or. (gmt2lst < 0 .and. lon < 0.0_r8)) then
                ...
                write(writeunit,formstr(3))adjustl(pathid(ipath)), 'W04',modnam,gmt2lst,&
                    'GMT TO LST IS INCORRECT SIGN FOR LONGITUDE OF',lon
                if (abs(gmt2lst) > 3) then
                    write(writeunit,formstr(2))adjustl(pathid(ipath)), 'E05',modnam,gmt2lst,'GMT TO LST IS MORE THAN THREE HOURS'
                    lbad=.true.
                endif
!                lbad=.true.
            endif
...
!   get elevation if entered
    if (nfield1 == 5) then
        lelev=.true.
!JAT 10/16/2024; AO68_SOUND; no longer issue a warning that station elevation is not needed for upper air data
!it will be needed to calculate surface level heights in the sounding if height is missing
...
!                   MODULE PBL (CHECK_OBS,UPP_DATES)
!
!   INPUT ARGUMENTS
!
...
!   iend:           end date (YYYYMMDD) to compare idate against
!   idate:          integer date YYYYMMDD
!   nd:             number of days to subtract/add when ihr < 24 or > 24
!
!   Logical variables
...
!   JAT 1/7/2026 A058 add nd (number of days to substract or add)
    integer(kind=4):: day1,lastday,nd
...
!           hour 0 will become hour 24 of the day before
            if (ihr <= 0) then
!               JAT 1/7/2025 A058 if ihr < -24 then will go back more than 1 day
                if (ihr < -24) then
                    nd=abs(ihr)/24
                else
                    nd=1
                ...
                ihr=24+ihr
!               reset the day
!               JAT 1/7/2025 A058 reset the day by substracting nd
                !iday=iday-1
                iday=iday-nd
...
!                   JAT 1/7/2025 A058 get new day
                    if (nd > 1) iday=iday-nd+1
                endif
            elseif (ihr > 24) then
!               JAT 1/7/2025 A058 if ihr > 24 then will go forward more than 1 day
                if (ihr > 36) then
                    nd=ihr/24
                else
                    nd=1
...
!   JAT 12/19/2024 MULTI_ISSUE (A074); humidity is defined as real but being set as integer; just get the real
!   value of the integer
!    humidity=int(100._r8*(vpact/vpsat))
    humidity=real(int(100._r8*(vpact/vpsat)),r8)
    return
    end function humidity
!*********************************************************************************************************
    real(kind=r8) function gravity(lat)
!=========================================================================================================
!   FUNCTION GRAVITY
!   CALCULATE GRAVITY AS A FUNCTION OF LATITUDE FOR COARE CALCULATIONS
!   ADAPTED FROM GRAVITY FUNCTION IN AERCOARE CODE
!
...
!=========================================================================================================
    ...
    real(kind=r8) :: gamma,c1,c2,c3,c4,sinlat
    character(len=10) :: modnam='GRAVITY' 
    
    gamma = 9.7803267715_r8
    c1 = 0.0052790414_r8
    c2 = 0.0000232718_r8
    c3 = 0.0000001262_r8
    c4 = 0.0000000007_r8
    
!   convert latitude to radians and get sine
    sinlat=dsin(lat*d2r)
    
!   calculate gravity
    gravity = gamma*(1.0+c1*(sinlat**2.0_r8)+c2*(sinlat**4.0_r8)+c3*(sinlat**6.0_r8)+c4*(sinlat**8.0_r8))
    return

...
! JAT   4/25/23 A040_GMT_INIT; initialize sfgmt2lst to -99
!    integer(kind=4) :: sfgmt2lst=-9
    integer(kind=4) :: sfgmt2lst=-99
...
!   JAT 12/4/2025 A067_GHCN increase nsfformats to 10 for new GHCNN format
    integer(kind=4), parameter :: nsfformats=10 !9
...
!                   name    laudit lmod lnomis lincbound missval lowbound upbound conv
    ...
    asoscommdates(312) = asosdata("SOUTH BEND","SOUTH BEND REGIONAL AIRPORT","IN",14848,"KSBN","SBN",41.7_r8,-86.33_r8,19960701, &
        "YES","NWS",33.00_r8,10.05_r8)
!   JAT 8/10/22 A012_HUF; reset WBAN back to 3868
...
!        -87.3_r8,19980305,"NO","FAA",26.00_r8,7.92_r8)
    asoscommdates(313) = asosdata("TERRE HAUTE","TERRE HAUTE INTERNATIONAL-HULMAN FIELD AIRPORT","IN",3868,"KHUF","HUF",39.45_r8,&
        -87.3_r8,19980305,"NO","FAA",26.00_r8,7.92_r8)
...
!   JAT 12/4/2025 A067_GHCNN increase add new GHCN format and change end date for ISHD to 8/29/2025
...
!               check to see if the optional ASOS flag is present or it could be blocking factor or type
                if (nfield == 4) then
!                   JAT 8/10/22 A013_ASOS_FLAG; read from formind(2)+1 to ilen and read as free format
!                    read(inpline1(formind(2):ilen),'(a)')asosflag
!                   JAT 2/19/2026 A067 (A067_GHCN) allow ASOS flag with GHCN data
                    read(inpline1(formind(2)+1:ilen),*)asosflag
                    if (trim(adjustl(asosflag)) == 'ASOS') then
                        !if (sfformats(7)%lsfform) then
                        if (sfformats(7)%lsfform .or. sfformats(10)%lsfform) then
                            lasos=.true.
                        else
!                           can't have ASOS flag for non-ISHD or GHCN data, issue fatal error
...
!   JAT A067_GHCN add call to read_ghcn
    if (sfstage1) then
        ...
        elseif (sfformats(7)%lsfform) then !ISHD
            call read_ishd
        elseif (sfformats(10)%lsfform) then !GHCN
            call read_ghcn
        else
...
!   JAT 6/20/2025 A092 (A046_A091_A092_A093); change modnam to HEADER
    !character(len=10) :: modnam='SF_PROC'
...
!   JAT 1/2/23 A017_SURF_ELEV; add surface elevation to header even if not user entered
!   this ensures consistency between a combined stage 1 and 2 run with separate stage 1 and 2
!   runs when needing surface elevation for possible pressure calculations
    !if (user_elev) write(celev,'(f10.3)')sfelev
    write(celev,'(f10.3)')sfelev
...
!                   missing and current ishobs is missing
!   miss_cloud:     denotes if some form of cloud cover is missing (true if current value in sfdata1 is not
!                   missing and current ishobs is missing
!   writedat:       denotes to write data to sfdata1
!   loverwrite:     denotes to overwrite current sfdata1 value for variable if overwriting data (based on temperature and winds)
!
!   Character variables
...
!   ishobs(5)   = sfdata1(5,:,:)    precipitation typeall 
...
!   JAT 12/29/2025 A062 (A067_GHCNN); add miss_cloud
!   JAT 2/25/2026 A100 (A067_GHCN); add loverwrite
    logical lfound,lcdrom,lgo,lgo2,lgo3,calcrh,spec_obs,reg_obs,gotwban,have_elev,asoshr1,miss_wind,miss_dir,miss_temp,&
        miss_precip,miss_cloud(3),writedat,loverwrite(28)
...
!   JAT 7/12/22 A005_savehr_uninit
...
!                        ishobs(9)=humidity(real(ishobs(7),r8)/10.0_r8,real(ishobs(8),r8)/10.0_r8)
                        ishobs(9)=int(humidity(real(ishobs(7),r8)/10.0_r8,real(ishobs(8),r8)/10.0_r8))
                    else
...
!                       JAT A085 (A084_A085) 1/10/2025; initially set ishobs(10) to sfvars(1)%missval
!                       this correctly sets the missing wind direction to sfvars(1)%missval, especially if user modified missing
!                       value via RANGE keyword
...
!                   get weather codes and evaluate
    f9:             do ifield=11,12
                        ...
                        w1 == 86) then
                            mw(ifield-10)=2
!                       JAT 8/25/22 A018_ISHD_W1; correct logic to say if w1 == 21 .or. w1 == 25)
                        !elseif (w1 == 21 .or. 21 == 25) then
                        elseif (w1 == 21 .or. w1 == 25) then
                            mw(ifield-10)=1
                        elseif (w1 <= 28) then
                            mw(ifield-10)=3
                        elseif (w1 <= 49) then
...
!                   JAT 3/6/2026 A100 (A067_GHCNN); only calculate pressure if the current value in 
!                   in sfdata1 is missing (could be first obs for the hour or a duplicate and if a duplicate,
!                   don't overwrite)
                    !if (ishobs(3) == missobs(3)) then
                    
                    if (ishobs(3) == missobs(3) .and. sfdata1(3,ihr,iday1) == real(sfvars(3)%missval,r8)) then
...
!                       don't overwrite)
                        !if (ishobs(2) == missobs(2)) then
                        if (ishobs(2) == missobs(2) .and. sfdata1(2,ihr,iday1) == real(sfvars(2)%missval,r8)) then
                            if (have_elev .and. .not. user_elev) then
...
!                       JAT 2/25/2026 A100 (A067_GHCNN) initialize loverwrite to true
                        loverwrite=.true.
!                       JAT 12/29/2025 A062 (A067_GHCNN) add cloud cover variables as variables to check
                        ...
                        call check_dup(iday1,ihr,real(ishobs(1),r8),real(ishobs(7),r8),real(ishobs(10),r8),real(ishobs(11),r8),&
                            real(ishobs(12),r8),real(ishobs(13),r8),real(ishobs(6),r8),miss_precip,miss_temp,miss_dir,miss_wind,&
                            miss_cloud,writedat)
...
!                           also reset precip type if resetting precip
!                           to ishobs(1) to sfdata1(1,ihr,iday1)
                            if (.not. miss_precip .and. (ishobs(1) > 0 .or. (sfdata1(1,ihr,iday1) == real(sfvars(1)%missval,r8) &
                                .and. ishobs(1) == 0))) then
                                sfdata1(1,ihr,iday1)=real(ishobs(1),r8)/10._r8   
                                sfdata1(5,ihr,iday1)=real(ishobs(5),r8)
                            endif
                                
!                           JAT 12/29/2025 A062 (A067_GHCNN); if miss_cloud(1), do not use ishobs(12)
                            if (miss_cloud(1)) ishobs(12)=int(sfdata1(4,ihr,iday1))/100
                
!                           JAT 12/29/2025 A062 (A067_GHCNN); if miss_cloud(2), do not use ishobs(13)
                            if (miss_cloud(2)) ishobs(13)=int(sfdata1(4,ihr,iday1))-(int(sfdata1(4,ihr,iday1))/100)*100
                        
!                           JAT 3/2/2026 A062 (A067_GHCNN); if total and opaque are not missing, don't let opaque be more than total cloud cover
                            if (ishobs(12) >= 0 .and. ishobs(12) <= 10 .and. ishobs(13) >= 0 .and. ishobs(13) <= 10 .and. &
                                ishobs(13) > ishobs(12)) ishobs(13)=ishobs(12)
                            
!                           JAT 12/29/202 A062 (A067_GHCNN); if miss_cloud(3), do not use ishobs(6)
                            if (miss_cloud(3)) loverwrite(6)=.false. !ishobs(6)=sfdata1(6,ihr,iday1)
                            
!                           JAT 2/25/2026 A100 (A067_GHCNN); do not reset station pressure, sea level pressure, dewpoint, or RH if 
!                           current sfdata1 value is not missing and new obs is missing; recalculate RH if necessary
                            if (ishobs(2) == sfvars(2)%missval .and. sfdata1(2,ihr,iday1) /= real(sfvars(2)%missval,r8)) &
                                loverwrite(2)=.false.
                            if (ishobs(3) == sfvars(3)%missval .and. sfdata1(3,ihr,iday1) /= real(sfvars(3)%missval,r8)) &
                                loverwrite(3)=.false.
                            if (ishobs(8) == sfvars(8)%missval .and. sfdata1(8,ihr,iday1) /= real(sfvars(8)%missval,r8)) &
                                loverwrite(8)=.false.
                            if (ishobs(9) == sfvars(9)%missval .and. sfdata1(9,ihr,iday1) /= real(sfvars(9)%missval,r8)) &
                                loverwrite(9)=.false.
...
!                               JAT 2/25/2026 A100 (A067_GHCN); if loverwrite is false for a variable skip
                                if (.not. loverwrite(ifield)) cycle v1
...
!                           if ceiliig height, ASOS ceiling or hazardous weather are audited, write variables
!                           these are the only extra variables from sfdata that are in ISHD data
!                           JAT 3/2/2026 A100 (A067_GHCNN); only overwrite if new obs is not missing
    ...
    v2:                         do ifield=nsfvars_keep+1,nsfvars
                                    ...
                                    if ((ifield == 12 .or. ifield == 20 .or. ifield == 21) .and. sfvars(ifield)%laudit) then
                                        ...
                                        if (ifield == 12 .and. ishobs(4) /= sfvars(12)%missval) &
                                            sfdata1(ifield1+nsfvars_keep,ihr,iday1)=ishobs(4) !ceiling  
                                        if (ifield == 20 .and. ishobs(15) /= sfvars(20)%missval) &
                                            sfdata1(ifield1+nsfvars_keep,ihr,iday1)=ishobs(15) !asos ceiling
                                        if (ifield == 21 .and. ishobs(14) /= sfvars(21)%missval) &
                                            sfdata1(ifield1+nsfvars_keep,ihr,iday1)=ishobs(14) !visibility
...
!*********************************************************************************************************
! JAT A067_GHCN add subroutine to read GHCN surface data
    subroutine read_ghcn
!=========================================================================================================
!   SUBROUTINE READ_GHCN
!   THIS SUBROUTINE READS GHCN FORMAT NWS DATA
!
...
!   3.              station pressure (hPa)
!   4.              sea level pressure (hPa)
!   5.              wind direction (degrees)
!   6.              wind speed (m/s)
!   7.              wind gust (m/s)
!   8.              1-hr precipitation (mm)
!   9.              relative humidity (whole percent)
!   10.             wet bulb temperature (C)
!   11.             present weather 1 from manual reports
!   12.             present weather 2 from manual reports
!   13.             present weather 3 from manual reports
!   14.             present weather 1 from ASOS/AWOS
!   15.             present weather 2 from ASOS/AWOS
!   16.             present weather 3 from ASOS/AWOS
!   17.             present weather 1 from automated sensors
!   18.             present weather 2 from automated sensors
!   19.             present weather 3 from automated sensors
!   20.             snow depth (mm)
!   21.             visibility (km)
!   22.             altimeter (millibars/hPa)
!   23.             3-hour pressure change (millibars/hPa)
!   24.             sky condition
!   25.             sky condition base height (m)
!   26.             ceiling height (m)
!   27.             sky cover 1 fraction of total celestial dome with sky coverage (oktas)
!   28.             sky cover base height 1 (m)
!   29.             sky cover summation 1
!   30.             sky cover summation base height 1
!   31.             sky cover 2 fraction of total celestial dome with sky coverage (oktas)
!   32.             sky cover base height 2 (m)
!   33.             sky cover summation 2
!   34.             sky cover summation base height 2
!   35.             sky cover 3 fraction of total celestial dome with sky coverage (oktas)
!   36.             sky cover base height 3 (m)
!   37.             sky cover summation 3
!   38.             sky cover summation base height 3
!   39.             sky cover 4 fraction of total celestial dome with sky coverage (oktas)
!   40.             sky cover base height 4 (m)
!   41.             sky cover summation 4
!   42.             sky cover summation base height 4
!   43.             3-hour precipitation (mm)
!   44.             6-hour precipitation (mm)
!   45.             9-hour precipitation (mm)
!   46.             12-hour precipitation (mm)
!   47.             15-hour precipitation (mm)
!   48.             18-hour precipitation (mm)
!   49.             21-hour precipitation (mm)
!   50.             24-hour precipitation (mm)
!   51:             5-minute precipitation (mm)
!   52.             15-minute precipitation (mm)
!   53.             remarks
...
!   sdate:          integer observation date (YYYYMMDD)
!   savedate:       previous observation's date (YYYYMMDD)
!   npipes:         number of pipe delimiters in line of data
!   npipes_header:  number of pipe delimiters in line of data
!   ipipe:          pipe delimiter counter
!   intvar:         integer value read from file
!   total_cloud:    total cloud cover
!   ncolon:         index of colon (:) in field
!   ndash:          index of dash (-) in field
!   ipcode:         precipitation type code
!   ipcode1:        2-element array of MW1 and MW2 weather codes
!   pvar:           station pressure variable number in ghcnobs
!   slpvar:         sea level pressure variable number in ghcnobs
!   altvar:         altimeter variable number in ghcnobs
!   iline:          line number
!   pipe_offset:    offset from npipes to use when incrementing ivar
!   nremarks:       number of fields in the remarks field (field 38)
!   nremarks1:      number of fields in the remarks field for previous line (field 38)
!   i:              loop counter
!   isky:           sky coverage loop counter
!   ivar:           variable loop counter
!   j1:             pipe counter
!   j2:             pipe counter
!   icloud:         2-element array (1=total cloud, 2=opaque cloud)
!   opaque_cloud:   oqaque cloud cover
!   imiss_total:    missing indicator for total cloud cover
!   imiss_opaque:   missing indicator for opaque cloud cover
!   
!
!   Real variables
!   lat:            latitude read from file
...
!   elev:           elevation read from file
!   p:              calculated pressure
!   rvar:           real variable read from file
!
!   Logical variables
!   lfound:         logical variable denoting certain value found
!   gotwban:        have valid WBAN
...
!   writedat:       denotes to write data to sfdata1
!   loverwrite:     denotes to overwrite current sfdata1 value for variable if overwriting data (based on temperature and winds)
!   infield:        character is in a field
!   inquote:        character is a quotation mark
!
!   Character variables
!   date_hr_str:    date and hour text string for error messages
!   formstr:        format for messages unique to read_ishd
...
!   call_sign3:     3-character call sign
!   call_sign4:     4-character call sign
!   aline:          character string of iline
!   remarks:        fields in the remarks field (field 38)
!   sky:            sky coverage field list
!   modnam:         Subroutine name
!=========================================================================================================
    use, intrinsic :: iso_fortran_env,only : output_unit
    use main1, only: data_dates,humidity,pressure,debug
    use file_units, only: ghcn_units,ghcn_files,sf_data_unit
    implicit none
    integer(kind=4) :: eof,ir,iflag1,iday1,iyear,imonth,iday,ifield,asky,w1,ifield1,ifile,savewban,iwarn,iflag,ihr,imin,savehr,&
        ivar,j1,j2,ndash,ipipe,npipes,npipes_header,intvar,total_cloud,ncolon,ipcode,ipcode1(2),pvar,slpvar,altvar,iline,&
        pipe_offset,nremarks,nremarks1,i,isky,opaque_cloud,icloud(2),imiss_total,imiss_opaque
    integer(kind=4),parameter :: nvar=53
    integer(kind=4) :: inc=30
    integer(kind=4) :: tenths(9)=[0,0,0,3,3,5,8,10,10]
    integer(kind=8) :: savedate=0
    integer(kind=8) :: sdate
    real(kind=r8) :: lat,lon,elev,rvar

    logical lfound,lgo,lgo2,lgo3,calcrh,spec_obs,reg_obs,gotwban,have_elev,asoshr1,miss_wind,miss_dir,miss_temp,&
        miss_precip,miss_cloud(3),writedat,have_report,loverwrite(nvar),infield,inquote
    character(len=5) :: rpt_type,last_rpt,rpt_use,rpt_list(17)
    character(len=8) :: iusaf 
    character(len=3) :: call_sign3
    character(len=4) :: call_sign4
    character(len=10000) :: datline,lastline
    character(len=20) :: date_hr_str
    character(len=60) :: formstr(6)
    character(len=4) :: sky(9)=[character(len=4) :: 'SKC','CLR','CLR1','FEW','FEW2','SCT','BKN','VV','OVC']
    character(len=20),allocatable, dimension (:) :: remarks
    character(len=10) :: modnam='READ_GHCN'
   
!   field:      text string of fields
!   pipe_loc:   pipe location in datline (column of text) of each pipe
!   ipipe:      pipe number in datline (first pipe of variable)
!   ivar:       integer value of variable if integer variable
!   sfvar:      index of variable in sfvars
!   rvar:       real value of variable if real variable
!   lreal:      logical flag indicating if variable is read in as integer or real
!   each variable will have 5 elements (5 pipe delimiters); these are:
...
!   5.  station id
    type obs
        !character(len=50) field(6)
        !integer(kind=4) pipe_loc(6),sfvar
        character(len=300), allocatable, dimension(:) :: field
        integer(kind=4),allocatable, dimension(:) :: pipe_loc
        integer(kind=4) :: sfvar,nfields
        logical :: lmissing
        real(kind=r8) :: rvar
    end type obs
      
    type(obs) :: ghcnobs(0:nvar)
    
    data rpt_list /'FM12','FM13','FM14','FM15','FM16','FM18','SAO','SAOSP','AERO','AUTO','SYAE','SYSA','SYMT','SYAU',&
        'SAAU','SSA','SMARS'/
...
!   5.  replace observation; GHCN is unique in that the report type is listed
    write(formstr(5),'(2(a))')trim(adjustl(msg_form)),'a,1x,a20,4(1x,a))'
...
!   initialize these variables
    ...
    iwban=0
    iline=1
!   set pressure and altimeter variable indices
    pvar=3
    slpvar=4
    altvar=22
    nremarks=0
    nremarks1=0
    imiss_total=sfvars(4)%missval/100
    imiss_opaque=sfvars(4)%missval-imiss_total*100
    
!   read the first line to determine if this a yearly file for a station or the period of record file for the station
!   JAT 3/27/2026 change length to 10000
!   JAT 3/27/2026 no longer need to differentiate between yearly and period of record, now have consistent columns
    !read(sf_data_unit,'(a10000)',iostat=eof)datline
    !a1:    do ifield=1,len_trim(datline)
    !   if (datline(ifield:ifield) == '|') npipes_header=npipes_header+1
    !enddo a1
    !if (npipes_header == 233) then
    !   lpor=.false.
    !   pipe_offset=0
    !else
    !   lpor=.true.
    ...
    !endif
    pipe_offset=5
!   allocate arrays in ghcnobs based on data file type
    a2: do ivar=0,nvar
        if (ivar == 0) then
!           JAT 3/27/2026 station info now has 11 fields
!           when reading a station based file with all years, the station information has 11 fields as
!           the date/time fields are separate; in the year based file for a station, the date/time is one field
            ghcnobs(ivar)%nfields=11 !10
        else
            ghcnobs(ivar)%nfields=6
        endif
        allocate(ghcnobs(ivar)%field(ghcnobs(ivar)%nfields),ghcnobs(ivar)%pipe_loc(ghcnobs(ivar)%nfields))
    a3: do ifield=1,ghcnobs(ivar)%nfields
            ghcnobs(ivar)%field(ifield)=''
            ghcnobs(ivar)%pipe_loc(ifield)=0
        enddo a3
    enddo a2
    ghcnobs%rvar=0.0_r8
    ghcnobs%lmissing=.true.
!   JAT 12/5/2025 A067_GHCN assign sfvars to each input variable
    ghcnobs%sfvar=0 !initialize
    ghcnobs(1)%sfvar=7 !dry bulb temperature
    ghcnobs(2)%sfvar=8 !dew-point temperature
    ghcnobs(3)%sfvar=3 !station pressure
    ghcnobs(4)%sfvar=2 !sea level pressure
    ghcnobs(5)%sfvar=10 !wind direction
    ghcnobs(6)%sfvar=11 !wind speed
    ghcnobs(8)%sfvar=1 !precipitation amount
    ghcnobs(9)%sfvar=9 !relative humidity
    ghcnobs(10)%sfvar=22 !wet bulb temperature
    ghcnobs(11:19)%sfvar=19 !present weather
    ghcnobs(21)%sfvar=21 !horizontal visibility 
    ghcnobs(26)%sfvar=12 !ceiling height
    

    
!   open the discard and replace files
    f1: do ifile=1,2
        open(unit=ghcn_units(ifile),file=ghcn_files(ifile),status='replace')
    enddo f1
!   initialize ghcnobs
    !ghcnobs%pipe_loc(1)=0
    !ghcnobs%pipe_loc(2)=0
    !ghcnobs%pipe_loc(3)=0
    !ghcnobs%pipe_loc(4)=0
    !ghcnobs%pipe_loc(5)=0
    !ghcnobs%pipe_loc(6)=0
    !ghcnobs%field(1)=''
    !ghcnobs%field(2)=''
    !ghcnobs%field(3)=''
    !ghcnobs%field(4)=''
...
!   read the file
!   first, read header
!   there are six pipes per variable; 1st pipe is before variable; past pipe before station id for variable
!   read(sf_data_unit,'(a7000)',iostat=eof)datline
    do while (eof >= 0 .and. .not. lbad .and. sdate <= sfend)
!       JAT 3/27/2026 change length to 10000
        read(sf_data_unit,'(a10000)',iostat=eof)datline
        lgo2=.false.
        ...
        ipipe=0
        npipes=0
        total_cloud=imiss_total
        opaque_cloud=imiss_opaque
        icloud(1)=imiss_total
        icloud(2)=imiss_opaque
        iline=iline+1
        if (eof == 0) then
!!           get the number of pipes
!   a4:     do ifield=1,len_trim(datline)
!               if (datline(ifield:ifield) == '|')npipes=npipes+1
!           enddo a4
!           if (npipes /= npipes_header) then
!!               issue error
!               write(aline,'(i20)')iline
!               write(msg_unit,ext_form(1))adjustl(pathid(ipath)),'E40',modnam,'MISMATCH IN NUMBER OF DATA DELIMITERS; LINE #:',trim(adjustl(aline))
!               lbad=.true.
!                write(ghcn_units(1),'(a)')trim(adjustl(datline)) !write to discard file, use for error check on this record
!               eof=2
!           endif
!           JAT 2/11/2026 A067_GHCNN check to see if a header line, this indicates possible concatenation of files, if a header, skip the record
            if (index(datline,'|Station_name|') > 0) eof=2
            
        endif
        if (eof == 0) then
!           reset ghcnobs
            
!           JAT 12/9/2025 A067_GHCNN reset variables to missing values
    v1:     do ivar=1,nvar
    a5:         do ifield=1,ghcnobs(ivar)%nfields
                    ghcnobs(ivar)%pipe_loc(ifield)=0
                    ghcnobs(ivar)%field(ifield)=''
                enddo a5
                if (ghcnobs(ivar)%sfvar == 0) cycle v1
                if (ivar == 8) then 
                    ghcnobs(ivar)%rvar=0.0_r8 !initialize precip to 0 (consistent with ISHD processing)
                else
                    ghcnobs(ivar)%rvar=real(sfvars(ghcnobs(ivar)%sfvar)%missval,r8)
                endif
            enddo v1
            ghcnobs%lmissing=.true.
...
!           get the location of the pipe delimiters and set locations for each variable
!           note for ivar = 0 (station info) pipe_loc(1) is column 1, not the first pipe
            ivar=0
            ipipe=1
    f2:     do ifield=1,len_trim(datline)
                if (datline(ifield:ifield) == '|') then
                    npipes=npipes+1
                    if (mod(npipes-pipe_offset,6) == 0 .and. npipes >= 6) then
                        ivar=ivar+1
                        ipipe=0
                    endif
                    !write(*,*)ifield,npipes,pipe_offset,ivar
                    ipipe=ipipe+1
                    ghcnobs(ivar)%pipe_loc(ipipe)=ifield
                endif       
            enddo f2
            
!************** date ****************************************
!           if (lpor) then
!!               date/time are separate fields
!                j1=ghcnobs(0)%pipe_loc(3)+1
!               j2=ghcnobs(0)%pipe_loc(8)-1
!           else
!               j1=ghcnobs(0)%pipe_loc(3)+1
!               j2=ghcnobs(0)%pipe_loc(4)-1
!           endif
            j1=ghcnobs(0)%pipe_loc(4)+1
            j2=ghcnobs(0)%pipe_loc(9)-1
            read(datline(j1:j2),'(i4,4(1x,i2.2))',iostat=iflag)iyear,imonth,iday,ihr,imin
            if (iflag == 0) then
!               first reset the hour if minute is > than 0, with no gmt2lst 
!               this will help to make sure to get the last hour of the day before sfstart
!               not concerned at this point if the minute fits the increment tested below.
...
!               now check the date to see if within data window
    ...
    !               check the minute to see if within acceptable bounds
                    if (imin == 0 .or. imin >= (60-inc)) then !obs on the hour accepted
                        ...
                        nsfc(10)=nsfc(10)+1
                        write(ghcn_units(1),'(a)')trim(adjustl(datline))  !write to discard file
                    endif
!                   if within the window and within 30 minutes of hour, proceed
                    if (lgo2) then
!                       check the observation type to see if a supported format and if special or
!                       regular observation type; loop through the variables until a non-missing
!                       report type is found; skip the variable if the difference between j1 and j2 is -1
                        ivar=1
                        have_report=.false.
                        rpt_type=''
                        
                        do while (ivar <= nvar .and. .not. have_report)
!                           j1 and j2 are the text columns to read
                            j1=ghcnobs(ivar)%pipe_loc(4)+1
                            j2=ghcnobs(ivar)%pipe_loc(5)-1
                            if (j2 > j1) then
                                ndash=index(datline(j1:j2),'-')
                                if (ndash == 0) then
                                    rpt_type=datline(j1:j2)
                                else
                                    read(datline(j1:j1+ndash-2),'(a)')rpt_type
                                endif
                                have_report=.true.
                            else
                                ivar=ivar+1
                        ...
                        if (lfound) then
                           lgo3=.true. 
                            ...
                            nsfc(9)=nsfc(9)+1
                            lgo3=.false.
                            write(ghcn_units(1),'(a)')trim(adjustl(datline))
                        endif    
...
!                   check to see if a special obs
                    if (trim(adjustl(rpt_type)) == 'SAOSP' .or. trim(adjustl(rpt_type)) == 'FM16') then
                        spec_obs=.true.
                    else
                        reg_obs=.true.
                    endif
...
!**************   station ID ****************************************
                    j1=4
                    j2=ghcnobs(0)%pipe_loc(2)-1
                    read(datline(j1:j2),'(i8)',iostat=iflag1)iwban
                    
                    ivar=1
                    lfound=.false.
                    
                    do while (ivar <= 37 .and. .not. lfound)
                        j1=ghcnobs(ivar)%pipe_loc(6)+1
                        j2=ghcnobs(ivar+1)%pipe_loc(1)-1
                        if (j2 > j1) then
                            lfound=.true.
                            if (index(datline(j1:j2),'ICAO') == 0) then
                                ndash=index(datline(j1:j2),'-')
                                read(datline(j1:j1+ndash-1),*)iusaf
                            endif
                        else
                            ivar=ivar+1
                        endif
...
!                       also keep the first occurrence for subsequent checks in the data
                        ...
                        write(msg_unit,ext_form(1))adjustl(pathid(ipath)),'E47',modnam,'INVALID STATION ID FIELD:',datline(j1:j2)
                        lbad=.true.
                    endif
!**************   station ID ****************************************
                    
!**************   call sign **************************************** 
                    j1=ghcnobs(1)%pipe_loc(6)+1
                    j2=ghcnobs(2)%pipe_loc(1)-1
                    if (index(datline(j1:j2),'ICAO') > 0) then
                        ndash=index(datline(j1:j2),'-')
                        read(datline(j1+ndash:j2),*)call_sign4
                        read(datline(j1+ndash+1:j2),*)call_sign3
                    endif
...
!**************  station coordinates *******************************
                    !if (lpor) then
                    !   j1=ghcnobs(0)%pipe_loc(8)+1
                    !    j2=ghcnobs(0)%pipe_loc(9)-1
                    !else
                    !    j1=ghcnobs(0)%pipe_loc(4)+1
                    !    j2=ghcnobs(0)%pipe_loc(5)-1
                    !endif
                    j1=ghcnobs(0)%pipe_loc(9)+1
                    j2=ghcnobs(0)%pipe_loc(10)-1
                    read(datline(j1:j2),*,iostat=iflag1)lat
                    if (iflag1 /= 0) then
                        write(msg_unit,ext_form(1))adjustl(pathid(ipath)),'E44',modnam,'INVALID COORDINATES FIELD:',&
                            datline(j1:j2)
                        lbad=.true.
                    endif
                    j1=ghcnobs(0)%pipe_loc(5)+1
                    j2=ghcnobs(0)%pipe_loc(6)-1
                    read(datline(j1:j2),*,iostat=iflag1)lon
                    if (iflag1 /= 0) then
                        write(msg_unit,ext_form(1))adjustl(pathid(ipath)),'E44',modnam,'INVALID COORDINATES FIELD:',&
                            datline(j1:j2)
                        lbad=.true.
...
!**************  station elevation *******************************
                    !if (lpor) then
                    !    j1=ghcnobs(0)%pipe_loc(10)+1
                    !else
                    !   j1=ghcnobs(0)%pipe_loc(6)+1 
                    !endif
                    j1=ghcnobs(0)%pipe_loc(11)+1
                    j2=ghcnobs(1)%pipe_loc(1)-1
                    read(datline(j1:j2),*,iostat=iflag1)elev
                    if (iflag1 == 0) then
                        if (elev == 9999.0_r8) then
!                           if missing, use user elevation if provided, otherwise suggest adding elevation
                            if (user_elev) then 
...
!**************  read/process actual data ******************************  
!                   JAT 12/8/2025 A067_GHCN; assign values to fields and numeric values for numeric variables (temperature, wind direction, etc.)
                    
    v2:             do ivar=1,nvar
    p1:                 do ipipe=1,6
                            j1=ghcnobs(ivar)%pipe_loc(ipipe)
                            if (ipipe < 6) then
                                j2=ghcnobs(ivar)%pipe_loc(ipipe+1)
                            elseif (ivar < nvar) then
                                j2=ghcnobs(ivar+1)%pipe_loc(1)
                            else
                                j2=len_trim(datline)+1
                            endif
                            if ((j2-j1) > 1) then
                                ghcnobs(ivar)%field(ipipe)=datline(j1+1:j2-1)
                                ghcnobs(ivar)%lmissing=.false.
                                if (ipipe == 1) then
                                    select case(ivar)
                                    !case (1,2,3,4,5,6,8,9,10,20,21,22,25,27,29)
                                    case (1,2,3,4,5,6,8,9,10,20,21,22) 
                                    
!                                       field is not empty, check for a decimal, if no decimal, read as integer then convert
!                                       to real; otherwise read the real value
!                                       cloud cover, if no number then check for text
                                        if (index(ghcnobs(ivar)%field(ipipe),'.') > 0) then
                                            read(ghcnobs(ivar)%field(ipipe),*,iostat=iflag)rvar
                                            if (iflag == 0)ghcnobs(ivar)%rvar=rvar
                                        else
                                            read(ghcnobs(ivar)%field(ipipe),*,iostat=iflag)intvar
                                            if (iflag == 0)ghcnobs(ivar)%rvar=real(intvar,r8)
                                        endif
                                    case (26) !ceiling height
                                        read(ghcnobs(ivar)%field(ipipe),*,iostat=iflag)intvar
                                        
                                        if (iflag == 0) then
                                            if (intvar == 22000) then
                                                intvar=300
                                            else
                                                intvar=int(real(intvar,r8)/100._r8) !convert to km and multiply by 10
                                            endif
                                            ghcnobs(ivar)%rvar=real(intvar,r8)
                                        endif
                                    case (24) !total/opaque cloud cover
!                                       convert semi-colons to commas for easier reading
                                        
    x1:                                 do i=1,len_trim(ghcnobs(ivar)%field(ipipe))
                                            if (ghcnobs(ivar)%field(ipipe)(i:i) == ';') ghcnobs(ivar)%field(ipipe)(i:i)=','
                                        enddo x1
                                        read(ghcnobs(ivar)%field(ipipe),*,iostat=iflag)icloud(1),icloud(2)
        
!                                       convert from oktas to tenths
    x2:                                 do i=1,2
                                            if (icloud(i) > 1) then
                                                if (icloud(i) <=5 ) then ! 2 to 5 oktas, add 1
                                                    icloud(i)=icloud(i)+1
                                                elseif (icloud(i) <= 8) then !6 to 8 oktas, add 2
                                                    icloud(i)=icloud(i)+2
                                                elseif (icloud(i) <= 10) then !obscured
                                                    icloud(i)=10
                                                endif
                                            endif
                                        enddo x2
                                        total_cloud=icloud(1)
                                        opaque_cloud=icloud(2)
                                    case (27,28,29,30,35,36,37,38) !sky cover
                                        ncolon=index(ghcnobs(ivar)%field(1),':')
                                        read(ghcnobs(ivar)%field(1)(ncolon+1:ncolon+2),*,iostat=iflag)intvar
                                        if (iflag == 0) then
                                            if (intvar > 1) then
                                                if (intvar <= 5) then !2 to 5 oktas, add 1
                                                    intvar=intvar+1
                                                elseif (intvar <=8) then ! 6 to 8 oktas, add 2
                                                    intvar=intvar+2
                                                elseif (intvar <=10) then !obscured
                                                    intvar=10
                                                else
                                                    intvar=sfvars(6)%missval
                                                endif
                                            endif   
                                        elseif (trim(adjustl(ghcnobs(ivar)%field(1))) == 'CLR') then !clear
                                            intvar=0
                                        elseif (trim(adjustl(ghcnobs(ivar)%field(1))) == 'FEW') then !FEW could be 1/10 to 3/10, set to 2/10
                                            intvar=2
                                        elseif (trim(adjustl(ghcnobs(ivar)%field(1))) == 'SCT') then !SCT could be 4/10 to 5/10, set to max 5/10
                                            intvar=5
                                        elseif (trim(adjustl(ghcnobs(ivar)%field(1))) == 'BKN') then !BKN could be 6/10 to 8/10, set to max 7/10
                                            intvar=7
                                        elseif (trim(adjustl(ghcnobs(ivar)%field(1))) == 'OVC' .or. &
                                            trim(adjustl(ghcnobs(ivar)%field(1))) == 'VV' .or. &
                                            trim(adjustl(ghcnobs(ivar)%field(1))) == 'X') then !OVC or obscured 10/10
                                            intvar=10   
                                        else
                                            intvar=sfvars(6)%missval
                                        endif
        ...
        !write(999,'(i4,4(1x,i2),52(1x,a50),1x,a300)')iyear,imonth,iday,ihr,imin,(ghcnobs(ivar)%field(1),ivar=1,nvar)
          !write(995,'(i4,4(1x,i2),1x,a50,9(1x,a50),1x,a300)')iyear,imonth,iday,ihr,imin,ghcnobs(24)%field(1),&
             ! (ghcnobs(ivar)%field(1),ivar=26,30),(ghcnobs(ivar)%field(1),ivar=35,38),ghcnobs(nvar)%field(1)       
                    

!                   set asky if total cloud and opaque are missing
                    if (total_cloud == 999) total_cloud=imiss_total
                    if (opaque_cloud == 999) opaque_cloud=imiss_opaque
                    
                    ivar=38
                    asky=sfvars(6)%missval !initialize
                    if (asoshr(ihr,iday1) .and. total_cloud == imiss_total .and. opaque_cloud == imiss_opaque) then
                        do while (ivar > 34 .and. asky == sfvars(6)%missval)
                            if (.not. ghcnobs(ivar)%lmissing) then
                                asky=int(ghcnobs(ivar)%rvar)
                            else
                                ivar=ivar-1
                            endif
                        enddo
                        if (asky == sfvars(6)%missval) then
                            ivar=30
                            if (.not. ghcnobs(ivar)%lmissing) asky=int(ghcnobs(ivar)%rvar)
    x3:                     do ifield=29,27,-1
                                if (.not. ghcnobs(ivar)%lmissing) then
                                    if (asky /= sfvars(6)%missval) then
                                        asky=max(asky,int(ghcnobs(ivar)%rvar))
                                    else
                                        asky=int(ghcnobs(ivar)%rvar)
                                    endif
                                endif
...
!                   check the remarks field if total cloud cover, opaque cloud cover, and asky are missing.  if it starts with MET, check the cloud data to get ASKY
                    if (total_cloud == imiss_total .and. opaque_cloud == imiss_opaque .and. asky == sfvars(6)%missval .and. &
                        (ghcnobs(nvar)%field(1)(1:3) == 'MET' .or. ghcnobs(nvar)%field(1)(1:3) == 'SYN')) then
                        !nremarks=1 !number of fields is 1 more than number of spaces
                        nremarks=0
                        infield=.false.
                        inquote=.false.
    r1:                 do i=1,len_trim(ghcnobs(nvar)%field(1))
                            !if (iachar(ghcnobs(nvar)%field(1)(i:i)) == '32') nremarks=nremarks+1
                            if (ghcnobs(nvar)%field(1)(i:i)=='/') ghcnobs(nvar)%field(1)(i:i)='_'
                            !if (ghcnobs(nvar)%field(1)(i+1:i+1) /=' ') nremarks=nremarks+1
                            if (.not. infield .and. iachar(ghcnobs(nvar)%field(1)(i:i)) == 34) then
                !               JAT 3/31/2026 beginning of a field using double quote
                                infield=.true.
                                inquote=.true.
                                nremarks=nremarks+1
                            elseif (.not. infield .and. iachar(ghcnobs(nvar)%field(1)(i:i)) /= 32) then
                !               JAT 3/31/2026 non-blank character but not a double quote
                                infield=.true.
                                nremarks=nremarks+1
                            elseif (inquote .and. iachar(ghcnobs(nvar)%field(1)(i:i)) == 34) then
                !               JAT 3/31/2026 end of field
                                infield=.false.
                                inquote=.false.
                            elseif (.not. inquote .and. infield .and. iachar(ghcnobs(nvar)%field(1)(i:i)) == 32) then
                !               JAT 3/31/2026 end of field
                                infield=.false.
                            endif
                        enddo r1
                        
                        if (nremarks /= nremarks1) then
                            if (allocated(remarks)) deallocate(remarks)
                            allocate(remarks(nremarks))
                        endif
                        remarks=''
                        read(ghcnobs(nvar)%field(1),*)(remarks(i),i=1,nremarks)
                        nremarks1=nremarks
                        if (ghcnobs(nvar)%field(1)(1:3) == 'MET') then

    r2:                     do i=1,nremarks
                                isky=1
                                lfound=.false.
    !                           look for sky conditions, as we proceed in remarks field, we get higher altitudes
    !                           highest level is the total cloud
                                do while (isky <= 9 .and. .not. lfound)
                                    !if (trim(adjustl(remarks(i))) == trim(adjustl(sky(isky)))) then
                                    if (remarks(i)(1:len_trim(sky(isky))) == trim(adjustl(sky(isky)))) then
                                        lfound=.true.
                                        if (asky == sfvars(6)%missval) then
                                            asky=tenths(isky)
                                        else
                                            asky=max(asky,tenths(isky))
                                        endif
                                    endif
                                    isky=isky+1
                                enddo
                        ...
                        !do ivar=26,24,-2
                        !   if (.not. ghcnobs(ivar)%lmissing) then
                        !        intvar=int(ghcnobs(ivar)%rvar)
                        !       if (asky /= 99) then
                        !           asky=max(asky,intvar)
                        !       else
                        !           asky=intvar
                        !       endif
                        !   endif
    ...
    !                       total cloud cover is part of 3rd field
                            intvar=99
                            read(remarks(3)(1:1),*,iostat=iflag)intvar  
                            if (iflag == 0) then
                                if (intvar <= 5) then
                                    asky=intvar+1
                                elseif (intvar <=8) then
                                    asky=intvar+2
    ...
    !write(1002,'(i4,3(1x,i2),1x,3(1x,a10),1x,i2.2,1x,a300)')iyear,imonth,iday,ihr,ghcnobs(24)%field(1),&
    !                   ghcnobs(26)%field(1),ghcnobs(28)%field(1),total_cloud,ghcnobs(38)%field(1)
!                   cloud layers if total cloud cover is missing (note opaque is always missing)

!                   relative humidity; calculate if missing but temperature and dewpoint are not missing
                    if (ghcnobs(9)%lmissing .and. .not. ghcnobs(1)%lmissing .and. .not. ghcnobs(2)%lmissing) &
                        ghcnobs(9)%rvar=humidity(ghcnobs(1)%rvar,ghcnobs(2)%rvar)
!                   relative humidity; calculate if missing but temperature and dewpoint are not missing
                    
!                   wind direction
                    ivar=10
                    if (.not. ghcnobs(5)%lmissing .and. ghcnobs(5)%rvar == real(sfvars(ghcnobs(ivar)%sfvar)%missval,r8)) then
                        if (ghcnobs(5)%field(2)(1:1) == 'C' .or. ghcnobs(6)%field(2)(1:1) == 'C' .or. &
                            ghcnobs(6)%rvar==0.0_r8) then !calm
                            ghcnobs(5)%rvar=0.0_r8
                            nsfc(11)=nsfc(11)+1
                        elseif (ghcnobs(5)%field(2)(1:1) == 'V' .or. &
                            ghcnobs(6)%rvar == real(sfvars(ghcnobs(ivar)%sfvar)%missval,r8)) then
                            nsfc(12)=nsfc(12)+1
                        endif
                    endif
!                   wind direction
...
!                   ceiling height
!                   use lowest level with 5/8 or more coverage
                    !lfound=.false.
                    !ivar=24
                    !ceil_ht=real(sfvars(12)%missval,r8)
                    !do while (ivar <= 28 .and. .not. lfound)
                    !   if (.not. ghcnobs(ivar)%lmissing .and. .not. ghcnobs(ivar+1)%lmissing .and. int(ghcnobs(ivar)%rvar) >= 6) then
                    !       ceil_ht=ghcnobs(ivar+1)%rvar/100._r8 !ceiling height in km*10 (ceiling height is in m so covert to km*10)
                    !       lfound=.true.
                    !   else
                    !       ivar=ivar+2
                    !   endif
...
!                   visibility scale to km*10
                    ivar=21
                    if (.not. ghcnobs(ivar)%lmissing) ghcnobs(ivar)%rvar=ghcnobs(ivar)%rvar*10._r8
!                   visibility scale to km*10
                    
!                   present weather, use MW1 and MW2
                    !ivar=11
                    ipcode=0
                    ipcode1=0
    vp:             do ivar=11,12
                        w1=0
                        if (.not. ghcnobs(ivar)%lmissing) then
                            !write(1003,'(i4,3(1x,i3),1x,a300)')iyear,imonth,iday,ihr,ghcnobs(ivar)%field(1)
!                           find the colon for weather code
                            ncolon=index(ghcnobs(ivar)%field(1),':')
                            if (ncolon == 0) then
!                               no real weather
                                read(ghcnobs(ivar)%field(1)(1:2),'(i2)')w1
                            else
                                read(ghcnobs(ivar)%field(1)(ncolon+1:ncolon+2),'(i2)')w1
                            endif
...
!                           JAT 8/25/22 A018_ISHD_W1; correct logic to say if w1 == 21 .or. w1 == 25)
                    ...
                    enddo vp
                    if (ipcode1(1) <= 2 .and. ipcode1(2) == ipcode1(1)) then
                        ipcode=ipcode1(1)
                    elseif ((ipcode1(1) == 1 .and. ipcode1(2) == 2) .or. (ipcode1(1) == 2 .and. ipcode1(2) == 1)) then
                        ipcode=3
                    else
                        ipcode=max(ipcode1(1),ipcode1(2))
                    endif
                    
!                   precip is 0 or missing, then ipcode =0
                    if (ghcnobs(8)%rvar == real(sfvars(1)%missval,r8) .or. ghcnobs(8)%rvar == 0.0) ipcode=0
                        
!                   present weather, use MW1 MW2
...
!                   station pressure
                    ...
                    if (ghcnobs(pvar)%lmissing .and. sfdata1(3,ihr,iday1) == real(sfvars(3)%missval,r8)) then
                        if (.not. ghcnobs(altvar)%lmissing .and. ghcnobs(slpvar)%lmissing) then
!                           calculate station pressure from altimeter and virtual temperature
!                           of 290K
                            ghcnobs(pvar)%rvar=pressure(2,sfelev,290._r8,ghcnobs(altvar)%rvar)
!                           now calculate sea level pressure from calculated station pressure
                        endif
                        if (ghcnobs(altvar)%lmissing .and. ghcnobs(slpvar)%lmissing) then
                            if (have_elev .and. .not. user_elev) &
!                               calculate station pressure from elevation in data file and standard atmosphere
                                ghcnobs(pvar)%rvar=1013.25_r8*(1.0_r8-((6.5e-3_r8/288.15_r8)*sfelev))**5.255_r8
                        endif
                        if (.not. ghcnobs(slpvar)%lmissing) then
                            if (have_elev .and. .not. user_elev) &
!                               calculate station pressure from elevation and sea level pressure
                                ghcnobs(pvar)%rvar=pressure(2,sfelev,290._r8,ghcnobs(slpvar)%rvar)
                        endif
                    else
                        if (ghcnobs(slpvar)%lmissing .and. sfdata1(2,ihr,iday1) == real(sfvars(2)%missval,r8)) then
                            if (have_elev .and. .not. user_elev) &
!                               calculate sea level pressure from elevation and station pressure
                                ghcnobs(slpvar)%rvar=pressure(1,sfelev,290._r8,ghcnobs(pvar)%rvar)
                        endif
                    endif
!**************  read/process actual data ******************************  
...
! non-special obs up to and including the hour (minute=0). only use special
! obs if there are no regular observations available for the hour.
           !write(998,'(i4,4(1x,i2),1x,3(1x,i3))')iyear,imonth,iday,ihr,imin,total_cloud,opaque_cloud,asky         
                    if (nomisshr(ihr,iday1)) then
!                       JAT 2/25/2026 A100 (A067_GHCNN) initialize loverwrite to true
                        loverwrite=.true.
            
                        call check_dup(iday1,ihr,ghcnobs(8)%rvar,ghcnobs(1)%rvar,ghcnobs(5)%rvar,ghcnobs(6)%rvar,&
                            real(total_cloud,r8),real(opaque_cloud,r8),real(asky,r8),miss_precip,miss_temp,miss_dir,miss_wind,&
                            miss_cloud,writedat)
            !write(996,'(i4,4(1x,i2),1x,l1,1x,i4,2(1x,i3,1x,l1),1x,i4,1x,i3,1x,l1)')iyear,imonth,iday,ihr,imin,writedat,&
            !   int(sfdata1(4,ihr,iday1)),total_cloud,miss_cloud(1),opaque_cloud,miss_cloud(2),int(sfdata1(6,ihr,iday1)),&
            !   asky,miss_cloud(3)
                        if (spec_obs .and. reg_obs)  then
...
!                           also increment duplicate counter
                            !increment non-used special; nsfc(3) remains unchanged since replacing special with special
                            if (spec_obs .and. (trim(adjustl(rpt_use)) == 'SAOSP' .or. trim(adjustl(rpt_use)) == 'FM16')) &
                            nsfc(6)=nsfc(6)+1
                            if (reg_obs) then
...
!                               if this is a regular obs and the last valid obs was a special obs,
!                               increment nsfc(6), take one away from nsfc(3), and increment nsfc(4)
                                if (trim(adjustl(rpt_use)) == 'SAOSP' .or. trim(adjustl(rpt_use)) == 'FM16') then
                                    nsfc(4)=nsfc(4)+1 !increment regular obs used only if last report was special
                                    nsfc(6)=nsfc(6)+1 !increment non-used special obs
                                    nsfc(3)=nsfc(3)-1 !take one way from used special obs
                                else
...
!                           write message that data is being overwritten and write last data record to replace records file
                            ...
                            write(ghcn_units(2),'(a)')trim(adjustl(lastline))
...
!                           precip code
                            ...
                            if (.not. miss_precip .and. (ghcnobs(8)%rvar > 0.0_r8 .or. (sfdata1(1,ihr,iday1) == &
                                real(sfvars(1)%missval,r8) .and. ghcnobs(8)%rvar == 0.0_r8))) then
                                sfdata1(1,ihr,iday1)=ghcnobs(8)%rvar      
                                sfdata1(5,ihr,iday1)=real(ipcode,r8)
                            endif
                            
                            
                            if (ghcnobs(4)%rvar == real(sfvars(2)%missval,r8) .and. &
                                sfdata1(2,ihr,iday1) /= real(sfvars(2)%missval,r8)) loverwrite(4)=.false. !sea level pressure
                            if (ghcnobs(3)%rvar == real(sfvars(3)%missval,r8) .and. &
                                sfdata1(3,ihr,iday1) /= real(sfvars(3)%missval,r8)) loverwrite(3)=.false. !station pressure
                            if (ghcnobs(2)%rvar == real(sfvars(8)%missval,r8) .and. &
                                sfdata1(8,ihr,iday1) /= real(sfvars(8)%missval,r8)) loverwrite(2)=.false. !dewpoint
                            if (ghcnobs(9)%rvar == real(sfvars(9)%missval,r8) .and. &
                                sfdata1(9,ihr,iday1) /= real(sfvars(9)%missval,r8)) loverwrite(9)=.false.   !RH
                            
    v3:                     do ivar=1,nvar
               !                 if (ghcnobs(ivar)%lmissing .or. ghcnobs(ivar)%sfvar == 0 .or. ivar == 12 .or. ivar == 20 .or. &
                                    !ivar ==21 .or. ivar == 4 .or. ivar == 6) cycle v3
                                if (.not. loverwrite(ivar) .or. ghcnobs(ivar)%lmissing .or. ghcnobs(ivar)%sfvar == 0 .or. &
                                    ghcnobs(ivar)%sfvar >= 12 .or. ivar == 8)cycle v3
                                sfdata1(ghcnobs(ivar)%sfvar,ihr,iday1)=ghcnobs(ivar)%rvar
    
                            enddo v3
        
!                           TSKC    
!                           JAT A067; do not use total_cloud if miss_cloud(1); use current value
                            if (miss_cloud(1)) total_cloud=int(sfdata1(4,ihr,iday1))/100
                            if (miss_cloud(2)) opaque_cloud=int(sfdata1(4,ihr,iday1))-(int(sfdata1(4,ihr,iday1))/100)*100
    
!                           JAT if total and opaque are not missing, don't let opaque be more than total cloud cover
                            if (total_cloud >= 0 .and. total_cloud <= 10 .and. opaque_cloud >= 0 .and. opaque_cloud <= 10 .and. &
                                opaque_cloud > total_cloud) opaque_cloud=total_cloud
                            sfdata1(4,ihr,iday1)=real((total_cloud*100+opaque_cloud),r8)
!                           JAT A067; do not use asky if miss_cloud(3); use current value   
                            if (miss_cloud(3) .and. asoshr(ihr,iday1)) asky=int(sfdata1(6,ihr,iday1))   
                            sfdata1(6,ihr,iday1)=real(asky,r8)


 
                            rpt_use=rpt_type !reset rpt_type
!                           if ceiling height, ASOS ceiling, visibility, or wet bulb temperature are audited, write variables
!                           these are the only extra variables from sfdata that are in ISHD data
                            if (sfvars(12)%laudit .or. sfvars(20)%laudit .or. sfvars(21)%laudit .or. sfvars(22)%laudit) then
                                ifield1=0 !used to determine where in sfdata1 this variable is
    v4:                         do ifield=nsfvars_keep+1,nsfvars
                                    if (sfvars(ifield)%laudit)ifield1=ifield1+1
                                    if ((ifield == 12 .or. ifield == 20 .or. ifield == 21 .or. ifield == 22) .and. &
                                        sfvars(ifield)%laudit) then
                                        if (ifield == 12 .and. ghcnobs(26)%rvar /= real(sfvars(ifield)%missval,r8)) &
                                            sfdata1(ifield1+nsfvars_keep,ihr,iday1)=ghcnobs(26)%rvar !ceiling
                                        !if (ifield == 20) sfdata1(ifield1+nsfvars_keep,ihr,iday1)=ishobs(15) !asos ceiling
                                        if (ifield == 21 .and. ghcnobs(21)%rvar /= real(sfvars(ifield)%missval,r8)) &
                                            sfdata1(ifield1+nsfvars_keep,ihr,iday1)=ghcnobs(21)%rvar !visibility
                                        if (ifield == 22 .and. ghcnobs(10)%rvar /= real(sfvars(ifield)%missval,r8)) &
                                            sfdata1(ifield1+nsfvars_keep,ihr,iday1)=ghcnobs(10)%rvar !wet bulb temperature
                                    endif
                                enddo v4
                            endif
                        endif
...
!                       write message if special obs
    ...
    v5:                 do ivar=1,nvar
                            !if (ghcnobs(ivar)%lmissing .or. ghcnobs(ivar)%sfvar == 0 .or. ivar == 10 .or. ivar == 12 .or. &
                            !   ivar == 20 .or. ivar ==21 .or. ghcnobs(ivar)%sfvar >= 12) cycle v5
                            if ((ghcnobs(ivar)%lmissing .and. ivar /= 8) .or. ghcnobs(ivar)%sfvar == 0 .or. &
                                ghcnobs(ivar)%sfvar >= 12 .or. ivar == 12) cycle v5
                            !write(*,*)ivar,ghcnobs(ivar)%sfvar
                            sfdata1(ghcnobs(ivar)%sfvar,ihr,iday1)=ghcnobs(ivar)%rvar
                        enddo v5
!                       TSKC    
                        sfdata1(4,ihr,iday1)=real((total_cloud*100+opaque_cloud),r8)
...
!                       these are the only extra variables from sfdata that are in ISHD data
                            if (sfvars(12)%laudit .or. sfvars(20)%laudit .or. sfvars(21)%laudit) then
                                ifield1=0 !used to determine where in sfdata1 this variable is
    v6:                         do ifield=nsfvars_keep+1,nsfvars
                                    if (sfvars(ifield)%laudit)ifield1=ifield1+1
                                    if ((ifield == 12 .or. ifield == 20 .or. ifield == 21 .or. ifield == 22) .and. &
                                        sfvars(ifield)%laudit) then
                                        if (ifield == 12) sfdata1(ifield1+nsfvars_keep,ihr,iday1)=ghcnobs(26)%rvar !ceiling
                                        !if (ifield == 20) sfdata1(ifield1+nsfvars_keep,ihr,iday1)=ishobs(15) !asos ceiling
                                        if (ifield == 21) sfdata1(ifield1+nsfvars_keep,ihr,iday1)=ghcnobs(21)%rvar !visibility
                                        if (ifield == 22) sfdata1(ifield1+nsfvars_keep,ihr,iday1)=ghcnobs(10)%rvar !wet bulb temperature
                                    endif
                                enddo v6
                            endif
                        !endif
...
!                   after processing, set last_rpt type
                    savedate=sdate
                    savehr=ihr      
...
!               after processing, set savedate and savehr
                !savedate=sdate
                !savehr=ihr 
            else !bad date string
                write(msg_unit,ext_form(1))adjustl(pathid(ipath)),'E42',modnam,'INVALID DATE FIELD:',datline(j1:j2)
                lbad=.true.
                write(ghcn_units(1),'(a)')trim(adjustl(datline)) !write to discard file, use for error check on this record
            endif
        endif
...
!   close the discard and replace files
    f12: do ifile=1,2
        close(ghcn_units(ifile))
    enddo f12
    return
    end subroutine read_ghcn
!*********************************************************************************************************
...
!   JAT 12/22/2025 A062 (A067_GHCNN); add miss_cloud
!   JAT 2/26/2026 A100 (A067_GHCN); add loverwrite
    !logical :: crossover,gotvis,lfound,asoshr1,lgo,lsavewban,miss_wind,miss_dir,miss_precip,miss_temp,writedat
    logical :: crossover,gotvis,lfound,asoshr1,lgo,lsavewban,miss_wind,miss_dir,miss_precip,miss_temp,miss_cloud(3),writedat,&
        loverwrite(12)
...
!   JAT 07/13/22 A005_SAVEHR_UNINIT
!   ivis set above comment this line out
    !ivis=0
    gotvis=.false.
    icent=0
    irec=0
...
! JAT 12/7/2023 A050_COARE: add temperature conversion for negative temperatures
...
!                   JAT 2/26/2026 A100 (A067_GHCN) set loverwrite to true
                    loverwrite=.true.
                    
                    if (nomisshr(ihr,iday1)) then
!                       enter a missing value for precip when checking for duplicates
    !                   JAT 12/22/2025 A062 (A067_GHCNN) add cloud cover variables as variables to check
...
!                       JAT 4/1/2026 A100 correct wdir and wspd reference
                        !call check_dup(iday1,ihr,real(sfvars(1)%missval,r8),cd144_obs(5),cd144_obs(10),cd144_obs(11),real(sky(1),r8),&
                        call check_dup(iday1,ihr,real(sfvars(1)%missval,r8),cd144_obs(5),cd144_obs(9),cd144_obs(8),real(sky(1),r8),&
                            real(sky(2),r8),real(sfvars(6)%missval,r8),miss_precip,miss_temp,miss_dir,miss_wind,miss_cloud,writedat)
!                       JAT 12/22/2025 A062 (A067_GHCNN); if miss_cloud(1), do not use sky(1)
                        if (miss_cloud(1)) sky(1)=int(sfdata1(4,ihr,iday1))/100
!                       JAT 12/22/2025 A062 (A067_GHCNN); if miss_cloud(2), do not use sky(2)
                        if (miss_cloud(2)) sky(2)=int(sfdata1(4,ihr,iday1))-(int(sfdata1(4,ihr,iday1))/100)*100
                        
!                       JAT 3/2/2026 A062 (A067_GHCNN); if total and opaque are not missing, don't let opaque be more than total cloud cover
                        if (sky(1) >= 0 .and. sky(1) <= 10 .and. sky(2) >= 0 .and. sky(2) <= 10 .and. sky(2) > sky(1)) &
                            sky(2)=sky(1)
                        
!                       JAT 12/22/2025 recalculate cd144_obs(3)
                        cd144_obs(3)=real((sky(1)*100+sky(2)),r8)
                        
!                       JAT 2/26/2026 A100 (A067_GHCN); do not overwrite valid obs with missing observation
                            
                        if (cd144_obs(1) == real(sfvars(2)%missval,r8) .and. &
                            sfdata1(2,ihr,iday1) /= real(sfvars(2)%missval,r8)) loverwrite(1)=.false. !sea level pressure
                    
                        if (cd144_obs(2) == real(sfvars(3)%missval,r8) .and. &
                            sfdata1(3,ihr,iday1) /= real(sfvars(2)%missval,r8)) loverwrite(2)=.false. !station pressure
                        
                        if (cd144_obs(6) == real(sfvars(8)%missval,r8) .and. &
                            sfdata1(8,ihr,iday1) /= real(sfvars(8)%missval,r8)) loverwrite(6)=.false. !dewpoint
                        
                        if (cd144_obs(7) == real(sfvars(9)%missval,r8) .and. &
                            sfdata1(9,ihr,iday1) /= real(sfvars(9)%missval,r8)) loverwrite(7)=.false. !RH
                        
                        if (cd144_obs(4) == real(sfvars(5)%missval,r8) .and. &
                            sfdata1(5,ihr,iday1) /= real(sfvars(5)%missval,r8)) loverwrite(4)=.false. !pwth
...
!                       note precip not in CD144 data and ASKY not in data
    v1:                 do svar=2,5
!                           JAT 2/26/2026 A100 (A067_GHCN) ; skip variable if not overwriting
                            if (.not. loverwrite(svar-1)) cycle v1
                            sfdata1(svar,ihr,iday1)=cd144_obs(svar-1)
                        enddo v1
                        
    v2:                 do svar=7,11
!                           JAT 2/26/2026 A100 (A067_GHCN) ; skip variable if not overwriting
                            if (.not. loverwrite(svar-2)) cycle v2
                            sfdata1(svar,ihr,iday1)=cd144_obs(svar-2)
                        enddo v2
!                       if ceiling height, visbility, or wet bulb temp audited, write
!                       JAT 3/2/2026 A100 (A067_GHCNN); do not overwrite valid values with missing values
    ...
    v3:                     do svar=nsfvars_keep+1,nsfvars
                                ...
                                if ((svar == 12 .or. svar >= 21) .and. sfvars(svar)%laudit) then
                                    if (svar == 12 .and. cd144_obs(10) /= real(sfvars(12)%missval,r8)) &
                                        sfdata1(ivar+nsfvars_keep,ihr,iday1)=cd144_obs(10) !ceiling
                                    if (svar >= 21 .and. cd144_obs(svar-10) /= real(sfvars(svar)%missval,r8)) &
                                        sfdata1(ivar+nsfvars_keep,ihr,iday1)=cd144_obs(svar-10) !visibility or wet bulb temperature
                                endif
                            enddo v3
                          endif  
                    endif
...
!   JAT 12/29/2025 A062 (A067_GHCNN); add miss_cloud
    ...
    logical :: crossover,lfound,asoshr1,lgo,lsavewban,miss_wind,miss_dir,miss_temp,miss_precip,miss_cloud(3),writedat
...
!                       JAT A062 (A067_GHCNN); change TSKC calculations to match other data types, treat each one separately
!                       currently if total or opqaue is missing the entire TSKC code is set to missing when one of them could
!                       actually be valid
    s1:                 do is = 1,2
                            if (sky(is) == -9999 .or. sky(is) == -99999 .or. sky(is) >= 10) then
                                if (is == 1) then
...
! whether to overwrite exsint obs with latest one.
                    ...
                    if (nomisshr(ihr,iday1)) then
!                       enter a missing value for precip when checking for duplicates
!                       JAT 12/29/2025 A062 (A067_GHCNN); add cloud cover check
                        !call check_dup(iday1,ihr,real(sfvars(1)%missval,r8),scram_obs(2),scram_obs(3),scram_obs(4),miss_precip,&
                        !    miss_temp,miss_dir,miss_wind,writedat)
                        call check_dup(iday1,ihr,real(sfvars(1)%missval,r8),scram_obs(2),scram_obs(3),scram_obs(4),&
                            real(sky(1),r8),real(sky(2),r8),real(sfvars(6)%missval,r8),miss_precip,miss_temp,miss_dir,&
                            miss_wind,miss_cloud,writedat)
...
!                       TSKC
                        if (.not. asoshr(ihr,iday1)) then
!                           JAT 12/29/2025 A062 (A067_GHCNN); only check missing cloud data if not an ASOS obs
!                           JAT 12/29/2025 A062 (A067_GHCNN); if miss_cloud(1), do not use sky(1)
                            if (miss_cloud(1)) sky(1)=int(sfdata1(4,ihr,iday1))/100
!                           JAT 12/29/2025 A062 (A067_GHCNN); if miss_cloud(2), do not use sky(2)
                            if (miss_cloud(2)) sky(2)=int(sfdata1(4,ihr,iday1))-(int(sfdata1(4,ihr,iday1))/100)*100
!                           JAT 3/2/2026 A062 (A067_GHCNN); if total and opaque are not missing, don't let opaque be more than total cloud cover
                            if (sky(1) >= 0 .and. sky(1) <= 10 .and. sky(2) >= 0 .and. sky(2) <= 10 .and. &
                                sky(2) > sky(1)) sky(2)=sky(1)
                            scram_obs(1)=real((sky(1)*100+sky(2)),r8)
                        endif
...
!                       JAT 3/2/2026 A100 (A067_GHCNN); only write if not missing
                        if (sfvars(12)%laudit) then
                            ivar=0
    v3:                     do svar=nsfvars_keep+1,nsfvars
                                if (sfvars(svar)%laudit) ivar=ivar+1
                                !if (svar == 12 .and. sfvars(svar)%laudit) sfdata1(ivar+nsfvars_keep,ihr,iday1)=scram_obs(5) !ceiling
                                if (svar == 12 .and. sfvars(svar)%laudit .and. scram_obs(5) /= real(sfvars(12)%missval,r8)) &
                                    sfdata1(ivar+nsfvars_keep,ihr,iday1)=scram_obs(5) !ceiling
                            enddo v3
                          endif  
                    endif
...
!   JAT 12/29/2025 A062 (A067_GHCNN); add miss_cloud
!   JAT 2/26/2026 A100 (A067_GHCNN); add loverwrite
    !logical :: crossover,lfound,gotpress,good_date,have_elev,asoshr1,lgo,miss_wind,miss_dir,miss_temp,miss_precip,writedat
    logical :: crossover,lfound,gotpress,good_date,have_elev,asoshr1,lgo,miss_wind,miss_dir,miss_temp,miss_precip,miss_cloud(3),&
        writedat,loverwrite(11)
...
!                       check date/hr
                        call new_obs(iyear,imonth,iday,ihr,savedate,savehr,lgo,iday1,sdate)
!                       JAT 3/13/23 A032_A033_A034; bug A033 set date_hr_str
                        write(date_hr_str,'(a5,1x,i8,1x,a2,1x,i2.2)')'DATE:',sdate,'HR',ihr                    
                        if (lgo) then !proceed
...
!                                           JAT 3/6/2026 A100 (A067_GHCNN); only calculate if current value in sfdata1 is missing
                                            !if (have_elev .or. user_elev) sam_obs(2)=1013.25_r8*(1.0_r8-((6.5e-3_r8/288.15_r8)&
                                            !*sfelev))**5.255_r8
                                            
                                            if ((have_elev .or. user_elev) .and. sfdata1(3,ihr,iday1) == &
                                                real(sfvars(3)%missval,r8)) sam_obs(2)=1013.25_r8*(1.0_r8-((6.5e-3_r8/288.15_r8)&
                                                *sfelev))**5.255_r8
                                        endif
...
!**************         process weather variables ******************************
!                           write the data
!                           JAT 2/26/2026 A100 (A067_GHNN); set loverwrite to true
                            loverwrite=.true.
...
!                               JAT 12/29/2025 A062 (A067_GHCNN) add cloud cover variables as variables to check
                                ...
                                call check_dup(iday1,ihr,sam_obs(1),sam_obs(5),sam_obs(8),sam_obs(9),real(sky(1),r8),&
                                    real(sky(2),r8),real(sfvars(6)%missval,r8),miss_precip,miss_temp,miss_dir,miss_wind,&
                                    miss_cloud,writedat)
...
!                               JAT 12/29/2025 recalculate sam_obs(3)
                                sam_obs(3)=real((sky(1)*100+sky(2)),r8)
                                
!                               JAT 2/26/2026 do not overwrite precip, station pressure, sea level pressure, RH, dewpoint
                                if (miss_precip .or. (sam_obs(1) == 0.0_r8 .and. sfdata1(1,ihr,iday1) /= &
                                    real(sfvars(1)%missval,r8))) then
                                    loverwrite(1)=.false.
                                    loverwrite(4)=.false.
                                endif
                            
                                if (sam_obs(2) == real(sfvars(3)%missval,r8) .and. sfdata1(3,ihr,iday1) /= &
                                    real(sfvars(3)%missval,r8)) loverwrite(2)=.false. !station pressure
                                if (sam_obs(6) == real(sfvars(8)%missval,r8) .and. sfdata1(8,ihr,iday1) /= &
                                    real(sfvars(8)%missval,r8)) loverwrite(6)=.false. !dewpoint
                                if (sam_obs(7) == real(sfvars(9)%missval,r8) .and. sfdata1(9,ihr,iday1) /= &
                                    real(sfvars(9)%missval,r8)) loverwrite(7)=.false. !RH
...
!                               note ASKY not in data
                                if (loverwrite(1)) sfdata1(1,ihr,iday1)=sam_obs(1)
    v4:                         do ivar=2,9
!                                   JAT 2/26/2026 A100 (A067_GHCNN); if not overwriting value, skip
                                    if (.not. loverwrite(ivar)) cycle v4
...
!                               JAT 3/20/2026 A100 (A067_GHCNN); only write if not missing (covers first obs for hour and duplicates)
    ...
    v5:                             do svar=nsfvars_keep+1,nsfvars
                                        ...
                                        if ((svar == 12 .or. svar == 21) .and. sfvars(svar)%laudit) then
                                            ...
                                            if (svar == 12 .and. sam_obs(10) /= real(sfvars(12)%missval,r8)) &
                                                sfdata1(ivar+nsfvars_keep,ihr,iday1)=sam_obs(10) !ceiling
                                            if (svar == 21 .and. sam_obs(11) /= real(sfvars(21)%missval,r8)) &
                                                sfdata1(ivar+nsfvars_keep,ihr,iday1)=sam_obs(11) !visibility
                                        endif
                                    enddo v5
                                endif  
                            endif                        
...
!
!   Real variables
!   huswo_obs:        11-element array of variables read from HUSWO format data file
!   rtemp:          real variable read from data file for real variables
!
!   Logical variables
!   lfound:         logical variable denoting station found in ASOS list
...
!   JAT 2/26/2026 A100 (A067_GHCNN); add loverwrite
    ...
    logical :: lfound,asoshr1,lgo,miss_wind,miss_dir,miss_temp,miss_precip,miss_cloud(3),writedat,loverwrite(14)
...
!**************         process variables ***********************************                    
!                       write the data
!                       JAT 2/26/2026 A100 (A067_GHCNN); set loverwrite to true
                        loverwrite=.true.
    ...
    !                       JAT 12/29/2025 A062 (A067_GHCNN) add cloud cover variables as variables to check
                            ...
                            !    miss_dir,miss_wind,writedat)
                            call check_dup(iday1,ihr,huswo_obs(1),huswo_obs(7),huswo_obs(10),huswo_obs(11),real(sky(1),r8),&
                                real(sky(2),r8),huswo_obs(6),miss_precip,miss_temp,miss_dir,miss_wind,miss_cloud,writedat)
...
!                           JAT 12/22/2025 recalculate huswo(4)
                            huswo_obs(4)=real(sky(1)*100+sky(2),r8)
                            
!                           JAT 2/26/2026 do not overwrite precip, station pressure, sea level pressure, RH, dewpoint
                            if (miss_precip .or. (huswo_obs(1) == 0.0_r8 .and. sfdata1(1,ihr,iday1) /= &
                                real(sfvars(1)%missval,r8))) then
                                loverwrite(1)=.false.
                                loverwrite(5)=.false.
                            endif
                            if (huswo_obs(2) == real(sfvars(2)%missval,r8) .and. sfdata1(2,ihr,iday1) /= &
                                real(sfvars(2)%missval,r8)) loverwrite(2)=.false. !sea level pressure
                            if (huswo_obs(3) == real(sfvars(3)%missval,r8) .and. sfdata1(3,ihr,iday1) /= &
                                real(sfvars(3)%missval,r8)) loverwrite(3)=.false. !station pressure
                            if (huswo_obs(8) == real(sfvars(8)%missval,r8) .and. sfdata1(8,ihr,iday1) /= &
                                real(sfvars(8)%missval,r8)) loverwrite(8)=.false. !dewpoint
                            if (huswo_obs(9) == real(sfvars(9)%missval,r8) .and. sfdata1(9,ihr,iday1) /= &
                                real(sfvars(9)%missval,r8)) loverwrite(9)=.false. !RH
...
!                           core variables
    ...
    v7:                     do ivar=1,11
!                               JAT 2/26/2026 A100 (A067_GHCNN); if not overwriting value, skip
                                if (.not. loverwrite(ivar)) cycle v7
                                sfdata1(ivar,ihr,iday1)=huswo_obs(ivar)
                            enddo  v7
!                           if ceiling height or visbility audited, write
!                           JAT 3/2/2026 A100 (A067_GHCNN); only overwrite if valid observation
                            if (sfvars(12)%laudit .or. sfvars(20)%laudit .or. sfvars(21)%laudit) then
                                ivar=0
    v8:                         do svar=nsfvars_keep+1,nsfvars
                                    if (sfvars(svar)%laudit) ivar=ivar+1
                                    if ((svar == 12 .or. svar == 21) .and. sfvars(svar)%laudit) then
                                        if (svar == 12 .and. huswo_obs(12) /= real(sfvars(12)%missval,r8)) &
                                            sfdata1(ivar+nsfvars_keep,ihr,iday1)=huswo_obs(12) !CLHT
                                        if ((svar == 20 .or. svar == 21) .and. huswo_obs(svar-7) /=&
                                             real(sfvars(svar)%missval,r8)) sfdata1(ivar+nsfvars_keep,ihr,iday1)=huswo_obs(svar-7) !ACHT or HZVS
                                    endif
                                enddo v8
                            endif  
                        endif                        
...
!========================================================================================================= 
    ...
    implicit none
!   JAT 12/29/2025 A062 (A067_GHCNN); add sky integer array
    integer(kind=4) :: i,eof,ifile,sky(2),dates1(2,3),gmt2lst,iwban,i1,irec,ivar,iday1,iflag,ihr,iyear,imonth,iday,savehr,iwarn
    integer(kind=8) :: savedate=0
    integer(kind=8) :: sdate
    real(kind=r8), allocatable, dimension(:) :: rsfvars
!   JAT 12/29/2025 A062 (A067_GHCNN); add miss_cloud
    logical :: lfound,asoshr1,lgo,miss_wind,miss_dir,miss_temp,miss_precip,miss_cloud(3),writedat
!   JAT 2/26/2026 A100 (A067_GHCNN); add loverwrite
    logical, allocatable, dimension(:) :: loverwrite
    ...
    character(len=6) :: form1
    character(len=20) :: date_hr_str
!  JAT 1/6/2025 A030_1MIN; increase formstr from 3-element to 4-element
!    character(len=80) :: formstr(3)
    character(len=80) :: formstr(4)
    character(len=10) :: modnam='READ_EXT'
    
!   JAT 2/26/2026 A100 (A067_GHCNN); allocate loverwrite to same dimension as rsfvars
    !allocate(rsfvars(nsfvars_keep))
    allocate(rsfvars(nsfvars_keep),loverwrite(nsfvars_keep))
...
!  JAT 1/6/2025 A030_1MIN; message that AERMINUTE output will not be read in
!   4. AERMINUTE output will not be read in
    write(formstr(4),'(2(a))')trim(adjustl(msg_form)),'a,1x,a)'
...
!               check to see if station is an ASOS station
                ...
                if (.not. lasos) write(msg_unit,formstr(2))adjustl(pathid(ipath)),'W54',modnam,'STATION',trim(adjustl(sfid)),&
                    'NOT LISTED IN ASOS LIST; CHECK STATION ID'
!   JAT 1/6/2025 A030_1MIN; if WBAN not in ASOS list and 1-MINUTE data to be read, do not read and issue warning that 
!   AERMINUTE output will not be read
                if (.not. lasos .and. lsfkeywords(9)) write(msg_unit,formstr(4))adjustl(pathid(ipath)),'W56',modnam,&
                    'DO NOT PROCESS 1-MINUTE DATA'
            elseif (irec == 3) then 
...
!                       JAT 12/29/2025 A062 (A067_GHCNN); calculate total and opaque sky cover for input to check_dup
                        sky(1)=int(rsfvars(4))/100
                        sky(2)=int(rsfvars(4))-sky(1)*100
...
!                       JAT 2/26/2026 A100 (A067_GHCNN); set loverwrite to true
                        ...
                        if (nomisshr(ihr,iday1)) then
                            ...
                            !    miss_dir,miss_wind,writedat)
                            call check_dup(iday1,ihr,rsfvars(1),rsfvars(7),rsfvars(10),rsfvars(11),real(sky(1),r8),&
                                real(sky(2),r8),rsfvars(6),miss_precip,miss_temp,miss_dir,miss_wind,miss_cloud,writedat)
                            if (writedat) write(msg_unit,ext_form(5))adjustl(pathid(ipath)),'I45',modnam,&
                                'OBSERVATION REPLACED FOR',date_hr_str
...
!                           JAT 3/2/2026 A062 (A067_GHCNN); if total and opaque are not missing, don't let opaque be more than total cloud cover
                            if (sky(1) >= 0 .and. sky(1) <= 10 .and. sky(2) >= 0 .and. sky(2) <= 10 .and. sky(2) > sky(1)) &
                                sky(2)=sky(1)
!                           JAT 12/22/2025 recalculate rsfvars(4)
                            rsfvars(4)=real((sky(1)*100+sky(2)),r8)
                            

                            
!                           JAT 12/29/2025 A062 (A067_GHCNN); if miss_cloud(3), do not reset asky (use previous value)
                            if (miss_cloud(3)) loverwrite(6)=.false. !rsfvars(6)=sfdata1(6,ihr,iday1)
                            
!                           JAT 2/26/2026 do not overwrite precip, station pressure, sea level pressure, RH, dewpoint
                            if (miss_precip .or. (rsfvars(1) == 0.0_r8 .and. sfdata1(1,ihr,iday1) /= &
                                real(sfvars(1)%missval,r8))) then
                                loverwrite(1)=.false.
                                loverwrite(5)=.false.
                            endif
                            if (rsfvars(2) == real(sfvars(2)%missval,r8) .and. sfdata1(2,ihr,iday1) /= &
                                real(sfvars(2)%missval,r8)) loverwrite(2)=.false. !sea level pressure
                            if (rsfvars(3) == real(sfvars(3)%missval,r8) .and. sfdata1(3,ihr,iday1) /= &
                                real(sfvars(3)%missval,r8)) loverwrite(3)=.false. !station pressure
                            if (rsfvars(8) == real(sfvars(8)%missval,r8) .and. sfdata1(8,ihr,iday1) /= &
                                real(sfvars(8)%missval,r8)) loverwrite(8)=.false. !dewpoint
                            if (rsfvars(9) == real(sfvars(9)%missval,r8) .and. sfdata1(9,ihr,iday1) /= &
                                real(sfvars(9)%missval,r8)) loverwrite(9)=.false. !RH
                        else
                            if (.not. lsfcobs(iday1))lsfcobs(iday1)=.true.
...
!                               JAT 2/26/2026 A100 (A067_GHCNN); if not overwriting value, skip
                                if (.not. loverwrite(ivar)) cycle l3
...
!   reset sfgmt2lst back to its original value
    ...
    if (allocated(rsfvars)) deallocate(rsfvars,loverwrite)
...
!*********************************************************************************************************
!   JAT 12/22/2025 A062 (A067_GHCNN); add cloud cover as a variable to check
!   there have been instances in ISHD data where a valid cloud cover was overwritten by a missing cloud 
!   cover; cloud cover will be like precip, don't overwrite a good value with missing value
!    subroutine check_dup(iday1,ihr,prcp,tmpd,wdir,wspd,miss_precip,miss_temp,miss_dir,miss_wind,writedat)
    subroutine check_dup(iday1,ihr,prcp,tmpd,wdir,wspd,cc1,cc2,sky,miss_precip,miss_temp,miss_dir,miss_wind,&
        miss_cloud,writedat)
!=========================================================================================================
...
!   CC1:            TOTAL CLOUD COVER
!   CC2:            OPAQUE CLOUD COVER
!   SKY:            ASKY
!
!   OUTPUT ARGUMENTS:
!   MISS_WIND:      DENOTES IF SPEED IS MISSING (TRUE IF CURRENT VALUE IN SFDATA1 IS NOT
...
!   MISS_PRECIP:    DENOTES IF PRECIP IS MISSING (TRUE IF CURRENT VALUE IN SFDATA1 IS NOT
!                   MISSING AND CURRENT ISHOBS IS MISSING
!   MISS_CLOUD      DENOTES IF ONE OF CLOUD COVER VARIABLES IS MISSING (TRUE IF CURRENT VALUE T
!                   IN SFDATA1 IS NO MISSING AND CURRENT ISHOBS IS MISSING
!   WRITEDAT:       DENOTES TO WRITE DATA TO SFDATA1
!
!   Variable definitions
!
...
!   miss_precip:    denotes if precip is missing (true if current value in sfdata1 is not
!                   missing and current ishobs is missing
!   miss_cloud      denotes if one of cloud cover variables is missing (true if current value to
!                   in sfdata1 is no missing and current ishobs is missing
!   writedat:       denotes to write data to sfdata1
!
!   Character variables
...
!   JAT 12/22/2025 A062 (A067_GHCNN); add integer variable icloud and misscloud
    integer(kind=4) :: i,icloud(3,2),misscloud(3)
!   JAT 12/22/2025 A062 (A067_GHCNN); add ccvr and sky as input variables
!    real(kind=r8), intent(in) :: prcp,tmpd,wdir,wspd
    real(kind=r8), intent(in) :: prcp,tmpd,wdir,wspd,cc1,cc2,sky
!   JAT 12/22/2025 A062 (A067_GHCNN); add miss_cloud as output variable
!    logical,intent(out):: miss_wind,miss_dir,miss_temp,miss_precip,writedat
    logical,intent(out):: miss_wind,miss_dir,miss_temp,miss_precip,miss_cloud(3),writedat
...
!   initialize the logical variables
    ...
    miss_temp=.false.
    miss_precip=.false.
!   JAT 12/22/2025 A062 (A067_GHCNN); set miss_cloud to false 
    miss_cloud=.false.
...
!   JAT 12/22/2025 A062 (A067_GHCNN); add logic for cloud cover; use same logic used in the clouds subroutine
!   in mod_pbl
!   assign values to icloud (1=opaque, 2=total, 3=ASOS)
!   current obs in sfdata1
    icloud(1,1)=int(sfdata1(4,ihr,iday1))/100 !total
    icloud(2,1)=int(sfdata1(4,ihr,iday1))-(int(sfdata1(4,ihr,iday1))/100)*100 !opaque
    icloud(3,1)=int(sfdata1(6,ihr,iday1)) !ASOS
    
!   obs just read in
    icloud(1,2)=int(cc1) !total
    icloud(2,2)=int(cc2) !opaque
    icloud(3,2)=int(sky) !ASOS
    
    misscloud(1)=sfvars(4)%missval/100
    misscloud(2)=sfvars(4)%missval-misscloud(1)*100
    misscloud(3)=sfvars(6)%missval
    
    l1: do i=1,3
        if (icloud(i,1) /= misscloud(i) .and. icloud(i,2) == misscloud(i)) miss_cloud(i)=.true.
    enddo l1
...
!   read in the header from the QAOUT file to get station information and dates
    ...
    do while (eof == 0 .and. .not. lbad .and. iline <=3)
        read(sf_qaout_unit,form1,iostat=eof)inpline1
        ilen=len_trim(inpline1)
        iline=iline+1
        if (eof == 0) then
    l1:     do i1=1,len_trim(inpline1)
...
!  JAT 1/6/2025 A030_1MIN; use formstr(3) for cases where AERMINUTE WBAN does not match
!   entered station WBAN
...
!   JAT 1/6/2025 A030_1MIN; if WBAN from AERMINUTE does not match the SURFACE WBAN then do not read AERMINUTE data
!    if (trim(adjustl(sf1minid)) /= trim(adjustl(sfid))) write(msg_unit,formstr(2))adjustl(pathid(ipath)),'W40',modnam,&    
    if (trim(adjustl(sf1minid)) /= trim(adjustl(sfid))) then
        write(msg_unit,formstr(2))adjustl(pathid(ipath)),'W40',modnam,&
            'AERMINUTE WBAN',trim(adjustl(sf1minid)),'DOES NOT MATCH SURFACE WBAN',trim(adjustl(sfid))
        write(msg_unit,formstr(3))adjustl(pathid(ipath)),'W56',modnam,'DO NOT PROCESS 1-MINUTE DATA'
        return
    endif

...
!   i1:         loop counter
!   istation:   upper air station indicator (1=primary, 2=secondary)
!
!   Logical variables
...
!   JAT 4/21/2025 A058 (A058_SOUND); add nstation to list of upperair variables
    !use upperair, only: upper_path,upstage1,up_test,up_stage2
    use upperair, only: upper_path,upstage1,up_test,up_stage2,nstation
    use surface, only: surf_path,sfstage1,sf_test,sf_stage2
    use onsite, only: os_path,osstage1,os_test,os_stage2!,osvars
    use pbl, only: pbl_path,pbl_test!,litems,lactions
       
    implicit none
!   JAT 4/21/2025 A058 (A058_SOUND); add istation for upper air data
!    integer(kind=4) ::  istatus,eof,i,i1 !,istage
    integer(kind=4) ::  istatus,eof,i,i1,istation
...
!               JAT 7/19/22 A007_READ_INPUT; add character string 'NO PATHS LISTED FOR' to write statement
                !write(writeunit,'(12x,a3,5x,a10,1x,a,1x,i1)')'E07',modnam,istage
                write(writeunit,'(12x,a3,5x,a10,1x,a,1x,i1)')'E07',modnam,'NO PATHS LISTED FOR',istage
            endif
            lbad1=.true.
        else
            if (lstage(1) .and. lstage(2)) then
...
!       check to see all other paths are missing, regardless of JOB pathway
    ...
    l3:     do i=2,npath
            ...
            if (lpath(2)) then
                ipath=2
                call up_test
!               JAT 4/21/2025 A058 (A058_SOUND); loop through stations to call up_stage2 and add istation as an input argument to up_stage2
                !if (.not. upstage1) call up_stage2
    s1:         do istation=1,nstation
                    if (.not. upstage1(istation)) call up_stage2(istation)

...
!   samt_var:       snow amount (JAT A029_SUBHR_SUM; added 3/10/23)
!   inso_var:       insolation
...
!   alb_var:        albedo
!   bo_var:         Bowen ratio
!   tsea_var:       sea surface temperature
!   sstd_var:       sea surface temperature depth
!   hwav_var:       significant wave height
!   twav_var:       significant wave period
!   rdow_var:       longwave downward radiation
!   formlen:        character string length of os_format1 and os_format
!
!   Real variables
...
! JAT   4/25/23 A040_GMT_INIT; initialize osgmt2lst to -99
!    integer(kind=4) :: osgmt2lst=-9
    integer(kind=4) :: osgmt2lst=-99
...
!   JAT 3/10/23 A029_SUBHR_SUM; add samt_var
    integer(kind=4) :: samt_var
!   JAT 7/5/22 A002_COARE; add new variables numbers for COARE variables
    integer(kind=4) :: tsea_var
    integer(kind=4) :: sstd_var
    integer(kind=4) :: hwav_var
    integer(kind=4) :: twav_var
    integer(kind=4) :: rdow_var
     
!   end variables set in onsite_init
    
!   JAT 7/5/22 A002_COARE; update nosvars to 47
    !integer(kind=4), parameter :: nosvars=42 !40
    integer(kind=4), parameter :: nosvars=47
...
!   JAT 7/6/22 A002_COARE; adjust noskeys for new COARE keywords
    !integer(kind=4), parameter :: noskeys=13
    integer(kind=4), parameter :: noskeys=14
! JAT 12/19/2024 MULTI_ISSUE (A70), new integer variable formlen that defines how long 
!   os_format1 and os_format can be
    integer(kind=4), parameter :: formlen=150
...
!   JAT 8/1/22 A008_SUB_HOUR; new obs_minutes array that contains the actual minute of the observation for each hour
    integer(kind=4), allocatable, dimension(:,:,:) :: obs_minutes
...
!   JAT 7/6/22 A002_COARE; add default sst_depth of 0.5 m
...
!   JAT 7/7/22 A002_COARE; add variable luse_sst_dep to denote to use SSTDEPTH instead of SSTD values if specified
...
! JAT 12/19/2024 MULTI_ISSUE (A070); increase length of os_format1 and os_format(2) to 150 using formlen
    !character(len=100), allocatable, dimension(:) :: os_format1(:)
    !character(len=100), allocatable, dimension(:) :: os_format(:)
    character(len=formlen), allocatable, dimension(:) :: os_format1(:)
    character(len=formlen), allocatable, dimension(:) :: os_format(:)
...
!   JAT 4/27/23 A002_COARE; correct min and max values for VPTG min is 0.005 max is 0.1 ( and 100)
    !osvars(18)=ovars('VPTG',.false.,.false.,.false.,.false.,.true.,.true.,0,0,-9,0,5,0,0,0.001_r8) !potential temperature lapse rate (C/m)
    osvars(18)=ovars('VPTG',.false.,.false.,.false.,.false.,.true.,.true.,0,0,-9,5,100,0,0,0.001_r8) !potential temperature lapse rate (C/m)
...
!   JAT 4/13/23; A002_COARE; make ceiling height a used variable    
!    osvars(26)=ovars('CLHT',.false.,.false.,.false.,.false.,.false.,.true.,0,0,999,0,300,0,0,1.0_r8) !ceiling height (km*10) (not used by AERMET)
    osvars(26)=ovars('CLHT',.false.,.false.,.false.,.false.,.true.,.true.,0,0,999,0,300,0,0,1.0_r8) !ceiling height (km*10) 
    osvars(27)=ovars('TSKC',.false.,.false.,.false.,.false.,.true.,.true.,0,0,99,0,10,0,0,1.0_r8) !total/opaque sky cover (tenths)
    
!   JAT A002_COARE
!   add new variables for COARE in AERMET
    osvars(28)=ovars('TSEA',.false.,.false.,.false.,.false.,.true.,.true.,0,0,99,-30,50,0,0,1.0_r8)  !sea surface temperature (C)
    osvars(29)=ovars('ZDEP',.false.,.false.,.false.,.false.,.true.,.true.,0,0,9999,-10,10,0,0,1.0_r8) !depth of sea surface temperature (m)
    osvars(30)=ovars('HWAV',.false.,.false.,.false.,.false.,.true.,.true.,0,0,9999,0,60,0,0,1.0_r8) ! significant wave height (m)
    osvars(31)=ovars('TWAV',.false.,.false.,.false.,.false.,.true.,.true.,0,0,9999,0,40,0,0,1.0_r8) ! significant wave period (seconds)
    osvars(32)=ovars('RDOW',.false.,.false.,.false.,.false.,.true.,.true.,0,0,-9999,0,1000,0,0,1.0_r8) !longwave downward radiation (W/m^2))
...
!   JAT 7/5/22 A002_COARE; update the indices for multi-level variables to reflect new single level variables
    ...
    osvars(33)=ovars('HTNN',.false.,.false.,.false.,.false.,.true.,.false.,0,0,9999,0,4000,0,0,1.0_r8) !height (m)
    osvars(34)=ovars('SANN',.false.,.false.,.false.,.false.,.true.,.false.,0,0,99,0,35,0,0,1.0_r8) !standard deviation of horizontal wind (degrees)
    osvars(35)=ovars('SENN',.false.,.false.,.false.,.false.,.false.,.false.,0,0,+99,0,25,0,0,1.0_r8) !standard deviation of vertical wind (degrees) (not used by AERMET)
    osvars(36)=ovars('SVNN',.false.,.false.,.false.,.false.,.false.,.false.,0,0,99,0,3,0,0,1.0_r8) !standard deviation of v-component of the wind (m/s) (not used by AERMET)
    osvars(37)=ovars('SWNN',.false.,.false.,.false.,.false.,.true.,.false.,0,0,99,0,3,0,0,1.0_r8) !standard deviation of w-component of the wind (m/s)
    osvars(38)=ovars('SUNN',.false.,.false.,.false.,.false.,.false.,.false.,0,0,+99,0,3,0,0,1.0_r8) !standard deviation of u-component of the wind (m/s) (not used by AERMET)
    osvars(39)=ovars('TTNN',.false.,.true.,.false.,.false.,.true.,.false.,0,0,99,-30,40,0,0,1.0_r8) !temperature (degrees C)
    osvars(40)=ovars('WDNN',.false.,.true.,.false.,.false.,.true.,.true.,0,0,999,0,360,0,0,1.0_r8) !wind direction (degrees from North)
    osvars(41)=ovars('WSNN',.false.,.true.,.false.,.false.,.true.,.true.,0,0,99,0,50,0,0,1.0_r8) !wind speed (m/s)
    osvars(42)=ovars('VVNN',.false.,.false.,.false.,.false.,.false.,.false.,0,0,999,0,5,0,0,1.0_r8) !vertical wind component (m/s) (not used by AERMET)
    osvars(43)=ovars('DPNN',.false.,.false.,.false.,.false.,.true.,.false.,0,0,99,-65,35,0,0,1.0_r8) !dew point temperature (degrees C) 
    osvars(44)=ovars('RHNN',.false.,.false.,.false.,.false.,.true.,.true.,0,0,999,0,100,0,0,1.0_r8) !relative humidity (whole percent) 
    osvars(45)=ovars('V1NN',.false.,.false.,.false.,.false.,.false.,.false.,0,0,999,0,100,0,0,1.0_r8) !user's vector #1 (user's units) (not used by AERMET)
    osvars(46)=ovars('V2NN',.false.,.false.,.false.,.false.,.false.,.false.,0,0,999,0,100,0,0,1.0_r8) !user's vector #2 (user's units) (not used by AERMET)
!JAT 12/16/2024 MULTI_ISSUE (A063); correct name for osvars(47) from V2NN to V3NN
!    osvars(47)=ovars('V2NN',.false.,.false.,.false.,.false.,.false.,.false.,0,0,999,0,100,0,0,1.0_r8) !user's vector #3 (user's units) (not used by AERMET)
    osvars(47)=ovars('V3NN',.false.,.false.,.false.,.false.,.false.,.false.,0,0,999,0,100,0,0,1.0_r8) !user's vector #3 (user's units) (not used by AERMET)    
...
!       JAT 3/10/23 A029_SUBHR_SUM; add samt_var;
        if (trim(adjustl(osvars(ivar)%varname)) == 'SAMT') samt_var=ivar !PAMT
...
!       JAT 7/5/22 A002_COARE initialize the COARE variables
        if (trim(adjustl(osvars(ivar)%varname)) == 'TSEA') tsea_var=ivar !sea surface temperature
        if (trim(adjustl(osvars(ivar)%varname)) == 'ZDEP') sstd_var=ivar !sea surface temperature depth
        if (trim(adjustl(osvars(ivar)%varname)) == 'HWAV') hwav_var=ivar !significant wave height
        if (trim(adjustl(osvars(ivar)%varname)) == 'TWAV') twav_var=ivar !significant wave period
        if (trim(adjustl(osvars(ivar)%varname)) == 'RDOW') rdow_var=ivar !longwave downward radiation
...
!       JAT A002_COARE; add SSTDEPTH to keyword logic
     !     if ((ikey >=4 .and. ikey <= 6) .or. (ikey >= 8 .and. ikey <= 10) .or. ikey == 27 .or. (ikey >= 12 .and. ikey <= 17) .or. &
              !ikey == 37) then
            ...
            elseif (ikey==27) then
                jj=ikey-14
!           elseif (ikey==37) then
!!              JAT 7/6/22 A002_COARE; add logic for SSTDEPTH
!               jj=ikey-23
...
!               JAT 7/6/22 A002_COARE; add SSTDEPTH to this list                
                !if (ikey == 4 .or. (ikey >=8 .and. ikey <= 10) .or. ikey == 16 .or. ikey == 27 .or. ikey == 37 ) then
...
!                    read(inpline1(formind(1):formind(2)),'(a)')landflag
                    read(inpline1(formind(1):formind(2)),*)landflag
...
!       JAT A002_COARE; add logic for SSTDEPTH
!       else if (ikey == 37) then !SSTDEPTH
!!           incorrect number of fields                  
!            if (nfield /=2) then
!                write(writeunit,formstr(2))adjustl(pathid(ipath)),'E03',modnam,nfield,&
!                    'IS AN INVALID NUMBER OF FIELDS FOR KEYWORD',trim(adjustl(keywrd(ikey)))
...
!!
!!   Character variables
!!   datafield:      data for SSTDEOTG keyword read from input line
!!   formstr:       formats for messages
!!   modnam:         Subroutine name
!!========================================================================================================= 
...
!    write(formstr(2),'(2(a))')trim(adjustl(msg_form)),'a,1x,f9.5,1x,a,1x,f9.5,1x,a)'
!
!    allocate(datafield(nfield1))
!      
!    datafield='0000000000000000000000000'
...
!           if (depth >=  real(osvars(sstd_var)%missval,r8)) then
!!               equal to or exceeds missing value; do not reset sst_depth
!               write(writeunit,formstr(2))adjustl(pathid(ipath)),'W65',modnam,&
!                    trim(adjustl(keywrd(ikey))),sst_depth,' M EXCEEDS MISSING INDICATOR',real(osvars(sstd_var)%missval,r8),'M; DO NOT USE'
!           else
!                write(writeunit,formstr(2))adjustl(pathid(ipath)),'W65',modnam,&
...
!========================================================================================================= 
    ...
    real(kind=r8) :: ht
!   JAT 3/24/23 A021_HT_CHECK; make formstr 2-element array
    !character(len=60) :: formstr
...
!   2. heights not in ascending order
    write(formstr(2),'(2(a))')trim(adjustl(msg_form)),'a,1x,f10.4,1x,a,1x,i2,1x,a,1x,f10.4,1x,a,1x,i2)'
...
!           JAT 3/24/23 A021_HT_CHECK; add check on os_heights to make sure in ascending order
            if (noshts > 1) then
                if (osheights1(noshts) <= osheights1(noshts-1)) then
                    lbad=.true.
                    write(msg_unit,formstr(2))adjustl(pathid(ipath)),'E62',modnam,'OSHEIGHTS HEIGHT',osheights1(noshts),&
                        'M LEVEL',noshts,'IS <= OSHEIGHTS HEIGHT',osheights1(noshts-1),'M LEVEL',noshts-1
                endif
            endif
        else
!           JAT 3/24/23 A021_HT_CHECK; formstr is now formstr(1)
            !write(writeunit,formstr)adjustl(pathid(ipath)),'E05',modnam,'INVALID HEIGHT LISTED FOR OSHEIGHTS:',&
            !    trim(adjustl(datafield(i)))
            write(writeunit,formstr(1))adjustl(pathid(ipath)),'E05',modnam,'INVALID HEIGHT LISTED FOR OSHEIGHTS:',&
                trim(adjustl(datafield(i)))
            lbad=.true.
        endif
    enddo l1
...
!               set lread for variable to true
                osvars(i)%lread=.true.
!               JAT 7/28/22 A002_COARE; set read_sst_depth 
                if (i == sstd_var) read_sst_depth=osvars(i)%lread
...
!                   JAT 12/6/2022 A028_ONSITE_ORDER; check to also see if ilev is less than firstlev
!                   this will account for variables not being read in ascending order, i.e. WS01 may be read after WS02
!                    if (osvars(i)%firstlev == 0) osvars(i)%firstlev=ilev !set first level being read for variable
                    if (osvars(i)%firstlev == 0 .or. ilev < osvars(i)%firstlev) osvars(i)%firstlev=ilev !set first level being read for variable
                endif
            else
...
!
!   Logical variables
!   l_long:         FORMAT string is more than allotted length of os_format1
!   Character variables
!   formstr:        formats for messages
...
! JAT 12/19/2024 MULTI_ISSUE (A070); add logical variable denoting if format statement is too long
    logical :: l_long
    character(len=12) :: keywrd1
    character(len=2) :: aread
! JAT 12/19/2024 MULTI_ISSUE (A070); increase formstr from 5 to 6
!    character(len=60) :: formstr(5)
...
! JAT 12/19/2024 MULTI_ISSUE (A070); initialize l_long to false.
    l_long=.false.
...
!   JAT 12/19/2024 MULTI_ISSUE (A070); new error message if FORMAT exceeds 150 characters
!   6. FORMAT character string exceeds limit (formlen)  
    write(formstr(6),'(2(a))')trim(adjustl(msg_form)),'a,1x,i2,1x,a,1x,i3)'
...
! JAT 12/19/2024 MULTI_ISSUE (A070) set os_format1 to exclude leading blanks
! also check the length of the string that represents the format statement, if more than the allocated value for
! osformat, issue an error
        if (iread < 10) then
            !os_format1(iread)=inpline1(iform+2:ilen)
            os_format1(iread)=trim(adjustl(inpline1(iform+2:ilen)))
            if ((ilen-iform-1) > formlen) l_long=.true.
        else
            !os_format1(iread)=inpline1(iform+3:ilen)
            os_format1(iread)=trim(adjustl(inpline1(iform+3:ilen)))
            if ((ilen-iform-2) > formlen) l_long=.true.
        endif
! JAT 12/19/2024 MULTI_ISSUE (A070) issue error message if l_long is true
        if (l_long) then
            write(writeunit,formstr(6))adjustl(pathid(ipath)),'E63',modnam,'FORMAT INDEX',iread,&
                'EXCEEDS CHARACTER STRING LENGTH',formlen
            lbad=.true.
        endif
    endif
...
!   ol_ow_msg:      denotes to write the message that data is overland but COARE overwater variable
!                   present
!
!
!   Character variables
...
!   JAT 7/5/22 A002_COARE; add ol_ow_msg to indicate whether to write the W60 message that data is overland
!   but COARE overwater variable present
!   logical :: lbad2
    logical :: lbad2,ol_ow_msg
...
!       JAT 7/5/22 A002_COARE; data is overland but overwater variables present
...
!   JAT 7/5/22 A002_COARE; ONSITE and OW flag are no longer invalid
!   can have OW flag with ONSITE data if processing buoy data for COARE
!   just comment this logic out
...
!   JAT 7/5/22 A002_COARE; add message that overland data is being read indicated but overwater variables sea
!   surface temperature, depth, and wave variables are read in.
!   only write the message for the first occurrence of one of the tested variables
!   also add to variables listed as not being used in stage 2
    l4: do ivar=1,nosvars
            if ((ivar == tsea_var .or. ivar == sstd_var .or. ivar == hwav_var .or. ivar == twav_var .or. ivar == rdow_var) .and. &
                osvars(ivar)%lread .and. .not. ol_ow_msg) then
                ol_ow_msg=.true.
!               JAT 3/12/24 A043_COARE; change message to say DATA indicates instead of DATA flag
                !write(msg_unit,formstr(2))adjustl(pathid(ipath)),'W60',modnam,&
                !    'LOCATION IS OVERLAND BUT DATA FLAG INDICATES OVERWATER'
                 write(msg_unit,formstr(2))adjustl(pathid(ipath)),'W60',modnam,&
                    'LOCATION IS OVERLAND BUT DATA INDICATES OVERWATER'
            endif
            ...
            if ((ivar == hflux_var .or. ivar == lflux_var .or. ivar == zo_var .or. ivar == vptg_var .or. ivar == l_var .or. &
                ivar == wstar_var .or. ivar == alb_var .or. ivar == bo_var .or. ivar == tsea_var .or. ivar == sstd_var .or. &
                ivar == hwav_var .or. ivar == twav_var .or. ivar == rdow_var) .and. osvars(ivar)%lread) &
...
!   JAT 7/7/22 A002_COARE; if SSTDEPTH included check to see if SSTD variable read in
!   if so, then issue warning that SSTD variables ignored and SSTDEPTH used
    
 !   if (osstage1 .and. valid_depth) then
    !   if (osvars(sstd_var)%lread) write(msg_unit,formstr(2))adjustl(pathid(ipath)),'W67',modnam,&
    !       'SSTDEPTH AND SSTD BOTH SPECIFIED. SSTDEPTH KEYWORD WILL BE USED AND SSTD VARIABLE IGNORED'
    !   luse_sst_dep=.true.
    !endif
...
!   JAT 7/19/22 A006_LINUX; correct if statement to use .neqv. instead of /=
...
!   JAT 7/19/22 A002_COARE
!   if reading in sea surface temperature depth, should have sea surface temperature, if not issue error
!   but can have temperature and no depth
    
    if (osvars(sstd_var)%lread .and. .not.  osvars(tsea_var)%lread) then
        write(msg_unit,formstr(2))adjustl(pathid(ipath)),'E61',modnam,'NO SEA SURFACE TEMPS WITH SEA SURFACE TEMP DEPTH'
        lbad=.true.
    endif   
...
!   JAT 7/7/22 A002_COARE; increase size of formstr for new messages related to COARE
    !character(len=60) :: formstr(4)
    character(len=60) :: formstr(5)
...
!   JAT 7/7/22 A002_COARE; alert user that negative SST depths will be converted to positive
!   5. negative depths set to positive
...
!   JAT 7/5/22 A002_COARE; as part of this fix, do not audit date/time variables
        if (ivar <=5) cycle vv
...
!           JAT 7/7/22 A002_COARE; reset read_sst_depth based on the osvars for SSTD
!           also alert user that negative depths will be reset to positive
            !if (ivar == sstd_var) read_sst_depth=osvars(ivar)%lread
            !   read_sst_depth=osvars(ivar)%lread
            !   if (osvars(ivar)%lread) write(msg_unit,formstr(5))adjustl(pathid(ipath)),'I68',modnam,&
            !       'NEGATIVE VALUES OF SSTD WILL BE RESET TO POSITIVE VALUES'
            !endif
    enddo l1
...
!       JAT 8/1/22 A008_SUB_HOUR; allocate new obs_minutes array
        allocate(obs_minutes(nobs_hr,24,nosdays))
        obs_minutes=0
...
!   now check that heights are in correct order 
    if (.not. lbados) then
!       JAT 3/24/23 A021_HT_CHECK; only call check_hts when OSHEIGHTS not used (luseohts=.false.)
!        if (luseoshts .or. osvars(height_var)%lread) call check_hts
        if (.not. luseoshts .and. osvars(height_var)%lread) call check_hts
    endif
...
!                       JAT 3/10/23 A029_SUBHR_SUM; add samt_var
                        if (ivar == samt_var) osvars(samt_var)%readvar=j
...
!                       JAT 7/5/22 A002_COARE, add COARE variables
                        if (ivar == tsea_var) osvars(tsea_var)%readvar=j
                        if (ivar == sstd_var) osvars(sstd_var)%readvar=j
                        if (ivar == hwav_var) osvars(hwav_var)%readvar=j
                        if (ivar == twav_var) osvars(twav_var)%readvar=j
                        if (ivar == rdow_var) osvars(rdow_var)%readvar=j
...
!               JAT 3/10/23 A029_SUBHR_SUM; add samt_var
                if (ivar == samt_var) osvars(samt_var)%readvar=j
!               JAT 7/5/22 A002_COARE; add COARE variables
                if (osvars(tsea_var)%lread) osvars(tsea_var)%readvar=osvars(tsea_var)%readvar+1
                if (osvars(sstd_var)%lread) osvars(sstd_var)%readvar=osvars(sstd_var)%readvar+1
                if (osvars(hwav_var)%lread) osvars(hwav_var)%readvar=osvars(hwav_var)%readvar+1
                if (osvars(twav_var)%lread) osvars(twav_var)%readvar=osvars(twav_var)%readvar+1
                if (osvars(rdow_var)%lread) osvars(rdow_var)%readvar=osvars(rdow_var)%readvar+1
...
!                   JAT 3/24/23 A021_HT_CHECK; if first or last level for height is not 1 or nlevel then
!                   reset to first or last level
                    if (ilev == 1 .and. osvars(height_var)%firstlev /= 1) osvars(height_var)%firstlev=1
                    if (ilev == nlevel .and. osvars(height_var)%nlevels /= nlevel) osvars(height_var)%nlevels=nlevel
                enddo ll1
...
!!               JAT 7/6/22 A002_COARE; reset the SSTD to SST_DEPTH if specified
!               if (read_index(ivar1) == sstd_var .and. luse_sst_dep) osdata1a(ivar1,:,:,:)=sst_depth
...
!   JAT A087 (A087_RH) 1/22/2025; reset osvars(height_var)%readvar to 1 because
!   height is always first variable in osdata1; however, usually height in osdata1 is usually 
!   just referred by osdata1(1,ilev,h,d) not osdata1(osvars(height_var)%readvar,ilev,h,d)
!   but changing here to make consistent with other variables and in case in future updates
!   coder forgets that height is always variable 1
    if (osvars(height_var)%readvar > 0 .or. luseoshts) osvars(height_var)%readvar=1
...
!   JAT 8/16/22 A008_SUB_HOUR deallocate obs_minutes
    if (allocated(obs_minutes)) deallocate(obs_minutes)
...
!   JAT 7/6/22 A002_COARE; SSTDEPTH
!   if (loskeywords(14)) write(os_qaout_unit,'(a,1x,f6.3)')'SSTDEPTH',sst_depth
...
!       JAT 4/13/23 A021_HT_CHECK; write heights if luseoshts used but heights not read in  
!        if (osvars(ivar)%lread .and. osvars(ivar)%luse) then
        if ((osvars(ivar)%lread .and. osvars(ivar)%luse) .or. (ivar == height_var .and. luseoshts)) then
...
!========================================================================================================= 
    ...
    irec1,savemin,imin2,iflag,ilev,iday1,iyear,imonth,iday,ihr,savehr,ivar,ivar1,imin
...
!   JAT 07/26/22 A008_SUB_HOUR; increase formstr from 8 to 9 for new message regarding number of
!   obs/hr less than OBS/HR value from control file
...
!   JAT 07/26/22 A008_SUB_HOUR; add new formstr for when number of obs/hour < NOBS/HR
!   9.  number of obs/hour in data file < nobs/hr in control file
    write(formstr(9),'(2(a))')trim(adjustl(msg_form)),'2(a,1x,i3,1x),a20)'
...
!   initialize these variables
    ...
    ivar1=0
    imin=0
!   JAT 8/1/22 A008_SUB_HOUR; imin1 no longer used
!   imin1=0
    ilev=0
    svar=0
...
!                   JAT 07/26/22 A008_SUB_HOUR; move the assignment of date_hr_str to below the check
!                   for savedate and sdate and ihr and savehr because it will also be used in the logic
                    !write(date_hr_str,'(a5,1x,i8,1x,a2,1x,i2.2)')'DATE:',sdate,'HR',ihr
                    
                    if (osvars(5)%lread .and. (savedate /= sdate .or. ihr /= savehr)) then
!                       JAT 07/26/22 A008_SUB_HOUR; have reached a new hour; check to make previous hour's number of obs/hour read
!                       in data file is not less than number of obs/hour specified in control file
!                       JAT 07/28/22 A008_SUB_HOUR; make message a warning, not an error
                        if (imin2 < nobs_hr .and. imin2 > 0) then
                            write(date_hr_str,'(a5,1x,i8,1x,a2,1x,i2.2)')'DATE:',savedate,'HR',savehr
                            !write(msg_unit,formstr(6))adjustl(pathid(ipath)),'E56',modnam,'NUMBER OF OBS/HR',imin2,&
!   JAT 1/6/2025 A030_1MIN; as part of A030_1MIN change W56 to W57. W56 will now alert user that 1-minute data
!   will not be read
                            !write(msg_unit,formstr(9))adjustl(pathid(ipath)),'W56',modnam,'NUMBER OF OBS/HR',imin2,&
                            write(msg_unit,formstr(9))adjustl(pathid(ipath)),'W57',modnam,'NUMBER OF OBS/HR',imin2,&
                                'IS LESS THAN INPUT OBS/HR',nobs_hr,date_hr_str
                            !lbad=.true.
                        endif
                        imin2=0 !reset minute counter
                    endif
!                   JAT 07/26/22 A008_SUB_HOUR; new location for assignment of date_hr_str                    
                    write(date_hr_str,'(a5,1x,i8,1x,a2,1x,i2.2)')'DATE:',sdate,'HR',ihr
...
!                       minute reading, skip.
!                       JAT 8/1/22 A008_SUB_HOUR; no longer check to see if the observation fits a standard
!                       # of minutes.  For example, if nobs_hr=3, no longer check to see if the observations are
!                       20 minutes apart and at 20, 40, and 60 minutes.
...
!                       JAT 8/1/22 A008_SUB_HOUR; now use imin2 instead imin1 for writing this message
                        !if (.not. noprint .and. (.not. osvars(5)%lread .or. (osvars(5)%lread .and. imin1 == 1))) &
                        if (.not. noprint .and. (.not. osvars(5)%lread .or. (osvars(5)%lread .and. imin2 == 1))) &
...
!                   JAT 8/1/22 A008_SUB_HOUR; assign imin to obs_minutes array
!                   JAT 1/3/22 A008_SUB_HOUR; only write to array when reading sub-hourly data
                    if (osvars(5)%lread) obs_minutes(imin2,ihr,iday1)=imin
...
!========================================================================================================= 
    ...
    implicit none
!   JAT 8/1/22 A008_SUB_HOUR; make h_minus_1 and orig_date allocatable and have a unique h_minus_1 and orig_date for each minute of the hour
!   this to account for minute 00 is the hour not hour minus 1
    !integer(kind=4) :: nobs_hr1,icalms,dircount,h_minus_1,ivar,ivar1,d,h,imin1,imin,ilev,iswitch,iv
    integer(kind=4) :: nobs_hr1,icalms,dircount,ivar,ivar1,d,h,imin1,imin,ilev,iswitch,iv
    integer(kind=4), allocatable, dimension(:) :: h_minus_1
    !integer(kind=8) :: orig_date
    integer(kind=8), allocatable, dimension(:)  :: orig_date(:)
...
!   JAT 8/1/22 A008_SUB_HOUR; allocate h_minus_1 and orig_date
    allocate(h_minus_1(nobs_hr))
    allocate(orig_date(nobs_hr))
!   JAT 07/13/22 A005_SAVEHR_UNINIT
!   initialize these variables
    nobs_hr1=0
    dircount=0
...
!   9.  data
!   JAT 8/2/22 A008_SUB_HOUR; modify format for wind direction to account for larger missing values
    !write(debugform(9),'(a)')'(t6,i8,1x,i2.2,t25,i2.2,t32,f7.3,a1)'
    write(debugform(9),'(a)')'(t6,i8,1x,i2.2,t25,i2.2,t28,f11.3,a1)'
...
!               JAT A008_SUB_HOUR; assign values to h_minus_1 and orig_date             
                !h_minus_1=h-1
                ...
                h_minus_1=0
                orig_date=0
    mm1:        do imin1=1,nobs_hr
                    if (imin1 == nobs_hr .and. imin == 0) then
                        h_minus_1(imin1)=h
                        orig_date(imin1)=os_info(d)%osdate
                    else
                        h_minus_1(imin1)=h-1
                        if (h_minus_1(imin1)==0 .and. d > 1) then
                            h_minus_1(imin1)=24
                            orig_date(imin1)=os_info(d-1)%osdate
                        else
                            orig_date(imin1)=os_info(d)%osdate
                        endif
                    endif
...
!                       JAT 8/1/22 A008_SUB_HOUR; set imin to obs_minutes; use imin as an argument instead of changing 
!                       audit since audit is also used for hourly data
                        !imin=(60/nobs_hr)*imin1
                        imin=obs_minutes(imin1,h,d)
...
!                       JAT 8/1/22 A008_SUB_HOUR; orig_date and h_minus_1 now part of array
                        !if (debug)write(debug_unit,debugform(4))orig_date,h_minus_1,imin,os_subhr_vals(imin1,ivar1,ilev,h,d),hrval
                        if (debug)write(debug_unit,debugform(4))orig_date(imin1),h_minus_1(imin1),imin,&
                            os_subhr_vals(imin1,ivar1,ilev,h,d),hrval
...
!                   number of obs.  other variables set to missing
                    if (nobs_hr1 < min_obs) then !not enough values; write message
                        if (.not. writelev) then
!                           JAT 3/10/23 A029_SUBHR_SUM; correct references to PRCP, SAMT, PAMT
                            !if (ivar == 10 .or. ivar == 11 .or. ivar == 21 .and. nobs_hr1 > 0) then !keep total but issue message about # of nobs
                            if (ivar == prcp_vars(1) .or. ivar == prcp_vars(2) .or. ivar == samt_var .and. nobs_hr1 > 0) then !keep total but issue message about # of nobs 
...
!                       JAT 3/10/23 A029_SUBHR_SUM; do not average precip and snow
                        if (ivar == prcp_vars(1) .or. ivar == prcp_vars(2) .or. ivar == samt_var) then
                            osdata1a(ivar1,ilev,h,d)=hrval
                        else
...
!                           JAT 8/1/22 A008_SUB_HOUR; set imin to obs_minutes;
                            !imin=(60/nobs_hr)*imin1
                            imin=obs_minutes(imin1,h,d)
                            iv=findvar(ivar)
...
!                                athresh,hrval
                            if (debug)write(debug_unit,debugform(8))orig_date(imin1),h_minus_1(imin1),imin,&
                                os_subhr_vals(imin1,ivar1,ilev,h,d),athresh,hrval
...
!                       JAT 7/11/22 A004_MISSING_WIND_SPEED; add a check against the missing data indicator in
!                       the logic below in case the missing indicator is negative. if the additional check isn't 
!                       done, AERMET will interpret the missing value as a calm and set the hourly average wind
!                       direction to 0 (calm)
                        !if (osdata1a(ivar1,ilev,h,d) < threshspeed) lcalm_obs_hr(ilev,h,d)=.true.
                        if (osdata1a(ivar1,ilev,h,d) < threshspeed .and. &
                            dabs(osdata1a(ivar1,ilev,h,d)-real(osvars(ivar)%missval,r8)) > eps) lcalm_obs_hr(ilev,h,d)=.true.
                    endif
                endif
!*************  WIND SPEED ****************************************** 
!*************  WIND DIRECTION ****************************************** 
...
!                               JAT 8/1/22 A008_SUB_HOUR; orig_date and h_minus_1 now part of array
                                ...
                                if (debug)write(debug_unit,debugform(9))orig_date(imin1),h_minus_1(imin1),imin,&
...
!   JAT 8/1/22 A008_SUB_HOUR; deallocate h_minus_1 and orig_date
    if (allocated(h_minus_1)) deallocate(h_minus_1)
    if (allocated(orig_date)) deallocate(orig_date)
    return
...
!========================================================================================================= 
    use, intrinsic :: iso_fortran_env,only : output_unit 
!   JAT 3/14/23 A031_OS_THRESH; add nkeys to main1 list
!   use main1, only : linelen,inpline1,ikey,ilen,keywrd,getdates,getloc,lpath
    use main1, only : linelen,inpline1,ikey,ilen,keywrd,getdates,getloc,nkeys
    use file_units, only: os_qaout_unit
    implicit none
...
!   JAT 7/6/22 A002_COARE; change number of formstr from 6 to 5 because
!   overwater flag is now valid for ONSITE data
    !character(len=60) :: formstr(6)
    character(len=60) :: formstr(5)
...
!   JAT 7/6/22 A002_COARE; this format no longer needed because the overwater flag is 
!   now valid for ONSITE data
...
!   initialize these variables
    ...
    l1:     do i1=1,len_trim(inpline1)
            ...
            enddo l1
!   JAT 3/14/23 A031_OS_THRESH; add check for ikey
            ikey=1
            lfound=.false.
            do while (ikey <= nkeys .and. .not. lfound)
                if (index(inpline1,trim(adjustl(keywrd(ikey)))) > 0) then
                    lfound=.true.
                else
                    ikey=ikey+1
                endif
...
!!          JAT 7/6/22 A002_COARE; check SSTDEPTH
!            if (index(inpline1,'SSTDEPTH') > 0) then
!                nfield=0
!!               get the number of fields 
!    l6:         do i=1,len_trim(inpline1)
!                    if (ichar(inpline1(i+1:i+1)) == 32 .and. ichar(inpline1(i:i)) /= 32) nfield=nfield+1
!                enddo l6
!                if (nfield /=2) then
!                    write(writeunit,formstr(1))adjustl(pathid(ipath)),'E03',modnam,nfield,&
...
!                    lbad=.true.
!                else
!                    i=index(inpline1,'SSTDEPTH')      
!                    call sstdepth(i+10)
!                endif
!           endif   
            
!           JAT 7/6/22 A002_COARE; overwater flag now valid for ONSITE data
            if (index(inpline1,'OVERWATER') > 0) overland=.false. 
...
!========================================================================================================= 
    ...
    use main1, only: eps
    implicit none
!   JAT 3/24/23 A021_HT_CHECK; add variable ivar
    !integer(kind=4) :: htvar,d,h,ilev,imin,imin1
    integer(kind=4) :: htvar,d,h,ilev,imin,imin1,ivar
    character(len=12) :: timestr
!   JAT 3/24/23 A021_HT_CHECK; increase formstr from 2 to 3 
!    character(len=80) :: formstr(2)
    character(len=80) :: formstr(3)
    character(len=10) :: modnam='CHECK_HTS'
!   JAT 3/24/23 A021_HT_CHECK; add variable lmatch
    logical :: lmatch
...
!   JAT 3/24/23 A021_HT_CHECK; message format for mismatch in number of height levels and other variables
    write(formstr(3),'(2(a))')trim(adjustl(msg_form)),'a,1x,i3,1x,a)'
...
!   JAT 3/24/23 A021_HT_CHECK; comments above only now applicable for cases not using OSHEIGHTS
    htvar=osvars(height_var)%readvar
    
!   JAT 3/24/23 A021_HT_CHECK; check to make sure the first level of the height variable is 1 and
!   the last level is equal to the last level for at least one multi-level variable
!   
    ivar=height_var+1
    lmatch=.true.
    if (osvars(height_var)%firstlev /= 1) then
!      JAT 6/1/2023 A041; correct gfortran error of trunacated line
        write(msg_unit,formstr(3))adjustl(pathid(ipath)),'E58',modnam,'FIRST HT LEVEL',osvars(height_var)%firstlev,&
            'IS NOT EQUAL TO 1'
        lbad=.true.
    else
        do while (ivar <= nosvars .and. lmatch)
            if (osvars(height_var)%nlevels < osvars(ivar)%nlevels) lmatch=.false.
                ivar=ivar+1
        enddo
        if (.not. lmatch) then
            write(msg_unit,formstr(3))adjustl(pathid(ipath)),'E58',modnam,'LAST HT LEVEL',osvars(height_var)%nlevels,&
                'IS LESS THAN ONE OF THE OTHER MULTI-LEVEL VARIABLES'
            lbad=.true.
        endif
    endif

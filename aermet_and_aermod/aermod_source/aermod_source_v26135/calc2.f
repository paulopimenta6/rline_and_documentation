...
C        MODIFICATIONS:
C
C        MODIFIED:    Allowed RLINE/RLINEXT sources to use the RL_GRIDWS
C                     Wood, 7/5/22
C
...
C........Added IF Statments for RLINE and RLINEXT source to use RL_GRIDWS - WOOD 6-28-2022
C         CALL GINTRP ( GRIDHT(NDX4HC), GRIDWS(NDX4HC),
C     &                  GRIDHT(NDX4HC+1), GRIDWS(NDX4HC+1),
C     &                  HHILL, UATHH )
...
C        determined later in the routine
         ...
         DO NL = 1, NLEV - 1
C - Wood 7/5/2022       LS(NL) = 0.5D0 * GRIDWS(NL) * GRIDWS(NL)

C begin - Use RLINE windspeeds if RLINE sourcetype - Wood 7/5/2022
           IF ((SRCTYP(ISRC) .EQ. 'RLINE') .OR. 
     &           (SRCTYP(ISRC) .EQ. 'RLINEXT')) THEN
     ...
     &                RL_GRIDWS(NL,I_ALPHA)
           ...
           END IF
C end -  Use RLINE windspeeds if RLINE sourcetype  - Wood 7/5/2022                                                                    
         END DO
...
C
         IF( IT .GT. 1 )THEN
            ...
            IF( IT .EQ. NLEV )THEN
               ...
               HTOP  = HHILL
            ELSE
C Wood 7/5/2022 WSTOP = GRIDWS(IT)
C Wood 7/5/2022 HTOP  = GRIDHT(IT)

C begin - Use RLINE wind speed if RLINE or RLINEXT source
               IF ((SRCTYP(ISRC) .EQ. 'RLINE') .OR. 
     &           (SRCTYP(ISRC) .EQ. 'RLINEXT')) THEN
                ...
                ELSE 
                    ...
                    HTOP  = GRIDHT(IT)
                END IF
C end - Use RLINE wind speed if RLINE or RLINEXT source                     
            END IF

            IB = IT - 1
            HBOT = GRIDHT(IB)
C Wood 7/5/2022 DWS = (WSTOP - GRIDWS(IB)) / (HTOP - HBOT)
...
C                 Close MAXIFILE and Reposition to End
     ...
     &                 POSITION='APPEND')
C - CRT 4/10/2026 D237 - Corrections to D158 - Do not need to store filename/unit for maxifile when restart is true
C - WSP 9/16/2024 D158 Dynamic File unit  Store FUNIT, FILENAME, and FILETYPE in array to add to output as table  - Starting       
CCRT                  FUNITARRAY(FUNITIND) = IMXUNT(IGRP,IAVE)
CCRT                  FNAMEARRAY(FUNITIND) = THRFIL(IGRP,IAVE)
CCRT                  FTYPARRAY(FUNITIND) = 'IMUNT'
CCRT                  FUNITIND = FUNITIND + 1
...
C - CRT 4/10/2026 D237 - Corrections to D158 - Do not need to store filename/unit for postfile when restart is true
C - WSP 9/16/2024 D158 Dynamic File unit  Store FUNIT, FILENAME, and FILETYPE in array to add to output as table  - Starting       
CCRT                  FUNITARRAY(FUNITIND) = IPSUNT(IGRP,IAVE)
CCRT                  FNAMEARRAY(FUNITIND) = PSTFIL(IGRP,IAVE)
CCRT                  FTYPARRAY(FUNITIND) = 'POSTFILE'
CCRT                  FUNITIND = FUNITIND + 1
C - WSP 9/16/2024 D158 Dynamic File unit  Store FUNIT, FILENAME, and FILETYPE in array to add to output as table  - Ending      
...
C               IF (ICYR .GE. ISTRT_WIND .AND. ICYR .LE. 99) THEN
C                  ICYR  = ISTRT_CENT*100 + ICYR
C                  ICDAT = ISTRT_CENT*100000000 + ICDAT8
C               ELSE IF (ICYR .LT. ISTRT_WIND) THEN
C                  ICYR  = (ISTRT_CENT+1)*100 + ICYR
C                  ICDAT = (ISTRT_CENT+1)*100000000 + ICDAT8
C               END IF

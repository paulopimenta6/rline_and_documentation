...
C     INOXLINE for NOX_FILE
      ILINE  = 0
CWSP      IQLINE = 0  ! CWSP 1/24/2024 D175 HOUREMIS Filename D121 Hourly Emissions File
      IQLINE(:) = 0  ! CWSP 1/24/2024 D175 HOUREMIS Filename D121 Hourly Emissions File
      IOLINE = 0
      IBLINE = 0
      INOXLINE = 0
...
C --- Initialize counters for arrays to store met data for MAXDCONT option
      ...
      IBARID  = 0 !Barrier ID counter for read in SBARRIER WSP RLM 2024 06 05
...
C     Variable Initializations
      MODNAM = 'GETFLD'
      FIELD(:) = ''
      FIELD1(:) = '' ! D144 Debug Options Limit - preserve filename
...
CCRT 3/6/2026, D144 Debug Options Limit - preserve filename case 
            FIELD1(I) = trim(RUNST1(LOCB(I):LOCE(I)))
         ELSE
...
C           Truncate Field at ILEN_FLD Characters
     ...
     &                                         LOCB(I)+ILEN_FLD-1)
CCRT 3/6/2026, D144 Debug Options Limit - preserve filename case 
            FIELD1(I) =  (RUNST1(LOCB(I):LOCB(I)+ILEN_FLD-1))
         END IF
      END DO
...
C        DATE:    December 15, 1993
C
C        MODIFIED:  To add the Aircraft related parameters (Aircraft Plume Rise)
...
CCRT  D063 Platform Downwash Debug
      PLATFMDBG  = .FALSE.  
      BARDBG  = .FALSE. !added for SBARRIER input debug 20240716 WSP
      EDGEDBG = .FALSE. !added for BAREDGE debug 20240910 WSP
      VBRDBG = .FALSE. !added for VBARRIER debug 20250806 WSP
      WAKE   = .FALSE.
      ECHO   = .TRUE.
      SCREEN = .FALSE.
      HOURLY = .FALSE.
      ...
      PollCrit = .FALSE.            ! CRT, 2/11/2026, D138 Scientific Notation, If true, criteria pollutant
...
C     8/9/2023, CRT - D176 COARE BETA Check (L_COARE)
      L_AdjUstar     = .FALSE.
      L_BULKRN       = .FALSE.
      L_MMIF_Data    = .FALSE.
...
C**  Added for Airport ID for Aircraft Sources; UNC-IE
...
C     Initialize End Date as largest valid date for a 4-byte integer.
      ...
      O3CONC = -999.0D0
      NO2Equil = 0.90D0
C      NO2Stack = 0.10D0 !D040 remove default NO2Stack value WSP 2/3/23
...
C - WSP 9/16/2024 D158 Dynamic File unit  Store FUNIT, FILENAME, and FILETYPE in array to add to output as table  - Starting       
            FUNITARRAY(FUNITIND) = INCUNT
            FNAMEARRAY(FUNITIND) = INCFIL
            FTYPARRAY(FUNITIND) = PATH // 'INCLUDED'
            FUNITIND = FUNITIND + 1

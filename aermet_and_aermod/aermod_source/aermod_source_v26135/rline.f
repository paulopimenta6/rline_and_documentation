...
C        DATE:        July 20, 2018
C
C        MODIFIED:    Terrain treatment included by adding the ZFLAG, ZHILL
C                     ZELEV heights
C                     Wood, 7/5/2022
C
...
C Wood 10/10/22 removed FOPT FROM MAIN1; NOT USED HERE
     ...
     &                 BAREDGE, !WSP edge effects 20240829
     &                 VBRDBG, VBRDBUNT, VBARSET !Veg barrier debug WSP 20250514
C     &                 FOPT !Wood 7/5/2022 
...
C     External Functions:
      DOUBLE PRECISION, EXTERNAL  :: SIGMAZ, MOST_WIND !added to calc initial sigma-z and VU10 for veg barriers WSP 20250319
...
CRLM Adding edge effects WSP 20240829
      LOGICAL          :: EDGESAVE, SHIFT_FLAG_SAV
      INTEGER          :: NBARR_SAV
      DOUBLE PRECISION :: XSHIFT_SAV, YSHIFT_SAV, HBU_SAV,
     &                    HBD_SAV, DWU_SAV, DWD_SAV, DW_PERU_SAV,
     &                    DW_PERD_SAV, ALPHA_U_SAV, ALPHA_D_SAV
C     EDGESAVE - logical flag to reset to saved barrier values     
C     *_SAV    - saved rline barrier values to use non barrier values
...
CRLM veg barrier WSP 20250312
C     If no solid barrier(s) and veg barrier(s) exist for source
      IF (NBARR .EQ. 0 .AND. VBARSET) THEN
        IF (VEGBARR(ISRC)%HT(1) .GT. 0.0D0 .OR. 
     &      VEGBARR(ISRC)%HT(2) .GT. 0.0D0) THEN
          CALL VEGBARRIERSET
        END IF
      END IF
...
C Set elevation of source, global variable in main1 - Wood 7/5/2022
      ZS = AZS(ISRC)
C     Calculate initial sigmaz and plume velocity for no barrier case
C     Used in vegetative barrier equations WSP 20250313
      IF (VEGBARDW) THEN
        I_ALPHA = 1 !Set locally in COMPUTE_MET and again in PLUME_CONC
        CALL EFFECTIVE_WIND(VDWD, ZS, 0.0D0) !UEFF
        VUI  = UEFF
C     Calc sigma-zi from no barrier sigma-za
        VSZI = SIGMAZ(VDWD)*(0.042D0*HVBD+1.118D0)
     &                     *(0.02873*LAID+0.7883D0)
        VU10 = MOST_WIND(10.0D0,1)
        
        IF (3.0D0*VSZI .GT. 2.2D0*HVBD) THEN
          VEGBARDW = .FALSE.
          IF (VBRDBG) THEN
            WRITE(VBRDBUNT, 300)
            WRITE(VBRDBUNT, 310) KURDAT, SRCID(ISRC), VSZI, VUI, VU10,
     &            VDWD, VDWD+WVBD, VDWD+WVBD+WAKED, 
     &            VDWD+WVBD+WAKED+3.0D0*HVBD
            WRITE(VBRDBUNT, *) "WARNING plume " //
     &        "has exited vegetative barrier high TKE regime"
          END IF
        ELSE
          IF (VBRDBG) THEN
            WRITE(VBRDBUNT, 300)
 ...
 300  FORMAT(/85('*')/,60X,'Region Start',/'DATETIME', 4X, 'SRCID', 8X,
     & 'SigmaZi', 5X, 'Ui', 5X, 'U10',6X,'1',7X,'2',7X,'3',7X,'4')          
 310  FORMAT(I8, 2X, A12,4X, F6.2, 3X, F6.2, 2X, F6.2,3X, F6.2, 2X, 
     &       F6.1, 2X, F6.1, 2X, F6.1)        
 400  FORMAT(/, 5X, "X", 13X, "Y", 10X, "Z", 5X, "Region", 2X, "SigmaZ", 
     &          5X, "U")
...
C begin - Add terrain treatment - Wood 7/5/2022
         ZFLAG = AZFLAG(IREC)
         ZELEV = AZELEV(IREC)
         ZHILL = AZHILL(IREC)                             
C end - Add terrain treatment - Wood 7/5/2022

CRLM Begin edge effect additions WSP 20240829
C   If edge effects are on and barrier(s) present,
C   If receptor not in barrier zone, 
C   assign no barrier values and save barrier values, 
C   reassign at end of receptor loop.
         IF (NBARR .GT. 0 .AND. BAREDGE) THEN
            IF (.NOT.REEDGE(IREC)) THEN
            !receptor not in barrier zone, 
            !set values to zero and save barrier values
              EDGESAVE = .TRUE.
              !Save barrier values
              NBARR_SAV      = NBARR
              SHIFT_FLAG_SAV = SHIFT_FLAG
              XSHIFT_SAV     = XSHIFT 
              YSHIFT_SAV     = YSHIFT 
              HBU_SAV        = HBU    
              HBD_SAV        = HBD    
            ...
            END IF
         END IF
CRLM  End  edge effect additions WSP 20240829
...
CRLM Begin edge effect addition WSP 20240829    
C        Reassign barrier values if switched for 
C        receptor barrier/edge zone location     
         IF (EDGESAVE) THEN
           EDGESAVE = .FALSE.
           NBARR      = NBARR_SAV      
           SHIFT_FLAG = SHIFT_FLAG_SAV 
           XSHIFT     = XSHIFT_SAV     
           YSHIFT     = YSHIFT_SAV     
           HBU        = HBU_SAV        
           HBD        = HBD_SAV        
           ...
           ALPHA_D    = ALPHA_D_SAV           
         END IF
CRLM  End  edge effect addition WSP 20240829         
...
C      PRINT *, "Xshift = ", Xshift, "Shift_flag = ", Shift_flag
      ...
      SUBROUTINE VEGBARRIERSET
C***********************************************************************
C        VEGBARRIERSET
C
C        PURPOSE:     Calculate distance between source and barrier 
C                     along wind direction. Determine number of barriers
C                     and which is downwind.
C
...
C     Variable Declarations:
      ...
      USE MAIN1, ONLY: ISRC, SRCID, PATH
      USE RLINE_DATA, ONLY: RLSOURCE, ZSBEGIN, ZSEND,
     &                      THETA_LINE, SM_NUM, BDW_FLAG,
     &                      VDWD, VDW_PERD, HVBD, WVBD, LAID, LADD, 
     &                      VBARR, VEGBARDW, WAKED, VEGBARR
...
C     Initialize Variables:
      I         = 2       ! assume barrier 2 is present, correct if necessary
      HVBD      = 0.0D0   ! height of downwind barrier
      VDW_PERD  = 0.0D0   ! perpendicular distance between source and downwind barrier
      VDWD      = 0.0D0   ! along wind distance between source and downwind barrier
      VBARR     = 0       !number of veg barriers present
      VEGBARDW  = .TRUE.  !vegetative barrier located downwind of source flag
      WVBD      = 0.0D0   !width of downwind barrier
      LAID      = 0.0D0   !leaf area index of downwind barrier
      LADD      = 0.0D0   !leaf area density of downwind barrier
      WAKED     = 0.0D0   !wake of downwind barrier

C     Calculate barriers distances and source height
          ZSBEGIN     = 0.5D0 * (ZSBEGIN + ZSEND)
          ZSEND       = ZSBEGIN

          HB(1)       = VEGBARR(ISRC)%HT(1)
          DW_PER(1)   = DABS(VEGBARR(ISRC)%DCL(1) -
     &                      RLSOURCE(ISRC)%DCL)
          DW(1)       = DW_PER(1) / (DABS(DSIN(THETA_LINE)) + SM_NUM)     ! distance between source and barrier along wind direction

          HB(2)       = VEGBARR(ISRC)%HT(2)
          DW_PER(2)   = DABS(VEGBARR(ISRC)%DCL(2) -
     &                      RLSOURCE(ISRC)%DCL)
          DW(2)       = DW_PER(2) / (DABS(DSIN(THETA_LINE)) + SM_NUM)
...
C     Assigning barriers 1 or 2 to upwind or downwind
        SELECT CASE(VBARR)
          CASE(0) ! NO BARRIERS
          ...
          CASE(1) ! ONE BARRIER
            ...
            IF(BDW_FLAG(ISRC,I) .EQ. 1) THEN                            ! located on downwind side
              ...
              WVBD = VEGBARR(ISRC)%WT(I)
              LAID = VEGBARR(ISRC)%LAI(I)
              LADD = VEGBARR(ISRC)%LM(I)
              WAKED = VEGBARR(ISRC)%WAKE(I)
            ELSE                                                        ! located on upwind side
              HVBD     = 0.0D0
              VDWD     = 0.0D0
              VDW_PERD = 0.0D0
     ...
     &               SRCID(ISRC)," are on the same side of the source."
            ...
            ELSE                                                        ! barriers are on opposite sides of source
              IF (BDW_FLAG(ISRC,1) .EQ. 1) THEN                         ! barrier 1 is downwind, barrier 2 is upwind
                ...
                WVBD = VEGBARR(ISRC)%WT(1)
                LAID = VEGBARR(ISRC)%LAI(1)
                LADD = VEGBARR(ISRC)%LM(1)
                WAKED = VEGBARR(ISRC)%WAKE(1)
              ELSE                                                      ! barrier 2 is downwind, barrier 1 is upwind
                HVBD     = HB(2)
                VDW_PERD = DW_PER(2)
                VDWD     = DW(2)
                
                WVBD = VEGBARR(ISRC)%WT(2)
                LAID = VEGBARR(ISRC)%LAI(2)
                LADD = VEGBARR(ISRC)%LM(2)
                WAKED = VEGBARR(ISRC)%WAKE(2)
              END IF
            END IF
...
C       If no downwind barrier, flip vegbar flag to false
        IF (HVBD .EQ. 0.0D0) VEGBARDW = .FALSE.

      
      END SUBROUTINE VEGBARRIERSET
...
C        DATE:        November, 2013
C
C        MODIFIED:    Debug file added for the grid wind speed.
C                     Create Wind speed array for use in calc2.f/CRITDS
...
C     Variable Declarations:
C Wood 10/10/22 removed ISRC, JDAY, GRIDSV, GRIDWS FROM MAIN1; NOT USED HERE
...
C     Local Variables:
C     DOUBLE PRECISION  :: SIGMAV_CALC, UREFCALC  ! D096
...
C      WSTAR_LOC = MAX(RLWSTAR, 0.0D0)                       ! D096

C     Calculate SIGMAV from WSTAR and USTAR variables
C     Calculate standard deviation of wind direction
C     SIGMAV_CALC = DSQRT((0.6D0 * WSTAR_LOC)**2 +          ! D096
C    &              (1.9D0 * USTAR)**2)                     ! D096
C     SIGMAV      = MAX(SIGMAV_CALC, SVMIN)                 ! D096

C     Obtain Sigma-V value from GRIDSV at the reference height by using GINTRP
...
C                                                                            --- CALL MOST_WIND
          I_ALPHA = 2
C          UH      = MOST_WIND(HBD,I_ALPHA) * WSPD_ADJ ! D096
          UH      = MOST_WIND(HBD,I_ALPHA) ! D096
      END IF

C begin ---     Create Wind speed array for use in calc2.f/CRITDS - Wood 7/5/2022
C         Modified to only use I_ALPHA=1 (no barrier), since the barrier open will still require FLAT MODELOPT.
C         Note, the array still has 2nd dimension with length 3, but they are set equal. - Wood 8/16/2022
      
      RL_GRIDWS(:,:) = 0.0D0 
C      ZIMECH_WS = MOST_WIND(ZI,1)  * WSPD_ADJ !Compute wind speed at top of boundary layer (ZI) Wood 8/16/2022 D096
      ...
      DO NP = 1, MXGLVL !Number of Grid levels 
         DO I_ALPHA = 1,1 ! For each Barrier case (none, downwind, upwind) - set to 1, since barriers will not consider terrain
            RL_GRIDWS(NP,:) = 
C     &                    MOST_WIND(GRIDHT(NP),I_ALPHA)  * WSPD_ADJ D096
     &                    MOST_WIND(GRIDHT(NP),I_ALPHA) !  D096

         END DO
      END DO
C end ---     Create Wind speed array for use in calc2.f/CRITDS - Wood 7/5/2022
C --- Write components and values to RLINE_GRIDWS.DBG
C     Added gridded wind speed debug file call Wood 10/11/22
C     To keep from being repeated for each source use the RLPROCESSED logical
      IF (RLINEDBG) THEN
         IF(.NOT. RLPROCESSED) THEN
            DO WS_I = 1,MXGLVL
...
C        References:
C***********************************************************************
C Wood 10/10/22 removed ISRC FROM MAIN1; NOT USED HERE
C      USE MAIN1, ONLY: STABLE, VONKAR, ZI, ISRC, PI
      USE MAIN1, ONLY: STABLE, VONKAR, ZI, PI
C Wood 10/10/22 removed SIGMAV FROM RLINE_DATA; NOT USED HERE
C      USE RLINE_DATA, ONLY: SIGMAV, LOGZMIN, LOGZMAX,
...
C        DATE:        November, 2013
C
C        MODIFIED:    Added RLINE debug file call to write out mean plume height
C                     WSP 10/10/22 formerly Wood
C
C        MODIFIED:    Incorporated RLINE2 barrier algorithms.
C                     Dianna Francisco & David Heist, US EPA ORD, 02/12/2021
C
...
C Wood 10/10/22 removed Z0_A, UST_A, LMO_A, DH_A FROM RLINE_DATA; NOT USED HERE
c      USE RLINE_DATA, ONLY: UEFF, SIGMAV, WSPD_ADJ, DISPHT,
       USE RLINE_DATA, ONLY: UEFF, SIGMAV, WSPD_ADJ,
C     &    I_ALPHA, Z0_A, DH_A, UST_A, LMO_A,
     &    I_ALPHA,
...
C                                                                            --- CALL MOST_WIND
C      UEFF = MOST_WIND(HEFF,I_ALPHA) * WSPD_ADJ ! D096
...
C                                                                            --- CALL MOST_WIND
C         UEFF   = MOST_WIND(MAX(ZBAR, HEFF), I_ALPHA) *     ! D096
C     &            WSPD_ADJ                                  ! D096
...
C     Added debug file call to write out mean plume height Wood 10/10/22
      IF (RLINEDBG) THEN
         WRITE(RLINEDBUNT,
     & '(A, A, F9.3)')
...
C     Variable Declarations
C Wood 10/10/22 removed LOGZMAX FROM RLINE_DATA; NOT USED HERE
C      USE RLINE_DATA, ONLY: LOGZMAX, LOGZMIN, DELZ, UMIN,
...
C      UMOST     = UMOST * WSPD_ADJ   ! D096
C      MOST_WIND = MAX(UMOST,UMIN)    ! D096
...
C***********************************************************************
C     PLUME_CONC Module of the AMS/EPA Regulatory Model - AERMOD
C
C        PURPOSE:     This code computes the direct plume contribution
C                     of a point source using Gaussian dispersion using
C                     receptor height. Two receptor heights are used in
C                     computing the terrain responding and impacting 
C                     plume states. The terrain weighting and combination
C                     with meander contibutions occur in POINT_CONC.
C                     See AERMOD v22112 MFD Section 5.1.
C
C        PROGRAMMER:  Originally in POINT_CONC - Last Modified 7/5/2022 
C                     Original Code by A. Venkatram, M. Snyder, D. Heist
C
C        DATE:        July 2022 WOOD 
C                     Moved code out of POINT_CONC to allow for terrain
C
C        INPUTS:      XD, YD, Z, DWDT, HEFF, HSHIFT, HBDEFF, HBMAX
C
C        OUTPUTS:     CONC_PLUME
C
C        CALLED FROM: POINT_CONC
C
C        CALLING
C        ROUTINES:    EFFECTIVE_WIND, VRTSBL
C
C***********************************************************************
...
C     &                      SIGMAZ0, SIGMAV, SZB, UEFF, XPER,
     ...
     &                      VEGBARDW, VSZI, HVBD, VDWD !vegetative barrier WSP 20250319
      IMPLICIT NONE
...
CRLM Add vegetative barrier equations for effective wind and sigma-z when downwind VBARRIER exists WSP 20250318
        IF (VEGBARDW .AND. XD .GT. VDWD) THEN
            CALL VEG_CALC(XD, SZ, UEFF) !calc sigma-z and u-eff for vegetative barrier     --- CALL VEG_CONC
        ELSE                                                                 --- CALL EFFECTIVE_WIND
...
CRLM        CALL EFFECTIVE_WIND(XD, Z, HSHIFT)
CRLM        SZ     = SIGMAZ(XD)
CRLM        SY     = SIGMAY(XD)
CRLM        UEFF_BU = F_UEFF * UEFF  ! reduce ueff immediately downwind of the barrier and then slowly increase back to ueff
        
C       Calculate vertical plume for gaussian mode                           --- CALL EXPX
C        VERT   = RT2BYPI * (EXPX(-0.5D0 * ((HEFF - ZRECEP) / SZ)**2)   ! D096
...
C       Corresponding to case numbers 1, 3, 6, 8, 10, 12 (meander only cases)
        HSHIFT = 0.0D0
C        CALL EFFECTIVE_WIND(XD, HEFF, HSHIFT)                               !D160 DKH 3/31/23 
...
C                     WSP, 10/11/22 Formally Wood
C                     Calculation of Vertical and Horizontal plumes to
C                     PLUME_CONC. Calcluatle the terrain adjustments to
C                     CONC_P and CONC_M
C                     Wood, 7/5/22
C
C        MODIFIED:    Added DEBUG output to RLINE.DBG file for each point
...
C     Variable Declarations:
C Wood 10/10/22 Removed CLIFT, CWRAP, IHOUR, L_FLATSRC, FLAT, RT2BYPI, SRT2PI,
C                       ZHILL FROM MAIN1, NOT USED
C      USE MAIN1,      ONLY: RT2BYPI, SRT2PI,RLINEDBG, RLINEDBUNT,
      USE MAIN1,      ONLY: RLINEDBG, RLINEDBUNT,
C     &                      ZELEV, ZFLAG, ZHILL, ZRT, ZS, STABLE, !WOOD 7/5/2022
     &                      ZELEV, ZFLAG, ZRT, ZS, STABLE, !WOOD 7/5/2022
C     &                       CLIFT, CWRAP, HCRIT, FOPT, IHOUR, ! WOOD 6-28-2022
     &                        HCRIT, FOPT, ! WOOD 6-28-2022
     &                        L_FLATSRC,ISRC,  !D173 WSP - 7/24/2023
C     &                       L_FLATSRC, FLAT, HSBL, PHEE, SZ, ZI, UNSTAB ! WOOD 6-28-2022
     &                        HSBL, PHEE, SZ, ZI, UNSTAB ! WOOD 6-28-2022
C JAT 06/22/21 D065, REMOVE XPER AS A VARIABLE USED FROM RLINE_DATA, NOT USED
C Wood 10/10/22 Removed ALPHA, ALPHA_U, ALPHA_D, SHIFT_FLAG, SIGMAZ0, SZB, UH
C                       XD_MIN,  FROM RLINE_DATA, NOT USED
      USE RLINE_DATA, ONLY: XR_ROT, YR_ROT, ZRECEP, UEFF,
     &                      XSBEGIN, XSEND, XR_ROT, YR_ROT,
     &                      ZRECEP, HBD, DWD, DW_PERD, HBU,
C     &                      SIGMAZ0, SIGMAV, SZB, UEFF, XPER,
     &                       SIGMAV, UEFF,
C     &                      NBARR, UH, XSHIFT, SHIFT_FLAG, XD_MIN,
     &                      NBARR,  XSHIFT,
C     &                      I_ALPHA, ALPHA, ALPHA_U, ALPHA_D,
     &                      I_ALPHA,
     &                      FRAN_SUM, SIGMAV_SUM, UEFF_SUM,
     &                      VERT_SUM, HORZ_SUM, CONC_P_SUM, CONC_M_SUM,
     &                      POINT_CONC_SUM, YSHIFT
...
C     Local Variables:
C Wood 10/10/22 Removed CQ, FQ,F_UEFF, F_UH, HMAX, A, IA_MAX, UEFF_BU NOT USED
      DOUBLE PRECISION  :: CONC_M, CONC_P, VERT, HORZ
C     DOUBLE PRECISION  :: SY, SZ, SIGMAZ0_ORIG  ! Wood moved to PLUME_CONC for terrain calculations 7/5/2022
      DOUBLE PRECISION  :: FRAN
      DOUBLE PRECISION  :: XD, YD, XMAX, DWDT
      DOUBLE PRECISION  :: HEFF, HSHIFT, HBMAX, HBDEFF
C      DOUBLE PRECISION  :: CQ, FQ, UEFF_BU
C      DOUBLE PRECISION  :: F_UEFF, F_UH, HMAX, A
C      INTEGER           :: IA_MAX
...
C     Declare flags:
C      logical ::  Direct_flag     ! TRUE = direct plume; FALSE = meander only plume !moved to PLUME_CONC - Wood 7/5/2022
C      logical ::  Gaussian_flag   ! TRUE = gaussian mode; FALSE = mixed-wake mode !moved to PLUME_CONC - Wood 7/5/2022
...
C     Initialize local variables
C      DIRECT_FLAG   = .FALSE. !moved to PLUME_CONC - Wood 7/5/2022
C      GAUSSIAN_FLAG = .FALSE. !moved to PLUME_CONC - Wood 7/5/2022
      CONC_P  = 0.0D0
      CONC_M  = 0.0D0
      FRAN    = 0.0D0
C      SZB     = 0.0D0 !moved to PLUME_CONC - Wood 7/5/2022
C      SY      = 0.0D0 !moved to PLUME_CONC - Wood 7/5/2022
C      SZ      = 0.0D0 !moved to PLUME_CONC - Wood 7/5/2022
      XMAX    = 0.0D0
C      VERT    = 0.0D0 !moved to PLUME_CONC - Wood 7/5/2022
C      HORZ    = 0.0D0 !moved to PLUME_CONC - Wood 7/5/2022
      DWDT    = 0.0D0
C      ALPHA   = 1.0D0 !moved to PLUME_CONC - Wood 7/5/2022
C      I_ALPHA = 1 !moved to PLUME_CONC - Wood 7/5/2022
C      IA_MAX  = 2 !moved to PLUME_CONC - Wood 7/5/2022                                               ! assume HBD > HBU (will correct this below if it's not)
...
C      SIGMAZ0_ORIG = SIGMAZ0  ! store intial sigmaz value from input file !moved to PLUME_CONC - Wood 7/5/2022
C      F_UEFF  = 1.0D0     ! factor for lowering ueff and then slowly increasing back to original ueff !moved to PLUME_CONC - Wood 7/5/2022

C Initialize WRAP and LIFT components - Wood 7/5/2022
         CWRAP_P = 0.0D0
         CWRAP_M = 0.0D0
         CLIFT_P = 0.0D0
         CLIFT_M = 0.0D0
...
C begin - Moved calculation of Vertical and Horizontal plumes to PLUME_CONC  - Wood 7/5/2022

C     Calculate direct plume concentration ----------------------------
C
C     Set flags for direct/meander and gaussian/mixed-wake
C     IF (XD .LT. 0.0001D0) THEN
C       NO DIRECT PLUME
C       DIRECT_FLAG = .FALSE.
C     ELSE
C       DIRECT PLUME
C       XD            = MAX(XD, XD_MIN)                                      ! shift in x has already occurred if needed
C       DIRECT_FLAG   = .TRUE.
C       GAUSSIAN_FLAG = .TRUE.
C
C       If there is a downwind barrier and receptor is downwind of it then mixed-wake mode
C       IF((HBD .GT. 0.0D0) .AND. (XD .GT. DWDT)) THEN
C         GAUSSIAN_FLAG = .FALSE.
C       END IF
C
C     END IF
C
C     Setting heff, szB, i_alpha, and alpha (and F_UEFF for upwind barriers)
C     IF(DIRECT_FLAG) THEN
C       IF(SHIFT_FLAG) THEN
C         Corresponding to case numbers 7, 11, 14 (shift cases)
C         HEFF    = MAX(Z, 0.75D0 * HBU)                                     ! source height is adjusted upwards due to the upwind barrier
C         SZB     = 0.0D0
C         I_ALPHA = 3                                                        ! set index for ALPHA_U
C         ALPHA   = ALPHA_U                                                  ! set alpha
C                                                                            --- CALL EXPX
C         F_UEFF  = 1.0D0 - 1.0D0 * EXPX(-1.0D0 * XD / (8.0D0 * HBU))        ! factor for reducing ueff in upwind barrier shift cases
C       ELSE
C         Corresponding to case numbers 2, 4, 5, 9, 13, 15 (no shift cases)
C         HEFF    = Z
C         SZB     = 0.0D0
C         I_ALPHA = 1                                                        ! set index for ALPHA = 1.0D0
C         ALPHA   = 1.0D0                                                    ! set alpha
C       END IF
C     ELSE
C       Corresponding to case numbers 1, 3, 6, 8, 10, 12 (meander only cases)
C       Note that heff is set in MEANDER subroutine
C       IF(NBARR .GT. 0) THEN
C         SZB     = 0.25D0 * HBMAX
C         IF(HBU .GT. HBD) IA_MAX = 3                                        ! If HBU > HBD, use ALPHA_U
C         I_ALPHA = IA_MAX                                                   ! set index for MAX(ALPHA_D, ALPHA_U)
C         ALPHA   = MAX(ALPHA_D, ALPHA_U)                                    ! set alpha
C       ELSE
C         I_ALPHA = 1
C         ALPHA   = 1.0D0
C       END IF
...
C
C
CC     Calculate vertical plume
C      IF((DIRECT_FLAG) .AND.
C     &  (GAUSSIAN_FLAG)) THEN
CC       Calculate with gaussian mode equations
CC       Corresponding to case numbers 2, 4, 7, 9, 11, 13 (gaussian cases)
C        HSHIFT = 0.0D0
CC                                                                            --- CALL EFFECTIVE_WIND
C        CALL EFFECTIVE_WIND(XD, Z, HSHIFT)
C         SZ     = SIGMAZ(XD)
C         SY     = SIGMAY(XD)
C        UEFF_BU = F_UEFF * UEFF                                              ! reduce ueff immediately downwind of the barrier and then slowly increase back to ueff
C
CC       Calculate vertical plume for gaussian mode                           --- CALL EXPX
C        VERT   = RT2BYPI * (EXPX(-0.5D0 * ((HEFF - ZRECEP) / SZ)**2)
C     &           + EXPX(-0.5D0 * ((HEFF + ZRECEP) / SZ)**2)) /
C     &           (2.0D0 * SZ * UEFF_BU)
CC      Calculate horizontal plume                                            --- CALL EXPX
C        HORZ   = 1.0D0 / (SRT2PI * SY) * EXPX(-0.5D0 * (YD / SY)**2)
C
C      ELSE IF((DIRECT_FLAG) .AND. (.NOT. GAUSSIAN_FLAG)) THEN
CC       Corresponding to case numbers 5, 14, 15 (mixed-wake cases)
CC       Calculate with mixed-wake mode equations in 2 steps
C
CC       Step 1: calculate plume spread between source and downwind barrier   --- CALL EFFECTIVE_WIND
C        HSHIFT  = HBD
C        CALL EFFECTIVE_WIND(DWDT, HEFF, HSHIFT)
C        SZB     = SIGMAZ(DWDT) + 0.1D0 * HBD                                 ! assigns vertical spread of plume at dwDt to szB
C        SIGMAZ0 = 0.0D0                                                      ! set to zero to not double count intial sigmaz
C
CC       Step 2: calculate plume spread beyond the downwind barrier           --- CALL EFFECTIVE_WIND
C        I_ALPHA = 2                                                          ! set index for ALPHA_D
C        ALPHA   = ALPHA_D                                                    ! set alpha
C        CALL EFFECTIVE_WIND(XD - DWDT, HEFF, HSHIFT)
C        SZ      = SIGMAZ(XD - DWDT)
C        SY      = SIGMAY(XD - DWDT)
C        SIGMAZ0 = SIGMAZ0_ORIG                                               ! reset sigmaz0 for next time thru subroutine
C
CC       Calculate vertical plume for mixed-wake mode                         --- CALL EXPX
C        CQ      = (1.0D0 / (SRT2PI * SZ * UEFF)) * 2.0D0 *
C     &             EXPX(-0.5D0 * (HEFF / SZ)**2)                             ! Venkatram et al. (2021)
C
C        HMAX    = MAX(HBD,12.0D0)                                            ! tallest barrier height affected by downwind barrier effect
C        A       = (1.0D0 - (0.75D0 * HBD)/HMAX)
CC                                                                            --- CALL EXPX
C        F_UH    = 1.0D0 - A * EXPX(-(XD - DWDT) / (9.0D0 * HBD))             ! UH factor; 9H is the scaling factor controlling rise back to original ueff
C
C        FQ      = 1.0D0 / (1.0D0 + F_UH * 0.5D0 * UH * CQ * HBDEFF)          ! Venkatram et al. (2021); assuming unit emissions
C                                 
CC                                                                            --- CALL EXPX
C        IF(ZRECEP .GT. HBDEFF) THEN
CC         Note: reflection term is about Z = HBDEFF, so the reflective source is at z = HBDEFF-HEFF
C          VERT  = FQ * (1.0D0 / (SRT2PI * SZ * UEFF)) *
C     &           (EXPX(-0.5D0 * ((ZRECEP - HBDEFF - HEFF) / SZ)**2) +
C     &            EXPX(-0.5D0 * ((ZRECEP - HBDEFF + HEFF) / SZ)**2))         ! Venkatram et al. (2021)
C        ELSE
C          VERT  = FQ * CQ                                                    ! Venkatram et al. (2021)
C        END IF
CC      Calculate horizontal plume                                            --- CALL EXPX
C        HORZ   = 1.0D0 / (SRT2PI * SY) * EXPX(-0.5D0 * (YD / SY)**2)
C      
C      ELSE
CC       Corresponding to case numbers 1, 3, 6, 8, 10, 12 (meander only cases)
C        HSHIFT = 0.0D0
C        CALL EFFECTIVE_WIND(XD, HEFF, HSHIFT)
C        VERT = 0.0D0                                                         ! no direct plume, so CONC_P should be zero
C        HORZ = 0.0D0
C
C      END IF

CC     Calculate total direct plume
C      CONC_P = VERT * HORZ
C
CC     Calculate meander concentration
CC     Get fraction of random kinetic energy (from AERMOD routines)           --- CALL MEANDR
C      CALL MEANDR(UEFF,SIGMAV,FRAN)
C                                        
C
C      CONC_M = MEANDER(X, Y, Z, HSHIFT)
C end - Moved calculation of Vertical and Horizontal plumes to PLUME_CONC  - Wood 7/5/2022

C begin - Terrain adjustments to CONC_P and CONC_M - Wood 7/5/2022

C     Calculate height of receptor above stack base, ZRT
CWSP         ZRT = ZELEV - ZS + ZFLAG
CWSP --- Begin: D173 WSP add 7/24/2023
      IF (L_FLATSRC(ISRC)) THEN 
         ZRT = ZFLAG
      ELSE
         ZRT = ZELEV - ZS + ZFLAG
      END IF
CWSP --- End: D173 WSP add 7/24/2023

C Calculate FOPT for I_ALPHA= 1 (i.e. no barrier) and HEFF = Z (set above) - Wood 7/5/2022
C note: I_ALPHA & HSHIFT are not set YET!
      I_ALPHA = 1

C  JAT 3/26/2026 D234 (D234_RLINE_NAN) at this point UEFF may not be set or has been set to zero.  If 
C  it is a stable hour or unstable and ze >= ZI, then SIGMAZ is called.
C  SIGMAZ uses UEFF in the calculations of URATIO where UEFF is the divisor
C  in the URATIO equation. Since UEFF is zero, this results in Infinity for URATIO
c  causing SIGMAZ and SZ to be NaN.  Call effective_wind to calculate UEFF if it is <= 0 in
C  case SIGMAZ needs to be called.
      IF (UEFF .LE. 0.0D0) CALL EFFECTIVE_WIND(XD,HEFF,HSHIFT)
C  Calculate height of the "effective reflecting surface" !MGS - 7/18/2022
      IF (STABLE .OR. (UNSTAB.AND.(ZS.GE.ZI))) THEN
         SZ = SIGMAZ(XD)
C        JAT 3/26/2026 D234 (D234_RLINE_NAN) change XD to be the absolute value
C        of XD for REFL_HT. XD is XARG in REFL_HT and used to calculate TTRAVL which
C        is a time so it should be positive.  Note that for other source types
C        XARG input to REFL_HT is positive
C        CALL REFL_HT (HEFF, XD, 0.0D0, SZ, HSBL)
         CALL REFL_HT (HEFF, DABS(XD), 0.0D0, SZ, HSBL)
      ELSEIF ( UNSTAB ) THEN
         HSBL = 0.0D0
      END IF
...
C     Calculate the fraction of plume below   HCRIT, PHEE                               ---   CALL PFRACT
      CALL PFRACT (HEFF)
...
C Re-Calculate FOPT if I_ALPHA changes from 1 in PLUME_CONC (i.e. there is a barrier) - Wood 7/5/2022
      IF((I_ALPHA .NE. 1) .OR. (HEFF .NE. Z)) THEN
      
C     Calculate height of the "effective reflecting surface" !Wood - 7/18/2022  
         IF (STABLE .OR. (UNSTAB.AND.(ZS.GE.ZI))) THEN
            SZ = SIGMAZ(XD)
C          JAT 3/26/2026 D234 (D234_RLINE_NAN) change XD to be the absolute value
C          of XD for REFL_HT. XD is XARG in REFL_HT and used to calculate TTRAVL which
C          is a time so it should be positive.  Note that for other source types
C          XARG input to REFL_HT is positive
c            CALL REFL_HT (HEFF, XD, SZ, 0.0D0, HSBL)
		  CALL REFL_HT (HEFF, DABS(XD), SZ, 0.0D0, HSBL)
         ELSEIF ( UNSTAB ) THEN
            HSBL = 0.0D0
         END IF
...
C     Terrain Weighting of CLIFT (Terrain-following) and CWRAP( horizontal plume) for MEANDER and Direct PLUME
...
C      CALL MEANDR(UEFF,SIGMAV,FRAN)    ! D096, move MEANDR call after MEANDER call
...
C end - Terrain adjustments to CONC_P and CONC_M - Wood 7/5/2022

C     Combine direct plume and meander contributions with the terrain included
...
C     Added HCRIT, FOPT and PHEE Wood 10/10/22
         WRITE(RLINEDBUNT,
     & '((A, E9.3), 2(" , ", A, E9.3))')
     &           '                           HCRIT = ', HCRIT,
     &           'FOPT = ', FOPT,
...
C      IF (STABLE) THEN
C         SIGMAY = 1.6D0 * SIGMAV / USTAR * SZ *
C     &           (1.0D0 + 2.5D0 * SZ / DABS(OBULEN))
C      ELSE
C         SIGMAY = 1.6D0 * SIGMAV / USTAR * SZ /
C     &            DSQRT(1.0D0 + 1.0D0 * SZ / DABS(OBULEN))
C      END IF
C     D096 Updated the SigmaY coefficients based on optimization WSP 4/5/23
      IF (STABLE) THEN
...
C      SIGMAZ_MAX = RT2BYPI * ZI       ! D096
      IF(FASTRLINE) THEN
        ...
        ELSE
          SIGZ   = PSZ3 * XD**PSZ4
        END IF
C        SIGMAZ = MIN(SIGZ, SIGMAZ_MAX)   ! D096
          SIGMAZ = SIGZ                   ! D096
        RETURN
      END IF
...
C     &          (1.0D0 + 3.0D0 * URATIO *       ! D096
C     &          (EXP(TWOTHIRDS * LOG(XBAR))))   ! D096
...
C     Variable Declarations:
      USE MAIN1, ONLY: AXR, AYR, NUMREC, NUMSRC, ISRC, SRCTYP, WDREF,
     &                PI, VBARSET !veg barrier
      USE RLINE_DATA, ONLY: X0, Y0, XSB_ROT, YSB_ROT, XSE_ROT, YSE_ROT,
     &                      THETAW, XRCP_ROT, YRCP_ROT, RLSOURCE,
     &                      BDW_FLAG, THETA_LINE,
     &                      VEGBARR !veg barrier WSP 20250312
      IMPLICIT NONE
...
CRLM            DCLWALL_LOC(1) = RLSOURCE(INDEX)%DCLWALL
CRLM            DCLWALL_LOC(2) = RLSOURCE(INDEX)%DCLWALL2
CRLM        vegetative barrier WSP 20250312
            IF (RLSOURCE(INDEX)%HTWALL  .GT. 0.0D0 .OR. 
     &          RLSOURCE(INDEX)%HTWALL2 .GT. 0.0D0) THEN
              DCLWALL_LOC(1) = RLSOURCE(INDEX)%DCLWALL
              DCLWALL_LOC(2) = RLSOURCE(INDEX)%DCLWALL2
            ELSE IF (VBARSET) THEN 
              IF (VEGBARR(INDEX)%HT(1) .GT. 0.0D0 .OR. 
     &            VEGBARR(INDEX)%HT(2) .GT. 0.0D0) THEN
                DCLWALL_LOC(1) = VEGBARR(INDEX)%DCL(1)
                DCLWALL_LOC(2) = VEGBARR(INDEX)%DCL(2)
              END IF
            END IF
...
C           After rotation, determine if the barrier is downwind of the roadway. If so, set a flag.
     ...
     &                     YR_TRAN * DCOS(THETAW)
      ...
      SUBROUTINE VEG_CALC (XD, VSZ, VUEFF)
C***********************************************************************
C        VEG_CALC
C
C        PURPOSE:     Calculate wind speed and sigma-z if vegetative barrier 
C                     is present downwind of source
C
C        PROGRAMMER:  Rebecca L. Miller, WSP
C
C        DATE:        March 2025
...
C        MODIFIED:    
C
C        INPUTS:      XD   - downwind distance
C
C        OUTPUTS:     VUEFF - effective wind speed
C                     VSZ   - sigma z
C
C        CALLED FROM: PLUME_CONC   
C   
C        REFERENCES:  Hashad et al. 2024
C
C***********************************************************************

C     Variable Declarations:
      USE MAIN1
      USE RLINE_DATA, ONLY: HVBD, WVBD, LAID, LADD, WAKED, VSZI, VUI,
     &                      VU10, WRTVDBG, ZRECEP, VDWD
      
      IMPLICIT NONE
      
C     Local Variables:
      DOUBLE PRECISION, INTENT(IN)   ::  XD
      DOUBLE PRECISION, INTENT(OUT)  ::  VSZ, VUEFF
      INTEGER                        ::  Region
...
C---  Region 1
C      IF (XD .LE. WVBD) THEN  
      IF (XD .GE. VDWD .AND. XD .LE. WVBD+VDWD) THEN
        Region = 1
        VSZ    = VSZI + (XD-VDWD)*(-0.037D0*HVBD**(-1.505D0) + 0.07D0)
        VUEFF  = VUI  + (XD-VDWD)*(0.02233D0*LADD**(-1.231D0)
     &                - 0.0149D0)
      END IF
      
C---  Region 2
C      IF (XD .GT. WVBD .AND. XD .LE. WVBD+WAKED) THEN
      IF (XD .GT. WVBD+VDWD .AND. XD .LE. WVBD+VDWD+WAKED) THEN
        Region = 2
        VSZ    = VSZI + WVBD*(-0.037D0*HVBD**(-1.505D0) + 0.07D0) 
     &                +(XD-WVBD-VDWD)*0.013D0
        VUEFF  = VUI  + WVBD*(0.02233D0*LADD**(-1.231D0) - 0.0149D0) 
     &                +(XD-WVBD-VDWD)*(0.08912D0*LADD + 0.7987D0)
     &                          *(-0.002252D0*VU10)     
      END IF

C---  Region 3
      IF (XD .GT. WVBD+VDWD+WAKED .AND. 
     &    XD .LE. WVBD+VDWD+WAKED+3.0D0*HVBD) THEN
        Region = 3
        VSZ    = VSZI + WVBD*(-0.037D0*HVBD**(-1.505D0) + 0.07D0) 
     &                + WAKED*0.013D0 
     &                +(XD-WVBD-WAKED-VDWD)*(0.000695D0)*HVBD*LAID
        VUEFF  = VUI  + WVBD*(0.02233D0*LADD**(-1.231D0) - 0.0149D0) 
     &                +WAKED*(0.08912D0*LADD + 0.7987D0)
     &                          *(-0.002252D0*VU10) 
     &                +(XD-WVBD-WAKED-VDWD)*(0.003063D0*LAID 
     &                 - 0.008317D0)*(0.4378D0*VU10   - 0.3306D0)      
      END IF
C---  Region 4
      IF (XD .GT. WVBD+VDWD+WAKED+3.0D0*HVBD) THEN
        Region = 4
        VSZ    = VSZI + WVBD*(-0.037D0*HVBD**(-1.505D0) + 0.07D0) 
     &                + WAKED*0.013D0 
     &                +(3.0D0*HVBD)*(0.000695D0)*HVBD*LAID
     &                +(XD-(VDWD + WVBD + WAKED + 3.0D0*HVBD))
     &                             *(0.000695D0)*HVBD*LAID
        U = DMAX1(VU10,1.0D0)
        C = ((0.1285D0*LADD**(-2.105D0) + 0.4923D0)
     &      *(0.3592D0*U**(-18.68D0)    + 0.9558D0))
        VUEFF  = VUI  + WVBD*(0.02233D0*LADD**(-1.231D0) - 0.0149D0) 
     &                +WAKED*(0.08912D0*LADD + 0.7987D0)
     &                          *(-0.002252D0*VU10) 
     &                +(3.0D0*HVBD)*(0.003063D0*LAID - 0.008317D0)
     &                             *(0.4378D0*VU10   - 0.3306D0)
     &                +(-0.4371D0*LADD**(-1.822D0) + 1.192D0)
     &                *(0.05351D0*VU10 - 0.01589D0)
     &                *((XD-(VDWD + WVBD + WAKED + 3.0D0*HVBD))
     &               **C)               

      END IF
      
      IF (VBRDBG .AND. WRTVDBG) THEN
        WRTVDBG = .FALSE.
        WRITE(VBRDBUNT,410) AXR(IREC), AYR(IREC), ZRECEP, Region, VSZ,
     &                      VUEFF        
 410  FORMAT(F12.2, 2X, F12.2, 2X,F6.1, 4X, I1, 5X, F6.2, 3X, F6.2)
      END IF

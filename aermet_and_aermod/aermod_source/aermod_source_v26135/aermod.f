...
C=======================================================================
C
C       This revised version of AERMOD (dated 26135) includes
C       updates relative to the previous version (dated 24142);
C       see AERMOD Model Change Bullentin (MCB) #19 and the AERMOD User's
C       Guide.
C
...
C                       May 15, 2026
C
C-----  MODIFIED FROM:  AERMOD (Version Dated 26135)
C
C=======================================================================
C
...
CMGS D086_Inconsistant_OPEN_DebugFiles_WSP(BEGIN) -- moved to coset.f/OPEN_ALL_DEBUGFILES
CMGSC --- Declare character strings for use in the header records of
CMGSC     debug files for NO2 options
CMGS      CHARACTER (LEN=18) :: NO2DebugString
CMGS      CHARACTER (LEN=50) :: NO2DebugStrNonEvt, NO2DebugStrEvent
CMGS D086_Inconsistant_OPEN_DebugFiles_WSP(END) -- moved to coset.f/OPEN_ALL_DEBUGFILES
...
C      LOGICAL :: L_OPENED, HIT_THRESH
      LOGICAL :: L_OPENED
      INTEGER :: I  ! CRT 5/13/2026 D242 Dynamic Funit E565
...
CMGS D086_Inconsistant_OPEN_DebugFiles_WSP(BEGIN) -- moved to coset.f/OPEN_ALL_DEBUGFILES
CMGS      NO2DebugString    = ''
CMGS      NO2DebugStrNonEvt = ''
CMGS      NO2DebugStrEvent  = ''
CMGS D086_Inconsistant_OPEN_DebugFiles_WSP(END) -- moved to coset.f/OPEN_ALL_DEBUGFILES
      NTOTHRS = 0
      NYEARS  = 0
      IBSTAT  = 0
      FUNITIND = 1 ! WSP 9/16/2024 D158 Dynamically Allocate FUNIT 
      FUNITINX = 100 ! WSP 9/16/2024 D158 Dynamically Allocate FUNIT  - Starting value for output files
      HOUREMISIND = 1 ! CWSP 1/24/2025 D175 HOUREMIS Filename D121 Hourly Emissions File
      HOUREMISINX = 1 ! CWSP 1/24/2025 D175 HOUREMIS Filename D121 Hourly Emissions File
      FATAL  = .FALSE.
      RUNERR = .FALSE.
      L_OPENED = .FALSE.
...
CWSP ! 9/16/2024 D158 Dynamic File unit  Store FUNIT, FILENAME, and FILETYPE in array to add to output as table  - Starting       
         FUNITARRAY(FUNITIND) = ITEVUT
         FNAMEARRAY(FUNITIND) = evtfil(1:levt)
         FTYPARRAY(FUNITIND) = 'INP_EVENTFILE'
         FUNITIND = FUNITIND + 1
CWSP ! 9/16/2024 D158 Dynamic File unit  Store FUNIT, FILENAME, and FILETYPE in array to add to output as table  - Ending      
...
CMGS D134_UniformDebugHeaders: Rearranging & Streamlining
CMGS Moved opening of NOx option debug files and writing of option specific 
CMGS  headers to a new subroutine in coset.f. Called here:
      CALL OPEN_ALL_DEBUGFILES

CCRT 11/7/2025 D086 Open Debug Files - Moved call statement for calculations 
C    from SOCARD called in SETUP before debug files are open.   
...
C         CALL FUNITBL(IOUNIT) ! WSP 9/16/2024 D158 Dynamically Allocate FUNIT - write File Unit Table to output file
...
CMGS             IF (PVMRMDBG) THEN
CMGS C ---          PVMRM Debug option selected; print header record to delimit
CMGS C              debug information related to MAXDCONT processing
CMGS                WRITE(PVMDBG,9001)
CMGS             END IF
...
C      CALL FUNITBL(IOUNIT)  ! WSP 9/16/2024 D158 Dynamically Allocate FUNIT - write File Unit Table to output file
C      CALL HOURITBL(IOUNIT) ! WSP 1/24/2024 D175 HOUREMIS Filename D121 Hourly Emissions File
...
C        Write Summary of Messages to optional SUMMFILE
         ...
         WRITE(ISUMUNT,9114)
         CALL SUMTBL(ISUMUNT)
C         CALL FUNITBL(ISUMUNT) ! WSP 9/16/2024 D158 Dynamically Allocate FUNIT - write File Unit Table to sumfile
...
9116     FORMAT(/4X,'************************************',
     ...
     &          /4X,'************************************'/)
      ...
      IF (ERRLST) THEN
CCRT      3/4/2026 D158 Dynamic File unit - move OPEN statement to coset.f, 
CCRT         subroutine ERRFIL - so an open error can be trapped during setup
CCRT         to avoid a runtime error
cC        OPEN and Write Out Permanent Error Message File    ---   CALL MSGWRT
c         OPEN(UNIT=IERWRT,FILE=MSGFIL,STATUS='REPLACE',
c     &        FORM='FORMATTED')
cCWSP ! 9/16/2024 D158 Dynamic File unit  Store FUNIT, FILENAME, and FILETYPE in array to add to output as table  - Starting       
c         FUNITARRAY(FUNITIND) = IERWRT
c         FNAMEARRAY(FUNITIND) = MSGFIL
c         FTYPARRAY(FUNITIND) = 'IERWRT'
c         FUNITIND = FUNITIND + 1
cCWSP ! 9/16/2024 D158 Dynamic File unit  Store FUNIT, FILENAME, and FILETYPE in array to add to output as table  - Ending      

CCRT     3/4/2026 D158 Dynamic File unit - Add INQUIRE statements to confirm error file is open
CCRT     Open file error is now trapped in coset.f during setup
         L_OPENED = .FALSE.
         INQUIRE (FILE=MSGFIL,OPENED=L_OPENED)
         IF (L_OPENED) THEN
            CALL MSGWRT
            CLOSE(IERWRT)
         END IF
...
CCRT  5/13/2026 D242 - Dynamic FUnit_E565 - close all files at completion
      DO I = 1,LBOUND(FUNITARRAY,1),UBOUND(FUNITARRAY,1)
         INQUIRE (UNIT=FUNITARRAY(I),OPENED=L_OPENED)
         IF (L_OPENED) CLOSE(FUNITARRAY(I))
      END DO   
...
C        DATE:    March 2, 1992
C
C        MODIFIED:   Streamlined for Aircraft sources, by adding nested 
C                    IF-statement instead of complicated IF-statements.
C                    M.G. Snyder, WSP 6/5/2023
C
C        MODIFIED:   Modified to handle the Aircraft's Engine parameters
C                    for VOLUME/AREA Source only for Aircraft Source Group.
...
CWSP      INTEGER :: I, ILSAVE  !CWSP 1/24/2024 D175 HOUREMIS Filename D121 Hourly Emissions Fil
      INTEGER :: I, JJ, ILSAVE  !CWSP 1/24/2024 D175 HOUREMIS Filename D121 Hourly Emissions Fil
...
C                  IQLINE = IQLINE + 1
CMKP              Check for aircraft source type for reading/setting
...
C**  Added for Aircraft Plume Rise; UNC-IE !D151 - MGS 6/5/23
                     IF (AFTSRC(ISRC) .EQ. 'Y') THEN
                     ...
                     END IF
C**  End Aircraft Plume Rise insert; April 2023 !D151 - MGS 6/5/23
...
C              Write sampled meteorology to SCIM'd met data file
            ...
            END IF
         END IF
CCRT  5/6/2026 D026 INTFILE/MAXIFILE Correct for changes in 26216r
C     where code was removed in RSINIT subroutine to advance IRSDATE
C     by one hour. Since no longer advanced, reconfigure conditional below
C     based on actual restart date/hour from restart file so last date/hour
C     is not duplicated.
C    
CCRT         IF (FULLDATE.GE.ISDATE .AND. FULLDATE.LE.IEDATE .AND.
         IF (((.not. RSTINP .and. FULLDATE .GE. ISDATE) .or.
     &        (RSTINP .and. .not. MULTYR .and. 
     &         FULLDATE .GT. ISDATE) .or. 
     &         (MULTYR .and. FULLDATE .GE. ISDATE)) .AND. 
     &        FULLDATE.LE.IEDATE .AND.
     &       ( (.NOT.L_LeapYear.AND.IPROC (JDAY).EQ.1) .OR.
     &              (L_LeapYear.AND.IPROCL(JDAY).EQ.1) ) .AND.
...
C*    Assign IQLINE counter to ILINE for passing to ERRHDL if needed, save as ILSAVE first
      ILSAVE = ILINE
      ILINE = IQLINE(IS) ! CWSP 1/24/2025 D175 HOUREMIS Filename D121 Hourly Emissions File 
C      ILINE  = IQLINE   !CWSP 1/24/2025 D175 HOUREMIS Filename D121 Hourly Emissions File 
...
CC --- Check for use of 2-digit year in HOUREMIS file, adjust to 4-digit
CC     year for comparison with FULLDATE based on met data file
...
C*                   M.G. Snyder WSP 6/5/2023
C*
C*         MODIFIED:  Modified to assign Aircraft Engine parameters to
C*                    calculate plume rise for VOLUME/AREA Sources
C*                    only for Aircraft Source Group.
C*                    Gavendra Pandey; UNC-IE, Chapel Hill, NC, USA
C                     04/01/2023
C*
...
C ---          Initialize ReadErr and EndErr for this sector
     ...
     &                                              O3CONC
               ELSE
C                 D001/D145 CRT 6/1/2023 - Delete erroneous read statement introduced with bug fix
C                  READ(IO3UNT(I),O3FORM(I),ERR=99,END=9991)
...
C        D001 Call CENT_DATE to determine the current Julian Day and Calculate Current Gregorian Date First Convert Year to 4-Digit Value Wood 9/15/22
         ...
         END IF
C ---  D001 remove original calculation of 4-Digit year Wood 9/15/22
CC ---          Check for use of 2-digit year in OZONEFIL file, adjust to 4-digit
CC              year for comparison with FULLDATE based on met data file
C               IF (IO3YR .LE. 99) THEN
C                  IO3YR2 = IO3YR
C                  IF (IO3YR2 .GE. ISTRT_WIND .AND.
C     &                                 IO3YR2 .LE. 99) THEN
C                     IO3YR  = ISTRT_CENT*100 + IO3YR2
C                  ELSE IF (IO3YR2 .LT. ISTRT_WIND) THEN
C                     IO3YR  = (ISTRT_CENT+1)*100 + IO3YR2
C                  END IF
C               END IF
...
C        D001 Call CENT_DATE to determine the current Julian Day and Calculate Current Gregorian Date First Convert Year to 4-Digit Value Wood 9/15/22
         ...
         END IF
C ---  D001 remove original calculation of 4-Digit year Wood 9/15/22
CC ---          Check for use of 2-digit year in BACKGRND file, adjust to 4-digit
CC              year for comparison with FULLDATE based on met data file
C               IF (IBGYR .LE. 99) THEN
C                  IBGYR2 = IBGYR
C                  IF (IBGYR2 .GE. ISTRT_WIND .AND.
C     &                                 IBGYR2 .LE. 99) THEN
C                     IBGYR  = ISTRT_CENT*100 + IBGYR2
C                  ELSE IF (IBGYR2 .LT. ISTRT_WIND) THEN
C                     IBGYR  = (ISTRT_CENT+1)*100 + IBGYR2
C                  END IF
C               END IF
...
C        D001 Call CENT_DATE to determine the current Julian Day and Calculate Current Gregorian Date First Convert Year to 4-Digit Value Wood 9/15/22
         ...
         END IF
C ---  D001 remove original calculation of 4-Digit year Wood 9/15/22
CC ---          Check for use of 2-digit year in NOX_FILE file, adjust to 4-digit
CC              year for comparison with FULLDATE based on met data file
C               IF (INOXYR .LE. 99) THEN
C                  INOXYR2 = INOXYR
C                  IF (INOXYR2 .GE. ISTRT_WIND .AND.
C     &                                 INOXYR2 .LE. 99) THEN
C                     INOXYR  = ISTRT_CENT*100 + INOXYR2
C                  ELSE IF (INOXYR2 .LT. ISTRT_WIND) THEN
C                     INOXYR  = (ISTRT_CENT+1)*100 + INOXYR2
C                  END IF
C               END IF
...
CWSP ! 9/16/2024 D158 Dynmaically Allocate FUNIT - Begin      
      SUBROUTINE FUNITBL(FILUNIT)
C***********************************************************************
C                 FUNITBL Module of the AMS/EPA Regulatory Model - AERMOD
C
C        PURPOSE: To Print Out The FUNIT Summary Table
C
C        DATE:    September 16, 2024
C***********************************************************************
C     Variable Declarations
      USE MAIN1
      IMPLICIT NONE
      ...
      INTEGER :: F_UNIT, FILUNIT
      
C     Variable Initialization
      MODNAM = 'FUNITBL'

C     Write Out The FUNIT Summary Table
      WRITE(FILUNIT,*) ''
      WRITE(FILUNIT,*) ' --------- Summary of FUNIT --------'
      WRITE(FILUNIT,*) 'Filetype        FUNIT     Filename'
      
      DO F_UNIT = 1, FUNITIND-1 
        WRITE(FILUNIT,8765) FTYPARRAY(F_UNIT), FUNITARRAY(F_UNIT),
     &  TRIM(ADJUSTL(FNAMEARRAY(F_UNIT)))
      END DO
CCRT  3/4/2026 D158 Dynamic File Units - expand format     
 8765 FORMAT (1X, A12, 5X, I10, 5X, A) 
C 8765 FORMAT (1X, A12, 5X, I4, 5X, A)

      RETURN
      END SUBROUTINE FUNITBL
CWSP ! 9/16/2024 D158 Dynmaically Allocate FUNIT - End      

CWSP @ 1/24/2024 D175 HOUREMIS Filename D121 Hourly Emissions File - Starting 
      SUBROUTINE HOURITBL(FILUNIT)
C***********************************************************************
C                 HOURITBL Module of the AMS/EPA Regulatory Model - AERMOD
C
C        PURPOSE: To Print Out The HOUREMIS Summary Table
C
C        DATE:    January 24, 2025
C***********************************************************************
C     Variable Declarations
      USE MAIN1
      IMPLICIT NONE
...
C     Write Out The FUNIT Summary Table
      WRITE(FILUNIT,*) ''
      WRITE(FILUNIT,*) ' --------- Summary of HOUREMIS File --------'
      WRITE(FILUNIT,*) 'FUNIT     Filename                  
     &  SRCID List        '
      
      DO F_UNIT = 1, NSRC 
        IF (QFLAG(F_UNIT).EQ.'HOURLY') THEN
           WRITE(FILUNIT,8766) HOUREMISARRAY(F_UNIT),
     &     TRIM(ADJUSTL(HOUREMISNAMEARRAY(F_UNIT))), HRSRCIDs(F_UNIT)
        END IF
      END DO
      
 8766 FORMAT (1X, I4, 5X, A, 5X, A)
      RETURN
      END SUBROUTINE HOURITBL
CWSP ! 1/24/2025 D175 HOUREMIS Filename D121 Hourly Emissions File - Ending
...
C
C        MODIFIED:  To include ARISE variables and allocate memory
C                   for these (Aircraft Plume Rise).
C                   Gavendra Pandey; UNC-IE, Chapel Hill, NC, USA
C                   04/01/2023
C
...
C     Variable Declarations
     ...
     &                      BDW_FLAG,
     &                      SLDBARR, !For SBARRIER inputs (WSP) 20240424
     &                      VEGBARR !VBARRIER inputs WSP 20250304
      USE BUOYANT_LINE
      IMPLICIT NONE
      CHARACTER MODNAM*12
...
CWSP D174 - WSP changed NSWP to NSRC
CWSP      ALLOCATE (ABW(NSWP), ABL(NSWP), ABH(NSWP), ABA(NSWP),
CWSP     &          SWXS(NSWP), SWYS(NSWP), STAT=IASTAT)
...
C        Initialize MOVES input unit flag and FIRSTHR flag
     ...
     &                      'RLINE Temporary Receptor Arrays!'
         END IF
CRLM VBARRIER WSP 2025 04 03         
C        Allocate vegetative barrier array
         IF (NVBR .GT. 0) THEN
           ALLOCATE(VEGBARR(NSRC), STAT=IASTAT)
           IF (IASTAT .NE. 0) THEN
              CALL ERRHDL(PATH,MODNAM,'E','409','Setup Arrays')
              ALLOC_ERR = .TRUE.
              WRITE(IOUNIT,*) '  Error Occurred During Allocation of ',
     &                        'Vegetative Barrier Arrays!'
           END IF
         END IF
      ENDIF
      
CRLM  SBARRIER Inputs (start) WSP 20240424
      IF (NSBR .GT.0) THEN
          ALLOCATE(SLDBARR(NSBR), STAT=IASTAT)
          IF (IASTAT .NE. 0) THEN
            CALL ERRHDL(PATH,MODNAM,'E','409','Setup Arrays')
            ALLOC_ERR = .TRUE.
            WRITE(IOUNIT,*) '  Error Occurred During Allocation of ',
     &                      'SBARRIER Array!'
         END IF
      END IF
CRLM  SBARRIER Inputs (end) WSP 20240424
...
C                    for ARISE (Aircraft Plume Rise) option.
C                    Gavendra Pandey; UNC-IE, Chapel Hill, NC, USA
C                    04/01/2023
C
C        MODIFIED    Added storage requirement estimate and allocations
...
C
C        MODIFIED:   To include new options related to ARCFTOPT option
...
C     &           RQF, RPDMAX, RVMAX, RPAIR, RHIANN
      ...
      INTEGER :: MaxFiles ! WSP 9/16/2024 D158 Dynamically Allocate FUNIT 
...
C     Initialize logical for the 'URBANSRC ALL' option
      L_URBAN_ALL = .FALSE.
C**  Added for Aircraft Plume Rise; UNC-IE
C     Initialize logical for aircraft option
      L_PRESET_ARCFT = .FALSE.
C     Initialize logical for the 'ARCFTSRC ALL' option
      L_ARCFT_ALL = .FALSE.
C**  End Aircraft insert; April 2022  
! Added for HBP, JAN 2023
C     Initialize logical for the 'HBPSRC ALL' option
      L_HBP_ALL = .FALSE.
! End insert for HBP
...
C --- Initalize DEBUG_FLS character variable D144 WSP 10/20/22
      DEBUG_FLS = ''
...
C        Process Cards to Determine Storage Requirements
         IF (PATH .EQ. 'CO') THEN
C           D144 added counter for the number of debugopt keywods WSP 10/19/22
            IF(KEYWRD .EQ. 'DEBUGOPT') THEN
...
CWSP ! 9/16/2024 D158 Dynamic File unit  Store FUNIT, FILENAME, and FILETYPE in array to add to output as table  - Beginning
C Calculate Maximum number of FUNITs needed = (MAX SOURCEGROUPS * MAX AVERTIME * 10 possible output types ) + 100 Hardcoded FUNITS in modules.f
      MaxFiles = (NGRP * NAVE * 10) + 100 
      FUNITIND = 1 ! Starts at 1 (initialized in aermod.f)
      FUNITINX = 100 ! First value for output files
      ALLOCATE (FUNITARRAY(MaxFiles)) ! FUNIT
      ALLOCATE (FNAMEARRAY(MaxFiles)) ! Filename
      ALLOCATE (FTYPARRAY(MaxFiles))  ! File type
      
CCRT  5/13/2026 D242 Dynamic Funit E565 - Initialize file arrays
C     Initialize file arrays
      FUNITARRAY(:) = 0
      FNAMEARRAY(:) = ''
      FTYPARRAY(:) = ''
      
C  Store already opened input and output file units
      FUNITARRAY(FUNITIND) = INUNIT
      FNAMEARRAY(FUNITIND) = inpfil
      FTYPARRAY(FUNITIND) = 'INPUT'
      FUNITIND = FUNITIND + 1
      
      FUNITARRAY(FUNITIND) = IOUNIT
      FNAMEARRAY(FUNITIND) = outfil
      FTYPARRAY(FUNITIND) = 'OUTPUT'
      
      FUNITIND = FUNITIND + 1
      
CWSP ! 9/16/2024 D158 Dynamic File unit  Store FUNIT, FILENAME, and FILETYPE in array to add to output as table  - Ending
CWSP 1/24/2025 D175 HOUREMIS Filename D121 Hourly Emissions File - Starting
      ALLOCATE (HOUREMISARRAY(NSRC)) ! HOUREMIS FUNIT
      HOUREMISARRAY(:) = 0
      ALLOCATE (HOUREMISNAMEARRAY(NSRC)) ! Filename
      HOUREMISNAMEARRAY(:) = ''
      ALLOCATE (HRSRCIDs(NSRC))  ! SRCID Name
      HRSRCIDs(:) = ''
      ALLOCATE (IQLINE(NSRC))  ! Line to read of HOUREMIS file
CWSP 1/24/2025 D175 HOUREMIS Filename D121 Hourly Emissions File - Ending
...
C        DATE:    September 24, 1996
C
C        MODIFIED:   To include the Aircraft Sources (ARISE).
C                    Gavendra Pandey; UNC-IE, Chapel Hill, NC, USA
C                    04/01/2023
C
...
C ---    D113 Added for SIDEWASH
         ...
         PREVSRCID = '        '
         PREVGRPID = '        '
C ---    D156 This routine is called twice when an include file is used. NUM_SRCIDS, TMPSRCID, and SAVESRCID
C        are used for AREACIRC source type. The following lines will clear out these three variables
C        so that the source IDs are not double counted the second time through. WSP 2/7/23
         NUM_SRCIDS = 0
         NSBR = 0 !number if SBARRIER BARID WSP 20240424
         NVBR = 0 !number of VBARRIER BARID WSP 20250225
...
C ---            NO2 method is specified that is not implemented for a source, 4/27/2023 CRT
...
C           Check that the required ALPHA flag is present for RLINEXT sources
C           or if processing INCLUDED keyword (MODELOPTs not set yet!) D150 AKP 05/23/2023
            IF(L_ALPHA .OR. L_PREINC) THEN
               NRLINES = NRLINES + 1
            ELSE
C              D150 CRT 6/1/2023 - Update error message code to specify ALPHA required
C               CALL ERRHDL(PATH,MODNAM,'E','199',' RLINEXT')
...
C**  Added for Aircraft Plume Rise; UNC-IE
     ...
     &                           .AND. L_PRESET_ARCFT) THEN
            L_ARCFT_ALL = .TRUE.
C --- Assign number of AIRCRAFT sources
         NAFTSRC = NSRC
C --- Check for number of ARCFTSRC
      ELSE IF (KEYWRD .EQ. 'ARCFTSRC') THEN
...
CRLM  SBARRIER inputs WSP 20240424 (start)         
      ELSE IF (KEYWRD .EQ. 'SBARRIER') THEN
      !BARID counter
        IF (NSBR .EQ. 0) SBARID = '            '
        IF (FIELD(3) .NE. SBARID) THEN
          NSBR = NSBR + 1
          SBARID = FIELD(3)
        END IF
CRLM  SBARRIER inputs WSP 20240424 (end)

CRLM  VBARRIER inputs WSP 20250225 (start)         
      ELSE IF (KEYWRD .EQ. 'VBARRIER') THEN
        NVBR = NVBR + 1
CRLM  VBARRIER inputs WSP 20250225 (end)
...
C        DATE:    September 24, 1996
C
C        MODIFIED:   To include the recursive keyword for D112
C                    Wood - 9/29/22 
C
C        INPUTS:  Pathway (RE) and Keyword
C
...
C     Check The Numerical Field
      ...
      END IF
      ISYEAR = NINT(FNUM)
C     Adjusted the if statement to set default window to 1950 only when the year is between 50 and 99 Wood 9/15/22
      IF (ISYEAR .GE. 50 .AND. ISYEAR .LT. 100) THEN
C      IF (ISYEAR .LT. 100) THEN
...
C        variables, ISTRT_CENT (=19) and ISTRT_WIND (=50).
         ...
         ISTRT_CENT = 19
         ISTRT_WIND = 50
C     Added a catch for dates which could occur in 20XX, values must be between 00 and 49 Wood 9/15/22 
...
C       D001 Call LONG_DATE and determine the longform date Wood 9/15/22
        CALL LONG_DATE(NHIDATMXD_BYYR(IR,JGRP,IVAL,I),IEDATE,ICYR2,ICYR)

C ---  D001 remove original calculation of year Wood 9/15/22
C                     IF (ICYR2 .GE. ISTRT_WIND .AND.
C     &                                    ICYR2 .LE. 99) THEN
C                        ICYR   = ISTRT_CENT*100 + ICYR2
C                        IEDATE = ISTRT_CENT*100000000 +
C     &                           NHIDATMXD_BYYR(IR,JGRP,IVAL,I)
C                     ELSE IF (ICYR2 .LT. ISTRT_WIND) THEN
C                        ICYR   = (ISTRT_CENT+1)*100 + ICYR2
C                        IEDATE = (ISTRT_CENT+1)*100000000 +
C     &                           NHIDATMXD_BYYR(IR,JGRP,IVAL,I)
C                     END IF
...
C***********************************************************************
C*                  AHRQREAD Module of AERMOD
C*
C*         PURPOSE: To Assign Hourly Source Parameters
C*
C*         PROGRAMMER:  Gavendra Pandey; UNC-IE, Chapel Hill, NC, USA
C*                      M.K. Porter EPA/OAQPS/AQMG 
C*                      M.G. Snyder WSP (updated/streamlined on 6/1/23)
C*
C*         DATE:    5/23/2023
C*
C*         INPUTS:  Current AIRCRAFT Source Number Being Processed
C*
C*         OUTPUTS: Source Arrays
C*
C*         Revision History:
C* 
C*         MODIFIED:  Streamlined original AHRQREAD to only handle aicraft 
C*                    sources. M.G. Snyder 6/1/2023
C*
C*         MODIFIED:  Modified to issue fatal error when aircraft plume
C*                    is turned on and hourly parameters are missing for
C*                    area and/or volume sources.  AHRQREAD was adapted
C*                    from HRQREAD originally submitted by UNC-IE. 
C*                    M.K. Porter EPA/OAQPS/AQMG 5/23/2023
C*
C*         MODIFIED:  Modified HRQREAD to read the Aircraft Engine parameters to
C*                    calculate the  plume rise for VOLUME/AREA Source
C*                    only for Aircraft Source Group.
C*                    Gavendra Pandey; UNC-IE, Chapel Hill, NC, USA
C*                    04/01/2023
C*
C*         STARTED WITH HRQREAD (last modified 03/18/2019)
C*
C*         CALLED FROM:  HRLOOP
C************************************************************************
C*
...
C* Catch any sourcetypes not defined as aircraft
...
C*       These sources are not defined under aircraft category
C*       - WRITE Error Message
         WRITE(DUMMY,'(I10.10)') FULLHRQ
...
C     based on the ISTRG PARAMETER (set in MAIN1)
      WRITE(RDFRM,9100) ISTRG, ISTRG
CCRT  6/25/2026 D175 - Multiple Hourly Emission Files - set emission file unit from array
      IHREMI = HOUREMISARRAY(IS)
 9100 FORMAT('(A',I4.4,',T1,',I4.4,'A1)')
      READ (IHREMI,RDFRM,END=888,ERR=99) RUNST1, (RUNST(I), I=1, ISTRG)
C*
...
C*       Assign zeros to all parameters
         ...
         END IF
C**  Added for Aircraft Plume Rise; UNC-IE
CMGS         IF ( AFTSRC(IS) .EQ. 'Y') THEN !already KNOW it is an aircraft source
...
C -------------------------- Begin too few parameters for source type
CMKP Check for missing fields for volume and area sources when Aircraft
C Plume Rise is specified.  Minimum of 15 fields required without HRLYSIG.
C Aircraft new fields (7 total): MFUEL,THRUST,VAA,AFR,BYPR,RPWR,SRCANGLE
      ELSE IF ((SRCTYP(IS) .EQ. 'VOLUME') .AND.
     &          (IFC .LE. 11)) THEN
         CALL ERRHDL(PATH,MODNAM,'E','807',SRCID(IS))
         RUNERR = .TRUE.
         GO TO 9990
CMGS Changed check to only check first 4 characters for AREA (D151 - 6/1/23)
CMGS      ELSE IF (SRCTYP(IS) .EQ. 'AREA' .AND.
      ELSE IF ((SRCTYP(IS)(1:4) .EQ. 'AREA') .AND.
     &         (IFC .LE. 10)) THEN
         CALL ERRHDL(PATH,MODNAM,'E','807',SRCID(IS))
         RUNERR = .TRUE.
         GO TO 9990
C -------------------------- End too few parameters for source type

C -------------------------- Begin correct # parameters for source type
      ELSE IF ((SRCTYP(IS) .EQ. 'VOLUME') .AND.
     &          (IFC .EQ. 18)) THEN
C*      Assign emission rate, release height, initial sigmas and
C*      Engine parameters for aircraft sources plume rise
C*      for VOLUME source.
C*       Assign logical variable indicating hourly sigmas, L_HRLYSIG
         IF (ILSAVE .EQ. 1) THEN
            L_HRLYSIG(IS) = .TRUE.
...
C*          should be used for missing values.
         ...
         IF (IMIT .NE. 1) THEN
            ...
            RUNERR = .TRUE.
            GO TO 9990
C ---    Check for negative values, could be a missing indicator
         ELSE IF ( HRMFUEL .LT. 0.0D0 ) THEN
C*          Assume fuel burn rates are missing; assign value of 0.0 and issue Warning;
C*          Note that AERMOD User's Guide (p. 3-38, 3-39) implies that blanks
C*          should be used for missing values.
            HRMFUEL = 0.0D0
            WRITE(DUMMY,'(I10.10)') FULLHRQ
...
C*          Assume thrusts are missing; assign value of 0.0 and issue Warning;
C*          Note that AERMOD User's Guide (p. 3-38, 3-39) implies that blanks
C*          should be used for missing values.
            HRTHRUST = 0.0D0
            WRITE(DUMMY,'(I10.10)') FULLHRQ
...
C*          Assume aircraft velocities are missing; assign value of 0.0 and issue Warning;
C*          Note that AERMOD User's Guide (p. 3-38, 3-39) implies that blanks
C*          should be used for missing values.
            HRVAA = 0.0D0
            WRITE(DUMMY,'(I10.10)') FULLHRQ
...
C*          Assume air fuel ratios are missing; assign value of 0.0 and issue Warning;
C*          Note that AERMOD User's Guide (p. 3-38, 3-39) implies that blanks
C*          should be used for missing values.
            HRAFR = 0.0D0
            WRITE(DUMMY,'(I10.10)') FULLHRQ
         ...
         IF (IMIT .NE. 1) THEN
            ...
            GO TO 9990
C ---    Check for negative values, could be a missing indicator,
C ---    whereas BYPR = -999 is for shaft-based engines aircraft and
C ---    BYPR greater than 0 for turbine-based engines aircraft
         ELSE IF ( HRBYPR .LT. 0.0D0 .AND. HRBYPR .NE. -999.0D0 ) THEN
C*          Assume bypass ratios are missing; assign value of 0.0 and issue Warning;
C*          Note that AERMOD User's Guide (p. 3-38, 3-39) implies that blanks
C*          should be used for missing values.
            HRBYPR = 0.0D0
            WRITE(DUMMY,'(I10.10)') FULLHRQ
         ...
         IF (IMIT .NE. 1) THEN
            ...
            GO TO 9990
C ---    Check for negative values, could be a missing indicator,
C ---    whereas RPWR = -99999 is for turbine-based engines aircraft
C ---    and greater than 0.0 for shaft-based engines aircraft
         ELSE IF ( HRRPWR .LT. 0.0D0 .AND. HRRPWR .NE. -99999.0D0 )THEN
C*          Assume rated power are missing; assign value of 0.0 and issue Warning;
C*          Note that AERMOD User's Guide (p. 3-38, 3-39) implies that blanks
C*          should be used for missing values.
            HRRPWR = 0.0D0
            WRITE(DUMMY,'(I10.10)') FULLHRQ
         ...
         IF (IMIT .NE. 1) THEN
            ...
            RUNERR = .TRUE.
            GO TO 9990
C ---    Check for unrealistics or negative values, could be a missing indicator,
         ELSE IF ( HRSRCANGLE .LT. -20.0D0 .OR.
     &             HRSRCANGLE .GT.  20.0D0 ) THEN
C*          Assume source angles are missing; assign value of 0.0 and issue Warning;
C*          Note that AERMOD User's Guide (p. 3-38, 3-39) implies that blanks
C*          should be used for missing values.
            HRSRCANGLE = 0.0D0
            WRITE(DUMMY,'(I10.10)') FULLHRQ
      ...
      ELSE IF ((SRCTYP(IS) .EQ. 'VOLUME') .AND.
     &          (IFC .EQ. 15)) THEN
C*       Assign emission rate and engine parameters for
C*       Aircraft volume sources
C*       Check logical variable indicating hourly sigmas, L_HRLYSIG
         IF (L_HRLYSIG(IS)) THEN
C*          WRITE Error Message; Hourly Sigmas must be used for all hours
            WRITE(DUMMY,'(I10.10)') FULLHRQ
...
C*          should be used for missing values.
      ...
      ELSE IF ((SRCTYP(IS)(1:4) .EQ. 'AREA') .AND.
     &         (IFC.EQ.17)) THEN
C*       Assign emission rate, release height, initial sigma and
C*      Engine parameters for aircraft sources plume rise
C*       for Area source.
C*       Assign logical variable indicating hourly sigmas, L_HRLYSIG
         IF (ILSAVE .EQ. 1) THEN
            L_HRLYSIG(IS) = .TRUE.
         ELSE IF (ILSAVE .GT. 1 .AND. .NOT. L_HRLYSIG(IS)) THEN
C*          This Area source should not include hourly sigmas;
C*          issue error message
            WRITE(DUMMY,'(I10.10)') FULLHRQ
            CALL ERRHDL(PATH,MODNAM,'E','345',DUMMY)
            HRQS = 0.0D0
...
C*          should be used for missing values.
         ...
         IF (IMIT .NE. 1) THEN
            ...
            GO TO 9990
C ---    Check for negative values, could be a missing indicator,
C ---    whereas BYPR = -9990 is for shaft-based engines aircraft and
C ---    BYPR greater than 0 for turbine-based engines aircraft
         ELSE IF ( HRBYPR .LT. 0.0D0 .AND. HRBYPR .NE. -999.0D0 ) THEN
C*          Assume bypass ratios are missing; assign value of 0.0 and issue Warning;
C*          Note that AERMOD User's Guide (p. 3-38, 3-39) implies that blanks
C*          should be used for missing values.
         ...
         IF (IMIT .NE. 1) THEN
            ...
            GO TO 9990
C ---    Check for negative values, could be a missing indicator,
C ---    whereas RPWR = -999099 is for turbine-based engines aircraft
C ---    and greater than 0.0 for shaft-based engines aircraft
         ELSE IF ( HRRPWR .LT. 0.0D0 .AND. HRRPWR .NE. -99999.0D0 )THEN
C*          Assume rated power are missing; assign value of 0.0 and issue Warning;
C*          Note that AERMOD User's Guide (p. 3-38, 3-39) implies that blanks
...
C*       Assign emission rate for area sources
         CALL STODBL(FIELD(8), ILEN_FLD, HRQS, IMIT)
         IF (IMIT .NE. 1) THEN
            CALL ERRHDL(PATH,MODNAM,'E','208','HOUREMIS')
            RUNERR = .TRUE.

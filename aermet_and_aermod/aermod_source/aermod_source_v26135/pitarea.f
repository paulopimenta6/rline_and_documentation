...
c----    Added 1/(2pi) when calculating meander/pancake plume Wood 5/5/22
        IF (L_APLUME) THEN
...
C        DATE:    July 7, 1993
C
C        MODIFIED:   To add the Aircraft's plume rise.
...
C        DATE:    July 7, 1993
C
C        MODIFIED BY Gavendra Pandey; UNC-IE, CHapel Hill, NC, USA to
C                    add the Aircraft's Plume Rise - 04/01/2023
C
C        MODIFIED BY R. Brode, PES, Inc. to move calculation of dispersion
C                    coefficients to a new ADIS subroutine - 7/21/94
C
...
C
C        INPUTS: XARG   - Downwind Distance (m) between source and receptor
C                V1     - Crosswind Distance
C
C        OUTPUTS: VN    - Lateral Dispersion Parameter (Effective Width)
C                 WIDTH - Vertical Dispersion Parameter
C
C        CALLED FROM:   ZBRENT
C                       PSIDE
C                       PLUMEF
...
C    Initialized variables that are not updated with calculating meander/pancake plume Wood 6/3/22
      WIDTH = 0.0D0
      VN = 0.0D0
...
C     Added calculation for Sigmay if calculating meander 5/4/22 Wood
C     XARG is the approximate distance from the receptor to the downwind distance of the source

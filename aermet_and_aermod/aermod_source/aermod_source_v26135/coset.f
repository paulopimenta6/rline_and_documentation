...
C        DATE:    March 2, 1992
C
C        MODIFIED:   To add the Aircraft Option (ARCFTOPT)
C                    (to calculate the plume rise for Aircraft sources).
...
C           D144 Track the number of DEBUGOPT keywords that have been checked WSP 10/19/22
            CURRENT_DEBUGOPT = CURRENT_DEBUGOPT+1
C           Process Error File Option                       ---   CALL DEBOPT
            CALL DEBOPT
C        D144 Allow DEBUGOPT to be a repeatable keyword commented out original WSP 10/11/22
C         IF (ICSTAT(22) .NE. 1) THEN
CC           WRITE Error Message: Repeat Non-repeatable Keyword
C            CALL ERRHDL(PATH,MODNAM,'E','135',KEYWRD)
C         ELSE
CC           Process Error File Option                       ---   CALL DEBOPT
C            CALL DEBOPT
C         END IF
...
C        Check for Keyword Out of Order
         IF(ICSTAT(4) .NE. 1) THEN
C           WRITE Error Message: Keyword Out of Order (Must Follow
C           MODELOPT)
            CALL ERRHDL(PATH,MODNAM,'E','140',KEYWRD)
         END IF
            IF (IFC .EQ. 3) THEN
C            Assign the airport name (optional)
             AFTID(1) = FIELD(3)
            END IF
C     Assign Logical for Aircraft Option
          ARCFT  = .TRUE.
...
C - CRT 5/12/2026 D158 Dynamic File unit - Correction - File has already been opened and assigned a unit 
C - WSP 9/16/2024 D158 Dynamic File unit  Store FUNIT, FILENAME, and FILETYPE in array to add to output as table  - Starting       
c            FUNITARRAY(FUNITIND) = IDPUNT
c            FNAMEARRAY(FUNITIND) = SAVFIL
c            FTYPARRAY(FUNITIND) = 'SAVEFILE'
c            FUNITIND = FUNITIND + 1
...
C - CRT 5/12/2026 D158 Dynamic File unit - Correction - File has already been opened and assigned a unit 
C - WSP 9/16/2024 D158 Dynamic File unit  Store FUNIT, FILENAME, and FILETYPE in array to add to output as table  - Starting       
c               FUNITARRAY(FUNITIND) = IDPUN2
c               FNAMEARRAY(FUNITIND) = SAVFL2
c               FTYPARRAY(FUNITIND) = 'SAVEFILE2'
c               FUNITIND = FUNITIND + 1
C - WSP 9/16/2024 D158 Dynamic File unit  Store FUNIT, FILENAME, and FILETYPE in array to add to output as table  - Ending 
C              Close SAVEFILE since it is re-opened in RSDUMP
               CLOSE (IDPUN2)
...
C - WSP 9/16/2024 D158 Dynamic File unit  Store FUNIT, FILENAME, and FILETYPE in array to add to output as table  - Starting       
                  FUNITARRAY(FUNITIND) = IRSUNT
                  FNAMEARRAY(FUNITIND) = INIFIL
                  FTYPARRAY(FUNITIND) = 'IRSUNT'
                  FUNITIND = FUNITIND + 1
C - WSP 9/16/2024 D158 Dynamic File unit  Store FUNIT, FILENAME, and FILETYPE in array to add to output as table  - Ending      
               END IF
            ELSE
...
CWSP         IF (.NOT. DFAULT .AND. L_ALPHA) THEN        ! Bug Fix D177 WSP 8/26/2024
CWSP            MODOPS(19) = 'ALPHA'                     ! Bug Fix D177 WSP 8/26/2024
         IF (.NOT. DFAULT .AND. BETA .AND. L_ALPHA) THEN ! Bug Fix D177 WSP 8/26/2024
            MODOPS(19) = 'ALPHA  BETA'                   ! Bug Fix D177 WSP 8/26/2024
         ELSE IF (.NOT. DFAULT .AND. L_ALPHA) THEN       ! Bug Fix D177 WSP 8/26/2024
            MODOPS(19) = 'ALPHA'                         ! Bug Fix D177 WSP 8/26/2024
...
C**  Added for Aircraft Plume Rise; UNC-IE
C ---    Add MODOPS field for Aircraft
         IF (NAFTSRC .EQ. NSRC .OR. NAFTSRC .GT. 0.0D0) THEN
C ---       Aircraft sources
            MODOPS(24) = 'AIRCRAFT'
C             Check for ALPHA and DFAULT conflicts
CWSP begin --- D151 ALPHA requirement MGS 5/18/2023
CWSP              IF (.NOT. BETA) THEN
CWSPC               WRITE Error Message     ! BETA Option Required for ARCFT
CWSP                CALL ERRHDL(PATH,MODNAM,'E','199','ARCFTOPT')
CWSP              END IF
CWSP          CHECK ALPHA ON MODELOPT 
              IF (.NOT. L_ALPHA) THEN
C               WRITE Error Message     ! ALPHA Option Required for ARCFT
                CALL ERRHDL(PATH,MODNAM,'E','198','ARCFTOPT')
              END IF
CWSP end  --- D151 ALPHA requirement MGS 5/18/2023
              IF (DFAULT) THEN
...
C -----  Wood 6/3/22 D128 Add ALPHA option for area meander , AREAMNDR 
         ELSE IF (KOPT .EQ. 'AREAMNDR') THEN
            IF (L_ALPHA) THEN
C ---          Assign AREAMNDR = .TRUE.
               L_AREAMNDR = .TRUE.
...
C              D154 Changed the warning message to an error is NOMINO3 is set without NO2 technique 1/31/23 WSP
C               CALL ERRHDL(PATH,MODNAM,'W','621',KOPT)
               CALL ERRHDL(PATH,MODNAM,'E','621',KOPT)
            END IF
            
CRLM Adding barrier edge effect model opt WSP 20240814
         ELSE IF (KOPT .EQ. 'BAREDGE') THEN
            BAREDGE = .TRUE.
            !can add warning messages for other required model opt
...
CCRT 2/11/2026, D138 Scientific Notation: Set PollCrit to true if POLLID is a criteria pollutant.
      SELECT CASE (POLLUT)
         CASE ("SOX", "SO2", "NOX", "NO2", "CO", 
     &         "TSP", "PM10", "PM-10", 
...
C - CRT 3/4/2026 D158 Dynamic File Unit catch error opening file
      OPEN(UNIT=IEVUNT,FILE=EVFILE,STATUS='REPLACE',
     &     FORM='FORMATTED', err=99)
c     &     FORM='FORMATTED')
C - WSP 9/16/2024 D158 Dynamic File unit  Store FUNIT, FILENAME, and FILETYPE in array to add to output as table  - Starting       
      FUNITARRAY(FUNITIND) = IEVUNT
      FNAMEARRAY(FUNITIND) = EVFILE
      FTYPARRAY(FUNITIND) = 'EVENTFILE'
      FUNITIND = FUNITIND + 1
C - WSP 9/16/2024 D158 Dynamic File unit  Store FUNIT, FILENAME, and FILETYPE in array to add to output as table  - Ending      
      RETURN
C - CRT 3/4/2026 D158 Dynamic File Unit catch error opening file
99    CALL ERRHDL(PATH,MODNAM,'E','500','EVENTFIL')
      RETURN
...
CCRT   3/4/2026 D158 Dynamic File unit moved open statement from aermod.f
CCRT        Added error trapping to avoid runtime open file error
C           OPEN Permanent Error Message File
            OPEN(UNIT=IERWRT,FILE=MSGFIL,STATUS='REPLACE',
     &        FORM='FORMATTED', err=99)
CWSP ! 9/16/2024 D158 Dynamic File unit  Store FUNIT, FILENAME, and FILETYPE in array to add to output as table  - Starting       
            FUNITARRAY(FUNITIND) = IERWRT
            FNAMEARRAY(FUNITIND) = MSGFIL
            FTYPARRAY(FUNITIND) = 'IERWRT'
            FUNITIND = FUNITIND + 1
         ELSE
...
99    CALL ERRHDL(PATH,MODNAM,'E','500','ERRORFIL')
...
C
C        MODIFIED    DEBUGOPT now repleatable. Added processing to read and store.
C                    Michelle G. Snyder (original attempt by Laura Kent in 2022)
C                    WSP 8/18/2025
C
C        MODIFIED    Modified to allow user to specify debug output for
C                    the Aircraft option.
C                    Gavendra Pandey; UNC-IE, Chapel Hill, NC, USA
C                    04/01/2023
C
...
! we move NBLP out of BUOYANT_LINE and into MAIN1 instead in modules.f?
      ...
      CHARACTER (LEN=ILEN_FLD) :: FIELD_sv   !D144 WSP 10/21/22
      DIMENSION   FIELD_sv(IFMAX)   !D144 WSP 10/21/22
CMGS D172_COSET_DEBUG_ANY_WSP: Made KOPT length 9 to avoid 8 character debug option and filename conflict.
CMGS      CHARACTER MODNAM*12, KOPT*8

      CHARACTER MODNAM*12, KOPT*9
...
CCRT  D113 Add Sidewash source variable ISW and initialize below
     ...
     &           I_SV,IFC_SV, LOCB_SV(IFMAX), LOCE_SV(IFMAX),   !D144 WSP 10/19/22
     &           LINEDBG,!D144 WSP 11/4/22 used to track instances of LINE keyword. LINE is not a separate debug option 
     &           IARCFT,! Added for Aircraft; UNC-IE
     &           IHBP, ! Added for HBPDEBUG; JAN 2023
     &           ILEN_TMP, !used for storing length fo filename for manipulation
     &           IBAR,   ! add SBARRIER debug 20240716 WSP
     &           IEDG,   ! add BAREDGE debug 20240910 WSP
     &           IVEG    !VBARRIER debug 20250508 WSP
CMGS D086_Inconsistant_OPEN_DebugFiles_WSP(BEGIN) -- moved from aermod.f
...
CMGS D086_Inconsistant_OPEN_DebugFiles_WSP(END) -- moved from aermod.f

CMGS  D172_COSET_DEBUG_ANY_WSP (BEGIN) 
CMGS      To update debugs add CO DEBUGOPT name to this array! (5 per line!!!)
CMGS      NOTE: "LINE" is included, but not a valid option ("AREA" should be used).
CMGS            If user puts "LINE" there will be an error issued at setup (E194).
      CHARACTER(len=8),dimension(23) ::  DEBUGOPT_ARRAY
      DEBUGOPT_ARRAY = [character(len=8) :: 
     &                "MODEL","METEOR","AREA","LINE","RLINE",
     &                "PRIME","PVMRM","OLM","ARM2","GRSM",
     &                "DEPOS","AWMADW","TTRM","TTRM2","PLATFORM",
     &                "URBANDB","BLPDBUG","SWPOINT","AIRCRAFT","HBPDBG",
     &                "SBARRIER", "BAREDGE", "VBARRIER"] !RLINE Barrier debug opts
CMGS  D172_COSET_DEBUG_ANY_WSP (END)
...
C     associated with debugopts
      ...
      IBLP = 0
      ISW = 0
      FIELD_sv = '' !D144 WSP 10/21/22
      IFC_SV  = 0 !D144 WSP 10/21/22
      LOCB_SV = 0 !D144 WSP 10/21/22
      LOCE_SV = 0 !D144 WSP 10/21/22
      LINEDBG = 0 !D144 WSP 11/4/22
      IARCFT = 0                                 ! Added for Aircraft; UNC-IE
      IHBP = 0 ! Added for HBP, JAN 2023
      IBAR = 0 !add SBARRIER debug 20240716 WSP
      IEDG = 0 !add BAREDGE debug 20240910 WSP
      IVEG = 0 !VBARRIER debug 20250508 WSP
...
C     D144 Save the debug options associated with this keyword WSP 10/21/22
      DO I = 3, IFC
CMGS     KOPT = FIELD(I_SV)
         DEBUG_FLS(DEBUG_INDEX) = ADJUSTL(FIELD(I))
         DEBUG_FLS1(DEBUG_INDEX) = ADJUSTL(FIELD1(I)) ! CRT 3/6/2026 D144, preserve case
         DEBUG_INDEX = DEBUG_INDEX + 1
      END DO

C     D144 Added if statement to only perform data processing after all debugopt keywords have been processed WSP 10/19/22
      IF(CURRENT_DEBUGOPT .EQ. DEBUG_COUNT) THEN
C --- First Check for Presence of Debug Switches; save index of option KEYWORD, next windex is (optional) filename
C        D144 Save original FIELD value and replace FIELD with DEBUG_FLS WSP 10/21/22
CMGS         FIELD_sv = FIELD
         FIELD = DEBUG_FLS
         FIELD1 = DEBUG_FLS1 ! CRT 3/6/2026 D144, preserve case

C        D144 Save the original IFC value and replace IFC with the total number of debug options used WSP 10/25/22
CMGS             IFC_SV = IFC
CMGS             IFC = IFC_DEBUG- 2   !Include -2 since the IFC_DEBUG starts at the first index instead of the third index

C        D144 Save the original LOCB AND LOCE value and replacewith the start end of all debug options used WSP 10/25/22
         LOCB_SV = LOCB
         LOCB = LOCB_DEBUG
         LOCE_SV = LOCE
         LOCE = LOCE_DEBUG

C        D144 Changed initial I value to 1 since FIELD has been set to DEBUG_FLS WSP 10/21/22
         I = 1  !max would be MAXFields
         DO WHILE(FIELD(I) .NE. '') 
CMGS        DO I = 1, IFC
C           DO I = 3, IFC
            KOPT = FIELD(I) ! this will trunctate to 8 digits, but that is fine here.
            IF (KOPT .EQ. 'MODEL') THEN
C           D144 Check if MODEL debug keyword was repeated WSP 11/3/22
               IF(DEBUG) THEN
C                 WRITE Warning Message   ! Nonrepeatable keyword
                  CALL ERRHDL(PATH,MODNAM,'E','135',KOPT)
               END IF
               DEBUG = .TRUE.
               NOPTS = NOPTS + 1
               IMOD = I
            ELSE IF (KOPT .EQ. 'METEOR') THEN
C              D144 Check if METEOR debug keyword was repeated WSP 11/3/22
               IF(METEORDBG) THEN
C                 WRITE Warning Message   ! Nonrepeatable keyword
                  CALL ERRHDL(PATH,MODNAM,'E','135',KOPT)
               END IF
               ...
               NOPTS = NOPTS + 1
               IMET = I
C ---       D144 Added error check for AREA and LINE debug keywords WSP 11/4/22
...
C ---             AREADBG option not already = .T.; assign all variables
                  ...
                  NOPTS = NOPTS + 1
                  IAREA = I
C ---          D144 Added separate error messages for if AREA is repeated or if AREA and LINE are used together
               ELSE IF (AREADBG. AND. LINEDBG .GT. 0) THEN
C ---             D144 replaced error message with one specific to LINE and AREA debug files WSP 11/4/22
C ---             D144 changed error code to 738, as 734 was taken by updates in 2023. WSP 8/18/25
C ---             LINEDBG already assigned = .T.; user entered both AREA and LINE options;
C                 issue ERROR message
                  CALL ERRHDL(PATH,MODNAM,'E','738',KOPT)
               ELSE
C ---             AREADBG already assigned = .T.; user repeated AREA option;
C                 issue ERROR message D144 WSP 11/4/22
C                 WRITE Warning Message   ! Nonrepeatable keyword
                  CALL ERRHDL(PATH,MODNAM,'E','135',KOPT)
               END IF
C                 ELSE 
CC ---               AREADBG already assigned = .T.; user may have entered
CC                   both AREA and LINE options; issue ERROR message
CC                   CALL ERRHDL(PATH,MODNAM,'E','194','AREADEBUG')
C                    AREADBG = .FALSE.
C                 END IF
            ELSE IF (KOPT .EQ. 'LINE') THEN
C              D144 add count to local tracker variable to know which
C              error message to use WSP 11/4/22
               LINEDBG = LINEDBG + 1
C ---          Check to see if AREADBG option has already been assigned .T.;
C              user may have entered both AREA and LINE
               IF (.NOT. AREADBG) THEN
C ---             AREADBG option not already = .T.; assign all variables
                  ...
                  NOPTS = NOPTS + 1
                  IAREA = I
               ELSE IF (LINEDBG .GT. 1) THEN
C ---             LINEDBG greater than 1; user repeated LINE option;
C                 issue ERROR message D144 WSP 11/4/22
C                 WRITE Warning Message   ! Nonrepeatable keyword
                  CALL ERRHDL(PATH,MODNAM,'E','135',KOPT)
C ---             D144 Added separate error messages for if AREA is repeated or if AREA and LINE are used together
               ELSE IF (AREADBG .AND. LINEDBG .GT. 0) THEN
C ---             D144 repalced error message with one specific to LINE and AREA debug files WSP 11/4/22
C ---             D144 changed error code to 738, as 734 was taken by updates in 2023. WSP 8/18/25
C ---             AREADBG already assigned = .T.; user entered both AREA and LINE options;
C                 issue ERROR message
                  CALL ERRHDL(PATH,MODNAM,'E','738',KOPT)
...
cC ---               AREADBG already assigned = .T.; user may have entered
cC                   both AREA and LINE options; issue ERROR message
cC                   CALL ERRHDL(PATH,MODNAM,'E','194','LINEDEBUG')
c                    AREADBG = .FALSE.
c                 END IF
            ELSE IF (KOPT .EQ. 'PRIME') THEN
C              D144 Check if PRIME debug keyword was repeated WSP 11/3/22
               IF(PRIMEDBG) THEN
C                 WRITE Warning Message   ! Nonrepeatable keyword
                  CALL ERRHDL(PATH,MODNAM,'E','135',KOPT)
               END IF
               ...
               IPRM = I
            ELSE IF (KOPT .EQ. 'PVMRM') THEN
C              D144 Check if PVMRM debug keyword was repeated WSP 11/3/22
               IF(PVMRMDBG) THEN
C                 WRITE Warning Message   ! Nonrepeatable keyword
                  CALL ERRHDL(PATH,MODNAM,'E','135',KOPT)
               END IF
               ...
               IPVM = I
            ELSE IF (KOPT .EQ. 'OLM') THEN
C              D144 Check if OLM debug keyword was repeated WSP 11/3/22
               IF(OLMDEBUG) THEN
C                 WRITE Warning Message   ! Nonrepeatable keyword
                  CALL ERRHDL(PATH,MODNAM,'E','135',KOPT)
               END IF
               ...
               IOLMD = I
            ELSE IF (KOPT .EQ. 'ARM2') THEN
C              D144 Check if ARM2 debug keyword was repeated WSP 11/3/22
               IF(ARM2DEBUG) THEN
C                 WRITE Warning Message   ! Nonrepeatable keyword
                  CALL ERRHDL(PATH,MODNAM,'E','135',KOPT)
               END IF
               ...
               IARM2 = I
            ELSE IF (KOPT .EQ. 'GRSM') THEN
C              D144 Check if GRSM debug keyword was repeated WSP 11/3/22
               IF(GRSMDEBUG) THEN
C                 WRITE Warning Message   ! Nonrepeatable keyword
                  CALL ERRHDL(PATH,MODNAM,'E','135',KOPT)
               END IF
               ...
               IGRSM = I
            ELSE IF (KOPT .EQ. 'DEPOS') THEN
C              D144 Check if DEPOS debug keyword was repeated WSP 11/3/22
               IF(DEPOSDBG) THEN
C                 WRITE Warning Message   ! Nonrepeatable keyword
                  CALL ERRHDL(PATH,MODNAM,'E','135',KOPT)
               END IF
...
C              D144 Check if PLATFORM debug keyword was repeated WSP 11/3/22
               IF(PLATFMDBG) THEN
C                 WRITE Warning Message   ! Nonrepeatable keyword
                  CALL ERRHDL(PATH,MODNAM,'E','135',KOPT)
               END IF
...
C              D144 Check if AWMADW debug keyword was repeated WSP 11/3/22
               IF(AWMADWDBG) THEN
C                 WRITE Warning Message   ! Nonrepeatable keyword
                  CALL ERRHDL(PATH,MODNAM,'E','135',KOPT)
               END IF
...
C              D144 Check if RLINE debug keyword was repeated WSP 11/3/22
               IF(RLINEDBG) THEN
C                 WRITE Warning Message   ! Nonrepeatable keyword
                  CALL ERRHDL(PATH,MODNAM,'E','135',KOPT)
               END IF
               ...
               IRLINE = I
            ELSE IF (KOPT .EQ. 'TTRM') THEN
C              D144 Check if TTRM debug keyword was repeated WSP 11/3/22
               IF(TTRMDBG) THEN
C                 WRITE Warning Message   ! Nonrepeatable keyword
                  CALL ERRHDL(PATH,MODNAM,'E','135',KOPT)
               END IF
               ...
               ITTRMD = I
            ELSE IF (KOPT .EQ. 'TTRM2') THEN
C              D144 Check if TTRM2 debug keyword was repeated WSP 11/3/22
               IF(TTRM2DBG) THEN
C                 WRITE Warning Message   ! Nonrepeatable keyword
                  CALL ERRHDL(PATH,MODNAM,'E','135',KOPT)
               END IF
...
C              D144 Check if URBANDB debug keyword was repeated WSP 11/3/22
               IF(URBDBUG) THEN
C                 WRITE Warning Message   ! Nonrepeatable keyword
                  CALL ERRHDL(PATH,MODNAM,'E','135',KOPT)
               END IF
...
C              D144 Check if BLPDBUG debug keyword was repeated WSP 11/3/22
               IF(BLPDBUG) THEN
C                 WRITE Warning Message   ! Nonrepeatable keyword
                  CALL ERRHDL(PATH,MODNAM,'E','135',KOPT)
               END IF
...
C              D144 Check if SWPOINT debug keyword was repeated WSP 11/3/22
               IF(SWDBG) THEN
C                 WRITE Warning Message   ! Nonrepeatable keyword
                  CALL ERRHDL(PATH,MODNAM,'E','135',KOPT)
               END IF
...
C              D144 Check if AIRCRAFT debug keyword was repeated WSP 8/18/25
               IF(ARCFTDEBUG) THEN
C                 WRITE Warning Message   ! Nonrepeatable keyword
                  CALL ERRHDL(PATH,MODNAM,'E','135',KOPT)
               END IF
...
C              D144 Check if HBPDBG debug keyword was repeated WSP 8/18/25
               IF(HBPDBG) THEN
C                 WRITE Warning Message   ! Nonrepeatable keyword
                  CALL ERRHDL(PATH,MODNAM,'E','135',KOPT)
               END IF
               ...
               IHBP = I
! End HBP add
CRLM BEGIN adding SBARRIER debug 20240716 WSP
            ELSE IF (KOPT .EQ. 'SBARRIER') THEN
                BARDBG = .TRUE.
                NOPTS  = NOPTS + 1
                IBAR   = I
CRLM END adding SBARRIER debug 20240716 WSP
    
CRLM BEGIN adding BAREDGE debug 20240910 WSP
            ELSE IF (KOPT .EQ. 'BAREDGE') THEN
                EDGEDBG = .TRUE.
                NOPTS  = NOPTS + 1
                IEDG   = I
CRLM  END  adding BAREDGE debug 20240910 WSP
    
CRLM BEGIN adding VBARRIER debug 20250508 WSP
            ELSE IF (KOPT .EQ. 'VBARRIER') THEN
                VBRDBG = .TRUE.
                NOPTS  = NOPTS + 1
                IVEG   = I
CRLM  END  adding VBARRIER debug 20250508 WSP
            END IF
            I = I + 1   !increment I now, since this is in a while loop
         END DO
...
C           Write Error Message:  HBPDBG debug without HBP option
...
C** Added for Aircraft Plume Rise; UNC-IE
CMGS Changed conditional to be consistant with debug conditionals above.
CMSG (D151 - WSP 6/1/2023)
CMGS      IF (ARCFTDEBUG .AND. .NOT. NAFTSRC .GT. 0.0D0) THEN
         IF (ARCFTDEBUG .AND. NAFTSRC .EQ. 0) THEN
C           Write Error Message:  ARCFT debug without Aircraft option
            CALL ERRHDL(PATH,MODNAM,'E','194','ARCFTDEBUG')
         END IF
C** End Aircraft Plume Rise insert; April 2023

CRLM BEGIN adding SBARRIER debug 20240716 WSP
      IF (BARDBG .AND. NSBR.EQ.0) THEN
C        Write Error Message:  SBARRIER debug without barrier inputs
         CALL ERRHDL(PATH,MODNAM,'E','194','BARDBG')
      END IF
CRLM END adding SBARRIER debug 20240716 WSP

CRLM BEGIN adding BAREDGE debug 20240910 WSP
    !invalid debug if BAREDGE modelopt not used
      IF (EDGEDBG .AND. .NOT.BAREDGE) THEN
C        Write Error Message:  BAREDGE debug without BAREDGE modelopt
         CALL ERRHDL(PATH,MODNAM,'E','194','BAREDGE')
      END IF
CRLM  END adding BAREDGE debug 20240910 WSP

CRLM BEGIN adding VBARRIER debug 20250508 WSP
C      IF (VBRDBG .AND. NVBR.EQ.0) THEN
CC        Write Error Message:  SBARRIER debug without barrier inputs
C         CALL ERRHDL(PATH,MODNAM,'E','194','VBRDBG')
C      END IF
CRLM END adding VBARRIER debug 20250508 WSP
...
C     follow the keyword option in the input file
         IF (DEBUG) THEN
CMGS  D172_COSET_DEBUG_ANY_WSP (BEGIN) 
CMGS          IF(IFC .GE. IMOD+1 .AND. 
            IF((FIELD(IMOD+1) .NE. '') .AND. 
     &      (.NOT. ANY(FIELD(IMOD+1) == DEBUGOPT_ARRAY))) THEN
CMGS  D172_COSET_DEBUG_ANY_WSP (END)

C ---       Assign user-specified filename for the MODEL debug option
CCRT 3/6/2026, D144 Debug Options Limit - preserve filename case 
CMGS            DBGFIL = RUNST1(LOCB(IMOD+1):LOCE(IMOD+1))
               DBGFIL = FIELD1(IMOD+1)
            ELSE
C ---          Assign default MODEL debug filename
         ...
         IF (METEORDBG) THEN
CMGS  D172_COSET_DEBUG_ANY_WSP (BEGIN) 
CMGS          IF(IFC .GE. IMET+1 .AND. 
            IF((FIELD(IMET+1) .NE. '') .AND.
     &         (.NOT. ANY(FIELD(IMET+1) == DEBUGOPT_ARRAY))) THEN
CMGS  D172_COSET_DEBUG_ANY_WSP (END)

C ---       Assign user-specified filename for the METEOR debug option
CCRT 3/6/2026, D144 Debug Options Limit - preserve filename case 
CMGS            DBMFIL = RUNST1(LOCB(IMET+1):LOCE(IMET+1))
               DBMFIL = FIELD1(IMET+1)
            ELSE
C ---          Assign default METEOR debug filename
         ...
         IF (AREADBG) THEN
CMGS  D172_COSET_DEBUG_ANY_WSP (BEGIN) 
CMGS          IF(IFC .GE. IAREA+1 .AND. 
            IF((FIELD(IAREA+1) .NE. '') .AND.
     &         (.NOT. ANY(FIELD(IAREA+1) == DEBUGOPT_ARRAY))) THEN
CMGS  D172_COSET_DEBUG_ANY_WSP (END)

C ---          Assign user-specified filename for the AREA debug option
CCRT 3/6/2026, D144 Debug Options Limit - preserve filename case 
CMGS            DBAREAFIL = RUNST1(LOCB(IAREA+1):LOCE(IAREA+1))
               DBAREAFIL = FIELD1(IAREA+1)
            ELSE
C ---          Assign default AREA debug filename
         ...
         IF (PRIMEDBG) THEN
CMGS  D172_COSET_DEBUG_ANY_WSP (BEGIN) 
CMGS          IF(IFC .GE. IPRM+1 .AND. 
            IF((FIELD(IPRM+1) .NE. '') .AND. 
     &         (.NOT. ANY(FIELD(IPRM+1) == DEBUGOPT_ARRAY))) THEN
CMGS  D172_COSET_DEBUG_ANY_WSP (END)

C ---          Assign user-specified filename for the PRIME debug option
CCRT 3/6/2026, D144 Debug Options Limit - preserve filename case 
CMGS            DBPRMFIL = RUNST1(LOCB(IPRM+1):LOCE(IPRM+1))
               DBPRMFIL = FIELD1(IPRM+1)
            ELSE
C ---          Assign default PRIME debug filename
         ...
         IF (PVMRMDBG) THEN
CMGS  D172_COSET_DEBUG_ANY_WSP (BEGIN) 
CMGS          IF(IFC .GE. IPVM+1 .AND. 
            IF((FIELD(IPVM+1) .NE. '') .AND.
     &          (.NOT. ANY(FIELD(IPVM+1) == DEBUGOPT_ARRAY))) THEN
CMGS  D172_COSET_DEBUG_ANY_WSP (END)

C ---       Assign user-specified filename for the PVMRM debug option
CCRT 3/6/2026, D144 Debug Options Limit - preserve filename case 
CMGS            DBPVFIL = RUNST1(LOCB(IPVM+1):LOCE(IPVM+1))
            DBPVFIL = FIELD1(IPVM+1)
            ELSE
...
C ---       Assign default filename for RELDISP debug file for PVMRM option
         ...
         IF (OLMDEBUG) THEN
CMGS  D172_COSET_DEBUG_ANY_WSP (BEGIN) 
CMGS          IF(IFC .GE. IOLM+1 .AND. 
CCRT  D172 2/4/2026 - Replace IOLM w/ IOLMD (local variable)
CCRT        Without this replacement, default filename will be used
            IF((FIELD(IOLMD+1) .NE. '') .AND.
     &         (.NOT. ANY(FIELD(IOLMD+1) == DEBUGOPT_ARRAY))) THEN
CMGS  D172_COSET_DEBUG_ANY_WSP (END)

C ---          Assign user-specified filename for the OLM debug option
CCRT 3/6/2026, D144 Debug Options Limit - preserve filename case 
CMGS            DBOLMFIL = RUNST1(LOCB(IOLMD+1):LOCE(IOLMD+1))
               DBOLMFIL = FIELD1(IOLMD+1)
            ELSE
C ---          Assign default OLM debug filename
         ...
         IF (ARM2DEBUG) THEN
CMGS D172_COSET_DEBUG_ANY_WSP (BEGIN) 
CMGS          IF(IFC .GE. IARM2+1 .AND. 
            IF((FIELD(IARM2+1).NE.'').AND.
     &         (.NOT. ANY(FIELD(IARM2+1) == DEBUGOPT_ARRAY))) THEN
CMGS  D172_COSET_DEBUG_ANY_WSP (END)

C ---          Assign user-specified filename for the ARM2 debug option
CCRT 3/6/2026, D144 Debug Options Limit - preserve filename case 
CMGS            DBARM2FIL = RUNST1(LOCB(IARM2+1):LOCE(IARM2+1))
               DBARM2FIL = FIELD1(IARM2+1)
            ELSE
...
C        CERC 11/30/20 Code for determining GRSM debug file name
         IF (GRSMDEBUG) THEN
CMGS     D172_COSET_DEBUG_ANY_WSP (BEGIN) 
CMGS          IF(IFC .GE. IGRSM+1 .AND. 
            IF((FIELD(IGRSM+1).NE.'') .AND.
     &        (.NOT. ANY(FIELD(IGRSM+1) == DEBUGOPT_ARRAY))) THEN
CMGS  D172_COSET_DEBUG_ANY_WSP (END)

C ---          Assign user-specified filename for the GRSM debug option
CCRT 3/6/2026, D144 Debug Options Limit - preserve filename case 
CMGS            DBGRSMFIL = RUNST1(LOCB(IGRSM+1):LOCE(IGRSM+1))
               DBGRSMFIL = FIELD1(IGRSM+1)
            ELSE
C ---          Assign default GRSM debug filename
         ...
         IF (AWMADWDBG) THEN
CMGS     D172_COSET_DEBUG_ANY_WSP (BEGIN) 
CMGS          IF(IFC .GE. IPRM2+1 .AND. 
            IF((FIELD(IPRM2+1) .NE. '') .AND.
     &         (.NOT. ANY(FIELD(IPRM2+1) == DEBUGOPT_ARRAY))) THEN
CMGS  D172_COSET_DEBUG_ANY_WSP (END)

C ---          Assign user-specified filename for the PRIME debug option
CCRT 3/6/2026, D144 Debug Options Limit - preserve filename case 
CMGS            DBAwmaDwFIL = RUNST1(LOCB(IPRM2+1):LOCE(IPRM2+1))
               DBAwmaDwFIL = trim(FIELD1(IPRM2+1))
            ELSE
C ---          Assign default AWMADW debug filename
         ...
         IF (RLINEDBG) THEN
CMGS     D172_COSET_DEBUG_ANY_WSP (BEGIN) 
CMGS          IF(IFC .GE. IRLINE+1 .AND. 
            IF((FIELD(IRLINE+1) .NE. '') .AND.
     &         (.NOT. ANY(FIELD(IRLINE+1) == DEBUGOPT_ARRAY))) THEN
CMGS           D172_COSET_DEBUG_ANY_WSP (END)

C ---          Assign user-specified filename for the RLINE debug option
CCRT 3/6/2026, D144 Debug Options Limit - preserve filename case 
CMGS            RLINEDBGFIL = RUNST1(LOCB(IRLINE+1):LOCE(IRLINE+1))
               RLINEDBGFIL = FIELD1(IRLINE+1)
C ---          Add filename for the RLINE gridded wind speed debug file Wood 10/10/22
CMGS            RLINEDBGFIL_WS = RUNST1(LOCB(IRLINE+1):LOCE(IRLINE+1)-4)//
CMGS     &                       "_GRIDWS"//
CMGS     &                       RUNST1(LOCE(IRLINE+1)-3:LOCE(IRLINE+1))
               ILEN_TMP = LEN(TRIM(FIELD(IRLINE+1)))
               RLINEDBGFIL_WS = FIELD1(IRLINE+1)(1:ILEN_TMP-4)//
     &                       "_GRIDWS"//
     &                       FIELD1(IRLINE+1)(ILEN_TMP-3:ILEN_TMP)
            ELSE
C ---          Assign default RLINE debug filename
               RLINEDBGFIL = 'RLINE.DBG'
C ---          Add filename for the RLINE gridded wind speed debug file Wood 10/10/22
C ---          Assign default RLINE Grid WS debug filename
...
CCRT     D063 Platform Downwash Debug
         IF (PLATFMDBG) THEN
CMGS        D172_COSET_DEBUG_ANY_WSP (BEGIN) 
CMGS          IF(IFC .GE. IPLATFM+1 .AND. 
            IF((FIELD(IPLATFM+1) .NE. '') .AND.
     &         (.NOT. ANY(FIELD(IPLATFM+1) == DEBUGOPT_ARRAY))) THEN
CMGS  D172_COSET_DEBUG_ANY_WSP (END)

C ---          Assign user-specified filename for the PRIME debug option
CCRT 3/6/2026, D144 Debug Options Limit - preserve filename case 
CMGS            PLATFMDBGFILE = RUNST1(LOCB(IPLATFM+1):LOCE(IPLATFM+1))
               PLATFMDBGFILE = FIELD1(IPLATFM+1)
            ELSE
...
CMGS         IF (TRIM(ADJUSTL(DBGFIL)) .EQ. '' .OR. FIELD(IDEP+1) .NE.
CMGS     &   'MODEL') DBGFIL='DEPOS.DBG'
            IF (TRIM(ADJUSTL(DBGFIL)) .EQ. '' .OR. FIELD(IDEP+1) .NE.
     &         'MODEL') DBGFIL='DEPOS.DBG'
CMGS  D172_COSET_DEBUG_ANY_WSP (BEGIN) 
CMGS          IF(IFC .GE. IDEP+1 .AND. 
            IF((FIELD(IDEP+1) .NE. '') .AND. 
     &        (.NOT. ANY(FIELD(IDEP+1) == DEBUGOPT_ARRAY))) THEN
CMGS  D172_COSET_DEBUG_ANY_WSP (END)
...
!        Added for TTRM; AECOM
         IF (TTRMDBG) THEN
CMGS     D172_COSET_DEBUG_ANY_WSP (BEGIN) 
CMGS          IF(IFC .GE. ITTRMD+1 .AND. 
            IF((FIELD(ITTRMD+1) .NE. '') .AND. 
     &         (.NOT. ANY(FIELD(ITTRMD+1) == DEBUGOPT_ARRAY))) THEN
CMGS  D172_COSET_DEBUG_ANY_WSP (END)

!              added for TTRM; AECOM
C ---          Assign user-specified filename for the TTRM debug option
CCRT 3/6/2026, D144 Debug Options Limit - preserve filename case 
CMGS            TTRMFIL = RUNST1(LOCB(ITTRMD+1):LOCE(ITTRMD+1))
               TTRMFIL = FIELD1(ITTRMD+1)
            ELSE
...
CRCO D095 Added for urban debug 8/3/2021
         IF (URBDBUG) THEN
CMGS     D172_COSET_DEBUG_ANY_WSP (BEGIN) 
CMGS          IF(IFC .GE. IURBD+1 .AND. 
            IF((FIELD(IURBD+1) .NE. '') .AND.
     &         (.NOT. ANY(FIELD(IURBD+1) == DEBUGOPT_ARRAY))) THEN
CMGS  D172_COSET_DEBUG_ANY_WSP (END)

C ---          Assign user-specified filename for the URBDBUG debug option
CCRT 3/6/2026, D144 Debug Options Limit - preserve filename case 
CMGS            URBFIL = RUNST1(LOCB(IURBD+1):LOCE(IURBD+1))
               URBFIL = FIELD1(IURBD+1)
CMGS            URBFIL1 = RUNST1(LOCB(IURBD+1):LOCE(IURBD+1)) // "1"
               ILEN_TMP = LEN(TRIM(FIELD(IURBD+1)))
               URBFIL1 = FIELD1(IURBD+1)(1:ILEN_TMP-4)//
     &                       "1"//
     &                       FIELD1(IURBD+1)(ILEN_TMP-3:ILEN_TMP)
!RCO - D168 Debug files. Add output file for second urban debug
CCRT 3/6/2026, D144 Debug Options Limit - preserve filename case 
CMGS            URBFIL2 = RUNST1(LOCB(IURBD+1):LOCE(IURBD+1)) // "2"
               URBFIL2 = FIELD1(IURBD+1)(1:ILEN_TMP-4)//
     &                       "2"//
     &                       FIELD1(IURBD+1)(ILEN_TMP-3:ILEN_TMP)
            ELSE
...
!        CRCO D095 BLP Debug
         IF (BLPDBUG) THEN
CMGS     D172_COSET_DEBUG_ANY_WSP (BEGIN) 
CMGS          IF(IFC .GE. IBLP+1 .AND. 
            IF((FIELD(IBLP+1) .NE. '') .AND.
     &         (.NOT. ANY(FIELD(IBLP+1) == DEBUGOPT_ARRAY))) THEN
CMGS  D172_COSET_DEBUG_ANY_WSP (END)

C ---          Assign user-specified filename for the DISTANCE debug option
CCRT 3/6/2026, D144 Debug Options Limit - preserve filename case 
CMGS            BLPFIL = RUNST1(LOCB(IBLP+1):LOCE(IBLP+1))
               BLPFIL = FIELD1(IBLP+1)
            ELSE
...
C        CRT, 3/25/2022 D113 Updated for SIDEWASH
         IF (SWDBG) THEN
CMGS        D172_COSET_DEBUG_ANY_WSP (BEGIN) 
CMGS          IF(IFC .GE. ISW+1 .AND. 
            IF((FIELD(ISW+1) .NE. '').AND.
     &         (.NOT. ANY(FIELD(ISW+1) == DEBUGOPT_ARRAY))) THEN
CMGS  D172_COSET_DEBUG_ANY_WSP (END)

C ---          Assign user-specified filename for the SIDEWASH debug option
CCRT 3/6/2026, D144 Debug Options Limit - preserve filename case 
CMGS            SWFIL = RUNST1(LOCB(ISW+1):LOCE(ISW+1))
               SWFIL = FIELD1(ISW+1)
            ELSE
...
C**      Added for Aircraft Plume Rise; UNC-IE
         IF (ARCFTDEBUG) THEN
CMGS        D172_COSET_DEBUG_ANY_WSP (BEGIN) 
CMGS          IF(IFC .GE. IARCFT+1 .AND. 
            IF((FIELD(IARCFT+1) .NE. '') .AND. 
     &         (.NOT. ANY(FIELD(IARCFT+1) == DEBUGOPT_ARRAY))) THEN
CMGS           D172_COSET_DEBUG_ANY_WSP (END)

C ---          Assign user-specified filename for the AIRCRAFT debug option
CCRT 3/6/2026, D144 Debug Options Limit - preserve filename case 
CMGS            DBARCFTFIL = RUNST1(LOCB(IARCFT+1):LOCE(IARCFT+1))
               DBARCFTFIL = FIELD1(IARCFT+1)
            ELSE
C ---          Assign default AIRCRAFT debug filename
...
! Added for HBP DEBUG, JAN 2023
         IF (HBPDBG) THEN
CMGS        D172_COSET_DEBUG_ANY_WSP (BEGIN) 
CMGS          IF(IFC .GE. IHBP+1 .AND. 
            IF((FIELD(IHBP+1) .NE. '') .AND. 
     &         (.NOT. ANY(FIELD(IHBP+1) == DEBUGOPT_ARRAY))) THEN
CMGS  D172_COSET_DEBUG_ANY_WSP (END)

C ---          Assign user-specified filename for the HBPBUG debug option
CCRT 3/6/2026, D144 Debug Options Limit - preserve filename case 
CMGS            HBPFIL = RUNST1(LOCB(IHBP+1):LOCE(IHBP+1))
               HBPFIL = FIELD1(IHBP+1)
            ELSE
C ---          Assign default HBPDBG debug filename
               HBPFIL = 'HBP_DEBUG.DBG'
            END IF
         END IF
! End HBP DEBUG, JAN 2023

C RLM BEGIN adding SBARRIER debug 20240716 WSP
         IF (BARDBG) THEN
            IF((FIELD(IBAR+1) .NE. '') .AND. 
     &         (.NOT. ANY(FIELD(IBAR+1) == DEBUGOPT_ARRAY))) THEN
C ---       Assign user-specified filename for the MODEL debug option
               SBARDBGFIL = FIELD1(IBAR+1)
          ELSE
C ---       Assign default MODEL debug filename
               SBARDBGFIL = 'SBARRIER.DBG'
          END IF
          END IF
C RLM END adding SBARRIER debug 20240716 WSP

C RLM BEGIN adding BAREDGE debug 20240910 WSP
         IF (EDGEDBG) THEN
            IF((FIELD(IEDG+1) .NE. '') .AND. 
     &         (.NOT. ANY(FIELD(IEDG+1) == DEBUGOPT_ARRAY))) THEN
C ---       Assign user-specified filename for the MODEL debug option
             EDGEDBGFIL = FIELD1(IEDG+1)
            ELSE
C ---       Assign default MODEL debug filename
             EDGEDBGFIL = 'BAREDGE.DBG'
            END IF
          END IF
C RLM END adding BAREDGE debug 20240910 WSP

C RLM BEGIN adding VBARRIER debug 20250508 WSP
         IF (VBRDBG) THEN
            IF((FIELD(IVEG+1) .NE. '') .AND. 
     &         (.NOT. ANY(FIELD(IVEG+1) == DEBUGOPT_ARRAY))) THEN       
C ---       Assign user-specified filename for the MODEL debug option
             VBARDBGFIL = FIELD1(IVEG+1)
            ELSE
C ---       Assign default MODEL debug filename
             VBARDBGFIL = 'VBARRIER.DBG'
            END IF
          END IF
C RLM END adding VBARRIER debug 20250508 WSP

CMGS D086_Inconsistant_OPEN_DebugFiles_WSP & D134_UniformDebugHeaders
CMGS Moved all debug files to open and write uniform headers in subroutine at end 
CMGS   of coset.f; called from aermod.f after SETUP.

C        Set FIELD back to original value D144 WSP 10/21/22
         FIELD = FIELD_sv

C        Set IFC valueack to original value D144 WSP 10/25/22
         IFC = IFC_SV

C        Set  LOCB AND LOCE to original value D144 WSP 10/25/22
         LOCB = LOCB_SV
         LOCE = LOCE_SV
...
C --- D144 Moved all data processing into the IF Statement to allow for a repeatable debugopt keyword WSP 10/20/22

 999  RETURN
      END
      
CMGS D086_Inconsistant_OPEN_DebugFiles_WSP (BEGIN)
      SUBROUTINE OPEN_DEBUGFILE(FUNIT,FNAME,NAM,DBGOPTION,DBGOPT,FTYP)
C************************************************************************
C               OPEN_DEBUGFILE Subroutine of the AMS/EPA Regulatory Model - AERMOD
C
C        PURPOSE: Consistantly open debug Files
C*C        PROGRAMMER: Roger Brode, Jeff Wang
C
C        DATE:    March 13, 2025
C
C        MODIFIED: 
C
C        INPUTS:  
C              FUNIT - File unit of debug file
C              FNAME - File name of debug file
C              NAM - calling routine
C              DBGOPTION - debug option used (can be longer that 12 characters)
C              DBGOPT - CO DEBUGOPT name used
C
C        OUTPUTS: NONE
C
C        ERROR HANDLING:   Checks if file is open already (if so use error #501)
C                          Checks if unable to open (if so use error #500)
C
C        CALLED FROM:   COCARD/DBGOPT and aermod.f/AERMOD
C***********************************************************************
...
C     Variable Declarations
      ...
      CHARACTER(LEN=*), INTENT(IN)  ::DBGOPTION
      CHARACTER(LEN=*), INTENT(IN)  ::DBGOPT
      CHARACTER(LEN=*), INTENT(IN)  ::FTYP
      LOGICAL    :: L_OPENED

C     Variable Initializations
C     Make the MONDAM a combination of OPEN_DEBUGFILE and calling routine for debugging.
      WRITE(MODNAM,'(A6,A6)') 'OPDBG:', TRIM(NAM)

      L_OPENED = .FALSE.
      INQUIRE (FILE=FNAME,OPENED=L_OPENED)
      IF (.NOT.L_OPENED) THEN
          OPEN(UNIT=FUNIT,FILE=FNAME,ERR=99,STATUS='REPLACE')
          FUNITARRAY(FUNITIND) = FUNIT
          FNAMEARRAY(FUNITIND) = FNAME
          FTYPARRAY(FUNITIND) = FTYP
          FUNITIND = FUNITIND + 1
          GO TO 1000
      ELSE
C ---    File is already opened, issue error message
         CALL ERRHDL(PATH,MODNAM,'E','501',DBGOPTION)
      END IF

 99   CALL ERRHDL(PATH,MODNAM,'E','500',DBGOPTION)
 1000 CONTINUE

CMGS: D134_Debug_File_Normalization_WSP (BEGIN)
CMGS:    Add standardized DEBUG headers to all files!
      WRITE(FUNIT,9001) VERSN, TITLE1(1:68), RUNDAT

      IF( EVONLY )THEN
C ---     Write header information for EVENT results
          WRITE(FUNIT,9002) C_METVER,
     &     '   EVENT Processing', RUNTIM
         ELSE
C ---       Write header information for Non-EVENT results
          WRITE(FUNIT,9002) C_METVER,
     &      'NonEVENT Processing', RUNTIM
      ENDIF

      WRITE(FUNIT, 9003)  MODOPS_String(1:LEN_TRIM(MODOPS_String))
      WRITE(FUNIT, 9004)  DBGOPT, TRIM(FNAME)

9001   FORMAT('*** AERMOD - VERSION ',A6,' ***',3X,'*** ',A68,
     &                                ' ***',8X,A8)
9002   FORMAT('*** AERMET - VERSION ',A6,' ***',3X,'*** ',
     &                             A19,49X,' ***',8X,A8/)
9003   FORMAT ( ' OPTIONS: ', A /)
9004   FORMAT('************ CO DEBUGOPT: ', 3X, A12,3X,
     &                            '(Filename: ',A,')','************'/)
CMGS: D134_Debug_File_Normalization_WSP (END)

      RETURN
      END 

CMGS D086_Inconsistant_OPEN_DebugFiles_WSP (END)

CMGS: D134_Debug_File_Normalization_WSP (BEGIN)
      SUBROUTINE OPEN_ALL_DEBUGFILES
C************************************************************************
C               OPEN_ALL_DEBUGFILES Subroutine of the AMS/EPA Regulatory Model - AERMOD
C
C        PURPOSE: Consistantly open ALL debug Files and write option specific headers
C*C        PROGRAMMER: Roger Brode, Jeff Wang
C
C        DATE:    June 23, 2025
C
...
C
C        CALLED FROM:   aermod.f/AERMOD after SETUP calls coset.f/DBGOPT
C***********************************************************************

C     Variable Declarations
      USE MAIN1
      USE RLINE_DATA, ONLY: VEGBARR, REEDGE
      IMPLICIT NONE
      CHARACTER MODNAM*12
      INTEGER  I, IR, IS
CMGS ---- Rearranged and stremlined
CMGS Debug options go in order of DEBUGOPT_ARRAY in coset.f/DEBOPT
CMGS As of 6/20/25:       DEBUGOPT_ARRAY = [character(len=8) :: 
CMGS      &                "MODEL","METEOR","AREA","LINE","RLINE",
CMGS      &                "PRIME","PVMRM","OLM","ARM2","GRSM",
CMGS      &                "DEPOS","AWMADW","TTRM","TTRM2","PLATFORM",
CMGS      &                "URBANDB","BLPDBUG","SWPOINT","AIRCRAFT","HBPDBG",
CRLM      &                "SBARRIER", "BAREDGE", "VBARRIER"]

C --- Declare character strings for use in the header records of
C     debug files for NO2 options
      CHARACTER (LEN=50) :: NO2DebugString
      IF( .NOT.EVONLY )THEN
C ---     Header record with column labels for Non-EVENT results
        NO2DebugString = 
     &       'Hourly non-EVENT results sorted by hour/rec/srcgrp/src'
      ELSE
        NO2DebugString = 
     &   'Hourly & Average EVENT results sorted by day/event/grp/src'
      END IF
201          FORMAT ( '******* NO2 ', A50 ,'*******'/)

CMGS D134_UniformDebugHeaders: DEBUGOPT_ARRAY[1] = "MODEL"
      IF (DEBUG .OR. DEPOSDBG) THEN
C        Open debug output file
         DUMMY = 'DebugFile'
         CALL OPEN_DEBUGFILE(DBGUNT,DBGFIL,MODNAM,DUMMY,'MODEL',
     &                       'DBGUNT')
      END IF

CMGS D134_UniformDebugHeaders: DEBUGOPT_ARRAY[2] = "METEOR"
      IF (METEORDBG) THEN
C        Open debug meteorology output file
         DUMMY = 'DbgMetFile'
         CALL OPEN_DEBUGFILE(DBMUNT,DBMFIL,MODNAM,DUMMY,'METEOR',
     &                       'Meteorology debug file')
      END IF

CMGS D134_UniformDebugHeaders: DEBUGOPT_ARRAY[3-4] = "AREA","LINE"
      IF (AREADBG .AND. (NAREA.GT.0 .OR. NCIRC.GT.0 .OR.
     &                   NLINE.GT.0 .OR. NPIT.GT.0)) THEN
C        Open debug AREA output file
         DUMMY = 'AreaDbgFile'
         CALL OPEN_DEBUGFILE(AREADBUNT,DBAREAFIL,MODNAM,DUMMY,'AREA',
     &                       'AREADBUNT')
      END IF

CMGS D134_UniformDebugHeaders: DEBUGOPT_ARRAY[5] = "RLINE"
      IF (RLINEDBG) THEN
C        Open RLINE DEBUG output file
         DUMMY = 'RLINEFIL'
         CALL OPEN_DEBUGFILE(RLINEDBUNT,RLINEDBGFIL,MODNAM,DUMMY, 
     &                       'RLINE','RLINEDBUNT')
C        Open second RLINE DEBUG output file for gridded wind speed Wood 10/10/22
         DUMMY = 'RLINEFIL_WS'
         CALL OPEN_DEBUGFILE(RLINEDBUNT_WS,RLINEDBGFIL_WS,MODNAM,DUMMY,
     &                       'RLINE','RLINEDBUNT_WS')
C ------ Write specific header to define columns
         WRITE(RLINEDBUNT_WS,2234)
2234    FORMAT (3X,'YR',2X,'MO',2X,'DY',2X,'HR',3X,'HT_m',5x,'WS_m_s')
      END IF

CMGS D134_UniformDebugHeaders: DEBUGOPT_ARRAY[6] = "PRIME"
      IF (PRIMEDBG .AND. NSEC .GT. 0) THEN
C        Open debug PRIME output file
         DUMMY = 'PrimeDbgFile'
         CALL OPEN_DEBUGFILE(PRMDBUNT,DBPRMFIL,MODNAM,DUMMY,'PRIME',
     &                       'PRMDBUNT')
      END IF
      

CMGS D134_UniformDebugHeaders: DEBUGOPT_ARRAY[7] = "PVMRM"
C --- Open file with PVMRM 
      IF (PVMRMDBG) THEN
         DUMMY = 'PVMRMFIL'
         CALL OPEN_DEBUGFILE(PVMDBG,DBPVFIL,MODNAM,DUMMY,'PVMRM',
     &                       'PVMDBG')
         WRITE(PVMDBG,201) NO2DebugString
C ------ Write specific header to define columns
         IF( .NOT.EVONLY )THEN
          WRITE(PVMDBG,9001)
9001          FORMAT(1x,'TYPE    DOM_SRCID     DATE    IREC    GRPID',
     ...
     &                'HRVAL         AVEVAL')
         ELSE
          WRITE(PVMDBG,90011) !this 90011 format only used here
...
C     the RELDISP debug file will automatically be named RELDISP.DBG
         DUMMY = 'RELDISPF'
         CALL OPEN_DEBUGFILE(RDISPUNT,RDISPFIL,MODNAM,DUMMY,'PVMRM',
     &                       'RDISPUNT')
      END IF

CMGS D134_UniformDebugHeaders: DEBUGOPT_ARRAY[8] = "OLM"
C --- Open file with OLM debugging output
      IF (OLMDEBUG) THEN
        DUMMY = 'OLMFIL'
        CALL OPEN_DEBUGFILE(OLMDBG,DBOLMFIL,MODNAM,DUMMY,'OLM',
     &                      'OLMDBG')
        WRITE(OLMDBG,201) NO2DebugString
C ------ Write specific header to define columns
        IF( .NOT.EVONLY )THEN
...
CMGS D134_UniformDebugHeaders: DEBUGOPT_ARRAY[9] = "ARM2"
C --- Open file with ARM2 debugging output
      IF (ARM2DEBUG) THEN
        DUMMY = 'ARM2FIL'
        CALL OPEN_DEBUGFILE(ARM2DBG,DBARM2FIL,MODNAM,DUMMY,'ARM2',
     &                      'ARM2DBG')
        WRITE(ARM2DBG,201) NO2DebugString
        IF( .NOT.EVONLY )THEN
C ---     Write header record with column labels for Non-EVENT results
          WRITE(ARM2DBG,9004)
...
CMGS D134_UniformDebugHeaders: DEBUGOPT_ARRAY[10] = "GRSM"
C     CERC 11/30/20
C --- Open file with GRSM debugging output
      IF (GRSMDEBUG) THEN
        DUMMY = 'GRSMFIL'
        CALL OPEN_DEBUGFILE(GRSMDBG,DBGRSMFIL,MODNAM,DUMMY,'GRSM',
     &                      'GRSMDBG')
        WRITE(GRSMDBG,201) NO2DebugString
C ---       Write header record with column labels
        IF( .NOT.EVONLY )THEN
            WRITE(GRSMDBG,90023)
...
CMGS D134_UniformDebugHeaders: DEBUGOPT_ARRAY[11] = "DEPOS"
C     Open file for GDEP output from gas dry deposition algorithms
      IF ((DEBUG .OR. DEPOSDBG) .AND. LDGAS) THEN
         DUMMY = 'GDEPFILE'
         CALL OPEN_DEBUGFILE(GDEPDBG,'GDEP.DAT',MODNAM,DUMMY,'DEPOS',
     &                       'GDEPDBG')
         WRITE(GDEPDBG,9901)
...
C     Open file for PDEP output from particle dry deposition algorithms
      IF ((DEBUG .OR. DEPOSDBG) .AND. LDPART) THEN
         DUMMY = 'PDEPFILE'
         CALL OPEN_DEBUGFILE(PDEPDBG,'PDEP.DAT',MODNAM,DUMMY,'DEPOS',
     &                       'PDEPDBG')
         WRITE(PDEPDBG,9902)
...
CMGS D134_UniformDebugHeaders: DEBUGOPT_ARRAY[12] = "AWMADW"
      IF (AWMADWDBG) THEN
C        Open debug AWMADW output file
         DUMMY = 'AwmaDwDbgFile'
         CALL OPEN_DEBUGFILE(AwmaDwDBUNT,DBAwmaDwFIL,MODNAM,
     &                       DUMMY,'AWMADW','AwmaDwDBUNT')
      END IF

CMGS D134_UniformDebugHeaders: DEBUGOPT_ARRAY[13] = "TTRM"
      IF (TTRMDBG) THEN
!        Open TTRM output file
         DUMMY = 'TTRMFIL'
         CALL OPEN_DEBUGFILE(TTRMUNT,TTRMFIL,MODNAM,DUMMY,'TTRM',
     &                       'TTRMUNT')
!     Added for TTRM, AECOM - Feb. 2021; Modified Nov. 2021
         IF( .NOT.EVONLY )THEN
            WRITE ( TTRMUNT, 6010 )
 6010     FORMAT (' KURDAT,XREC,YREC,SRCGRP,SRCID,O3CONC (UG/M3),',
...
CMGS D134_UniformDebugHeaders: DEBUGOPT_ARRAY[14] = "TTRM2"

CMGS D134_UniformDebugHeaders: DEBUGOPT_ARRAY[15] = "PLATFORM"
CCRT  D063 Platform Downwash Debug
      IF (PLATFMDBG) THEN
C        Open debug PLATFORM output file
         DUMMY = 'PLATFMDBGFILE'
         CALL OPEN_DEBUGFILE(PLATFMDBUNT,PLATFMDBGFILE,MODNAM,
     &                       DUMMY,'PLATFORM','PLATFMDBUNT')
      END IF

CMGS D134_UniformDebugHeaders: DEBUGOPT_ARRAY[16] = "URBANDB"
CRCO D095 Added for urban debug 8/3/2021
! -- Open URBDBUG debug file, if selected;
      IF (URBDBUG) THEN
!        Open URBDBUG output file
         DUMMY = 'URBFIL'
         CALL OPEN_DEBUGFILE(URBUNT,URBFIL,MODNAM,DUMMY,'URBANDB',
     &                       'URBUNT')
C        Write the column headings to the debug output file
         WRITE( URBUNT, 2222 )
...
C Profile info for turbulence values
         DUMMY = 'URBFIL1'
         CALL OPEN_DEBUGFILE(URBUNT1,URBFIL1,MODNAM,DUMMY,'URBANDB',
     &                       'URBUNT1')
...
CMGS D134_UniformDebugHeaders: DEBUGOPT_ARRAY[17] = "BLPDBUG"
!CRCO D095 BLP debug file
      IF (BLPDBUG) THEN
!        Open BLPDBUG output file
         DUMMY = 'BLPFIL'
         CALL OPEN_DEBUGFILE(BLPUNT,BLPFIL,MODNAM,DUMMY,'BLPDBUG',
     &                       'BLPUNT')
CRCO D095 Added for BLP debug 12/8/21
C        D140 Added BLP column header - Wood 9/29/22
         WRITE( BLPUNT,2225 )
...
CMGS D134_UniformDebugHeaders: DEBUGOPT_ARRAY[18] = "SWPOINT"
CCRT  D113 Sidewash Debug, 3/25/2022
      IF (SWDBG) THEN
C        Open debug Sidewash output file
         DUMMY = 'SWFIL'
         CALL OPEN_DEBUGFILE(SWDBGUNT,SWFIL,MODNAM,DUMMY,'SWPOINT',
     &                       'SWDBGUNT')
      END IF

CMGS D134_UniformDebugHeaders: DEBUGOPT_ARRAY[19] = "AIRCRAFT"
C** Added for Aircraft Plume Rise; UNC-IE
      IF (ARCFTDEBUG) THEN
!       Open Aircraft output file
        DUMMY = 'ARCFTFIL'
         CALL OPEN_DEBUGFILE(ARCFTDBG,DBARCFTFIL,MODNAM,DUMMY,
     &                       'AIRCRAFT','ARCFTDBG')
! Write Header
...
CMGS D134_UniformDebugHeaders: DEBUGOPT_ARRAY[20] = "HBPDBG"
! Added for HBP debug, JAN 2023
      IF (HBPDBG) THEN
!        Open HBP output file
         DUMMY = 'HBPFIL'
         CALL OPEN_DEBUGFILE(HBPUNT,HBPFIL,MODNAM,DUMMY,'HBPDBG',
     &                       'HBPUNT')
...
CMGS D134_Debug_File_Normalization_WSP: Additional TTRM2 files; moved from coset.f/DEBOPT
!     Open TTRM2 debug files; the filenames are hard-wired
      IF (RUNTTRM2 .AND. TTRM2DBG) THEN
         DUMMY = 'AFTTTRM2FIL'
         CALL OPEN_DEBUGFILE(TTRM2TMP(1),'AFTER_TTRM.DBG',MODNAM,DUMMY,
     &                       'TTRM2','TTRM2TMP(1)')
!       Assign 2nd TTRM2 debug file based on selected method
        IF (ARM2) THEN
         DUMMY = 'AFTARM2FIL'
         CALL OPEN_DEBUGFILE(TTRM2TMP(2),'AFTER_ARM2.DBG',MODNAM,DUMMY,
     &                       'ARM2/TTRM2','TTRM2TMP(2)')
        ELSEIF (OLM) THEN
         DUMMY = 'AFTOLMFIL'
         CALL OPEN_DEBUGFILE(TTRM2TMP(2),'AFTER_OLM.DBG',MODNAM,DUMMY,
     &                       'OLM/TTRM2','TTRM2TMP(2)')
        ELSE
         DUMMY = 'AFTPVMRMFIL'
         CALL OPEN_DEBUGFILE(TTRM2TMP(2),'AFTER_PVMRM.DBG',MODNAM,DUMMY,
     &                       'PVMRM/TTRM2','TTRM2TMP(2)')
        ENDIF
         DUMMY = 'MRGTRM2FIL'
         CALL OPEN_DEBUGFILE(TTRM2TMP(3),'TTRM2_MERGE.DBG',MODNAM,DUMMY,
     &                       'TTRM2','TTRM2TMP(3)')
...
C CRLM BEGIN adding SBARRIER debug 20240716 WSP
C--- Opened in soset with barrier calculations
C DEBUGOPT_ARRAY[21] = "SBARRIER"
C        IF (BARDBG) THEN
C C        Open SBARRIER debug output file
C           DUMMY = 'SBARDBGFIL'
C           CALL OPEN_DEBUGFILE(SBRDBUNT,SBARDBGFIL,MODNAM,DUMMY,
C      &                        'SBARRIER','SBRDBUNT')

C        END IF
C CRLM END adding SBARRIER debug 20240716 WSP

C CRLM BEGIN adding BAREDGE debug 20240910 WSP
C DEBUGOPT_ARRAY[22] = "BAREDGE"
       IF (EDGEDBG) THEN
C        Open BAREDGE debug output file
          DUMMY = 'EDGEDBGFIL'
          CALL OPEN_DEBUGFILE(EDGEDBUNT,EDGEDBGFIL,MODNAM,DUMMY,
     &                        'BAREDGE','EDGEDBUNT')

          WRITE(EDGEDBUNT, 100) 
          WRITE(EDGEDBUNT, 101)
          WRITE(EDGEDBUNT, 102) (AYR(IR), AXR(IR), REEDGE(IR), 
     &                                              IR=1,NREC)

 100  FORMAT(/'*********   Barrier Influence Zone   ********'/,
     &       /45('*')/)      
 101  FORMAT(/7X,'Y-COORD',8X,'X-COORD',5X,'Barrier',5X/,5X,'(METERS)',
     &        7X,'(METERS)',5X,'Zone',7X)
 102  FORMAT(/3X, F12.2, 3X, F12.2, 6X, L1, 9X)     

       END IF
C CRLM END adding BAREDGE debug 20240910 WSP

C CRLM BEGIN adding VBARRIER debug 20250508 WSP
C DEBUGOPT_ARRAY[23] = "VBARRIER"
       IF (VBRDBG) THEN
C        Open SBARRIER debug output file
          DUMMY = 'VBARDBGFIL'
          CALL OPEN_DEBUGFILE(VBRDBUNT,VBARDBGFIL,MODNAM,DUMMY,
     &                        'VBARRIER','VBRDBUNT')
...
C         Check RLINEXT source
            IF (SRCTYP(IS) .EQ. 'RLINEXT') THEN
              WRITE(VBRDBUNT,1002) SRCID(IS), 1, VEGBARR(IS)%HT(1),
     &        VEGBARR(IS)%WT(1), VEGBARR(IS)%DCL(1), VEGBARR(IS)%LAI(1),
     &        VEGBARR(IS)%LM(1), VEGBARR(IS)%WAKE(1)
              
              WRITE(VBRDBUNT,1002) SRCID(IS), 2, VEGBARR(IS)%HT(2),
     &        VEGBARR(IS)%WT(2), VEGBARR(IS)%DCL(2), VEGBARR(IS)%LAI(2),
     &        VEGBARR(IS)%LM(2), VEGBARR(IS)%WAKE(2)
            END IF
          END DO
          
 1001    FORMAT(/85('*')/, 'SRCID', 9X, 'BAR#', 4X 'HT', 5X, 'WT',
     &        5X, 'DCL', 4X, 'LAI', 3X, 'LAD', 3X, 
     &        'WAKE')    
 1002    FORMAT(A12, 3X, I1, 4X, F5.1, 2X, F5.1, 2X, F6.1, 2X,
     &         F4.1, 2X, F4.1, 2X, F5.1)          

       END IF
C CRLM END adding VBARRIER debug 20250508 WSP    
...
C              Error Message: Invalid sector field
               ...
               GO TO 999
C JAT 4/14/2025 D212 (D212_O3_NOX_VAL); comment out ELSE statement to avoid an error when not including
C optional ozone units. AERMOD 24142 gives an error if no concentration units are included because
C the number of fields is 4, but FIELD(3)(1:4) = SECT, which is allowed so there should be no error.
c            ELSE
C              Error Message: No Numerical Parameters
c               CALL ERRHDL(PATH,MODNAM,'E','201',KEYWRD)
c               GO TO 999
            END IF
         ELSE IF (IFC .GT. 5) THEN
C           Error Message: Too Many Parameters
...
C              Error Message: Invalid sector field
               ...
               GO TO 999
C JAT 4/14/2025 D212 (D212_O3_NOX_VAL); comment out ELSE statement to avoid an error when not including
C optional NOX units. AERMOD 24142 gives an error if no concentration units are included because
C the number of fields is 4, but FIELD(3)(1:4) = SECT, which is allowed so there should be no error.
c            ELSE
C              Error Message: No Numerical Parameters
c               CALL ERRHDL(PATH,MODNAM,'E','201',KEYWRD)
...
C - WSP 9/16/2024 D158 Dynamic File unit  Store FUNIT, FILENAME, and FILETYPE in array to add to output as table  - Starting       
            FUNITARRAY(FUNITIND) = IO3UNT(IO3SECT)
            FNAMEARRAY(FUNITIND) = OZONFL(IO3SECT)
            FTYPARRAY(FUNITIND) = 'OZONEfile'
            FUNITIND = FUNITIND + 1
C - WSP 9/16/2024 D158 Dynamic File unit  Store FUNIT, FILENAME, and FILETYPE in array to add to output as table  - Ending      
...
C - WSP 9/16/2024 D158 Dynamic File unit  Store FUNIT, FILENAME, and FILETYPE in array to add to output as table  - Starting       
            FUNITARRAY(FUNITIND) = INOXUNT(INOXSECT)
            FNAMEARRAY(FUNITIND) = NOXFL(INOXSECT)
            FTYPARRAY(FUNITIND) = 'NOXfile'
            FUNITIND = FUNITIND + 1
C - WSP 9/16/2024 D158 Dynamic File unit  Store FUNIT, FILENAME, and FILETYPE in array to add to output as table  - Ending      
...
C     D157 WSP 3/28/23 Changed ARMRATIO restictions to match user's guide
C     DFAULT ARMRATIO is 0.5 <= ARMRATIO <= 0.9
C     NONDFAULT ARMRATIO is 0.0 < ARMRATIO <= 1.0
      ...
      ELSE IF (ARM2_Min .GT. 1.0D0) THEN
         CALL ERRHDL(PATH,MODNAM,'E','380',' ARM2Min')
C     D157 CRT 5/31/2023 remove warning message if ARMRATIO is within default
C      ELSE IF (ARM2_Min .GT. 0.50D0) THEN
C         CALL ERRHDL(PATH,MODNAM,'W','736',' ARM2Min')
      END IF

CC     Check range of value for ARM2_Min
C      IF (ARM2_Min .LE. 0.0D0) THEN
C         WRITE(DUMMY,'(''ARM2Min <= 0'')')
C         CALL ERRHDL(PATH,MODNAM,'E','380',DUMMY)
C      ELSE IF (ARM2_Min .GT. 1.0D0) THEN
C         WRITE(DUMMY,'(''ARM2Min > 1'')')
C         CALL ERRHDL(PATH,MODNAM,'E','380',DUMMY)
C      ELSE IF (ARM2_Min .LT. 0.50D0) THEN
C         IF( DFAULT )THEN
C            WRITE(DUMMY,'(''ARM2Min <0.5'')')
C            CALL ERRHDL(PATH,MODNAM,'E','380',DUMMY)
C         ELSE
C            WRITE(DUMMY,'(''ARM2Min <0.5'')')
C            CALL ERRHDL(PATH,MODNAM,'W','380',DUMMY)
C         END IF
C      ELSE IF (ARM2_Min .GT. 0.50D0) THEN
C         WRITE(DUMMY,'(''ARM2Min >0.5'')')
C         CALL ERRHDL(PATH,MODNAM,'W','380',DUMMY)
...
C     NONDFAULT ARMRATIO is 0.0 < ARMRATIO <= 1.0
      ...
      ELSE IF (ARM2_Max .GT. 1.0D0) THEN
         CALL ERRHDL(PATH,MODNAM,'E','380',' ARM2Max')
C     D157 CRT 5/31/2023 remove warning message if ARMRATIO is within default
C      ELSE IF (ARM2_Max .LT. 0.90D0) THEN
C         CALL ERRHDL(PATH,MODNAM,'W','736',' ARM2Max')
      END IF
C     D157 Check that ARMmax > ARMmin
      IF (ARM2_Max .LT. ARM2_Min) THEN
         WRITE(DUMMY,'(''ARM2 Max<Min'')')
            CALL ERRHDL(PATH,MODNAM,'E','380',DUMMY)
      END IF
CC     Check range of value for ARM2_Max
C      IF (ARM2_Max .LE. 0.0D0) THEN
C         WRITE(DUMMY,'(''ARM2Max <= 0'')')
C         CALL ERRHDL(PATH,MODNAM,'E','380',DUMMY)
C      ELSE IF (ARM2_Max .GT. 1.0D0) THEN
C         WRITE(DUMMY,'(''ARM2Max > 1'')')
C         CALL ERRHDL(PATH,MODNAM,'E','380',DUMMY)
C      ELSE IF (ARM2_Max .LT. ARM2_Min) THEN
C         WRITE(DUMMY,'(''ARM2 Max<Min'')')
C         CALL ERRHDL(PATH,MODNAM,'E','380',DUMMY)
C      ELSE IF (ARM2_Max .LT. 0.90D0) THEN
C         IF( DFAULT )THEN
C            WRITE(DUMMY,'(''ARM2Max <0.9'')')
C            CALL ERRHDL(PATH,MODNAM,'E','380',DUMMY)
C         ELSE
C            WRITE(DUMMY,'(''ARM2Max <0.9'')')
C            CALL ERRHDL(PATH,MODNAM,'W','380',DUMMY)

...
C              Calculate OpenPit Source Values                      ---   CALL OCALC
               CALL OCALC
C              D097 WSP 3/29/2023 removed the lmitor of BLPROCESSED being FALSE following RLINE example
C            ELSE IF (SRCTYP(ISRC) .EQ. 'BUOYLINE') THEN
...
C     10/29/2025, CRT - D138 Scientific Notation      
      CHARACTER(LEN=15) :: GRPAVE_TestC   ! event group average conc as character string.

      INTEGER :: IEVYR, IASTAT
      DOUBLE PRECISION :: GRPAVE_Test ! event group average conc as real double.
...
C              10/29/2025, CRT - D138 Scientific Notation
C              Modify how the original concentration and event concentration are
C              stored for comparison - event concentration to a character variable
C              to round value to the same number of decimal places the orig conc
C              is displayed in input file based on FILEFORM = FIX or EXP, then read
C              back to double precision variable 
               IF( EV_OrigConc(IEVENT) .GT. 0.0D0 ) THEN
CRTC ---             Since "original" EVENT concentration is read from input file
CRTC                 with 5 decimal places, first round the internal results to 
CRTC                 5 decimal places to avoid spurious messages if the original
CRTC                 concentration is less than 10.0.
CRT                  IF( GRPAVE(IDXEV(IEVENT)) .LT. 10.0D0 )THEN
CRT                     GRPAVE_Test = DBLE(IDNINT(1.0D5 * 
CRT     &                             GRPAVE(IDXEV(IEVENT))))/
CRT     &                             1.0D5
CRT                  ELSE
CRTC ---                Use original value for comparison
CRT                     GRPAVE_Test = GRPAVE(IDXEV(IEVENT))
CRT                  END IF

C                 10/29/2025, CRT - D138 Scientific Notation
C                 Read event conc into character variable using same
C                 format used in event input file
                  IF (FILE_FORMAT == 'FIX') THEN
                     WRITE(GRPAVE_TestC,'(F12.5)') 
     &                     GRPAVE(IDXEV(IEVENT))
                  ELSEIF (FILE_FORMAT == 'EXP') THEN
                     WRITE(GRPAVE_TestC,'(E13.6)') 
     &                     GRPAVE(IDXEV(IEVENT))
                  END IF
                  
C                 Read event conc stored as string into double precision
C                 variable for comparison to original concentration 
                  READ(GRPAVE_TestC,*)GRPAVE_Test
...
CC        Determine The Current Julian Day and Calculate Current Gregorian Date
CC        First Convert Year to 4-Digit Value
C         IF (IYEAR .GE. ISTRT_WIND .AND. IYEAR .LE. 99) THEN
C            IYR = ISTRT_CENT*100 + IYEAR
C         ELSE IF (IYEAR .LT. ISTRT_WIND) THEN
C            IYR = (ISTRT_CENT+1)*100 + IYEAR
C         ELSE
CC           Input IYEAR must be 4-digit:  Save to IYR and convert to 2-digit
C            IYR   = IYEAR
C            IYEAR = IYR - 100 * (IYR/100)
C         END IF
...
C*                      M.G. Snyder, WSP 6/6/2023
C*
C*         MODIFIED:    Modified to read Aircraft Engine Parameters for
...
CMGS               CALL HRQREAD (IS) !D151 - 6/5/2023
...
C**  Added for Aircraft Plume Rise; UNC-IE !D151 - MGS 6/6/23
                 IF (AFTSRC(IS) .EQ. 'Y') THEN
                  ...
                  END IF
C**  End Aircraft Plume Rise insert; April 2023 !D151 - MGS 6/6/23
...
C                 D001 Call CENT_DATE to determine the Current Julian Day and Calculate Current Gregorian Date First Convert Year to 4-Digit Value Wood 9/15/22
                  ...
                  END IF
C ---  D001 remove original calculation of 4-Digit year Wood 9/15/22
CC ---             Check for use of 2-digit year in background file, adjust to 4-digit
CC                 year for comparison with FULLDATE based on met data file

...
C        DATE:    March 2, 1992
C
C        MODIFIED:   Updated the code to add the Aircraft plume rise
C                    debug option print
C                    Gavendra Pandey; UNC-IE, NC, Chapel Hill, USA
C                    04/01/2023
C
...
c     CRT 8/9/2023 D176 COARE processing in AERMET      
      IF (L_COARE) THEN
...
CCCRT     3/24/2021, D076 TTRM NO2
CCCRT     update for TTRM debug opt display in .out and .sum
CC ---    Loop through the 11 different options that are flagged
C         DO I = 1, 11
C            IF (LEN_TRIM(DEBUG_OPTS(I)) .GT. 0) THEN
C               NOPS = NOPS + 1
C               ILEN = 10*(NOPS-1)
C               DEBUG_OPTS_String =
C     &         DEBUG_OPTS_String(1:ILEN)//' '//DEBUG_OPTS(I)
C            END IF
C         END DO

C** Updated for Aircraft Plume Rise; UNC-IE
C     update for Aircraft debug opt display in .out and .sum
C ---    Loop through the 12 different options that are flagged
         DO I = 1, 12
...
C
C        MODIFIED:   To include the Aircraft source data
...
C     Variable Declarations
      USE MAIN1
      USE RLINE_DATA, ONLY: RLSOURCE, RLMOVESCONV, VEGBARR
      USE BUOYANT_LINE
      IMPLICIT NONE
      CHARACTER MODNAM*12
...
C     Barriers, If Any
     ...
     &      RLSOURCE(I)%BARID, RLSOURCE(I)%HTWALL, RLSOURCE(I)%DCLWALL, !add BARID for SBARRIER 
     &      RLSOURCE(I)%BARID2,RLSOURCE(I)%HTWALL2,RLSOURCE(I)%DCLWALL2 !add BARID2 for SBARRIER
            END IF
         END IF
      END DO
      
C     Write out RLINE source configuration parameters for
C     Vegetative barriers, if any
      IF (VBARSET) THEN
        INDC = 0
        DO I = 1, NUMSRC
           IF ((SRCTYP(I) .EQ. 'RLINE') .OR.
     &             (SRCTYP(I) .EQ. 'RLINEXT')) THEN
              IF (VEGBARR(I)%HT(1) .GT. 0.0D0 .OR.
     &            VEGBARR(I)%HT(2) .GT. 0.0D0) THEN
                 INDC = INDC + 1
              IF (MOD(INDC-1,40) .EQ. 0) THEN
                 WRITE(IOUNIT,8112)
              END IF
              WRITE(IOUNIT,8113) SRCID(I), VEGBARR(I)%HT(1),
     &        VEGBARR(I)%WT(1), VEGBARR(I)%DCL(1), VEGBARR(I)%LAI(1),
     &        VEGBARR(I)%LM(1), VEGBARR(I)%WAKE(1),
     &        VEGBARR(I)%HT(2),
     &        VEGBARR(I)%WT(2), VEGBARR(I)%DCL(2), VEGBARR(I)%LAI(2),
     &        VEGBARR(I)%LM(2), VEGBARR(I)%WAKE(2)
              END IF
           END IF
        END DO  
      END IF
...
C 9075 FORMAT(1X,A12,2X,A5,2X,E11.5,2F10.1,A8,F9.2,1X,F8.2,1X,
C    &       F8.2,5X,A3,3X,A7)
C**  Updated for Aircraft; UNC-IE
 ...
 9076 FORMAT(//50X,'*** AREA SOURCE DATA ***'//15X,
     ...
     & 14X,'BY'/66(' -')/)
C 9077 FORMAT(1X,A12,2X,A5,2X,E11.5,2F10.1,A8,F9.2,3(1X,F9.2),1X,F8.2,
C    &       5X,A3,3X,A7)
C**  Updated for Aircraft; UNC-IE
 ...
 9078 FORMAT(//48X,'*** AREACIRC SOURCE DATA ***'//15X,
     ...
     & /63(' -')/)
C 9079 FORMAT(1X,A12,2X,A5,2X,E11.5,2F10.1,A8,F9.2,2X,F9.2,4X,I4,4X,
C     &       F8.2,5X,A3,3X,A7)
C**  Updated for Aircraft; UNC-IE
 ...
 9080 FORMAT(//48X,'*** AREAPOLY SOURCE DATA ***'//15X,
     ...
     & /63(' -')/)
C 9081 FORMAT(1X,A12,2X,A5,2X,E11.5,2F10.1,A8,F9.2,4X,I4,5X,F8.2,5X,
C     & A3,3X,A7)
C**  Updated for Aircraft; UNC-IE 
...
CRLM add BARID for SBARRIER inputs to 9112 and 9113 WSP 20240528 START
CRLM 9112 FORMAT(//14X, 'BARRIER',6X,'BARRIER',7X,'BARRIER',7X,'BARRIER',
CRLM     & /3X,'SOURCE',5X,'HTWALL',7X,'DCLWALL',7X,'HTWALL2',6X,'DCLWALL2',
CRLM     & /4X,'ID',7X,'(METERS)',5X,'(METERS)',6X,'(METERS)',6X,'(METERS)',
CRLM     & /66(' -')/)
CRLM 
CRLM 9113 FORMAT(1X,A12,1X,F6.2,8X,F6.2,8X,F6.2,8X,F6.2)

 9112 FORMAT(//31X, 'BARRIER',6X,'BARRIER',21X,'BARRIER',6X,'BARRIER',
     &  /3X,'SOURCE',8X,'BARRIER',7X,'HTWALL',7X,'DCLWALL',7X,'BARRIER',
     &  7X,'HTWALL2',5X,'DCLWALL2', 
     &  /4X,'ID',12X,'ID1',9X,'(METERS)',5X,'(METERS)',8X,'ID2',9X,
     &  '(METERS)',5X,'(METERS)',
     &  /66(' -')/)
   
 9113 FORMAT(3X,A12,2X,A12,2X,F6.2,7X,F6.2,5X,A12,5X,F6.2,7X,F6.2)

CRLM add BARID for SBARRIER inputs to 9112 and 9113 WSP 20240528 END

CRLM VBARRIER summary BEGIN
 8112 FORMAT(//50X,'*** Vegetative Barriers ***',//3X,'SOURCE',
     & 9X, 'HT',6X, 'WT',5X,'DCL',3X,'LAI',4X, 'LAD',4X,'WAKE',7X,'HT',
     & 6X,'WT',5X,'DCL',3X,'LAI',4X, 'LAD',4X,'WAKE'/5X,'ID',
     & 10X,'(M)',5X,'(M)',5X,'(M)',9X,'(1/M)',4X,'(M)',6X,
     & '(M)',5X,'(M)',5X,'(M)',9X,'(1/M)',4X,'(M)', /66(' -')/)
 8113 FORMAT(3X, A12, 2X, F5.1, 2X, F5.1, 2X, F6.1, 2X,
     &       F4.1, 3X, F4.1, 3X, F5.1, 5X,F5.1, 2X, F5.1, 2X, 
     &       F6.1, 2X,F4.1, 3X, F4.1, 3X, F5.1/)
CRLM VBARRIER summary END

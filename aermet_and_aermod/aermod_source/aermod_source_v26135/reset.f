...
!added for barrier edge effect 20240814 WSP
         IF (BAREDGE .AND. SBARDCL) THEN 
           CALL BAREDGEZONE
         ELSE IF (BAREDGE .AND. .NOT.SBARDCL) THEN
           !BAREDGE requires SBARRIER inputs
           CALL ERRHDL(PATH,MODNAM,'W','274',KEYWRD)
           BAREDGE = .FALSE.
         END IF
...
C     Set the receptor flag by BL group
     ...
     &          XR_SCS(NR,KK) .GE. (XLMIN - 0.1D0)) THEN
      ...
      SUBROUTINE BAREDGEZONE
C***********************************************************************
C     BAREDGEZONE Module of the AMS/EPA Regulatory Model - AERMOD
C
C     PURPOSE: Find barrier influence zone relative to edge zones
C
C     PROGRAMMER: Rebecca L Miller, WSP
C
C     DATE:    14 AUGUST 2024
C
C     INPUTS:  Receptor locations and SBARRIER end points
C
C     OUTPUTS: Edge zone receptor array
C
C     CALLED FROM: RECARD
C***********************************************************************
C     Variable Declarations
      ...
      INTERFACE
C       Define external BARZONE function, rotates barrier to determine 
C       if rotated receptors are in edge zones.
        FUNCTION BARZONE(BXS, BYS, BHT, REZONE)
          USE MAIN1, ONLY: NREC, AXR, AYR
          DOUBLE PRECISION, INTENT(IN) :: BXS(2), BYS(2), BHT !Barrier Xs,Ys,Height
          LOGICAL, INTENT(IN), DIMENSION(NREC)  :: REZONE  !input zone
          LOGICAL, DIMENSION(NREC)              :: BARZONE !output zone
        END FUNCTION BARZONE
      END INTERFACE      
      
      INTEGER :: IASTAT, IB, IS, IDLOC, I
      DOUBLE PRECISION, DIMENSION(2)    :: BXS, BYS
      LOGICAL, DIMENSION(NSBR)          :: IDCHECK
      CHARACTER(len=12), DIMENSION(NSBR):: IDLIST
      DOUBLE PRECISION :: BLENGTH
      
C     IASTAT    - allocation function error indicator
C     IB        - barrier loop index
C     IS        - source loop index
C     IDLOC     - barrier id index/location in array
C     BXS, BYS  - Barrier begin and end X, Y coordinates
C     IDCHECK   - Which barrier IDs have been checked for edge zones
C     IDLIST    - BARID names list
C     BLENGTH   - Barrier length
...
C     List of all BARIDs      
      DO IB = 1, NSBR
        IDLIST(IB) = SLDBARR(IB)%BARID
      END DO
      
C     Allocate REEDGE for number of receptors
C     REEDGE logical array indicating if receptor is in barrier or edge zone     
      IF (.NOT.ALLOCATED(REEDGE)) THEN
        ALLOCATE(REEDGE(NREC), STAT=IASTAT)
        IF (IASTAT .NE. 0) THEN
          CALL ERRHDL(PATH,MODNAM,'E','270','REEDGE')
        END IF
      END IF 
 
      REEDGE(:) = .FALSE. !T = barrier on; F = barrier off/edge zone  
      
C     Find BARID's associated with RLINEXT sources
      DO IS = 1, NSRC
        IF (SRCTYP(IS) .EQ. 'RLINEXT') THEN
            !Check for barrier1
            IF (RLSOURCE(IS)%HTWALL  .GT. 0.0D0) THEN
              IDLOC = FINDLOC(IDLIST, RLSOURCE(IS)%BARID, DIM=1)
              IF (.NOT.IDCHECK(IDLOC)) THEN
                IDCHECK(IDLOC) = .TRUE.
                BXS(1) = SLDBARR(IDLOC)%XBB(1)
                BXS(2) = SLDBARR(IDLOC)%XBE(SLDBARR(IDLOC)%NUMSEG)
                BYS(1) = SLDBARR(IDLOC)%YBB(1)
                BYS(2) = SLDBARR(IDLOC)%YBE(SLDBARR(IDLOC)%NUMSEG)
                
                BLENGTH = DSQRT((BXS(1) - BXS(2))**2 + 
     &                          (BYS(1) - BYS(2))**2)
                IF (BLENGTH .LE. (2.0D0*6.5D0*RLSOURCE(IS)%HTWALL)) THEN
                  !W: Barrier not long enough for edge effects
                  CALL ERRHDL(PATH,MODNAM,'W','275','REEDGE')
                  BAREDGE = .FALSE.
                END IF
                
                REEDGE = BARZONE(BXS, BYS, RLSOURCE(IS)%HTWALL, REEDGE)
              END IF
            END IF
            
            
            !Check for barrier2
            IF (RLSOURCE(IS)%HTWALL2 .GT. 0.0D0) THEN
              IDLOC = FINDLOC(IDLIST, RLSOURCE(IS)%BARID2, DIM=1)
              IF (.NOT.IDCHECK(IDLOC)) THEN
                  IDCHECK(IDLOC) = .TRUE.
                  BXS(1) = SLDBARR(IDLOC)%XBB(1)
                  BXS(2) = SLDBARR(IDLOC)%XBE(SLDBARR(IDLOC)%NUMSEG)
                ...
                BLENGTH = DSQRT((BXS(1) - BXS(2))**2 + 
     &                            (BYS(1) - BYS(2))**2)
                IF (BLENGTH .LE.(2.0D0*6.5D0*RLSOURCE(IS)%HTWALL2)) THEN
                  !W: Barrier not long enough for edge effects
                  CALL ERRHDL(PATH,MODNAM,'W','275','REEDGE')
                  BAREDGE = .FALSE.
                END IF
                
                REEDGE = BARZONE(BXS, BYS, RLSOURCE(IS)%HTWALL2, REEDGE)

              END IF
            END IF
            
        END IF !SRCTYPE      
      END DO !IS, source loop
    
      
      RETURN
      END SUBROUTINE BAREDGEZONE
      
      FUNCTION BARZONE(BXS, BYS, BHT, REZONE)
C***********************************************************************
C       REBARZONE Function
C       Purpose: Rotates SBARRIER to x-axis and receptors relative to 
C                barrier to identify barrier influence and edge effect
C                zones.
C
C       Programmer: Rebecca L Miller WSP
C       Date: 20240821
C
C       Inputs:
C           BXS     - barrier begin and end X-coordinates
C           BYS     - barrier begin and end Y-coordinates
C           BHT     - barrier height
C           REZONE  - array indicating barrier/edge zone for receptors
C
C       Outputs:
C           BARZONE - updates REZONE for input barrier zone
C
C       Called from BAREDGEZONE
C***********************************************************************
      USE MAIN1, ONLY: NREC, AXR, AYR
      
      IMPLICIT NONE
      
      DOUBLE PRECISION, INTENT(IN) :: BXS(2), BYS(2), BHT
      LOGICAL, INTENT(IN), DIMENSION(NREC)          :: REZONE
      
      LOGICAL, DIMENSION(NREC) :: BARZONE !output barrier zone
      
      DOUBLE PRECISION                  :: RXR, RYR !rotated receptor X,Y
      DOUBLE PRECISION                  :: BXR(2), BYR !retated barrier Xs,Y
      DOUBLE PRECISION                  :: BANGLE !angle
      INTEGER                           :: IR !receptor loop index
...
C     Rotate barrier to x-axis by BANGLE   
      BANGLE = DABS(DATAN2((BYS(2) - BYS(1)),(BXS(2) - BXS(1))))
      
      BXR(1) =  BXS(1)*DCOS(BANGLE) + BYS(1)*DSIN(BANGLE)
      BXR(2) =  BXS(2)*DCOS(BANGLE) + BYS(2)*DSIN(BANGLE)
      BYR    = -BXS(1)*DSIN(BANGLE) + BYS(1)*DCOS(BANGLE)
      
C     Loop receptors
      DO IR = 1, NREC
        !Skip receptor if already in barrier zone (TRUE)
        IF (.NOT.REZONE(IR)) THEN !not already in a zone
          !Rotate receptor coordinates
          RXR =  AXR(IR)*DCOS(BANGLE) + AYR(IR)*DSIN(BANGLE)
          RYR = -AXR(IR)*DSIN(BANGLE) + AYR(IR)*DCOS(BANGLE)

          !Check if receptor in barrier zone
          IF (RXR.GE.(BXR(1)+(6.5D0*BHT))  .AND. 
     &        RXR.LE.(BXR(2)-(6.5D0*BHT))  .AND.
     &        RYR.GE.(BYR   -(40.0D0*BHT)) .AND.
     &        RYR.LE.(BYR   +(40.0D0*BHT))) THEN
            BARZONE(IR) = .TRUE. !in barrier zone
          END IF
        END IF
      END DO !IR receptor loop

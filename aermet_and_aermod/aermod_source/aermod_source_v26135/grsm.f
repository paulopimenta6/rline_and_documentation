...
C --- Local variables
      INTEGER::NUMGRP_USE, IGRP_USE, nStat, I
...
c MKP DOUBLE PRECISION::NO2Frac !NO2 fraction !D231
      DOUBLE PRECISION, ALLOCATABLE::NO2Frac(:) !NO2Frac(GROUP), different for each group when background included D231
      DOUBLE PRECISION::GRPBACKNO2,GRPBACKNO    !total ensemble and instantaneous concs that include background for NO2Frac D231
      DOUBLE PRECISION::NO2FracMDC              !for MAXDCONT accounting output D231
      DOUBLE PRECISION::BGCONC_IN, BGCONC_OUT, BGCONC_PPB
      LOGICAL::L_DoFullConv, L_GRPBACK
...
C --- Initialise
      ...
      IF(nStat/=0)THEN
        CALL ERRHDL(PATH, MODNAM, 'E','615','GRSM') 
      END IF
      IF(ALLOCATED(NO2Frac)) DEALLOCATE(NO2Frac,STAT=nStat) ! D231 MKP
      ALLOCATE(NO2Frac(NUMGRP),STAT=nStat)                  ! D231 MKP
      IF(nStat/=0)THEN                                      ! D231 MKP
        CALL ERRHDL(PATH, MODNAM, 'E','615','GRSM')         ! D231 MKP
      END IF                                                ! D231 MKP
...
C MKP D211      DO IGRP = 1, NUMGRP_USE
C MKP D211        
C MKP D211        IF(EVONLY)THEN
C MKP D211          !Event only
C MKP D211          IGRP_USE=IDXEV(IEVENT)
C MKP D211        ELSE
C MKP D211          !Normal calculation use actual group index
C MKP D211          IGRP_USE=IGRP
C MKP D211        END IF
     ...
     +                                                     STAT=nStat)
            ...
            TotalNOx = 0.0D0
            DO ISRC = 1, NUMSRC
C MKP D211  IF (IGROUP(ISRC,IGRP_USE) == 0) CYCLE
...
C MKP D211      IF(GRP_BACK(IGRP_USE).AND..NOT.L_NIGHTHOUR)THEN
                IF(L_BACKGRND.AND..NOT.L_NIGHTHOUR)THEN ! MKP D211, MKP D231 add logic for SO BACKGRND
...
C MKP D211  IF(SUM(CHI(IREC,:,1),MASK=IGROUP(:,IGRP_USE)==1)/=0.0D0)THEN
            IF(SUM(CHI(IREC,:,1))/=0.0D0)THEN ! MKP D211
              TTRAVCHM(IREC)=
C MKP D211     &        SUM(CHI_TTRAVCHM(IREC,:),MASK=IGROUP(:,IGRP_USE)==1)/
C MKP D211     &        SUM(CHI(IREC,:,1),MASK=IGROUP(:,IGRP_USE)==1)
     &        SUM(CHI_TTRAVCHM(IREC,:))/      ! MKP D211
     &        SUM(CHI(IREC,:,1))              ! MKP D211
            ELSE
              TTRAVCHM(IREC)=0.0D0  
            END IF
...
C           Issue fatal error message; Group ALL not found
            IF (GRSM) THEN
               CALL ERRHDL(PATH,MODNAM,'W','299','GRSM Option')
            END IF
         END IF
c MKP D231 accounting for groups with background NO2+NOX in NO2Frac()
           GRPBACKNO2=0.0D0
           GRPBACKNO =0.0D0
           L_GRPBACK = .FALSE.
           GRP_BACK_LOOP: DO IGRP = 1, NUMGRP ! MKP D231
C MKP D211  IF(GRP_BACK(IGRP_USE).AND..NOT.L_NIGHTHOUR)THEN
            IF(IGRP.EQ.INDX_GRPALL.AND.GRP_BACK(IGRP)
     &                         .AND..NOT.L_NIGHTHOUR) THEN
              GRPBACKNO2 = ENSCONCNO2 + NO2CONC_BG ! MKP D231 this is NO2 from equilibrium
              GRPBACKNO  = ENSCONCNO  + NOCONC_BG  ! MKP D231 this is NO  from equilibrium
              L_GRPBACK  = .TRUE.
            END IF
C           IF(L_BACKGRND.AND..NOT.L_NIGHTHOUR)THEN ! MKP D211
C             ENSCONCNO2 = ENSCONCNO2 + NO2CONC_BG ! MKP D231 this is NO2 from equilibrium
C             ENSCONCNO  = ENSCONCNO  + NOCONC_BG  ! MKP D231 this is NO  from equilibrium
C           END IF                                 ! MKP D231
            
C ---       Calculate secondary NO2 fraction after chemistry
            IF(L_GRPBACK.AND.(GRPBACKNO2+GRPBACKNO).GE.0.0D0) THEN ! D231, D243 check for NOX>=0, else set NO2Frac=0.0D0
              NO2Frac(IGRP) = GRPBACKNO2 / (GRPBACKNO2 + GRPBACKNO) ! includes background if L_BACKGRND true...SO BACKGRND...
C MKP D243    Set numerical bounds in case of very low NOX impacts (e.g., <~.1ppb) with negative NO2 or NO post-chem values
              IF(NO2Frac(IGRP).GE.0.0D0) THEN
                NO2Frac(IGRP) = MIN(1.0D0, NO2Frac(IGRP)) ! D243, NO2Frac should always be positive here, and <=1
              ELSE ! D243, NO2Frac less than zero
                NO2Frac(IGRP) = 1.0D0
              END IF
              NO2FracMDC = NO2Frac(IGRP) ! save for MAXDCONT accounting run D231
            ELSE IF (.NOT.L_GRPBACK.AND.
     &               (ENSCONCNO2+ENSCONCNO).GE.0.0D0)THEN ! D231, D243 check for NOX>=0, else set NO2Frac=0.0D0
              NO2Frac(IGRP) = ENSCONCNO2 / (ENSCONCNO2 + ENSCONCNO) ! D231 no background in NO2Frac for this group 
C MKP D243    Set numerical bounds in case of very low NOX impacts (e.g., <~.1ppb) with negative NO2 or NO post-chem values
              IF(NO2Frac(IGRP).GE.0.0D0) THEN
                NO2Frac(IGRP) = MIN(1.0D0, NO2Frac(IGRP)) ! D243, NO2Frac should always be positive here, and <=1
              ELSE ! D243, NO2Frac less than zero
...
C ---       Calculate post chemistry concentrations from secondary NO2 fraction, partitioning background and plume
            ...
            ELSE IF (L_GRPBACK) THEN
              !Day-time
              !Background NO2 (other background concs not o/p)
              BGCONC_OUT = NO2Frac(IGRP) * NOXCONC_BG/NO2_PPB ! D231 by group 
              BGCONC_PPB = NO2Frac(IGRP) * NOXCONC_BG         ! D231 by group
            ELSE !D243 output unconverted NO2 daytime background if not added to ALL SRCGROUP
              BGCONC_OUT = BGCONC_IN
              BGCONC_PPB = BGCONC_IN*NO2_PPB
            END IF
           END DO GRP_BACK_LOOP ! MKP D231
         ...
         GROUP_LOOP: DO IGRP = 1, NUMGRP ! MKP D211 
          SOURCE_LOOP: DO ISRC = 1, NUMSRC          

C MKP D211  IF( IGROUP(ISRC,IGRP_USE) .NE. 1 )THEN
C ---         This source is not included in the current SRCGROUP
C MKP D211    CYCLE SOURCE_LOOP            
C MKP D211  END IF
 
            !Plume, D231 updated to include background in NO2Frac by group
          DO ITYP = 1,NUMTYP
           IF(L_GRPBACK)THEN
               HRVAL(ITYP) = NO2FracMDC * CHI(IREC,ISRC,ITYP)
           ELSE
               HRVAL(ITYP) = NO2Frac(IGRP) * CHI(IREC,ISRC,ITYP)
           END IF
          END DO 
...
C ---         Write debug file
     ...
     &           CHI(IREC,ISRC,1), NO2Frac(IGRP), BGCONC_PPB, HRVAL(1),
...
C ---         Write debug file
     ...
     &                 ANO2_RATIO(isrc),CHI(IREC,ISRC,1), NO2Frac(IGRP),
...
C MKP D211          IF (GRP_BACK(IGRP_USE)) THEN
          IF (GRP_BACK(IGRP)) THEN ! MKP D211
            IF( .NOT.EVONLY )THEN  
...
C ---         Call EV_SUMBACK to update BACKGROUND contributions
            ...
            ENDIF
          ENDIF
         END DO GROUP_LOOP ! MKP D211
        END DO  RECEPTOR_LOOP !Loop over points
C MKP D211      END DO  !Loop over groups
...
C        DATE: November 2020
C
C        MODIFIED: To add the plume rise for VOLUME/AREA Aircraft Sources.
C                  Gavendra Pandey; UNC-IE, Chapel Hill, NC, USA
C                  04/01/2023
C                  Updated to optimize the calculation on 1/8/2026
C                  Gavendra Pandey; UNC-IE, Chapel Hill, NC, USA
C
...
C      DOUBLE PRECISION::VSEQ, HSPRIM, ZPLM, DHFOLD, SVPM, SVPM2, UPM
      DOUBLE PRECISION::VSEQ, HSPRIM, ZPLM, DHFOLD, SVPM, UPM   ! Removed SWPM; Added earlier for ARISE; UNC-IE 
...
C        Calculate Buoyancy Fluxes                          ---   CALL AFLUXES
         CALL AFLUXES
...
C        Calculate Distance to Final Rise                   ---   CALL ADELTAH
         CALL ADELTAH(XARG)
...
C           Check for XMIXED smaller than 1.25*XFINAL
            IF (XMIXED .LT. 1.25D0*XFINAL) THEN
               XFINAL = 0.8D0 * XMIXED
               CALL PLUME_TRAJ (XFINAL)               ! Change from ACBLPRD to PLUME_TRAJ on June 2025 by UNC-IE
               DHCRIT = DHP1
            END IF
         END IF
C**  End Aircraft Plume Rise insert; April 2023; Updated on January 2026
...
C**  Added for Aircraft Plume Rise; UNC-IE
     ...
     &          SRCTYP(ISRC)(1:4) .EQ. 'AREA') .AND.
     &         (AFTSRC(ISRC) .EQ. 'Y')) THEN
C       Calculate the plume rise                  ---   CALL ADELTAH
        CALL ADELTAH ( XARG )
...
C     &          (1.0D0/(SQRT((2.0D0/3.0D0)*EPS*TIME**3)+SIGYINIT)))**-1
C
C MKP  Previous SIGY, SIGZ, and SIGR2 coding from v22112, as per 2017
C      JAWMA paper
C      SIGY_I=MIN(SIGY_NM,SQRT((2.0D0/3.0D0)*EPS*TIME**3+SIGYINIT**2))
C      SIGZ_I=MIN(SIGZ,SQRT((2.0D0/3.0D0)*EPS*TIME**3+SIGYINIT**2))
C      SIGR2 = EPS*TIME**3.0D0/3.0D0
C
C MKP  See 2023-01-17 technical paper from CERC on changes to
C instantaneous plume spread calculations.

...
C
     ...
     &         UREF, WDREF, UREFHT, TA, TREFHT, IPCODE, PRATE, RH,
     &         SFCP, NCLOUD
C  ****** FOR HIGHLY BUOYANT PLUME ****** added code JAN 2023--kja
...
CC              Convert ISYR to Four Digits
...
C ---       Check for ZICONV > ZIURB; if so then disable morning transition
            ...
            END IF
C JAT D136_URBAN_TRANS; ADD ELSE STATEMENT TO CYCLE IF THE NOURBANTRAN INVOKED AND
C HOUR IS CONVECTIVE
C THIS WILL KEEP THE URBAN DEBUG FILE FROM HAVING NaNs WHEN FIRST HOUR OF MET FILE IS
C CONVECTIVE
         ELSE
           L_MorningTrans(IURB) = .FALSE.
           CYCLE

...
C               IEDATE = ISTRT_CENT*100000000 + IEDATE
C            ELSE IF (IEYR .LT. ISTRT_WIND) THEN
C               IEYR   = (ISTRT_CENT+1)*100 + IEYR
C               IEDATE = (ISTRT_CENT+1)*100000000 + IEDATE
C            END IF
...
C            D001 Call LONG_DATE_Opt2 and determine the longform date without updating the short date Wood 9/15/22
             CALL LONG_DATE_Opt2(ITEMPDATE,ITEMPDATE,ITEMPYEAR) !Determine the longform date
C ---        D001 remove original calculation of year Wood 9/15/22
C               IF (ITEMPYEAR.GE.ISTRT_WIND .AND. ITEMPYEAR.LE.99) THEN
C                  ITEMPDATE = ISTRT_CENT*100000000 + ITEMPDATE
C               ELSE IF (ITEMPYEAR .LT. ISTRT_WIND) THEN
C                  ITEMPDATE = (ISTRT_CENT+1)*100000000 + ITEMPDATE
C               END IF
...
C     Variable Initializations
      MODNAM = 'EVPER'
      FOUND = .FALSE.
CCRT  5/2/2026 initialize variables
      ISDX = 0
      IMN = 0
      IDY = 0
      IEVYR2 = 0
...
CC        Convert to 4-digit year
C         IF (IEVYR2 .GE. ISTRT_WIND .AND. IEVYR2 .LE. 99) THEN
C            IEVYR4 = ISTRT_CENT*100 + IEVYR2
C         ELSE IF (IEVYR2 .LT. ISTRT_WIND) THEN
C            IEVYR4 = (ISTRT_CENT+1)*100 + IEVYR2
C         END IF

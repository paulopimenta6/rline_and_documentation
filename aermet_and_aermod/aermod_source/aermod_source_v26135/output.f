...
C     8/8/2025, CRT: D138 EXP Format
C     Add variables for concentration output format (fixed or exponential)
C     Add variables for format statements based on output format (fixed or exponential)      
      CHARACTER CFMT*5   
      CHARACTER*300 FMT9013,FMT9045,FMT9047  ! format statements to match former labels
...
C     8/8/2025, 8/22/2025 CRT: D138 EXP Format
C     Initialize concentration output format to fixed
C     Set to 'EXP' if specified by the user   
C     Create format statement accordingly      
      CFMT = "F13.5" 
      IF (FILE_FORMAT == 'EXP') CFMT = "E13.6"   
      
      write(FMT9013,'(A)') "(2X,F10.2,1X,'|',1X,9("//CFMT//"))"
      write(FMT9045,'(A)') "(6X,2(F12.2,2X),"//CFMT//")"   
      write(FMT9047,'(A)') "(2X,A12,': ',F12.2,2X,F10.2,2X,"//CFMT//")"
     ...
     &                                                    NCPP*NX)
                  ...
                  IF (NY .EQ. NPPY) THEN
                     DO K = 1+NRPP*(NY-1), NUMYPT(I)
                        ...
                        END IF
                        IF (NX .EQ. NPPX) THEN
                           WRITE(IOUNIT,FMT9013) YCOVAL, !8/22/2025 CRT: D138 EXP Format
...
C           Print Out The Coord. & Concentrations For Discrete Cart Receptors
     ...
     &                                  PERLBL(ITYP)
                  ...
                  IF (MOD(INDC,2) .NE. 0) THEN
                     WRITE(BUF132(1:60),FMT9045) AXR(IREC),
     &                     AYR(IREC), ANNVAL(IREC,IGRP,ITYP)
                  ELSE
                     WRITE(BUF132(61:120),FMT9045) AXR(IREC),
     &                     AYR(IREC), ANNVAL(IREC,IGRP,ITYP)
...
C           Print Out The Coord. & Concentrations For Discrete Polar Receptors
     ...
     &                                  PERLBL(ITYP)
                  ...
                  END IF
                  IF (MOD(INDC,2) .NE. 0) THEN
                     WRITE(BUF132(1:65),FMT9047) SRCID(IREF(IREC)),
     &                               DIST, DIR, ANNVAL(IREC,IGRP,ITYP)
                  ELSE
                     WRITE(BUF132(66:130),FMT9047) SRCID(IREF(IREC)),
     &                               DIST, DIR, ANNVAL(IREC,IGRP,ITYP)
                     WRITE(IOUNIT,9090) BUF132
                     WRITE(BUF132,9095)
                  END IF
...
C     End Source Group Loop
 ...
 9010 FORMAT(66(' -')/)
c 9013 FORMAT(2X,F10.2,1X,'|',1X,9(F13.5)) !8/22/2025, CRT: D138 EXP Format
...
C 8/8/2025, CRT: D138 EXP Format
C 9045 FORMAT(6X,2(F12.2,2X),F13.5)
C 9047 FORMAT(2X,A12,': ',F12.2,2X,F10.2,2X,F13.5)
...
C     CRT, 10/24/2028: D138 EXP Format
C     Add variables for concentration output format (fixed or exponential)
C     Add variables for format statements based on output format (fixed or exponential)
      CHARACTER CFMT*5
      CHARACTER*300 FMT9001          ! format statements to match former labels

C     Variable Initialization
      MODNAM = 'PRTNHI'

C     10/24/2025, CRT: D138 EXP Format
C     Initialize concentration output format to fixed
C     Set to 'EXP' if specified by the user   
C     Create format statement accordingly      
      CFMT = "F17.5" 
      IF (FILE_FORMAT == 'EXP') CFMT = "E17.6"
      
      write(FMT9001,'(A)') "(3X,'EVENTPER',1X,A10,1X,"
     &   //"I3,2X,A8,3X,I8.8,1X,"//CFMT//")"   
...
C                    KAVE > 24 Means MONTH Average; Write Out as 72 (=720/10)
     ...
     &                       'H',IT1,'H',KWRT,IGRP_TMP
                  END IF
                  WRITE(ITEVUT,FMT9001) NAMEEV, KAVE(IAVE),
...
C     First Output Type Only (i.e., ITYP = 1)
 ...
 9000 FORMAT('EV STARTING')
c9001 FORMAT(3X,'EVENTPER',1X,A10,1X,I3,2X,A8,3X,I8.8,1X,E17.6)
c9001 FORMAT(3X,'EVENTPER',1X,A10,1X,I3,2X,A8,3X,I8.8,1X,F17.5)
...
C     8/22/2025, CRT: D138 EXP Format
C     Add variables for concentration output format (fixed or exponential)
C     Add variables for format statements based on output format (fixed or exponential)      
      CHARACTER CFMT*5   
      CHARACTER*300 FMT9013,FMT9047  ! format statements to match former labels
...
C     Create format statement accordingly      
      ...
      write(FMT9013,'(A)') "(1X,F9.1,1X,'|',5("//CFMT//",A1,"
     &                   //"'(',I8.8,')',:))"
     
      write(FMT9047,'(A)') "(1X,A12,': ',F11.2,1X,F10.2,1X,"
     &                   //CFMT//",A1,1X,'(',I8.8,')')"
...
C           Print Out The Coord. & Concentrations For Discrete Cart Receptors
     ...
     &                                  OUTLBL(ITYP)
                  ...
                  END IF
                  IF (MOD(INDC,2) .NE. 0) THEN
                     IF (FILE_FORMAT .EQ. 'EXP') THEN ! WSP 4/17/2025 D138_Scientific_Notation 
                        WRITE(BUF132(1:65),9046) AXR(IREC), AYR(IREC), ! WSP 4/17/2025 D138_Scientific_Notation 
     &                     HIVALU(IREC,IHNUM,IGRP,IAVE,ITYP),
     &                     HCLMSG(IREC,IHNUM,IGRP,IAVE,ITYP),
     &                     NHIDAT(IREC,IHNUM,IGRP,IAVE,ITYP)
                     ELSE ! WSP 4/17/2025 D138_Scientific_Notation 
                     WRITE(BUF132(1:65),9045) AXR(IREC), AYR(IREC),
     &                     HIVALU(IREC,IHNUM,IGRP,IAVE,ITYP),
     &                     HCLMSG(IREC,IHNUM,IGRP,IAVE,ITYP),
     &                     NHIDAT(IREC,IHNUM,IGRP,IAVE,ITYP)
                     END IF ! WSP 4/17/2025 D138_Scientific_Notation 
                  ELSE
                     IF (FILE_FORMAT .EQ. 'EXP') THEN ! WSP 4/17/2025 D138_Scientific_Notation 
                        WRITE(BUF132(66:130),9046) AXR(IREC), AYR(IREC), ! WSP 4/17/2025 D138_Scientific_Notation 
     &                     HIVALU(IREC,IHNUM,IGRP,IAVE,ITYP),
     &                     HCLMSG(IREC,IHNUM,IGRP,IAVE,ITYP),
     &                     NHIDAT(IREC,IHNUM,IGRP,IAVE,ITYP)
                     ELSE  ! WSP 4/17/2025 D138_Scientific_Notation 
...
C           Print Out The Coord. & Concentrations For Discrete Polar Receptors
     ...
     &                                  OUTLBL(ITYP)
                  ...
                  END IF
                  IF (MOD(INDC,2) .NE. 0) THEN
                     WRITE(BUF132(1:66),FMT9047) SRCID(IREF(IREC)), !8/22/2025, CRT: D138 EXP Format
     &                       DIR, HIVALU(IREC,IHNUM,IGRP,IAVE,ITYP),
     &                      DIST, HCLMSG(IREC,IHNUM,IGRP,IAVE,ITYP),
     &                            NHIDAT(IREC,IHNUM,IGRP,IAVE,ITYP)
                  ELSE
                     WRITE(BUF132(67:132),FMT9047) SRCID(IREF(IREC)), !8/22/2025, CRT: D138 EXP Format
     &                       DIR, HIVALU(IREC,IHNUM,IGRP,IAVE,ITYP),
     &                      DIST, HCLMSG(IREC,IHNUM,IGRP,IAVE,ITYP),
     &                            NHIDAT(IREC,IHNUM,IGRP,IAVE,ITYP)
                     WRITE(IOUNIT,9090) BUF132
...
c 9013 FORMAT(1X,F9.1,1X,'|',5(F13.5,A1,'(',I8.8,')',:)) !8/22/2025, CRT: D138 EXP Format
 ...
 9046 FORMAT(6X,2(F11.2,2X),E13.6,A1,1X,'(',I8.8,')') ! WSP 4/17/2025 D138_Scientific_Notation 
c 9047 FORMAT(1X,A12,': ',F11.2,1X,F10.2,1X,F13.5,A1,1X,'(',I8.8,')') ! CRT 8/22/2025 D138_Scientific_Notation
...
C     Add variables for format statements based on output format (fixed or exponential)
      CHARACTER CFMT*5                        
      CHARACTER*300 FMT2,FMT3  ! format statements to match former labels
...
C     Create format statement accordingly      
      ...
      write(FMT2,'(A)') "(1X,I4,'.',1X,"//CFMT//",A1,'(',I8.8,') AT',"
     &         //"1X,'(',F10.2,', ',F10.2,')  ',A2,5X,"
     &         //"I4,'.',1X,"//CFMT//",A1,'(',I8.8,') AT',1X,"
     &         //"'(',F10.2,', ',F10.2,')  ',A2)"  

      write(FMT3,'(A)') "(1X,I4,'.',1X,"//CFMT//",A1,'(',I8.8,') AT',"
     &          //"1X,'(',F10.2,', ',F10.2,')  ',A2)"    
...
C                 Set Start Row of Loop for This Page, JSTRT
                  ...
                  DO J = JSTRT, JSTRT+NROWS-1
                     ...
                     ELSE
                        ...
                        NTY2 = RECTYP(KMAX2)
                     END IF
                     WRITE(IOUNIT,FMT2) J, RMXVAL(J,IGRP,IAVE,ITYP),
...
C                 Odd Number of Max Values - Print Out Last Value
                  ...
                  ELSE
                     ...
                     NTY1 = RECTYP(KMAX1)
                  END IF
                  WRITE(IOUNIT,FMT3) J, RMXVAL(J,IGRP,IAVE,ITYP),
     &               MCLMSG(J,IGRP,IAVE,ITYP), MXDATE(J,IGRP,IAVE,ITYP),
     &                  XR1, YR1, NTY1
               END IF
            ELSE
...
C           WRITE Out Explanation of Receptor Types
 ...
 1    FORMAT(1X,'RANK',8X,A4,4X,'(YYMMDDHH) AT',6X,
     ...
     &           /66(' -'))
C 8/8/2025, CRT: D138 EXP Format
C 2    FORMAT(1X,I4,'.',1X,F13.5,A1,'(',I8.8,') AT',1X,
C    &          '(',F10.2,', ',F10.2,')  ',A2,5X,
C    &          I4,'.',1X,F13.5,A1,'(',I8.8,') AT',1X,
C    &          '(',F10.2,', ',F10.2,')  ',A2)
C 3    FORMAT(1X,I4,'.',1X,F13.5,A1,'(',I8.8,') AT',1X,
C    &          '(',F10.2,', ',F10.2,')  ',A2)
 9011 FORMAT(/40X,'** ',A4,' OF ',A8,' IN ',A40,' **'/)
...
C     4/18/2025, CRT: D138 EXP Format
C     Add variables for concentration output format (fixed or exponential)
C     Add variables for format statements based on output format (fixed or exponential)
      CHARACTER CFMT*5                        
      CHARACTER*300 FMT1002,FMT1003,FMT1004  ! format statements to match former labels
      CHARACTER*300 FMT1005,FMT1012,FMT1013  ! format statements to match former labels
      CHARACTER*300 FMT1014,FMT1015          ! format statements to match former labels
...
C     Create format statement accordingly      
      CFMT = "F14.5" 
      IF (FILE_FORMAT == 'EXP') CFMT = "E14.6"
      
      write(FMT1002,'(A)') "(A8,' HIGH ',A5,"   
     &   //"' HIGH VALUE IS',"//CFMT
     &   //",A1,' ON ',"
     &   //"I8.8,': AT ','(',"
     &   //"2(F11.2,', '),"
     ...
     &   //"2(F8.2,', '),F7.2,')')"
      ...
      write(FMT1012,'(A)') "(/A8,A5,' HIGHEST VALUE IS',"//CFMT
     &   //",' AT ','(',2(F11.2,', '),2(F8.2,', '),F7.2,')'"
     &   //",2X,A2,2X,A8)"
      
      write(FMT1013,'(A)') "(8X,A5,' HIGHEST VALUE IS',"//CFMT  
     &   //",' AT ','(',2(F11.2,', '),2(F8.2,', '),F7.2,')'"
     &   //",2X,A2,2X,A8)"

      write(FMT1014,'(A)') "(/A8,A5,' HIGHEST VALUE IS',"//CFMT
     &   //",' AT ','(',2(F11.2,', '),2(F8.2,', '),F7.2,')')"  
     
       write(FMT1015,'(A)') "(8X,A5,' HIGHEST VALUE IS',"//CFMT  
     &   //",' AT ','(',2(F11.2,', '),2(F8.2,', '),F7.2,')')" 
...
C        Calculate Number of Groups Per Page, NGPP
     ...
     &                           CHIDEP(3,ITYP)
            ...
            DO IVAL = 1, NHIANN
               ...
               IF (IVAL .EQ. 1 .AND. INDMX .NE. 0) THEN
                  WRITE(IOUNT,FMT1012) GRPID(IGRP), RANK(IVAL), ! 4/18/2025, CRT: D138 EXP Format
     ...
     &                  RECTYP(INDMX), NETID(INDMX)
               ELSE IF (IVAL .EQ. 1 .AND. INDMX .EQ. 0) THEN
                  ...
                  AZHIL1 = 0.0D0
                  AZFLG1 = 0.0D0
                  WRITE(IOUNT,FMT1014) GRPID(IGRP), RANK(IVAL), ! 4/18/2025, CRT: D138 EXP Format
     ...
     &               AZFLG1
               ELSE IF (INDMX .EQ. 0) THEN
                  ...
                  AZHIL1 = 0.0D0
                  AZFLG1 = 0.0D0
                  WRITE(IOUNT,FMT1015) RANK(IVAL), ! 4/18/2025, CRT: D138 EXP Format
     &               AMXVAL(IVAL,IGRP,ITYP), AXR1, AYR1, AZELV1, AZHIL1,
     &               AZFLG1
               ELSE
                  WRITE(IOUNT,FMT1013) RANK(IVAL), ! 4/18/2025, CRT: D138 EXP Format
     &                  AMXVAL(IVAL,IGRP,ITYP), AXR(INDMX), AYR(INDMX),
     &                  AZELEV(INDMX), AZHILL(INDMX), AZFLAG(INDMX),
     &                  RECTYP(INDMX), NETID(INDMX)
               END IF
...
C ---          Assign character label for rank
     ...
     &                                 CHIDEP(2,ITYP),CHIDEP(3,ITYP)
                  ...
                  IF (INDLOC .EQ. 0) THEN
                     ...
                     WRITE(IOUNT,trim(FMT1004)) GRPID(IGRP), CHRVAL, ! 4/18/2025, CRT: D138 EXP Format
     ...
     &                  XR2, YR2, ZE2, ZH2, ZF2
                  ELSE
                     ...
                     WRITE(IOUNT,trim(FMT1002)) GRPID(IGRP), CHRVAL, ! 4/18/2025, CRT: D138 EXP Format
     &                  HMAX(I,IGRP,IAVE,ITYP),
     &                  HMCLM(I,IGRP,IAVE,ITYP),
     &                  HMDATE(I,IGRP,IAVE,ITYP),
     ...
     &                  NETID(INDLOC)
               ...
               ELSE
                  IF (INDLOC .EQ. 0) THEN
                     ...
                     WRITE(IOUNT,FMT1005) CHRVAL, ! 4/18/2025, CRT: D138 EXP Format
     &                  HMAX(I,IGRP,IAVE,ITYP),
     &                  HMCLM(I,IGRP,IAVE,ITYP),
     &                  HMDATE(I,IGRP,IAVE,ITYP),
     &                  XR2, YR2, ZE2, ZH2, ZF2
                  ELSE
                     ...
                     WRITE(IOUNT,FMT1003) CHRVAL, ! 4/18/2025, CRT: D138 EXP Format
     &                  HMAX(I,IGRP,IAVE,ITYP),
     &                  HMCLM(I,IGRP,IAVE,ITYP),
     &                  HMDATE(I,IGRP,IAVE,ITYP),
     &                  XR2, YR2, ZE2, ZH2, ZF2, RECTYP(INDLOC),
...
C     End loop through averaging periods
      END DO
C 4/18/2025, CRT: D138 EXP Format
C                 Format labels replaced with string variables above
c 1002 FORMAT(A8,' HIGH ',A5,' HIGH VALUE IS',F14.5,A1,' ON ',
c     &       I8.8,': AT ','(',2(F11.2,', '),2(F8.2,', '),F7.2,')',
c     &       2X,A2,2X,A8)
c 1003 FORMAT(8X,' HIGH ',A5,' HIGH VALUE IS',F14.5,A1,' ON ',
c     &       I8.8,': AT ','(',2(F11.2,', '),2(F8.2,', '),F7.2,')',
c     &       2X,A2,2X,A8)
c 1004 FORMAT(A8,' HIGH ',A5,' HIGH VALUE IS',F14.5,A1,' ON ',
c     &       I8.8,': AT ','(',2(F11.2,', '),2(F8.2,', '),F7.2,')')
c 1005 FORMAT(8X,' HIGH ',A5,' HIGH VALUE IS',F14.5,A1,' ON ',
c     &       I8.8,': AT ','(',2(F11.2,', '),2(F8.2,', '),F7.2,')')
c     
c 1012 FORMAT(/A8,A5,' HIGHEST VALUE IS',F14.5,' AT ',
c     &       '(',2(F11.2,', '),2(F8.2,', '),F7.2,')',2X,A2,2X,A8)
c 1013 FORMAT(8X,A5,' HIGHEST VALUE IS',F14.5,' AT ',
c     &       '(',2(F11.2,', '),2(F8.2,', '),F7.2,')',2X,A2,2X,A8)
c 1014 FORMAT(/A8,A5,' HIGHEST VALUE IS',F14.5,' AT ',
c     &       '(',2(F11.2,', '),2(F8.2,', '),F7.2,')')
c 1015 FORMAT(8X,A5,' HIGHEST VALUE IS',F14.5,' AT ',
c     &       '(',2(F11.2,', '),2(F8.2,', '),F7.2,')')
...
C     Add variables for concentration output format (fixed or exponential)
C     Add variables for format statements based on output format (fixed or exponential)     
      CHARACTER*300 FMT1011,FMT1012,FMT1013,FMT1014  ! format statements to match former labels
...
C     1024/2025, CRT: D138 EXP Format
C     Initialize concentration output format to fixed
C     Set to 'EXP' if specified by the user   
C     Create format statement accordingly  
      FMT1011 =   "(/'OU STARTING',/'   EVENTOUT  ',A6,"
      IF (FILE_FORMAT == 'EXP') THEN
         FMT1011 = trim(FMT1011)//"/'   FILEFORM EXP',"
      END IF
      FMT1011 =    trim(FMT1011)//"/'OU FINISHED')"
      
      FMT1012 =  "(/' OU STARTING',/'    EVENTOUT  ',A6,"
      IF (FILE_FORMAT == 'EXP') THEN
         FMT1012 = trim(FMT1012)//"/'    FILEFORM EXP',"
      END IF
      FMT1012 =    trim(FMT1012)//"/' OU FINISHED')"
      
      FMT1013 = "(/'  OU STARTING',/'     EVENTOUT  ',A6,"
      IF (FILE_FORMAT == 'EXP') THEN
         FMT1013 = trim(FMT1013)//"/'     FILEFORM EXP',"
      END IF
      FMT1013 =    trim(FMT1013)//"/'  OU FINISHED')"
      
      FMT1014 ="(/'    OU STARTING',/'      EVENTOUT  ',A6,"
      IF (FILE_FORMAT == 'EXP') THEN
         FMT1014 = trim(FMT1014)//"/'       FILEFORM EXP',"
      END IF
      FMT1014 =    trim(FMT1014)//"/'    OU FINISHED')"
...
C     Write OU Pathway Images to EVENT File, Allowing For Column Shift
      IF (LOCB(1) .EQ. 1) THEN
         WRITE(IEVUNT,FMT1011) EVPARM
      ELSE IF (LOCB(1) .EQ. 2) THEN
         WRITE(IEVUNT,FMT1012) EVPARM
      ELSE IF (LOCB(1) .EQ. 3) THEN
         WRITE(IEVUNT,FMT1013) EVPARM
      ELSE IF (LOCB(1) .EQ. 4) THEN
         WRITE(IEVUNT,FMT1014) EVPARM
      END IF

      CLOSE(UNIT=IEVUNT)

C 4/18/2025, CRT: D138 EXP Format
C 1011 FORMAT(/'OU STARTING',
C     &       /'   EVENTOUT  ',A6,
C     &       /'OU FINISHED')
C 1012 FORMAT(/' OU STARTING',
C     &       /'    EVENTOUT  ',A6,
C     &       /' OU FINISHED')
C 1013 FORMAT(/'  OU STARTING',
C     &       /'     EVENTOUT  ',A6,
C     &       /'  OU FINISHED')
C 1014 FORMAT(/'   OU STARTING',
C     &       /'      EVENTOUT  ',A6,
C     &       /'   OU FINISHED')
...
C     Add variables for format statements based on output format (fixed or exponential)
      CHARACTER CFMT*5                        
      CHARACTER*300 FMT1901,FMT1902,FMT1903,FMT1904  ! format statements to match former labels
...
C     Create format statement accordingly      
      ...
      write(FMT1901,'(A)') "(3X,'EVENTPER',1X,A10,1X,I3,"
     &   //"2X,A8,3X,I8.8,1X,"//CFMT//")" 
     
      write(FMT1902,'(A)') "(4X,'EVENTPER',1X,A10,1X,I3,"
     &   //"2X,A8,3X,I8.8,1X,"//CFMT//")" 
     
      write(FMT1903,'(A)') "(5X,'EVENTPER',1X,A10,1X,I3,"
     &   //"2X,A8,3X,I8.8,1X,"//CFMT//")" 
     
      write(FMT1904,'(A)') "(6X,'EVENTPER',1X,A10,1X,I3,"
     &   //"2X,A8,3X,I8.8,1X,"//CFMT//")" 
...
C                    Write EVENTPER & EVENTLOC Cards, Allowing for Col. Shift
                     IF (LOCB(1) .EQ. 1) THEN
                        WRITE(IEVUNT,FMT1901) NAMEEV,IAVEP,GID,KDATE,
     &                                        CONC1
                        WRITE(IEVUNT,1911) NAMEEV, XR2, YR2, ZE2, ZH2,
     &                                     ZF2
                     ELSE IF (LOCB(1) .EQ. 2) THEN
                        WRITE(IEVUNT,FMT1902) NAMEEV,IAVEP,GID,KDATE,
     &                                        CONC1
                        WRITE(IEVUNT,1912) NAMEEV, XR2, YR2, ZE2, ZH2,
     &                                     ZF2
                     ELSE IF (LOCB(1) .EQ. 3) THEN
                        WRITE(IEVUNT,FMT1903) NAMEEV,IAVEP,GID,KDATE,
     &                                        CONC1
                        WRITE(IEVUNT,1913) NAMEEV, XR2, YR2, ZE2, ZH2,
     &                                     ZF2
                     ELSE IF (LOCB(1) .EQ. 4) THEN
                        WRITE(IEVUNT,FMT1904) NAMEEV,IAVEP,GID,KDATE,
     &                                        CONC1
...
C 4/18/2025, CRT: D138 EXP Format
C                 Format labels replaced with string variables above
c 1901 FORMAT(3X,'EVENTPER',1X,A10,1X,I3,2X,A8,3X,I8.8,1X,F17.5)
c 1902 FORMAT(4X,'EVENTPER',1X,A10,1X,I3,2X,A8,3X,I8.8,1X,F17.5)
c 1903 FORMAT(5X,'EVENTPER',1X,A10,1X,I3,2X,A8,3X,I8.8,1X,F17.5)
c 1904 FORMAT(6X,'EVENTPER',1X,A10,1X,I3,2X,A8,3X,I8.8,1X,F17.5)
...
C ---                   Assign character label for rank
     ...
     &                                  PERLBL(ITYP)
                  ...
                  END IF
                  IF (MOD(INDC,2) .NE. 0) THEN
                     WRITE(BUF132(1:60),FMT9045) AXR(IREC), AYR(IREC),
     &                     SUMHNH(IREC,IGRP,N)
                  ELSE
                     WRITE(BUF132(61:120),FMT9045) AXR(IREC), AYR(IREC),
     &                     SUMHNH(IREC,IGRP,N)
                     WRITE(IOUNIT,9090) BUF132
                     WRITE(BUF132,9095)
                  END IF
...
C     First Output Type Only (i.e., ITYP = 1)
 ...
 9011 FORMAT(/40X,'** ',A4,' OF ',A8,' IN ',A40,' **'/)
 9010 FORMAT(66(' -')/)
c 9013 FORMAT(2X,F10.2,1X,'|',1X,9(F13.5))
 9016 FORMAT(3X,' Y-COORD  |',48X,'X-COORD (METERS)')
 9017 FORMAT(3X,' (METERS) |',1X,9(1X,F12.2,:))
 9018 FORMAT(3X,'DIRECTION |',48X,'DISTANCE (METERS)')
 9019 FORMAT(3X,'(DEGREES) |',1X,9(1X,F12.2,:))
...
C     Add variables for format statements based on output format (fixed or exponential)
      CHARACTER CFMT*5                        
      CHARACTER*300 FMT1012,FMT1013  ! format statements to match former labels
      CHARACTER*300 FMT1014,FMT1015  ! format statements to match former labels
...
C ---             Assign character label for rank
     ...
     &                               CHIDEP(3,ITYP)
               ...
               DO IVAL = 1, NMXPM
                  ...
                  IF (IVAL .EQ. 1 .AND. INDMX .NE. 0) THEN
                     WRITE(IOUNT,FMT1012) GRPID(IGRP), RANK(IVAL), ! 8/22/2025, CRT: D138 EXP Format
     ...
     &                     RECTYP(INDMX), NETID(INDMX)
                  ELSE IF (IVAL .EQ. 1 .AND. INDMX .EQ. 0) THEN
                     ...
                     AZHIL1 = 0.0D0
                     AZFLG1 = 0.0D0
                     WRITE(IOUNT,FMT1014) GRPID(IGRP), RANK(IVAL), ! 8/22/2025, CRT: D138 EXP Format
     ...
     &                   AZHIL1, AZFLG1
                  ELSE IF (INDMX .EQ. 0) THEN
                     ...
                     AZHIL1 = 0.0D0
                     AZFLG1 = 0.0D0
                     WRITE(IOUNT,FMT1015) RANK(IVAL), ! 8/22/2025, CRT: D138 EXP Format
     &                   MXPMVAL(IVAL,IGRP,N), AXR1, AYR1, AZELV1, 
     &                   AZHIL1, AZFLG1
                  ELSE
                     WRITE(IOUNT,FMT1013) RANK(IVAL), ! 8/22/2025, CRT: D138 EXP Format
     &                     MXPMVAL(IVAL,IGRP,N), AXR(INDMX), AYR(INDMX),
     &                     AZELEV(INDMX), AZHILL(INDMX), AZFLAG(INDMX),
     &                     RECTYP(INDMX), NETID(INDMX)
                  END IF
...
C
C        MODIFIED:   To add the Aircraft Engine Parameter in VOLUME/AREA
C                    Source for Aircraft source group.
...
C**  Added for Aircraft Plume Rise; UNC-IE
     ...
     &                             AFTSRC(ISRC) .EQ. 'Y') THEN
                  ...
                  ASRCANGLE(ISRC) = AASRCANGLE(IHR_NDX,IYR_NDX,ISRC)
                  ARPWR(ISRC) = AARPWR(IHR_NDX,IYR_NDX,ISRC)
C**  End Aircarft Plume Rise insert; April 2023
               ELSE IF (SRCTYP(ISRC) .EQ. 'VOLUME' .AND.
...
c      FUNITARRAY(FUNITIND) = IMXDCUNT(IGRP1)
c      FNAMEARRAY(FUNITIND) = MAXDCONT_FILE(IGRP1)
c      FTYPARRAY(FUNITIND) = 'IMXDCUNT'

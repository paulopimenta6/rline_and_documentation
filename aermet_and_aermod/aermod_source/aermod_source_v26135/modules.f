...
CRLM increase IERRN from 509 to 515 for added VBARRIER E/W messages WSP 20250806
CRLM increase IERRN from 506 to 509 for added BAREDGE E/W messages WSP 20240911
CRLM increase IERRN from 500 to 506 for added SBARRIER E/W messages WSP 20240619
CRLM  IKN increased from 120 to 121 to add SBARRIER WSP 20240507
CRLM  IKN increased from 121 to 122 to add VBARRIER WSP 20250225
...
C     &                      IERRN=317
CCRCO 2/18/2021 - need to check IKN and IERRN after merge
C      INTEGER, PARAMETER :: IFMAX=150, IKN=117, ISTRG=512, ILEN_FLD=200,
C     &                      IERRN=500
      INTEGER, PARAMETER :: IFMAX=150, IKN=122, ISTRG=512, ILEN_FLD=200,
     &                      IERRN=515
...
C     &                    R00 = 2.0D0, ALPHAM = 0.10D0,          ! Removed R00 on January 13th 2026 as it is computed within ARISE.f
     &                    ALPHAM = 0.10D0,                              
     &                    PAA = 1.013D+5, RAA = 287.058D0,
     &                    HFUEL = 4.3D+7, CPA = 1003.0D0,
     &                    NENG = 2.0D0              ! Added NENG on January 13th 2026
C    Description of the variables/constants used in ARISE
C    R00    = Aircraft Engine Exhaust Radius (m)(as Initial Plume Radius)
C    ALPHAM = Entrainment Constant (Momentum Part of ARISE)
C    PAA    = Ambient Pressure (N/m2)
C    RAA    = Gas Constant of Air (J/kg-K)
C    HFUEL  = Heating Value of Fuel (J/kg)
C    CPA    = Specific Heat of Air (J/kg-K)
C    NENG   = Number of Engines                                   ! Added on January 13th 2026
...
C     Wood 10/10/22 added GRID_WS debug RLINEDBUNT_WS
     ...
     &           SBRDBUNT,  !added for SBARRIER input debug 20240716 WSP
     &           EDGEDBUNT, !added for BAREDGE debug 20240910 WSP
     &           VBRDBUNT   !VBARRIER debug 2025 05 08 WSP
...
C**   RLINEDBUNT_WS = Debug Output File for RLINE sourcesgridded wind speed profile (Init. to 32)
C**   ARCFTDBG = Debug Output File for Aircraft Plume Rise (Init. to 32)
C**   SBRDBUNT = Debug output file for SBARRIER inputs (Init. to 47)
C**   EDGEDBUNT = Debug output file for BAREDGE processing (Init. to 48)
C**   VBRDBUNT = Debug output file for VBARRIER processing (Init. to 49)
...
CCRT 3/4/2026 D233 Case-sensitive Filenames 
      CHARACTER (LEN=ILEN_FLD) :: FIELD1

      INTEGER ::  LOCB(IFMAX), LOCE(IFMAX), IFC, IDC1, IPNUM, IPPNUM
      DIMENSION   FIELD(IFMAX), KEYWD(IKN), RUNST(ISTRG)
CCRT 3/4/2026 D233 Case-sensitive Filenames
      DIMENSION   FIELD1(IFMAX)
...
CWSP       INTEGER :: ILINE, IQLINE, IBLINE, IOLINE, INOXLINE, IERROR, IFTL, 
      INTEGER :: ILINE, IBLINE, IOLINE, INOXLINE, IERROR, IFTL, ! Removed IQLINE to make it an array for 1/24/2024 D175 HOUREMIS Filename D121 Hourly Emissions File - Ending
...
CCRT  D138  Add PollCrit
     ...
     &        PollCrit, ! CRT, 2/11/2026, D138 Scientific Notation, If true, criteria pollutant
     &        BARDBG, !added for SBARRIER input debug 20240716 WSP
     &        BAREDGE, EDGEDBG, !added for barrier edge effect 20240814 WSP
     &        VBRDBG !VBARRIER 20250508 WSP
...
CCRT 8/9/2023, D176 - COARE Beta Check - add logical to indicate met is from AERMET/COARE (L_COARE)
...
CCRT  D113  Add Sidewash Debug SWFIL
     ...
     &                            SBARDBGFIL, !added for SBARRIER input debug 20240716 WSP
     &                            EDGEDBGFIL, !added for BAREDGE debug 20240910 WSP
     &                            VBARDBGFIL  ! VBARRIER debug 20250508 WSP
...
C --- Declare arrays for O3SECTs and ANOXSECTs by hour for EVENT processing
     ...
     &            NSBR,                ! For SBARRIER inputs (WSP) 20240424  
     &            NVBR                 ! For VBARRIER inputs WSP 20250225
C**  NAFT = Maximum Number of Airports (Here, it is fixed as 1 in ARISE)
C    NSBR = number of BARID inputs with SBARRIER (WSP) 20240424 
C    NVBR = number of BARID inputs with VBARRIER (WSP) 20250225
...
C     associated with debugopts D144 WSP 10/11/22
      INTEGER :: DEBUG_COUNT = 0, CURRENT_DEBUGOPT = 0, DEBUG_INDEX = 1,
     &           IFC_DEBUG = 2, !Initalize with 2 to include CO and DEBUGOPT
     &           LOCB_DEBUG(IFMAX), LOCE_DEBUG(IFMAX)
      
CCRT  3/6/2026, D144 - Added DEBUG_FLS1 to preserve case      
C      CHARACTER (LEN=ILEN_FLD) :: DEBUG_FLS
      CHARACTER (LEN=ILEN_FLD) :: DEBUG_FLS, DEBUG_FLS1
C      DIMENSION   DEBUG_FLS(IFMAX)
      DIMENSION   DEBUG_FLS(IFMAX), DEBUG_FLS1(IFMAX)
...
C     are validated.
     ...
     &          SBARID*12 ! Added for SBARRIER input WSP 2024 04 25
CRLM     &          VBARID*12  ! Added for VBARRIER input WSP 20250225
     

C**  AFTSRC = Used in SOurce Pathway to read the Aircraft Source IDs
C**  AFTID  = Used in COmmon Pathway for Airport ID (this is not used in
C**             calculation)

      LOGICAL :: SBARDCL !Added for SBARRIER input WSP 2024 04 25
      LOGICAL :: VBARSET !Added for VBARRIER input calculations WSP 2025 02 26
      INTEGER :: IBARID, INUMSEG !Added for SBARRIER input WSP 2024 05 08
CRLM  SBARDCL  logical switch to calculate DCL for SBARRIER inputs
CRLM  VBARDCL  logical switch to calculate DCL for VBARRIER inputs
CRLM  IBARID   BARID counter used in soset.f/SBARRIER_INPUTS
CRLM  INUMSEG  segment counter used in soset.f/SBARRIER_INPUTS    
...
C*--- Added for AREA source meander !Wood 4/14/22
      LOGICAL :: L_APLUME
...
C**  Added for Aircraft Plume Rise; UNC-IE
     ...
     &                     HRSRCANGLE, HRRPWR
      DOUBLE PRECISION :: PLUME_SPREAD           ! Added on January 6th 2025 for plume spread factor in aircraft plume rise; UNC-IE
C**  End Aircraft Plume Rise insert; April 2023
...
C---  Add tubulence dissipation rate (epsilon) profile for PVMRM/GRSM
     &                    GRIDEPS(MXGLVL),
C     Added  RL_GRIDWS array for RLINE windspeeds when used in CRITDS - WOOD 6-28-2022
     &                    RL_GRIDWS(MXGLVL,3)                                                                             
...
C**   RL_GRIDWS = RLine Gridded wind speed (m/s) Wind Speed Array for determining critical height Added by - WOOD 6-28-2022                                                                                                                             
...
C**  Added for Aircraft Plume Rise; UNC-IE
     &           NAFTSRC
C**  NAFTSRC = Number of Aircraft Sources
C**  End Aircraft Plume Rise insert; April 2023
...
C**  Added for Aircraft Plume Rise; UNC-IE
C---- AIRCRAFT Plume Rise Modification -----------------------------
     &                    MFUEL, THRUST, VAA, AFR, BYPR, SRCANGLE,
     &                    RPWR
C**  Aircraft Engine Parameters
C**    MFUEL    = Fuel Burn Rate (kg/s)
C**    THRUST   = Thrust (N)
C**    VAA      = Aircraft Velocity (m/s)
C**    AFR      = Air Fuel Ratio
C**    BYPR     = Engine By Pass Ratio
C**    SRCANGLE = Source Angle for Airborne Sources (degree)
C**    RPWR     = Rated Power for Shaft-based Engines (kilo-Watt)
C**  End Aircraft Plume Rise insert; April 2023 

C**  Added for Aircraft Plume Rise; UNC-IE
C---- AIRCRAFT Plume Rise Modification -----------------------------
      DOUBLE PRECISION :: VE, QEE, TE, RHOE, R00
C**  Aircraft Plume Rise Parameters
C**    MAIR     = Mass Flow Rate (kg/s)
C**    VE       = Exhaust Velocity (m/s)
C**    QEE      = Heat Rejection (W)
C**    TE       = Exhaust Temperature (K)
C**    RHOE     = Exhaust Density (kg/m3)
C**    R00      = Initial Plume Radius (m)
C**  End Aircraft Plume Rise insert; June 2025 
...
CWSP 10/1/2024 D158 Dynamic File unit Starting
C     &     PDEPDBG/30/, AWMADWDBUNT/931/, RLINEDBUNT/932/,
C     &     PLATFMDBUNT/933/, RDISPUNT/ 3/,                       ! RDISPUNT is for RELDISP debug file for PVMRM
C     &     TTRMUNT/9937/, URBUNT/937/, URBUNT1/938/,BLPUNT/939/,
C     &     TTRM2TMP/9938, 9939, 9940/,URBUNT2/941/,
C     &     SWDBGUNT/8837/, ! Added for sidewash
C     &     RLINEDBUNT_WS/8932/, ! Added for the gridded wind speed profile in RLINE Wood 10/10/22
C     &     ARCFTDBG/32/,        ! Added for Aircraft Plume Rise; UNC-IE
C     &     HBPUNT/731/ ! Added for HBPDEBUG; Jan. 2023  
     &     PDEPDBG/30/, AWMADWDBUNT/34/, RLINEDBUNT/35/,
     &     PLATFMDBUNT/36/, RDISPUNT/ 3/,                       ! RDISPUNT is for RELDISP debug file for PVMRM
     &     TTRMUNT/43/, URBUNT/37/, URBUNT1/38/,BLPUNT/39/,
     &     TTRM2TMP/44, 45, 46/,URBUNT2/40/,
     &     SWDBGUNT/41/, ! Added for sidewash
     &     RLINEDBUNT_WS/42/, ! Added for the gridded wind speed profile in RLINE Wood 10/10/22
     &     ARCFTDBG/32/,        ! Added for Aircraft Plume Rise; UNC-IE
     &     HBPUNT/33/, ! Added for HBPDEBUG; Jan. 2023
CWSP 10/1/2024 D158 Dynamic File unit Ending 
     &     SBRDBUNT/47/, !added for SBARRIER input debug 20240716 WSP
     &     EDGEDBUNT/48/, !added for BAREDGE debug 20240910 WSP
     &     VBRDBUNT/49/ !VBARRIER debug 20250508 WSP

CWSP 9/16/2024 D158 Dynamic File unit  Store FUNIT, FILENAME, and FILETYPE in array to add to output as table - Starting
      INTEGER :: FUNITIND  ! Index value in array (initialized in aermod.f)
      INTEGER :: FUNITINX  ! File counter for output files (initialized in aermod.f)
      INTEGER, ALLOCATABLE :: FUNITARRAY(:)                   ! FUNIT
      CHARACTER(len=200), ALLOCATABLE :: FNAMEARRAY(:)        ! Filename
      CHARACTER(len=20), ALLOCATABLE :: FTYPARRAY(:)          ! File type
...
CWSP 1/24/2025 D175 HOUREMIS Filename D121 Hourly Emissions File - Starting 
      INTEGER :: HOUREMISIND                                  ! Index value in array (initialized in aermod.f)
      INTEGER :: HOUREMISINX                                  ! File counter for HOUREMIS files (initialized in aermod.f)
      INTEGER, ALLOCATABLE :: HOUREMISARRAY(:)                 ! HOUREMIS FUNIT
      INTEGER, ALLOCATABLE :: IQLINE(:)                       ! Line of HOUREMIS to read - formerly was an INTEGER
      CHARACTER(len=200), ALLOCATABLE :: HOUREMISNAMEARRAY(:) ! Filename
      CHARACTER(len=200), ALLOCATABLE :: HRSRCIDs(:)          ! SRCIDs expected in HOUREMIS file
...
C     WILL GET A WARNING MESSAGE AND THEY WILL BE IGNORED.      
     ...
     &   'SBARRIER', !Added for barrier input RLM WSP 20240507
     &   'VBARRIER'/ !Added for vegetative barrier RLM WSP 20250225

C**  ARCFTOPT  =  To identify the Aircraft Sources in COmmon Pathway
C**  ARCFTSRC  =  To identify the Aircraft Sources in SOurce Pathway
...
C               34 = ord_dwnw                      45 = hbpsrc
C               35 = noxsectr                      47 = platform
C               36 = noxvalue                      48 = arcftsrc       ! Added for Aircraft Plume Rise; UNC-IE
C               37 = nox_vals                      50 = finished
C               38 = nox_unit
C               39 = nox_file
C               40 = arcftopt                                          ! Added for Aircraft Plume Rise; UNC-IE
C               50 = finished
C               
...
C     Wood 6/3/22 D128 added AREAMNDR as an ALPHA option to CO pathway
      ECD = ECD+1
      ERRCOD(ECD)='134'
...
CRLM Adding 270 for barrier edge effects 20240911 WSP
      ECD = ECD+1
      ERRCOD(ECD)='270'
      ERRMSG(ECD)='Error allocating receptor edge zone array'
...
CRLM Adding 274-275 for barrier edge effects 20240911 WSP
      ECD = ECD+1
      ERRCOD(ECD)='274'
      ERRMSG(ECD)='SBARRIER Input required for BAREDGE'
      
      ECD = ECD+1
      ERRCOD(ECD)='275'
      ERRMSG(ECD)='SBARRIER not long enough (<2*6.5*Ht) for BAREDGE'
...
CRLM Adding 304-309 for RLINE Barrier Input processing WSP 20240619
CRLM Skipping 305, comment description below
      ECD = ECD+1
      ERRCOD(ECD)='304'
      ERRMSG(ECD)='Gap >1m between barrier segments '
      
      ECD = ECD+1
      ERRCOD(ECD)='306'
      ERRMSG(ECD)='Number of segments does not match input ' 

      ECD = ECD+1
      ERRCOD(ECD)='307'
      ERRMSG(ECD)='Segment(s) not parallel '   

      ECD = ECD+1
      ERRCOD(ECD)='308'
      ERRMSG(ECD)='Barrier does not meet overlap threshold '

      ECD = ECD+1
      ERRCOD(ECD)='309'
      ERRMSG(ECD)='Source length must be > 0m '      
...
CRLM Adding 311 for RLINE Barrier Input processing WSP 20240619     
      ECD = ECD+1
      ERRCOD(ECD)='311'
      ERRMSG(ECD)='Barrier distance from source >10*barrier_height ' 

CRLM Adding 312 for RLINE Barring Input processing WSP 20240730
      ECD = ECD+1
      ERRCOD(ECD)='312'
      ERRMSG(ECD)='Large debug file for # of barrier & source pairs'      
...
C JAT 5/13/2026 26XXX_DEV_BRANCH comment out 311 and 312 since they have been
c moved to 834 and 835
c      ECD = ECD+1
c      ERRCOD(ECD)='311'
c      ERRMSG(ECD)='Attempt to Define Duplicate HOUREMIS file for SRC:'
      
c      ECD = ECD+1
c      ERRCOD(ECD)='312'
c      ERRMSG(ECD)='Attempt to Define Duplicate HOUREMIS files'
CWSP 1/24/2024 D175 HOUREMIS Filename D121 Hourly Emissions File - Ending 
      ECD = ECD+1
      ERRCOD(ECD)='314'
...
CRLM Adding W 326 for Barrier Input processing WSP 20240810
      ECD = ECD+1
      ERRCOD(ECD)='326'
      ERRMSG(ECD)='Input barrier elevations >0 will be ignored.'      
      ...
      ECD = ECD+1
      ERRCOD(ECD)='338'
      ERRMSG(ECD)='Neg Emis Rate Invalid for ARM2/OLM/PVMRM/GRSM. Src: ' ! MKP D227
...
C     Vegetative barrier [BEGIN]
      ECD = ECD+1
      ERRCOD(ECD)='371'
      ERRMSG(ECD)='VBARRIER height out of range 2.0< HT <10.0'
      
      ECD = ECD+1
      ERRCOD(ECD)='372'
      ERRMSG(ECD)='VBARRIER width out of range 2.5< WT <13.0'
      
      ECD = ECD+1
      ERRCOD(ECD)='373'
      ERRMSG(ECD)='VBARRIER LAI out of range 4.0< LAI <10.92'
      
      ECD = ECD+1
      ERRCOD(ECD)='374'
      ERRMSG(ECD)='VBARRIER LAD out of range 0.55< LAD <3.75'
      
      ECD = ECD+1
      ERRCOD(ECD)='375'
      ERRMSG(ECD)='VBARRIER DCL on same side. Only closest used:'
      
      ECD = ECD+1
      ERRCOD(ECD)='376'
      ERRMSG(ECD)='VBARRIER ignored with RBARRIER or SBARRIER input.'
C     Vegetative barrier [END]  
...
C     CRT D176 COARE Beta Check - COARE used to process met      
      ECD = ECD+1
      ERRCOD(ECD)='422'
      ERRMSG(ECD)='Meteorological data processed with COARE in AERMET'
      
C     CRT D176 COARE Beta Check - BULKRN cannot be used with COARE      
      ECD = ECD+1
      ERRCOD(ECD)='423'
...
CCRT  4/5/2022 D091 Missing BLPINPUT Checks - Added Messages (507, 508, 509)
      ...
      ECD = ECD+1
      ERRCOD(ECD)='565'
C      ERRMSG(ECD)='Possible Conflict With Dynamically Allocated FUNIT' ! WSP 10/7/2024 D158 Dynamically Allocated FUNIT
      ERRMSG(ECD)='Duplicate Input FUNIT:' ! WSP 10/7/2024 D158 Dynamically Allocated FUNIT
      ...
      ECD = ECD+1
      ERRCOD(ECD)='595'
      ERRMSG(ECD)='EXP used w/DFAULT & criteria poll. Reset to FIX.'
...
C    D128 4/5/23: Added warning message when the AREA meander keyword is used without an AREA source
      ECD = ECD+1
      ERRCOD(ECD)='668'
...
C     Added error statement for RLINE barrier or depressed sources missing 
C     FLAT option Wood 10/10/22
      ECD = ECD+1
      ERRCOD(ECD)='713'
...
C ---   D161 Added error message for sources which NO2 options are not implemented 2/20/23 WSP
      ECD = ECD+1
      ERRCOD(ECD)='722'
...
C ---   D161 Added error message for sources which NO2 options are not implemented 4/21/23 CRT
      ECD = ECD+1
      ERRCOD(ECD)='726'
      ERRMSG(ECD)='ARM2 NO2 processing not currently configured for '
      
C     D164 2/21/23 WSP Message when SCREEN option is used with incompatible sources
      ECD = ECD+1
      ERRCOD(ECD)='731'
...
C     ERRMSG(ECD)='ARMRATIO in DFAULT range, 0.5 <= ARMRATIO <= 0.9 '

C     D157 CRT 5/31/2023 - Modify warning when ARMRATIO is outside of default range.
      ECD = ECD+1
      ERRCOD(ECD)='737'
      ERRMSG(ECD)='ARMRATIO outside of DFAULT range of 0.5 - 0.9 '
C     ERRMSG(ECD)='Non-Default ARMRATIO range, 0 < ARMRATIO <= 1 '

C --- D144 Added for when AREA and LINE debug optiosn are called together WSP 11/4/22; 
C --- D144 Changed number to 738, after merging with D134 (which contains D172 and D086. WSP 8/18/25
      ECD = ECD+1
      ERRCOD(ECD)='738'
      ERRMSG(ECD)='AREA and LINE debug options are equivalent '
...
C     D167 Warning message when 'FLAT' is used in the source elevation
C     field on the SO LOCATION pathway WSP 3/6/23
      ECD = ECD+1
      ERRCOD(ECD)='752'
...
C**  End Aircraft Plume Rise insert; April 2023
CWSP 1/24/2024 D175 HOUREMIS Filename D121 Hourly Emissions File - Starting 
CJAT 5/13/2026 26XXX_DEV_BRANCH; change 311 and 312 to 834 and 835 to avoid
C duplicate error messages with RLINE barrier debugging
      ECD = ECD+1
c      ERRCOD(ECD)='311'
      ERRCOD(ECD)='834'
      ERRMSG(ECD)='Attempt to Define Duplicate HOUREMIS file for SRC:'
      
      ECD = ECD+1
C      ERRCOD(ECD)='312'
      ERRCOD(ECD)='835'
      ERRMSG(ECD)='Attempt to Define Duplicate HOUREMIS files'
CWSP 1/24/2024 D175 HOUREMIS Filename D121 Hourly Emissions File - Ending 
...
C
C     MODIFIED:   Rebecca L. Miller, WSP, 4/24/2024****
...
C     L_RBARRIER  = flag to indicate key word of "RBARRIER" in source section of input file
      ...
      TYPE AVSOURCE
         ...
         CHARACTER(len=12) ::  BARID, BARID2 !SBARRIER inputs (WSP) 20240424
      END TYPE  
...
C     BARID       = barrier ID for barrier 1 from SBARRIER inputs (WSP) 20240424
C     BARID2      = barrier ID for barrier 2 from SBARRIER inputs (WSP) 20240424

C     Allocatable array of sources
      TYPE(AVSOURCE),    ALLOCATABLE  :: RLSOURCE(:)
      
CRLM  SBARRIER Inputs WSP (start)
C     Create and allocate barrier variables
      INTEGER, PARAMETER :: MAXSBSEG=50 !max solid barrier segments for allocation
      TYPE AVBARRIER
        CHARACTER(LEN=12)  ::  BARID
        INTEGER            ::  NUMSEG
        DOUBLE PRECISION, DIMENSION(MAXSBSEG)  ::  XBB, YBB, XBE, YBE
        DOUBLE PRECISION, DIMENSION(MAXSBSEG)  ::  HT, Z
      END TYPE
C     BARID   = input barrier ID
C     NUMSEG  = number of segments in BARID
C     XBB     = array of x-coordinates of beginning points of barrier segments
C     YBB     = array of y-coordinates of beginning points of barrier segments
C     XBE     = array of x-coordinates of end points of barrier segments
C     YBE     = array of y-coordinates of end points of barrier segments
C     HT      = array of heights of barrier segments
C     Z       = array of elevations of barrier segments   

C     Allocatable array of sources
      TYPE(AVBARRIER), ALLOCATABLE  :: SLDBARR(:)
CRLM  SBARRIER Inputs WSP(end)

CRLM SBARRIER edge effects WSP 20240821
      LOGICAL, ALLOCATABLE :: REEDGE(:) !Receptor edge zone
      
CRLM  VBARRIER Inputs WSP (start)
C     Create and allocate veg barrier variables
      TYPE AVEGBAR
C         DOUBLE PRECISION, DIMENSION(2) :: HT, WT, DCL, LM, LAI, WAKE
        DOUBLE PRECISION, DIMENSION(2)  ::  HT   = 0.0D0
        DOUBLE PRECISION, DIMENSION(2)  ::  WT   = 0.0D0
        DOUBLE PRECISION, DIMENSION(2)  ::  DCL  = 0.0D0
        DOUBLE PRECISION, DIMENSION(2)  ::  LM   = 0.0D0
        DOUBLE PRECISION, DIMENSION(2)  ::  LAI  = 0.0D0
        DOUBLE PRECISION, DIMENSION(2)  ::  WAKE = 0.0D0
      END TYPE
C     HT      = veg barrier heights (input)
C     WT      = veg barrier widths  (input)
C     DCL     = distance from center line (input)
C     LM      = veg barrier max leaf area density (input)
C     LAI     = veg barrier lead area index (input)
C     WAKE    = calculated veg barrier wake region

C     Allocatable array of sources
      TYPE(AVEGBAR), ALLOCATABLE  :: VEGBARR(:)
      
      LOGICAL :: WRTVDBG !for debug write
CRLM  VBARRIER Inputs WSP(end)
...
CRLM vegetative barrier variables WSP 20250313
      DOUBLE PRECISION     :: VDWD, VDW_PERD, HVBD, WVBD, LAID, LADD 
      DOUBLE PRECISION     :: WAKED
      INTEGER              :: VBARR
      LOGICAL              :: VEGBARDW
      DOUBLE PRECISION     :: VSZI, VUI, VU10
C     VDWD         = along wind distance between source and downwind vegetative barrier
C     VDW_PERD     = perpendicular distance between source and downwind vegetative barrier
C     HVBD         = height of downwind vegetative barrier
C     WVBD         = width of downwind vegetative barrier
C     LAID         = leaf area index of downwind vegetative barrier    
C     LADD         = leaf area density of downwind vegetative barrier
C     WAKED        = wake of downwind vegetative barrier
C     VBARR        = number of vegetative barriers   
C     VEGBARDW     = vegetative barrier located downwind of source flag   
C     VSZI         = initial sigmaz for no barrier case vegetative barrier calculations
C     VUI          = initial effective wind for vegetative barrier calculations

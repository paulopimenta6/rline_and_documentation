...
C     D001 IF ISYEAR is 2-digits then call CENT_DATE to determine the Current Julian Day and Calculate Current Gregorian Date First Convert Year to 4-Digit Value
...
CC     Check for 2-digit Input and Convert ISYEAR to Four Digits
C      IF (ISYEAR .GE. ISTRT_WIND .AND. ISYEAR .LE. 99) THEN
C         ISYEAR = ISTRT_CENT*100 + ISYEAR
C      ELSE IF (ISYEAR .LT. ISTRT_WIND) THEN
C         ISYEAR = (ISTRT_CENT+1)*100 + ISYEAR
C      END IF
...
C     D001 IF ISYEAR is 2-digits then call CENT_DATE to determine the Current Julian Day and Calculate Current Gregorian Date First Convert Year to 4-Digit Value
C     Check for 2-digit Input and Convert IUYEAR to Four Digits
      IF (IUYEAR .LE. 99) THEN
         CALL CENT_DATE(IUYEAR,IUYEAR)    
      END IF
C ---  D001 remove original calculation of 4-Digit year Wood 9/15/22
CC     Convert IUYEAR to Four Digits
C      IF (IUYEAR .GE. ISTRT_WIND .AND. IUYEAR .LE. 99) THEN
C         IUYEAR = ISTRT_CENT*100 + IUYEAR
C      ELSE IF (IUYEAR .LT. ISTRT_WIND) THEN
C         IUYEAR = (ISTRT_CENT+1)*100 + IUYEAR
C      END IF
...
C     D001 IF ISYEAR is 2-digits then call CENT_DATE to determine the Current Julian Day and Calculate Current Gregorian Date First Convert Year to 4-Digit Value
C     Check for 2-digit Input and Convert ISYR to Four Digits
      IF (ISYR .LE. 99) THEN
         CALL CENT_DATE(ISYR,ISYR)
      END IF
C ---  D001 remove original calculation of 4-Digit year Wood 9/15/22
CC        Convert ISYR and IEYR to Four Digits
C         IF (ISYR .GE. ISTRT_WIND .AND. ISYR .LE. 99) THEN
C            ISYR = ISTRT_CENT*100 + ISYR
C         ELSE IF (ISYR .LT. ISTRT_WIND) THEN
C            ISYR = (ISTRT_CENT+1)*100 + ISYR
C         END IF

C     D001 IF ISYEAR is 2-digits then call CENT_DATE to determine the Current Julian Day and Calculate Current Gregorian Date First Convert Year to 4-Digit Value
C     Check for 2-digit Input and Convert IEYR to Four Digits
      IF (IEYR .LE. 99) THEN
         CALL CENT_DATE(IEYR,IEYR)  
      END IF
C ---  D001 remove original calculation of 4-Digit year Wood 9/15/22
C         IF (IEYR .GE. ISTRT_WIND .AND. IEYR .LE. 99) THEN
C            IEYR = ISTRT_CENT*100 + IEYR
C         ELSE IF (IEYR .LT. ISTRT_WIND) THEN
C            IEYR = (ISTRT_CENT+1)*100 + IEYR
C         END IF
...
C	JAT 6/5/2025 D215_STARTEND; if month is February and non-leap year set IENDDY to 28; otherwise
C	for all other months and leap year February use IDYMAX
		  if (IENDMN .NE. 2 .OR. (MOD(IEYR,4) .EQ. 0)) THEN
            IENDDY = IDYMAX(IENDMN)
		  ELSE
			  IENDDY=28
		  ENDIF
c            IENDDY = IDYMAX(IENDMN)
         END IF
      END IF
...
C                    WRITE Error Message    ! Invalid Julian Day
                  ...
                  END IF
C ---             Check for consistency with STARTEND inputs, if provided
C ---              D146 Changed conditional to reference JDAYB and JDAYE rather than JDAY WSP 10/18/22
                  IF (JDAYB.LT.ISJDAY .OR. JDAYE.GT.IEJDAY) THEN
C                  IF (JDAY.LT.ISJDAY .OR. JDAY.GT.IEJDAY) THEN

C                    WRITE Warning Message  ! Julian Day Out-of-Range
C  ---               D146 Changed warning to reference JDAYE and JDAYB rather than JDAY WSP 10/18/22
                     WRITE(DUMMY,'(I3,"-",I3)') JDAYB, JDAYE
C                     WRITE(DUMMY,'(I8)') JDAY
                     CALL ERRHDL(PATH,MODNAM,'W','350',DUMMY)
                  END IF
...
C - WSP 9/16/2024 D158 Dynamic File unit  Store FUNIT, FILENAME, and FILETYPE in array to add to output as table  - Starting       
            FUNITARRAY(FUNITIND) = MFUNIT
            FNAMEARRAY(FUNITIND) = METINP
            FTYPARRAY(FUNITIND) = 'SURFFILE'
            FUNITIND = FUNITIND + 1
...
C - WSP 9/16/2024 D158 Dynamic File unit  Store FUNIT, FILENAME, and FILETYPE in array to add to output as table  - Starting       
            FUNITARRAY(FUNITIND) = MPUNIT
            FNAMEARRAY(FUNITIND) = PROINP
            FTYPARRAY(FUNITIND) = 'PROFFILE'
            FUNITIND = FUNITIND + 1
C - WSP 9/16/2024 D158 Dynamic File unit  Store FUNIT, FILENAME, and FILETYPE in array to add to output as table  - Ending      
            MPOPEN = .TRUE.
...
c     CRT 8/9/2023 ISSUE D176; Add 'COARE' flag to accomodate COARE in AERMET
      IF( INDEX(BUFFER,'COARE') .NE. 0 )THEN 
C        Set COARE logical to true and MODOPS
         L_COARE = .TRUE.
         MODOPS(27) = 'COARE'
...
C        CRT 8/9/2023 ISSUE D176; Also cannot use BULKRN with COARE
C        CRT8/11/2023 ISSUE D176: Modify MODOPS to 27 for BULKRN, also used for COARE
C         IF( .NOT. L_MMIF_Data )THEN
         IF( .NOT. L_COARE )THEN
            MODOPS(27) = 'BULKRN'
         ELSE
...
C - WSP 9/16/2024 D158 Dynamic File unit  Store FUNIT, FILENAME, and FILETYPE in array to add to output as table  - Starting       
               FUNITARRAY(FUNITIND) = ISUNIT
               FNAMEARRAY(FUNITIND) = SCIM_SFCFIL
               FTYPARRAY(FUNITIND) = 'ISUNIT'
               FUNITIND = FUNITIND + 1
C - WSP 9/16/2024 D158 Dynamic File unit  Store FUNIT, FILENAME, and FILETYPE in array to add to output as table  - Ending      
            ELSE
...
C - WSP 9/16/2024 D158 Dynamic File unit  Store FUNIT, FILENAME, and FILETYPE in array to add to output as table  - Starting       
               FUNITARRAY(FUNITIND) = ISUNIT
               FNAMEARRAY(FUNITIND) = SCIM_PROFIL
               FTYPARRAY(FUNITIND) = 'ISUNIT'
               FUNITIND = FUNITIND + 1
C - WSP 9/16/2024 D158 Dynamic File unit  Store FUNIT, FILENAME, and FILETYPE in array to add to output as table  - Ending     
            ELSE
...
C              Also Check for Filename Larger Than ILEN_FLD Characters
               ...
               OPEN(UNIT=IPUNIT,FILE=SCIM_PROFIL,STATUS='REPLACE')
C - WSP 9/16/2024 D158 Dynamic File unit  Store FUNIT, FILENAME, and FILETYPE in array to add to output as table  - Starting       
               FUNITARRAY(FUNITIND) = IPUNIT
               FNAMEARRAY(FUNITIND) = SCIM_PROFIL
               FTYPARRAY(FUNITIND) = 'IPUNIT'
               FUNITIND = FUNITIND + 1
C - WSP 9/16/2024 D158 Dynamic File unit  Store FUNIT, FILENAME, and FILETYPE in array to add to output as table  - Ending      
            ELSE
C              WRITE Error Message:  SCIM_PROFIL Field is Too Long
...
C***********************************************************************
C                 CENT_DATE Module of the AMS/EPA Regulatory Model - AERMOD
C
C     PURPOSE:    Processes the input two digit date and calculates the 
C                 four digit date assuming that the only century options
C                 are 1900 and 2000
C
C     PROGRAMMER: Wood
C
C        DATE:    9/15/22
C
C     INPUTS:     TwoDigitYear, FourDigitYear defined in the subroutine call 
C                 ISTART_WIND and ISTRT_CENT from MAIN1
C
C     OUTPUT:     FourDigitYear - 4 digit year
C                 TwoDigitYear  - 2 digit year
C
C     CALLED FROM:   AERMOD, EVCALC, MESET, and METEXT
C
C     ERROR HANDLING:   Checks if the input two digit year is actaully two digits;
C                       Checks that the input four digit year is actually four digits;
C                       
C***********************************************************************
C     Variable Declarations
...
C      Assuming that TwoDigitYear is a two digit value for the year from the SFC file and ISTRT_WIND is 
...
CCRT  5/1/2026 D171 SURFDATA Year - reset ISTRT_WIND and ISTRT_CENT based 
c     on current met year if different from SURFDATA Year for screen display
      IF (TwoDigitYear .NE. ISTRT_WIND) THEN
         ISTRT_WIND = TwoDigitYear
         IF (TwoDigitYear .GE. 50 .AND. 
     &       TwoDigitYear .LT. 100) THEN
            ISTRT_CENT = 19
         ELSE IF (TwoDigitYear .LT. 50) THEN
            ISTRT_CENT = 20
         END IF
      END IF
...
CCRT   4/28/2026 D185 - Met Data Gap - Need to correct for case where 2-digit year equals ISTRT_WIND
CCRT      IF (TwoDigitYear > ISTRT_WIND .AND. TwoDigitYear .LE. 99) THEN
      IF (TwoDigitYear >= ISTRT_WIND .AND. TwoDigitYear .LE. 99) THEN
          FourDigitYear = ISTRT_CENT*100 + TwoDigitYear
crt   5/1/2026 D171 SURFDATA - this condition is no longer needed with update to HRLOOP.          
c      ELSE IF (TwoDigitYear < ISTRT_WIND) THEN !CMGS Add 1 to the century (4/10/2024)
c          FourDigitYear = (ISTRT_CENT+1)*100 + TwoDigitYear  !CMGS Add 1 to the century (4/10/2024)
...
C***********************************************************************
C                 LONG_DATE Module of the AMS/EPA Regulatory Model - AERMOD
C
C     PURPOSE:    Processes the 2-digit year from the user and the SFC file 
C                 to determine the ten digit date and two digit date
C
C     PROGRAMMER: Wood
C
C     DATE:    9/15/22
C
C     INPUTS:     EightDigit, TenDigit, TwoDigit, OutputTwoDigit defined in the subroutine call 
C                 ISTART_WIND and ISTRT_CENT from MAIN1
C
C     OUTPUT:     TenDigitYear   - 10-digit date
C                 OutputTwoDigit - 2-digit date
C
C     CALLED FROM:   CALC2, AERMOD, EVSET, and OUSET
C
C     ERROR HANDLING:   Checks that the input 2-digit year is 2-digits;
C                       
C***********************************************************************
C     Variable Declarations
      USE MAIN1, ONLY: ISTRT_WIND, ISTRT_CENT
...
CCRT   5/3/2026 D171 SURFDATA Year ISTRT_WIND now set to current met year. 
C      2-digit met year and ISTRT_WIND should always be equal from CENT_DATE.
c          IF (TwoDigit .NE. ISTRT_WIND .AND. TwoDigit .LE. 99) THEN
           IF (TwoDigit .LE. 99) THEN
...
C                 LONG_DATE Module of the AMS/EPA Regulatory Model - AERMOD
C
C     PURPOSE:    PURPOSE:    Processes the 2-digit year from the user and the 
C                             SFC file to determine the ten digit date only
C
C     PROGRAMMER: Wood
C
C        DATE:    9/15/22
C
C     INPUTS:     EightDigit,TenDigit,TwoDigit defined in the subroutine call 
C                 ISTART_WIND and ISTRT_CENT from MAIN1
C
C     OUTPUT:     TenDigit   - 10-digit date
C
C     CALLED FROM:   EVSET
C
C     ERROR HANDLING:   Checks that the input 2-digit year is 2-digits;

...
CCRT 2/13/2026, D138 Scientific Notation - This block no longer needed
C    since code was modified to apply scientific notation to standard
C    .out file.
C      IF (IOSTAT(13) .GT. 0 .AND. FILE_FORMAT .EQ. 'EXP') THEN
C         IF (.NOT.MXFILE .AND. .NOT.PPFILE .AND. .NOT.PLFILE .AND.
C     &       .NOT.RKFILE .AND.
C     &       .NOT.ANPOST .AND. .NOT.ANPLOT .AND. .NOT.SEASONHR .AND.
C     &       .NOT.MXDAILY .AND. .NOT.MXDAILY_BYYR .AND. 
C     &                                            .NOT.L_MAXDCONT) THEN
CC           Write Warning Message: FILEFORM keyword ignored
C            CALL ERRHDL(PATH,MODNAM,'W','399','FILEFORM')
C         END IF
C      END IF
...
C     Variable Declarations
      ...
      INTEGER :: IFUNIT_loc  ! 10/2/2024 WSP D158 Dynamically Allocated FUNIT
...
C     Variable Initializations
      ...
      IDAT   = 0
      IDAT8  = 0
      IRSDATE = 0  ! CRT 5/14/2026 D158 MULTYEAR - Initialize restart date
...
C           Write Error Message:Invalid Numerical Field
            ...
            GO TO 999
         END IF
CWSP ! 10/2/2024 WSP Dynamically Allocated FUNIT - remove no longer needed warning message STARTING
C        Check for Conflict With System Files
CWSP !         IF (NINT(FNUM) .LE. 30) THEN
CWSP !           WRITE Error Message:  Invalid File Unit Specified
CWSP !            CALL ERRHDL(PATH,MODNAM,'E','560',KEYWRD)
CWSP !            GO TO 999
CWSP !         ELSE IF (NINT(FNUM) .GT. 100) THEN
CWSP ! 10/2/2024 WSP Dynamically Allocated FUNIT - remove no longer needed warning message ENDING
         IF (ANY(NINT(FNUM) == FUNITARRAY)) THEN ! 10/2/2024 WSP D158 Dynamically Allocated FUNIT
           IFUNIT_loc = FINDLOC(FUNITARRAY, NINT(FNUM), DIM=1)! 10/2/2024 WSP D158 Dynamically Allocated FUNIT
           IF (FNAMEARRAY(IFUNIT_loc) .NE. THRFIL(INDGRP,INDAVE)) THEN ! 10/2/2024 WSP D158 Dynamically Allocated FUNIT
C           WRITE Warning Message:  Suspect File Unit Specified
C           Unit May Conflict With Dynamically Allocate File Units
            WRITE(DUMMY,'(i8)') NINT(FNUM) ! 10/2/2024 WSP D158 Dynamically Allocated FUNIT
CWSP !            CALL ERRHDL(PATH,MODNAM,'W','565',KEYWRD) ! 10/7/2024 WSP D158 Dynamically Allocated FUNIT 
            CALL ERRHDL(PATH,MODNAM,'E','565',DUMMY) ! 10/2/2024 WSP D158 Dynamically Allocated FUNIT
           END IF  ! 10/2/2024 WSP D158 Dynamically Allocated FUNIT
...
CWSP !         IMXUNT(INDGRP,INDAVE) = 100 + INDGRP*10 + INDAVE ! 10/2/2024 WSP D158 Dynamically Allocated FUNIT
         IMXUNT(INDGRP,INDAVE) = FUNITINX  ! 10/2/2024 WSP D158 Dynamically Allocated FUNIT
         FUNITINX = FUNITINX + 1  ! 10/2/2024 WSP D158 Dynamically Allocated FUNIT
CWSP ! 10/2/2024 WSP Dynamically Allocated FUNIT - remove no longer needed warning message STARTING
CWSP !             WRITE Warning Message: Dynamic FUnit Allocation May Have Conflict
CWSP !              CALL ERRHDL(PATH,MODNAM,'W','565',KEYWRD)
CWSP !            END IF
CWSP ! 10/2/2024 WSP Dynamically Allocated FUNIT - remove no longer needed warning message STARTING
      END IF
...
CWSP ! 9/16/2024 D158 Dynamically Allocated FUNIT  Store FUNIT, FILENAME, and FILETYPE in array to add to output as table  - Starting       
               FUNITARRAY(FUNITIND) = IMXUNT(INDGRP,INDAVE)
               FNAMEARRAY(FUNITIND) = THRFIL(INDGRP,INDAVE)
               FTYPARRAY(FUNITIND) = 'MAXIFILE'
               FUNITIND = FUNITIND + 1
CWSP ! 9/16/2024 D158 Dynamically Allocated FUNIT  Store FUNIT, FILENAME, and FILETYPE in array to add to output as table  - Ending      
            ELSE
...
C                  IF (IRSYR .GE. ISTRT_WIND .AND. IRSYR .LE. 99) THEN
C                     IRSYR = ISTRT_CENT*100 + IRSYR
C                     IDAT  = ISTRT_CENT*100000000 + IDAT8
C                  ELSE IF (IRSYR .LT. ISTRT_WIND) THEN
C                     IRSYR = (ISTRT_CENT+1)*100 + IRSYR
C                     IDAT  = (ISTRT_CENT+1)*100000000 + IDAT8
C                  END IF
...
CWSP ! 10/2/2024 WSP Dynamically Allocated FUNIT - remove no longer needed warning message STARTING
C        Check for Conflict With System Files
CWSP !         IF (NINT(FNUM) .LE. 30) THEN! 10/2/2024 WSP D158 Dynamically Allocated FUNIT                                                                                           
C           WRITE Error Message:  Invalid File Unit Specified
CWSP !            CALL ERRHDL(PATH,MODNAM,'E','560',KEYWRD)
CWSP !            GO TO 999
CWSP !         IF (NINT(FNUM) .LE. 100) THEN! 10/2/2024 WSP D158 Dynamically Allocated FUNIT
C           WRITE Error Message:  Invalid File Unit Specified
CWSP !            CALL ERRHDL(PATH,MODNAM,'E','560',KEYWRD)
CWSP !            GO TO 999
CWSP !         ELSE IF (NINT(FNUM) .GT. 100) THEN! 10/2/2024 WSP D158 Dynamically Allocated FUNIT
CWSP ! 10/2/2024 WSP Dynamically Allocated FUNIT - remove no longer needed warning message ENDING
         IF (ANY(NINT(FNUM) == FUNITARRAY)) THEN ! 10/2/2024 WSP D158 Dynamically Allocated FUNIT
           IFUNIT_loc = FINDLOC(FUNITARRAY, NINT(FNUM), DIM=1)! 10/2/2024 WSP D158 Dynamically Allocated FUNIT
           IF (FNAMEARRAY(IFUNIT_loc) .NE. PSTFIL(INDGRP,INDAVE)) THEN ! 10/2/2024 WSP D158 Dynamically Allocated FUNIT
C           WRITE Warning Message:  Suspect File Unit Specified
C           Unit May Conflict With Dynamically Allocate File Units
CWSP !            CALL ERRHDL(PATH,MODNAM,'W','565',KEYWRD)
            WRITE(DUMMY,'(i8)') NINT(FNUM) ! 10/2/2024 WSP D158 Dynamically Allocated FUNIT
...
CWSP !         IPSUNT(INDGRP,INDAVE) = 200 + INDGRP*10 + INDAVE ! 10/2/2024 WSP D158 Dynamically Allocated FUNIT
         IPSUNT(INDGRP,INDAVE) = FUNITINX  ! 10/2/2024 WSP D158 Dynamically Allocated FUNIT
         FUNITINX = FUNITINX + 1  ! 10/2/2024 WSP D158 Dynamically Allocated FUNIT
CWSP ! 10/2/2024 WSP Dynamically Allocated FUNIT - remove no longer needed warning message STARTING
CWSP !         IF (INDGRP .GE. 10 .OR. INDAVE .GE. 10) THEN ! 10/2/2024 WSP D158 Dynamically Allocated FUNIT
CWSP !         IF (ANY(IPSUNT(INDGRP,INDAVE) == FUNITARRAY)) THEN ! 10/2/2024 WSP D158 Dynamically Allocated FUNIT
C           WRITE Warning Message: Dynamic FUnit Allocation May Have Conflict
C            CALL ERRHDL(PATH,MODNAM,'W','565',KEYWRD)
CWSP !         END IF
CWSP ! 10/2/2024 WSP Dynamically Allocated FUNIT - remove no longer needed warning message ENDING
      END IF
...
CWSP ! 9/16/2024 D158 Dynamically Allocated FUNIT  Store FUNIT, FILENAME, and FILETYPE in array to add to output as table  - Starting       
                  FUNITARRAY(FUNITIND) = IPSUNT(INDGRP,INDAVE)
                  FNAMEARRAY(FUNITIND) = PSTFIL(INDGRP,INDAVE)
                  FTYPARRAY(FUNITIND) = 'POSTFILE'
                  FUNITIND = FUNITIND + 1
CWSP ! 9/16/2024 D158 Dynamically Allocated FUNIT  Store FUNIT, FILENAME, and FILETYPE in array to add to output as table  - Ending   
               ELSE
C                 File does not exist; restart option will not work
...
CWSP ! 10/2/2024 WSP Dynamically Allocated FUNIT - remove no longer needed warning message ENDING
         IF (ANY(NINT(FNUM) == FUNITARRAY)) THEN ! 10/2/2024 WSP D158 Dynamically Allocated FUNIT
           IFUNIT_loc = FINDLOC(FUNITARRAY, NINT(FNUM), DIM=1)! 10/2/2024 WSP D158 Dynamically Allocate FUNIT
           IF (FNAMEARRAY(IFUNIT_loc) .NE. 
     &     PLTFIL(INDVAL,INDGRP,INDAVE)) THEN ! 10/2/2024 WSP D158 Dynamically Allocate FUNIT
C           WRITE Warning Message:  Suspect File Unit Specified
C           Unit May Conflict With Dynamically Allocate File Units
CWSP !            CALL ERRHDL(PATH,MODNAM,'W','565',KEYWRD)  ! 10/2/2024 WSP D158 Dynamically Allocate FUNIT
            WRITE(DUMMY,'(i8)') NINT(FNUM) ! 10/2/2024 WSP D158 Dynamically Allocate FUNIT
            CALL ERRHDL(PATH,MODNAM,'E','565', DUMMY) ! 10/2/2024 WSP D158 Dynamically Allocate FUNIT
           END IF  ! 10/2/2024 WSP D158 Dynamically Allocate FUNIT
...
CWSP !         IPLUNT(INDVAL,INDGRP,INDAVE) = (INDVAL+3)*100+INDGRP*10+INDAVE ! 10/2/2024 WSP D158 Dynamically Allocated FUNIT
         IPLUNT(INDVAL,INDGRP,INDAVE) = FUNITINX  ! 10/2/2024 WSP D158 Dynamically Allocated FUNIT
         FUNITINX = FUNITINX + 1  ! 10/2/2024 WSP D158 Dynamically Allocated FUNIT
CWSP ! 10/2/2024 WSP D158 Dynamically Allocated FUNIT Removed because no longer needed - STARTING           
CWSP !         IF (INDGRP .GE. 10 .OR. INDAVE .GE. 10) THEN ! 10/2/2024 WSP D158 Dynamically Allocated FUNIT
C           WRITE Warning Message: Dynamic FUnit Allocation May Have Conflict
CWSP !            CALL ERRHDL(PATH,MODNAM,'W','565',KEYWRD)
CWSP !         END IF
CWSP ! 10/2/2024 WSP D158 Dynamically Allocated FUNIT Removed because no longer needed - ENDING           
      END IF
...
C        First Time File is Identified - OPEN File
     ...
     &        IOSTAT=IOERRN,STATUS='REPLACE')
CWSP ! 9/16/2024 D158 Dynamically Allocated FUNIT  Store FUNIT, FILENAME, and FILETYPE in array to add to output as table  - Starting       
         FUNITARRAY(FUNITIND) = IPLUNT(INDVAL,INDGRP,INDAVE)
         FNAMEARRAY(FUNITIND) = PLTFIL(INDVAL,INDGRP,INDAVE)
         FTYPARRAY(FUNITIND) = 'PLOTFILE'
         FUNITIND = FUNITIND + 1
CWSP ! 9/16/2024 D158 Dynamically Allocated FUNIT  Store FUNIT, FILENAME, and FILETYPE in array to add to output as table  - Ending      
      END IF
...
CWSP ! 10/2/2024 WSP Dynamically Allocated FUNIT - remove no longer needed warning message ENDING
         IF (ANY(NINT(FNUM) == FUNITARRAY)) THEN ! 10/2/2024 WSP D158 Dynamically Allocated FUNIT
           IFUNIT_loc = FINDLOC(FUNITARRAY, NINT(FNUM), DIM=1)! 10/2/2024 WSP D158 Dynamically Allocate FUNIT
           IF (FNAMEARRAY(IFUNIT_loc) .NE. ANNPST(INDGRP)) THEN ! 10/2/2024 WSP D158 Dynamically Allocate FUNIT
C           WRITE Warning Message:  Suspect File Unit Specified
C           Unit May Conflict With Dynamically Allocate File Units
CWSP !            CALL ERRHDL(PATH,MODNAM,'W','565',KEYWRD)  ! 10/2/2024 WSP D158 Dynamically Allocate FUNIT
            WRITE(DUMMY,'(i8)') NINT(FNUM) ! 10/2/2024 WSP D158 Dynamically Allocate FUNIT
...
CWSP !         IAPUNT(INDGRP) = 300 + INDGRP*10 - 5 ! 10/2/2024 WSP D158 Dynamically Allocated FUNIT
         IAPUNT(INDGRP) = FUNITINX  ! 10/2/2024 WSP D158 Dynamically Allocated FUNIT
         FUNITINX = FUNITINX + 1  ! 10/2/2024 WSP D158 Dynamically Allocated FUNIT
CWSP ! 10/2/2024 WSP D158 Dynamically Allocated FUNIT Removed because no longer needed - STARTING           
CWSP !         IF (INDGRP .GE. 10) THEN ! 10/2/2024 WSP D158 Dynamically Allocated FUNIT
C           WRITE Warning Message: Dynamic FUnit Allocation May Have Conflict
CWSP !            CALL ERRHDL(PATH,MODNAM,'W','565',KEYWRD)
CWSP !         END IF
CWSP ! 10/2/2024 WSP D158 Dynamically Allocated FUNIT Removed because no longer needed - ENDING           
...
CWSP ! 9/16/2024 D158 Dynamically Allocated FUNIT  Store FUNIT, FILENAME, and FILETYPE in array to add to output as table  - Starting       
            FUNITARRAY(FUNITIND) = IAPUNT(INDGRP)
            FNAMEARRAY(FUNITIND) = ANNPST(INDGRP)
            FTYPARRAY(FUNITIND) = 'ANNPST'
            FUNITIND = FUNITIND + 1
CWSP ! 9/16/2024 D158 Dynamically Allocated FUNIT  Store FUNIT, FILENAME, and FILETYPE in array to add to output as table  - Ending  
         ELSE IF (FIELD(5) .EQ. 'PLOT') THEN
            OPEN(IAPUNT(INDGRP),ERR=99,FILE=ANNPST(INDGRP),
...
CWSP ! 10/2/2024 WSP Dynamically Allocated FUNIT - remove no longer needed warning message STARTING
C        Check for Conflict With System Files
C         IF (NINT(FNUM) .LE. 30) THEN
C           WRITE Error Message:  Invalid File Unit Specified
C            CALL ERRHDL(PATH,MODNAM,'E','560',KEYWRD)
C            GO TO 999
C         ELSE IF (NINT(FNUM) .GT. 100) THEN
CWSP ! 10/2/2024 WSP Dynamically Allocated FUNIT - remove no longer needed warning message ENDING
         IF (ANY(NINT(FNUM) == FUNITARRAY)) THEN ! 10/2/2024 WSP D158 Dynamically Allocated FUNIT
           IFUNIT_loc = FINDLOC(FUNITARRAY, NINT(FNUM), DIM=1)! 10/2/2024 WSP D158 Dynamically Allocate FUNIT
           IF (FNAMEARRAY(IFUNIT_loc) .NE. ANNPLT(INDGRP)) THEN ! 10/2/2024 WSP D158 Dynamically Allocate FUNIT
C           WRITE Warning Message:  Suspect File Unit Specified
C           Unit May Conflict With Dynamically Allocate File Units
CWSP !            CALL ERRHDL(PATH,MODNAM,'W','565',KEYWRD)  ! 10/2/2024 WSP D158 Dynamically Allocate FUNIT
            WRITE(DUMMY,'(i8)') NINT(FNUM) ! 10/2/2024 WSP D158 Dynamically Allocate FUNIT
      ...
      ELSE
C        Dynamically Allocate File Unit (300's)
CWSP !         IPPUNT(INDGRP) = 300 + INDGRP*10 ! 10/2/2024 WSP D158 Dynamically Allocated FUNIT
         IPPUNT(INDGRP) = FUNITINX  ! 10/2/2024 WSP D158 Dynamically Allocated FUNIT
         FUNITINX = FUNITINX + 1  ! 10/2/2024 WSP D158 Dynamically Allocated FUNIT
CWSP ! 10/2/2024 WSP D158 Dynamically Allocated FUNIT Removed because no longer needed - STARTING           
CWSP !         IF (INDGRP .GE. 10) THEN! 10/2/2024 WSP D158 Dynamically Allocated FUNIT
C           WRITE Warning Message: Dynamic FUnit Allocation May Have Conflict
...
C        First Time File is Identified - OPEN File
     ...
     &        IOSTAT=IOERRN,STATUS='REPLACE')
CWSP ! 9/16/2024 D158 Dynamically Allocated FUNIT  Store FUNIT, FILENAME, and FILETYPE in array to add to output as table  - Starting       
         FUNITARRAY(FUNITIND) = IPPUNT(INDGRP)
         FNAMEARRAY(FUNITIND) = ANNPLT(INDGRP)
         FTYPARRAY(FUNITIND) = 'ANNPLT'
         FUNITIND = FUNITIND + 1
CWSP ! 9/16/2024 D158 Dynamically Allocated FUNIT  Store FUNIT, FILENAME, and FILETYPE in array to add to output as table  - Ending      
      END IF
...
C           Unit May Conflict With Dynamically Allocate File Units
      ...
      ELSE
C        Dynamically Allocate File Unit (300's)
CWSP !         ITXUNT(INDAVE) = 300 + INDAVE ! 10/2/2024 WSP D158 Dynamically Allocated FUNIT
         ITXUNT(INDAVE) = FUNITINX  ! 10/2/2024 WSP D158 Dynamically Allocated FUNIT
         FUNITINX = FUNITINX + 1  ! 10/2/2024 WSP D158 Dynamically Allocated FUNIT
CWSP ! 10/2/2024 WSP D158 Dynamically Allocated FUNIT Removed because no longer needed - STARTING           
C         IF (INDAVE .GE. 5) THEN ! 10/2/2024 WSP D158 Dynamically Allocated FUNIT
C           WRITE Warning Message: Dynamic FUnit Allocation May Have Conflict
CWSP !            CALL ERRHDL(PATH,MODNAM,'W','565',KEYWRD)
CWSP !         END IF
CWSP ! 10/2/2024 WSP D158 Dynamically Allocated FUNIT Removed because no longer needed - ENDING           
...
CWSP ! 9/16/2024 D158 Dynamically Allocated FUNIT  Store FUNIT, FILENAME, and FILETYPE in array to add to output as table  - Starting       
         FUNITARRAY(FUNITIND) = ITXUNT(INDAVE)
         FNAMEARRAY(FUNITIND) = TOXFIL(INDAVE)
         FTYPARRAY(FUNITIND) = 'TOXXFILE'
         FUNITIND = FUNITIND + 1
CWSP ! 9/16/2024 D158 Dynamically Allocated FUNIT  Store FUNIT, FILENAME, and FILETYPE in array to add to output as table  - Ending      
      END IF
...
C         ELSE IF (NINT(FNUM) .GT. 100) THEN
CWSP ! 10/2/2024 WSP Dynamically Allocated FUNIT - remove no longer needed warning message ENDING
CWSP !       ISHUNT(INDGRP) = NINT(FNUM) ! 10/2/2024 WSP D158 Dynamically Allocated FUNIT
            ISHUNT(INDGRP) = FUNITINX  ! 10/2/2024 WSP D158 Dynamically Allocated FUNIT
            FUNITINX = FUNITINX + 1  ! 10/2/2024 WSP D158 Dynamically Allocated FUNIT
      ELSE
C        Dynamically Allocate File Unit (300's)
C         ISHUNT(INDGRP) = 302 + INDGRP*10 ! 10/2/2024 WSP D158 Dynamically Allocated FUNIT
         ISHUNT(INDGRP) = FUNITINX ! 10/2/2024 WSP D158 Dynamically Allocated FUNIT
         FUNITINX = FUNITINX + 1  ! 10/2/2024 WSP D158 Dynamically Allocated FUNIT
CWSP ! 10/2/2024 WSP D158 Dynamically Allocated FUNIT Removed because no longer needed - STARTING           
CWSP !         IF (INDGRP .GE. 10) THEN ! 10/2/2024 WSP D158 Dynamically Allocated FUNIT
...
C        First Time File is Identified - OPEN File
     ...
     &        IOSTAT=IOERRN,STATUS='REPLACE')
CWSP ! 9/16/2024 D158 Dynamically Allocated FUNIT  Store FUNIT, FILENAME, and FILETYPE in array to add to output as table  - Starting       
         FUNITARRAY(FUNITIND) = ISHUNT(INDGRP)
         FNAMEARRAY(FUNITIND) = SEAHRS(INDGRP)
         FTYPARRAY(FUNITIND) = 'SEAHR'
         FUNITIND = FUNITIND + 1
CWSP ! 9/16/2024 D158 Dynamically Allocated FUNIT  Store FUNIT, FILENAME, and FILETYPE in array to add to output as table  - Ending      
      END IF
...
CWSP ! 10/2/2024 WSP D158 Dynamically Allocated FUNIT Removed because no longer needed - STARTING           
         IF (ANY(NINT(FNUM) == FUNITARRAY)) THEN ! 10/2/2024 WSP D158 Dynamically Allocated FUNIT
           IFUNIT_loc = FINDLOC(FUNITARRAY, NINT(FNUM), DIM=1)! 10/2/2024 WSP D158 Dynamically Allocate FUNIT
           IF (FNAMEARRAY(IFUNIT_loc) .NE. RNKFIL(INDAVE)) THEN ! 10/2/2024 WSP D158 Dynamically Allocate FUNIT
C           WRITE Warning Message:  Suspect File Unit Specified
C           Unit May Conflict With Dynamically Allocate File Units
CWSP !            CALL ERRHDL(PATH,MODNAM,'W','565',KEYWRD)  ! 10/2/2024 WSP D158 Dynamically Allocate FUNIT
            WRITE(DUMMY,'(i8)') NINT(FNUM) ! 10/2/2024 WSP D158 Dynamically Allocate FUNIT
      ...
      ELSE
C        Dynamically Allocate File Unit (100's)
C         IRKUNT(INDAVE) = 100 + INDAVE ! 10/2/2024 WSP D158 Dynamically Allocated FUNIT
         IRKUNT(INDAVE) = FUNITINX  ! 10/2/2024 WSP D158 Dynamically Allocated FUNIT
         FUNITINX = FUNITINX + 1  ! 10/2/2024 WSP D158 Dynamically Allocated FUNIT
CWSP ! 10/2/2024 WSP D158 Dynamically Allocated FUNIT Removed because no longer needed - STARTING  
CWSP !         IF (INDGRP .GE. 10) THEN ! 10/2/2024 WSP D158 Dynamically Allocated FUNIT
C           WRITE Warning Message: Dynamic FUnit Allocation May Have Conflict
...
CWSP ! 9/16/2024 D158 Dynamically Allocated FUNIT  Store FUNIT, FILENAME, and FILETYPE in array to add to output as table  - Starting       
         FUNITARRAY(FUNITIND) = IRKUNT(INDAVE)
         FNAMEARRAY(FUNITIND) = RNKFIL(INDAVE)
         FTYPARRAY(FUNITIND) = 'RANKFILE'
         FUNITIND = FUNITIND + 1
CWSP ! 9/16/2024 D158 Dynamically Allocated FUNIT  Store FUNIT, FILENAME, and FILETYPE in array to add to output as table  - Ending      
      END IF
...
CWSP ! 10/2/2024 WSP Dynamically Allocated FUNIT - remove no longer needed warning message ENDING
         IF (ANY(NINT(FNUM) == FUNITARRAY)) THEN ! 10/2/2024 WSP D158 Dynamically Allocated FUNIT
           IFUNIT_loc = FINDLOC(FUNITARRAY, NINT(FNUM), DIM=1)! 10/2/2024 WSP D158 Dynamically Allocate FUNIT
           IF (FNAMEARRAY(IFUNIT_loc) .NE. EVLFIL(INDSRC)) THEN ! 10/2/2024 WSP D158 Dynamically Allocate FUNIT
C           WRITE Warning Message:  Suspect File Unit Specified
C           Unit May Conflict With Dynamically Allocate File Units
CWSP !            CALL ERRHDL(PATH,MODNAM,'W','565',KEYWRD)  ! 10/2/2024 WSP D158 Dynamically Allocate FUNIT
            WRITE(DUMMY,'(i8)') NINT(FNUM) ! 10/2/2024 WSP D158 Dynamically Allocate FUNIT
...
C     IELUNT(INDSRC) = 400 + INDSRC*5  ! 10/2/2024 WSP D158 Dynamically Allocated FUNIT
        IELUNT(INDSRC) = FUNITINX  ! 10/2/2024 WSP D158 Dynamically Allocated FUNIT
        FUNITINX = FUNITINX + 1  ! 10/2/2024 WSP D158 Dynamically Allocated FUNIT
      END IF
...
C        First Time File is Identified - OPEN File
     ...
     &        FORM='FORMATTED',IOSTAT=IOERRN,STATUS='REPLACE')
CWSP ! 9/16/2024 D158 Dynamically Allocated FUNIT  Store FUNIT, FILENAME, and FILETYPE in array to add to output as table  - Starting       
         FUNITARRAY(FUNITIND) = IELUNT(INDSRC)
         FNAMEARRAY(FUNITIND) = EVLFIL(INDSRC)
         FTYPARRAY(FUNITIND) = 'EVALFILE'
         FUNITIND = FUNITIND + 1
CWSP ! 9/16/2024 D158 Dynamically Allocated FUNIT  Store FUNIT, FILENAME, and FILETYPE in array to add to output as table  - Ending      
      END IF
...
C     Open SUMMFILE
     ...
     &     IOSTAT=IOERRN,STATUS='REPLACE')
CWSP ! 9/16/2024 D158 Dynamically Allocated FUNIT  Store FUNIT, FILENAME, and FILETYPE in array to add to output as table  - Starting       
      FUNITARRAY(FUNITIND) = ISUMUNT
      FNAMEARRAY(FUNITIND) = SUMFIL
      FTYPARRAY(FUNITIND) = 'SUMFIL'
      FUNITIND = FUNITIND + 1
CWSP ! 9/16/2024 D158 Dynamically Allocated FUNIT  Store FUNIT, FILENAME, and FILETYPE in array to add to output as table  - Ending      
      GO TO 999
...
C        Error Message: Too Many Fields
         ...
         GO TO 999
      END IF
CCRT, 2/11/2026 D138 Scientific Notation
C     Update rules for scientific notation
C     * Non-DFAULT option when modeling criteria poll,
C       Reset to FIX and output warning if DFAULT, criteria poll, and EXP
C     * Keep EXP for all other polls, w/ or w/o DFAULT keyword
      IF (FIELD(3)(1:3) .EQ. 'FIX') THEN
C        Output file formats will use FIXed format
         FILE_FORMAT = 'FIX'
         
      ELSE IF (FIELD(3)(1:3) .EQ. 'EXP' .AND. DFAULT 
     &         .AND. PollCrit) THEN
         FILE_FORMAT = 'FIX'
C        Write Warning Message:  Invalid format specified
         CALL ERRHDL(PATH,MODNAM,'W','595',POLLUT)
...
CWSP ! 10/2/2024 WSP Dynamically Allocated FUNIT - remove no longer needed warning message ENDING
         IF (ANY(NINT(FNUM) == FUNITARRAY)) THEN ! 10/2/2024 WSP D158 Dynamically Allocated FUNIT
           IFUNIT_loc = FINDLOC(FUNITARRAY, NINT(FNUM), DIM=1)! 10/2/2024 WSP D158 Dynamically Allocate FUNIT
           IF (FNAMEARRAY(IFUNIT_loc) .NE. MAXDLY(INDGRP)) THEN ! 10/2/2024 WSP D158 Dynamically Allocate FUNIT
CWSP !         ELSE IF (NINT(FNUM) .GT. 100) THEN
C           WRITE Warning Message:  Suspect File Unit Specified
C           Unit May Conflict With Dynamically Allocate File Units
CWSP !            CALL ERRHDL(PATH,MODNAM,'W','565',KEYWRD)  ! 10/2/2024 WSP D158 Dynamically Allocate FUNIT
...
CWSP !         IMDUNT(INDGRP) = 501 + (INDGRP-1)*2  ! 10/2/2024 WSP D158 Dynamically Allocated FUNIT
         IMDUNT(INDGRP) = FUNITINX  ! 10/2/2024 WSP D158 Dynamically Allocated FUNIT
         FUNITINX = FUNITINX + 1  ! 10/2/2024 WSP D158 Dynamically Allocated FUNIT
CWSP ! 10/2/2024 WSP D158 Dynamically Allocated FUNIT Removed because no longer needed - STARTING           
...
C           Read Start Date From File and Rewind.
     ...
     &              IOSTAT=IOERRN,FORM='FORMATTED',STATUS='OLD')
CWSP ! 9/16/2024 D158 Dynamically Allocated FUNIT  Store FUNIT, FILENAME, and FILETYPE in array to add to output as table  - Starting       
               FUNITARRAY(FUNITIND) = IMDUNT(INDGRP)
               FNAMEARRAY(FUNITIND) = MAXDLY(INDGRP)
               FTYPARRAY(FUNITIND) = 'MAXDLY'
               FUNITIND = FUNITIND + 1
CWSP ! 9/16/2024 D158 Dynamically Allocated FUNIT  Store FUNIT, FILENAME, and FILETYPE in array to add to output as table  - Ending      
            ELSE
C              File does not exist; restart option will not work
...
CWSP ! 10/2/2024 WSP Dynamically Allocated FUNIT - remove no longer needed warning message ENDING
         IF (ANY(NINT(FNUM) == FUNITARRAY)) THEN ! 10/2/2024 WSP D158 Dynamically Allocated FUNIT
           IFUNIT_loc = FINDLOC(FUNITARRAY, NINT(FNUM), DIM=1)! 10/2/2024 WSP D158 Dynamically Allocate FUNIT
           IF (FNAMEARRAY(IFUNIT_loc) .NE. MAXDLY_BYYR(INDGRP)) THEN ! 10/2/2024 WSP D158 Dynamically Allocate FUNIT
CWSP !     ELSE IF (NINT(FNUM) .GT. 100) THEN
C           WRITE Warning Message:  Suspect File Unit Specified
C           Unit May Conflict With Dynamically Allocate File Units
CWSP !            CALL ERRHDL(PATH,MODNAM,'W','565',KEYWRD)  ! 10/2/2024 WSP D158 Dynamically Allocate FUNIT
...
C         IMDUNT_BYYR(INDGRP) = 601 + (INDGRP-1)*2  ! 10/2/2024 WSP D158 Dynamically Allocated FUNIT
         IMDUNT_BYYR(INDGRP) = FUNITINX   ! 10/2/2024 WSP D158 Dynamically Allocated FUNIT
         FUNITINX = FUNITINX + 1   ! 10/2/2024 WSP D158 Dynamically Allocated FUNIT
CWSP ! 10/2/2024 WSP D158 Dynamically Allocated FUNIT Removed because no longer needed - STARTING           
CWSP !         IF (INDGRP .GE. 10) THEN ! 10/2/2024 WSP D158 Dynamically Allocated FUNIT
C           WRITE Warning Message: Dynamic FUnit Allocation May Have Conflict
...
C           Read Start Date From File and Rewind.
     ...
     &              IOSTAT=IOERRN,FORM='FORMATTED',STATUS='OLD')
CWSP ! 9/16/2024 D158 Dynamically Allocated FUNIT  Store FUNIT, FILENAME, and FILETYPE in array to add to output as table  - Starting       
               FUNITARRAY(FUNITIND) = IMDUNT_BYYR(INDGRP)
               FNAMEARRAY(FUNITIND) = MAXDLY_BYYR(INDGRP)
               FTYPARRAY(FUNITIND) = 'MAXDLY_BYYR'
               FUNITIND = FUNITIND + 1
CWSP ! 9/16/2024 D158 Dynamically Allocated FUNIT  Store FUNIT, FILENAME, and FILETYPE in array to add to output as table  - Ending      
            ELSE
C              File does not exist; restart option will not work
...
CWSP !            IMXDCUNT(INDGRP) = 701 + (INDGRP)*2 ! WSP 10/2/2024 D158 Dynamically Allocated FUNIT
            IMXDCUNT(INDGRP) = FUNITINX
            FUNITINX = FUNITINX + 1
CWSP ! 10/2/2024 WSP D158 Dynamically Allocated FUNIT Removed because no longer needed - STARTING           
CWSP !            IF (INDGRP .GE. 10) THEN
...
CWSP ! 10/2/2024 WSP D158 Dynamically Allocated FUNIT Removed because no longer needed -ENDING           
               IMXDCUNT(INDGRP) = NINT(FNUM)
CWSP !            ELSE
CWSP !               IMXDCUNT(INDGRP) = NINT(FNUM)
CWSP !           END IF
         ELSE
C           Dynamically Allocate File Unit (700's)
CWSP            IMXDCUNT(INDGRP) = 701 + (INDGRP)*2 ! WSP 10/2/2024 D158 Dynamically Allocated FUNIT
            ...
            FUNITINX = FUNITINX + 1
CWSP ! 10/2/2024 WSP D158 Dynamically Allocated FUNIT Removed because no longer needed - STARTING           
C            IF (INDGRP .GE. 10) THEN 
C              WRITE Warning Message: Dynamic FUnit Allocation May Have Conflict
CWSP!               CALL ERRHDL(PATH,MODNAM,'W','565',KEYWRD)
C            END IF
CWSP ! 10/2/2024 WSP D158 Dynamically Allocate FUNIT Removed because no longer needed - ENDING           
         END IF
...
C ---    This is a new file; open the file
     ...
     &           IOSTAT=IOERRN,FORM='FORMATTED',STATUS='REPLACE')
CWSP ! WSP 9/16/2024 D158 Dynamic File unit  Store FUNIT, FILENAME, and FILETYPE in array to add to output as table  - Starting       
         FUNITARRAY(FUNITIND) = IMXDCUNT(INDGRP)
         FNAMEARRAY(FUNITIND) = MAXDCONT_FILE(INDGRP)
         FTYPARRAY(FUNITIND) = 'MAXDCONT_FILE'
         FUNITIND = FUNITIND + 1
CWSP ! WSP 9/16/2024 D158 Dynamic File unit  Store FUNIT, FILENAME, and FILETYPE in array to add to output as table  - Ending    
      END IF

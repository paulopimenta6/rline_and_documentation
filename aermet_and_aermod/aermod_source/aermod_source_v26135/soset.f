...
C
C        MODIFIED:   Added code for Aircraft Source Group
C                    (to identify the aircraft sources).
C                    Gavendra Pandey; UNC-IE, Chapel Hill, NC, USA
C                    04/01/2023
C
...
C         Added FLAT restriction in addition to the ALPHA restriction Wood 10/7 
...
C           Process RBARRIER keyword                        ---   CALL RLINEBAR_INPUTS
         ...
         ELSE
C           Add error message if FLAT is not included
            IF (.NOT. FLAT) THEN
C ---                ERROR MESSAGE THAT BARRIER SOURCE REQUIRES 'FLAT' MODELOPT
                     CALL ERRHDL(PATH,MODNAM,'E','713','')
            END IF
C           Add if statement to only write error messge if the ALPHA option is not included
            IF (.NOT. L_ALPHA) THEN
C               WRITE Error Message:  "Non-DFAULT ALPHA option required"
...
CRLM  Barrier Inputs WSP (start)
C     Check for SBARRIER input
      ELSE IF (KEYWRD .EQ. 'SBARRIER') THEN
        IF(L_ALPHA .AND. FLAT) THEN
          SBARDCL = .TRUE.
C         Process SBARRIER keyword                        ---   CALL SBARRIER_INPUTS
          CALL SBARRIER_INPUTS
        ELSE
          IF (.NOT. FLAT) THEN
C           ERROR MESSAGE THAT BARRIER SOURCE REQUIRES 'FLAT' MODELOPT
            CALL ERRHDL(PATH,MODNAM,'E','713','')
          ...
          IF (.NOT. L_ALPHA) THEN
C           WRITE Error Message:  "Non-DFAULT ALPHA option required"
C           for SBARRIER defining source configuration parameters
            CALL ERRHDL(PATH,MODNAM,'E','198',' RBARRIER ')
          END IF
        END IF
C     Check for Barrier+Source group input
C     ELSE IF (KEYWRD .EQ. 'SBARSRCGRP') THEN      
CRLM  Barrier Inputs WSP (end)

CRLM  Veg Barrier Inputs WSP (start)
C     Check for VBARRIER input
      ELSE IF (KEYWRD .EQ. 'VBARRIER') THEN
        IF(L_ALPHA .AND. FLAT) THEN
C         Process SBARRIER keyword                        ---   CALL VBARRIER_INPUTS
          CALL VBARRIER_INPUTS
          VBARSET = .TRUE. !Switch to later call vbarrier set up calculations
        ELSE
          IF (.NOT. FLAT) THEN
C           ERROR MESSAGE THAT BARRIER SOURCE REQUIRES 'FLAT' MODELOPT
            CALL ERRHDL(PATH,MODNAM,'E','713','')
...
C           for SBARRIER defining source configuration parameters
          ...
          END IF
        END IF     
CRLM Veg Barrier Inputs WSP (end)
...
C           Add error message if FLAT is not included
            IF (.NOT. FLAT) THEN
C ---          ERROR MESSAGE THAT DEPRESSED SOURCE REQUIRES 'FLAT' MODELOPT
               CALL ERRHDL(PATH,MODNAM,'E','713','')
            END IF
C           Add if statement to only write error messge if the ALPHA option is not included
            IF (.NOT. L_ALPHA) THEN
...
C              for RDEPRESS defining source configuration parameters
         ...
         END IF
C    Original portion
C      ELSE IF (KEYWRD .EQ. 'RDEPRESS') THEN
C         IF(L_ALPHA) THEN
CC           Set the RDEPRESS logical to true
C            L_RDEPRESS = .TRUE.
CC           Process RDEPRESS keyword                        ---   CALL RLINEDPR_INPUTS
C            CALL RLINEDPR_INPUTS
C         ELSE
CC           WRITE Error Message:  "Non-DFAULT ALPHA option required"
CC           for RDEPRESS defining source configuration parameters
C            CALL ERRHDL(PATH,MODNAM,'E','198',' RDEPRESS ')
C         END IF
C
...
C           Process BL_INPUTS keyword                       ---   CALL BLPGRP
            CALL BLPGRP
C         END IF
C**  Added for Aircraft Plume Rise; UNC-IE
...
C        Check for SRCGROUP Card Out Of Order
         ...
         IF(ARCFT) THEN
C            Process the Aircraft Source Card  --- CALL AIRCRAFT
            CALL AIRCRAFT
C           check for HOURLY Flag means check for hourly emission file
C           that is must for Aircraft Sources
            IF (.NOT. HOURLY)THEN
C            WRITE Error Message for Hourly Emission File with Aircraft Sources
             CALL ERRHDL(PATH,MODNAM,'E','823','HOUREMIS')
            END IF
          ELSE
C           Write Error Message; Aircraft Source Group defined without ARCFTOPT card
            CALL ERRHDL(PATH,MODNAM,'E','821','ARCFTOPT')
          END IF
C**  End Aircraft Plume Rise insert; April 2023
...
C        Check for SRCGROUP Card Out Of Order
         ...
         IF (HBPLUME) THEN
           IF (L_ALPHA) THEN
C             Process the HBP Source Card                   ---   CALL HBPSOURCE
              CALL HBPSOURCE
           ELSE
C           WRITE Error Message:  "Non-DFAULT ALPHA option required"
C           for HBPSRC defining source configuration parameters
              CALL ERRHDL(PATH,MODNAM,'E','198','HBPSRCID')
           ENDIF              
         ELSE
C           Write Error Message:  HBP source defined without HBPLUME card
...
C ---       Process SBARRIER inputs                         --- CALL SBARRIER_DCL
            IF (SBARDCL) CALL SBARRIER_DCL
C ---       Process VBARRIER inputs                         --- CALL VBARRIER_SETUP
            IF (VBARSET) CALL VBARRIER_SETUP !RLM WSP 2025 02 26
...
CCRT 11/7/2025 D086 Open Debug Files - Moved call statement for calculations 
C           from SOCARD called in SETUP before debug files are open to aermod.f 
C           after setup is called and after debug files are open.  
C ---       Calculate settling velocity and related time-invariant
C           deposition data                                 ---   CALL VDP1
c            IF (LDPART .OR. LDGAS) THEN
c               CALL VDP1
c            END IF
...
C        DATE:    November 8, 1993
C
C        MODIFIED:   To add the ARCFTSRC source group option; Aircraft.
C                    Gavendra Pandey; UNC-IE, Chapel Hill, NC, USA
C                    04/01/2023
C
...
C     Variable Declarations
      ...
      LOGICAL :: FOPEN
C       INTEGER :: I, J, N, ITOTSRC, ITOTGRP, NumBack, LNUM, KK  !CWSP 1/24/2024 D175 HOUREMIS Filename D121 Hourly Emissions File 
      INTEGER :: I, J, JJ, N, ITOTSRC, ITOTGRP, NumBack, LNUM, KK !CWSP 1/24/2024 D175 HOUREMIS Filename D121 Hourly Emissions File 
...
C              First save ILINE and reset to 1 to trigger L_HRLYSIG check
               ILSAVE = ILINE
CWSP ! 1/24/2025 D175 HOUREMIS Filename D121 Hourly Emissions File - Starting 
                DO JJ = 1, NSRC
                    IF (HOUREMISARRAY(JJ) == HOUREMISARRAY(ISRC)) THEN
                        IQLINE(JJ) = IQLINE(JJ) +1
                    END IF
                END DO
CWSP ! 1/24/2025 D175 HOUREMIS Filename D121 Hourly Emissions File - Ending
C               IQLINE = IQLINE + 1
               ILINE  = 1
               KURDAT = 0
CMGS           Check for aircraft source type for reading/setting
C              aircraft plume rise parameters. 
CMGS               CALL HRQREAD(I) !D151 - 6/5/2023
               IF((AFTSRC(I) .EQ. 'Y')) THEN
C*               Retrieve AIRCRAFT Source Parameters for This Hour     ---   CALL AHRQREAD
                 CALL AHRQREAD(I)
               ELSE
C*               Retrieve Source Parameters for This Hour     ---   CALL HRQREAD
            ...
            END IF
         END IF
CMGS           END - Check for aircraft source type
...
C     D128: Warning message when AREAMNDR is used without an AREA Source present
      IF (L_AREAMNDR .AND. NUMAREA .EQ. 0) THEN
          CALL ERRHDL(PATH,'RLINE','W','668','AREAMNDR')
      END IF

C     Check for open HOUREMIS file; if so, rewind file
      DO I = 1, NUMSRC    ! CWSP ! 1/24/2025 D175 HOUREMIS Filename D121 Hourly Emissions File - Ending
         IF (HOURLY .AND. HOUREMISARRAY(I) .GT. 0) THEN
C        Check for Hourly Emissions File
            INQUIRE( UNIT = HOUREMISARRAY(I), OPENED = FOPEN)
c            INQUIRE (UNIT=IHREMI,OPENED=FOPEN)   CWSP ! 1/24/2025 D175 HOUREMIS Filename D121 Hourly Emissions File - Ending

            IF (FOPEN) THEN
C               REWIND IHREMI  ! CWSP 1/24/2025 D175 HOUREMIS Filename D121 Hourly Emissions File
               REWIND (HOUREMISARRAY(I)) ! CWSP 2/24/2025 D175 HOUREMIS Filename D121 Hourly Emissions File
               IQLINE = 0
            END IF
         END IF
      END DO             !! CWSP ! 1/24/2025 D175 HOUREMIS Filename D121 Hourly Emissions File - Ending
...
C        Write Error Message:  No HBP sources defined with HBP
...
C        Write Error Message:  No Aircraft sources defined with ARCFTOPT
...
C     Chack that SCREEN option is FALSE when RLINE, BUOYLINE, SWPOINT, AREA, and LINE sources are present
C     D164 2/21/23 WSP
C     D164 6/7/2023 CRT - MDT decided this should be a warning rather than an error. Updated 'E' to 'W'.
C                         Update to separate if statments so a warning will be issued for each source type.
      IF(SCREEN) THEN
         IF (NBLTOTAL .GT. 0) THEN
C              Write Error Message:SCREEN incompatible with
C              Write Warning Message:SCREEN incompatible with
C            CALL ERRHDL(PATH,MODNAM,'E','731','BUOYLINE')
            CALL ERRHDL(PATH,MODNAM,'W','731','BUOYLINE')
         END IF
         IF (NRLINES .GT. 0) THEN
C              Write Error Message:SCREEN incompatible with
C              Write Warning Message:SCREEN incompatible with
C            CALL ERRHDL(PATH,MODNAM,'E','731','RLINE/RLINEXT')
            CALL ERRHDL(PATH,MODNAM,'W','731','RLINE/RLINEXT')
         END IF
         IF (NSWP .GT. 0) THEN
C              Write Error Message:SCREEN incompatible with
C              Write Warning Message:SCREEN incompatible with
C            CALL ERRHDL(PATH,MODNAM,'E','731','SWPOINT')
            CALL ERRHDL(PATH,MODNAM,'W','731','SWPOINT')
         END IF
         IF (NAREA .GT. 0) THEN
C              Write Error Message:SCREEN incompatible with
C              Write Warning Message:SCREEN incompatible with
C            CALL ERRHDL(PATH,MODNAM,'E','731','AREA')
            CALL ERRHDL(PATH,MODNAM,'W','731','AREA')
         END IF
         IF (NPOLY .GT. 0) THEN
C              Write Error Message:SCREEN incompatible with
C              Write Warning Message:SCREEN incompatible with
C            CALL ERRHDL(PATH,MODNAM,'E','731','AREAPOLY')
            CALL ERRHDL(PATH,MODNAM,'W','731','AREAPOLY')
         END IF
         IF (NCIRC .GT. 0) THEN
C              Write Error Message:SCREEN incompatible with
C              Write Warning Message:SCREEN incompatible with
C            CALL ERRHDL(PATH,MODNAM,'E','731','AREACIRC')
            CALL ERRHDL(PATH,MODNAM,'W','731','AREACIRC')
         END IF
         IF (NPIT .GT. 0) THEN
C              Write Error Message:SCREEN incompatible with
C              Write Warning Message:SCREEN incompatible with
C            CALL ERRHDL(PATH,MODNAM,'E','731','OPENPIT')
            CALL ERRHDL(PATH,MODNAM,'W','731','OPENPIT')
         END IF
         IF (NLINE .GT. 0) THEN
C              Write Error Message:SCREEN incompatible with
C              Write Warning Message:SCREEN incompatible with
C            CALL ERRHDL(PATH,MODNAM,'E','731','LINE')
...
CMKP  Check for negative NOX emissions for ARM2 and issue fatal error. D227 
CMKP  ARM2 calculations intended for total NOX emissions converted     D227
CMKP  to total NO2 impacts from all sources at each receptor.          D227
      ELSE IF (ARM2) THEN
...
C           Issue fatal error message; Group ALL not found
         ...
         END IF
         DO I = 1, NUMSRC                                            ! D227
            IF (AQS(I) .LT. 0.0D0) THEN                              ! D227
C              Write Error Message:  Negative emission rate for ARM2   D227
               CALL ERRHDL(PATH,MODNAM,'E','338',SRCID(I))           ! D227
            END IF                                                   ! D227
         END DO                                                      ! D227
      END IF                                                         ! D227
...
C
C        MODIFIED:    Removed the FLAT restictrion for RLINE/RLINEXT
C                     sources. The restriction still remains for barrier
C                     and depressed RLINE sources
C                     Wood, 7/5/22
C
...
C ---
C           RLINE source now has terrain, treat like other sources - Wood 7/6/2022
C ---          Read elevation for LINE or BUOYLINE source
C               IF (SRCTYP(ISRC).EQ.'LINE' .OR.
C     &             SRCTYP(ISRC).EQ.'BUOYLINE') THEN
               IF (SRCTYP(ISRC).EQ.'LINE' .OR.
     &             SRCTYP(ISRC).EQ.'BUOYLINE' .OR.
...
C                    source, to be simulated with 'FLAT' terrain option
                     L_FLATSRC(ISRC) = .TRUE.
                     NUMFLAT = NUMFLAT + 1
C ---                D167 Warning message about ZELEV and ZHILL inputs being ignored WSP 3/26/23
...
C                       WRITE Error Message:  Source elevation is missing
                     ...
                     END IF
                  END IF
C              Added RLINE source to IF-statment above with LINE and BL - Wood 7/6/2022
C ---          Read elevation for RLINE source
C               ELSE IF (SRCTYP(ISRC).EQ.'RLINE') THEN
C begin        Deleted flat restriction for RLINE source Wood 6-14-22
C                 IF(.NOT.FLAT) THEN
C ---                ERROR MESSAGE THAT RLINE SOURCE REQUIRES 'FLAT' MODELOPT
C                    CALL ERRHDL(PATH,MODNAM,'E','188','')
C                 END IF
C                  IF (IFC .EQ. 8 .AND. ELEV) THEN
C                    Write Warning Message for No Source Elevation with ELEV
C                     CALL ERRHDL(PATH,MODNAM,'W','205','ZS = 0.0')
C                     L_FLATSRC(ISRC) = .TRUE.
C                     NUMFLAT = NUMFLAT + 1
C                     AZS(ISRC) = 0.0D0
C                  ELSE IF (IFC .EQ. 9 .AND. FIELD(9) .EQ. 'FLAT') THEN
C ---                Source has been identified as a terrain-following
C                    source, to be simulated with 'FLAT' terrain option

CWSP --- Begin: D173 deleted erronious lines. WSP 7/25/2023
CWSP                      L_FLATSRC(ISRC) = .TRUE.
CWSP                      NUMFLAT = NUMFLAT + 1
CWSP                      AZS(ISRC) = 0.0D0
CWSP C ---                D167 Warning message about ZELEV and ZHILL inputs being ignored WSP 3/26/23
CWSP                         CALL ERRHDL(PATH,MODNAM,'W','752',SOID)
CWSP                   ELSE IF (IFC .EQ. 9) THEN
CWSP --- End: D173 deleted erronious lines. WSP 7/25/2023

C                    Retrieve Source lower elevation
C                     CALL STODBL(FIELD(9), ILEN_FLD, AZS(ISRC), IMIT)
C                    Check The Numerical Field
C                     IF (IMIT .NE. 1) THEN
C                        CALL ERRHDL(PATH,MODNAM,'E','208',KEYWRD)
C                     END IF
C                     IF (AZS(ISRC) .NE. 0.0D0) THEN
C ---                   ERROR MESSAGE THAT RLINE SOURCE REQUIRES Zs = FLAT or = 0.0
C                        CALL ERRHDL(PATH,MODNAM,'E','267',SOID)
C                     ELSE
C ---                   Source has been identified as a terrain-following
C                       source, to be simulated with 'FLAT' terrain option
C                        L_FLATSRC(ISRC) = .TRUE.
...
C                        AZS(ISRC) = 0.0D0
C                   END IF
C                  END IF !check field of RLINE source
C end        Deleted flat restriction for RLINE source Wood 6-14-22

C ---          Read elevation for RLINEXT source
               ELSE IF (SRCTYP(ISRC).EQ.'RLINEXT') THEN

C begin        Deleted flat restriction for RLINEXT source Wood 6-14-22                                                                       
C                  IF(.NOT.FLAT) THEN
C ---                ERROR MESSAGE THAT RLINEXT SOURCE REQUIRES 'FLAT' MODELOPT
C                     CALL ERRHDL(PATH,MODNAM,'E','188','')
C                  END IF

C                  IF (IFC .EQ. 10 .AND. ELEV) THEN
C                    Write Warning Message for No Source Elevation with ELEV
C                     CALL ERRHDL(PATH,MODNAM,'W','205','ZS = 0.0')
C                     L_FLATSRC(ISRC) = .TRUE.
C                     NUMFLAT = NUMFLAT + 1
C                     AZS(ISRC) = 0.0D0
C                  ELSE IF (IFC .EQ. 11 .AND. FIELD(11) .EQ. 'FLAT') THEN
C ---                Source has been identified as a terrain-following
C                    source, to be simulated with 'FLAT' terrain option
C                     L_FLATSRC(ISRC) = .TRUE.
C                     NUMFLAT = NUMFLAT + 1
C                     AZS(ISRC) = 0.0D0
C                  ELSE IF (IFC .EQ. 11) THEN
C                    Retrieve Source lower elevation
C                     CALL STODBL(FIELD(11), ILEN_FLD, AZS(ISRC), IMIT)
C                    Check The Numerical Field
C                     IF (IMIT .NE. 1) THEN
C                        CALL ERRHDL(PATH,MODNAM,'E','208',KEYWRD)
C                     END IF
...
C                        AZS(ISRC) = 0.0D0
C                     END IF
C end        Deleted flat restriction for RLINEXT source Wood 6-14-22

C begin           RLINEXT source now has terrain, treat like other source - Wood 7/5/2022
                  IF (IFC .EQ. 10) THEN
C                    No Source Elevation Field - Default to 0.0
                     AZS(ISRC) = 0.0D0
                     IF (ELEV) THEN
...
C                       WRITE Error Message:  Source elevation is missing
                     ...
                     END IF
                  END IF
C end           RLINEXT source now has terrain, treat like other source - Wood 7/5/2022
C                 END IF !check field of RLINEXT source
...
CMGS Removed warning when HS >100m for aircraft souces (D151 - WSP 5/25/2023)
      IF (AHS(ISDX) .LT. 0.0D0) THEN
C        WRITE Error Message:  Negative Release Height
         CALL ERRHDL(PATH,MODNAM,'E','209',' HS ')
CMGS      ELSE IF (AHS(ISDX) .GT. 100.0D0) THEN
...
C     Variable Declarations
      ...
      INTEGER :: I, K, IH, IL
      INTEGER :: J, MNUM        ! CWSP 1/24/2025 D175 HOUREMIS Filename D121 Hourly Emissions File

      LOGICAL FOPEN, INGRP
      LOGICAL MATCH_FOUND ! CWSP 1/24/2025 D175 HOUREMIS Filename D121 Hourly Emissions File
      LOGICAL RMARK
      CHARACTER (LEN=12) :: LOWID, HIGID, LID1, LID2, HID1, HID2, TEMPID
...
CWSP ! 1/24/2025 D175 HOUREMIS Filename D121 Hourly Emissions File - Starting 
            ! Loop through all elements in HOUREMISARRAY
            MATCH_FOUND = .FALSE.
            DO J = 1, NUMSRC
                IF (TRIM(HOUREMISNAMEARRAY(J)) .NE. "" .AND. 
     &           TRIM(HOUREMISNAMEARRAY(J)).NE. TRIM(HRFILE) .AND.
     &           HRSRCIDs(J) .NE. SRCID(J)) THEN
C                  WRITE Error Message:  Attempt to Define Duplicate HOUREMIS file
C JAT 5/13/2026 26XXX_DEV_BRANCH     change error code to 835
c                   CALL ERRHDL(PATH,MODNAM,'E','312','HOUREMIS')
                 CALL ERRHDL(PATH,MODNAM,'E','835','HOUREMIS')
                END IF
                IF (TRIM(HOUREMISNAMEARRAY(J)).EQ. TRIM(HRFILE)) THEN
                   MATCH_FOUND = .TRUE.
                   IHREMI = HOUREMISARRAY(J) ! Set IHREMI to whatever FUNIT previously used
            ...
            IF (.NOT. MATCH_FOUND) then
               ...
               FUNITINX = FUNITINX + 1
          ELSE
C              WRITE Warning Message:  Attempt to Define Duplicate HOUREMIS files
C JAT 5/13/2026 26XXX_DEV_BRANCH comment out message to avoid confusing warning message
c when a user uses an older AERMOD file where all hourly sources have to be in the same emissions file
C               CALL ERRHDL(PATH,MODNAM,'W','312','HOUREMIS')
            END IF
CWSP ! 1/24/2025 D175 HOUREMIS Filename D121 Hourly Emissions File - Ending
         ELSE
...
C - WSP 9/16/2024 D158 Dynamic File unit  Store FUNIT, FILENAME, and FILETYPE in array to add to output as table  - Starting       
               FUNITARRAY(FUNITIND) = IHREMI
               FNAMEARRAY(FUNITIND) = HRFILE
               FTYPARRAY(FUNITIND) = 'HOUREMIS'
               FUNITIND = FUNITIND + 1
C - WSP 9/16/2024 D158 Dynamic File unit  Store FUNIT, FILENAME, and FILETYPE in array to add to output as table  - Ending      
            END IF
         END IF
...
C     Set Up The Source Group Array
      IF (TEMPID .EQ. 'ALL') THEN
         DO K = 1, NUMSRC
            QFLAG(K) = 'HOURLY'
CWSP 1/24/2025 D175 HOUREMIS Filename D121 Hourly Emissions File - Starting
C           Check if the array has already been filled out, which means there are duplicate files/SRCIDs listed
            HOUREMISARRAY(K) = IHREMI     !! FUNIT            
            HOUREMISNAMEARRAY(K) = HRFILE !! Filename
            HRSRCIDs(K) =  SRCID(K) !CWSP 1/24/2025 D175 HOUREMIS Filename D121 Hourly Emissions File
CWSP 1/24/2025 D175 HOUREMIS Filename D121 Hourly Emissions File - Ending
C        Multiple_BuoyLines_D41_Wood: Begin
...
CWSP 1/24/2025 D175 HOUREMIS Filename D121 Hourly Emissions File - Starting 
C           Check if the array has already been filled out, which means there are duplicate files/SRCIDs listed
            IF (HOUREMISARRAY(K) == 0) THEN
               HOUREMISARRAY(K) = IHREMI     !! FUNITARRAY
            ELSE IF (HOUREMISARRAY(K) /= IHREMI) THEN
C              WRITE Error Message:  Possible Conflict With Dynamically Allocated FUNIT
               CALL ERRHDL(PATH,MODNAM,'E','565','HOUREMIS')
            END IF
            IF (HOUREMISNAMEARRAY(K) == '') THEN
               HOUREMISNAMEARRAY(K) = HRFILE !! Filename
          ELSE IF (HOUREMISNAMEARRAY(K) /= HRFILE) THEN
C              WRITE Error Message:  Attempt to Define Duplicate HOUREMIS file
CJAT 5/13/2026 26XXX_DEV_BRANCH change 312 to 835
!               CALL ERRHDL(PATH,MODNAM,'E','312','HOUREMIS')
             CALL ERRHDL(PATH,MODNAM,'E','835','HOUREMIS')
            END IF
            IF (HRSRCIDs(K) == '') THEN
               HRSRCIDs(K) =  SRCID(K) !CWSP 1/24/2025 D175 HOUREMIS Filename D121 Hourly Emissions File
          ELSE IF (HRSRCIDs(K) /= SRCID(K)) THEN
C              WRITE Error Message:  Attempt to Define Duplicate HOUREMIS file for SRC:
CJAT 5/13/2026 26XXX_DEV_BRANCH change 311 to 834
c               CALL ERRHDL(PATH,MODNAM,'E','311',SRCID(K))
             CALL ERRHDL(PATH,MODNAM,'E','834',SRCID(K))
            END IF
CWSP 1/24/2025 D175 HOUREMIS Filename D121 Hourly Emissions File - Ending
C              Multiple_BuoyLines_D41_Wood: Begin
...
C              Multiple_BuoyLines_D41_Wood: End
      ...
      HOUREMISIND = HOUREMISIND + 1  !CWSP 1/24/2025 D175 HOUREMIS Filename D121 Hourly Emissions File
      GO TO 999
...
C***********************************************************************
C                 HBPSOURCE Module of AERMOD Model
C
C        PURPOSE: Processes HBP Sources in Source Card
C
C        PROGRAMMER: Carlos Szembek (AECOM)
C
C        DATE:    January 18, 2023
C
C        INPUTS:  Input Runstream Image Parameters
C
C        OUTPUTS: Array of flags for HBP Sources
C
C        CALLED FROM: SOCARD
C***********************************************************************
C
...
C     Check for 'HBPSRCID ALL' option identified in PRESET
      IF (L_HBP_ALL) THEN
         IF (IFC .EQ. 3 .AND. FIELD(3) .EQ. 'ALL') THEN
            DO K = 1, NUMSRC
...
C           First check for whether file is open by file unit
     ...
     &              ACTION='READ',STATUS='OLD')
C - WSP 9/16/2024 D158 Dynamic File unit  Store FUNIT, FILENAME, and FILETYPE in array to add to output as table  - Starting       
               FUNITARRAY(FUNITIND) = IBGUNT(IBGSECT)
               FNAMEARRAY(FUNITIND) = BKGRND_File(IBGSECT)
               FTYPARRAY(FUNITIND) = 'BACKGROUND'
               FUNITIND = FUNITIND + 1
C - WSP 9/16/2024 D158 Dynamic File unit  Store FUNIT, FILENAME, and FILETYPE in array to add to output as table  - Ending      
            ELSE
...
C        WRITE Error Message:  Negative barrier height: HTWALL
 ...
 999  RETURN
      ...
      SUBROUTINE VBARRIER_INPUTS
C***********************************************************************
C
C     PURPOSE: Process RLINE vegetative barrier (VBARRIER) params
C
C     PROGRAMMER: Rebecca L Miller, WSP
C
C     DATE:    26 FEB 2025
C
C     INPUTS:  Input Runstream Image Parameters
C
C     OUTPUTS: RLSOURCE vegetative barrier parameters are populated
C
C     CALLED FROM: SOCARD
C***********************************************************************
C     Variable Declarations
      ...
      INTEGER             :: INDEXS !source index
      CHARACTER (LEN=12)  :: SOID   !source ID
      LOGICAL             :: FOUND  !source index/ID found
...
C     Check The Number Of The Fields
      IF (IFC .LT. 8) THEN
C        Error Message: Missing fields
         CALL ERRHDL(PATH,MODNAM,'E','201',KEYWRD)
         GO TO 999
...
C        Error Message: Too Many Parameters
         CALL ERRHDL(PATH,MODNAM,'E','202',KEYWRD)
         GO TO 999
      ELSE IF (IFC .GT. 8 .AND. IFC .LT. 13) THEN
C        Error Message: Missing fields
         CALL ERRHDL(PATH,MODNAM,'E','201',KEYWRD)
         GO TO 999
      END IF
...
C---     Fetch and check each of the numerical fields
C HT
         CALL STODBL(FIELD(4), ILEN_FLD, VEGBARR(INDEXS)%HT(1), IMIT)
C        Check The Numerical Field
         IF (IMIT .NE. 1) THEN
            CALL ERRHDL(PATH,MODNAM,'E','208',KEYWRD)
         END IF
C        Input height out of range         
         IF (VEGBARR(INDEXS)%HT(1) < 2.0D0 .OR. 
     &       VEGBARR(INDEXS)%HT(1) > 10.0D0) THEN
           CALL ERRHDL(PATH,MODNAM,'E','371',KEYWRD)
         END IF         
C WT
         CALL STODBL(FIELD(5), ILEN_FLD, VEGBARR(INDEXS)%WT(1), IMIT)
C        Check The Numerical Field
         IF (IMIT .NE. 1) THEN
            CALL ERRHDL(PATH,MODNAM,'E','208',KEYWRD)
         END IF
C        Input width out of range         
         IF (VEGBARR(INDEXS)%WT(1) < 2.5D0 .OR. 
     &       VEGBARR(INDEXS)%WT(1) > 13.0D0) THEN
           CALL ERRHDL(PATH,MODNAM,'E','372',KEYWRD)
         END IF
C DCL         
         CALL STODBL(FIELD(6), ILEN_FLD, VEGBARR(INDEXS)%DCL(1), IMIT)
C        Check The Numerical Field
         IF (IMIT .NE. 1) THEN
            CALL ERRHDL(PATH,MODNAM,'E','208',KEYWRD)
         END IF
...
C LAI
         CALL STODBL(FIELD(7), ILEN_FLD, VEGBARR(INDEXS)%LAI(1), IMIT)
C        Check The Numerical Field
         IF (IMIT .NE. 1) THEN
            CALL ERRHDL(PATH,MODNAM,'E','208',KEYWRD)
         END IF
C        Input LAI out of range
         IF (VEGBARR(INDEXS)%LAI(1) < 4.0D0 .OR. 
     &       VEGBARR(INDEXS)%LAI(1) > 10.92D0) THEN
           CALL ERRHDL(PATH,MODNAM,'E','373',KEYWRD)
         END IF         
C LM
         CALL STODBL(FIELD(8), ILEN_FLD, VEGBARR(INDEXS)%LM(1), IMIT)
C        Check The Numerical Field
         IF (IMIT .NE. 1) THEN
            CALL ERRHDL(PATH,MODNAM,'E','208',KEYWRD)
         END IF
C        Input LM out of range
         IF (VEGBARR(INDEXS)%LM(1) < 0.55D0 .OR. 
     &       VEGBARR(INDEXS)%LM(1) > 3.75D0) THEN
           CALL ERRHDL(PATH,MODNAM,'E','374',KEYWRD)
         END IF 
         
C---     Fetch and check numerical fields for second barrier
         IF (IFC .EQ. 13) THEN
C HT2
           CALL STODBL(FIELD(9), ILEN_FLD, VEGBARR(INDEXS)%HT(2), 
     &                 IMIT)
C          Check The Numerical Field
           IF (IMIT .NE. 1) THEN
              CALL ERRHDL(PATH,MODNAM,'E','208',KEYWRD)
           END IF
C          Input height out of range         
           IF (VEGBARR(INDEXS)%HT(2) < 2.0D0 .OR. 
     &         VEGBARR(INDEXS)%HT(2) > 10.0D0) THEN
             CALL ERRHDL(PATH,MODNAM,'E','371',KEYWRD)
           END IF            
C WT2
           CALL STODBL(FIELD(10), ILEN_FLD, VEGBARR(INDEXS)%WT(2), IMIT)
C          Check The Numerical Field
           IF (IMIT .NE. 1) THEN
            CALL ERRHDL(PATH,MODNAM,'E','208',KEYWRD)
           END IF
C          Input width out of range         
           IF (VEGBARR(INDEXS)%WT(2) < 2.5D0 .OR. 
     &         VEGBARR(INDEXS)%WT(2) > 13.0D0) THEN
             CALL ERRHDL(PATH,MODNAM,'E','372',KEYWRD)
           END IF         
C DCL2           
           CALL STODBL(FIELD(11), ILEN_FLD, VEGBARR(INDEXS)%DCL(2), 
     &                 IMIT)
C          Check The Numerical Field
           IF (IMIT .NE. 1) THEN
              CALL ERRHDL(PATH,MODNAM,'E','208',KEYWRD)
           END IF         
C LAI2
           CALL STODBL(FIELD(12), ILEN_FLD, VEGBARR(INDEXS)%LAI(2),IMIT)
C          Check The Numerical Field
           IF (IMIT .NE. 1) THEN
            CALL ERRHDL(PATH,MODNAM,'E','208',KEYWRD)
           END IF
C          Input LAI out of range
           IF (VEGBARR(INDEXS)%LAI(2) < 4.0D0 .OR. 
     &         VEGBARR(INDEXS)%LAI(2) > 10.92D0) THEN
             CALL ERRHDL(PATH,MODNAM,'E','373',KEYWRD)
           END IF          
C LM2
           CALL STODBL(FIELD(13), ILEN_FLD, VEGBARR(INDEXS)%LM(2), IMIT)
C          Check The Numerical Field
           IF (IMIT .NE. 1) THEN
            CALL ERRHDL(PATH,MODNAM,'E','208',KEYWRD)
           END IF 
C          Input LM out of range
           IF (VEGBARR(INDEXS)%LM(2) < 0.55D0 .OR. 
     &         VEGBARR(INDEXS)%LM(2) > 3.75D0) THEN
             CALL ERRHDL(PATH,MODNAM,'E','374',KEYWRD)
           END IF
         END IF
        ELSE
...
C     Check DCL 1&2 directions      
      IF (VEGBARR(INDEXS)%DCL(1).LT. 0.0D0 .AND. 
     &    VEGBARR(INDEXS)%DCL(2).LT. 0.0D0 .OR.
     &    VEGBARR(INDEXS)%DCL(1).GT. 0.0D0 .AND.
     &    VEGBARR(INDEXS)%DCL(2).GT. 0.0D0) THEN
      
C       Zero out furthest barrier
        IF (DABS(VEGBARR(INDEXS)%DCL(1)) .LT. 
     &      DABS(VEGBARR(INDEXS)%DCL(2))) THEN
        !zero barrier 2
          CALL ERRHDL(PATH,MODNAM,'W','375', 'VBARRIER 1')
          VEGBARR(INDEXS)%HT(2)  = 0.0D0
          VEGBARR(INDEXS)%WT(2)  = 0.0D0
          VEGBARR(INDEXS)%DCL(2) = 0.0D0
          VEGBARR(INDEXS)%LAI(2) = 0.0D0
          VEGBARR(INDEXS)%LM(2)  = 0.0D0
        ELSE IF (DABS(VEGBARR(INDEXS)%DCL(2)) .LT. 
     &            DABS(VEGBARR(INDEXS)%DCL(1))) THEN
        !zero barrier 1
          CALL ERRHDL(PATH,MODNAM,'W','375','VBARRIER 2')
          VEGBARR(INDEXS)%HT(1)  = 0.0D0
          VEGBARR(INDEXS)%WT(1)  = 0.0D0
          VEGBARR(INDEXS)%DCL(1) = 0.0D0
          VEGBARR(INDEXS)%LAI(1) = 0.0D0
          VEGBARR(INDEXS)%LM(1)  = 0.0D0
        END IF      
      END IF
      
 999  RETURN      
      END SUBROUTINE VBARRIER_INPUTS  

      SUBROUTINE VBARRIER_SETUP
C***********************************************************************
C
C     PURPOSE: RLINE vegetative barrier (VBARRIER) set up calculations
C
C     PROGRAMMER: Rebecca L Miller, WSP
C
C     DATE:    26 FEB 2025
C
C     INPUTS:  Vegetative barrier params: HT, WT, LM (RLSOURCE)
C
C     OUTPUTS: Wake for vegetative barriers
C
C     CALLED FROM: SOCARD
C***********************************************************************
C     Variable Declarations
...
C---- Calculate vegetative barrier wake zone
C     Loop through number of sources
      DO IS = 1,NSRC
C     Check RLINEXT source
        IF (SRCTYP(IS) .EQ. 'RLINEXT') THEN
         IF(RLSOURCE(IS)%HTWALL  .GT. 0.0D0 .OR.
     &      RLSOURCE(IS)%HTWALL2 .GT. 0.0D0) THEN
           CALL ERRHDL(PATH,MODNAM,'W','376',SRCID(IS))
           VEGBARR(IS)%HT(1) = 0.0D0
           VEGBARR(IS)%HT(2) = 0.0D0
         ELSE
          
          IF (VEGBARR(IS)%HT(1)  .GT. 0.0D0) THEN
C         Calculate wake for first veg barrier
            VEGBARR(IS)%WAKE(1) = VEGBARR(IS)%HT(1) * (3.03D0 * 
     &                          (VEGBARR(IS)%WT(1)**(-2.086D0))
     &                          +0.1042D0) * (39.0053D0 * 
     &                          (VEGBARR(IS)%LM(1)**(-0.7284D0)))
          END IF
C         Check if second barrier present          
          IF (VEGBARR(IS)%HT(2) .GT. 0.0D0) THEN
C         Calculate wake2 for second veg barrier
            VEGBARR(IS)%WAKE(2) = VEGBARR(IS)%HT(2) * (3.03D0 *
     &                           (VEGBARR(IS)%WT(2)**(-2.086D0))
     &                           +0.1042D0) * (39.0053D0 * 
     &                           (VEGBARR(IS)%LM(2)**(-0.7284D0)))
          END IF
      ...
      END SUBROUTINE VBARRIER_SETUP
      
      
      SUBROUTINE SBARRIER_INPUTS
C***********************************************************************
C
C     PURPOSE: Process RLINE solid barrier (SBARRIER) params
C
C     PROGRAMMER: Rebecca L Miller, WSP
C
C     DATE:    10 APRIL 2024
C
C     INPUTS:  Input Runstream Image Parameters
C
C     OUTPUTS: SLDBARR parameters are populated
C
C     CALLED FROM: SOCARD
C***********************************************************************
C     Variable Declarations
      ...
      CHARACTER(LEN=12) :: INBAR
      DOUBLE PRECISION  :: DPNUMSEG !used for conversion, character to DP to integer
      DOUBLE PRECISION :: XTEMP, YTEMP !for coordinate orientation flip
...
C     Retrieve Barrier ID Character Substring
      INBAR = FIELD(4)
      IF (INBAR.EQ.'STA') THEN
C       Barrier ID counter
        IBARID = IBARID + 1
C       read in BARID
        SLDBARR(IBARID)%BARID = FIELD(3)
C       read in number of segments
        CALL STODBL(FIELD(5), ILEN_FLD, DPNUMSEG, IMIT)
        SLDBARR(IBARID)%NUMSEG = INT(DPNUMSEG)
        IF (IMIT .NE. 1) THEN
          CALL ERRHDL(PATH,MODNAM,'E','208',KEYWRD)
        END IF
C       NUMSEG range 1-50
        IF (SLDBARR(IBARID)%NUMSEG.LT.1 .OR. 
     &      SLDBARR(IBARID)%NUMSEG.GT.50) THEN
          CALL ERRHDL(PATH,MODNAM,'E','320',SLDBARR(IBARID)%BARID)
        END IF
C       Initialize segment counter/reset for new BARID        
        INUMSEG = 1
        
      ELSE IF (INBAR.EQ.'END') THEN
        IF (SLDBARR(IBARID)%NUMSEG .NE. INUMSEG-1) THEN
C         user input number of segments does not match segments read in
          CALL ERRHDL(PATH,MODNAM,'E','306',SLDBARR(IBARID)%BARID)
        END IF
        GO TO 999
        
      ELSE !read in segment information
C       read in XBB      
        CALL STODBL(FIELD(4), ILEN_FLD, SLDBARR(IBARID)%XBB(INUMSEG),
     &              IMIT)
        IF (IMIT .NE. 1) THEN
          CALL ERRHDL(PATH,MODNAM,'E','208',KEYWRD)
        END IF
C       read in YBB
        CALL STODBL(FIELD(5), ILEN_FLD, SLDBARR(IBARID)%YBB(INUMSEG),
     &              IMIT)
        IF (IMIT .NE. 1) THEN
          CALL ERRHDL(PATH,MODNAM,'E','208',KEYWRD)
        END IF
C       read in XBE
        CALL STODBL(FIELD(6), ILEN_FLD, SLDBARR(IBARID)%XBE(INUMSEG),
     &              IMIT)
        IF (IMIT .NE. 1) THEN
          CALL ERRHDL(PATH,MODNAM,'E','208',KEYWRD)
        END IF
C       read in YBE
        CALL STODBL(FIELD(7), ILEN_FLD, SLDBARR(IBARID)%YBE(INUMSEG),
     &              IMIT)
        IF (IMIT .NE. 1) THEN
          CALL ERRHDL(PATH,MODNAM,'E','208',KEYWRD)
        END IF
        
C       Check order of end points, switch order if not West to East (or North to South)
        IF (SLDBARR(IBARID)%XBB(INUMSEG) .GT. 
     &      SLDBARR(IBARID)%XBE(INUMSEG)) THEN !switch order (W-E orientation)
            XTEMP = SLDBARR(IBARID)%XBB(INUMSEG)
            YTEMP = SLDBARR(IBARID)%YBB(INUMSEG)
            SLDBARR(IBARID)%XBB(INUMSEG) = SLDBARR(IBARID)%XBE(INUMSEG)
            SLDBARR(IBARID)%XBE(INUMSEG) = XTEMP
            SLDBARR(IBARID)%YBB(INUMSEG) = SLDBARR(IBARID)%YBE(INUMSEG)
            SLDBARR(IBARID)%YBE(INUMSEG) = YTEMP            
        ELSE IF ((SLDBARR(IBARID)%XBB(INUMSEG) .EQ. 
     &            SLDBARR(IBARID)%XBE(INUMSEG)) .AND. 
     &           (SLDBARR(IBARID)%YBB(INUMSEG) .LT. 
     &            SLDBARR(IBARID)%YBE(INUMSEG))) THEN !N-S orientation
            !switch order (N-S orientation)
            XTEMP = SLDBARR(IBARID)%XBB(INUMSEG)
            YTEMP = SLDBARR(IBARID)%YBB(INUMSEG)
            SLDBARR(IBARID)%XBB(INUMSEG)= SLDBARR(IBARID)%XBE(INUMSEG)
            SLDBARR(IBARID)%XBE(INUMSEG)= XTEMP
...
C       read in HT        
        CALL STODBL(FIELD(8), ILEN_FLD, SLDBARR(IBARID)%HT(INUMSEG),
     &              IMIT)
        IF (IMIT .NE. 1) THEN
          CALL ERRHDL(PATH,MODNAM,'E','208',KEYWRD)
        END IF
C       Input height should be >0
        IF (SLDBARR(IBARID)%HT(INUMSEG).LE.2.0D0 .OR.
     &      SLDBARR(IBARID)%HT(INUMSEG).GT.12.0D0) THEN
          CALL ERRHDL(PATH,MODNAM,'E','320',SLDBARR(IBARID)%BARID)
        END IF
C       read in Z elevation        
        CALL STODBL(FIELD(9), ILEN_FLD, SLDBARR(IBARID)%Z(INUMSEG),
     &              IMIT)
        IF (IMIT .NE. 1) THEN
          CALL ERRHDL(PATH,MODNAM,'E','208',KEYWRD)
        END IF
        IF (SLDBARR(IBARID)%Z(INUMSEG).GT.0.0D0 .OR. 
     &      SLDBARR(IBARID)%Z(INUMSEG).LT.0.0D0) THEN
          !barrier elevations currently ignored/assumed zero
          CALL ERRHDL(PATH,MODNAM,'W','326',KEYWRD)
        END IF
C       increase segment counter        
        INUMSEG = INUMSEG + 1
      END IF

 999  RETURN      
      END SUBROUTINE SBARRIER_INPUTS
      
      SUBROUTINE SBARRIER_DCL
C***********************************************************************
C     This module calculates the DCL (magnitude and sign) from user
C     input end points for a solid barrier and RLINEXT source.
C     This module determines if barrier is impactful to the source,
C     e.g., within 10*height of source, parallel, and overlaps source.
C
C     SBARRIER_DCL Module of the AMS/EPA Regulatory Model - AERMOD
C
C     PURPOSE:
C
C     PROGRAMMER: Rebecca L Miller, WSP
...
C     Variable Declarations
      ...
      INTEGER          :: IS, IB, IBS, I !loop indeces
      DOUBLE PRECISION :: XSB, XSE, YSB, YSE
      DOUBLE PRECISION :: Blength, Slength, Glength
      DOUBLE PRECISION :: RXBE, RYBE, SLOPE, B, BSDIST
      DOUBLE PRECISION :: SXBB, SXBE, SYBB, SYBE, XMP, YMP
      DOUBLE PRECISION :: SBa, SBb, SEa, SEb
      DOUBLE PRECISION :: SBal, SBbe, SBch, 
     &                    SEal, SEbe, SEch
      DOUBLE PRECISION :: DCLSB, DCLSE, DCL, HT
      DOUBLE PRECISION :: MPang, Sang, BSANG, PARTHETA
      DOUBLE PRECISION :: STHETA, THETADIF, THETAI,
     &                    ALI,BEI, SCHI, SSLGTH, FSBOVRLP
      LOGICAL          :: LEFTEXT, RIGHTEXT, DCLpos, GAP, PAR
      DOUBLE PRECISION, ALLOCATABLE :: BSDCL(:), BSOVRLAP(:), 
     &                                 BSRLHT(:), SOVRLP(:)
      INTEGER  :: ALLOCATESTATUS, ALLOCERROR
      LOGICAL  :: INFL
            
      !XSB, XSE, YSB, YSE - source enpoints, stored locally incase of order switch
      !Blength  - barrier length
      !Slength  - source length
      !Glength  - gap length between barrier segments
      !RXBE, RYBE - segment rotation cordinates
      !SLOPE    - slope for rotation line about segment midpoint
      !B        - intercept for rotation line about segment midpoint
      !BSDIST   - distance of rotated segment to find new end points
      !SXBB, SXBE, SYBB, SYBE - segment end points
      !XMP, YMP - segment midpoint coordinates
      !SBa, SBb - side length (a,b) relative to source beginning point
      !SEa, SEb - side length (a,b) relative to source end point
      !SBal, SBbe, SBch - angles of barrier relative to source beginning point
      !SEal, SEbe, SEch - angles of barrier relative to source end point
      !DCLSB    - DCL at source beginning point
      !DCLSE    - DCL at source end point
      !DCL      - weighted average segment DCL
      !HT       - weighted average segment height
      !MPang    - midpoint to midpoint angle
      !Sang     - source line angle
      !BSANG    - barrier-segment angle
      !PARTHETA - parallel angle check
      !STHETA   - segment-segment angle
      !THETADIF - angle difference between consecutive segments
      !THETAI   - angle of segment(IBS) used in THETADIF
      !ALI, BEI, SCHI - angles of aegment-to-segment overlap triangle
      !SSLGTH   - length between consecutive segments
      !FSBOVRLP - final segment to barrier overlap %
      !LEFTEXT  - source extends to the left of barrier
      !RIGHTEXT - source extends to the right of barrier
      !DCLpos   - DCL sign, intially assumed positive
      !GAP      - logical switch for gap check warning
      !PAR      - logical switch for parallel warning
      !BSDCL    - segment DCL array
      !BSOVRLAP - segment overlap array
      !BSRLHT   - segment relative height array, currently ignored/set to barrier segment height only
      !SOVRLP   - segment-segment overlap      
      !ALLOCATESTATUS  - flag for intermediate array allocation error
      !ALLOCERROR      - status flag for error of allocation
      !INFL            - flag for if barrier is set to zero or not, "influence"
      
      DOUBLE PRECISION, EXTERNAL  :: WEIGHT_SUM !function
...
C     Debug file size warning
      IF (BARDBG .AND. NSBR*NRLINES.GE.50) THEN
        CALL ERRHDL(PATH,MODNAM,'W','312',"SBAR DEBUG")
      END IF

CRLM Debug file opened here instead of SUBROUTINE OPEN_ALL_DEBUGFILES    
      IF (BARDBG) THEN
C        Open SBARRIER debug output file
          DUMMY = 'SBARDBGFIL'
          CALL OPEN_DEBUGFILE(SBRDBUNT,SBARDBGFIL,MODNAM,DUMMY,
     &                        'SBARRIER','SBRDBUNT')
       END IF
      
C     Loop each barrier and source pairing to calculate DCL
C     Loop through number of sources
      DO IS = 1,NSRC
C     Check RLINEXT source
        IF (SRCTYP(IS) .EQ. 'RLINEXT') THEN
C       Check order of end points, switch order if not West to East (or North to South)
C       Local switch only
            IF (RLSOURCE(IS)%XSB .GT. RLSOURCE(IS)%XSE) THEN 
            !switch order (W-E orientation)
              XSB = RLSOURCE(IS)%XSE
              XSE = RLSOURCE(IS)%XSB
              YSB = RLSOURCE(IS)%YSE
              YSE = RLSOURCE(IS)%YSB            
            ELSE IF ((RLSOURCE(IS)%XSB .EQ. RLSOURCE(IS)%XSE) .AND. !N-S orientation
     &               (RLSOURCE(IS)%YSB .LT. RLSOURCE(IS)%YSE)) THEN 
            !switch order (N-S orientation)
              XSB = RLSOURCE(IS)%XSE
              XSE = RLSOURCE(IS)%XSB
              YSB = RLSOURCE(IS)%YSE
...
C         Calculate source length
          Slength = DSQRT((XSB - XSE)**2 + (YSB - YSE)**2)
          IF (Slength .LE. 0.0D0) THEN
C           Source length must be greater than 0          
            CALL ERRHDL(PATH,MODNAM,'E','309', SRCID(IS))
          END IF
C         Calculate source angle
          Sang  = MODULO(DATAN2((YSE - YSB),(XSE - XSB))*RTODEG,360.0D0)

C         Initialize BARID(2) to NOBARID
          RLSOURCE(IS)%BARID  = "NOBARID"
          RLSOURCE(IS)%BARID2 = "NOBARID"

C         Loop through barriers (by BARID)
          DO IB = 1,NSBR
C           ALLOCATE BSDCL, BSOVRLAP, BSRLHT, SOVRLP
            ALLOCATE(BSDCL(SLDBARR(IB)%NUMSEG),
     &               BSOVRLAP(SLDBARR(IB)%NUMSEG), 
     &               BSRLHT(SLDBARR(IB)%NUMSEG),
     &               SOVRLP(SLDBARR(IB)%NUMSEG),
     &               STAT=ALLOCATESTATUS)
C           Initialize BSDCL, BSOVRLAP, BSRLHT, SOVRLP
            BSDCL(:)    = 0.0D0
            BSOVRLAP(:) = 0.0D0
            BSRLHT(:)   = 0.0D0            
            SOVRLP(:)   = 0.0D0

C           Segment debug headers            
            IF (BARDBG) WRITE(SBRDBUNT,100)
          
C             Loop segments
              DO IBS = 1,SLDBARR(IB)%NUMSEG
              
C               Relative height of each segment
                BSRLHT(IBS) = SLDBARR(IB)%HT(IBS) 
C     &                        + SLDBARR(IB)%Z(IBS) 
C     &                        - AZS(IS)
     
C************** Check for gaps between segments
                IF (IBS .GT. 1) THEN
C                 If end points are the same, skip gap check     
                  IF (SLDBARR(IB)%XBE(IBS-1).EQ.SLDBARR(IB)%XBB(IBS) 
     &               .AND.
     &                SLDBARR(IB)%YBE(IBS-1).EQ.SLDBARR(IB)%YBB(IBS)) 
     &            THEN
                    !skip gap check
                    GO TO 99
                  END IF
                
C                 Find angle between segment IBS(begin) and IBS-1(end)
C                  STHETA = MODULO(DATAN2((SLDBARR(IB)%YBB(IBS) - 
C     &                     SLDBARR(IB)%YBE(IBS-1)),(SLDBARR(IB)%XBB(IBS) 
C     &                     - SLDBARR(IB)%XBE(IBS-1)))*RTODEG,360.0D0)
C                 Angle relative to previous segment angle
C                 Find angle of previous segment and subtract from STHETA
                  STHETA = MODULO((DATAN2((SLDBARR(IB)%YBE(IBS-1) - 
     &                     SLDBARR(IB)%YBB(IBS)),(SLDBARR(IB)%XBE(IBS-1) 
     &                   - SLDBARR(IB)%XBB(IBS)))*RTODEG)
     &                   -(DATAN2((SLDBARR(IB)%YBB(IBS-1) - 
     &                     SLDBARR(IB)%YBE(IBS-1)),
     &                    (SLDBARR(IB)%XBB(IBS-1) - 
     &                     SLDBARR(IB)%XBE(IBS-1)))*RTODEG),360.0D0)
C                 Calculate length between segment IBS(begin) and IBS-1(end)                  
                  SSLGTH = DSQRT((SLDBARR(IB)%XBB(IBS) - 
     &                           SLDBARR(IB)%XBE(IBS-1))**2
     &                        + (SLDBARR(IB)%YBB(IBS) - 
     &                           SLDBARR(IB)%YBE(IBS-1))**2)
                  
                  IF (STHETA .LE. 90.0D0 .OR. STHETA .GE. 270.0D0) THEN
                    Glength = SSLGTH
                    !no segment-to-segment overlap
                  ELSE IF (STHETA.GT.90.0D0 .AND. STHETA.LT.270.0D0)
     &            THEN
C                   Compare angle of two segments
                    THETAI = MODULO(DATAN2((SLDBARR(IB)%YBB(IBS) - 
     &                     SLDBARR(IB)%YBE(IBS)),(SLDBARR(IB)%XBB(IBS) 
     &                   - SLDBARR(IB)%XBE(IBS)))*RTODEG, 180.0D0)
                    THETADIF = MODULO(THETAI -
     &                    (DATAN2((SLDBARR(IB)%YBB(IBS-1) - 
     &                     SLDBARR(IB)%YBE(IBS-1)),
     &                    (SLDBARR(IB)%XBB(IBS-1) - 
     &                     SLDBARR(IB)%XBE(IBS-1)))*RTODEG), 180.0D0)
                    IF (THETADIF .GT. 90.0D0) THEN
C                   Find gap between closest points, begin point of seg(IBS) is not closest point                     
                      ALI  = MODULO(STHETA - 90.0D0,  180.0D0)
                      SCHI = MODULO(STHETA - THETAI,  180.0D0)
                      BEI  = MODULO(180 - ALI - SCHI, 180.0D0)

                      Glength = SSLGTH*(DSIND(SCHI)/DSIND(BEI))
C                   Calculate segment-to-segment overlap, SOVRLP
                    !Blength from IBS-1                      
                      SOVRLP(IBS) = Glength*DTAND(ALI)/Blength
                    ELSE
C                   Begin point of seg(IBS) is closest point to find gap
C                   Glength, perpendicular line to segment(IBS-1) from segment(IBS) beginning point
                      ALI = 180.0D0 - STHETA
C                   Distance from segment(IBS-1) end point and segment(IBS) beginning point
C                   triangle hypotenuse
                      Glength     = SSLGTH*DSIND(ALI)
C                   Calculate segment-to-segment overlap, SOVRLP
                    !Blength from IBS-1
                      SOVRLP(IBS) = SSLGTH*DCOSD(ALI)/Blength
                    END IF

C These equations only work if segments are parallel to each other. Updated above. 20241016 WSP
C                 Calculate perpendicular length between segments, Glength
C                    SSa =     DSQRT((SLDBARR(IB)%XBB(IBS) - 
C     &                               SLDBARR(IB)%XBE(IBS-1))**2
C     &                            + (SLDBARR(IB)%YBB(IBS) - 
C     &                               SLDBARR(IB)%YBE(IBS-1))**2)
C                    SSb =     DSQRT((SLDBARR(IB)%XBE(IBS) - 
C     &                               SLDBARR(IB)%XBE(IBS-1))**2
C     &                            + (SLDBARR(IB)%YBE(IBS) - 
C     &                               SLDBARR(IB)%YBE(IBS-1))**2)
C                    SSc =     DSQRT((SLDBARR(IB)%XBB(IBS) - 
C     &                               SLDBARR(IB)%XBE(IBS))**2
C     &                            + (SLDBARR(IB)%YBB(IBS) - 
C     &                               SLDBARR(IB)%YBE(IBS))**2)     
C                    !angles for segment-to-segment comparison triangle
C                    SSal = DACOS((SSb**2 + SSc**2 - SSa**2)
C     &                        / (2*SSb*SSc))
C                    !perpendicular gap length
C                    Glength = SSb*DSIN(SSal)
C                 Calculate segment-to-segment overlap, SOVRLP
C                    !Blength from IBS-1
C                    SOVRLP(IBS) = (SSc-(SSb*DCOS(SSal)))/Blength

                  ELSE !Error, check input order of segment end points
                  END IF
                  
                  IF (.NOT.GAP .AND. DABS(Glength).GT.1.0D0) THEN 
                    !Gap greater than 1m between segment IBS-1 and IBS
                    GAP = .TRUE. !W304 called below, once per barid
                  END IF
                        
                END IF !Gap check between segments
   99 CONTINUE
   
C************** Parallel check between segment and source
C               Compare angles; parallel if within 3deg
C               Nonparallel barrier segments will be rotated about midpoint 
C               to parallel with source to find "effective" DCL

                BSANG = MODULO(DATAN2((SLDBARR(IB)%YBE(IBS)-
     &                  SLDBARR(IB)%YBB(IBS)),(SLDBARR(IB)%XBE(IBS)-
     &                  SLDBARR(IB)%XBB(IBS)))*RTODEG,360.0D0)
     
                PARTHETA = MODULO(Sang-BSANG, 180.0D0)
                IF (PARTHETA .EQ. 90.0D0) THEN
                  !segment perpendicular to source, skip segment
                  PAR = .TRUE.
                  !rotation methods fail when 90deg, rest of methods will show zero overlap
                  SXBB = SLDBARR(IB)%XBB(IBS)
                  SXBE = SLDBARR(IB)%XBE(IBS)
                  SYBB = SLDBARR(IB)%YBB(IBS)
                  SYBE = SLDBARR(IB)%YBE(IBS)                  
                ELSE IF (PARTHETA .GT. 3.0D0) THEN
                  !Segment not parallel, rotate to source
                  PAR = .TRUE.

                  !rotate to parallel with source about segment midpoint
                  XMP = (SLDBARR(IB)%XBB(IBS) + SLDBARR(IB)%XBE(IBS))/2
                  YMP = (SLDBARR(IB)%YBB(IBS) + SLDBARR(IB)%YBE(IBS))/2
                  !rotated end points (RXBE,RYBE)
C               only CW rotation needed because of angle difference and mod 180deg
C                  IF (BSANG .GT. 270.0D0) THEN !rotate CCW
C                    RXBE = ((SLDBARR(IB)%XBE(IBS)-XMP)
C     &                     *DCOSD(MODULO(DABS(Sang-BSANG),90.0D0)) 
C     &                     -(SLDBARR(IB)%YBE(IBS)-YMP)
C     &                     *DSIND(MODULO(DABS(Sang-BSANG),90.0D0))) +XMP
C                    
C                    RYBE = ((SLDBARR(IB)%YBE(IBS)-YMP)
C     &                     *DCOSD(MODULO(DABS(Sang-BSANG),90.0D0)) 
C     &                     +(SLDBARR(IB)%XBE(IBS)-XMP)
C     &                     *DSIND(MODULO(DABS(Sang-BSANG),90.0D0))) +YMP
C                  ELSE !rotate CW
                    RXBE = ((SLDBARR(IB)%XBE(IBS)-XMP)
     &                     *DCOSD(MODULO(DABS(Sang-BSANG),180.0D0))
     &                     +(SLDBARR(IB)%YBE(IBS)-YMP)
     &                     *DSIND(MODULO(DABS(Sang-BSANG),180.0D0)))+XMP
                    
                    RYBE = ((SLDBARR(IB)%YBE(IBS)-YMP)
     &                     *DCOSD(MODULO(DABS(Sang-BSANG),180.0D0))
     &                     -(SLDBARR(IB)%XBE(IBS)-XMP)
     &                     *DSIND(MODULO(DABS(Sang-BSANG),180.0D0)))+YMP
C                  END IF
                  !find slope and intercept of MP to rotated end point line
                  SLOPE = (RYBE - YMP)/(RXBE - XMP)
                  B = YMP - (SLOPE*XMP)
                  !Distance about midpoint
                  BSDIST = ((DSQRT((SLDBARR(IB)%XBB(IBS) - 
     &                             SLDBARR(IB)%XBE(IBS))**2
     &                          + (SLDBARR(IB)%YBB(IBS) - 
     &                             SLDBARR(IB)%YBE(IBS))**2))/2)
     &                          *DCOSD(MODULO(DABS(BSANG-Sang),90.0D0))
                  !New end points, rotated about midpoint of segment with new distance
                  SXBB = XMP - DSQRT(BSDIST**2 /(1+SLOPE**2))
                  SXBE = XMP + DSQRT(BSDIST**2 /(1+SLOPE**2))
                  SYBB = (SLOPE*SXBB) + B
                  SYBE = (SLOPE*SXBE) + B
                ...
                Blength = DSQRT((SXBB - SXBE)**2
     &                        + (SYBB - SYBE)**2) 
 
C              Calculate source-begining point triangle sides
               SBa = DSQRT((SXBB - XSB)**2
     &                   + (SYBB - YSB)**2)
               SBb = DSQRT((SXBE - XSB)**2
     &                   + (SYBE - YSB)**2)               
C              Calculate source-end point triangle sides
               SEa = DSQRT((SXBB - XSE)**2
     &                   + (SYBB - YSE)**2)
               SEb = DSQRT((SXBE - XSE)**2
     &                   + (SYBE - YSE)**2)
                
C               Calculate source-begining point triangle angles
                SBal = DACOS((SBb**2 + Blength**2 - SBa**2)
     &                        / (2*SBb*Blength))
                SBbe = DACOS((SBa**2 + Blength**2 - SBb**2)
     &                        / (2*SBa*Blength))
                SBch = DACOS((SBa**2 + SBb**2 - Blength**2)
     &                        / (2*SBa*SBb))
C               Calculate source-end point triangle angles
                SEal = DACOS((SEb**2 + Blength**2 - SEa**2)
     &                        / (2*SEb*Blength))
                SEbe = DACOS((SEa**2 + Blength**2 - SEb**2)
     &                        / (2*SEa*Blength))
                SEch = DACOS((SEa**2 + SEb**2 - Blength**2)
     &                        / (2*SEa*SEb))
                
C               Calculate DCL for both source end points
                DCLSB = SBb*(DSIN(SBal))
                DCLSE = SEb*(DSIN(SEal))
                
C  Can fail at certain angles when nonparallel, new methods above. WSP 20241016
CC               Compare DCLs to check if parallel
C                DCLratio = DCLSB/DCLSE
C                IF (.NOT.PAR .AND. 
C    &             (DCLratio .LT. 0.95 .OR. DCLratio .GT. 1.05)) THEN
CC                 Not parallel
C                  PAR = .TRUE.
C                END IF
             
C               Determine overlap
C               Average/weight final DCL magnitude according to overlap
                IF ((SBal*RTODEG.GT.90 .AND. SEal*RTODEG.GT.90).OR.
     &              (SBbe*RTODEG.GT.90 .AND. SEbe*RTODEG.GT.90).OR.
     &              (SBch*RTODEG.GT.90 .AND. SEch*RTODEG.GT.90))   
     &              THEN
                !no overlap (same angle GT 90deg)
                !skip segment, not included in any averages
                !warning
                  !BSHT, BSDCL, BSOVRLAP initialized to 0 already
                ELSE IF (SBal*RTODEG.LE.90 .AND. SEal*RTODEG.LE.90 
     &                   .AND.                                     
     &                   SBbe*RTODEG.LE.90 .AND. SEbe*RTODEG.LE.90)
     &                   THEN
                  !full overlap, barrier fully covers source (alpha and beta LE 90deg)
                  BSOVRLAP(IBS) = Blength/Slength
                  BSDCL(IBS) = (DCLSB + DCLSE)/2
                ELSE
                !partial overlap
                 IF (SBbe*RTODEG.GT.90 .AND. SEbe*RTODEG.LE.90) THEN
                   !source extends to the left
                   LEFTEXT  = .TRUE.
                 END IF
                 IF (SBal*RTODEG.LT.90 .AND. SEal*RTODEG.GT.90) THEN
                   !source extends to the right
                   RIGHTEXT = .TRUE.
                 END IF
                  
                 IF (LEFTEXT .AND. RIGHTEXT) THEN
                   !both left and right extension
                   BSOVRLAP(IBS) = Blength/Slength
                   BSDCL(IBS) = (DCLSB + DCLSE)/2
                 ELSE IF (LEFTEXT) THEN
                   BSOVRLAP(IBS) = DABS((SEa*DCOS(SEbe)) / Slength)
                   BSDCL(IBS) = ((1-BSOVRLAP(IBS))*DCLSB) + 
     &                              (BSOVRLAP(IBS)*DCLSE)
                 ELSE IF (RIGHTEXT) THEN
                   BSOVRLAP(IBS) = DABS((SBb*DCOS(SBal)) / Slength)
                   BSDCL(IBS) = (BSOVRLAP(IBS)*DCLSB) + 
     &                         ((1-BSOVRLAP(IBS))*DCLSE)
                 ELSE
                  !partial overlap with no extensions...
                  !Check order of end points
                 END IF
                 
                END IF

C               DCL sign, direction from source mid point to barrier mid point
                !midpoint to midpoint angle
                MPang = MODULO(DATAN2(
     &                  ((SLDBARR(IB)%YBB(IBS)+SLDBARR(IB)%YBE(IBS))/2 -
     &                   (YSB+YSE)/2),        
     &                  ((SLDBARR(IB)%XBB(IBS)+SLDBARR(IB)%XBE(IBS))/2
     &                  -(XSB+XSE)/2))
     &                  *RTODEG, 360.0D0)
                !DCL assumed positive, check if negative
                IF (Sang .EQ. 0 .OR. Sang .EQ. 180) THEN
                !source is horizontal, check north/south orientation
                  IF (MPang .GT. 180) THEN
                    !South Negative
                    DCLpos = .FALSE.
                  END IF
                ELSE
                !check east/west orientation
                  IF (MPang .GT. 90 .AND. MPang .LE. 270) THEN
                    !West - Negative
                    DCLpos = .FALSE.
                  END IF
                END IF
                !update DCL sign if negative
                IF (.NOT.DCLpos) THEN
                  BSDCL(IBS) = -BSDCL(IBS)
                  DCLpos = .TRUE. !reset
                END IF
                
C               Reset logicals for next segment                
                LEFTEXT  = .FALSE.
                RIGHTEXT = .FALSE.
                
C               Debug output for each segment
                IF (BARDBG) THEN
                  WRITE(SBRDBUNT,101) SRCID(IS), SLDBARR(IB)%BARID,
     &            IBS, BSRLHT(IBS), Blength, Glength,
     &            BSOVRLAP(IBS)*100, PARTHETA, BSDCL(IBS)
                END IF
...
C             Gap check
              IF (GAP) THEN
                CALL ERRHDL(PATH,MODNAM,'W','304',SLDBARR(IB)%BARID)
                GAP = .FALSE.
              END IF
C             Parallel check
              IF (PAR) THEN
                CALL ERRHDL(PATH,MODNAM,'W','307',SLDBARR(IB)%BARID)
                PAR = .FALSE.
              END IF
              
              !"combine" segments for single height and DCL per BARID
              !if weights (overlap) sum to less than 50%, weight_sum set to 0.0D0
              DCL = WEIGHT_SUM(SRCID(IS), SLDBARR(IB)%BARID, 
     &              SLDBARR(IB)%NUMSEG, BSOVRLAP, BSDCL)
              HT  = WEIGHT_SUM(SRCID(IS), SLDBARR(IB)%BARID, 
     &              SLDBARR(IB)%NUMSEG, BSOVRLAP, BSRLHT)
     
C             Check overlap, subtracting segment-to-segment overlap
              DO I = 1,SLDBARR(IB)%NUMSEG
                FSBOVRLP = FSBOVRLP + DABS(BSOVRLAP(I)*(1-SOVRLP(I)))
              END DO
              
              !if FSBOVRLP<90%, W308 set DCL & HT to 0.0D0
              IF (FSBOVRLP .LT. 0.9D0) THEN
                CALL ERRHDL('SO','SBAR_DCL','W','308',SLDBARR(IB)%BARID)
                INFL = .FALSE.
              END IF              
     
C             Check barrier distance from source
              IF (DCL .GT. 10.0D0*HT) THEN
                !outside allowed distance warning
                CALL ERRHDL(PATH,MODNAM,'W','311',SLDBARR(IB)%BARID)
                INFL = .FALSE.
              END IF 

C             Debug output for final barrier height and dcl
              IF (BARDBG) THEN
                WRITE(SBRDBUNT,200) !headers
                WRITE(SBRDBUNT,201) SRCID(IS), SLDBARR(IB)%BARID, 
     &                              HT, DCL, FSBOVRLP*100, INFL
                IF (.NOT.INFL) THEN
                  WRITE(SBRDBUNT,205)
                END IF
                IF (FSBOVRLP .LT. 0.9D0) THEN
                  !barrier does not meet overlap threshold
                  WRITE(SBRDBUNT,202)
                END IF
                IF (DCL .GT. 10.0D0*HT) THEN
                  !barrier too far away
                  WRITE(SBRDBUNT,203)
                END IF
              ...
              IF (.NOT.INFL) THEN
                DCL = 0.0D0
                HT  = 0.0D0
                INFL = .TRUE. !reset flag
              END IF        

C             Assign BARID (DCL and HT) to RLSOURCE for RLINE processing
C             Keep closest (DCL) barrier to source. Can have one barrier on each side of source (sign).
              IF (DCL .EQ. 0.0D0 .OR. HT .EQ. 0.0D0) THEN
              !Barrier does not meet criteria to influence source
              !too far away from source; not enough overlap with source
              ELSE
               IF (RLSOURCE(IS)%DCLWALL .EQ. 0.0D0) THEN
               !DCLWALL and HTWALL empty, assign values
                  RLSOURCE(IS)%DCLWALL = DCL
                  RLSOURCE(IS)%HTWALL  = HT
                  RLSOURCE(IS)%BARID   = SLDBARR(IB)%BARID
               ELSE IF ((RLSOURCE(IS)%DCLWALL .LT. 0.0D0 .AND.
     &                    DCL .LT. 0.0D0) .OR. 
     &                   (RLSOURCE(IS)%DCLWALL .GT. 0.0D0 .AND.
     &                    DCL .GT. 0.0D0)) THEN
C              Check sign of DCLWALL and DCL. If the same, keep closer of the two DCL.
                 IF (DABS(DCL) .LT. DABS(RLSOURCE(IS)%DCLWALL)) THEN
                   RLSOURCE(IS)%DCLWALL = DCL
                   RLSOURCE(IS)%HTWALL  = HT
                   RLSOURCE(IS)%BARID   = SLDBARR(IB)%BARID 
                 END IF
               ELSE 
C              If opposite sign, assign to DCLWALL2 if empty or keep closer DCL.               
                 IF (RLSOURCE(IS)%DCLWALL2 .EQ. 0.0D0 .OR. 
     &              (DABS(DCL) .LT. DABS(RLSOURCE(IS)%DCLWALL2))) THEN
                   RLSOURCE(IS)%DCLWALL2 = DCL
                   RLSOURCE(IS)%HTWALL2  = HT
                   RLSOURCE(IS)%BARID2   = SLDBARR(IB)%BARID
                 END IF
               END IF
             END IF

            DEALLOCATE(BSDCL, BSOVRLAP, BSRLHT, SOVRLP, STAT=ALLOCERROR)
            !reset overlap for next barid
            FSBOVRLP = 0.0D0
          END DO !IB, NBARID
          
        ELSE !not RLINE source; continue
        END IF !RLINEXT

      END DO !IS, NSRC
      
100    FORMAT(/85('*')/, 'SRCID', 9X, 'BARID', 9X, 'SEGNUM',
     &        2X, 'RELHT', 2X, 'LENGTH', 4X, 'GAP', 2X, 
     &        'OVERLAP', 4X, 'ANGLE', 7X, 'DCL',
     &        /37X, '(M)', 4X, '(M)', 6X, '(M)', 4X, '(%)',
     &        7X, 'DIF', 8X, '(M)')
101    FORMAT(A12, 2X, A12, 4X, I2, 4X, F4.1, 3X, F6.1, 2X,
     &         F5.1, 2X, F5.1, 5X, F6.2, 4X, F8.2)
200    FORMAT(/'SRCID', 9X, 'BARID', 10X, 'HT', 7X, 'DCL', 3X, 'OVERLAP', 
     &         4X, 'INFLUENCE',
     &        /29X, '(M)', 6X, '(M)', 5X, '(%)')
201    FORMAT(A12, 2X, A12, 2X, F4.1, 2X, F8.2, 3X, F5.1, 8X, L1/)
202    FORMAT('Barrier does not meet overlap threshold (90%).'/)
203    FORMAT('Barrier too far away (>10*Height).'/)
204    FORMAT(85('*')/)
205    FORMAT('Effective DCL and HT will be zero for calculations.'/)

      END SUBROUTINE SBARRIER_DCL
      
      DOUBLE PRECISION FUNCTION WEIGHT_SUM(SRCID,BARID,NSEG,WEIGHTS,VAR)
C***********************************************************************
C       WEIGHT_SUM Function
C       Purpose: Calculated weighted sum for SBARRIER segments DCL and height
C
C       Programmer: Rebecca L Miller WSP
C       Date: 20240529
C
C       Inputs:
C           SRCID    - source ID for error handling
C           BARID    - barrier ID for error handling
C           NSEG     - number of segments, array index
C           WEIGHTS  - weights, barrier segment overlap percentage
C           VAR      - value to be summed, segment DCL or height
C       Outputs:
C           WSUM     - calculated weighted sum
C
C       Called from SBARRIER_DCL
C***********************************************************************
      IMPLICIT NONE
      
      CHARACTER, INTENT(IN)         :: SRCID, BARID
      INTEGER,   INTENT(IN)         :: NSEG
      DOUBLE PRECISION, INTENT(IN)  :: WEIGHTS(NSEG), VAR(NSEG)  
      
      INTEGER          :: I     !loop index
      DOUBLE PRECISION :: WSUM, WNORM !weight sum for normalization
...
C     Loop NSEG
      DO I = 1,NSEG
        WSUM  = WSUM + (WEIGHTS(I)*VAR(I))
        WNORM = WNORM + WEIGHTS(I)
      END DO
      
C     Normalize by sum of weights
      WSUM = WSUM/WNORM
C     Check sum of weights equals one
C      IF (WNORM .LT. 0.5D0) THEN
C      !less than 50% overlap
C      !no influence from barrier
C        CALL ERRHDL('SO','SBAR_DCL','W','308',BARID)
C         !now called outside of function
C        WSUM = 0.0D0
C      ELSE IF (WNORM .NE. 1.0D0) THEN
C      !WARNING
C      END IF
...
C        WRITE Error Message:  WBOTTOM is negative or is greater than WTOP
 ...
 999  RETURN
      ...
      SUBROUTINE AIRCRAFT 
C***********************************************************************
C                AIRCRAFT  Module of AERMOD Model
C
C        PURPOSE: Processes Aircraft  Source Card
C
...
C        INPUTS:  Input Runstream Image Parameters
C
C        OUTPUTS: Array of flags for Aircraft Sources
C
C        CALLED FROM: SOCARD
C***********************************************************************
C
...
C     Check for 'ARCFTSRC ALL' option identified in PRESET
      IF (L_ARCFT_ALL) THEN
         IF (IFC .EQ. 3 .AND. FIELD(3) .EQ. 'ALL') THEN
            AFTSRC(:) = 'Y'
            GO TO 999
         ELSE
C           WRITE Error Message:  ARCFTSRC ALL

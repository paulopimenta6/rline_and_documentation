...
CCRT     2/20/2026 D138 Scientific Notation - FIX or EXP
 9071 FORMAT(/3X,'*** GROUP VALUE = ',E14.6,' ***'/)
...
CCRT     2/20/2026 D138 Scientific Notation - FIX or EXP
     ...
     &         AZELEV(IEVENT),AZFLAG(IEVENT)
         ELSE IF (FILE_FORMAT .EQ. 'EXP') THEN
            WRITE(IOUNIT,9059) EVNAME(IEVENT), GRPAVE(IDXEV(IEVENT)),
     &         EVAPER(IEVENT),EVDATE(IEVENT), AXR(IEVENT),AYR(IEVENT),
     &         AZELEV(IEVENT),AZFLAG(IEVENT)
         END IF
...
CCRT     2/20/2026 D138 Scientific Notation - FIX or EXP
 9059 FORMAT(29X,'*** SOURCE CONTRIBUTIONS FOR EVENT: ',A10,
     &       4X,'GROUP VALUE = ',E14.6,' ***'/1X,'---> AVE. PER.: ',
     &       I3,' HRS;','  END DATE:  ',I8.8,
     &       ';  LOCATION (XR,YR,ZELEV,ZFLAG):',4F11.2,' (M)')

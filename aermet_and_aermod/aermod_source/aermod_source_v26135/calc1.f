...
c ---    not execute if neither source contributes)
         PRMVAL = 0.0D0
c ---    Initialise source 1 concentration for GRSM
         IF(GRSM)THEN
           PRMVAL_Src1 = 0.0D0
         ENDIF
...
c              If this is source 1, store the concentration for GRSM
...
C        DATE:     November 10, 2000
C
C        MODIFIED: Modified for Aircraft's Plume Rise for
C                  VOLUME/AREA Sources only for Aircraft Source Group.
...
C**  Added for Aircraft Plume Rise; UNC-IE
     ...
     &                 AFTSRC(ISRC) .EQ. 'Y') THEN
            CALL PLUME_TRAJ ( xtmp )                           ! Change from ACBLPRD to PLUME_TRAJ on June 2025 by UNC-IE
            HTEFF = MIN (HSP+DHP1, ZI)
...
C**  End Aircraft Plume Rise insert; April 2023
         ELSE
            HTEFF = HSP
C ---       Assign DHCRIT = 0.0  and XFINAL = 0.0 for non-POINT and non-aircraft sources
            DHCRIT = 0.0D0
            XFINAL = 0.0D0
         END IF
...
C
C        MODIFIED:
C                  Modified to add Aircraft's plume rise for
C                  VOLUME Source only for Aircraft Source Group.
C                  Gavendra Pandey; UNC-IE, Chapel Hill, NC, USA
C                  04/01/2023
C
C                  Modified to optimize Aircraft's plume rise for
C                  VOLUME Source only for Aircraft Source Group.
C                  Gavendra Pandey; UNC-IE, Chapel Hill, NC, USA
C                  01/08/2026
C
...
C**  Added for Aircraft Plume Rise; UNC-IE
      INTEGER :: NDXZMID
      DOUBLE PRECISION :: ZMID, FRAN3
      DOUBLE PRECISION :: VALABV, VBELOW, XAFT
...
C        Find Aircraft Index for This Source

          FM  = 0.0D0                                       ! Momentum Flux is not used
          HSP = HS

C         Calculate Buoyancy Flux                            ---   CALL AFLUXES
          CALL AFLUXES

C         Fixed the maximum downwind distance
          XAFT = 20000.0D0
        
C         Calculate initial plume rise estimate              ---   CALL ADELTAH
          CALL ADELTAH (XAFT)
...
C         Calculate the plume penetration factor              ---   CALL PENFCT
          ...
          DHCRIT = DHP
          XMIXED = ZI * UAVG / SWAVG
C         If unstable, the plume might be broken up by mixing before XMAX.
C         Special chek for unstable conditions.
          IF (UNSTAB .AND. HS.LT.ZI) THEN
C            Check for XMIXED smaller than 1.25*XFINAL
             IF (XMIXED .LT. 1.25D0*XFINAL) THEN
                XFINAL = 0.8D0 * XMIXED
          ...
          END IF
          IF(ARCFTDEBUG) THEN
            WRITE(ARCFTDBG,6002) X, XFINAL, DHCRIT, XMIXED
 6002       FORMAT(/,5X,'VCALC SUMMARY:',
     &          ' Downwind Distance = ',F10.1,' M;',
     &          ' Final Distance = ',F10.1,' M;',
     &          ' Critical Rise = ',F6.1,' M;',
     &          ' Downwind Distance Full Mixing = ',F8.1,' M')
          END IF
         ELSE
C**  End Aircraft Plume Rise insert; April 2023; Updated on January 2026
...
CRWB     stack height and "final" plume height.  R. Brode, PES, 1/22/98
C----    Define ZMID=midpoint between stack height and "final" plume height
         ZMID = MIN( 4000.0D0, (HS + 0.5D0*DHP) )  ! Change DHFAER to DHP on January 2026 by UNC-IE
...
C        DATE:     November 10, 2000
C
C        CHANGES:  Modified to add Aircraft's plume rise for
C                  VOLUME Source only for Aircraft Source Group.
C                  Gavendra Pandey; UNC-IE, Chapel Hill, NC, USA
C                  04/01/2023
C
...
C --- Initialize FRAN
      FRAN  = 0.0D0
C --- Initialize FRAN3                          ! Added for ARISE; UNC-IE
      FRAN3 = 0.0D0                             ! Added for ARISE; UNC-IE
...
C           Calculate FSUBY for random component   ---   CALL FYPAN
       ...
       END IF
      ELSE
C**  End Aircraft Plume Rise insert; February 2023
...
C        DATE:    March 2, 1992
C
C        MODIFIED:   Modified to add Aircraft's plume rise for
C                    AREA Source only for Aircraft Source Group.
C                    Gavendra Pandey; UNC-IE, Chapel Hill, NC, USA
C                    04/01/2023
C
C                    Modified to optimize Aircraft's plume rise for
...
C     Variable Initializations
      ...
      FRAN3 = 0.0D0                    !1/13/26 UNC-IE initalized  FRAN3
C     Initialize __VAL arrays (1:NUMTYP)
      HRVAL   = 0.0D0
      AERVAL  = 0.0D0
      AEROUT  = 0.0D0
...
C         Fixed the maximum downwind distance
          XAFT = 20000.0D0      ! Initial guess of 20 km
...
C**  Added for Aircraft Plume Rise; UNC-IE
            IF (AFTSRC(ISRC) .EQ. 'Y') THEN
               HE = HSP + DHCRIT
            ELSE
C**  End Aircraft Plume Rise insert; January 2026
               HE = HS           
            END IF         ! Added for ARISE; UNC-IE
...
C ---         Removed this check if calculating meander since the lateral plume width is extended 
C ---             and this check would artificially reduce the width of the plume Wood 6/10/22
...
C                    Set DQCOR(NPD) and WQCOR(NPD) arrays to 1.0
               ...
               DO ITYP = 1, NUMTYP
C ---       Area meander option added Wood 6/3/22
C ---       Calculate coherent plume component using downwind distance, X
                  L_APLUME = .TRUE.
                  AEROUT_PLM = 0.0D0

C                 Calculate Area Source Integral - Plume         ---   CALL AREAIN
                  CALL AREAIN
                  AEROUT_PLM = HRVAL(ITYP)

C                 Check if user selected to calculate meander
C                 Calculate Area Source Integral - meander/pancake plume         ---   CALL AREAIN
                  IF (L_AREAMNDR) THEN
                     AEROUT_PAN = 0.0D0
                     L_APLUME = .FALSE.
                     CALL AREAIN
...
C**  End Aircraft Plume Rise insert; January 2026
                     END IF
C                Concentration calculated based on FRAN wieghting between plume and meander/pancake plume components
                 HRVAL(ITYP) = FRAN*AEROUT_PAN + (1.0D0-FRAN)*AEROUT_PLM
                 ELSE 
C                    Concentration equivalent to the plume component only
...
C
C        MODIFIED:
C                    Added Aircraft's Engine Parameters for
C                    Volume and Area Sources only for Aircraft Source Group
C                    Gavendra Pandey, UNC-IE Chapel Hill, USA,
C                    04/01/2023
C
...
C        PROGRAMMER: Roger Brode, Jeff Wang
C
C        MODIFIED:Modified to add plume rise for
C                 VOLUME Source only for Aircraft Source Group.
C                 Gavendra Pandey; UNC-IE, Chapel Hill, NC, USA
C                 04/01/2023
C
...
C        Specify BID Terms                                 --- CALL BID
         CALL BID
         SY = DSQRT( SYB*SYB + SYAMB*SYAMB + VSIGY*VSIGY )  ! Updated on April 24, 2026 by UNC-IE
      ELSE
C**  End Aircraft Plume Rise insert; April 2023
C     Set BID terms to zero
        SYB  = 0.0D0
...
C   DATE:    September 30, 1993
C
C   MODIFIED: Modified to add aircraft speed for Aircraft Source Group
C             Gavendra Pandey; UNC-IE, Chapel Hill, NC, USA
C             04/01/2023
C
C   MODIFIED: Updated based on Aircraft Plume Rise modifications
C             Gavendra Pandey; UNC-IE, Chapel Hill, NC, USA
C             01/26/2026
C
...
C
C   CALLED FROM:   PCALC, VALC, ACALC
C
C   Assumptions:
C
...
C
C              Modified to add plume rise for
C              Volume/Area Source only for Aircraft Source Group
C              Gavendra Pandey; UNC-IE, Chapel Hill, NC, USA, 04/01/2023
c
c ARGUMENTS:
...
C           Calculate the plume rise               ---   CALL ADELTAH
         ...
         END IF
      END IF
C**  End Aircraft Plume Rise insert; April 2026
...
CWSP --- Begin: Moved to end of calc1.f - D174 WSP 7/25/2023
      SUBROUTINE SWCALC
C***********************************************************************
C        PURPOSE: Calculates concentration or deposition values
...
C
 ...
 898  CONTINUE
      RETURN
      END
CWSP --- End: Moved to end of calc1.f - D174 WSP 7/25/2023
